...
test( 'dnrm2: huge plus medium triggers abig+amed combine', function t() {
	...
	const big = 1.0e+200;
	const x = new Float64Array( [ big, 3.0, 4.0 ] );
	const r = dnrm2( 3, x, 1, 0 );
...
test( 'dnrm2: two huge elements', function t() {
	const big = 1.0e+200;
	const x = new Float64Array( [ big, big ] );
	const r = dnrm2( 2, x, 1, 0 );
...
test( 'dnrm2: two tiny elements', function t() {
	const tiny = 1.0e-200;
	const x = new Float64Array( [ tiny, tiny ] );
	const r = dnrm2( 2, x, 1, 0 );
...
test( 'dnrm2: tiny plus medium triggers asml+amed combine', function t() {
	...
	const tiny = 1.0e-200;
	const x = new Float64Array( [ tiny, 3.0, 4.0 ] );
	const r = dnrm2( 3, x, 1, 0 );
	approxEqual( r, 5.0, 1e-12, 'r' );
});
...
test( 'dnrm2: tiny plus medium where tiny dominates ymax', function t() {
	...
	const tiny = 1.0e-160; // just below TSML (1.49e-154), large for an asml.
	const x = new Float64Array( [ tiny, 1e-100, 1e-100 ] );
	const r = dnrm2( 3, x, 1, 0 );
...
test( 'dnrm2: mixed tiny and big where only abig matters', function t() {
	...
	const tiny = 1.0e-200;
	const big = 1.0e+200;
	const x = new Float64Array( [ big, tiny ] );
	const r = dnrm2( 2, x, 1, 0 );
	approxEqual( r, big, 1e-12, 'r' );
});

test( 'dnrm2: NaN propagates', function t() {
	const x = new Float64Array( [ 1.0, NaN, 3.0 ] );
	const r = dnrm2( 3, x, 1, 0 );
...
test( 'dnrm2: stride with offset and large N', function t() {
	...
	const x = new Float64Array( [ 99.0, 3.0, 99.0, 99.0, 4.0, 99.0, 99.0, 0.0, 99.0, 99.0, 0.0 ] );
	const r = dnrm2( 4, x, 3, 1 );
	approxEqual( r, 5.0, 1e-12, 'r' );
});
