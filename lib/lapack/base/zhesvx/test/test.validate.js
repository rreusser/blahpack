/**
* Property-based validation for zhesvx, following the /blahpack-validate process.
*
* Step 0 classification: `z` -> complex scalar; `sy` -> COMPLEX HERMITIAN dense
* (schemes.dense, logical.hermitian — `A(j,i) = conj(A(i,j))`, real diagonal; the
* Hermitian, NOT the complex-symmetric, family, matching zhetrf which factors
* A = A^H);
* `svx` (EXPERT complex Hermitian-indefinite DRIVER: Bunch-Kaufman factor (zhetrf)
* + condition estimate (zhecon) + solve (zhetrs) + iterative refinement (zherfs) +
* error bounds, in ONE call) -> a COMPOSITE of the three properties already proven
* for its constituent routines (zhesv residual, zhecon rcond, zherfs FERR/BERR).
* We drive zhesvx with fact='not-factored' (factor A into AF, no equilibration
* path exists, so X solves the ORIGINAL A0*X = B0 directly) and assert, against the
* ORIGINAL full complex Hermitian A0:
...
*       reciprocal condition number (anorm = ‖A0‖₁ = ‖A0‖∞ since A0 Hermitian — the
*       norm zhesvx forms for zhecon — and ‖A0⁻¹‖₁ from a trusted zhesv A0*X=I), in
*       (0,1].
*   (c) BERR tiny and >= 0; the ACTUAL forward error <= FERR*C; FERR in [0,1),
*       Xtrue from the trusted zhesv.
* Only the uplo triangle of A/AF is realized; the opposite stays poisoned. zhesvx
...
const NRHS = [ 1, 2 ];
const TIGHT = schemes.dense.pivotLayouts()[ 0 ]; // tight col-major (valid pivot layout)
...
// 1-norm (max abs column sum) of the FULL complex Hermitian logical matrix.
// Because A0 is Hermitian this also equals its inf-norm — the norm zhesvx forms
// for zhecon.
...
// Independent ‖A0⁻¹‖₁: solve A0·X = I with the trusted zhesv on a fresh copy;
...
function invNorm1( A0, n, uplo ) {
	...
	const work = sc.alloc( Math.max( n, 1 ) );
	const info = zhesv( uplo, n, n, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], ipiv, 1, 0, Br.data, Br.args[ 0 ], Br.args[ 1 ], Br.args[ 2 ], work, 1, 0 );
	if ( info !== 0 ) {
		throw new Error( 'oracle zhesv failed (info='+info+'); A0 singular?' );
	}
	let mx = 0.0;
	let s, i, j;
	for ( j = 0; j < n; j++ ) {
...
function trueSolution( A0, B0, N, nrhs, uplo ) {
	...
	const ipiv = new Int32Array( Math.max( N, 1 ) );
	const work = sc.alloc( Math.max( N, 1 ) );
	const info = zhesv( uplo, N, nrhs, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], ipiv, 1, 0, Br.data, Br.args[ 0 ], Br.args[ 1 ], Br.args[ 2 ], work, 1, 0 );
	if ( info !== 0 ) {
		throw new Error( 'oracle zhesv failed (info='+info+'); A0 singular?' );
	}
	return Br;
...
// Drive zhesvx (fact='not-factored') on realized copies; return the physical X
...
function drive( uplo, N, nrhs, A0, B0, layout ) {
	...
	const AFr = schemes.dense.realize( sc, A0, { 'part': uplo }, layout ); // overwritten by zlacpy+zhetrf
	const Br = schemes.dense.realize( sc, B0, { 'part': 'full' }, layout );
	const Xr = schemes.dense.realize( sc, B0, { 'part': 'full' }, layout ); // overwritten by zlacpy+zhetrs
	const ipiv = new Int32Array( Math.max( N, 1 ) );
	const rcond = new Float64Array( 1 );
	const FERR = new Float64Array( Math.max( nrhs, 1 ) );
	const BERR = new Float64Array( Math.max( nrhs, 1 ) );
	const WORK = sc.alloc( Math.max( 2 * N, 1 ) ); // NaN-poisoned complex scratch
	const RWORK = poisonReal( N ); // NaN-poisoned real scratch
	const info = zhesvx( 'not-factored', uplo, N, nrhs, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], AFr.data, AFr.args[ 0 ], AFr.args[ 1 ], AFr.args[ 2 ], ipiv, 1, 0, Br.data, Br.args[ 0 ], Br.args[ 1 ], Br.args[ 2 ], Xr.data, Xr.args[ 0 ], Xr.args[ 1 ], Xr.args[ 2 ], rcond, FERR, 1, 0, BERR, 1, 0, WORK, 1, 0, RWORK, 1, 0 );
	return {
		'Xr': Xr,
		'rcond': rcond,
		'FERR': FERR,
...
// nrhs x every pivot-valid storage layout (col AND row order). zhetrf does an
...
test( 'zhesvx: expert-driver residual (uplo x N x nrhs x pivot layout)', function t() {
...
function runResidual( uplo, N, nrhs, layout ) {
	...
	const A0 = logical.hermitian( sc, rng, N ); // full complex Hermitian indefinite oracle
	const B0 = logical.general( sc, rng, N, nrhs );
	const d = drive( uplo, N, nrhs, A0, B0, layout );
	const tag = 'zhesvx '+uplo+' N='+N+' nrhs='+nrhs;

	checked( 'zhesvx', 'residual', function run() {
		if ( d.info !== 0 ) {
			throw new Error( tag+': zhesvx returned info='+d.info+' (expected 0 for well-conditioned indefinite input)' );
		}
		let j;
		for ( j = 0; j < nrhs; j++ ) {
...
// uplo x N x nrhs at the tight col-major layout. The oracle (zhesv) is exercised
// per case for the true rcond and the true solution.
test( 'zhesvx: rcond estimate + FERR/BERR bounds (uplo x N x nrhs)', function t() {
	UPLOS.forEach( function eachUplo( uplo ) {
		SIZES_SMALL.forEach( function eachN( N ) {
			NRHS.forEach( function eachNrhs( nrhs ) {
...
function runBounds( uplo, N, nrhs ) {
	...
	const d = drive( uplo, N, nrhs, A0, B0, TIGHT );
	const tag = 'zhesvx '+uplo+' N='+N+' nrhs='+nrhs;

	if ( d.info !== 0 ) {
		throw new Error( tag+': zhesvx returned info='+d.info+' (expected 0)' );
	}

	// (b) rcond estimates 1/κ₁(A0) within factor F, in (0,1].
	checked( 'zhesvx', 'property', function run() {
	...
	checked( 'zhesvx', 'structural', function run() {
...
// Step 4: layout-invariance fuzz. zhesvx is a ONE-SHOT indefinite driver: zhetrf's
// pivot search is data-dependent and factor+condition+solve+refinement are
// coupled (see the zhetrf/zhesv LEARNINGS notes). Bit-exact invariance therefore
...
// [rcond] ++ FERR ++ BERR.
test( 'zhesvx: bit-exact across pure-addressing layouts (one-shot driver)', function t() {
	UPLOS.forEach( function eachUplo( uplo ) {
		runInvariance( uplo );
	});
});
...
function runInvariance( uplo ) {
	...
	checked( 'zhesvx', 'layout-invariance', function run() {
		layoutInvariant( variants, function build( layout ) {
			...
			return out;
		}, { 'label': 'zhesvx '+uplo+' pure-addressing layout invariance' } );
	});
}
