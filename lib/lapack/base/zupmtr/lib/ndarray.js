...
function zupmtr( side, uplo, trans, M, N, AP, strideAP, offsetAP, TAU, strideTAU, offsetTAU, C, strideC1, strideC2, offsetC, WORK, strideWork, offsetWork ) { // eslint-disable-line max-len, max-params
	...
	if ( M > 0 && N > 0 ) {
		// Caller owns the workspace; assert it is sufficiently large so an under-sized (or non-array) buffer is a loud RangeError, not a silent NaN. ZUPMTR needs N (side='left') or M (side='right') complex elements.
		need = ( side === 'left' ) ? N : M;
