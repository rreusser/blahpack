...
import zrotgWrap from './../lib/zrotg.js';
...
function call( fRe, fIm, gRe, gIm ) {
	const a = new Complex128Array([ fRe, fIm ]);
	const b = new Complex128Array([ gRe, gIm ]);
...
function close( actual, expected, tol, msg ) {
	...
	const s = Math.max( Math.abs( expected ), 1.0 );
...
test( 'zrotg (wrapper): (3+0i, 4+0i) yields c=0.6, s=0.8+0i', function t() {
	...
	const b = new Complex128Array( [ 4.0, 0.0 ] );
	const c = new Float64Array( 1 );
	const s = new Complex128Array( 1 );
	zrotgWrap( a, b, c, s );
	const sFlat = reinterpret( s, 0 );
...
test( 'zrotg: f==0, g general scaled (very small) (Case 2d)', function t() {
	...
	const tiny = RTMIN / 2.0;
	const r = call( 0, 0, tiny * 3.0, tiny * 4.0 );
...
test( 'zrotg: f==0, g general scaled (very large) (Case 2d big)', function t() {
	...
	const big = 1e200;
	const r = call( 0, 0, big * 3.0, big * 4.0 );
	close( r.c, 0.0, 1e-15, 'c' );
...
test( 'zrotg: nonzero f and g complex (Case 3)', function t() {
	const expR = Math.sqrt( 194.0 );
	const r = call( 3, 4, 5, 12 );
	const rmag = Math.sqrt( ( r.rRe * r.rRe ) + ( r.rIm * r.rIm ) );
...
test( 'zrotg: small f and g, scaled algorithm', function t() {
	...
	const tiny = RTMIN / 4.0;
	const r = call( tiny * 3, 0, tiny * 4, 0 );
...
test( 'zrotg: complex unscaled, large h2 ratio (else branch in unscaled S)', function t() {
	...
	const r = call( 1, 0, 1e10, 1e10 );
	close( inv( r ), 1.0, 1e-12, 'identity' );
});

test( 'zrotg: with offsets in arrays', function t() {
	const a = new Complex128Array([ 99, 99, 3, 4 ]);
	const b = new Complex128Array([ 99, 99, 99, 99, 5, 12 ]);
	const c = new Float64Array( 5 );
	const s = new Complex128Array( 3 );
...
test( 'zrotg: extreme f underflow forces cc fallback in scaled branch', function t() {
	...
	const f = SAFMIN * 1e-10;
	const r = call( f, 0, 1.0, 0 );
