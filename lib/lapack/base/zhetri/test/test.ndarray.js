...
const rawLines = readFileSync( path.join( fixtureDir, 'zhetri.jsonl' ), 'utf8' ).trim().split( '\n' ); // eslint-disable-line node/no-sync, max-len
...
* The fixture stores entries column-major using LDA=NMAX=10 (so 2_NMAX=20 reals per column).
_ For our test, we use LDA=N (tight), so we must extract entries from the fixture using the column-major
_ convention with stride 2_NMAX between columns.
*/
function checkA( buf, expected, N, lda, ldaFixture, msg, tol ) {
	let col, row, idxA, idxF;
	for ( col = 0; col < N; col++ ) {
		for ( row = 0; row < N; row++ ) {
			idxA = ( col * lda + row ) * 2;
...
function buildAFromFixture( N, fixtureA, ldaFixture ) {
	...
	const buf = ( arr.buffer ) ? new Float64Array( arr.buffer ) : new Float64Array( N * N * 2 );
	let col, row, idxA, idxF;
	for ( col = 0; col < N; col++ ) {
		for ( row = 0; row < N; row++ ) {
			idxA = ( col * N + row ) * 2;
...
function runCase( factoredName, expectedName, uplo, N ) {
	let expected, IPIV, WORK, info, A, buf;
	...
	const factored = findCase( factoredName );
	expected = findCase( expectedName );
