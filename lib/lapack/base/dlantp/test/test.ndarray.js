...
import dlantp from './../lib/ndarray.js';


// FIXTURES //

const fixtureDir = path.join( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures' );
const rawLines = readFileSync( path.join( fixtureDir, 'dlantp.jsonl' ), 'utf8' ).trim().split( '\n' ); // eslint-disable-line node/no-sync, max-len
...
const AP_3X3_UPPER = new Float64Array([ 2.0, 3.0, 5.0, -1.0, 2.0, 7.0 ]);

// 3x3 lower, packed: 2,3,-1,5,2,7
const AP_3X3_LOWER = new Float64Array([ 2.0, 3.0, -1.0, 5.0, 2.0, 7.0 ]);

// 4x4 upper, packed: 2,3,5,-1,2,7,4,-6,1,8
const AP_4X4_UPPER = new Float64Array([ 2.0, 3.0, 5.0, -1.0, 2.0, 7.0, 4.0, -6.0, 1.0, 8.0 ]);

// 4x4 lower, packed: 2,3,-1,4,5,2,-6,7,1,8
const AP_4X4_LOWER = new Float64Array([ 2.0, 3.0, -1.0, 4.0, 5.0, 2.0, -6.0, 7.0, 1.0, 8.0 ]);

// 1x1, single = -5.5
const AP_1X1 = new Float64Array([ -5.5 ]);
...
function runCase( name, norm, uplo, diag, N, AP ) {
	...
	const got = dlantp( norm, uplo, diag, N, AP, 1, 0, WORK, 1, 0 );
