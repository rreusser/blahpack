...
/**
* Estimates the reciprocal of the condition number of a complex Hermitian positive definite matrix A.
*
* @param {string} order - storage layout ('row-major' or 'column-major')
* @param {string} uplo - `'upper'` or `'lower'`
* @param {NonNegativeInteger} N - order of the matrix A
* @param {Complex128Array} A - input matrix (Cholesky factor from `zpotrf`)
...
*/
function zpocon( order, uplo, N, A, LDA, anorm, rcond, WORK, strideWork, RWORK, strideRWork ) {
	let sa1, sa2;
	...
	const ow = stride2offset( 2*N, strideWork );
	const or = stride2offset( N, strideRWork );
	return base( uplo, N, A, sa1, sa2, 0, anorm, rcond, WORK, strideWork, ow, RWORK, strideRWork, or );
}
