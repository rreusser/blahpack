/**
* Property-based validation for zsyrfs, following the /blahpack-validate process.
*
* Step 0 classification: `z` -> complex scalar; `sy` -> COMPLEX-SYMMETRIC (no
* conjugation) INDEFINITE dense (schemes.dense, logical.symmetric); `rfs`
* (iterative refinement + error bounds) -> three independent properties. zsyrfs
* refines an approximate solution X to A*X = B and returns FERR (forward-error
* bound) and BERR (componentwise backward error). The Bunch-Kaufman factor AF +
* IPIV are produced by the already-validated zsytrf; the un-refined initial X by
* zsytrs; the TRUE solution independently by the trusted zsysv on fresh copies.
* We assert, against the ORIGINAL full complex-symmetric A0:
...
import zsytrs from '../../zsytrs/lib/ndarray.js';
...
function trueSolution( A0, B0, N, nrhs, uplo ) {
	...
	const info = zsysv( uplo, N, nrhs, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], IPIV, 1, 0, Br.data, Br.args[ 0 ], Br.args[ 1 ], Br.args[ 2 ], WORK, 1, 0 );
	if ( info !== 0 ) {
		throw new Error( 'oracle zsysv failed (info='+info+'); matrix singular?' );
	}
	return Br;
}

// Steps 2-3-5: three properties across uplo flags, a size sweep, and nrhs.
test( 'zsyrfs: refinement residual + BERR + FERR bound (uplo x N x nrhs)', function t() {
...
function runProperty( uplo, N, nrhs ) {
	...
	const A0 = logical.symmetric( sc, rng, N ); // full complex-symmetric oracle
	const B0 = logical.general( sc, rng, N, nrhs );

	// A = original symmetric (uplo triangle); AF/IPIV = its Bunch-Kaufman factor:
	const Ar = schemes.dense.realize( sc, A0, { 'part': uplo }, TIGHT );
	const AFr = schemes.dense.realize( sc, A0, { 'part': uplo }, TIGHT );
	const IPIV = new Int32Array( N );
	zsytrf( uplo, N, AFr.data, AFr.args[ 0 ], AFr.args[ 1 ], AFr.args[ 2 ], IPIV, 1, 0 );

	// B (RHS, unchanged) and X (initial un-refined solve, refined in place):
	const Br = schemes.dense.realize( sc, B0, { 'part': 'full' }, TIGHT );
	const Xr = schemes.dense.realize( sc, B0, { 'part': 'full' }, TIGHT );
	zsytrs( uplo, N, nrhs, AFr.data, AFr.args[ 0 ], AFr.args[ 1 ], AFr.args[ 2 ], IPIV, 1, 0, Xr.data, Xr.args[ 0 ], Xr.args[ 1 ], Xr.args[ 2 ] );
	...
	zsyrfs( uplo, N, nrhs, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], AFr.data, AFr.args[ 0 ], AFr.args[ 1 ], AFr.args[ 2 ], IPIV, 1, 0, Br.data, Br.args[ 0 ], Br.args[ 1 ], Br.args[ 2 ], Xr.data, Xr.args[ 0 ], Xr.args[ 1 ], Xr.args[ 2 ], FERR, 1, 0, BERR, 1, 0, WORK, 1, 0, RWORK, 1, 0 );

	const Xtrue = trueSolution( A0, B0, N, nrhs, uplo );

	const tag = 'zsyrfs '+uplo+' N='+N+' nrhs='+nrhs;

	// (a) residual: refined X remains a valid solution of A0*X = B0.
	checked( 'zsyrfs', 'residual', function run() {
		let j;
		for ( j = 0; j < nrhs; j++ ) {
	...
	checked( 'zsyrfs', 'structural', function run() {
...
// per layout together with the fixed A0 and B0; zsyrfs's internal kernels (zsymv,
// zsytrs, zaxpy, zlacn2) run over identical values, so only ADDRESSING changes
// and the refined X + FERR + BERR must reproduce bit-for-bit. The fixed IPIV is
// consumed by zsytrs (no pivot search of its own), so the full col/row families
...
test( 'zsyrfs: bit-exact within storage-order family (col / row)', function t() {
	UPLOS.forEach( function eachUplo( uplo ) {
...
function runInvariance( uplo, variants, fam ) {
	...
	zsytrf( uplo, N, AF0.data, AF0.args[ 0 ], AF0.args[ 1 ], AF0.args[ 2 ], IPIV0, 1, 0 );
	...
	zsytrs( uplo, N, nrhs, AF0.data, AF0.args[ 0 ], AF0.args[ 1 ], AF0.args[ 2 ], IPIV0, 1, 0, X0r.data, X0r.args[ 0 ], X0r.args[ 1 ], X0r.args[ 2 ] );
	const Xinit = readMat( X0r, N, nrhs );

	checked( 'zsyrfs', 'layout-invariance', function run() {
		layoutInvariant( variants, function build( layout ) {
			...
			zsyrfs( uplo, N, nrhs, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], AFr.data, AFr.args[ 0 ], AFr.args[ 1 ], AFr.args[ 2 ], IPIV0, 1, 0, Br.data, Br.args[ 0 ], Br.args[ 1 ], Br.args[ 2 ], Xr.data, Xr.args[ 0 ], Xr.args[ 1 ], Xr.args[ 2 ], FERR, 1, 0, BERR, 1, 0, WORK, 1, 0, RWORK, 1, 0 );
			...
			return out;
		}, { 'label': 'zsyrfs '+uplo+' layout invariance '+fam+'-major' } );
	});
}
