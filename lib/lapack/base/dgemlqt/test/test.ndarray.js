...
import dgemlqt from './../lib/ndarray.js';


// FIXTURES //

const fixtureDir = path.join( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures' );
const rawLines = readFileSync( path.join( fixtureDir, 'dgemlqt.jsonl' ), 'utf8' ).trim().split( '\n' );
...
test( 'dgemlqt: left, no-transpose -> Q*I (small, MB=2, K=3)', function t() {
	...
	const info = dgemlqt( 'left', 'no-transpose', 4, 4, 3, 2, vt.V, 1, 3, 0, vt.T, 1, 2, 0, C, 1, 4, 0, WORK, 1, 0 );
...
test( 'dgemlqt: left, transpose -> Q^T*I (small)', function t() {
	...
	const C = eye( 4, 4, 4 );
	const WORK = new Float64Array( 4 * 2 );
	const info = dgemlqt( 'left', 'transpose', 4, 4, 3, 2, vt.V, 1, 3, 0, vt.T, 1, 2, 0, C, 1, 4, 0, WORK, 1, 0 );
	assert.equal( info, tc.info );
	assertArrayClose( toArray( C ), tc.c, 1e-14, 'c' );
});
...
test( 'dgemlqt: right, no-transpose, rectangular C(4,3) (small)', function t() {
	...
	const info = dgemlqt( 'right', 'no-transpose', 4, 3, 3, 2, vt.V, 1, 3, 0, vt.T, 1, 2, 0, C, 1, 4, 0, WORK, 1, 0 );
	assert.equal( info, tc.info );
	assertArrayClose( toArray( C ), tc.c, 1e-14, 'c' );
});
...
test( 'dgemlqt: right, transpose, rectangular C(4,3) (small)', function t() {
	...
	const C = rect43();
	const WORK = new Float64Array( 4 * 2 );
	const info = dgemlqt( 'right', 'transpose', 4, 3, 3, 2, vt.V, 1, 3, 0, vt.T, 1, 2, 0, C, 1, 4, 0, WORK, 1, 0 );
	assert.equal( info, tc.info );
	assertArrayClose( toArray( C ), tc.c, 1e-14, 'c' );
});
...
test( 'dgemlqt: left, transpose on dense 4x4 (small)', function t() {
	...
	const tc = findCase( 'left_trans_dense' );
	const vt = smallVT();
	const C = dense44();
	const WORK = new Float64Array( 4 * 2 );
	const info = dgemlqt( 'left', 'transpose', 4, 4, 3, 2, vt.V, 1, 3, 0, vt.T, 1, 2, 0, C, 1, 4, 0, WORK, 1, 0 );
...
test( 'dgemlqt: M=0 quick return', function t() {
	...
	const info = dgemlqt( 'left', 'no-transpose', 0, 4, 0, 2, vt.V, 1, 3, 0, vt.T, 1, 2, 0, C, 1, 1, 0, WORK, 1, 0 );
	assert.equal( info, tc.info );
});
...
test( 'dgemlqt: N=0 quick return', function t() {
	...
	const C = new Float64Array( 1 );
	const WORK = new Float64Array( 1 );
	const info = dgemlqt( 'left', 'no-transpose', 4, 0, 0, 2, vt.V, 1, 3, 0, vt.T, 1, 2, 0, C, 1, 4, 0, WORK, 1, 0 );
	assert.equal( info, tc.info );
});
...
test( 'dgemlqt: K=0 quick return (C unchanged)', function t() {
	...
	const WORK = new Float64Array( 4 * 2 );
	const info = dgemlqt( 'left', 'no-transpose', 4, 4, 0, 2, vt.V, 1, 3, 0, vt.T, 1, 2, 0, C, 1, 4, 0, WORK, 1, 0 );
	assert.equal( info, tc.info );
...
test( 'dgemlqt: left, no-transpose (multi-block, M=10, K=8, MB=3)', function t() {
	...
	const vt = blockVT();
	const C = new Float64Array( 100 );
	fillCos( C, 10, 10, 10 );
	const WORK = new Float64Array( 10 * 3 );

	// V is K(=8)-by-Q(=10) column-major: strideV1=1, strideV2=8.
	const info = dgemlqt( 'left', 'no-transpose', 10, 10, 8, 3, vt.V, 1, 8, 0, vt.T, 1, 3, 0, C, 1, 10, 0, WORK, 1, 0 );
	assert.equal( info, tc.info );
...
test( 'dgemlqt: left, transpose (multi-block)', function t() {
	...
	const tc = findCase( 'left_trans_block' );
	const vt = blockVT();
	const C = new Float64Array( 100 );
	fillCos( C, 10, 10, 10 );
	const WORK = new Float64Array( 10 * 3 );
	const info = dgemlqt( 'left', 'transpose', 10, 10, 8, 3, vt.V, 1, 8, 0, vt.T, 1, 3, 0, C, 1, 10, 0, WORK, 1, 0 );
	assert.equal( info, tc.info );
	assertArrayClose( toArray( C ), tc.c, 1e-13, 'c' );
});
...
test( 'dgemlqt: right, no-transpose (multi-block)', function t() {
	...
	fillCos( C, 10, 10, 10 );
	const WORK = new Float64Array( 10 * 3 );
	const info = dgemlqt( 'right', 'no-transpose', 10, 10, 8, 3, vt.V, 1, 8, 0, vt.T, 1, 3, 0, C, 1, 10, 0, WORK, 1, 0 );
	assert.equal( info, tc.info );
	assertArrayClose( toArray( C ), tc.c, 1e-13, 'c' );
});
...
test( 'dgemlqt: right, transpose (multi-block)', function t() {
	...
	const tc = findCase( 'right_trans_block' );
	const vt = blockVT();
	const C = new Float64Array( 100 );
	fillCos( C, 10, 10, 10 );
	const WORK = new Float64Array( 10 * 3 );
	const info = dgemlqt( 'right', 'transpose', 10, 10, 8, 3, vt.V, 1, 8, 0, vt.T, 1, 3, 0, C, 1, 10, 0, WORK, 1, 0 );
	assert.equal( info, tc.info );
	assertArrayClose( toArray( C ), tc.c, 1e-13, 'c' );
});
...
test( 'dgemlqt: left, no-transpose (single-block, MB == K)', function t() {
	...
	fillSin( C, 5, 5, 5 );
	const WORK = new Float64Array( 5 * 4 );

	// V is K(=4)-by-Q(=5) column-major: strideV1=1, strideV2=4.
	const info = dgemlqt( 'left', 'no-transpose', 5, 5, 4, 4, vt.V, 1, 4, 0, vt.T, 1, 4, 0, C, 1, 5, 0, WORK, 1, 0 );
	assert.equal( info, tc.info );
	assertArrayClose( toArray( C ), tc.c, 1e-13, 'c' );
});
...
test( 'dgemlqt: left, transpose (single-block)', function t() {
	...
	const tc = findCase( 'left_trans_single' );
	const vt = singleVT();
	const C = new Float64Array( 25 );
	fillSin( C, 5, 5, 5 );
	const WORK = new Float64Array( 5 * 4 );
	const info = dgemlqt( 'left', 'transpose', 5, 5, 4, 4, vt.V, 1, 4, 0, vt.T, 1, 4, 0, C, 1, 5, 0, WORK, 1, 0 );
	assert.equal( info, tc.info );
	assertArrayClose( toArray( C ), tc.c, 1e-13, 'c' );
});
...
test( 'dgemlqt: round trip Q^T * Q * C === C (left, multi-block)', function t() {
	...
	const vt = blockVT();
	const C = new Float64Array( 100 );
	const Corig = new Float64Array( 100 );
	fillCos( C, 10, 10, 10 );
	Corig.set( C );
	const WORK = new Float64Array( 10 * 3 );
