...
/**
* Computes the one norm, Frobenius norm, infinity norm, or largest absolute value of a real trapezoidal or triangular matrix.
*
...
* @param {(Float64Array|null)} WORK - workspace (>= `M` elements for the infinity norm); auto-allocated when `null`
...
*/
function dlantr( order, norm, uplo, diag, M, N, A, LDA, WORK, strideWork ) {
	let sa1, sa2;
	...
	if ( WORK === null || WORK === void 0 ) {
		WORK = new Float64Array( max( 1, M ) );
		strideWork = 1;
	}
	const ow = stride2offset( M, strideWork );
	return base( norm, uplo, diag, M, N, A, sa1, sa2, 0, WORK, strideWork, ow );
}
