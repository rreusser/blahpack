...
import dlantb from './../lib/ndarray.js';


// FIXTURES //

const fixtureDir = path.join( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures' );
const rawLines = readFileSync( path.join( fixtureDir, 'dlantb.jsonl' ), 'utf8' ).trim().split( '\n' ); // eslint-disable-line node/no-sync, max-len
...
// 1x1 K=0, LDAB=4
const AB_1X1 = buildAB( 4, 1, [ 1, 1, 5.0 ] );

// K=0 diagonal, N=4, LDAB=4
const AB_K0_DIAG = buildAB( 4, 4, [
	1, 1, 3.0,
	1, 2, -7.0,
	1, 3, 2.0,
	1, 4, -4.0
...
function runCase( name, norm, uplo, diag, N, K, AB ) {
	...
	const got = dlantb( norm, uplo, diag, N, K, AB, 1, LDAB, 0, WORK, 1, 0 );
