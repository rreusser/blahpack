...
function runCase( name ) {
	...
	const transr = mapFlag( 'transr', tc.transr );
	const side = mapFlag( 'side', tc.side );
	const uplo = mapFlag( 'uplo', tc.uplo );
	const trans = mapFlag( 'trans', tc.trans );
	const diag = mapFlag( 'diag', tc.diag );
	const M = tc.M;
	const N = tc.N;
	const alpha = tc.alpha;

	const A = new Float64Array( tc.A );
	const B = new Float64Array( tc.B_in.length );
