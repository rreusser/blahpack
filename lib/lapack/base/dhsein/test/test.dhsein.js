...
import dhsein from './../lib/dhsein.js';
...
test( 'dhsein: column-major path runs (1x1)', function t() {
	...
	const info = dhsein( 'column-major', 'right', 'no-source', 'no-init', SELECT, 1, 1, H, 1, WR, 1, WI, 1, VL, 1, VR, 1, 1, 1, WORK, 1, IFAILL, 1, 0, IFAILR, 1, 0 );
...
test( 'dhsein: row-major path runs (1x1)', function t() {
	...
	const IFAILL = new Int32Array( 1 );
	const IFAILR = new Int32Array( 1 );
	const info = dhsein( 'row-major', 'right', 'no-source', 'no-init', SELECT, 1, 1, H, 1, WR, 1, WI, 1, VL, 1, VR, 1, 1, 1, WORK, 1, IFAILL, 1, 0, IFAILR, 1, 0 );
	assert.equal( info.info, 0, 'info=0' );
	assert.equal( info.m, 1, 'm=1' );
} );
