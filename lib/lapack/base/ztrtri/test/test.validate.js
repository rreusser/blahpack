/**
* Property-based validation for ztrtri, following the /blahpack-validate process.
*
...
* delegates to ztrmm/ztrsm/ztrti2) -> RECONSTRUCTION: A0 * inv(A0) = I. The FULL
...
const SIZES = SIZES_SMALL.concat( [ 48, 65, 100 ] ); // add larger N to exercise blocking
...
test( 'ztrtri: inverse reconstruction A*inv(A)=I (size sweep x uplo x diag x all layouts)', function t() {
		...
		DIAGS.forEach( function eachDiag( diag ) {
			...
			SIZES.forEach( function eachN( n ) {
				schemes.dense.layouts().forEach( function eachLayout( layout ) {
					...
					const R = schemes.dense.realize( sc, A0, { 'part': uplo, 'unit': unit }, layout );
					ztrtri( uplo, diag, n, R.data, R.args[ 0 ], R.args[ 1 ], R.args[ 2 ] );
					const invA = readInv( R, n, uplo, unit, sc.one );
					const P = ref.matmul( sc, A0, invA );
					checked( 'ztrtri', 'reconstruct', function run() {
						assertInvResidual( A0, invA, P, n, 'ztrtri '+uplo+' '+diag+' n='+n, 100 );
					});
				});
			});
		});
...
// the blocked ztrmm/ztrsm path is exercised). The blocked Level-3 kernels reorder
// their summation on a col<->row storage-order flip (~1 ULP, not a defect — see
// the dgels/zpotf2 LEARNINGS entries), so bit-equality is asserted only within
...
test( 'ztrtri: bit-exact within storage-order family (col / row)', function t() {
	UPLOS.forEach( function eachUplo( uplo ) {
		DIAGS.forEach( function eachDiag( diag ) {
...
function runInvariance( uplo, diag, variants, fam ) {
	...
	const unit = ( diag === 'unit' );
	const SEED = 0xF00D;
	checked( 'ztrtri', 'layout-invariance', function run() {
		...
		}, { 'label': 'ztrtri '+uplo+' '+diag+' layout invariance '+fam+'-major' } );
	});
}
