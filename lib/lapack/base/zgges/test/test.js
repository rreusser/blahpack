...
function runCase( tc, jobvsl, jobvsr ) {
	...
	const res = zgges( jobvsl, jobvsr, 'not-sorted', nosel, N, A, 1, N, 0, B, 1, N, 0, ALPHA, 1, 0, BETA, 1, 0, VSL, 1, N, 0, VSR, 1, N, 0, WORK, 1, 0, RWORK, 1, 0, BWORK, 1, 0 ); // eslint-disable-line max-len
...
test( 'zgges returns info=0 and sdim=0 for N=0', function t() {
	const res = zgges( 'no-vectors', 'no-vectors', 'not-sorted', nosel, 0, new Complex128Array( 0 ), 1, 0, 0, new Complex128Array( 0 ), 1, 0, 0, new Complex128Array( 0 ), 1, 0, new Complex128Array( 0 ), 1, 0, new Complex128Array( 0 ), 1, 0, 0, new Complex128Array( 0 ), 1, 0, 0, new Complex128Array( 1 ), 1, 0, new Float64Array( 1 ), 1, 0, new Uint8Array( 1 ), 1, 0 ); // eslint-disable-line max-len
	assert.equal( res.info, 0 );
	assert.equal( res.sdim, 0 );
});

test( 'zgges: 2x2_diag_no_vectors', function t() {
	const tc = findCase( '2x2_diag_no_vectors' );
	runCase( tc, 'no-vectors', 'no-vectors' );
});

test( 'zgges: 2x2_both_vectors', function t() {
	const tc = findCase( '2x2_both_vectors' );
	runCase( tc, 'compute-vectors', 'compute-vectors' );
});

test( 'zgges: 3x3_right_only', function t() {
	const tc = findCase( '3x3_right_only' );
	runCase( tc, 'no-vectors', 'compute-vectors' );
});

test( 'zgges: 3x3_left_only', function t() {
	const tc = findCase( '3x3_left_only' );
	runCase( tc, 'compute-vectors', 'no-vectors' );
});

test( 'zgges: 4x4_complex_both', function t() {
	const tc = findCase( '4x4_complex_both' );
	runCase( tc, 'compute-vectors', 'compute-vectors' );
});
...
test( 'zgges: small matrix triggers A-scaling (ilascl)', function t() {
	...
	const RWORK = new Float64Array( ( 2 * N ) + Math.max( 8 * N, 1 ) );
	const BWORK = new Uint8Array( Math.max( 1, N ) );
	const res = zgges( 'compute-vectors', 'compute-vectors', 'not-sorted', nosel, N, A, 1, N, 0, B, 1, N, 0, ALPHA, 1, 0, BETA, 1, 0, VSL, 1, N, 0, VSR, 1, N, 0, WORK, 1, 0, RWORK, 1, 0, BWORK, 1, 0 ); // eslint-disable-line max-len
...
test( 'zgges: sorted eigenvalues with selection function', function t() {
	...
	const RWORK = new Float64Array( ( 2 * N ) + Math.max( 8 * N, 1 ) );
	const BWORK = new Uint8Array( Math.max( 1, N ) );
	const res = zgges( 'compute-vectors', 'compute-vectors', 'sorted', selctg, N, A, 1, N, 0, B, 1, N, 0, ALPHA, 1, 0, BETA, 1, 0, VSL, 1, N, 0, VSR, 1, N, 0, WORK, 1, 0, RWORK, 1, 0, BWORK, 1, 0 ); // eslint-disable-line max-len
	assert.equal( res.info, 0, 'info' );
