...
function runProperty( nm, N, kl, ku ) {
	var rng = new RNG( 0x4000 + ( N * 100 ) + ( kl * 10 ) + ku );
	...
	var info = zgbcon( nm.api, N, kl, ku, Af.data, Af.args[ 0 ], Af.args[ 1 ], Af.args[ 2 ], ipiv, 1, 0, anorm, rcond, WORK, 1, 0, RWORK, 1, 0 );

	var Ainv = bandedInverse( A0, N, kl, ku );
	var invnorm = nm.fn( sc, Ainv );
	var trueRcond = 1.0 / ( anorm * invnorm );

	var label = 'zgbcon ' + nm.api + ' N=' + N + ' kl=' + kl + ' ku=' + ku;
