...
* @param {integer} strideB2 - stride of the second dimension of B
* @param {NonNegativeInteger} offsetB - starting index for B
* @param {Float64Array} work - caller-provided workspace array of length at least `MN + max(MN,nrhs)*NB` (where `MN = min(M,N)`, `NB = 32`), plus an extra `(NB+1)*NB` when the blocked path is taken (`MN > NB`) because the JS dgeqrf/dgelqf/dormqr store the block-reflector T factor in a separate buffer
...
* var work = new Float64Array( 66 );
*
* var info = dgels( 'no-transpose', 2, 2, 1, A, 1, 2, 0, B, 1, 2, 0, work, 1, 0 );
* // info => 0
*/
function dgels( trans, M, N, nrhs, A, strideA1, strideA2, offsetA, B, strideB1, strideB2, offsetB, work, strideWork, offsetWork ) {
	...
	// Workspace: MN slots for TAU, plus factorization/application scratch. Unlike
	// reference LAPACK (whose blocked DGEQRF packs the T factor inside its N*NB
	// buffer), the JS dgeqrf/dgelqf store T in a SEPARATE trailing NB*NB block and
	// dormqr/dormlq need an additional (NB+1)*NB. So on the blocked path
	// (min(M,N) > NB) the reference formula `MN + max(MN,nrhs)*NB` under-allocates
	// by up to (NB+1)*NB and produces silent all-NaN output. Add that headroom only
	// when the blocked path is actually taken (the unblocked path is unaffected).
	const minWork = Math.max( 1, MN + ( Math.max( MN, nrhs ) * NB ) + ( ( MN > NB ) ? ( ( NB + 1 ) * NB ) : 0 ) );
	...
	return base( trans, M, N, nrhs, A, strideA1, strideA2, offsetA, B, strideB1, strideB2, offsetB, work, strideWork, offsetWork );
}
