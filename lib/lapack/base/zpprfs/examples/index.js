/**
* @license Apache-2.0
*
* Copyright (c) 2026 Ricky Reusser.
*
* Derived from the LAPACK 3.12.0 reference implementation (BSD-3-Clause).
*
* Licensed under the Apache License, Version 2.0 (the "License");
* you may not use this file except in compliance with the License.
* You may obtain a copy of the License at
*
*    http://www.apache.org/licenses/LICENSE-2.0
*
* Unless required by applicable law or agreed to in writing, software
* distributed under the License is distributed on an "AS IS" BASIS,
* WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
* See the License for the specific language governing permissions and
* limitations under the License.
*/

import Complex128Array from '@stdlib/array/complex128/lib/index.js';
import Float64Array from '@stdlib/array/float64/lib/index.js';
import zpprfs from './../lib/index.js';

// Original 2x2 Hermitian positive definite matrix A (upper packed):

// A = [ 4+0i, 1-1i ]

//     [ 1+1i, 3+0i ]
const AP = new Complex128Array( [ 4.0, 0.0, 1.0, -1.0, 3.0, 0.0 ] );

// Cholesky factor (upper, from zpptrf):
const AFP = new Complex128Array( [ 2.0, 0.0, 0.5, -0.5, 1.5811, 0.0 ] );

// Right-hand side B (1 column):
const B = new Complex128Array( [ 6.0, 2.0, 5.0, 3.0 ] );

// Initial solution X (e.g., from zpptrs):
const X = new Complex128Array( [ 1.0, 1.0, 1.0, 1.0 ] );

const FERR = new Float64Array( 1 );
const BERR = new Float64Array( 1 );
const WORK = new Complex128Array( 4 );
const RWORK = new Float64Array( 2 );

const info = zpprfs.ndarray( 'upper', 2, 1, AP, 1, 0, AFP, 1, 0, B, 1, 2, 0, X, 1, 2, 0, FERR, 1, 0, BERR, 1, 0, WORK, 1, 0, RWORK, 1, 0 ); // eslint-disable-line max-len
console.log( 'info:', info ); // eslint-disable-line no-console
console.log( 'FERR:', FERR ); // eslint-disable-line no-console
console.log( 'BERR:', BERR ); // eslint-disable-line no-console
