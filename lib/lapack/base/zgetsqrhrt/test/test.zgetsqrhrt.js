...
import zgetsqrhrt from './../lib/zgetsqrhrt.js';
...
test( 'zgetsqrhrt has expected arity', function t() {
	assert.strictEqual( zgetsqrhrt.length, 11, 'has expected arity' );
});
...
test( 'zgetsqrhrt: column-major succeeds on a small problem', function t() {
	...
	const data = [ 2.0, 0.0, 1.0, 0.0, 3.0, 0.0, 1.0, 0.0, 1.0, 0.0, 4.0, 0.0, 2.0, 0.0, 3.0, 0.0 ];
	for ( i = 0; i < data.length; i++ ) {
		view[ i ] = data[ i ];
	}
	const T = new Complex128Array( 2 * 2 );
	const W = new Complex128Array( 256 );
	const info = zgetsqrhrt( 'column-major', 4, 2, 4, 2, 2, A, 4, T, 2, W );
