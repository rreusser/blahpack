...
/**
* Complex multiply: (ar+ai_i)_(br+bi*i).
*
* @private
...
function csqrt( ar, ai, out ) {
	...
	const t = Math.sqrt( ( Math.abs( ar ) + r ) / 2.0 );
	if ( ar >= 0.0 ) {
		out[ 0 ] = t;
		out[ 1 ] = ai / ( 2.0 * t );
	} else {
...
function zlahqr( wantt, wantz, N, ilo, ihi, H, strideH1, strideH2, offsetH, W, strideW, offsetW, iloz, ihiz, Z, strideZ1, strideZ2, offsetZ ) {
	let kdefl, rtemp, info, Zv, sz1, sz2, oZ, tst, aa, ab, ba, bb, h10, h21;
	let jlo, jhi, t1r, t1i, v2r, v2i, t2r, t2i, sumr, sumi, scr, sci, tempr;
	let tempi, absc, tr, ti, ur, ui, xr, xi, yr, yi, sx, s, temp, converged;
	let its, h11sr, h11si, hkjr, hkji, hk1jr, hk1ji, i, i1, i2, j, k, l, m;
	...
	const VV = new Complex128Array( 2 );
	const vv = reinterpret( VV, 0 );
	const tauBuf = new Complex128Array( 1 );
	const tauV = reinterpret( tauBuf, 0 );
