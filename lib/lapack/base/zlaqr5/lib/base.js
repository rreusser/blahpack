...
import zlaqr1 from '../../zlaqr1/lib/base.js';
...
function zlaqr5( wantt, wantz, kacc22, N, ktop, kbot, nshfts, s, strideS, offsetS, H, strideH1, strideH2, offsetH, iloz, ihiz, Z, strideZ1, strideZ2, offsetZ, V, strideV1, strideV2, offsetV, U, strideU1, strideU2, offsetU, nv, WV, strideWV1, strideWV2, offsetWV, nh, WH, strideWH1, strideWH2, offsetWH ) {
	let bmp22, ndcol, krcol, incol, mtop, mbot, jtop, jbot, jlen, jcol, jrow;
	let refR, refI, tst1, tst2, h11, h12, h21, h22, scl, dR, dI, aa, kms, m22;
	let nu, k1, i2, i4, Zv, t1R, t1I, t2R, t2I, t3R, t3I, sZ1, sZ2, oZ, j, k, m;
	...
	const sV1 = strideV1 * 2;
	const sV2 = strideV2 * 2;
	const oV = offsetV * 2;

	const Uv = reinterpret( U, 0 );
	const sU1 = strideU1 * 2;
	const sU2 = strideU2 * 2;
	const oU = offsetU * 2;

	// Scratch for VT(3), beta, tau, alpha
	const VTca = new Complex128Array( 3 );
	const VT = reinterpret( VTca, 0 );
	const betaCA = new Complex128Array( 1 );
	const betaV = reinterpret( betaCA, 0 );
	const tauCA = new Complex128Array( 1 );
	const tauV = reinterpret( tauCA, 0 );
	const alphaCA = new Complex128Array( 1 );
	const alphaV = reinterpret( alphaCA, 0 );
	...
	for ( incol = ktop - 2 * nbmps + 1; incol <= kbot - 2; incol += 2 * nbmps ) {
		...
		for ( krcol = incol; krcol <= Math.min( incol + 2 * nbmps - 1, kbot - 2 ); krcol++ ) {
			...
			if ( bmp22 ) {
				...
				// V(2, M22)
				const v2mR = Vv[ oV + sV1 + ( m22 - 1 ) * sV2 ]; // eslint-disable-line no-var
				const v2mI = Vv[ oV + sV1 + ( m22 - 1 ) * sV2 + 1 ]; // eslint-disable-line no-var
				...
				for ( j = jtop; j <= Math.min( kbot, k + 3 ); j++ ) {
					const hjk1 = cget( Hv, sH1, sH2, oH, j, k + 1 ); // eslint-disable-line no-var
					const hjk2 = cget( Hv, sH1, sH2, oH, j, k + 2 ); // eslint-disable-line no-var
				...
				for ( j = k + 1; j <= jbot; j++ ) {
					const hk1j = cget( Hv, sH1, sH2, oH, k + 1, j ); // eslint-disable-line no-var
					const hk2j = cget( Hv, sH1, sH2, oH, k + 2, j ); // eslint-disable-line no-var
				...
				if ( accum ) {
					...
					for ( j = Math.max( 1, ktop - incol ); j <= kdu; j++ ) {
						const ujk1 = cget( Uv, sU1, sU2, oU, j, kms + 1 ); // eslint-disable-line no-var
						const ujk2 = cget( Uv, sU1, sU2, oU, j, kms + 2 ); // eslint-disable-line no-var
					...
					t1I = Vv[ oV + ( m22 - 1 ) * sV2 + 1 ];
					for ( j = iloz; j <= ihiz; j++ ) {
						const zjk1 = cget( Zv, sZ1, sZ2, oZ, j, k + 1 ); // eslint-disable-line no-var
						const zjk2 = cget( Zv, sZ1, sZ2, oZ, j, k + 2 ); // eslint-disable-line no-var
			...
			for ( m = mbot; m >= mtop; m-- ) {
				...
				if ( k === ktop - 1 ) {
					...
					const hk3k2 = cget( Hv, sH1, sH2, oH, k + 3, k + 2 ); // eslint-disable-line no-var
					...
					const hk3k0 = cget( Hv, sH1, sH2, oH, k + 3, k ); // eslint-disable-line no-var
					const hk3k1 = cget( Hv, sH1, sH2, oH, k + 3, k + 1 ); // eslint-disable-line no-var
					const hk3k2b = cget( Hv, sH1, sH2, oH, k + 3, k + 2 ); // eslint-disable-line no-var
					if ( ( hk3k0[ 0 ] !== 0.0 || hk3k0[ 1 ] !== 0.0 ) || ( hk3k1[ 0 ] !== 0.0 || hk3k1[ 1 ] !== 0.0 ) || ( hk3k2b[ 0 ] === 0.0 && hk3k2b[ 1 ] === 0.0 ) ) {
						...
						const vt2cR = VT[ 2 ]; // eslint-disable-line no-var -- CONJ(VT(2))
						const vt2cI = -VT[ 3 ]; // eslint-disable-line no-var

						// REFSUM = H(K+1,K) + CONJ(VT(2))*H(K+2,K)
						var hk1k = cget( Hv, sH1, sH2, oH, k + 1, k ); // eslint-disable-line no-var
						const hk2k = cget( Hv, sH1, sH2, oH, k + 2, k ); // eslint-disable-line no-var
						...
						// T1c = CONJ(VT(1))
						const cT1R = tauV[ 0 ]; // eslint-disable-line no-var
						const cT1I = -tauV[ 1 ]; // eslint-disable-line no-var

						// T2c = T1c*VT(2)
						const cT2R = cT1R * VT[ 2 ] - cT1I * VT[ 3 ]; // eslint-disable-line no-var
						const cT2I = cT1R * VT[ 3 ] + cT1I * VT[ 2 ]; // eslint-disable-line no-var

						// T3c = T1c*VT(3)
						const cT3R = cT1R * VT[ 4 ] - cT1I * VT[ 5 ]; // eslint-disable-line no-var
						const cT3I = cT1R * VT[ 5 ] + cT1I * VT[ 4 ]; // eslint-disable-line no-var

						// Check: CABS1(H(K+2,K)-REFSUM*T2) + CABS1(REFSUM*T3) > ULP*(CABS1(H(K,K))+...)
						const rt2R = refR * cT2R - refI * cT2I; // eslint-disable-line no-var
						const rt2I = refR * cT2I + refI * cT2R; // eslint-disable-line no-var
						const rt3R = refR * cT3R - refI * cT3I; // eslint-disable-line no-var
						const rt3I = refR * cT3I + refI * cT3R; // eslint-disable-line no-var
						const chk2 = hk2k[ 0 ] - rt2R; // eslint-disable-line no-var
						const chk2i = hk2k[ 1 ] - rt2I; // eslint-disable-line no-var
				...
				for ( j = jtop; j <= Math.min( kbot, k + 3 ); j++ ) {
					const hjk1c = cget( Hv, sH1, sH2, oH, j, k + 1 ); // eslint-disable-line no-var
					const hjk2c = cget( Hv, sH1, sH2, oH, j, k + 2 ); // eslint-disable-line no-var
					const hjk3c = cget( Hv, sH1, sH2, oH, j, k + 3 ); // eslint-disable-line no-var
				...
				const hk1k1c = cget( Hv, sH1, sH2, oH, k + 1, k + 1 ); // eslint-disable-line no-var
				const hk2k1c = cget( Hv, sH1, sH2, oH, k + 2, k + 1 ); // eslint-disable-line no-var
				const hk3k1c = cget( Hv, sH1, sH2, oH, k + 3, k + 1 ); // eslint-disable-line no-var
			...
			for ( m = mbot; m >= mtop; m-- ) {
				...
				for ( j = Math.max( ktop, krcol + 2 * m ); j <= jbot; j++ ) {
					const hk1jd = cget( Hv, sH1, sH2, oH, k + 1, j ); // eslint-disable-line no-var
					const hk2jd = cget( Hv, sH1, sH2, oH, k + 2, j ); // eslint-disable-line no-var
					const hk3jd = cget( Hv, sH1, sH2, oH, k + 3, j ); // eslint-disable-line no-var
			...
			if ( accum ) {
				for ( m = mbot; m >= mtop; m-- ) {
					...
					t3I = t1I * v3R - t1R * v3I;
					for ( j = i2; j <= i4; j++ ) {
						const ujk1d = cget( Uv, sU1, sU2, oU, j, kms + 1 ); // eslint-disable-line no-var
						const ujk2d = cget( Uv, sU1, sU2, oU, j, kms + 2 ); // eslint-disable-line no-var
						const ujk3d = cget( Uv, sU1, sU2, oU, j, kms + 3 ); // eslint-disable-line no-var
				...
				for ( m = mbot; m >= mtop; m-- ) {
					...
					t3I = t1I * v3R - t1R * v3I;
					for ( j = iloz; j <= ihiz; j++ ) {
						const zjk1d = cget( Zv, sZ1, sZ2, oZ, j, k + 1 ); // eslint-disable-line no-var
						const zjk2d = cget( Zv, sZ1, sZ2, oZ, j, k + 2 ); // eslint-disable-line no-var
						const zjk3d = cget( Zv, sZ1, sZ2, oZ, j, k + 3 ); // eslint-disable-line no-var
		...
		if ( accum ) {
			let jtopAcc; // eslint-disable-line no-var
			let jbotAcc; // eslint-disable-line no-var
			if ( wantt ) {
				jtopAcc = 1;
				jbotAcc = N;
			} else {
