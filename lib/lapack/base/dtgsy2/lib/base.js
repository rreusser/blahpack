...
import dlatdf from '../../dlatdf/lib/base.js';
...
* @param {Int32Array} IWORK - caller-owned integer workspace of length M+N+2
...
function dtgsy2( trans, ijob, M, N, A, strideA1, strideA2, offsetA, B, strideB1, strideB2, offsetB, C, strideC1, strideC2, offsetC, D, strideD1, strideD2, offsetD, E, strideE1, strideE2, offsetE, F, strideF1, strideF2, offsetF, scale, rdsum, rdscal, IWORK, strideIWork, offsetIWork, pq ) {
	let scaloc, alpha, ierr, info, isp1, jsp1, zdim, res, is, ie, js, je, mb;
	let nb, ii, jj, kk, p, q, i, j, k;

	info = 0;
	const notran = ( trans === 'no-transpose' );

	// Fixed-size scratch for the (at most 8x8) Kronecker-product linear solve
	// (LDZ = 8). These do NOT scale with M or N — they are genuine local
	// bookkeeping arrays mirroring the reference Fortran locals Z(LDZ,LDZ),
	// RHS(LDZ), IPIV(LDZ), JPIV(LDZ), not caller-owned workspace. Sizes are
	// written as bare literals (64 = LDZ*LDZ, 8 = LDZ) so the fixed-size
	// exemption is recognized.
	const Z = new Float64Array( 64 );
	const RHS = new Float64Array( 8 );
	const IPIV = new Int32Array( 8 );
	const JPIV = new Int32Array( 8 );
