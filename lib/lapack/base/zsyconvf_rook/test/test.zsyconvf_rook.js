...
test( 'zsyconvf_rook returns 0 for N=0 (row-major)', function t() {
	const info = zsyconvf_rook( 'row-major', 'upper', 'convert', 0, new Complex128Array( 0 ), 1, new Complex128Array( 0 ), 1, new Int32Array( 0 ), 1, 0 );
	assert.strictEqual( info, 0, 'returns 0' );
});

test( 'zsyconvf_rook returns 0 for N=0 (column-major)', function t() {
	const info = zsyconvf_rook( 'column-major', 'lower', 'revert', 0, new Complex128Array( 0 ), 1, new Complex128Array( 0 ), 1, new Int32Array( 0 ), 1, 0 );
	assert.strictEqual( info, 0, 'returns 0' );
});
