...
* @param {(Float64Array|null)} WORK - caller-provided workspace (length `>= N` when `N > 0` and `nrhs > 0`); `null` requests internal allocation
* @param {integer} strideWork - stride for `WORK`
* @throws {TypeError} if a string argument is not a valid option
* @throws {RangeError} if a numerical argument does not satisfy constraints
* @returns {integer} info status code
*/
function dsysv( uplo, N, nrhs, A, LDA, IPIV, strideIPIV, B, LDB, WORK, strideWork ) { // eslint-disable-line max-len, max-params
	...
	const ow = stride2offset( max( 1, N ), strideWork );
