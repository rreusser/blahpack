...
test( 'zsytrf_rook: upper_4x4', function t() {
	...
	info = zsytrfRook( 'upper', n, A, 1, n, 0, IPIV, 1, 0, new Complex128Array( 1280 ), 1, 0 );
...
test( 'zsytrf_rook: lower_4x4', function t() {
	...
	Av[ 31 ] = -2; // (3,3)
	IPIV = new Int32Array( n );
	info = zsytrfRook( 'lower', n, A, 1, n, 0, IPIV, 1, 0, new Complex128Array( 1280 ), 1, 0 );
	assert.equal( info, tc.info, 'info' );
	expected = extractSubmatrix( tc.A, n, 50 );
	Av = reinterpret( A, 0 );
	assertArrayClose( toArray( Av ), expected, 1e-12, 'A' );
...
test( 'zsytrf_rook: n0 quick return', function t() {
	...
	info = zsytrfRook( 'lower', 0, A, 1, 1, 0, IPIV, 1, 0, new Complex128Array( 1280 ), 1, 0 );
...
test( 'zsytrf_rook: n1', function t() {
	...
	info = zsytrfRook( 'upper', 1, A, 1, 1, 0, IPIV, 1, 0, new Complex128Array( 1280 ), 1, 0 );
	assert.equal( info, tc.info, 'info' );
	Av = reinterpret( A, 0 );
...
test( 'zsytrf_rook: stride/offset support (lower)', function t() {
	...
	info = zsytrfRook( 'lower', 3, A, 1, 3, 2, IPIV, 2, 1, new Complex128Array( 96 ), 1, 0 );
	assert.equal( info, 0, 'info' );
});
