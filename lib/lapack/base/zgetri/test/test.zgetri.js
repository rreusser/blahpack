...
test( 'zgetri has expected arity', function t() {
	assert.strictEqual( zgetri.length, 8, 'has expected arity' );
});

test( 'zgetri throws TypeError for invalid order', function t() {
	assert.throws( function throws() {
		zgetri( 'invalid', new Float64Array( 4 ), new Float64Array( 4 ), 2, new Float64Array( 4 ), 1, new Float64Array( 4 ), 1 );
	}, TypeError );
});

test( 'zgetri throws RangeError for negative N', function t() {
	assert.throws( function throws() {
		zgetri( 'row-major', -1, new Float64Array( 4 ), 2, new Float64Array( 4 ), 1, new Float64Array( 4 ), 1 );
	}, RangeError );
});
