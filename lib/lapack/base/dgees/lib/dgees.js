...
*/
function dgees( jobvs, sort, select, N, A, LDA, sdim, WR, strideWR, WI, strideWI, VS, LDVS, WORK, strideWork, BWORK, strideBWork ) { // eslint-disable-line max-len, max-params
	...
	if ( WORK === null || WORK === void 0 ) {
		const minWork = ( N === 0 ) ? 1 : Math.max( 1, 3 * N );
		WORK = new Float64Array( minWork );
		strideWork = 1;
	}
	if ( BWORK === null || BWORK === void 0 ) {
		const minBwork = Math.max( 1, 3 * N);
		BWORK = new Uint8Array( minBwork );
		strideBWork = 1;
	}
	const owork = stride2offset( N, strideWork );
	const obwork = stride2offset( N, strideBWork );
	if ( N < 0 ) {
	...
	return base( jobvs, sort, select, N, A, sa1, sa2, 0, sdim, WR, strideWR, owr, WI, strideWI, owi, VS, svs1, svs2, 0, WORK, strideWork, owork, BWORK, strideBWork, obwork ); // eslint-disable-line max-len
}
