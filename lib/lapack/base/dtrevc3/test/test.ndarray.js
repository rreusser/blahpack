...
test( 'dtrevc3: right eigenvectors, all, 4x4', function t() {
	...
	M = 0;
	info = dtrevc3( 'right', 'all', SELECT, 1, 0, N, T, 1, N, 0, VL, 1, N, 0, VR, 1, N, 0, N, M, WORK, 1, 0 ); // eslint-disable-line max-len
...
test( 'dtrevc3: left eigenvectors, all, 4x4', function t() {
	...
	WORK = new Float64Array( 3 * N );
	M = 0;
	info = dtrevc3( 'left', 'all', SELECT, 1, 0, N, T, 1, N, 0, VL, 1, N, 0, VR, 1, N, 0, N, M, WORK, 1, 0 ); // eslint-disable-line max-len
	assert.strictEqual( info, tc.info, 'info' );
...
test( 'dtrevc3: both eigenvectors, all, 4x4', function t() {
	...
	WORK = new Float64Array( 3 * N );
	M = 0;
	info = dtrevc3( 'both', 'all', SELECT, 1, 0, N, T, 1, N, 0, VL, 1, N, 0, VR, 1, N, 0, N, M, WORK, 1, 0 ); // eslint-disable-line max-len
	assert.strictEqual( info, tc.info, 'info' );
	assertArrayClose( toArray( VR ), tc.VR, 1e-12, 'VR' );
	assertArrayClose( toArray( VL ), tc.VL, 1e-12, 'VL' );
});
...
test( 'dtrevc3: right eigenvectors, selected, 4x4', function t() {
	...
	WORK = new Float64Array( 3 * N );
	M = 0;
	info = dtrevc3( 'right', 'selected', SELECT, 1, 0, N, T, 1, N, 0, VL, 1, N, 0, VR, 1, N, 0, N, M, WORK, 1, 0 ); // eslint-disable-line max-len
	assert.strictEqual( info, tc.info, 'info' );
	assertArrayClose( toArray( VR ), tc.VR, 1e-12, 'VR' );
});
...
test( 'dtrevc3: right backtransform, 4x4', function t() {
	...
	WORK = new Float64Array( 3 * N );
	M = 0;
	info = dtrevc3( 'right', 'backtransform', SELECT, 1, 0, N, T, 1, N, 0, VL, 1, N, 0, VR, 1, N, 0, N, M, WORK, 1, 0 ); // eslint-disable-line max-len
	assert.strictEqual( info, tc.info, 'info' );
	assertArrayClose( toArray( VR ), tc.VR, 1e-12, 'VR' );
});
...
test( 'dtrevc3: right N=1', function t() {
	...
	M = 0;
	info = dtrevc3( 'right', 'all', SELECT, 1, 0, N, T, 1, N, 0, VL, 1, N, 0, VR, 1, N, 0, 1, M, WORK, 1, 0 ); // eslint-disable-line max-len
	tc = right_n_1;
	assert.strictEqual( info, tc.info, 'info' );
	assertArrayClose( toArray( VR ), tc.VR, 1e-12, 'VR' );
});
...
test( 'dtrevc3: left selected complex 4x4', function t() {
	...
	WORK = new Float64Array( 3 * N );
	M = 0;
	info = dtrevc3( 'left', 'selected', SELECT, 1, 0, N, T, 1, N, 0, VL, 1, N, 0, VR, 1, N, 0, N, M, WORK, 1, 0 ); // eslint-disable-line max-len
	assert.strictEqual( info, tc.info, 'info' );
	assertArrayClose( toArray( VL ), tc.VL, 1e-12, 'VL' );
});
...
test( 'dtrevc3: left backtransform, 4x4', function t() {
	...
	WORK = new Float64Array( 3 * N );
	M = 0;
	info = dtrevc3( 'left', 'backtransform', SELECT, 1, 0, N, T, 1, N, 0, VL, 1, N, 0, VR, 1, N, 0, N, M, WORK, 1, 0 ); // eslint-disable-line max-len
	assert.strictEqual( info, tc.info, 'info' );
	assertArrayClose( toArray( VL ), tc.VL, 1e-12, 'VL' );
});
...
test( 'dtrevc3: both backtransform 6x6', function t() {
	...
	WORK = new Float64Array( 3 * N );
	M = 0;
	info = dtrevc3( 'both', 'backtransform', SELECT, 1, 0, N, T, 1, N, 0, VL, 1, N, 0, VR, 1, N, 0, N, M, WORK, 1, 0 ); // eslint-disable-line max-len
	assert.strictEqual( info, tc.info, 'info' );
	assertArrayClose( toArray( VR ), tc.VR, 1e-12, 'VR' );
	assertArrayClose( toArray( VL ), tc.VL, 1e-12, 'VL' );
});
