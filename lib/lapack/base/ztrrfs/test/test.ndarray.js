...
function callZtrrfs( uplo, trans, diag, N, nrhs, A, B, X ) {
	...
	const info = ztrrfs( uplo, trans, diag, N, nrhs, A, 1, N, 0, B, 1, N, 0, X, 1, N, 0, FERR, 1, 0, BERR, 1, 0, ws.WORK, 1, 0, ws.RWORK, 1, 0 ); // eslint-disable-line max-len
	const result = {
		'info': info,
		'ferr': FERR,
		'berr': BERR
...
test( 'ztrrfs: upper_no_trans', function t() {
	...
	const result = callZtrrfs( 'upper', 'no-transpose', 'non-unit', 3, 1, A, B, X );
...
test( 'ztrrfs: lower_conj_trans', function t() {
	...
	const A = new Complex128Array( [ 2, 1, 1, 2, 3, 1, 0, 0, 4, 1, 5, 2, 0, 0, 0, 0, 6, 1 ] );
	const B = new Complex128Array( [ 7, -7, 11, -9, 5, -7 ] );
	const X = new Complex128Array( tc.x );
	const result = callZtrrfs( 'lower', 'conjugate-transpose', 'non-unit', 3, 1, A, B, X );
	assert.equal( result.info, tc.info );
	assert.equal( result.berr[ 0 ], tc.berr[ 0 ] );
	assert.ok( result.ferr[ 0 ] < 1e-13, 'ferr should be small, got ' + result.ferr[ 0 ] );
});
...
test( 'ztrrfs: upper_unit_diag', function t() {
	...
	const A = new Complex128Array( [ 1, 0, 0, 0, 0, 0, 2, 1, 1, 0, 0, 0, 3, 2, 4, 1, 1, 0 ] );
	const B = new Complex128Array( [ 14, 8, 14, 3, 3, 0 ] );
	const X = new Complex128Array( tc.x );
	const result = callZtrrfs( 'upper', 'no-transpose', 'unit', 3, 1, A, B, X );
	assert.equal( result.info, tc.info );
	assert.equal( result.berr[ 0 ], tc.berr[ 0 ] );
...
test( 'ztrrfs: lower_no_trans', function t() {
	...
	const A = new Complex128Array( [ 3, 1, 2, 1, 1, 1, 0, 0, 5, 2, 4, 1, 0, 0, 0, 0, 7, 1 ] );
	const B = new Complex128Array( [ 3, 1, 7, 3, 12, 3 ] );
	const X = new Complex128Array( tc.x );
	const result = callZtrrfs( 'lower', 'no-transpose', 'non-unit', 3, 1, A, B, X );
	assert.equal( result.info, tc.info );
	assert.equal( result.berr[ 0 ], tc.berr[ 0 ] );
	assert.ok( result.ferr[ 0 ] < 1e-13, 'ferr should be small, got ' + result.ferr[ 0 ] );
});
...
test( 'ztrrfs: multi_rhs', function t() {
	...
	const tc = multi_rhs;
	const A = new Complex128Array( [ 2, 1, 0, 0, 0, 0, 1, 2, 4, 1, 0, 0, 3, 1, 5, 2, 6, 1 ] );
	const B = new Complex128Array( [ 6, 4, 9, 3, 6, 1, 7, 5, 15, -1, 7, -5 ] );
	const X = new Complex128Array( tc.x );
	const result = callZtrrfs( 'upper', 'no-transpose', 'non-unit', 3, 2, A, B, X );
	assert.equal( result.info, tc.info );
	assert.equal( result.berr[ 0 ], tc.berr[ 0 ] );
...
test( 'ztrrfs: n_zero', function t() {
	...
	const info = ztrrfs( 'upper', 'no-transpose', 'non-unit', 0, 1, A, 1, 1, 0, B, 1, 1, 0, X, 1, 1, 0, FERR, 1, 0, BERR, 1, 0, WORK, 1, 0, RWORK, 1, 0 ); // eslint-disable-line max-len
...
test( 'ztrrfs: nrhs_zero', function t() {
	...
	const X = new Complex128Array( 9 );
	const FERR = new Float64Array( 1 );
	const BERR = new Float64Array( 1 );
	const WORK = new Complex128Array( 6 );
	const RWORK = new Float64Array( 3 );
	const info = ztrrfs( 'upper', 'no-transpose', 'non-unit', 3, 0, A, 1, 3, 0, B, 1, 3, 0, X, 1, 3, 0, FERR, 1, 0, BERR, 1, 0, WORK, 1, 0, RWORK, 1, 0 ); // eslint-disable-line max-len
	assert.equal( info, tc.info );
});
...
test( 'ztrrfs: lower_unit_conj_trans', function t() {
	...
	const A = new Complex128Array( [ 1, 0, 2, 1, 3, 2, 0, 0, 1, 0, 5, 1, 0, 0, 0, 0, 1, 0 ] );
	const B = new Complex128Array( [ 6, -3, 6, -1, 1, 0 ] );
	const X = new Complex128Array( tc.x );
	const result = callZtrrfs( 'lower', 'conjugate-transpose', 'unit', 3, 1, A, B, X );
	assert.equal( result.info, tc.info );
	assert.equal( result.berr[ 0 ], tc.berr[ 0 ] );
	assert.ok( result.ferr[ 0 ] < 1e-12, 'ferr should be small, got ' + result.ferr[ 0 ] );
});
...
test( 'ztrrfs: upper_conj_trans', function t() {
	...
	const A = new Complex128Array( [ 2, 1, 0, 0, 0, 0, 1, 2, 4, 1, 0, 0, 3, 1, 5, 2, 6, 1 ] );
	const B = new Complex128Array( [ 2, -1, 5, -3, 14, -4 ] );
	const X = new Complex128Array( tc.x );
	const result = callZtrrfs( 'upper', 'conjugate-transpose', 'non-unit', 3, 1, A, B, X );
	assert.equal( result.info, tc.info );
	assert.equal( result.berr[ 0 ], tc.berr[ 0 ] );
	assert.ok( result.ferr[ 0 ] < 1e-13, 'ferr should be small, got ' + result.ferr[ 0 ] );
});
...
test( 'ztrrfs: upper_unit_conj_trans', function t() {
	...
	const A = new Complex128Array( [ 1, 0, 0, 0, 0, 0, 2, 1, 1, 0, 0, 0, 3, 2, 4, 1, 1, 0 ] );
	const B = new Complex128Array( [ 1, 0, 3, -1, 8, -3 ] );
	const X = new Complex128Array( tc.x );
	const result = callZtrrfs( 'upper', 'conjugate-transpose', 'unit', 3, 1, A, B, X );
	assert.equal( result.info, tc.info );
	assert.equal( result.berr[ 0 ], tc.berr[ 0 ] );
	assert.ok( result.ferr[ 0 ] < 1e-12, 'ferr should be small, got ' + result.ferr[ 0 ] );
});
...
test( 'ztrrfs: lower_unit_no_trans', function t() {
	...
	const A = new Complex128Array( [ 1, 0, 2, 1, 3, 2, 0, 0, 1, 0, 5, 1, 0, 0, 0, 0, 1, 0 ] );
	const B = new Complex128Array( [ 1, 0, 3, 1, 9, 3 ] );
	const X = new Complex128Array( tc.x );
	const result = callZtrrfs( 'lower', 'no-transpose', 'unit', 3, 1, A, B, X );
	assert.equal( result.info, tc.info );
	assert.equal( result.berr[ 0 ], tc.berr[ 0 ] );
	assert.ok( result.ferr[ 0 ] < 1e-12, 'ferr should be small, got ' + result.ferr[ 0 ] );
});
