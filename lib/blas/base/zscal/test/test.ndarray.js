...
test( 'zscal: non-unit stride (strideX=2, za=(0,1))', function t() {
	...
	const tc = zscal_stride;
	const za = new Complex128( 0.0, 1.0 );
	const zx = new Complex128Array([
		1.0, 2.0, 99.0, 99.0, 3.0, 4.0, 99.0, 99.0, 5.0, 6.0
	]);
	const result = base( 3, za, zx, 2, 0 );
	assert.strictEqual( result, zx );
...
test( 'zscal: za=(1,0) is identity (no-op)', function t() {
	...
	const tc = zscal_za_one;
	const za = new Complex128( 1.0, 0.0 );
	const zx = new Complex128Array( [ 7.0, 8.0, 9.0, 10.0 ] );
	const result = base( 2, za, zx, 1, 0 );
	assert.strictEqual( result, zx );
