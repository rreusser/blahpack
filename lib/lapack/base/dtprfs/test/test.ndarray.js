...
import dtprfs from './../lib/ndarray.js';
...
test( 'dtprfs: upper_no_trans', function t() {
	...
	const AP = new Float64Array([ 2, 1, 4, 3, 5, 6 ]);
	const B = new Float64Array([ 13, 23, 18 ]);
	const X = new Float64Array( tc.x );
	const result = callDtprfs( 'upper', 'no-transpose', 'non-unit', 3, 1, AP, B, X );
...
test( 'dtprfs: lower_trans', function t() {
	...
	const AP = new Float64Array([ 2, 1, 3, 4, 5, 6 ]);
	const B = new Float64Array([ 13, 23, 18 ]);
	const X = new Float64Array( tc.x );
	const result = callDtprfs( 'lower', 'transpose', 'non-unit', 3, 1, AP, B, X );
	assert.equal( result.info, tc.info );
	assert.equal( result.berr[ 0 ], tc.berr[ 0 ] );
	assert.ok( result.ferr[ 0 ] < 1e-13, 'ferr should be small, got ' + result.ferr[ 0 ] );
});
...
test( 'dtprfs: upper_unit_no_trans', function t() {
	...
	const AP = new Float64Array([ 1, 2, 1, 3, 4, 1 ]);
	const B = new Float64Array([ 14, 14, 3 ]);
	const X = new Float64Array( tc.x );
	const result = callDtprfs( 'upper', 'no-transpose', 'unit', 3, 1, AP, B, X );
	assert.equal( result.info, tc.info );
	assert.equal( result.berr[ 0 ], tc.berr[ 0 ] );
...
test( 'dtprfs: lower_no_trans', function t() {
	...
	const AP = new Float64Array([ 3, 2, 1, 5, 4, 7 ]);
	const B = new Float64Array([ 3, 12, 30 ]);
	const X = new Float64Array( tc.x );
	const result = callDtprfs( 'lower', 'no-transpose', 'non-unit', 3, 1, AP, B, X );
	assert.equal( result.info, tc.info );
	assert.equal( result.berr[ 0 ], tc.berr[ 0 ] );
	assert.ok( result.ferr[ 0 ] < 1e-13, 'ferr should be small, got ' + result.ferr[ 0 ] );
});
...
test( 'dtprfs: multi_rhs', function t() {
	...
	const B = new Float64Array([ 13, 23, 18, 31, 50, 36 ]);
	const X = new Float64Array( tc.x );
	const result = callDtprfs( 'upper', 'no-transpose', 'non-unit', 3, 2, AP, B, X );
	assert.equal( result.info, tc.info );
	assert.equal( result.berr[ 0 ], tc.berr[ 0 ] );
...
test( 'dtprfs: n_one', function t() {
	...
	const X = new Float64Array( tc.x );
	const result = callDtprfs( 'upper', 'no-transpose', 'non-unit', 1, 1, AP, B, X );
	assert.equal( result.info, tc.info );
	assert.equal( result.berr[ 0 ], tc.berr[ 0 ] );
	assert.ok( result.ferr[ 0 ] < 1e-13, 'ferr should be small, got ' + result.ferr[ 0 ] );
});
...
test( 'dtprfs: lower_unit_trans', function t() {
	...
	const AP = new Float64Array([ 1, 2, 3, 1, 5, 1 ]);
	const B = new Float64Array([ 14, 17, 3 ]);
	const X = new Float64Array( tc.x );
	const result = callDtprfs( 'lower', 'transpose', 'unit', 3, 1, AP, B, X );
	assert.equal( result.info, tc.info );
	assert.equal( result.berr[ 0 ], tc.berr[ 0 ] );
	assert.ok( result.ferr[ 0 ] < 1e-12, 'ferr should be small, got ' + result.ferr[ 0 ] );
});
...
test( 'dtprfs: upper_trans', function t() {
	...
	// A = [2 1 3; 0 4 5; 0 0 6] packed upper = [2, 1, 4, 3, 5, 6]
	const AP = new Float64Array([ 2, 1, 4, 3, 5, 6 ]);
	const B = new Float64Array([ 2, 9, 31 ]);
	const X = new Float64Array( tc.x );
	const result = callDtprfs( 'upper', 'transpose', 'non-unit', 3, 1, AP, B, X );
	assert.equal( result.info, tc.info );
	assert.equal( result.berr[ 0 ], tc.berr[ 0 ] );
	assert.ok( result.ferr[ 0 ] < 1e-13, 'ferr should be small, got ' + result.ferr[ 0 ] );
});
...
test( 'dtprfs: upper_unit_trans', function t() {
	...
	// A = [1 2 3; 0 1 4; 0 0 1] packed upper = [1, 2, 1, 3, 4, 1]
	const AP = new Float64Array([ 1, 2, 1, 3, 4, 1 ]);
	const B = new Float64Array([ 1, 4, 14 ]);
	const X = new Float64Array( tc.x );
	const result = callDtprfs( 'upper', 'transpose', 'unit', 3, 1, AP, B, X );
	assert.equal( result.info, tc.info );
	assert.equal( result.berr[ 0 ], tc.berr[ 0 ] );
	assert.ok( result.ferr[ 0 ] < 1e-12, 'ferr should be small, got ' + result.ferr[ 0 ] );
});
...
test( 'dtprfs: lower_unit_no_trans', function t() {
	...
	// L = [1 0 0; 2 1 0; 3 5 1] packed lower = [1, 2, 3, 1, 5, 1]
	const AP = new Float64Array([ 1, 2, 3, 1, 5, 1 ]);
	const B = new Float64Array([ 1, 4, 16 ]);
	const X = new Float64Array( tc.x );
	const result = callDtprfs( 'lower', 'no-transpose', 'unit', 3, 1, AP, B, X );
	assert.equal( result.info, tc.info );
	assert.equal( result.berr[ 0 ], tc.berr[ 0 ] );
	assert.ok( result.ferr[ 0 ] < 1e-12, 'ferr should be small, got ' + result.ferr[ 0 ] );
});
