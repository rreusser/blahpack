...
import zlatrz from './../lib/zlatrz.js';
...
*/
function buildA3x5() {
	const out = new Complex128Array( 15 );
	const av = reinterpret( out, 0 );
	av[ 0 ] = 4.0;
	av[ 1 ] = 0.5;
	av[ 6 ] = 1.0;
	av[ 7 ] = -0.2;
...
test( 'zlatrz column-major: 3x5 L=2 reduces A and produces non-zero TAU', function t() {
	...
	const out = zlatrz( 'column-major', 3, 5, 2, A, 3, TAU, 1, work, 1 );
	...
	const sum = Math.abs( tv[ 0 ] ) + Math.abs( tv[ 2 ] ) + Math.abs( tv[ 4 ] );
...
test( 'zlatrz row-major: agrees with column-major on the same matrix', function t() {
	...
	const workCM = new Complex128Array( 3 );
	const workRM = new Complex128Array( 3 );
	const TAUcm = new Complex128Array( 3 );
	const TAUrm = new Complex128Array( 3 );

	const Acm = buildA3x5();
	const av = reinterpret( Acm, 0 );

	// Build the row-major equivalent of the same logical matrix:
	const Arm = new Complex128Array( 15 );
	const rv = reinterpret( Arm, 0 );
	...
	const tcv = reinterpret( TAUcm, 0 );
	const trv = reinterpret( TAUrm, 0 );
	for ( i = 0; i < 6; i++ ) {
