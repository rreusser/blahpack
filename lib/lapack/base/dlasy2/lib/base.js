...
// DATA arrays (0-based)
const LOCU12 = [ 2, 3, 0, 1 ];  // Fortran: 3, 4, 1, 2
const LOCL21 = [ 1, 0, 3, 2 ];  // Fortran: 2, 1, 4, 3
const LOCU22 = [ 3, 2, 1, 0 ];  // Fortran: 4, 3, 2, 1
...
function dlasy2( ltranl, ltranr, isgn, n1, n2, TL, strideTL1, strideTL2, offsetTL, TR, strideTR1, strideTR2, offsetTR, B, strideB1, strideB2, offsetB, scale, X, strideX1, strideX2, offsetX, xnorm ) {
	let info, smin, temp, ipsv, jpsv, xmax, tau1, btmp, bet, gam, tmp, ip, jp;
	let k, i, j;
	...
	btmp = new Float64Array( 4 );
	const t16 = new Float64Array( 16 ); // 4x4 stored column-major with stride 4
...
function solve2x2( tmp, btmp, smin, n1, scale, X, strideX1, strideX2, offsetX, xnorm ) {
	let info, temp, u11, u22;

	info = 0;

	// Partial pivoting
	const ipiv = idamax( 4, tmp, 1, 0 );
	u11 = tmp[ ipiv ];
