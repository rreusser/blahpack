...
import zsytf2rk from './../../zsytf2_rk/lib/base.js';
import zlasyfrk from './../../zlasyf_rk/lib/base.js';
...
*/
function zsytrfrk( uplo, N, A, strideA1, strideA2, offsetA, e, strideE, offsetE, IPIV, strideIPIV, offsetIPIV, WORK, strideWork, offsetWork ) {
	var ldwork;
	var result;
	var iinfo;
	var ipidx;
	...
	if ( uplo === 'upper' ) {
		...
		while ( k >= 1 ) {
			if ( k > nb ) {
				...
				result = zlasyfrk( 'upper', k, nb, A, strideA1, strideA2, offsetA, e, strideE, offsetE, IPIV, strideIPIV, offsetIPIV, WORK, strideWork, ldwork * strideWork, offsetWork );
				kb = result.kb;
				iinfo = result.info;
			} else {
		...
		while ( k <= N ) {
			if ( k <= N - nb ) {
				...
				result = zlasyfrk( 'lower', N - k + 1, nb, A, strideA1, strideA2, offsetA + ( ( k - 1 ) * strideA1 ) + ( ( k - 1 ) * strideA2 ), e, strideE, offsetE + ( ( k - 1 ) * strideE ), IPIV, strideIPIV, offsetIPIV + ( ( k - 1 ) * strideIPIV ), WORK, strideWork, ldwork * strideWork, offsetWork );
				kb = result.kb;
				iinfo = result.info;
			} else {
