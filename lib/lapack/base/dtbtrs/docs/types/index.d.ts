...
interface Routine {
	...
	*/
	...
	/**
	* Solves a triangular banded system of the form `A*X = B`, `A^T*X = B`, or `A^H*X = B` where A is a triangular band matrix of order N with kd super- or sub-diagonals, and B is an N-by-NRHS matrix using alternative indexing semantics.
	*
	* @param uplo - specifies whether the upper or lower triangular part is referenced
	* @param trans - specifies whether the matrix should be transposed
	* @param diag - specifies whether the matrix is unit triangular
	...
	* @returns result
	*/
	ndarray( uplo: MatrixTriangle, trans: TransposeOperation, diag: DiagonalType, N: number, kd: number, nrhs: number, AB: Float64Array, strideAB1: number, strideAB2: number, offsetAB: number, B: Float64Array, strideB1: number, strideB2: number, offsetB: number ): number;
}
