...
import zlarzt from './../lib/ndarray.js';


// VARIABLES //

const fixtureDir = path.join( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures' );
const lines = readFileSync( path.join( fixtureDir, 'zlarzt.jsonl' ), 'utf8' ).trim().split( '\n' ); // eslint-disable-line max-len, node/no-sync
...
test( 'zlarzt: K=1, backward/rowwise', function t() {
	...
	const tc = findCase( 'k1_backward_rowwise' );
	const K = 1;
	const N = 5;

	// Column-major packing: strideV1=1 (rows), strideV2=K (columns)
	const V = new Complex128Array( new Float64Array( tc.V ) );
...
test( 'zlarzt: K=3, backward/rowwise', function t() {
	...
	const tc = findCase( 'k3_backward_rowwise' );
	const K = 3;
	const N = 8;
...
test( 'zlarzt: K=3, backward/rowwise, zero tau', function t() {
	...
	const tc = findCase( 'k3_zero_tau' );
	const K = 3;
	const N = 8;

	// Same V entries as test 2
	const tc2 = findCase( 'k3_backward_rowwise' );
	const V = new Complex128Array( new Float64Array( tc2.V ) );
	const TAU = new Complex128Array( new Float64Array( tc.TAU ) );
	const T = new Complex128Array( K * K );
...
test( 'zlarzt: N=0 quick return', function t() {
	...
	const K = 2;
	const T = new Complex128Array( K * K );
	const V = new Complex128Array( K * 4 );
...
test( 'zlarzt: all-zero tau', function t() {
	...
	const V = new Complex128Array( K * N );
	const TAU = new Complex128Array( K ); // all zero
	const T = new Complex128Array( K * K );
...
test( 'zlarzt: non-unit strides for V', function t() {
	...
	const V = new Complex128Array( 2 * N );
	const Vref = new Complex128Array( new Float64Array( tc.V ) );
	const Vv = reinterpret( V, 0 );
	const Vrefv = reinterpret( Vref, 0 );
