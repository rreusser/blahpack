...
function runCase( name, transr, uplo ) {
	...
	const ARF = new Complex128Array( new Float64Array( tc.ARF ) );
	const AP = new Complex128Array( ( N * ( N + 1 ) / 2 ) | 0 );
	const expected = new Float64Array( tc.AP );

	const info = ztfttp( transr, uplo, N, ARF, 1, 0, AP, 1, 0 );
	assert.equal( info, 0, name + ': info' );

	const result = reinterpret( AP, 0 );
...
test( 'ztfttp: N=0 quick return', function t() {
	...
	const AP = new Complex128Array( 3 );
	const info = ztfttp( 'no-transpose', 'lower', 0, new Complex128Array( 0 ), 1, 0, AP, 1, 0 );
	assert.equal( info, 0, 'info should be 0' );
});

test( 'ztfttp: N=1, normal', function t() {

	const tc = fixtures.n1_N;
	const ARF = new Complex128Array( new Float64Array( [ 42.0, 7.0 ] ) );
	const AP = new Complex128Array( 1 );
	const info = ztfttp( 'no-transpose', 'lower', 1, ARF, 1, 0, AP, 1, 0 );
...
test( 'ztfttp: N=1, conjugate-transpose', function t() {
	...
	const tc = fixtures.n1_C;
	const ARF = new Complex128Array( new Float64Array( [ 99.0, -3.0 ] ) );
	const AP = new Complex128Array( 1 );
	const info = ztfttp( 'conjugate-transpose', 'upper', 1, ARF, 1, 0, AP, 1, 0 );
...
test( 'ztfttp: N=5, no-transpose, lower, strideARF=2', function t() {
	...
	const tc = fixtures.n5_N_L;
	const N = tc.n;
	const expected = new Float64Array( tc.AP );

	// Create ARF with stride 2 (interleave with dummy complex elements):
	const ARFdata = new Float64Array( tc.ARF.length * 2 );
	for ( i = 0; i < tc.ARF.length / 2; i += 1 ) {
		ARFdata[ i * 4 ] = tc.ARF[ i * 2 ];
		ARFdata[ ( i * 4 ) + 1 ] = tc.ARF[ ( i * 2 ) + 1 ];
	}
	const ARF = new Complex128Array( ARFdata );
	const AP = new Complex128Array( ( N * ( N + 1 ) / 2 ) | 0 );

	const info = ztfttp( 'no-transpose', 'lower', N, ARF, 2, 0, AP, 1, 0 );
	assert.equal( info, 0, 'info' );
...
test( 'ztfttp: N=5, no-transpose, lower, strideAP=2', function t() {
	...
	const N = tc.n;
	const ARF = new Complex128Array( new Float64Array( tc.ARF ) );
	const AP = new Complex128Array( ( N * ( N + 1 ) / 2 ) * 2 );

	const info = ztfttp( 'no-transpose', 'lower', N, ARF, 1, 0, AP, 2, 0 );
	assert.equal( info, 0, 'info' );
...
test( 'ztfttp: N=6, conjugate-transpose, upper, with offset', function t() {
	...
	const tc = fixtures.n6_C_U;
	const N = tc.n;
	const NT = ( N * ( N + 1 ) / 2 ) | 0;
	const expected = new Float64Array( tc.AP );

	// Prepend 3 dummy complex elements:
	const ARFdata = new Float64Array( tc.ARF.length + 6 );
	for ( i = 0; i < tc.ARF.length; i += 1 ) {
		ARFdata[ i + 6 ] = tc.ARF[ i ];
	}
	const ARF = new Complex128Array( ARFdata );
	const AP = new Complex128Array( NT + 5 );

	const info = ztfttp( 'conjugate-transpose', 'upper', N, ARF, 1, 3, AP, 1, 5 );
	assert.equal( info, 0, 'info' );
