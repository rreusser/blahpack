...
import zhetri3x from './../lib/ndarray.js';


// FIXTURES //

const fixtureDir = path.join( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures' ); // eslint-disable-line max-len
const lines = readFileSync( path.join( fixtureDir, 'zhetri_3x.jsonl' ), 'utf8' ).trim().split( '\n' ); // eslint-disable-line node/no-sync, max-len
...
function runFixture( uplo, N, nb, tc ) {
	...
	const info = zhetri3x( uplo, N, A, 1, LDA, 0, e, 1, 0, ipiv, 1, 0, work, 1, 0, nb );
...
test( 'zhetri_3x: N=1 lower trivial inverse', function t() {
	...
	const info = zhetri3x( 'lower', 1, A, 1, LDA, 0, e, 1, 0, ipiv, 1, 0, work, 1, 0, 1 );
	assert.equal( info, tc.info, 'info' );
	const Aview = new Float64Array( A.buffer );
...
test( 'zhetri_3x: N=0 quick return', function t() {
	...
	const info = zhetri3x( 'lower', 0, A, 1, 1, 0, e, 1, 0, ipiv, 1, 0, work, 1, 0, 1 );
