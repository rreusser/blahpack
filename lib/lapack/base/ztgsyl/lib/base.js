...
import ztgsy2 from '../../ztgsy2/lib/base.js';
...
* @param {Complex128Array} WORK - caller-owned complex workspace; must hold at least `2*M*N` elements from `offsetWork` when `trans` is `'no-transpose'` and `ijob` is 1 or 2 (otherwise unused). base.js never allocates.
* @param {integer} strideWork - stride for WORK (must be 1; the workspace is a packed M-by-N buffer)
* @param {NonNegativeInteger} offsetWork - starting index for WORK
* @param {Int32Array} IWORK - caller-owned integer workspace of at least `M+N+2` elements from `offsetIWork`
* @param {integer} strideIWork - stride for IWORK
* @param {NonNegativeInteger} offsetIWork - starting index for IWORK
* @returns {integer} info - 0 if successful
*/
function ztgsyl( trans, ijob, M, N, A, strideA1, strideA2, offsetA, B, strideB1, strideB2, offsetB, C, strideC1, strideC2, offsetC, D, strideD1, strideD2, offsetD, E, strideE1, strideE2, offsetE, F, strideF1, strideF2, offsetF, scale, dif, WORK, strideWork, offsetWork, IWORK, strideIWork, offsetIWork ) {
	let dscale, scaloc, scale2, iround, isolve, ifunc, linfo, info, dsum, is;
	let ie, js, je, mb, nb, pq, p, q, i, j, k;
	...
	// Partition the caller-owned complex WORK buffer. When needed (notran &&
	// ijob in {1,2}) it holds two packed M-by-N copies of C and F, contiguous
	// with leading dimension M: W[0..M*N) and W[M*N..2*M*N). base.js never
	// allocates — the caller (wrapper/ndarray) owns and sizes the buffer.
	const W = ( WORK && offsetWork ) ? WORK.subarray( offsetWork ) : WORK;
	...
	const SCALV = new Float64Array( 1 );
	const DSUMV = new Float64Array( 1 );
	const DSCLV = new Float64Array( 1 );
	...
	if ( ( mb <= 1 && nb <= 1 ) || ( mb >= M && nb >= N ) ) {
		...
		for ( iround = 0; iround < isolve; iround++ ) {
			...
			if ( isolve === 2 && iround === 0 ) {
				...
				zlacpy( 'full', M, N, C, strideC1, strideC2, offsetC, W, 1, M, 0 );
				zlacpy( 'full', M, N, F, strideF1, strideF2, offsetF, W, 1, M, M * N );
				zlaset( 'full', M, N, CZERO, CZERO, C, strideC1, strideC2, offsetC );
				zlaset( 'full', M, N, CZERO, CZERO, F, strideF1, strideF2, offsetF );
			} else if ( isolve === 2 && iround === 1 ) {
				zlacpy( 'full', M, N, W, 1, M, 0, C, strideC1, strideC2, offsetC );
				zlacpy( 'full', M, N, W, 1, M, M * N, F, strideF1, strideF2, offsetF );
				scale[ 0 ] = scale2;
			}
		}
		return info;
