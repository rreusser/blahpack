...
import dzasum from '../../../../blas/base/dzasum/lib/base.js';
import idamax from '../../../../blas/base/idamax/lib/base.js';
import izamax from '../../../../blas/base/izamax/lib/base.js';
...
import zdotu from '../../../../blas/base/zdotu/lib/base.js';
import ztpsv from '../../../../blas/base/ztpsv/lib/base.js';
import dlamch from '../../dlamch/lib/base.js';
import zladiv from '../../zladiv/lib/base.js';
...
const SMLNUM = dlamch( 'safe-minimum' ) / dlamch( 'precision' );
const BIGNUM = ONE / SMLNUM;

// Scratch Complex128Array buffers for zladiv calls:
const ZLADIV_X = new Complex128Array( 1 );
const ZLADIV_Y = new Complex128Array( 1 );
const ZLADIV_OUT = new Complex128Array( 1 );
const ZLADIV_Xv = reinterpret( ZLADIV_X, 0 );
const ZLADIV_Yv = reinterpret( ZLADIV_Y, 0 );
const ZLADIV_OUTv = reinterpret( ZLADIV_OUT, 0 );
		...
		if ( conjugate ) {
			if ( upper ) {
				if ( j > 0 ) {
					...
					dotResult = zdotc( j, AP, strideAP, ( ip / 2 ) - ( j * strideAP ), x, strideX, offsetX );
					csumjR = real( dotResult );
					csumjI = imag( dotResult );
				}
			} else if ( j < N - 1 ) {
				dotResult = zdotc( N - j - 1, AP, strideAP, ( ip / 2 ) + strideAP, x, strideX, offsetX + ( ( j + 1 ) * strideX ) );
				csumjR = real( dotResult );
				csumjI = imag( dotResult );
			}
		} else if ( upper ) {
			if ( j > 0 ) {
				dotResult = zdotu( j, AP, strideAP, ( ip / 2 ) - ( j * strideAP ), x, strideX, offsetX );
				csumjR = real( dotResult );
				csumjI = imag( dotResult );
			}
		} else if ( j < N - 1 ) {
			dotResult = zdotu( N - j - 1, AP, strideAP, ( ip / 2 ) + strideAP, x, strideX, offsetX + ( ( j + 1 ) * strideX ) );
			csumjR = real( dotResult );
			csumjI = imag( dotResult );
		}
	} else if ( upper ) {
...
function zlatps( uplo, trans, diag, normin, N, AP, strideAP, offsetAP, x, strideX, offsetX, scale, CNORM, strideCNORM, offsetCNORM ) {
	let uscal_re, uscal_im, tjjs_re, tjjs_im, jfirst, tscal, jlast, xbnd, xmax;
	let grow, jinc, jlen, rec, tjj, xj, ip, jr, ji, j, i;

	const sap = strideAP * 2;
	const sc = strideCNORM;
	...
	const av = reinterpret( AP, 0 );
	const xv = reinterpret( x, 0 );
	const sx = strideX * 2;
	const ox = offsetX * 2;
	const csumj = [ ZERO, ZERO ];
	...
	if ( normin === 'no' ) {
		if ( upper ) {
			...
			// `ipk` is the PURE packed index (complex elements) of the first
			// off-diagonal element of column j; physical offset scales by strideAP.
			ip = 0;
			for ( j = 0; j < N; j++ ) {
				CNORM[ offsetCNORM + ( j * sc ) ] = dzasum( j, AP, strideAP, offsetAP + ( ip * strideAP ) );
				ip += ( j + 1 );
			}
		} else {
			// Lower packed: column j has N-j-1 elements below diagonal
			ip = 0; // pure packed index of the diagonal of column j
			for ( j = 0; j < N - 1; j++ ) {
				CNORM[ offsetCNORM + ( j * sc ) ] = dzasum( N - j - 1, AP, strideAP, offsetAP + ( ( ip + 1 ) * strideAP ) );
				ip += ( N - j );
			}
	...
	const imax = idamax( N, CNORM, sc, offsetCNORM );
	const tmax = CNORM[ offsetCNORM + ( imax * sc ) ];
	if ( tmax <= BIGNUM * HALF ) {
		tscal = ONE;
	} else {
	...
	if ( notran ) {
		...
		if ( tscal !== ONE ) {
			...
			ip = ( offsetAP + ( ( ( ( jfirst * ( jfirst + 1 ) ) / 2 ) + jfirst ) * strideAP ) ) * 2;
			jlen = N;
					...
					if ( tscal === ONE ) {
						...
						if ( upper ) {
							if ( j > 0 ) {
								...
								zaxpy( j, new Complex128( -xv[ jr ] * tscal, -xv[ ji ] * tscal ), AP, strideAP, ( ip / 2 ) - ( j * strideAP ), x, strideX, offsetX );
								i = izamax( j, x, strideX, offsetX );
								xmax = cabs1( xv, ox + ( i * sx ) );
							}
						} else if ( j < N - 1 ) {
							zaxpy( N - j - 1, new Complex128( -xv[ jr ] * tscal, -xv[ ji ] * tscal ), AP, strideAP, ( ip / 2 ) + strideAP, x, strideX, offsetX + ( ( j + 1 ) * strideX ) );
