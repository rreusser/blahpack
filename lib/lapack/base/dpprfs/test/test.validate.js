/**
* Property-based validation for dpprfs, following the /blahpack-validate process.
*
* Step 0 classification: `d` -> real scalar; `pp` -> SPD PACKED (schemes.packed,
* logical.positiveDefinite); `rfs` (iterative refinement + error bounds) -> three
* independent properties. dpprfs refines an approximate solution X to A*X = B for a
* symmetric-positive-definite A held in PACKED storage, returning FERR (forward-
* error bound) and BERR (componentwise backward error). The packed Cholesky factor
* AFP is produced by the already-validated dpptrf; the un-refined initial X by
* dpptrs; the TRUE solution independently by the trusted dppsv. We assert, against
* the ORIGINAL full symmetric A0:
...
import dpprfs from './../lib/ndarray.js';
import dpptrf from '../../dpptrf/lib/ndarray.js';
import dpptrs from '../../dpptrs/lib/ndarray.js';
...
// Independent TRUE solution: solve A0*X = B0 on fresh copies with trusted dppsv
// (packed AP, dense B).
function trueSolution( A0, B0, N, nrhs, uplo ) {
	...
	var info = dppsv( uplo, N, nrhs, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Br.data, Br.args[ 0 ], Br.args[ 1 ], Br.args[ 2 ] );
	if ( info !== 0 ) {
...
test( 'dpprfs: refinement residual + BERR + FERR bound (uplo x N x nrhs)', function t() {
...
function runProperty( uplo, N, nrhs ) {
	...
	// AP = original SPD (uplo triangle, packed); AFP = its packed Cholesky factor:
	var Ar = schemes.packed.realize( sc, A0, { 'part': uplo }, TIGHTP );
	var AFr = schemes.packed.realize( sc, A0, { 'part': uplo }, TIGHTP );
	dpptrf( uplo, N, AFr.data, AFr.args[ 0 ], AFr.args[ 1 ] );

	// B (RHS, unchanged) and X (initial un-refined solve, refined in place):
	var Br = schemes.dense.realize( sc, B0, { 'part': 'full' }, TIGHT );
	var Xr = schemes.dense.realize( sc, B0, { 'part': 'full' }, TIGHT );
	dpptrs( uplo, N, nrhs, AFr.data, AFr.args[ 0 ], AFr.args[ 1 ], Xr.data, Xr.args[ 0 ], Xr.args[ 1 ], Xr.args[ 2 ] );

	var FERR = new Float64Array( nrhs );
	var BERR = new Float64Array( nrhs );
	var WORK = new Float64Array( 3 * N );
	var IWORK = new Int32Array( N );
	dpprfs( uplo, N, nrhs, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], AFr.data, AFr.args[ 0 ], AFr.args[ 1 ], Br.data, Br.args[ 0 ], Br.args[ 1 ], Br.args[ 2 ], Xr.data, Xr.args[ 0 ], Xr.args[ 1 ], Xr.args[ 2 ], FERR, 1, 0, BERR, 1, 0, WORK, 1, 0, IWORK, 1, 0 );

	var Xtrue = trueSolution( A0, B0, N, nrhs, uplo );

	var tag = 'dpprfs '+uplo+' N='+N+' nrhs='+nrhs;

	// (a) residual: refined X remains a valid solution of A0*X = B0.
	checked( 'dpprfs', 'residual', function run() {
		var j;
		for ( j = 0; j < nrhs; j++ ) {
	...
	checked( 'dpprfs', 'structural', function run() {
		var xcol;
		var tcol;
		var berrCap;
		var eActual;
...
// Step 4: PACKED layout-invariance fuzz. The packed Cholesky factor AFP and the
// initial X are produced ONCE (fixed values), then AP / AFP are re-realized per
// PACKED layout (strides 1/2/3/-1/-2 + offset/pad) together with the fixed A0 and
// the fixed initial X (dense, tight); dpprfs's internal kernels (dspmv, dpptrs,
// daxpy, dlacn2) run over identical values, so ONLY packed ADDRESSING changes and
// the refined X + FERR + BERR must reproduce bit-for-bit. Packed storage is a
// SINGLE addressing family (unlike dense col/row, the packed loops do not switch
// summation form on stride), so all packed layouts must agree exactly — a divergence
// on a non-unit / negative packed stride is a genuine storage-mapping bug.
var packedLayouts = schemes.packed.layouts();

test( 'dpprfs: bit-exact across the packed layout family', function t() {
	UPLOS.forEach( function eachUplo( uplo ) {
		runInvariance( uplo );
	});
});
...
function runInvariance( uplo ) {
	...
	// Factor ONCE and read the fixed packed factor triangle + fixed initial X:
	var AF0 = schemes.packed.realize( sc, A0, { 'part': uplo }, TIGHTP );
	dpptrf( uplo, N, AF0.data, AF0.args[ 0 ], AF0.args[ 1 ] );
	...
	dpptrs( uplo, N, nrhs, AF0.data, AF0.args[ 0 ], AF0.args[ 1 ], X0r.data, X0r.args[ 0 ], X0r.args[ 1 ], X0r.args[ 2 ] );
	var Xinit = readMat( X0r, N, nrhs );

	checked( 'dpprfs', 'layout-invariance', function run() {
		layoutInvariant( packedLayouts, function build( layout ) {
			...
			return out;
		}, { 'label': 'dpprfs '+uplo+' packed layout invariance' } );
	});
}
