...
test( 'dppcon: N=0 quick return', function t() {
	...
	rcond[ 0 ] = 0.5;
	const info = dppcon( 'upper', 0, new Float64Array( 0 ), 1, 0, 0.0, rcond, new Float64Array( 0 ), 1, 0, new Int32Array( 0 ), 1, 0 );
...
test( 'dppcon: anorm=0 returns rcond=0 (upper)', function t() {
	...
	const rcond = new Float64Array( 1 );
	rcond[ 0 ] = 0.5;
	const info = dppcon( 'upper', N, AP, 1, 0, 0.0, rcond, new Float64Array( 3 * N ), 1, 0, new Int32Array( N ), 1, 0 );
	assert.strictEqual( info, 0 );
...
test( 'dppcon: identity 3x3 upper (rcond=1)', function t() {
	...
	const rcond = new Float64Array( 1 );
	const info = dppcon( 'upper', N, AP, 1, 0, anorm, rcond, new Float64Array( 3 * N ), 1, 0, new Int32Array( N ), 1, 0 );
	assert.strictEqual( info, 0 );
...
test( 'dppcon: identity 4x4 lower (rcond=1)', function t() {
	...
	const rcond = new Float64Array( 1 );
	const info = dppcon( 'lower', N, AP, 1, 0, anorm, rcond, new Float64Array( 3 * N ), 1, 0, new Int32Array( N ), 1, 0 );
	assert.strictEqual( info, 0 );
	assert.ok( close( rcond[ 0 ], 1.0, 1e-9 ) );
});
...
test( 'dppcon: N=1 simple', function t() {
	...
	dpptrf( 'upper', N, AP, 1, 0 );
	const rcond = new Float64Array( 1 );
	const info = dppcon( 'upper', N, AP, 1, 0, anorm, rcond, new Float64Array( 3 ), 1, 0, new Int32Array( 1 ), 1, 0 );
	assert.strictEqual( info, 0 );
