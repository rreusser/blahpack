...
function invNorm1( A0, n, uplo ) {
	...
	const info = dsysv( uplo, n, n, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], ipiv, 1, 0, Br.data, Br.args[ 0 ], Br.args[ 1 ], Br.args[ 2 ], work, 1, 0 );
	if ( info !== 0 ) {
...
function drive( uplo, N, nrhs, A0, B0, layout ) {
	...
	const AFr = schemes.dense.realize( sc, A0, { 'part': uplo }, layout ); // overwritten by dlacpy+dsytrf
	const Br = schemes.dense.realize( sc, B0, { 'part': 'full' }, layout );
	const Xr = schemes.dense.realize( sc, B0, { 'part': 'full' }, layout ); // overwritten by dlacpy+dsytrs
	const ipiv = new Int32Array( Math.max( N, 1 ) );
	const rcond = new Float64Array( 1 );
	const FERR = new Float64Array( Math.max( nrhs, 1 ) );
	const BERR = new Float64Array( Math.max( nrhs, 1 ) );
	const WORK = sc.alloc( Math.max( 3 * N, 1 ) ); // NaN-poisoned real scratch
	const IWORK = new Int32Array( Math.max( N, 1 ) );
	const info = dsysvx( 'not-factored', uplo, N, nrhs, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], AFr.data, AFr.args[ 0 ], AFr.args[ 1 ], AFr.args[ 2 ], ipiv, 1, 0, Br.data, Br.args[ 0 ], Br.args[ 1 ], Br.args[ 2 ], Xr.data, Xr.args[ 0 ], Xr.args[ 1 ], Xr.args[ 2 ], rcond, FERR, 1, 0, BERR, 1, 0, WORK, 1, 0, IWORK, 1, 0 );
	return {
		'Xr': Xr,
		'rcond': rcond,
		'FERR': FERR,
...
function runResidual( uplo, N, nrhs, layout ) {
	...
	const tag = 'dsysvx '+uplo+' N='+N+' nrhs='+nrhs;
