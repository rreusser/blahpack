...
import zsytrs3 from './../lib/ndarray.js';


// FIXTURES //

const fixtureDir = path.join( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures' ); // eslint-disable-line max-len
const lines = readFileSync( path.join( fixtureDir, 'zsytrs_3.jsonl' ), 'utf8' ).trim().split( '\n' ); // eslint-disable-line node/no-sync, max-len
...
function runFixture( uplo, tc ) {
	...
	const info = zsytrs3( uplo, n, nrhs, A, 1, NMAX, 0, e, 1, 0, ipiv, 1, 0, B, 1, NMAX, 0 ); // eslint-disable-line max-len
...
test( 'zsytrs3: N=0 quick return', function t() {
	...
	const info = zsytrs3( 'upper', 0, 1, A, 1, 1, 0, e, 1, 0, ipiv, 1, 0, B, 1, 1, 0 );
...
test( 'zsytrs3: NRHS=0 quick return', function t() {
	...
	const info = zsytrs3( 'upper', 3, 0, A, 1, 3, 0, e, 1, 0, ipiv, 1, 0, B, 1, 3, 0 );
	assert.equal( info, 0, 'info' );
});
