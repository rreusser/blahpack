...
function zptcon( N, d, strideD, offsetD, e, strideE, offsetE, anorm, rcond, RWORK, strideRWork, offsetRWork ) {
	let absEi, pr, pd, ie, er, ei, i;

	const sr = strideRWork;
	const sd = strideD;
	const se = strideE * 2; // complex stride → double stride
	...
	const ix = idamax( N, RWORK, sr, offsetRWork );
	const ainvnm = Math.abs( RWORK[ offsetRWork + ( ix * sr ) ] );
