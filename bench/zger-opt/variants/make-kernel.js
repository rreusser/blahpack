...
/**
* Returns a register-blocked complex rank-1 update kernel.
*
* ## Method
*
* Complex analog of the shipped `dger` blocked kernel (`bench/dger-opt/`). The
* kernel picks whichever traversal walks `A`'s smaller-stride dimension in the
* inner loop and register-blocks the other dimension four wide:
*
* -   **column form** (four columns per pass, hoisted `temp = alpha*y[j+k]`)
*     when the first dimension has the smaller stride;
* -   **row form** (four rows per pass, hoisted `x[i+k]`) otherwise.
*
* In every branch each `A[i,j]` receives exactly the reference fused update
* `x[i] * (alpha*conj?(y[j]))` with the identical floating-point expression
* `re += xr*tr - xi*ti; im += xr*ti + xi*tr`, and the reference
* `y[j] !== 0` column guard is preserved. No dependent arithmetic is
* reordered, so the kernel is verified **bit-identically** against the
* reference (`bench/zger-opt/check.mjs`).
*
* The `conj` flag (zgerc conjugates `y`, zgeru does not) is resolved once
* here, outside all loops, by branching only where `temp` is formed — never
* in the hot inner loop.
*
* @private
* @param {boolean} conj - if `true`, conjugate `y` (zgerc); else zgeru
* @returns {Function} rank-1 update kernel
*/
function makeZger( conj ) {
	/**
	* Perform the complex rank-1 update `A := alpha*x*y**T + A` (zgeru) or
	* `A := alpha*x*y**H + A` (zgerc).
	*
	* @private
	...
	*/
	return function zger( M, N, alpha, x, strideX, offsetX, y, strideY, offsetY, A, strideA1, strideA2, offsetA ) {
		var alphaRe;
		var alphaIm;
		var sa1;
		var sa2;
