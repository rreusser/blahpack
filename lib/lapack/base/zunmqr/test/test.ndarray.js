...
import left_notrans_4x4 from './fixtures/left_notrans_4x4.json' with { type: 'json' };
import left_conjtrans_4x4 from './fixtures/left_conjtrans_4x4.json' with { type: 'json' };
import right_notrans_4x4 from './fixtures/right_notrans_4x4.json' with { type: 'json' };
...
test( 'zunmqr: left, no transpose (Q*I)', function t() {
	const tc = left_notrans_4x4;
	const qr = qr4x3();
	const LDC = 6;
	const C = eye4in6();
	const WORK = new Complex128Array( 200 );
	const info = zunmqr('left', 'no-transpose', 4, 4, 3, qr.A, 1, qr.LDA, 0, qr.TAU, 1, 0, C, 1, LDC, 0, WORK, 1, 0 );
...
test( 'zunmqr: left, conjugate transpose (Q^H*I)', function t() {
	const tc = left_conjtrans_4x4;
	const qr = qr4x3();
	const LDC = 6;
	const C = eye4in6();
	const WORK = new Complex128Array( 200 );
	const info = zunmqr('left', 'conjugate-transpose', 4, 4, 3, qr.A, 1, qr.LDA, 0, qr.TAU, 1, 0, C, 1, LDC, 0, WORK, 1, 0 );
	assertClose( info, tc.info, 1e-14, 'info' );
	const Cv = reinterpret( C, 0 );
	assertArrayClose( Array.from( Cv.subarray( 0, tc.c.length ) ), tc.c, 1e-10, 'c' );
});
...
test( 'zunmqr: right, no transpose (I*Q)', function t() {
	const tc = right_notrans_4x4;
	const qr = qr4x3();
	const LDC = 6;
	const C = eye4in6();
	const WORK = new Complex128Array( 200 );
	const info = zunmqr('right', 'no-transpose', 4, 4, 3, qr.A, 1, qr.LDA, 0, qr.TAU, 1, 0, C, 1, LDC, 0, WORK, 1, 0 );
	assertClose( info, tc.info, 1e-14, 'info' );
	const Cv = reinterpret( C, 0 );
	assertArrayClose( Array.from( Cv.subarray( 0, tc.c.length ) ), tc.c, 1e-10, 'c' );
});
...
test( 'zunmqr: M=0 quick return', function t() {
	...
	const WORK = new Complex128Array( 20 );
	const info = zunmqr('left', 'no-transpose', 0, 4, 0, A, 1, 1, 0, TAU, 1, 0, C, 1, 1, 0, WORK, 1, 0 );
	assertClose( info, tc.info, 1e-14, 'info' );
});
...
test( 'zunmqr: N=0 quick return', function t() {
	...
	const C = new Complex128Array( 5 );
	const WORK = new Complex128Array( 20 );
	const info = zunmqr('left', 'no-transpose', 4, 0, 0, A, 1, 4, 0, TAU, 1, 0, C, 1, 4, 0, WORK, 1, 0 );
	assertClose( info, tc.info, 1e-14, 'info' );
});
...
test( 'zunmqr: K=0 quick return', function t() {
	...
	const C = new Complex128Array( 5 );
	const WORK = new Complex128Array( 20 );
	const info = zunmqr('left', 'no-transpose', 4, 4, 0, A, 1, 4, 0, TAU, 1, 0, C, 1, 4, 0, WORK, 1, 0 );
	assertClose( info, tc.info, 1e-14, 'info' );
});
...
test( 'zunmqr: left, no transpose, rectangular C (4x2)', function t() {
	...
	const WORK = new Complex128Array( 200 );
	const info = zunmqr('left', 'no-transpose', 4, 2, 3, qr.A, 1, qr.LDA, 0, qr.TAU, 1, 0, C, 1, LDC, 0, WORK, 1, 0 );
	assertClose( info, tc.info, 1e-14, 'info' );
	assertArrayClose( Array.from( Cv.subarray( 0, tc.c.length ) ), tc.c, 1e-10, 'c' );
});
...
function qrLarge( M, K ) {
	const LDA = M;
	const A = deterministicMatrix( M, K, LDA, 54321 );
...
function eyeComplex( M ) {
	const C = new Complex128Array( M * M );
	const Cv = reinterpret( C, 0 );
	let i;
	for ( i = 0; i < M; i++ ) {
		Cv[ 2 * ( i + i * M ) ] = 1.0;
...
test( 'zunmqr: blocked path, left, conjugate transpose (K=35, backward iteration)', function t() {
	...
	const C = eyeComplex( M );
	let info = zunmqr('left', 'conjugate-transpose', M, N, K, qr.A, 1, qr.LDA, 0, qr.TAU, 1, 0, C, 1, LDC, 0, WORK, 1, 0 );
...
test( 'zunmqr: blocked path, right, no transpose (K=35, covers side=R blocked)', function t() {
	...
	const M = N;
	const LDC = M;
	const qr = qrLarge( N, K );
	const WORK = new Complex128Array( M * 64 );

	// Step 1: C = I, apply Q from right (I*Q)
	const C = eyeComplex( M );
	let info = zunmqr('right', 'no-transpose', M, N, K, qr.A, 1, qr.LDA, 0, qr.TAU, 1, 0, C, 1, LDC, 0, WORK, 1, 0 );
...
test( 'zunmqr: blocked path, right, conjugate transpose (K=35, covers side=R blocked backward)', function t() {
	...
	const C = eyeComplex( M );
	let info = zunmqr('right', 'conjugate-transpose', M, N, K, qr.A, 1, qr.LDA, 0, qr.TAU, 1, 0, C, 1, LDC, 0, WORK, 1, 0 );
