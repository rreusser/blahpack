...
import dlaqp2 from './../lib/ndarray.js';

// FIXTURES //

import basic_4x3 from './fixtures/basic_4x3.json' with { type: 'json' };
...
import pivot_reorder from './fixtures/pivot_reorder.json' with { type: 'json' };
import collinear_recomp from './fixtures/collinear_recomp.json' with { type: 'json' };
...
function computeNorms( A, M, N, LDA, startRow ) {
	const VN1 = new Float64Array( N );
	const VN2 = new Float64Array( N );
	let sum, val, j, i;
	for ( j = 0; j < N; j++ ) {
		sum = 0.0;
...
test( 'dlaqp2: basic_4x3', function t() {
	...
	const JPVT = new Int32Array( [ 0, 1, 2 ] );
	const TAU = new Float64Array( 3 );
	const WORK = new Float64Array( 3 );
	const norms = computeNorms( A, 4, 3, 4, 0 );
...
test( 'dlaqp2: square_3x3', function t() {
	...
	const TAU = new Float64Array( 3 );
	const WORK = new Float64Array( 3 );
	const norms = computeNorms( A, 3, 3, 3, 0 );
...
test( 'dlaqp2: n_one', function t() {
	...
	const norms = computeNorms( A, 3, 1, 3, 0 );
...
test( 'dlaqp2: one_by_one', function t() {
	...
	const TAU = new Float64Array( 1 );
	const WORK = new Float64Array( 1 );
	const VN1 = new Float64Array( [ 7.0 ] );
	const VN2 = new Float64Array( [ 7.0 ] );
...
test( 'dlaqp2: n_zero', function t() {
	...
	const TAU = new Float64Array( 1 );
	const WORK = new Float64Array( 1 );
	const VN1 = new Float64Array( 1 );
	const VN2 = new Float64Array( 1 );
...
test( 'dlaqp2: offset_1', function t() {
	...
	const TAU = new Float64Array( 3 );
	const WORK = new Float64Array( 3 );
	const norms = computeNorms( A, 4, 3, 4, 1 );
...
test( 'dlaqp2: collinear_recomp', function t() {
	...
	const TAU = new Float64Array( 3 );
	const WORK = new Float64Array( 3 );
	const norms = computeNorms( A, 6, 3, 6, 0 );
