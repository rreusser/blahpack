...
test( 'dsytri_rook quick return for N=0', function t() {
	const info = dsytriRook( 'column-major', 'upper', 0, new Float64Array( 0 ), 1, new Int32Array( 0 ), 1, new Float64Array( 0 ) );
...
test( 'dsytri_rook 1x1 row-major', function t() {
	...
	const ipiv = new Int32Array( [ 0 ] );
	const work = new Float64Array( 1 );
	const info = dsytriRook( 'row-major', 'upper', 1, A, 1, ipiv, 1, work );
	assert.equal( info, 0, 'info' );
	assert.equal( A[ 0 ], 0.25, 'A[0]' );
});
