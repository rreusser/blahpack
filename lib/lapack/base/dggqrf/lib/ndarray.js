...
*/
function dggqrf( N, M, p, A, strideA1, strideA2, offsetA, TAUA, strideTAUA, offsetTAUA, B, strideB1, strideB2, offsetB, TAUB, strideTAUB, offsetTAUB, WORK, strideWork, offsetWork ) { // eslint-disable-line max-len, max-params
	if ( N < 0 ) {
	...
	// Blocked minimum: this driver forwards ONE shared WORK to blocked dgeqrf(N,M),
	// dormqr('L',N,P,kA), and dgerqf(N,P), each of which (NB=32) stores a trailing
	...
	var minWork = Math.max( 1, N, M, p,
		( Math.min( N, M ) > 32 ) ? ( ( M * 32 ) + ( 32 * 32 ) ) : M, // dgeqrf(N,M)
		( Math.min( N, M ) > 32 ) ? ( ( p * 32 ) + ( 33 * 32 ) ) : p, // dormqr('L',N,P,kA)
		( Math.min( N, p ) > 32 ) ? ( ( N * 32 ) + ( 32 * 32 ) ) : N  // dgerqf(N,P)
	);
