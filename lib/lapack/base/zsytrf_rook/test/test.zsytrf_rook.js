...
import zsytrfRook from './../lib/zsytrf_rook.js';
...
test( 'zsytrf_rook has expected arity', function t() {
	assert.strictEqual( zsytrfRook.length, 9, 'has expected arity' );
});
...
test( 'zsytrf_rook returns 0 for N=0', function t() {
	const info = zsytrfRook( 'column-major', 'upper', 0, new Complex128Array( 1 ), 1, new Int32Array( 1 ), 1 ); // eslint-disable-line max-len
	assert.strictEqual( info, 0, 'info is 0' );
});

test( 'zsytrf_rook column-major basic 2x2', function t() {
	const IPIV = new Int32Array( 2 );
	const A = new Complex128Array( [ 4.0, 0.0, 1.0, 0.0, 1.0, 0.0, 3.0, 0.0 ] );
	const info = zsytrfRook( 'column-major', 'lower', 2, A, 2, IPIV, 1, null, 1 );
...
test( 'zsytrf_rook row-major basic 2x2 (transposes via swapped strides)', function t() {
	const IPIV = new Int32Array( 2 );
	const A = new Complex128Array( [ 4.0, 0.0, 1.0, 0.0, 1.0, 0.0, 3.0, 0.0 ] );
	const info = zsytrfRook( 'row-major', 'lower', 2, A, 2, IPIV, 1, null, 1 );
	assert.strictEqual( info, 0, 'info' );
});
