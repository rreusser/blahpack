...
import zlanhb from './../lib/ndarray.js';


// FIXTURES //

const fixtureFile = path.join( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures', 'zlanhb.jsonl' );
...
function buildAB( ldab, ncols, cells ) {
	const buf = new Float64Array( 2 * ldab * ncols );
	let i, idx;
...
const AB_UPPER_K2 = buildAB( LDAB, 5, [
	3, 1, 1.0, 0.0,
...
// Lower, K=2, N=5
const AB_LOWER_K2 = buildAB( LDAB, 5, [
...
// 1x1 K=0 (real diag = 5)
const AB_1X1 = buildAB( LDAB, 1, [ 1, 1, 5.0, 0.0 ] );

// K=0 diagonal-only, N=4
const AB_K0_DIAG = buildAB( LDAB, 4, [
	1, 1, 3.0, 0.0,
	1, 2, -7.0, 0.0,
	1, 3, 2.0, 0.0,
	1, 4, -4.0, 0.0
...
// K=1, N=4 upper
const AB_UPPER_K1 = buildAB( LDAB, 4, [
	2, 1, 2.0, 0.0,
...
// K=1, N=4 lower
const AB_LOWER_K1 = buildAB( LDAB, 4, [
...
function runCase( name, norm, uplo, N, K, AB ) {
	...
	const got = zlanhb( norm, uplo, N, K, AB, 1, LDAB, 0, WORK, 1, 0 );
...
test( 'zlanhb: invalid norm returns 0 from base', function t() {
	...
	const got = zlanhb( 'invalid', 'upper', 4, 1, AB_UPPER_K1, 1, LDAB, 0, WORK, 1, 0 );
