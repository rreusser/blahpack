...
import dsytrfAa from './../lib/ndarray.js';


// FIXTURES //

const FIXTURE_PATH = path.resolve( import.meta.dirname, '../../../../../test/fixtures/dsytrf_aa.jsonl' );
...
test( 'dsytrf_aa: 4x4 lower (positive definite)', function t() {
	...
	const info = dsytrfAa( 'lower', 4, A, 1, 4, 0, ipiv, 1, 0, new Float64Array( 1320 ), 1, 0 );
...
test( 'dsytrf_aa: 4x4 upper (positive definite)', function t() {
	...
	A[ 15 ] = 8;
	const ipiv = new Int32Array( 4 );
	const info = dsytrfAa( 'upper', 4, A, 1, 4, 0, ipiv, 1, 0, new Float64Array( 1320 ), 1, 0 );
	assert.equal( info, tc.info, 'info' );
	assertArrayClose( A, tc.a, 1e-13, 'a' );
	assert.deepEqual( toArray( ipiv ), convertIPIV( tc.ipiv ), 'ipiv' );
});
...
test( 'dsytrf_aa: N=0 quick return', function t() {
	const info = dsytrfAa( 'lower', 0, new Float64Array( 0 ), 1, 1, 0, new Int32Array( 0 ), 1, 0, new Float64Array( 1 ), 1, 0 );
...
test( 'dsytrf_aa: N=1 quick return', function t() {
	...
	const info = dsytrfAa( 'lower', 1, A, 1, 1, 0, ipiv, 1, 0, new Float64Array( 1320 ), 1, 0 );
	assert.equal( info, tc.info, 'info' );
	assertArrayClose( A, tc.a, 1e-13, 'a' );
	assert.deepEqual( toArray( ipiv ), convertIPIV( tc.ipiv ), 'ipiv' );
});
...
test( 'dsytrf_aa: 5x5 lower', function t() {
	...
	const info = dsytrfAa( 'lower', 5, A, 1, 5, 0, ipiv, 1, 0, new Float64Array( 1320 ), 1, 0 );
	assert.equal( info, tc.info, 'info' );
...
test( 'dsytrf_aa: 5x5 upper', function t() {
	...
	A[ 24 ] = 4;
	const ipiv = new Int32Array( 5 );
	const info = dsytrfAa( 'upper', 5, A, 1, 5, 0, ipiv, 1, 0, new Float64Array( 1320 ), 1, 0 );
	assert.equal( info, tc.info, 'info' );
	assertArrayClose( A, tc.a, 1e-12, 'a' );
	assert.deepEqual( toArray( ipiv ), convertIPIV( tc.ipiv ), 'ipiv' );
});
...
test( 'dsytrf_aa: 40x40 lower (exercise blocked path)', function t() {
	...
	const info = dsytrfAa( 'lower', 40, A, 1, 40, 0, ipiv, 1, 0, new Float64Array( 1320 ), 1, 0 );
	assert.equal( info, tc.info, 'info' );
...
test( 'dsytrf_aa: 40x40 upper (exercise blocked path)', function t() {
	...
	const ipiv = new Int32Array( 40 );
	const info = dsytrfAa( 'upper', 40, A, 1, 40, 0, ipiv, 1, 0, new Float64Array( 1320 ), 1, 0 );
	assert.equal( info, tc.info, 'info' );
	assertArrayClose( A, tc.a, 1e-10, 'a' );
});
...
test( 'dsytrf_aa: ndarray non-trivial offset (lower)', function t() {
	...
	const pad = 3;
	const tc = FIXTURES[ '5x5_lower' ];
	const A = new Float64Array( 25 + pad );
	A[ 0 + pad ] = 1;
	A[ 1 + pad ] = -2;
	A[ 2 + pad ] = 0;
	A[ 3 + pad ] = 3;
	...
	const info = dsytrfAa( 'lower', 5, A, 1, 5, pad, ipiv, 1, 1, new Float64Array( 1320 ), 1, 0 );
	assert.equal( info, tc.info, 'info' );
	for ( k = 0; k < 25; k++ ) {
...
test( 'dsytrf_aa: row-major upper layout produces finite output', function t() {
	...
	A[ 24 ] = 4;
	const ipiv = new Int32Array( 5 );
	const info = dsytrfAa( 'upper', 5, A, 5, 1, 0, ipiv, 1, 0, new Float64Array( 1320 ), 1, 0 );
	assert.equal( info, 0, 'info' );
