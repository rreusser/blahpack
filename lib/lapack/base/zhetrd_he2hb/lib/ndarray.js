...
function zhetrd_he2hb( uplo, N, kd, A, strideA1, strideA2, offsetA, AB, strideAB1, strideAB2, offsetAB, TAU, strideTAU, offsetTAU, WORK, strideWork, offsetWork ) {
	...
	// so an under-sized buffer is a loud RangeError, not a silent NaN from an
	// out-of-bounds read. A `null` WORK triggers internal allocation, so only a
	// caller-provided buffer is checked, and only on the reduction path (N>kd+1;
	// the N===0 and N<=kd+1 cases are quick returns that need no workspace). With
	// FACTOPTNB=32, the minimum size is `N*kd + N*max(kd,32) + 2*kd*kd`.
	if ( WORK && N > kd+1 ) {
		need = ( N*kd ) + ( N * Math.max( kd, 32 ) ) + ( 2*kd*kd );
		if ( ( WORK.length - offsetWork ) < need ) {
			throw new RangeError( format( 'invalid argument. WORK array must have at least %d elements from offset %d. Provided length: %d.', need, offsetWork, WORK.length ) );
		}
	}
