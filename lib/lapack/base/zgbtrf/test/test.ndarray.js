...
test( 'zgbtrf: 4x4 tridiagonal (KL=1, KU=1)', function t() {
	...
	const AB = new Complex128Array( 4 * 4 );
	...
	const info = zgbtrf( 4, 4, 1, 1, AB, 1, 4, 0, IPIV, 1, 0 );
...
test( 'zgbtrf: 3x3 with KL=1, KU=2', function t() {
	...
	const AB = new Complex128Array( 5 * 3 );
	const ABv = reinterpret( AB, 0 );
	...
	const info = zgbtrf( 3, 3, 1, 2, AB, 1, 5, 0, IPIV, 1, 0 );
	assert.strictEqual( info, tc.info );
	assertArrayClose( Array.from( ABv ), tc.AB, 1e-10, 'AB' );
	let i;
	for ( i = 0; i < tc.ipiv.length; i++ ) {
...
test( 'zgbtrf: N=0 quick return', function t() {
	...
	const info = zgbtrf( 3, 0, 1, 1, AB, 1, 4, 0, IPIV, 1, 0 );
	assert.strictEqual( info, tc.info );
});
...
test( 'zgbtrf: M=0 quick return', function t() {
	...
	const AB = new Complex128Array( 4 );
	const IPIV = new Int32Array( 1 );
	const info = zgbtrf( 0, 3, 1, 1, AB, 1, 4, 0, IPIV, 1, 0 );
	assert.strictEqual( info, tc.info );
});

test( 'zgbtrf: 1x1 matrix', function t() {
	const tc = one_by_one;
	const AB = new Complex128Array( [ 7, 2 ] );
	const IPIV = new Int32Array( 1 );
	const info = zgbtrf( 1, 1, 0, 0, AB, 1, 1, 0, IPIV, 1, 0 );
	assert.strictEqual( info, tc.info );
...
test( 'zgbtrf: singular matrix', function t() {
	const tc = singular;
	const AB = new Complex128Array( 2 * 2 );
	const ABv = reinterpret( AB, 0 );
	ABv[ (0 + 1 * 2) * 2 ] = 1; ABv[ (0 + 1 * 2) * 2 + 1 ] = 1;
	const IPIV = new Int32Array( 2 );
	const info = zgbtrf( 2, 2, 0, 1, AB, 1, 2, 0, IPIV, 1, 0 );
	assert.strictEqual( info, tc.info );
});

test( 'zgbtrf: pivoting 2x2', function t() {
	const tc = pivot_2x2;
	const AB = new Complex128Array( 4 * 2 );
	const ABv = reinterpret( AB, 0 );
	...
	const IPIV = new Int32Array( 2 );
	const info = zgbtrf( 2, 2, 1, 1, AB, 1, 4, 0, IPIV, 1, 0 );
	assert.strictEqual( info, tc.info );
	assertArrayClose( Array.from( ABv ), tc.AB, 1e-10, 'AB' );
	let i;
	for ( i = 0; i < tc.ipiv.length; i++ ) {
...
test( 'zgbtrf: 5x5 pentadiagonal (KL=2, KU=2)', function t() {
	...
	const ku = 2;
	const kv = ku + kl; // = 4
	const AB = new Complex128Array( LDAB * 5 );
	const ABv = reinterpret( AB, 0 );
	...
	const info = zgbtrf( 5, 5, kl, ku, AB, 1, LDAB, 0, IPIV, 1, 0 );
	assert.strictEqual( info, tc.info );
	assertArrayClose( Array.from( ABv ), tc.AB, 1e-10, 'AB' );
	let i;
	for ( i = 0; i < tc.ipiv.length; i++ ) {
...
test( 'zgbtrf: blocked path 100x100 with KL=33 KU=33', function t() {
	...
	const AB_orig = new Complex128Array( LDAB * N );
	const AB_origv = reinterpret( AB_orig, 0 );
	const AB = new Complex128Array( LDAB * N );
	const ABv = reinterpret( AB, 0 );
	const IPIV = new Int32Array( N );
	const b = new Complex128Array( N );
	const bv = reinterpret( b, 0 );
	let i, j, re, im, resid, bnorm, Are, Aim;
	...
	const info = zgbtrf( N, N, kl, ku, AB, 1, LDAB, 0, IPIV, 1, 0 );
	assert.strictEqual( info, tc.info, 'info should match reference' );

	// Compare the factorization itself against the Fortran reference output.
	// The solve residual below is too forgiving on its own: this matrix is
	// strongly diagonally dominant and the residual is normalized by
