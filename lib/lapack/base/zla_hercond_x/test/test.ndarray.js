...
import zla_hercond_x from './../lib/ndarray.js';


// FIXTURES //

const fixtureDir = path.join( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures' );
const lines = readFileSync( path.join( fixtureDir, 'zla_hercond_x.jsonl' ), 'utf8' ).trim().split( '\n' ); // eslint-disable-line node/no-sync
...
function runCase( tc, uplo, N, tol, label ) {
	...
	const IPIV = toIPIV( tc.ipiv );
	const X = toComplex( tc.x );
	const WORK = new Complex128Array( 2 * N );
	const RWORK = new Float64Array( N );
	const result = zla_hercond_x( uplo, N, A, 1, N, 0, AF, 1, N, 0, IPIV, 1, 0, X, 1, 0, WORK, 1, 0, RWORK, 1, 0 );
