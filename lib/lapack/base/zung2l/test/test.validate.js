...
/**
* Property-based validation for zung2l, following the /blahpack-validate process.
*
* Step 0 classification: `z` -> complex scalar; `un`/`g2l` -> UNBLOCKED formation of
* the orthonormal factor Q (M x N economy, N <= M) from the Householder reflectors
* that `zgeqlf`/`dgeql2` leave in the LAST K columns of A (with tau in TAU). zung2l
...
*       every (M,N).
*
* zung2l is the UNBLOCKED kernel (dlarf -> dgemv/dger); the storage col<->row flip
...
const BLOCKED = false; // zung2l is the UNBLOCKED kernel (dlarf only)
const LogicalMatrix = logical.LogicalMatrix;
const NB = 32; // geqlf hardcoded block size (governs the geqlf WORK length only)
const ROUTINE = 'zung2l';
...
test( ROUTINE+': QᴴQ = I and A = Q*L ((M,N) sweep x all layouts)', function t() {
	PAIRS.forEach( function eachPair( pr ) {
		...
		ALL_LAYOUTS.forEach( function eachLayout( layout, li ) {
			...
			zung2l( M, N, K, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], Tr.data, Tr.args[ 0 ], Tr.args[ 1 ], Wo.data, Wo.args[ 0 ], Wo.args[ 1 ] );
...
function runInvariance( variants, fam ) {
	...
	checked( ROUTINE, 'layout-invariance', function run() {
		layoutInvariant( variants, function build( layout, i ) {
			...
			zung2l( MI, NI, K, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], Tr.data, Tr.args[ 0 ], Tr.args[ 1 ], Wo.data, Wo.args[ 0 ], Wo.args[ 1 ] );
...
// zung2l advertises and genuinely uses WORK length N (dlarf's dgemv/dger scratch).
...
test( ROUTINE+': advertised WORK minimum suffices (Step 4c)', function t() {
	...
	function run( len ) {
		...
		zung2l( M, N, K, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], Tr.data, Tr.args[ 0 ], Tr.args[ 1 ], Wo, 1, 0 );
