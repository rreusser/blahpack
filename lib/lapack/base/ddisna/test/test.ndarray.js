...
import ddisna from './../lib/ndarray.js';
...
const OVERFLOW = 1.7976931348623157e+308;
...
test( 'ddisna returns 0 for valid input', function t() {
	...
	const d = new Float64Array( [ 1.0, 2.0, 4.0 ] );
	const info = ddisna( 'eigenvalues', 3, 0, d, 1, 0, sep, 1, 0 );
...
test( 'ddisna returns -2 for negative M', function t() {
	...
	const info = ddisna( 'eigenvalues', -1, 0, d, 1, 0, sep, 1, 0 );
...
test( 'ddisna returns -3 for negative N (singular values)', function t() {
	const sep = new Float64Array( 1 );
	const d = new Float64Array( [ 1.0 ] );
	const info = ddisna( 'left-vectors', 3, -1, d, 1, 0, sep, 1, 0 );
	assert.strictEqual( info, -3 );
});

test( 'ddisna returns -4 for unsorted eigenvalues', function t() {
	const sep = new Float64Array( 3 );
	const d = new Float64Array( [ 1.0, 3.0, 2.0 ] );
	const info = ddisna( 'eigenvalues', 3, 0, d, 1, 0, sep, 1, 0 );
...
test( 'ddisna returns 0 for k=0 (quick return)', function t() {
	...
	const info = ddisna( 'eigenvalues', 0, 0, d, 1, 0, sep, 1, 0 );
	assert.strictEqual( info, 0 );
});
...
test( 'ddisna: two eigenvalues (increasing)', function t() {
	...
	const d = new Float64Array( [ 1.0, 3.0 ] );
	const info = ddisna( 'eigenvalues', 2, 0, d, 1, 0, sep, 1, 0 );
	assert.strictEqual( info, 0 );
...
test( 'ddisna: three eigenvalues (increasing)', function t() {
	...
	// sep[2] = 3
	const sep = new Float64Array( 3 );
	const d = new Float64Array( [ 1.0, 2.0, 5.0 ] );
	const info = ddisna( 'eigenvalues', 3, 0, d, 1, 0, sep, 1, 0 );
	assert.strictEqual( info, 0 );
...
test( 'ddisna: three eigenvalues (decreasing)', function t() {
	...
	// sep[2] = 1
	const sep = new Float64Array( 3 );
	const d = new Float64Array( [ 5.0, 2.0, 1.0 ] );
	const info = ddisna( 'eigenvalues', 3, 0, d, 1, 0, sep, 1, 0 );
	assert.strictEqual( info, 0 );
...
test( 'ddisna: four eigenvalues', function t() {
	...
	// sep[2] = min(3, 4) = 3
	// sep[3] = 4 (last gap)
	const sep = new Float64Array( 4 );
	const d = new Float64Array( [ 1.0, 3.0, 6.0, 10.0 ] );
	const info = ddisna( 'eigenvalues', 4, 0, d, 1, 0, sep, 1, 0 );
	assert.strictEqual( info, 0 );
	assert.strictEqual( sep[ 0 ], 2.0 );
	assert.strictEqual( sep[ 1 ], 2.0 );
	assert.strictEqual( sep[ 2 ], 3.0 );
...
test( 'ddisna: left singular vectors (M > N, increasing)', function t() {
	...
	const info = ddisna( 'left-vectors', 5, 3, d, 1, 0, sep, 1, 0 );
	assert.strictEqual( info, 0 );
	assert.strictEqual( sep[ 0 ], 1.0 );
	assert.strictEqual( sep[ 1 ], 2.0 );
...
test( 'ddisna: right singular vectors (M < N, increasing)', function t() {
	...
	const sep = new Float64Array( 3 );
	const d = new Float64Array( [ 1.0, 3.0, 7.0 ] );
	const info = ddisna( 'right-vectors', 3, 5, d, 1, 0, sep, 1, 0 );
	assert.strictEqual( info, 0 );
	assert.strictEqual( sep[ 0 ], 1.0 );
	assert.strictEqual( sep[ 1 ], 2.0 );
	assert.strictEqual( sep[ 2 ], 4.0 );
...
test( 'ddisna: left singular vectors (M > N, decreasing)', function t() {
	...
	const sep = new Float64Array( 3 );
	const d = new Float64Array( [ 7.0, 3.0, 1.0 ] );
	const info = ddisna( 'left-vectors', 5, 3, d, 1, 0, sep, 1, 0 );
	assert.strictEqual( info, 0 );
...
test( 'ddisna: right singular vectors (M > N, no extra adjustment)', function t() {
	...
	const sep = new Float64Array( 3 );
	const d = new Float64Array( [ 1.0, 3.0, 7.0 ] );
	const info = ddisna( 'right-vectors', 5, 3, d, 1, 0, sep, 1, 0 );
	assert.strictEqual( info, 0 );
	assert.strictEqual( sep[ 0 ], 2.0 );
	assert.strictEqual( sep[ 1 ], 2.0 );
	assert.strictEqual( sep[ 2 ], 4.0 );
...
test( 'ddisna: threshold clamping for small gaps', function t() {
	...
	// sep[2] = 1
	const thresh = Math.max( EPS * 2.0, SAFMIN );
	const sep = new Float64Array( 3 );
	const d = new Float64Array( [ 1.0, 1.0, 2.0 ] );
	const info = ddisna( 'eigenvalues', 3, 0, d, 1, 0, sep, 1, 0 );
	assert.strictEqual( info, 0 );
...
test( 'ddisna: stride support for d', function t() {
	...
	const sep = new Float64Array( 3 );
	const d = new Float64Array( [ 1.0, 99.0, 3.0, 99.0, 6.0 ] );
	const info = ddisna( 'eigenvalues', 3, 0, d, 2, 0, sep, 1, 0 );
	assert.strictEqual( info, 0 );
	assert.strictEqual( sep[ 0 ], 2.0 );
	assert.strictEqual( sep[ 1 ], 2.0 );
	assert.strictEqual( sep[ 2 ], 3.0 );
...
test( 'ddisna: stride support for SEP', function t() {
	...
	const d = new Float64Array( [ 1.0, 3.0, 6.0 ] );
	const info = ddisna( 'eigenvalues', 3, 0, d, 1, 0, sep, 2, 0 );
	assert.strictEqual( info, 0 );
	assert.strictEqual( sep[ 0 ], 2.0 );
...
test( 'ddisna: offset support', function t() {
	...
	const sep = new Float64Array( 4 );
	const d = new Float64Array( [ 99.0, 99.0, 1.0, 4.0, 8.0 ] );
	const info = ddisna( 'eigenvalues', 3, 0, d, 1, 2, sep, 1, 1 );
	assert.strictEqual( info, 0 );
...
test( 'ddisna: negative singular values return -4', function t() {
	...
	const info = ddisna( 'left-vectors', 3, 2, d, 1, 0, sep, 1, 0 );
	assert.strictEqual( info, -4 );
});
...
test( 'ddisna: singular values with M=N (no extra adjustment)', function t() {
	...
	const sep = new Float64Array( 3 );
	const d = new Float64Array( [ 1.0, 3.0, 7.0 ] );
	const info = ddisna( 'left-vectors', 3, 3, d, 1, 0, sep, 1, 0 );
	assert.strictEqual( info, 0 );
	assert.strictEqual( sep[ 0 ], 2.0 );
	assert.strictEqual( sep[ 1 ], 2.0 );
	assert.strictEqual( sep[ 2 ], 4.0 );
