...
test( 'dgeqr2p has expected arity', function t() {
	assert.strictEqual( dgeqr2p.length, 8, 'has expected arity' );
});

test( 'dgeqr2p throws TypeError for invalid order', function t() {
	assert.throws( function throws() {
		dgeqr2p( 'invalid', 2, 2, new Float64Array( 4 ), 2, new Float64Array( 4 ), 2, new Float64Array( 4 ) );
	}, TypeError );
});

test( 'dgeqr2p throws RangeError for negative M', function t() {
	assert.throws( function throws() {
		dgeqr2p( 'row-major', -1, 2, new Float64Array( 4 ), 2, new Float64Array( 4 ), 2, new Float64Array( 4 ) );
	}, RangeError );
});

test( 'dgeqr2p throws RangeError for negative N', function t() {
	assert.throws( function throws() {
		dgeqr2p( 'row-major', 2, -1, new Float64Array( 4 ), 2, new Float64Array( 4 ), 2, new Float64Array( 4 ) );
	}, RangeError );
});

test( 'dgeqr2p throws RangeError when LDA is too small (row-major)', function t() {
	assert.throws( function throws() {
		dgeqr2p( 'row-major', 2, 3, new Float64Array( 6 ), 1, new Float64Array( 3 ), 1, new Float64Array( 3 ) );
	}, RangeError );
});

test( 'dgeqr2p throws RangeError when LDA is too small (column-major)', function t() {
	assert.throws( function throws() {
		dgeqr2p( 'column-major', 3, 2, new Float64Array( 6 ), 1, new Float64Array( 2 ), 1, new Float64Array( 2 ) );
	}, RangeError );
});
...
test( 'dgeqr2p computes with row-major layout', function t() {
	...
	const info = dgeqr2p( 'row-major', 3, 2, A, 2, TAU, 1, WORK );
