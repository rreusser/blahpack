...
test( 'ztrttf: N=0 quick return', function t() {
	...
	assert.equal( ztrttf( 'no-transpose', 'lower', 0, A, 1, 1, 0, ARF, 1, 0 ), 0 );
});

test( 'ztrttf: N=1, TRANSR=no-transpose (copies element)', function t() {
	const tc = n1_n;

	const A = new Complex128Array( [ 3.0, 4.0 ] );
	const ARF = new Complex128Array( 1 );
	assert.equal( ztrttf( 'no-transpose', 'lower', 1, A, 1, 1, 0, ARF, 1, 0 ), 0 );
	const arv = reinterpret( ARF, 0 );
...
test( 'ztrttf: N=1, TRANSR=conjugate-transpose (conjugates element)', function t() {
	...
	const A = new Complex128Array( [ 3.0, 4.0 ] );
	const ARF = new Complex128Array( 1 );
	assert.equal( ztrttf( 'conjugate-transpose', 'lower', 1, A, 1, 1, 0, ARF, 1, 0 ), 0 );
	const arv = reinterpret( ARF, 0 );
	assertArrayClose( Array.from( arv ), tc.ARF, 1e-14, 'ARF' );
});
...
test( 'ztrttf: N=5, TRANSR=N, UPLO=L (odd, normal, lower)', function t() {
	...
	const ARF = new Complex128Array( N * ( N + 1 ) / 2 );
	const info = ztrttf( 'no-transpose', 'lower', N, A, 1, N, 0, ARF, 1, 0 );
...
test( 'ztrttf: N=5, TRANSR=N, UPLO=U (odd, normal, upper)', function t() {
	...
	const A = new Complex128Array( tc.A );
	const ARF = new Complex128Array( N * ( N + 1 ) / 2 );
	const info = ztrttf( 'no-transpose', 'upper', N, A, 1, N, 0, ARF, 1, 0 );
	assert.equal( info, tc.info );
	const arv = reinterpret( ARF, 0 );
	assertArrayClose( Array.from( arv ), tc.ARF, 1e-14, 'ARF' );
});
...
test( 'ztrttf: N=5, TRANSR=C, UPLO=L (odd, conj-transpose, lower)', function t() {
	...
	const A = new Complex128Array( tc.A );
	const ARF = new Complex128Array( N * ( N + 1 ) / 2 );
	const info = ztrttf( 'conjugate-transpose', 'lower', N, A, 1, N, 0, ARF, 1, 0 );
	assert.equal( info, tc.info );
	const arv = reinterpret( ARF, 0 );
	assertArrayClose( Array.from( arv ), tc.ARF, 1e-14, 'ARF' );
});
...
test( 'ztrttf: N=5, TRANSR=C, UPLO=U (odd, conj-transpose, upper)', function t() {
	...
	const A = new Complex128Array( tc.A );
	const ARF = new Complex128Array( N * ( N + 1 ) / 2 );
	const info = ztrttf( 'conjugate-transpose', 'upper', N, A, 1, N, 0, ARF, 1, 0 );
	assert.equal( info, tc.info );
	const arv = reinterpret( ARF, 0 );
	assertArrayClose( Array.from( arv ), tc.ARF, 1e-14, 'ARF' );
});
