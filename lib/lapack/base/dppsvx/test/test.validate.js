/**
* Property-based validation for dppsvx, following the /blahpack-validate process.
*
...
* proven for its constituent routines (dppsv/dpprfs residual, dppcon rcond,
* dpprfs FERR/BERR). We drive dppsvx with fact='not-factored' (factor A in place,
...
*       dppsv solve of A0*X=I), and in (0,1].
...
* wrong triangle / a stride-offset addressing bug (the zpptri class) trips a NaN.
* NOTE base returns a plain integer `info` and writes `equed[0]` into the caller's
* array (unlike the dense dposvx twin, whose base returns an object); the ndarray
* `rcond` out-array after offsetX is exercised directly.
*/
...
function invNorm1( A0, n, uplo ) {
	...
	var info = dppsv( uplo, n, n, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Br.data, Br.args[ 0 ], Br.args[ 1 ], Br.args[ 2 ] );
	if ( info !== 0 ) {
...
// Drive dppsvx (fact='not-factored', no equilibration) and return the physical X
...
function drive( uplo, N, nrhs, A0, B0, apLayout, bLayout ) {
	...
	var AFr = schemes.packed.realize( sc, A0, { 'part': uplo }, apLayout ); // overwritten by dcopy+dpptrf
	var Br = schemes.dense.realize( sc, B0, { 'part': 'full' }, bLayout );
	var Xr = schemes.dense.realize( sc, B0, { 'part': 'full' }, bLayout ); // overwritten by dlacpy+dpptrs
	var equed = [ '' ];
	...
	var info = dppsvx( 'not-factored', uplo, N, nrhs, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], AFr.data, AFr.args[ 0 ], AFr.args[ 1 ], equed, s, 1, 0, Br.data, Br.args[ 0 ], Br.args[ 1 ], Br.args[ 2 ], Xr.data, Xr.args[ 0 ], Xr.args[ 1 ], Xr.args[ 2 ], rcond, FERR, 1, 0, BERR, 1, 0, WORK, 1, 0, IWORK, 1, 0 );
	return {
		'Xr': Xr,
		'rcond': rcond,
		'FERR': FERR,
...
// and nrhs.
test( 'dppsvx: expert-driver residual + rcond + FERR/BERR (uplo x N x nrhs)', function t() {
...
function runProperty( uplo, N, nrhs ) {
	...
	var tag = 'dppsvx '+uplo+' N='+N+' nrhs='+nrhs;

	if ( d.info !== 0 ) {
		throw new Error( tag+': dppsvx returned info='+d.info+' (expected 0 for well-conditioned SPD input)' );
	}
	if ( d.equed !== 'none' ) {
	...
	checked( 'dppsvx', 'residual', function run() {
		var j;
		for ( j = 0; j < nrhs; j++ ) {
	...
	checked( 'dppsvx', 'property', function run() {
	...
	checked( 'dppsvx', 'structural', function run() {
...
// and dppsvx's inner kernels (dpptrf/dpptrs via dtpsv, dppcon via dlatps/dlacn2,
// dpprfs via dspmv/daxpy) walk AP / B / X by stride without reordering their
...
test( 'dppsvx: bit-exact across the storage layout family (packed AP + dense B/X)', function t() {
	UPLOS.forEach( function eachUplo( uplo ) {
		runInvariance( uplo );
	});
});
...
function runInvariance( uplo ) {
	...
	checked( 'dppsvx', 'layout-invariance', function run() {
		layoutInvariant( variants, function build( idx ) {
			var rng = new RNG( SEED ); // identical values every variant
			var A0 = logical.positiveDefinite( sc, rng, N );
			var B0 = logical.general( sc, rng, N, nrhs );
			var d = drive( uplo, N, nrhs, A0, B0, apLayouts[ idx % apLayouts.length ], bLayouts[ idx % bLayouts.length ] );
			...
			return out;
		}, { 'label': 'dppsvx '+uplo+' layout invariance' } );
	});
}
