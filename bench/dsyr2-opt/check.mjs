...
// uplo, layouts, general/negative strides, remainders, zero guards, and
// alpha special values. The blocked kernel only reschedules memory — every
// element receives exactly the reference update
// `(x[i]*(alpha*y[j])) + (y[i]*(alpha*x[j]))` (`j` the column index) with
// the reference `x[j] !== 0 || y[j] !== 0` guard — so the gate is the
// bit-identical tier (docs/optimization-policy.md): the FULL storage buffer
// of A must match bitwise (Object.is), which also proves the unstored
			...
			for ( const [ sx, sy ] of [ [ 1, 1 ], [ 2, 1 ], [ 1, 2 ], [ -1, 1 ], [ 1, -1 ] ] ) {
				for ( const alpha of [ 0.0, 1.0, 0.7, -0.2 ] ) {
					for ( const zeros of [ false, true ] ) {
						const sizeA = ( Math.abs( sc.sa1 ) * Math.max( N, 1 ) ) + ( Math.abs( sc.sa2 ) * Math.max( N, 1 ) ) + 8;
						const oa = 2 + ( sc.sa1 < 0 ? Math.abs( sc.sa1 ) * ( N - 1 ) : 0 ) + ( sc.sa2 < 0 ? Math.abs( sc.sa2 ) * ( N - 1 ) : 0 );
						const A = rand( sizeA, false );
						const x = rand( Math.max( 1, N * Math.abs( sx ) ) + 2, zeros );
						const ox = sx < 0 ? ( N - 1 ) * Math.abs( sx ) : 1;
						const y = rand( Math.max( 1, N * Math.abs( sy ) ) + 2, zeros );
						const oy = sy < 0 ? ( N - 1 ) * Math.abs( sy ) : 1;
						const A0 = A.slice();
						const A1 = A.slice();
						v0( uplo, N, alpha, x, sx, ox, y, sy, oy, A0, sc.sa1, sc.sa2, oa );
						v1( uplo, N, alpha, x, sx, ox, y, sy, oy, A1, sc.sa1, sc.sa2, oa );
						ncases += 1;
						let bad = -1;
						...
						if ( bad >= 0 ) {
							failures += 1;
							if ( failures < 20 ) {
								console.log( `FAIL N=${N} ${uplo} ${sc.tag} sx=${sx} sy=${sy} a=${alpha} z=${zeros}: A[${bad}] v0=${A0[ bad ]} v1=${A1[ bad ]}` );
							}
						}
					}
				}
