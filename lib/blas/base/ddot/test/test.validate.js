...
import ddot from './../lib/ndarray.js';
...
// L2 property: got == sum_i x_i*y_i, validated against an independent inline oracle across a size sweep (incl N=0,1).
test( 'ddot: dot product equals sum_i x_i*y_i (property)', function t() {
	checked( 'ddot', 'property', function run() {
		SIZES.forEach( function eachN( N ) {
			...
			const got = ddot( N, X.data, X.args[ 0 ], X.args[ 1 ], Y.data, Y.args[ 0 ], Y.args[ 1 ] );
			...
			check.assertFinite( sc, [ got ], 'ddot N='+N );
			check.assertScaled( sc.abs( sc.sub( got, exp ) ), sc.abs( exp ), check.tol( N, 20 ), 'ddot N='+N );
		});
	});
});

// L3 layout invariance: output is bit-exact across all vector layouts (padded / gapped / negative strides).
test( 'ddot: output is bit-exact across storage layouts', function t() {
	const N = 17;
	const SEED = 0xD07;
	const vL = schemes.vectorLayouts();
	checked( 'ddot', 'layout-invariance', function run() {
