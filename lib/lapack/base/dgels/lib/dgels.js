...
*/
function dgels( order, trans, M, N, nrhs, A, LDA, B, LDB, work, strideWork ) {
	let lwork, MN, sa1, sa2, sb1, sb2;
	...
	if ( order === 'row-major' && LDB < max( 1, nrhs ) ) {
		throw new RangeError( format( 'invalid argument. Ninth argument must be greater than or equal to max(1,nrhs). Value: `%d`.', LDB ) );
	}
	if ( order === 'column-major' && LDB < max( 1, max( M, N ) ) ) {
		throw new RangeError( format( 'invalid argument. Ninth argument must be greater than or equal to max(1,M,N). Value: `%d`.', LDB ) );
	}
	...
	if ( work === null || work === void 0 ) {
		...
		// Blocked path (MN > NB) needs an extra (NB+1)*NB beyond the reference
		// LWORK because the JS dgeqrf/dgelqf/dormqr store the block-reflector T
		// factor separately; without it the factorization reads past WORK and
		// produces silent all-NaN output. See lib/base.js.
		lwork = Math.max( 1, MN + ( Math.max( MN, nrhs ) * NB ) + ( ( MN > NB ) ? ( ( NB + 1 ) * NB ) : 0 ) );
		work = new Float64Array( lwork );
		strideWork = 1;
	}
	return base( trans, M, N, nrhs, A, sa1, sa2, 0, B, sb1, sb2, 0, work, strideWork, 0 );
}
