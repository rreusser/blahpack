...
import dlahqr from '../../dlahqr/lib/base.js';
...
* @param {Float64Array} work - caller-provided workspace array (minimum `max(1,N)` elements; larger improves performance via dlaqr0)
* @param {integer} strideWork - stride length for `work`
* @param {NonNegativeInteger} offsetWork - starting index for `work`
* @returns {integer} info - 0 on success, >0 if failed to converge
*/
function dhseqr( job, compz, N, ilo, ihi, H, strideH1, strideH2, offsetH, WR, strideWR, offsetWR, WI, strideWI, offsetWI, Z, strideZ1, strideZ2, offsetZ, work, strideWork, offsetWork ) { // eslint-disable-line max-params
	let workl, info, kbot, HL, i;

	// Decode parameters
	const wantt = ( job === 'schur' );
	const initz = ( compz === 'initialize' );
	const wantz = initz || ( compz === 'update' );
	...
	if ( USE_DLAQR0 && N > Math.max( NTINY, NMIN ) ) {
		// Large matrix: use dlaqr0 directly with caller-provided workspace.
		info = dlaqr0( wantt, wantz, N, ilo, ihi, H, strideH1, strideH2, offsetH, WR, strideWR, offsetWR, WI, strideWI, offsetWI, ilo, ihi, Z, strideZ1, strideZ2, offsetZ, work, strideWork, offsetWork );
	} else {
		...
		if ( info > 0 ) {
			...
			if ( N >= NL ) {
				// Matrix is big enough to use dlaqr0 directly with caller's workspace
				info = dlaqr0( wantt, wantz, N, ilo, kbot, H, strideH1, strideH2, offsetH, WR, strideWR, offsetWR, WI, strideWI, offsetWI, ilo, ihi, Z, strideZ1, strideZ2, offsetZ, work, strideWork, offsetWork );
			} else {
				// Matrix is smaller than NL — embed in an NL-by-NL scratch matrix
				// to give dlaqr0 more room to work. HL mirrors the reference
				// Fortran local array HL(NL,NL): it is a FIXED-size (NL=49)
				// algorithm-internal temporary — NOT caller-owned, problem-scaled
				// workspace — so it is allocated here with a bare literal size
				// (49*49 = 2401), the fixed-bookkeeping form. A fresh, zero-filled
				// buffer each call preserves the reference behavior exactly (this
				// rare-failure path only runs when N < NL and dlahqr fails).
				HL = new Float64Array( 2401 );
				...
				// Run dlaqr0 on the embedded matrix using a small local buffer.
				// This mirrors the reference Fortran local array WORKL(NL): a
				// FIXED-size (NL=49) internal scratch buffer, not caller-owned
				// workspace. The caller's `work` may be smaller than NL when
				// N < NL, so — as the reference does — a dedicated NL-element
				// buffer is allocated here with a bare literal size (49), the
				// fixed-bookkeeping form.
				workl = new Float64Array( 49 );
				info = dlaqr0( wantt, wantz, NL, ilo, kbot, HL, 1, NL, 0, WR, strideWR, offsetWR, WI, strideWI, offsetWI, ilo, ihi, Z, strideZ1, strideZ2, offsetZ, workl, 1, 0 );
