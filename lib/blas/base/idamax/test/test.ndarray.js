...
test( 'idamax: basic (N=5, stride=1)', function t() {
	...
	const tc = basic;
	const x = new Float64Array( [ 1.0, -3.0, 2.0, 5.0, -4.0 ] );
	const result = idamax( 5, x, 1, 0 );
...
test( 'idamax: negative values (N=4, stride=1)', function t() {
	...
	const tc = negative;
	const x = new Float64Array( [ -2.0, -7.0, -1.0, -3.0 ] );
	const result = idamax( 4, x, 1, 0 );
	assert.strictEqual( result, tc.result - 1 );
});
...
test( 'idamax: non-unit stride (N=3, stride=2)', function t() {
	...
	const tc = stride;
	const x = new Float64Array( [ 1.0, 99.0, 10.0, 99.0, 2.0 ] );
	const result = idamax( 3, x, 2, 0 );
	assert.strictEqual( result, tc.result - 1 );
});
...
test( 'idamax: first element is max', function t() {
	...
	const tc = first_max;
	const x = new Float64Array( [ 100.0, 1.0, 2.0 ] );
	const result = idamax( 3, x, 1, 0 );
	assert.strictEqual( result, tc.result - 1 );
});
...
test( 'idamax: last element is max', function t() {
	...
	const tc = last_max;
	const x = new Float64Array( [ 1.0, 2.0, 100.0 ] );
	const result = idamax( 3, x, 1, 0 );
	assert.strictEqual( result, tc.result - 1 );
});
...
test( 'idamax: offset parameter works', function t() {
	...
	const x = new Float64Array( [ 99.0, 1.0, 10.0, 2.0 ] );
	const result = idamax( 3, x, 1, 1 );
	assert.strictEqual( result, 1 );
});

test( 'idamax: all equal elements returns 0', function t() {

	const x = new Float64Array( [ 5.0, 5.0, 5.0, 5.0 ] );
	const result = idamax( 4, x, 1, 0 );
	assert.strictEqual( result, 0 );
});
