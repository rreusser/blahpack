...
import dsytrsRook from './../lib/dsytrs_rook.js';
...
test( 'dsytrs_rook column-major small solve', function t() {
	...
	const info = dsytrsRook( 'column-major', 'lower', 1, 1, A, 1, ipiv, 1, b, 1 );
...
test( 'dsytrs_rook row-major small solve', function t() {
	...
	const b = new Float64Array( [ 8.0 ] );
	const ipiv = new Int32Array( [ 0 ] );
	const info = dsytrsRook( 'row-major', 'lower', 1, 1, A, 1, ipiv, 1, b, 1 );
	assert.equal( info, 0, 'info' );
	assert.equal( b[ 0 ], 2.0, 'b' );
});
