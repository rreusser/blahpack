...
*/
function dormhr( side, trans, M, N, ilo, ihi, A, strideA1, strideA2, offsetA, TAU, strideTAU, offsetTAU, C, strideC1, strideC2, offsetC, WORK, strideWork, offsetWork ) { // eslint-disable-line max-len, max-params
	...
	// Caller owns the workspace; dormhr forwards WORK straight to the BLOCKED
	// dormqr with K = nh = IHI-ILO reflectors. The unblocked path (NB >= nh,
	// NB=32) needs only nw = max(1,N) for side='left' (max(1,M) for 'right'); the
	// BLOCKED path (NB < nh) additionally stores the block reflector T in a
	// separate trailing segment, needing nw*NB + (NB+1)*NB. Advertising only the
	// unblocked nw would accept a buffer the blocked path over-reads → silent NaN.
	// The degenerate cases (M=0/N=0/nh=0) quick-return in base and touch no WORK.
	const nh = ihi - ilo;
	if ( M > 0 && N > 0 && nh > 0 ) {
		const nb = 32;
		const nw = ( side === 'left' ) ? Math.max( 1, N ) : Math.max( 1, M );
		const minWork = ( nb >= nh ) ? nw : ( ( nw * nb ) + ( ( nb + 1 ) * nb ) );
