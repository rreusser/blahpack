...
test( 'drscl: stride2', function t() {
	const tc = stride2;
	const x = new Float64Array( [ 10.0, 99.0, 20.0, 99.0, 30.0 ] );
	drscl( 3, 10.0, x, 2, 0 );
	assertArrayClose( x, tc.x, 1e-14, 'x' );
});
...
test( 'drscl: identity', function t() {
	const tc = identity;
	const x = new Float64Array( [ 3.0, 7.0 ] );
	drscl( 2, 1.0, x, 1, 0 );
	assertArrayClose( x, tc.x, 1e-14, 'x' );
});
...
test( 'drscl: very_small_sa (triggers BIGNUM branch)', function t() {
	const tc = very_small_sa;
	const x = new Float64Array( [ 1e-308, 2e-308 ] );
	drscl( 2, 1e-309, x, 1, 0 );
	assertArrayClose( x, tc.x, 1e-14, 'x' );
});

test( 'drscl: negative_sa', function t() {
	const tc = negative_sa;
	const x = new Float64Array( [ 6.0, -9.0, 12.0 ] );
	drscl( 3, -3.0, x, 1, 0 );
	assertArrayClose( x, tc.x, 1e-14, 'x' );
});
...
test( 'drscl: offset', function t() {
	...
	const x = new Float64Array( [ 999.0, 10.0, 20.0, 30.0 ] );
	drscl( 3, 10.0, x, 1, 1 );
