...
import dlatsqr from './../lib/ndarray.js';


// FIXTURES //

const fixtureDir = path.join( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures' );
const fixtureRaw = readFileSync( path.join( fixtureDir, 'dlatsqr.jsonl' ), 'utf8' ); // eslint-disable-line node/no-sync
...
function runCase( cfg ) {
	...
	const T = new Float64Array( nb * N * nblk );
	const WORK = new Float64Array( nb * N );
	const info = dlatsqr( M, N, mb, nb, A, 1, M, 0, T, 1, nb, 0, WORK, 1, 0 );
...
test( 'dlatsqr: M=0 quick return', function t() {
	...
	const info = dlatsqr( 0, 0, 4, 1, A, 1, 1, 0, T, 1, 1, 0, WORK, 1, 0 );
...
test( 'dlatsqr: N=0 quick return', function t() {
	...
	const T = new Float64Array( 0 );
	const WORK = new Float64Array( 0 );
	const info = dlatsqr( 3, 0, 4, 1, A, 1, 3, 0, T, 1, 1, 0, WORK, 1, 0 );
	assert.strictEqual( info, 0, 'returns 0' );
});
...
test( 'dlatsqr: works with non-trivial offsets', function t() {
	...
	const T = new Float64Array( ( nb * N * nblk ) + oT );
	const WORK = new Float64Array( ( nb * N ) + oWORK );
	...
	const info = dlatsqr( M, N, mb, nb, A, 1, M, oA, T, 1, nb, oT, WORK, 1, oWORK );
