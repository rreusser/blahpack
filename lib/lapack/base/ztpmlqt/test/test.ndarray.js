...
const rawLines = readFileSync( path.join( fixtureDir, 'ztpmlqt.jsonl' ), 'utf8' ).trim().split( '\n' );
...
function loadVT( name ) {
	...
	const Vbuf = new Float64Array( tc.V );
	const Tbuf = new Float64Array( tc.T );
	return {
...
function initRightB() {
	...
	const B = new Complex128Array( 12 );
	const Ar = reinterpret( A, 0 );
	const Br = reinterpret( B, 0 );
	for ( jj = 0; jj < 4; jj++ ) {
		for ( ii = 0; ii < 3; ii++ ) {
			off = ( ( jj * 3 ) + ii ) * 2;
...
function initLeftC() {
	let off, ii, jj;
	const A = new Complex128Array( 36 );
	const B = new Complex128Array( 48 );
	const Ar = reinterpret( A, 0 );
	const Br = reinterpret( B, 0 );
	for ( jj = 0; jj < 6; jj++ ) {
		for ( ii = 0; ii < 6; ii++ ) {
...
function initRightD() {
	...
	const B = new Complex128Array( 15 );
	const Ar = reinterpret( A, 0 );
	const Br = reinterpret( B, 0 );
	for ( jj = 0; jj < 3; jj++ ) {
		for ( ii = 0; ii < 3; ii++ ) {
...
test( 'ztpmlqt: SIDE=left, TRANS=no-transpose (Case A: M=4, N=3, K=3, L=2, MB=2)', function t() {
	const inputs = initLeftA();
	const WORK = new Complex128Array( 32 );
	const vt = loadVT( 'factors_left_A' );
	const info = ztpmlqt( 'left', 'no-transpose', 4, 3, 3, 2, 2, vt.V, 1, 3, 0, vt.T, 1, 2, 0, inputs.A, 1, 3, 0, inputs.B, 1, 4, 0, WORK, 1, 0 );
...
test( 'ztpmlqt: SIDE=left, TRANS=conjugate-transpose (Case A)', function t() {
	...
	const WORK = new Complex128Array( 32 );
	const vt = loadVT( 'factors_left_A' );
	const info = ztpmlqt( 'left', 'conjugate-transpose', 4, 3, 3, 2, 2, vt.V, 1, 3, 0, vt.T, 1, 2, 0, inputs.A, 1, 3, 0, inputs.B, 1, 4, 0, WORK, 1, 0 );
...
test( 'ztpmlqt: SIDE=right, TRANS=no-transpose (Case B: M=3, N=4, K=4, L=2, MB=2)', function t() {
	const inputs = initRightB();
	const WORK = new Complex128Array( 64 );
	const vt = loadVT( 'factors_right_B' );
	const info = ztpmlqt( 'right', 'no-transpose', 3, 4, 4, 2, 2, vt.V, 1, 4, 0, vt.T, 1, 2, 0, inputs.A, 1, 3, 0, inputs.B, 1, 3, 0, WORK, 1, 0 );
...
test( 'ztpmlqt: SIDE=right, TRANS=conjugate-transpose (Case B)', function t() {
	...
	const WORK = new Complex128Array( 64 );
	const vt = loadVT( 'factors_right_B' );
	const info = ztpmlqt( 'right', 'conjugate-transpose', 3, 4, 4, 2, 2, vt.V, 1, 4, 0, vt.T, 1, 2, 0, inputs.A, 1, 3, 0, inputs.B, 1, 3, 0, WORK, 1, 0 );
...
test( 'ztpmlqt: SIDE=left, TRANS=no-transpose blocked (Case C: M=8, N=6, K=6, L=3, MB=3)', function t() {
	const inputs = initLeftC();
	const WORK = new Complex128Array( 256 );
	const vt = loadVT( 'factors_left_C' );
	const info = ztpmlqt( 'left', 'no-transpose', 8, 6, 6, 3, 3, vt.V, 1, 6, 0, vt.T, 1, 3, 0, inputs.A, 1, 6, 0, inputs.B, 1, 8, 0, WORK, 1, 0 );
...
test( 'ztpmlqt: SIDE=left, TRANS=conjugate-transpose blocked (Case C)', function t() {
	...
	const WORK = new Complex128Array( 256 );
	const vt = loadVT( 'factors_left_C' );
	const info = ztpmlqt( 'left', 'conjugate-transpose', 8, 6, 6, 3, 3, vt.V, 1, 6, 0, vt.T, 1, 3, 0, inputs.A, 1, 6, 0, inputs.B, 1, 8, 0, WORK, 1, 0 );
...
test( 'ztpmlqt: SIDE=left, TRANS=no-transpose, L=0 (Case D)', function t() {
	const inputs = initLeftD();
	const WORK = new Complex128Array( 64 );
	const vt = loadVT( 'factors_left_D' );
	const info = ztpmlqt( 'left', 'no-transpose', 5, 4, 3, 0, 2, vt.V, 1, 3, 0, vt.T, 1, 2, 0, inputs.A, 1, 3, 0, inputs.B, 1, 5, 0, WORK, 1, 0 );
...
test( 'ztpmlqt: SIDE=left, TRANS=conjugate-transpose, L=0 (Case D)', function t() {
	...
	const WORK = new Complex128Array( 64 );
	const vt = loadVT( 'factors_left_D' );
	const info = ztpmlqt( 'left', 'conjugate-transpose', 5, 4, 3, 0, 2, vt.V, 1, 3, 0, vt.T, 1, 2, 0, inputs.A, 1, 3, 0, inputs.B, 1, 5, 0, WORK, 1, 0 );
...
test( 'ztpmlqt: SIDE=right, TRANS=no-transpose, L=0 (Case D)', function t() {
	const inputs = initRightD();
	const WORK = new Complex128Array( 64 );
	const vt = loadVT( 'factors_left_D' );
	const info = ztpmlqt( 'right', 'no-transpose', 3, 5, 3, 0, 2, vt.V, 1, 3, 0, vt.T, 1, 2, 0, inputs.A, 1, 3, 0, inputs.B, 1, 3, 0, WORK, 1, 0 );
...
test( 'ztpmlqt: SIDE=right, TRANS=conjugate-transpose, L=0 (Case D)', function t() {
	...
	const WORK = new Complex128Array( 64 );
	const vt = loadVT( 'factors_left_D' );
	const info = ztpmlqt( 'right', 'conjugate-transpose', 3, 5, 3, 0, 2, vt.V, 1, 3, 0, vt.T, 1, 2, 0, inputs.A, 1, 3, 0, inputs.B, 1, 3, 0, WORK, 1, 0 );
...
test( 'ztpmlqt: SIDE=left, TRANS=no-transpose, L=K (Case E, fully triangular V)', function t() {
	const inputs = initLeftE();
	const WORK = new Complex128Array( 32 );
	const vt = loadVT( 'factors_left_E' );
	const info = ztpmlqt( 'left', 'no-transpose', 3, 2, 3, 3, 2, vt.V, 1, 3, 0, vt.T, 1, 2, 0, inputs.A, 1, 3, 0, inputs.B, 1, 3, 0, WORK, 1, 0 );
...
test( 'ztpmlqt: SIDE=left, TRANS=conjugate-transpose, L=K (Case E)', function t() {
	...
	const WORK = new Complex128Array( 32 );
	const vt = loadVT( 'factors_left_E' );
	const info = ztpmlqt( 'left', 'conjugate-transpose', 3, 2, 3, 3, 2, vt.V, 1, 3, 0, vt.T, 1, 2, 0, inputs.A, 1, 3, 0, inputs.B, 1, 3, 0, WORK, 1, 0 );
...
test( 'ztpmlqt: SIDE=left, TRANS=no-transpose, MB=K (Case F: single inner iteration)', function t() {
	const inputs = initLeftF();
	const WORK = new Complex128Array( 64 );
	const vt = loadVT( 'factors_single' );
	const info = ztpmlqt( 'left', 'no-transpose', 5, 4, 4, 2, 4, vt.V, 1, 4, 0, vt.T, 1, 4, 0, inputs.A, 1, 4, 0, inputs.B, 1, 5, 0, WORK, 1, 0 );
...
test( 'ztpmlqt: SIDE=left, TRANS=conjugate-transpose, MB=K (Case F)', function t() {
	...
	const WORK = new Complex128Array( 64 );
	const vt = loadVT( 'factors_single' );
	const info = ztpmlqt( 'left', 'conjugate-transpose', 5, 4, 4, 2, 4, vt.V, 1, 4, 0, vt.T, 1, 4, 0, inputs.A, 1, 4, 0, inputs.B, 1, 5, 0, WORK, 1, 0 );
...
test( 'ztpmlqt: quick return when M=0', function t() {
	...
	const Acopy = Float64Array.from( reinterpret( A, 0 ) );
	const Bcopy = Float64Array.from( reinterpret( B, 0 ) );
	const info = ztpmlqt( 'left', 'no-transpose', 0, 3, 3, 2, 2, V, 1, 1, 0, T, 1, 2, 0, A, 1, 1, 0, B, 1, 1, 0, WORK, 1, 0 );
	assert.equal( info, 0, 'info' );
...
test( 'ztpmlqt: quick return when N=0', function t() {
	...
	const Acopy = Float64Array.from( reinterpret( A, 0 ) );
	const Bcopy = Float64Array.from( reinterpret( B, 0 ) );
	const info = ztpmlqt( 'left', 'no-transpose', 4, 0, 3, 2, 2, V, 1, 1, 0, T, 1, 2, 0, A, 1, 1, 0, B, 1, 1, 0, WORK, 1, 0 );
	assert.equal( info, 0, 'info' );
	assertArrayClose( reinterpret( A, 0 ), Acopy, 0, 'A unchanged' );
	assertArrayClose( reinterpret( B, 0 ), Bcopy, 0, 'B unchanged' );
});
...
test( 'ztpmlqt: quick return when K=0', function t() {
	...
	const Acopy = Float64Array.from( reinterpret( A, 0 ) );
	const Bcopy = Float64Array.from( reinterpret( B, 0 ) );
	const info = ztpmlqt( 'left', 'no-transpose', 4, 3, 0, 0, 1, V, 1, 1, 0, T, 1, 1, 0, A, 1, 1, 0, B, 1, 1, 0, WORK, 1, 0 );
	assert.equal( info, 0, 'info' );
	assertArrayClose( reinterpret( A, 0 ), Acopy, 0, 'A unchanged' );
	assertArrayClose( reinterpret( B, 0 ), Bcopy, 0, 'B unchanged' );
});
