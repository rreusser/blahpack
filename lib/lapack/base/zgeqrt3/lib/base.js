...
function zgeqrt3( M, N, A, strideA1, strideA2, offsetA, T, strideT1, strideT2, offsetT ) {
	...
	const N1 = ( N / 2 ) | 0;
	const N2 = N - N1;
	const j1 = N1; // 0-based col index where the right (N2-col) block starts; equivalent to Fortran J1-1.
	const i1 = Math.min( N, M - 1 ); // 0-based row index of the (M-N) trailing rows; equivalent to Fortran I1-1.
	...
	const aOff = offsetA * 2;
	const tOff = offsetT * 2;
	for ( j = 0; j < N2; j++ ) {
		for ( i = 0; i < N1; i++ ) {
	...
	const j1off = offsetT + ( j1 * strideT2 );
