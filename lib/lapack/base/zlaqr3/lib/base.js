...
import ztrexc from '../../ztrexc/lib/base.js';
import zunmhr from '../../zunmhr/lib/base.js';
...
const NMIN_CROSSOVER = 75; // ILAENV(12) for ZLAQR3
...
* @private
*/
function zlaqr3( wantt, wantz, N, ktop, kbot, nw, H, strideH1, strideH2, offsetH, iloz, ihiz, Z, strideZ1, strideZ2, offsetZ, ns, nd, SH, strideSH, offsetSH, V, strideV1, strideV2, offsetV, nhp, T, strideT1, strideT2, offsetT, nvp, WV, strideWV1, strideWV2, offsetWV, WORK, strideWork, offsetWork ) {
	let infqr, ltop, kcol, krow, kln, sR, sI, foo, ifst, ilst, knt, betaCA;
	let betaV, tauCA, tauV, i, j;
	...
	const SHv = reinterpret( SH, 0 );
	const sSH = strideSH * 2;
	const oSH = offsetSH * 2;
	...
	if ( kbot === kwtop ) {
		...
		// Deflation test uses the spike magnitude `S` (zero when the window
		// starts at KTOP), NOT a direct read of H(kwtop,kwtop-1): when
		// kwtop === ktop === 1 that subdiagonal is out of bounds, which would
		// yield NaN and wrongly refuse to deflate the top eigenvalue.
		if ( ( Math.abs( sR ) + Math.abs( sI ) ) <= Math.max( smlnum, ULP * cabs1( Hv, oH + ( kwtop - 1 ) * sh1 + ( kwtop - 1 ) * sh2 ) ) ) {
			ns = 0;
			nd = 1;
			if ( kwtop > ktop ) {
	...
	if ( jw > NMIN_CROSSOVER ) {
		infqr = zlaqr4( true, true, jw, 1, jw, T, strideT1, strideT2, offsetT, SH, strideSH, offsetSH + ( kwtop - 1 ) * strideSH, 1, jw, V, strideV1, strideV2, offsetV, WORK, strideWork, offsetWork );
	} else {
	...
	for ( knt = infqr + 1; knt <= jw; knt++ ) {
		...
		const vTest = ( Math.abs( sR ) + Math.abs( sI ) ) * cabs1( Vv, oV + ( ns - 1 ) * sv2 );
	...
	if ( ns < jw || ( sR === 0.0 && sI === 0.0 ) ) {
		if ( ns > 1 && ( sR !== 0.0 || sI !== 0.0 ) ) {
			...
			const oWk = offsetWork * 2;
			const sWk = strideWork * 2;
			for ( i = 0; i < ns; i++ ) {
		...
		if ( ns > 1 && ( sR !== 0.0 || sI !== 0.0 ) ) {
			zunmhr( 'right', 'no-transpose', jw, ns, 1, ns, T, strideT1, strideT2, offsetT, WORK, strideWork, offsetWork, V, strideV1, strideV2, offsetV, WORK, strideWork, offsetWork + jw * strideWork );
		}
