...
test( 'dgeql2 has expected arity', function t() {
	assert.strictEqual( dgeql2.length, 8, 'has expected arity' );
});

test( 'dgeql2 throws TypeError for invalid order', function t() {
	assert.throws( function throws() {
		dgeql2( 'invalid', 2, 2, new Float64Array( 4 ), 2, new Float64Array( 4 ), 2, new Float64Array( 4 ) );
	}, TypeError );
});

test( 'dgeql2 throws RangeError for negative M', function t() {
	assert.throws( function throws() {
		dgeql2( 'row-major', -1, 2, new Float64Array( 4 ), 2, new Float64Array( 4 ), 2, new Float64Array( 4 ) );
	}, RangeError );
});

test( 'dgeql2 throws RangeError for negative N', function t() {
	assert.throws( function throws() {
		dgeql2( 'row-major', 2, -1, new Float64Array( 4 ), 2, new Float64Array( 4 ), 2, new Float64Array( 4 ) );
	}, RangeError );
});
