...
* @param {Float64Array} v - Arnoldi vectors in column-major order, `n` by `kev+np`
* @param {integer} ldv - leading dimension of `v`
* @param {Float64Array} h - symmetric tridiagonal matrix in 2-column column-major layout (subdiagonal in column 0, diagonal in column 1), `kev+np` by 2
...
* @param {Float64Array} q - workspace to accumulate the rotations in column-major order, `kev+np` by `kev+np`
...
* }
*
* var h = new Float64Array( [ 0.0, 1.0, 0.5, 3.0, 1.0, 2.0 ] ); // 3x2, ldh=3
...
*/
function dsapps( n, kev, np, shift, strideShift, v, ldv, h, ldh, resid, strideResid, q, ldq, workd, strideWorkd ) {
	if ( n < 0 ) {
	...
	return base( n, kev, np, shift, strideShift, stride2offset( np, strideShift ), v, 1, ldv, 0, h, 1, ldh, 0, resid, strideResid, stride2offset( n, strideResid ), q, 1, ldq, 0, workd, strideWorkd, stride2offset( Math.max( 2 * n, 0 ), strideWorkd ) );
}
