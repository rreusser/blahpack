...
import dtpmqrt from './../lib/ndarray.js';


// FIXTURES //

const fixtureDir = path.join( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures' );
const rawLines = readFileSync( path.join( fixtureDir, 'dtpmqrt.jsonl' ), 'utf8' ).trim().split( '\n' );
...
test( 'dtpmqrt: SIDE=left, TRANS=no-transpose (Case A: M=4, N=3, K=3, L=2, NB=2)', function t() {
	...
	const info = dtpmqrt( 'left', 'no-transpose', 4, 3, 3, 2, 2, vt.V, 1, 4, 0, vt.T, 1, 2, 0, inputs.A, 1, 3, 0, inputs.B, 1, 4, 0, WORK, 1, 0 );
...
test( 'dtpmqrt: SIDE=left, TRANS=transpose (Case A)', function t() {
	...
	const WORK = new Float64Array( 32 );
	const vt = loadVT( 'factors_left_A' );
	const info = dtpmqrt( 'left', 'transpose', 4, 3, 3, 2, 2, vt.V, 1, 4, 0, vt.T, 1, 2, 0, inputs.A, 1, 3, 0, inputs.B, 1, 4, 0, WORK, 1, 0 );
	const tc = findCase( 'left_trans_A' );
	assert.equal( info, 0, 'info' );
	assertArrayClose( inputs.A, tc.A, 1e-12, 'A' );
	assertArrayClose( inputs.B, tc.B, 1e-12, 'B' );
});
...
test( 'dtpmqrt: SIDE=right, TRANS=no-transpose (Case B: M=3, N=4, K=4, L=2, NB=2)', function t() {
	...
	const info = dtpmqrt( 'right', 'no-transpose', 3, 4, 4, 2, 2, vt.V, 1, 4, 0, vt.T, 1, 2, 0, inputs.A, 1, 3, 0, inputs.B, 1, 3, 0, WORK, 1, 0 );
...
test( 'dtpmqrt: SIDE=right, TRANS=transpose (Case B)', function t() {
	...
	const WORK = new Float64Array( 64 );
	const vt = loadVT( 'factors_right_B' );
	const info = dtpmqrt( 'right', 'transpose', 3, 4, 4, 2, 2, vt.V, 1, 4, 0, vt.T, 1, 2, 0, inputs.A, 1, 3, 0, inputs.B, 1, 3, 0, WORK, 1, 0 );
	const tc = findCase( 'right_trans_B' );
	assert.equal( info, 0, 'info' );
	assertArrayClose( inputs.A, tc.A, 1e-12, 'A' );
	assertArrayClose( inputs.B, tc.B, 1e-12, 'B' );
});
...
test( 'dtpmqrt: SIDE=left, TRANS=no-transpose blocked (Case C: M=8, N=6, K=6, L=3, NB=3)', function t() {
	...
	const info = dtpmqrt( 'left', 'no-transpose', 8, 6, 6, 3, 3, vt.V, 1, 8, 0, vt.T, 1, 3, 0, inputs.A, 1, 6, 0, inputs.B, 1, 8, 0, WORK, 1, 0 );
...
test( 'dtpmqrt: SIDE=left, TRANS=transpose blocked (Case C)', function t() {
	...
	const WORK = new Float64Array( 256 );
	const vt = loadVT( 'factors_left_C' );
	const info = dtpmqrt( 'left', 'transpose', 8, 6, 6, 3, 3, vt.V, 1, 8, 0, vt.T, 1, 3, 0, inputs.A, 1, 6, 0, inputs.B, 1, 8, 0, WORK, 1, 0 );
	const tc = findCase( 'left_trans_C' );
	assert.equal( info, 0, 'info' );
	assertArrayClose( inputs.A, tc.A, 1e-11, 'A' );
	assertArrayClose( inputs.B, tc.B, 1e-11, 'B' );
});
...
test( 'dtpmqrt: SIDE=left, TRANS=no-transpose, L=0 (Case D)', function t() {
	...
	const info = dtpmqrt( 'left', 'no-transpose', 5, 4, 3, 0, 2, vt.V, 1, 5, 0, vt.T, 1, 2, 0, inputs.A, 1, 3, 0, inputs.B, 1, 5, 0, WORK, 1, 0 );
...
test( 'dtpmqrt: SIDE=left, TRANS=transpose, L=0 (Case D)', function t() {
	...
	const WORK = new Float64Array( 64 );
	const vt = loadVT( 'factors_left_D' );
	const info = dtpmqrt( 'left', 'transpose', 5, 4, 3, 0, 2, vt.V, 1, 5, 0, vt.T, 1, 2, 0, inputs.A, 1, 3, 0, inputs.B, 1, 5, 0, WORK, 1, 0 );
	const tc = findCase( 'left_trans_D' );
	assert.equal( info, 0, 'info' );
	assertArrayClose( inputs.A, tc.A, 1e-12, 'A' );
	assertArrayClose( inputs.B, tc.B, 1e-12, 'B' );
});
...
test( 'dtpmqrt: SIDE=right, TRANS=no-transpose, L=0 (Case D)', function t() {
	...
	const WORK = new Float64Array( 64 );
	const vt = loadVT( 'factors_left_D' );
	const info = dtpmqrt( 'right', 'no-transpose', 3, 4, 3, 0, 2, vt.V, 1, 5, 0, vt.T, 1, 2, 0, inputs.A, 1, 3, 0, inputs.B, 1, 3, 0, WORK, 1, 0 );
...
test( 'dtpmqrt: SIDE=right, TRANS=transpose, L=0 (Case D)', function t() {
	...
	const WORK = new Float64Array( 64 );
	const vt = loadVT( 'factors_left_D' );
	const info = dtpmqrt( 'right', 'transpose', 3, 4, 3, 0, 2, vt.V, 1, 5, 0, vt.T, 1, 2, 0, inputs.A, 1, 3, 0, inputs.B, 1, 3, 0, WORK, 1, 0 );
	const tc = findCase( 'right_trans_D' );
	assert.equal( info, 0, 'info' );
	assertArrayClose( inputs.A, tc.A, 1e-12, 'A' );
	assertArrayClose( inputs.B, tc.B, 1e-12, 'B' );
});
...
test( 'dtpmqrt: SIDE=left, TRANS=no-transpose, L=K (Case E, fully triangular V)', function t() {
	...
	const info = dtpmqrt( 'left', 'no-transpose', 3, 2, 3, 3, 2, vt.V, 1, 3, 0, vt.T, 1, 2, 0, inputs.A, 1, 3, 0, inputs.B, 1, 3, 0, WORK, 1, 0 );
...
test( 'dtpmqrt: SIDE=left, TRANS=transpose, L=K (Case E)', function t() {
	...
	const WORK = new Float64Array( 32 );
	const vt = loadVT( 'factors_left_E' );
	const info = dtpmqrt( 'left', 'transpose', 3, 2, 3, 3, 2, vt.V, 1, 3, 0, vt.T, 1, 2, 0, inputs.A, 1, 3, 0, inputs.B, 1, 3, 0, WORK, 1, 0 );
	const tc = findCase( 'left_trans_E' );
	assert.equal( info, 0, 'info' );
	assertArrayClose( inputs.A, tc.A, 1e-12, 'A' );
	assertArrayClose( inputs.B, tc.B, 1e-12, 'B' );
});
...
test( 'dtpmqrt: SIDE=left, TRANS=no-transpose, NB=K (Case F: single inner iteration)', function t() {
	...
	const info = dtpmqrt( 'left', 'no-transpose', 5, 4, 4, 2, 4, vt.V, 1, 5, 0, vt.T, 1, 4, 0, inputs.A, 1, 4, 0, inputs.B, 1, 5, 0, WORK, 1, 0 );
...
test( 'dtpmqrt: SIDE=left, TRANS=transpose, NB=K (Case F)', function t() {
	...
	const WORK = new Float64Array( 64 );
	const vt = loadVT( 'factors_single' );
	const info = dtpmqrt( 'left', 'transpose', 5, 4, 4, 2, 4, vt.V, 1, 5, 0, vt.T, 1, 4, 0, inputs.A, 1, 4, 0, inputs.B, 1, 5, 0, WORK, 1, 0 );
	const tc = findCase( 'left_trans_F' );
	assert.equal( info, 0, 'info' );
	assertArrayClose( inputs.A, tc.A, 1e-12, 'A' );
	assertArrayClose( inputs.B, tc.B, 1e-12, 'B' );
});
...
test( 'dtpmqrt: quick return when M=0', function t() {
	...
	const info = dtpmqrt( 'left', 'no-transpose', 0, 3, 3, 2, 2, V, 1, 1, 0, T, 1, 2, 0, A, 1, 1, 0, B, 1, 1, 0, WORK, 1, 0 );
	assert.equal( info, 0, 'info' );
...
test( 'dtpmqrt: quick return when N=0', function t() {
	...
	const Acopy = Float64Array.from( A );
	const Bcopy = Float64Array.from( B );
	const info = dtpmqrt( 'left', 'no-transpose', 4, 0, 3, 2, 2, V, 1, 1, 0, T, 1, 2, 0, A, 1, 1, 0, B, 1, 1, 0, WORK, 1, 0 );
	assert.equal( info, 0, 'info' );
	assertArrayClose( A, Acopy, 0, 'A unchanged' );
	assertArrayClose( B, Bcopy, 0, 'B unchanged' );
});
...
test( 'dtpmqrt: quick return when K=0', function t() {
	...
	const Acopy = Float64Array.from( A );
	const Bcopy = Float64Array.from( B );
	const info = dtpmqrt( 'left', 'no-transpose', 4, 3, 0, 0, 1, V, 1, 1, 0, T, 1, 1, 0, A, 1, 1, 0, B, 1, 1, 0, WORK, 1, 0 );
	assert.equal( info, 0, 'info' );
	assertArrayClose( A, Acopy, 0, 'A unchanged' );
	assertArrayClose( B, Bcopy, 0, 'B unchanged' );
});
