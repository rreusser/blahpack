...
*/
function dggbal( order, job, N, A, LDA, B, LDB, LSCALE, strideLSCALE, RSCALE, strideRSCALE, WORK ) {
	let sa1, sa2, sb1, sb2;
	...
	const ol = stride2offset( N, strideLSCALE );
	const or = stride2offset( N, strideRSCALE );
	if ( WORK === null || WORK === void 0 ) {
		const minWork = ( job === 'scale' || job === 'both' ) ? Math.max( 1, 6*N ) : 1;
		WORK = new Float64Array( minWork );
	}
	return base( job, N, A, sa1, sa2, 0, B, sb1, sb2, 0, LSCALE, strideLSCALE, ol, RSCALE, strideRSCALE, or, WORK, 1, 0 );
}
