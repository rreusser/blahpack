...
var RE = S.real; // d, e real
var NAME = 'zunmtr';
var TRANS_C = 'conjugate-transpose'; // op(Q) = Qᴴ for a complex routine
var TCODE_C = 'c';
...
function formQ( Flog, tauVals, nq, uplo ) {
	...
	var Tr = schemes.realizeVector( sc, tauVals, TIGHT_VEC );
...
function reduce( uplo, nq, rng ) {
	var A0 = logical.hermitian( sc, rng, nq );
	...
	var tR = schemes.realizeVector( sc, poison( Math.max( nq - 1, 0 ) ), TIGHT_VEC );
...
function runCase( M, N, side, uplo, trans, tcode, layout ) {
	...
	var Tr = schemes.realizeVector( sc, ( red.taus.length ? red.taus : poison( 1 ) ), TIGHT_VEC );
				...
				[ [ colLayouts, 'col' ], [ rowLayouts, 'row' ] ].forEach( function eachFam( fam ) {
					checked( NAME, 'layout-invariance', function run() {
						layoutInvariant( fam[ 0 ], function build( layout, i ) {
							var Ar = schemes.dense.realize( sc, red.F, { 'part': uplo }, layout );
							var Tr = schemes.realizeVector( sc, red.taus, VEC_LAYOUTS[ i % VEC_LAYOUTS.length ] );
							var Cr = schemes.dense.realize( sc, C0, { 'part': 'full' }, layout );
							var Wo = poisonedWork( sc, ormWork( side, M, N ) );
...
test( 'zunmtr: advertised WORK minimum suffices on the blocked path (Step 4c)', function t() {
	[ [ 'left', 80, 50 ], [ 'right', 50, 80 ] ].forEach( function eachCase( cfg ) {
		...
		function run( len ) {
			...
			var Tr = schemes.realizeVector( sc, red.taus, TIGHT_VEC );
