...
test( 'main export computes a valid RRR for a 4x4 cluster', function t() {
	...
	var N = 4;
	info = dlarrf( N, d, 1, l, 1, ld, 1, 1, 4, w, 1, wgap, 1, werr, 1, 4.0, 1.0, 1.0, PIVMIN, sigma, dplus, 1, lplus, 1, work, 1 ); // eslint-disable-line max-len
...
test( 'ndarray computes a valid RRR for a 4x4 cluster', function t() {
	...
	var w = new Float64Array( [ 0.95, 1.95, 2.95, 4.05 ] );
	var N = 4;
	info = dlarrf.ndarray( N, d, 1, 0, l, 1, 0, ld, 1, 0, 1, 4, w, 1, 0, wgap, 1, 0, werr, 1, 0, 4.0, 1.0, 1.0, PIVMIN, sigma, dplus, 1, 0, lplus, 1, 0, work, 1, 0 ); // eslint-disable-line max-len
	assert.strictEqual( info, 0, 'info is 0 on success' );
	assert.ok( sigma[ 0 ] >= 0.9 && sigma[ 0 ] <= 1.0, 'sigma is in the lower gap' ); // eslint-disable-line max-len
