...
test( 'zspcon: N=0 quick return', function t() {
	...
	rcond[ 0 ] = 0.5;
	const info = zspcon( 'upper', 0, new Complex128Array( 0 ), 1, 0, new Int32Array( 0 ), 1, 0, 1.0, rcond, new Complex128Array( 0 ), 1, 0 );
...
test( 'zspcon: anorm=0 returns rcond=0 (upper)', function t() {
	...
	const rcond = new Float64Array( 1 );
	rcond[ 0 ] = 0.5;
	const info = zspcon( 'upper', N, AP, 1, 0, IPIV, 1, 0, 0.0, rcond, new Complex128Array( 2 * N ), 1, 0 );
	assert.strictEqual( info, 0 );
...
test( 'zspcon: identity 3x3 upper (rcond=1)', function t() {
	...
	const anorm = packedNorm1SymUpper( AP, N );
	const IPIV = new Int32Array( N );
	zsptrf( 'upper', N, AP, 1, 0, IPIV, 1, 0 );
	const rcond = new Float64Array( 1 );
	const info = zspcon( 'upper', N, AP, 1, 0, IPIV, 1, 0, anorm, rcond, new Complex128Array( 2 * N ), 1, 0 );
	assert.strictEqual( info, 0 );
...
test( 'zspcon: identity 4x4 lower (rcond=1)', function t() {
	...
	const anorm = packedNorm1SymLower( AP, N );
	const IPIV = new Int32Array( N );
	zsptrf( 'lower', N, AP, 1, 0, IPIV, 1, 0 );
	const rcond = new Float64Array( 1 );
	const info = zspcon( 'lower', N, AP, 1, 0, IPIV, 1, 0, anorm, rcond, new Complex128Array( 2 * N ), 1, 0 );
	assert.strictEqual( info, 0 );
	assert.ok( close( rcond[ 0 ], 1.0, 1e-9 ) );
});
...
test( 'zspcon: singular matrix - zero diagonal in factored D (upper)', function t() {
	...
	const rcond = new Float64Array( 1 );
	rcond[ 0 ] = 0.5;
	const info = zspcon( 'upper', N, AP, 1, 0, IPIV, 1, 0, 1.0, rcond, new Complex128Array( 2 * N ), 1, 0 );
	assert.strictEqual( info, 0 );
	assert.strictEqual( rcond[ 0 ], 0.0 );
});
...
test( 'zspcon: singular matrix - zero diagonal in factored D (lower)', function t() {
	...
	const rcond = new Float64Array( 1 );
	rcond[ 0 ] = 0.5;
	const info = zspcon( 'lower', N, AP, 1, 0, IPIV, 1, 0, 1.0, rcond, new Complex128Array( 2 * N ), 1, 0 );
	assert.strictEqual( info, 0 );
	assert.strictEqual( rcond[ 0 ], 0.0 );
});
...
test( 'zspcon: N=1 simple', function t() {
	...
	zsptrf( 'upper', N, AP, 1, 0, IPIV, 1, 0 );
	const rcond = new Float64Array( 1 );
	const info = zspcon( 'upper', N, AP, 1, 0, IPIV, 1, 0, 5.0, rcond, new Complex128Array( 2 ), 1, 0 );
	assert.strictEqual( info, 0 );
