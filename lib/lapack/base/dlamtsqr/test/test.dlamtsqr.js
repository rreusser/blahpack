...
test( 'dlamtsqr.ndarray executes a tiny no-trans column-major problem (round-trip)', function t() {
	...
	const info = dlamtsqr( 'column-major', 'left', 'no-transpose', 4, 2, 2, 4, 2, Av, 4, Tv, 2, Cv, 4, WORK, 1 );
...
test( 'dlamtsqr layout: row-major path executes for SIDE=R', function t() {
	...
	const info = dlamtsqr( 'row-major', 'right', 'no-transpose', 3, 4, 2, 8, 2, Av, 4, Tv, 2, Cv, 4, WORK, 1 );
	assert.equal( info, 0, 'info' );
});
