...
function runCase( side, eigsrc, H, N, w, SELECT ) {
	...
	var res = zhsein( side, eigsrc, 'no', SELECT, 1, 0, N, H, 1, LD, 0, w, 1, 0, VL, 1, LD, 0, VR, 1, LD, 0, LD, 0, WORK, 1, 0, RWORK, 1, 0, IFAILL, 1, 0, IFAILR, 1, 0 ); // eslint-disable-line max-len
	return {
		'info': res.info,
		'm': res.m,
		'VL': VL,
...
test( 'zhsein: right_selective_5x5', function t() {
	...
	w = makeW( [ [ 1.0, 0.0 ], [ 2.0, 1.0 ], [ 3.0, -1.0 ], [ 4.0, 0.5 ], [ 5.0, -0.5 ] ] ); // eslint-disable-line max-len
...
test( 'zhsein: both_fromqr_block_5x5', function t() {
	...
	w = makeW( [ [ 2.0, 0.5 ], [ 3.0, -0.5 ], [ 4.0, 1.0 ], [ 5.0, -1.0 ], [ 6.0, 0.5 ] ] ); // eslint-disable-line max-len
...
test( 'zhsein: N=0 returns immediately', function t() {
	var res = runCase( 'both', 'no', new Complex128Array( 1 ), 0, new Complex128Array( 1 ), new Uint8Array( [ 0 ] ) ); // eslint-disable-line max-len
...
test( 'zhsein: invalid initv returns info=-3', function t() {
	...
	var res = zhsein( 'right', 'no', 'bad', SELECT, 1, 0, 1, H, 1, LD, 0, w, 1, 0, VL, 1, LD, 0, VR, 1, LD, 0, LD, 0, WORK, 1, 0, RWORK, 1, 0, IFAILL, 1, 0, IFAILR, 1, 0 ); // eslint-disable-line max-len
...
test( 'zhsein: mm < m returns info=-13', function t() {
	...
	var IFAILL = new Int32Array( LD );
	var IFAILR = new Int32Array( LD );
	var res = zhsein( 'right', 'no', 'no', SELECT, 1, 0, 2, H, 1, LD, 0, w, 1, 0, VL, 1, LD, 0, VR, 1, LD, 0, 1, 0, WORK, 1, 0, RWORK, 1, 0, IFAILL, 1, 0, IFAILR, 1, 0 ); // eslint-disable-line max-len
