...
import zlaWwaddw from './../lib/zla_wwaddw.js';
...
test( 'the function supports negative strides', function t5() {
	const x = new Complex128Array( [ 3, 4, 1, 2 ] );
	const y = new Complex128Array( [ 0.3, 0.4, 0.1, 0.2 ] );
	const w = new Complex128Array( [ 30, 40, 10, 20 ] );
	zlaWwaddw( 2, x, -1, y, -1, w, -1 );
	const xv = reinterpret( x, 0 );
...
test( 'the function returns x for n = 0', function t6() {
	...
	const r = zlaWwaddw( 0, x, 1, y, 1, w, 1 );
	const xv = reinterpret( x, 0 );
