...
test( 'the function returns 0 for a quick-return case (M=0)', function t() {
	const info = dlatsqr( 'column-major', 0, 0, 1, 1, new Float64Array( 1 ), 1, new Float64Array( 1 ), 1, new Float64Array( 1 ) );
...
test( 'the function works in column-major layout for a small TSQR case', function t() {
	...
	const A = new Float64Array( [ 1.0, 2.0, 3.0, 4.0, 0.5, 1.5, 2.5, 3.5 ] );
	const T = new Float64Array( 4 );
	const WORK = new Float64Array( 4 );
	const info = dlatsqr( 'column-major', 4, 2, 8, 2, A, 4, T, 2, WORK );
	assert.strictEqual( info, 0, 'returns 0' );
});
