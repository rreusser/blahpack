/**
* Property-based validation for dpbrfs, following the /blahpack-validate process.
*
* Step 0 classification: `d` -> real scalar; `pb` -> SPD BANDED (schemes.banded
* with half-bandwidth kd, logical.positiveDefiniteBanded — a real symmetric PD
* band matrix under conj=identity); `rfs` (iterative refinement + error bounds)
* -> three independent properties. dpbrfs refines an approximate solution X to
* A*X = B and returns FERR (forward-error bound) and BERR (componentwise
* backward error). The Cholesky band factor AFB is produced by the
* already-validated dpbtrf; the un-refined initial X by dpbtrs; the TRUE solution
* independently by the trusted dpbsv. We assert, against the ORIGINAL full
* symmetric band A0 (both triangles reconstructed by the oracle):
...
* Only the uplo triangle of AB / AFB is realized (band storage); everything
* outside the band and the opposite triangle stays poisoned, so a read of the
* wrong triangle / out of band trips a NaN.
*/
...
import dpbrfs from './../lib/ndarray.js';
import dpbtrf from '../../dpbtrf/lib/ndarray.js';
import dpbtrs from '../../dpbtrs/lib/ndarray.js';
...
var TIGHT_D = schemes.dense.layouts()[ 0 ];  // tight col-major
...
// Independent TRUE solution: solve A0*X = B0 on fresh copies with trusted dpbsv.
function trueSolution( A0, B0, N, kd, nrhs, uplo ) {
	...
	var info = dpbsv( uplo, N, kd, nrhs, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], Br.data, Br.args[ 0 ], Br.args[ 1 ], Br.args[ 2 ] );
	if ( info !== 0 ) {
...
test( 'dpbrfs: refinement residual + BERR + FERR bound (uplo x N x kd x nrhs)', function t() {
	UPLOS.forEach( function eachUplo( uplo ) {
		NS.forEach( function eachN( N ) {
			KDS.forEach( function eachKd( kd0 ) {
				var kd = Math.min( kd0, N - 1 );
...
function runProperty( uplo, N, kd, nrhs ) {
	...
	var A0 = logical.positiveDefiniteBanded( sc, rng, N, kd ); // full SPD band oracle
	var B0 = logical.general( sc, rng, N, nrhs );

	// AB = original SPD band (uplo triangle); AFB = its Cholesky band factor:
	var Ar = schemes.banded.realize( sc, A0, { 'part': uplo, 'k': kd }, TIGHT_B );
	var AFr = schemes.banded.realize( sc, A0, { 'part': uplo, 'k': kd }, TIGHT_B );
	dpbtrf( uplo, N, kd, AFr.data, AFr.args[ 0 ], AFr.args[ 1 ], AFr.args[ 2 ] );

	// B (RHS, unchanged) and X (initial un-refined solve, refined in place):
	var Br = schemes.dense.realize( sc, B0, { 'part': 'full' }, TIGHT_D );
	var Xr = schemes.dense.realize( sc, B0, { 'part': 'full' }, TIGHT_D );
	dpbtrs( uplo, N, kd, nrhs, AFr.data, AFr.args[ 0 ], AFr.args[ 1 ], AFr.args[ 2 ], Xr.data, Xr.args[ 0 ], Xr.args[ 1 ], Xr.args[ 2 ] );

	var FERR = new Float64Array( nrhs );
	var BERR = new Float64Array( nrhs );
	var WORK = new Float64Array( 3 * N );
	var IWORK = new Int32Array( N );
	dpbrfs( uplo, N, kd, nrhs, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], AFr.data, AFr.args[ 0 ], AFr.args[ 1 ], AFr.args[ 2 ], Br.data, Br.args[ 0 ], Br.args[ 1 ], Br.args[ 2 ], Xr.data, Xr.args[ 0 ], Xr.args[ 1 ], Xr.args[ 2 ], FERR, 1, 0, BERR, 1, 0, WORK, 1, 0, IWORK, 1, 0 );

	var Xtrue = trueSolution( A0, B0, N, kd, nrhs, uplo );

	var tag = 'dpbrfs '+uplo+' N='+N+' kd='+kd+' nrhs='+nrhs;

	// (a) residual: refined X remains a valid solution of A0*X = B0.
	checked( 'dpbrfs', 'residual', function run() {
		var j;
		for ( j = 0; j < nrhs; j++ ) {
	...
	checked( 'dpbrfs', 'structural', function run() {
		var xcol;
		var tcol;
		var berrCap;
		var eActual;
...
// fixed A0 and B0; dpbrfs's internal kernels (dsbmv, dpbtrs, daxpy, dcopy,
// dlacn2) walk the band/vectors by stride and do NOT reorder their reductions
...
// strides) and ALL dense layouts for B/X — a single family (as dpbsv itself is).
var aLayouts = schemes.banded.layouts();
var bLayouts = schemes.dense.layouts();

test( 'dpbrfs: bit-exact across storage layouts (single family)', function t() {
	UPLOS.forEach( function eachUplo( uplo ) {
		runInvariance( uplo );
	});
});
...
function runInvariance( uplo ) {
	...
	var AF0 = schemes.banded.realize( sc, A0, { 'part': uplo, 'k': kd }, TIGHT_B );
	dpbtrf( uplo, N, kd, AF0.data, AF0.args[ 0 ], AF0.args[ 1 ], AF0.args[ 2 ] );
	var Lfac = readBandFactor( AF0, N, kd, uplo );

	var X0r = schemes.dense.realize( sc, B0, { 'part': 'full' }, TIGHT_D );
	dpbtrs( uplo, N, kd, nrhs, AF0.data, AF0.args[ 0 ], AF0.args[ 1 ], AF0.args[ 2 ], X0r.data, X0r.args[ 0 ], X0r.args[ 1 ], X0r.args[ 2 ] );
	var Xinit = readMat( X0r, N, nrhs );

	checked( 'dpbrfs', 'layout-invariance', function run() {
		layoutInvariant( aLayouts, function build( aL, idx ) {
			var Ar = schemes.banded.realize( sc, A0, { 'part': uplo, 'k': kd }, aL );
			var AFr = schemes.banded.realize( sc, Lfac, { 'part': uplo, 'k': kd }, aL );
			...
			return out;
		}, { 'label': 'dpbrfs '+uplo+' layout invariance' } );
	});
}
