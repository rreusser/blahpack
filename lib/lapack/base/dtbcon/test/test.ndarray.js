...
test( 'dtbcon: N=0 quick return (rcond=1)', function t() {
	const rcond = new Float64Array( 1 );
	rcond[ 0 ] = 0.5;
	const info = dtbcon( 'one-norm', 'upper', 'non-unit', 0, 0, new Float64Array( 0 ), 1, 1, 0, rcond, new Float64Array( 0 ), 1, 0, new Int32Array( 0 ), 1, 0 );
...
test( 'dtbcon: zero matrix returns rcond=0', function t() {
	const N = 3;
	const KD = 0;
	const AB = new Float64Array( (KD+1)*N );
	const rcond = new Float64Array( 1 );
	rcond[ 0 ] = 0.5;
	const work = new Float64Array( 3 * N );
	const iwork = new Int32Array( N );
	const info = dtbcon( 'one-norm', 'upper', 'non-unit', N, KD, AB, 1, KD+1, 0, rcond, work, 1, 0, iwork, 1, 0 );
	assert.strictEqual( info, 0 );
...
test( 'dtbcon: identity 3x3 KD=0 upper non-unit (inf-norm)', function t() {
	...
	const work = new Float64Array( 3 * N );
	const iwork = new Int32Array( N );
	const info = dtbcon( 'inf-norm', 'upper', 'non-unit', N, KD, AB, 1, KD+1, 0, rcond, work, 1, 0, iwork, 1, 0 );
	assert.strictEqual( info, 0 );
	assert.ok( close( rcond[ 0 ], 1.0, 1e-9 ) );
});
...
test( 'dtbcon: upper unit KD=1 4x4 (one-norm)', function t() {
	...
	const work = new Float64Array( 3 * N );
	const iwork = new Int32Array( N );
	const info = dtbcon( 'one-norm', 'upper', 'unit', N, KD, AB, 1, KD+1, 0, rcond, work, 1, 0, iwork, 1, 0 );
	assert.strictEqual( info, 0 );
...
test( 'dtbcon: upper unit KD=1 4x4 (inf-norm)', function t() {
	...
	const work = new Float64Array( 3 * N );
	const iwork = new Int32Array( N );
	const info = dtbcon( 'inf-norm', 'upper', 'unit', N, KD, AB, 1, KD+1, 0, rcond, work, 1, 0, iwork, 1, 0 );
	assert.strictEqual( info, 0 );
	assert.ok( rcond[ 0 ] > 0.0 && rcond[ 0 ] <= 1.0 );
});
...
test( 'dtbcon: lower non-unit diagonal KD=0 4x4 (one-norm)', function t() {
	...
	const AB = packLowerBand( A, N, KD );
	const rcond = new Float64Array( 1 );
	const work = new Float64Array( 3 * N );
	const iwork = new Int32Array( N );
	const info = dtbcon( 'one-norm', 'lower', 'non-unit', N, KD, AB, 1, KD+1, 0, rcond, work, 1, 0, iwork, 1, 0 );
	assert.strictEqual( info, 0 );
...
test( 'dtbcon: lower non-unit diagonal KD=0 4x4 (inf-norm)', function t() {
	...
	const work = new Float64Array( 3 * N );
	const iwork = new Int32Array( N );
	const info = dtbcon( 'inf-norm', 'lower', 'non-unit', N, KD, AB, 1, KD+1, 0, rcond, work, 1, 0, iwork, 1, 0 );
	assert.strictEqual( info, 0 );
	assert.ok( close( rcond[ 0 ], 0.25, 1e-9 ) );
});
...
test( 'dtbcon: lower unit KD=1 4x4 (one-norm)', function t() {
	...
	const work = new Float64Array( 3 * N );
	const iwork = new Int32Array( N );
	const info = dtbcon( 'one-norm', 'lower', 'unit', N, KD, AB, 1, KD+1, 0, rcond, work, 1, 0, iwork, 1, 0 );
	assert.strictEqual( info, 0 );
	assert.ok( rcond[ 0 ] > 0.0 && rcond[ 0 ] <= 1.0 );
});
...
test( 'dtbcon: lower unit KD=1 4x4 (inf-norm)', function t() {
	...
	const work = new Float64Array( 3 * N );
	const iwork = new Int32Array( N );
	const info = dtbcon( 'inf-norm', 'lower', 'unit', N, KD, AB, 1, KD+1, 0, rcond, work, 1, 0, iwork, 1, 0 );
	assert.strictEqual( info, 0 );
	assert.ok( rcond[ 0 ] > 0.0 && rcond[ 0 ] <= 1.0 );
});
