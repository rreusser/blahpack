...
function runCase( name, transr, uplo ) {
	...
	const N = tc.n;
	const ARF = new Float64Array( tc.ARF );
	const AP = new Float64Array( tc.AP.length );
	const expected = new Float64Array( tc.AP );
	const info = dtfttp( transr, uplo, N, ARF, 1, 0, AP, 1, 0 );
...
test( 'dtfttp: N=0 quick return', function t() {
	...
	const AP = new Float64Array( [ -1.0, -1.0, -1.0 ] );
	const info = dtfttp( 'no-transpose', 'lower', 0, new Float64Array( 0 ), 1, 0, AP, 1, 0 ); // eslint-disable-line max-len
...
test( 'dtfttp: N=1, normal', function t() {
	...
	const tc = n1N;
	const ARF = new Float64Array( [ 42.0 ] );
	const AP = new Float64Array( 1 );
	const info = dtfttp( 'no-transpose', 'lower', 1, ARF, 1, 0, AP, 1, 0 );
...
test( 'dtfttp: N=1, transpose', function t() {
	...
	const tc = n1T;
	const ARF = new Float64Array( [ 99.0 ] );
	const AP = new Float64Array( 1 );
	const info = dtfttp( 'transpose', 'upper', 1, ARF, 1, 0, AP, 1, 0 );
	assert.equal( info, 0, 'info' );
	assert.deepStrictEqual( AP, new Float64Array( tc.AP ), 'AP' );
});
...
test( 'dtfttp: N=5, no-transpose, lower, strideARF=2', function t() {
	...
	const expected = new Float64Array( tc.AP );
	const N = tc.n;
	const ARF = new Float64Array( tc.ARF.length * 2 );
	for ( i = 0; i < tc.ARF.length; i += 1 ) {
		ARF[ i * 2 ] = tc.ARF[ i ];
	}
	const AP = new Float64Array( tc.AP.length );
	const info = dtfttp( 'no-transpose', 'lower', N, ARF, 2, 0, AP, 1, 0 );
	assert.equal( info, 0, 'info' );
...
test( 'dtfttp: N=5, no-transpose, lower, strideAP=2', function t() {
	...
	const ARF = new Float64Array( tc.ARF );
	const N = tc.n;
	const AP = new Float64Array( tc.AP.length * 2 );
	const info = dtfttp( 'no-transpose', 'lower', N, ARF, 1, 0, AP, 2, 0 );
	assert.equal( info, 0, 'info' );
...
test( 'dtfttp: N=6, transpose, upper, with offset', function t() {
	...
	const tc = n6TU;
	const N = tc.n;
	const ARF = new Float64Array( tc.ARF.length + 3 );
	for ( i = 0; i < tc.ARF.length; i += 1 ) {
		ARF[ i + 3 ] = tc.ARF[ i ];
	}
	const AP = new Float64Array( tc.AP.length + 5 );
	const expected = new Float64Array( tc.AP );
	const info = dtfttp( 'transpose', 'upper', N, ARF, 1, 3, AP, 1, 5 );
	assert.equal( info, 0, 'info' );
	for ( i = 0; i < tc.AP.length; i += 1 ) {
