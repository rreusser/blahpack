...
import zlaic1 from './../lib/ndarray.js';


// FIXTURES //

const fixtureDir = path.join( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures' );
const lines = readFileSync( path.join( fixtureDir, 'zlaic1.jsonl' ), 'utf8' ).trim().split( '\n' ); // eslint-disable-line max-len, node/no-sync
...
function runCase( caseName, job, j, xVals, wVals, sest, gammaR, gammaI ) { // eslint-disable-line max-params
	...
	const x = new Complex128Array( xVals );
	const w = new Complex128Array( wVals );
	const gamma = new Complex128( gammaR, gammaI );
