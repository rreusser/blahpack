...
function drive( uplo, N, nrhs, A0, B0, apLayout, bLayout ) {
	...
	const AFr = schemes.packed.realize( sc, A0, { 'part': uplo }, apLayout ); // overwritten by dcopy+dpptrf
	const Br = schemes.dense.realize( sc, B0, { 'part': 'full' }, bLayout );
	const Xr = schemes.dense.realize( sc, B0, { 'part': 'full' }, bLayout ); // overwritten by dlacpy+dpptrs
	const equed = [ '' ];
	...
	const info = dppsvx( 'not-factored', uplo, N, nrhs, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], AFr.data, AFr.args[ 0 ], AFr.args[ 1 ], equed, s, 1, 0, Br.data, Br.args[ 0 ], Br.args[ 1 ], Br.args[ 2 ], Xr.data, Xr.args[ 0 ], Xr.args[ 1 ], Xr.args[ 2 ], rcond, FERR, 1, 0, BERR, 1, 0, WORK, 1, 0, IWORK, 1, 0 );
	return {
		'Xr': Xr,
		'rcond': rcond,
		'FERR': FERR,
...
function runProperty( uplo, N, nrhs ) {
	...
	const tag = 'dppsvx '+uplo+' N='+N+' nrhs='+nrhs;
