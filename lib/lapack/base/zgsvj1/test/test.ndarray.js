...
test( 'ndarray: matches fixture novec_4x3_n1_1 on unit stride', function t() {
	...
	info = ndarrayFn( 'no-v', M, N, 1, a, 1, M, 0, d, 1, 0, sva, 1, 0, 0, V, 1, 1, 0, EPS, SFMIN, TOL, 5, work, 1, 0 ); // eslint-disable-line max-len
...
test( 'ndarray: matches fixture vec_5x4_n1_2 with compute-v', function t() {
	...
	work = new Complex128Array( M );
	info = ndarrayFn( 'compute-v', M, N, 2, a, 1, M, 0, d, 1, 0, sva, 1, 0, 0, V, 1, N, 0, EPS, SFMIN, TOL, 5, work, 1, 0 ); // eslint-disable-line max-len
...
test( 'ndarray: honors offsetA into a larger A buffer (apply_4x3_n1_1)', function t() { // eslint-disable-line max-len
	...
	work = new Complex128Array( M );
	info = ndarrayFn( 'apply-v', M, N, 1, a, 1, M, padA, d, 1, 0, sva, 1, 0, 3, V, 1, 3, 0, EPS, SFMIN, TOL, 3, work, 1, 0 ); // eslint-disable-line max-len
	aSlice = [];
	for ( i = 0; i < 2 * M * N; i++ ) {
...
test( 'ndarray: non-unit strideD and strideSVA with offsets', function t() {
	...
	V = new Complex128Array( 1 );
	work = new Complex128Array( M );
	info = ndarrayFn( 'no-v', M, N, 1, a, 1, M, 0, d, 2, 1, sva, 3, 2, 0, V, 1, 1, 0, EPS, SFMIN, TOL, 5, work, 1, 0 ); // eslint-disable-line max-len
	assertArrayClose( toRePairs( a ), tc.a, 1e-12, 'a' );
	for ( i = 0; i < N; i++ ) {
...
test( 'ndarray: non-unit work stride with offset (novec_4x3_n1_1)', function t() { // eslint-disable-line max-len
	...
	info = ndarrayFn( 'no-v', M, N, 1, a, 1, M, 0, d, 1, 0, sva, 1, 0, 0, V, 1, 1, 0, EPS, SFMIN, TOL, 5, work, 2, 3 ); // eslint-disable-line max-len
	assertArrayClose( toRePairs( a ), tc.a, 1e-12, 'a' );
	assertArrayClose( toRePairs( d ), tc.d, 1e-12, 'd' );
	assertArrayClose( toArray( sva ), tc.sva, 1e-12, 'sva' );
	assert.equal( info, tc.info, 'info' );
...
test( 'ndarray: 14x14 block-tiled (vec_14x14_block)', function t() {
	...
	work = new Complex128Array( M );
	info = ndarrayFn( 'compute-v', M, N, 5, a, 1, M, 0, d, 1, 0, sva, 1, 0, 0, V, 1, N, 0, EPS, SFMIN, TOL, 4, work, 1, 0 ); // eslint-disable-line max-len
...
test( 'ndarray: n1=N (no off-diagonal block — early convergence)', function t() { // eslint-disable-line max-len
	...
	V = new Complex128Array( 1 );
	work = new Complex128Array( M );
	info = ndarrayFn( 'no-v', M, N, N, a, 1, M, 0, d, 1, 0, sva, 1, 0, 0, V, 1, 1, 0, EPS, SFMIN, TOL, 3, work, 1, 0 ); // eslint-disable-line max-len
	assertArrayClose( toRePairs( a ), tc.a, 1e-12, 'a' );
	assertArrayClose( toRePairs( d ), tc.d, 1e-12, 'd' );
	assertArrayClose( toArray( sva ), tc.sva, 1e-12, 'sva' );
	assert.equal( info, tc.info, 'info' );
...
test( 'ndarray: n1=0 (no first block — immediate convergence)', function t() {
	...
	V = new Complex128Array( 1 );
	work = new Complex128Array( M );
	info = ndarrayFn( 'no-v', M, N, 0, a, 1, M, 0, d, 1, 0, sva, 1, 0, 0, V, 1, 1, 0, EPS, SFMIN, TOL, 2, work, 1, 0 ); // eslint-disable-line max-len
	assertArrayClose( toRePairs( a ), tc.a, 1e-12, 'a' );
	assertArrayClose( toRePairs( d ), tc.d, 1e-12, 'd' );
	assertArrayClose( toArray( sva ), tc.sva, 1e-12, 'sva' );
	assert.equal( info, tc.info, 'info' );
...
test( 'ndarray: returns -2 for negative M', function t() {
	var info = ndarrayFn( 'no-v', -1, 1, 0, new Complex128Array( 1 ), 1, 1, 0, new Complex128Array( 1 ), 1, 0, new Float64Array( 1 ), 1, 0, 0, new Complex128Array( 1 ), 1, 1, 0, EPS, SFMIN, TOL, 1, new Complex128Array( 1 ), 1, 0 ); // eslint-disable-line max-len
	assert.equal( info, -2 );
});

test( 'ndarray: returns -3 when N > M', function t() {
	var info = ndarrayFn( 'no-v', 2, 3, 0, new Complex128Array( 6 ), 1, 2, 0, new Complex128Array( 3 ), 1, 0, new Float64Array( 3 ), 1, 0, 0, new Complex128Array( 1 ), 1, 1, 0, EPS, SFMIN, TOL, 1, new Complex128Array( 2 ), 1, 0 ); // eslint-disable-line max-len
	assert.equal( info, -3 );
});

test( 'ndarray: returns -4 for negative n1', function t() {
	var info = ndarrayFn( 'no-v', 2, 2, -1, new Complex128Array( 4 ), 1, 2, 0, new Complex128Array( 2 ), 1, 0, new Float64Array( 2 ), 1, 0, 0, new Complex128Array( 1 ), 1, 1, 0, EPS, SFMIN, TOL, 1, new Complex128Array( 2 ), 1, 0 ); // eslint-disable-line max-len
	assert.equal( info, -4 );
});

test( 'ndarray: returns -15 for negative mv with apply-v', function t() {
	var info = ndarrayFn( 'apply-v', 2, 2, 1, new Complex128Array( 4 ), 1, 2, 0, new Complex128Array( 2 ), 1, 0, new Float64Array( 2 ), 1, 0, -1, new Complex128Array( 4 ), 1, 2, 0, EPS, SFMIN, TOL, 1, new Complex128Array( 2 ), 1, 0 ); // eslint-disable-line max-len
	assert.equal( info, -15 );
});

test( 'ndarray: returns -21 for tol <= eps', function t() {
	var info = ndarrayFn( 'no-v', 2, 2, 1, new Complex128Array( 4 ), 1, 2, 0, new Complex128Array( 2 ), 1, 0, new Float64Array( 2 ), 1, 0, 0, new Complex128Array( 1 ), 1, 1, 0, EPS, SFMIN, EPS, 1, new Complex128Array( 2 ), 1, 0 ); // eslint-disable-line max-len
...
test( 'ndarray: returns -28 for undersized work', function t() {
	var info = ndarrayFn( 'no-v', 2, 2, 1, new Complex128Array( 4 ), 1, 2, 0, new Complex128Array( 2 ), 1, 0, new Float64Array( 2 ), 1, 0, 0, new Complex128Array( 1 ), 1, 1, 0, EPS, SFMIN, TOL, 1, new Complex128Array( 1 ), 1, 0 ); // eslint-disable-line max-len
...
test( 'ndarray: returns nsweep-1 when not converged', function t() {
	...
	work = new Complex128Array( M );
	info = ndarrayFn( 'compute-v', M, N, 5, a, 1, M, 0, d, 1, 0, sva, 1, 0, 0, V, 1, N, 0, EPS, SFMIN, TOL, 1, work, 1, 0 ); // eslint-disable-line max-len
...
test( 'ndarray: handles zero-norm column sentinel (aapp = 0)', function t() {
	...
	V = new Complex128Array( 1 );
	work = new Complex128Array( M );
	info = ndarrayFn( 'no-v', M, N, 2, a, 1, M, 0, d, 1, 0, sva, 1, 0, 0, V, 1, 1, 0, EPS, SFMIN, TOL, 3, work, 1, 0 ); // eslint-disable-line max-len
	assert.equal( typeof info, 'number', 'returns a number' );
	for ( i = 0; i < N; i++ ) {
...
test( 'ndarray: small-norm branch (aaqq < 1)', function t() {
	...
	work = new Complex128Array( M );
	info = ndarrayFn( 'compute-v', M, N, 2, a, 1, M, 0, d, 1, 0, sva, 1, 0, 0, V, 1, N, 0, EPS, SFMIN, TOL, 4, work, 1, 0 ); // eslint-disable-line max-len
	assert.equal( typeof info, 'number', 'returns a number' );
	for ( i = 0; i < N; i++ ) {
...
test( 'ndarray: large-norm branch (aaqq >= big/aapp triggers ZCOPY+ZLASCL)', function t() { // eslint-disable-line max-len
	...
	V = new Complex128Array( 1 );
	work = new Complex128Array( M );
	info = ndarrayFn( 'no-v', M, N, 1, a, 1, M, 0, d, 1, 0, sva, 1, 0, 0, V, 1, 1, 0, EPS, SFMIN, TOL, 3, work, 1, 0 ); // eslint-disable-line max-len
	assert.equal( typeof info, 'number', 'returns a number' );
});
