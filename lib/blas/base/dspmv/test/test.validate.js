/**
* Property-based validation for dspmv, following the /blahpack-validate process.
*
* Step 0 classification: `d` -> real scalar; `sp` -> symmetric packed
* (schemes.packed, logical.symmetric); matrix-vector `y = alpha*A*x + beta*y`
...
// alpha/beta combinations, including the beta=0 (overwrite) and beta=1
// (accumulate) special paths, plus a general random pair per (uplo, n).
function scalarCombos( rng ) {
	return [
		[ sc.random( rng ), 0.0 ], // beta = 0
		[ sc.random( rng ), 1.0 ], // beta = 1
		[ sc.random( rng ), sc.random( rng ) ] // general
	];
}
...
test( 'dspmv: symmetric packed matrix-vector residual (size sweep x uplo x alpha/beta)', function t() {
	[ 'upper', 'lower' ].forEach( function eachUplo( uplo ) {
		SIZES.forEach( function eachN( n ) {
			...
			scalarCombos( rng ).forEach( function eachAB( ab ) {
				...
				dspmv( uplo, n, sc.apiScalar( alpha ), AP.data, AP.args[ 0 ], AP.args[ 1 ], X.data, X.args[ 0 ], X.args[ 1 ], sc.apiScalar( beta ), Y.data, Y.args[ 0 ], Y.args[ 1 ] );
				var expected = [];
				var got = [];
				for ( i = 0; i < n; i++ ) {
				...
				checked( 'dspmv', 'residual', function run() {
					assertVecClose( got, expected, n, 'dspmv '+uplo+' n='+n+' beta='+beta );
				});
			});
		});
	});
