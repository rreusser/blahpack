...
import dpftrf from '../../dpftrf/lib/base.js';
...
function runTest( name, transr, uplo, N ) {
	...
	const expected = tc.a;
	const actual = toArray( a, expected.length );
...
test( 'dpftri: n_zero', function t() {
	...
	const info = dpftri( 'no-transpose', 'lower', 0, a, 1, 0 );
...
test( 'dpftri: singular (info > 0)', function t() {
	...
	const tc = singular;
	const a = new Float64Array( [ 1.0, 0.5, 0.3, 0.0, 1.0, 0.2 ] );
	const info = dpftri( 'no-transpose', 'lower', 3, a, 1, 0 );
