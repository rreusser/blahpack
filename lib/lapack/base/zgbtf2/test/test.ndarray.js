...
test( 'zgbtf2: 4x4 tridiagonal (KL=1, KU=1)', function t() {
	...
	const info = zgbtf2( 4, 4, 1, 1, AB, 1, 4, 0, IPIV, 1, 0 );
...
test( 'zgbtf2: 3x3 with KL=1, KU=2', function t() {
	...
	const info = zgbtf2( 3, 3, 1, 2, AB, 1, 5, 0, IPIV, 1, 0 );
	assert.strictEqual( info, tc.info );
	assertArrayClose( toArray( ABv ), tc.AB, 1e-10, 'AB' );
	for ( i = 0; i < tc.ipiv.length; i++ ) {
		assert.strictEqual( IPIV[ i ], tc.ipiv[ i ] - 1, 'ipiv[' + i + ']' );
...
test( 'zgbtf2: N=0 quick return', function t() {
	...
	const info = zgbtf2( 3, 0, 1, 1, AB, 1, 4, 0, IPIV, 1, 0 );
	assert.strictEqual( info, tc.info );
});
...
test( 'zgbtf2: M=0 quick return', function t() {
	...
	const AB = new Complex128Array( 4 );
	const IPIV = new Int32Array( 1 );
	const info = zgbtf2( 0, 3, 1, 1, AB, 1, 4, 0, IPIV, 1, 0 );
	assert.strictEqual( info, tc.info );
});
...
test( 'zgbtf2: 1x1 matrix', function t() {
	...
	const IPIV = new Int32Array( 1 );
	const info = zgbtf2( 1, 1, 0, 0, AB, 1, 1, 0, IPIV, 1, 0 );
	assert.strictEqual( info, tc.info );
...
test( 'zgbtf2: singular matrix', function t() {
	...
	const info = zgbtf2( 2, 2, 0, 1, AB, 1, 2, 0, IPIV, 1, 0 );
	assert.strictEqual( info, tc.info );
});
...
test( 'zgbtf2: 5x3 tall matrix (M > N)', function t() {
	...
	const tc = tall_5x3;
	const AB = new Complex128Array( 4 * 3 );
	const ABv = reinterpret( AB, 0 );
	ABv[ (2 + 0 * 4) * 2 ] = 4;
	ABv[ (2 + 0 * 4) * 2 + 1 ] = 1;
	ABv[ (3 + 0 * 4) * 2 ] = -1;
	...
	ABv[ (3 + 2 * 4) * 2 + 1 ] = 0;
	const IPIV = new Int32Array( 3 );
	const info = zgbtf2( 5, 3, 1, 1, AB, 1, 4, 0, IPIV, 1, 0 );
	assert.strictEqual( info, tc.info );
	assertArrayClose( toArray( ABv ), tc.AB, 1e-10, 'AB' );
	for ( i = 0; i < tc.ipiv.length; i++ ) {
		assert.strictEqual( IPIV[ i ], tc.ipiv[ i ] - 1, 'ipiv[' + i + ']' );
...
test( 'zgbtf2: pivoting 2x2', function t() {
	...
	ABv[ (2 + 1 * 4) * 2 + 1 ] = 0;
	const IPIV = new Int32Array( 2 );
	const info = zgbtf2( 2, 2, 1, 1, AB, 1, 4, 0, IPIV, 1, 0 );
	assert.strictEqual( info, tc.info );
	assertArrayClose( toArray( ABv ), tc.AB, 1e-10, 'AB' );
	for ( i = 0; i < tc.ipiv.length; i++ ) {
		assert.strictEqual( IPIV[ i ], tc.ipiv[ i ] - 1, 'ipiv[' + i + ']' );
