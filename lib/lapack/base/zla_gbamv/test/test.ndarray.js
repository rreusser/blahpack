...
import zla_gbamv from './../lib/ndarray.js';


// FIXTURES //

const fixtureDir = path.join( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures' );
const lines = readFileSync( path.join( fixtureDir, 'zla_gbamv.jsonl' ), 'utf8' ).trim().split( '\n' ); // eslint-disable-line node/no-sync
...
test( 'zla_gbamv: trans_basic (4x4 band, alpha=1 beta=1)', function t() {
	const base = findCase( 'notrans_basic' );
	const tc = findCase( 'trans_basic' );
	const AB = new Complex128Array( base.AB );
	const x = new Complex128Array( base.x );
	const y = new Float64Array([
		1.0,
		-2.0,
		3.0,
...
test( 'zla_gbamv: beta_one_alpha_zero (quick return path)', function t() {
	const tc = findCase( 'beta_one_alpha_zero' );
...
test( 'zla_gbamv: alpha_beta_scale (complex values, alpha=2 beta=0.5)', function t() {
	const tc = findCase( 'alpha_beta_scale' );
	const AB = new Complex128Array([
		0.0,
		0.0,
		1.0,
...
test( 'zla_gbamv: neg_incx_incy (INCX=-2, INCY=-2)', function t() {
	const tc = findCase( 'neg_incx_incy' );
	const AB = new Complex128Array([
		0.0,
		0.0,
		1.0,
...
test( 'zla_gbamv: kl_zero_ku_zero (diagonal-only)', function t() {
	const tc = findCase( 'kl_zero_ku_zero' );
	const AB = new Complex128Array([
		2.0,
		1.0,
		-1.0,
