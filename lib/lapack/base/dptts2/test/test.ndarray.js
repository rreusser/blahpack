...
import dptts2 from './../lib/ndarray.js';
...
test( 'dptts2: n_eq_1_multi_rhs', function t() {
	const tc = n_eq_1_multi_rhs;
	const d = new Float64Array( [ 4.0 ] );
	const e = new Float64Array( [] );

	// B is 1x2, col-major (LDB=1): [8, 12]
	const B = new Float64Array( [ 8.0, 12.0 ] );
...
test( 'dptts2: returns B', function t() {
	...
	const result = dptts2( 1, 1, d, 1, 0, e, 1, 0, B, 1, 1, 0 );
	assert.equal( result, B );
});

test( 'dptts2: supports offsets', function t() {
	// Put actual data starting at offset 2 for d, 1 for e, 3 for B
	const d = new Float64Array( [ 0.0, 0.0, 4.0, 3.0 ] );
	const e = new Float64Array( [ 0.0, 0.5 ] );
	...
	const B = new Float64Array( [ 0.0, 0.0, 0.0, 8.0, 10.0 ] );
...
test( 'dptts2: supports strides', function t() {
	// D with stride 2: [4, ?, 3]
	const d = new Float64Array( [ 4.0, 999.0, 3.0 ] );
	...
	const B = new Float64Array( [ 8.0, 999.0, 10.0 ] );
