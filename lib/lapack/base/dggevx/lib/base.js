...
import dtgsna from '../../dtgsna/lib/base.js';
...
const SMLNUM_BASE = dlamch( 'safe-minimum' );
const SMLNUM = Math.sqrt( SMLNUM_BASE ) / EPS;
...
* @param {Float64Array} work - caller-provided real workspace; minimum length max(1,8*N) for sense='none', max(1,10*N) for sense='eigenvalues', max(1,2*N*(N+4)+16) for sense='right-vectors' or 'both'; stride-1 assumed
* @param {integer} strideWork - stride for work (must be 1)
* @param {NonNegativeInteger} offsetWork - starting index for work
* @param {Int32Array} iwork - caller-provided integer workspace of length max(1,N+6); not referenced when sense='none' or 'eigenvalues'
* @param {integer} strideIwork - stride for iwork (must be 1)
* @param {NonNegativeInteger} offsetIwork - starting index for iwork
* @param {Uint8Array} bwork - caller-provided logical workspace of length max(1,N); not referenced when sense='none'
...
function dggevx( balanc, jobvl, jobvr, sense, N, A, strideA1, strideA2, offsetA, B, strideB1, strideB2, offsetB, ALPHAR, strideALPHAR, offsetALPHAR, ALPHAI, strideALPHAI, offsetALPHAI, BETA, strideBETA, offsetBETA, VL, strideVL1, strideVL2, offsetVL, VR, strideVR1, strideVR2, offsetVR, LSCALE, strideLSCALE, offsetLSCALE, RSCALE, strideRSCALE, offsetRSCALE, RCONDE, strideRCONDE, offsetRCONDE, RCONDV, strideRCONDV, offsetRCONDV, work, strideWork, offsetWork, iwork, strideIwork, offsetIwork, bwork, strideBwork, offsetBwork ) { // eslint-disable-line max-params
	let iwrk1Len, SNSWORK, workVL, workVR, workCN, snaJob, oVLdst, ilascl;
	let ilbscl, anrmto, bnrmto, chtemp, compvl, compvr, oBsrc, icols, abnrm;
	let bbnrm, pair, jVal, iIdx, oVLI, ierr, info, mm;

	const ilvl = ( jobvl === 'compute-vectors' );
	const ilvr = ( jobvr === 'compute-vectors' );
	const ilv = ilvl || ilvr;
	const wantsn = ( sense === 'none' );
	const wantse = ( sense === 'eigenvalues' );
	const wantsv = ( sense === 'right-vectors' );
	const wantsb = ( sense === 'both' );
	...
	// Work array layout (stride-1 from offsetWork):
	//   [offsetWork   .. offsetWork+N)   TAU (QR reflectors, length N)
	//   [offsetWork+N .. end)            scratch (oSCR): pre-loop scratch for
	//        dgeqrf/dormqr/dorgqr/dhgeqz/dtgevc, then reused for the sense-loop
	//        buffers workVL/workVR/SNSWORK (see the sense loop below).

	// Minimum work size (matches the ndarray/wrapper asserts):
	//   sense='none':                  max(1, 8*N)                  (N TAU + 7N scratch)
	//   sense='eigenvalues':           max(1, 11*N)                 (N TAU + 4N vecs + 6N SNSWORK)
	//   sense='right-vectors'/'both':  max(1, 5*N + 2*N*(N+2)+16)   (N TAU + 4N vecs + dtgsna workspace)

	const oTAU = offsetWork;                   // TAU lives at [offsetWork .. offsetWork+N-1]
	const oSCR = offsetWork + N;              // scratch starts right after TAU

	// Unused-but-required sentinel for dtgevc 'backtransform' mode
	const SELECT = new Uint8Array( 1 );

	// Sense-loop scratch (workVL, workVR, SNSWORK/workCN) is carved from the
	// caller work array inside the loop below; see the layout comment there.

	// Tiny per-eigenvalue output buffers for dtgsna (2 elements each, not Fortran workspace)
	const rcondeS = new Float64Array( 2 );	const rcondvS = new Float64Array( 2 );	const MCNT = new Int32Array( 1 );

	// Scale A if max element outside range [SMLNUM, BIGNUM]
	const anrm = dlange( 'max', N, N, A, strideA1, strideA2, offsetA, work, 1, oSCR );
	ilascl = false;
	anrmto = 0.0;
	...
	const bnrm = dlange( 'max', N, N, B, strideB1, strideB2, offsetB, work, 1, oSCR );
	ilbscl = false;
	bnrmto = 0.0;
	...
	const bal = dggbal( balanc, N, A, strideA1, strideA2, offsetA, B, strideB1, strideB2, offsetB, LSCALE, strideLSCALE, offsetLSCALE, RSCALE, strideRSCALE, offsetRSCALE, work, 1, oSCR );
	...
	abnrm = dlange( 'one-norm', N, N, A, strideA1, strideA2, offsetA, work, 1, oSCR );
	if ( ilascl ) {
		work[ oSCR ] = abnrm;
		dlascl( 'general', 0, 0, anrmto, anrm, 1, 1, work, 1, 1, oSCR );
		abnrm = work[ oSCR ];
	}
	bbnrm = dlange( 'one-norm', N, N, B, strideB1, strideB2, offsetB, work, 1, oSCR );
	if ( ilbscl ) {
		work[ oSCR ] = bbnrm;
		dlascl( 'general', 0, 0, bnrmto, bnrm, 1, 1, work, 1, 1, oSCR );
		bbnrm = work[ oSCR ];
	}
	...
	// QR factorize the submatrix B(ilo:ihi, ilo:N) (or B(ilo:ihi, ilo:ihi)); TAU stored in work[oTAU..]
	dgeqrf( irows, icols, B, strideB1, strideB2, oBI, work, 1, oTAU, work, 1, oSCR );

	// Apply the orthogonal transformation to A: A(ilo:ihi, ilo:...) = Q^T * A(ilo:ihi, ilo:...)
	dormqr( 'left', 'transpose', irows, icols, irows, B, strideB1, strideB2, oBI, work, 1, oTAU, A, strideA1, strideA2, oAI, work, 1, oSCR );
	...
	if ( ilvl ) {
		...
		dorgqr( irows, irows, irows, VL, strideVL1, strideVL2, oVLI, work, 1, oTAU, work, 1, oSCR );
	}
	if ( ilvr ) {
	...
	ierr = dhgeqz( chtemp, compvl, compvr, N, ilo - 1, ihi - 1, A, strideA1, strideA2, offsetA, B, strideB1, strideB2, offsetB, ALPHAR, strideALPHAR, offsetALPHAR, ALPHAI, strideALPHAI, offsetALPHAI, BETA, strideBETA, offsetBETA, VL, strideVL1, strideVL2, offsetVL, VR, strideVR1, strideVR2, offsetVR, work, 1, oSCR );
	if ( ierr !== 0 ) {
		if ( ierr > 0 && ierr <= N ) {
			info = ierr;
	...
	if ( ilv || !wantsn ) {
		if ( ilv ) {
			...
			ierr = dtgevc( chtemp, 'backtransform', SELECT, 1, 0, N, A, strideA1, strideA2, offsetA, B, strideB1, strideB2, offsetB, VL, strideVL1, strideVL2, offsetVL, VR, strideVR1, strideVR2, offsetVR, N, 0, work, 1, oSCR );
			if ( ierr !== 0 ) {
				info = N + 2;
				return cleanup( info, ilascl, ilbscl, anrmto, anrm, bnrmto, bnrm, N, ALPHAR, strideALPHAR, offsetALPHAR, ALPHAI, strideALPHAI, offsetALPHAI, BETA, strideBETA, offsetBETA, ilo, ihi, abnrm, bbnrm );
			}
		...
		if ( !wantsn ) {
			...
			// Sense-loop scratch is carved from the caller work array past TAU;
			// the pre-loop scratch region at oSCR is free once eigenvectors have
			// been back-transformed. Layout, all stride-1 from oSCR:
			//   [oSCR    .. oSCR+2N)  workVL (2 cols x N rows, col-major ld=N)
			//   [oSCR+2N .. oSCR+4N)  workVR (2 cols x N rows, col-major ld=N)
			//   [oSCR+4N .. end    )  SNSWORK (dtgsna) / workCN (dtgevc 'selected')
			// dtgevc and dtgsna run sequentially, so SNSWORK and workCN share space.
			// SNSWORK needs max(iwrk1Len, 6N); the budgeted work length guarantees it.
			if ( wantsv || wantsb ) {
				iwrk1Len = ( 2 * N * ( N + 2 ) ) + 16;
			} else {
				iwrk1Len = Math.max( N, 1 );
			}
			workVL = work.subarray( oSCR, oSCR + ( 2 * N ) );
			workVR = work.subarray( oSCR + ( 2 * N ), oSCR + ( 4 * N ) );
			SNSWORK = work.subarray( oSCR + ( 4 * N ) );
			workCN = SNSWORK; // dtgevc scratch; same buffer (sequential usage)
			pair = false;
			for ( iIdx = 0; iIdx < N; iIdx++ ) {
				...
				for ( jVal = 0; jVal < N; jVal++ ) {
					bwork[ offsetBwork + jVal ] = 0;
				}
				bwork[ offsetBwork + iIdx ] = 1;
				if ( mm === 2 ) {
					bwork[ offsetBwork + iIdx + 1 ] = 1;
				}

				// Compute a pair of eigenvectors of (S,T) for DTGSNA.
				if ( wantse || wantsb ) {
					ierr = dtgevc( 'both', 'selected', bwork, 1, offsetBwork, N, A, strideA1, strideA2, offsetA, B, strideB1, strideB2, offsetB, workVL, 1, N, 0, workVR, 1, N, 0, mm, 0, workCN, 1, 0 );
					if ( ierr !== 0 ) {
						info = N + 2;
						return cleanup( info, ilascl, ilbscl, anrmto, anrm, bnrmto, bnrm, N, ALPHAR, strideALPHAR, offsetALPHAR, ALPHAI, strideALPHAI, offsetALPHAI, BETA, strideBETA, offsetBETA, ilo, ihi, abnrm, bbnrm );
					}
				...
				rcondvS[ 1 ] = ZERO;
				MCNT[ 0 ] = 0;
				dtgsna( snaJob, 'selected', bwork, 1, offsetBwork, N, A, strideA1, strideA2, offsetA, B, strideB1, strideB2, offsetB, workVL, 1, N, 0, workVR, 1, N, 0, rcondeS, 1, 0, rcondvS, 1, 0, mm, MCNT, SNSWORK, 1, 0, iwork, 1, offsetIwork );
