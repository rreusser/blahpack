...
function refZtrmm( side, uplo, transa, diag, M, N, alphaR, alphaI, A, lda, B, ldb ) {
	...
	const Bnew = new Float64Array( B.length );
	let i, j, k, sumR, sumI, aR, aI, bR, bI, idxA, idxB, inRange;
	const isUpper = ( uplo === 'upper' );
	for ( j = 0; j < N; j++ ) {
		for ( i = 0; i < M; i++ ) {
			sumR = 0;
			sumI = 0;
...
function checkAgainstRef( side, uplo, transa, diag, M, N, alphaR, alphaI, Adata, Bdata ) {
	const lda = ( side === 'left' ) ? M : N;
	const ldb = M;
	const A = cArr( Adata );
	const B = cArr( Bdata );
	const alpha = new Complex128( alphaR, alphaI );
	const expected = refZtrmm( side, uplo, transa, diag, M, N, alphaR, alphaI, Adata, lda, Bdata, ldb );
...
test( 'ndarray alpha=0 zeroes B (left/upper/no-transpose)', function t() {
	const B = cArr( [ 7, 8, 9, 10, 11, 12, 13, 14 ] );
	const A = cArr( [ 1, 1, 0, 0, 2, 2, 3, 3 ] );
...
test( 'ndarray transpose vs conjugate-transpose differ for complex A (left/upper)', function t() {
	...
	//   B := A^H * B = [[1,0],[-2i,1]]
	const Adata = [ 1, 0, 0, 0, 0, 2, 1, 0 ];
	const Bdata = [ 1, 0, 0, 0, 0, 0, 1, 0 ];
	const A1 = cArr( Adata.slice() );
	const B1 = cArr( Bdata.slice() );
	...
	const A2 = cArr( Adata.slice() );
	const B2 = cArr( Bdata.slice() );
...
test( 'ndarray hand-verified left/upper/no-transpose/non-unit', function t() {
	...
	// A*B = A = [[2,3],[0,4]]
	const A = cArr( [ 2, 0, 0, 0, 3, 0, 4, 0 ] );
	const B = cArr( [ 1, 0, 0, 0, 0, 0, 1, 0 ] );
...
test( 'ndarray hand-verified right/lower/no-transpose/unit', function t() {
	...
	const A = cArr( [ 999, 999, 5, 0, 0, 0, 999, 999 ] ); // unit: diagonal ignored
	const B = cArr( [ 1, 0, 1, 0, 1, 0, 1, 0 ] );
...
test( 'ndarray M=3 N=2 left/upper/no-transpose/non-unit', function t() {
	...
	const B = cArr( [ 1, 0, 0, 1, 1, 1, 2, 0, 0, 2, 1, 2 ] );
