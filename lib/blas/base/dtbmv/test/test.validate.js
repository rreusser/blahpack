/**
* Property-based validation for dtbmv, following the /blahpack-validate process.
*
...
* matvec oracle on the FULL logical A, sweeping uplo x trans x diag.
*/
...
test( 'dtbmv: triangular-banded matrix-vector residual (uplo x trans x diag x N x K sweep)', function t() {
			...
			DIAG.forEach( function eachDiag( diag ) {
				...
				NS.forEach( function eachN( N ) {
					bands( N ).forEach( function eachK( K ) {
						...
						dtbmv( uplo, trans, diag, N, K, R.data, R.args[ 0 ], R.args[ 1 ], R.args[ 2 ], X.data, X.args[ 0 ], X.args[ 1 ] );
						var expected = ref.matvec( sc, A, x, { 'trans': code } );
						var got = [];
						for ( i = 0; i < N; i++ ) {
							got.push( X.read( i ) );
						}
						checked( 'dtbmv', 'residual', function run() {
							assertResidual( got, expected, 'dtbmv '+uplo+' '+trans+' '+diag+' N='+N+' K='+K, N );
						});
					});
				});
			});
...
test( 'dtbmv: output is bit-exact across storage layouts', function t() {
	var N = 11;
	var K = 3;
	var SEED = 0xF11E;
	...
	UPLO.forEach( function eachUplo( uplo ) {
		TRANS.forEach( function eachTrans( tr ) {
			...
			DIAG.forEach( function eachDiag( diag ) {
				var unit = ( diag === 'unit' );
				checked( 'dtbmv', 'layout-invariance', function run() {
					...
					}, { 'label': 'dtbmv '+uplo+' '+trans+' '+diag+' layout invariance' } );
				});
			});
		});
	});
