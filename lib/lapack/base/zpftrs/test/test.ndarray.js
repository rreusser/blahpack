...
function runTest( name, transr, uplo, N, nrhs ) {
	...
	const B = new Complex128Array( tc.b_in );
	const info = zpftrs( transr, uplo, N, nrhs, A, 1, 0, B, 1, N, 0 );
...
test( 'zpftrs: n_zero (N=0 returns immediately)', function t() {
	...
	const info = zpftrs( 'no-transpose', 'lower', 0, 1, new Complex128Array( 0 ), 1, 0, B, 1, 0, 0 ); // eslint-disable-line max-len
	assert.equal( info, 0 );
});
...
test( 'zpftrs: n_one (N=1, NRHS=1)', function t() {
	...
	const A = new Complex128Array( tc.a );
	const B = new Complex128Array( tc.b_in );
	const info = zpftrs( 'no-transpose', 'lower', 1, 1, A, 1, 0, B, 1, 1, 0 );
	const Bv = reinterpret( B, 0 );
	assert.equal( info, tc.info );
	assertArrayClose( Bv, tc.b_out, 1e-14, 'b_out' );
});
