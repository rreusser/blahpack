...
import zunbdb3 from './../lib/ndarray.js';


// FIXTURES //

const fixtureDir = path.join( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures' );
const lines = readFileSync( path.join( fixtureDir, 'zunbdb3.jsonl' ), 'utf8' ).trim().split( '\n' ); // eslint-disable-line node/no-sync
...
function buildBuffer( packedInterleaved, P, Q ) {
	...
	const buf = new Complex128Array( LDX_PAD * Q );
...
function runFixtureCase( name, M, P, Q, hasPhi ) {
	...
	const X11 = buildBuffer( tc.X11in, P, Q );
	const X21 = buildBuffer( tc.X21in, M - P, Q );

	const THETA = new Float64Array( LDX_PAD );
	const PHI = new Float64Array( LDX_PAD );
	const TAUP1 = new Complex128Array( LDX_PAD );
	const TAUP2 = new Complex128Array( LDX_PAD );
	const TAUQ1 = new Complex128Array( LDX_PAD );
	const WORK = new Complex128Array( LDX_PAD * LDX_PAD );

	const info = zunbdb3( M, P, Q, X11, 1, LDX_PAD, 0, X21, 1, LDX_PAD, 0, THETA, 1, 0, PHI, 1, 0, TAUP1, 1, 0, TAUP2, 1, 0, TAUQ1, 1, 0, WORK, 1, 0 );
...
test( 'zunbdb3.ndarray: q0_quick_return — loop body never executes', function t() {
	...
	const X11 = new Complex128Array( LDX_PAD * LDX_PAD );
	const X21 = new Complex128Array( LDX_PAD * LDX_PAD );
	const THETA = new Float64Array( LDX_PAD );
	const PHI = new Float64Array( LDX_PAD );
	const TAUP1 = new Complex128Array( LDX_PAD );
	const TAUP2 = new Complex128Array( LDX_PAD );
	...
	const info = zunbdb3( 2, 2, 0, X11, 1, LDX_PAD, 0, X21, 1, LDX_PAD, 0, THETA, 1, 0, PHI, 1, 0, TAUP1, 1, 0, TAUP2, 1, 0, TAUQ1, 1, 0, WORK, 1, 0 );
...
test( 'zunbdb3.ndarray: returns info=0 when called with M=Q=0 (degenerate)', function t() {
	...
	const X11 = new Complex128Array( 1 );
	const X21 = new Complex128Array( 1 );
	const THETA = new Float64Array( 1 );
	const PHI = new Float64Array( 1 );
	const TAUP1 = new Complex128Array( 1 );
	const TAUP2 = new Complex128Array( 1 );
	const TAUQ1 = new Complex128Array( 1 );
	const WORK = new Complex128Array( 1 );

	const info = zunbdb3( 0, 0, 0, X11, 1, 1, 0, X21, 1, 1, 0, THETA, 1, 0, PHI, 1, 0, TAUP1, 1, 0, TAUP2, 1, 0, TAUQ1, 1, 0, WORK, 1, 0 );
	assert.strictEqual( info, 0, 'info' );
});
