...
import dsytf2Rook from '../../dsytf2_rook/lib/base.js';
import dlasyfRook from '../../dlasyf_rook/lib/base.js';
...
*/
function dsytrfRook( uplo, N, A, strideA1, strideA2, offsetA, IPIV, strideIPIV, offsetIPIV, WORK, strideWork, offsetWork ) {
	let ldwork, result, iinfo, info, nb, kb, k, j;
	...
	if ( uplo === 'upper' ) {
		...
		while ( k >= 1 ) {
			if ( k > nb ) {
				...
				result = dlasyfRook( 'upper', k, nb, A, sa1, sa2, offsetA, IPIV, strideIPIV, offsetIPIV, WORK, strideWork, ldwork * strideWork, offsetWork );
				kb = result.kb;
				iinfo = result.info;
			} else {
		...
		while ( k < N ) {
			if ( k <= N - nb - 1 ) {
				// Use blocked code:
				result = dlasyfRook( 'lower', N - k, nb, A, sa1, sa2, offsetA + (k * sa1) + (k * sa2), IPIV, strideIPIV, offsetIPIV + (k * strideIPIV), WORK, strideWork, ldwork * strideWork, offsetWork );
				kb = result.kb;
				iinfo = result.info;
			} else {
				// Use unblocked code:
