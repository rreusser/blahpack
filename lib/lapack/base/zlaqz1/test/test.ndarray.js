...
import zlaqz1 from './../lib/ndarray.js';
...
const lines = readFileSync( path.join( fixtureDir, 'zlaqz1.jsonl' ), 'utf8' ).trim().split( '\n' ); // eslint-disable-line node/no-sync
...
function buildMatrix( fn ) {
	let v, i, j;
	const out = new Complex128Array( NMAX * NMAX );
	for ( j = 0; j < NMAX; j++ ) {
		for ( i = 0; i < NMAX; i++ ) {
			v = fn( i, j );
			out.set( v, ( j * NMAX ) + i );
...
test( 'zlaqz1: move bulge down (normal operation)', function t() {
	const tc = findCase( 'move_bulge_down' );
	const A = buildMatrix( gen1A );
	const B = buildMatrix( gen1B );
...
test( 'zlaqz1: edge remove shift (k+1 == ihi)', function t() {
	const tc = findCase( 'edge_remove_shift' );
	const A = buildMatrix( gen2A );
	const B = buildMatrix( gen2B );
...
test( 'zlaqz1: move bulge with ilq=ilz=false (Q,Z untouched)', function t() {
	const tc = findCase( 'move_bulge_no_qz' );
	const A = buildMatrix( gen3A );
	const B = buildMatrix( gen3B );
