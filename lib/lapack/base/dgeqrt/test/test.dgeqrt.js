...
import dgeqrt from './../lib/dgeqrt.js';
...
test( 'dgeqrt accepts column-major valid call', function t() {
	const A = new Float64Array( [ 2, 1, 3, 1, 1, 4, 2, 3, 3, 2, 5, 1 ] ); // 4x3 column-major
	const T = new Float64Array( 6 ); // 2x3
	const WORK = new Float64Array( 12 );
	const info = dgeqrt( 'column-major', 4, 3, 2, A, 4, T, 2, WORK );
	assert.strictEqual( info, 0, 'returns 0' );
});

test( 'dgeqrt accepts row-major valid call', function t() {
	const A = new Float64Array( [ 2, 1, 3, 1, 4, 2, 3, 2, 5, 1, 3, 1 ] ); // 4x3 row-major
	const T = new Float64Array( 6 ); // 2x3 row-major (LDT=3)
	const WORK = new Float64Array( 12 );
	const info = dgeqrt( 'row-major', 4, 3, 2, A, 3, T, 3, WORK );
	assert.strictEqual( info, 0, 'returns 0' );
});
