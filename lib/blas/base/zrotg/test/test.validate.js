...
test( 'zrotg: property (Givens defining relations) across a0/b0 cases', function t() {
	checked( 'zrotg', 'property', function run() {
		const t100 = check.tol( 1, 100 );
		CASES.forEach( function each( mk, k ) {
			const rng = new RNG( 0x100 + k );
			const in0 = mk( rng );
			const a0 = in0[ 0 ];
			const b0 = in0[ 1 ];
			const A = schemes.realizeVector( sc, [ a0 ], { 'stride': 1 } );
			const B = schemes.realizeVector( sc, [ b0 ], { 'stride': 1 } );
			const C = schemes.realizeVector( scr, [ 0.0 ], { 'stride': 1 } );
			const Sr = schemes.realizeVector( sc, [ Z( 0.0, 0.0 ) ], { 'stride': 1 } );
			zrotg( A.data, A.args[ 1 ], B.data, B.args[ 1 ], C.data, C.args[ 1 ], Sr.data, Sr.args[ 1 ] );
			const c = C.read( 0 );
			const s = Sr.read( 0 );
			const r = A.read( 0 ); // a overwritten with r
			check.assertFinite( scr, [ c ], 'zrotg c k='+k );
			check.assertFinite( sc, [ s, r ], 'zrotg s,r k='+k );

			// (i) c^2 + |s|^2 = 1:
			const s2 = ( s.re*s.re ) + ( s.im*s.im );
			check.assertScaled( Math.abs( ( ( c*c ) + s2 ) - 1.0 ), 1.0, t100, 'zrotg (i) c^2+|s|^2=1 k='+k );

			const scale = sc.abs( a0 ) + sc.abs( b0 );

			// (ii) c*a0 + s*b0 = r:
			const lhs = sc.add( sc.scale( a0, c ), sc.mul( s, b0 ) );
			check.assertScaled( sc.abs( sc.sub( lhs, r ) ), scale, t100, 'zrotg (ii) c*a+s*b=r k='+k );

			// (iii) -conj(s)*a0 + c*b0 = 0:
			const res = sc.add( sc.neg( sc.mul( sc.conj( s ), a0 ) ), sc.scale( b0, c ) );
...
test( 'zrotg: layout invariance (bit-exact across per-operand offsets)', function t() {
	const variants = [ 0, 1, 2, 4, 7 ];
	const a0 = Z( 1.5, -0.5 );
	const b0 = Z( -0.7, 2.1 );
	checked( 'zrotg', 'layout-invariance', function run() {
		layoutInvariant( variants, function build( v, idx ) {
			const la = variants[ idx ];
			const lb = variants[ ( idx + 1 ) % variants.length ];
			const lc = variants[ ( idx + 2 ) % variants.length ];
			const ls = variants[ ( idx + 3 ) % variants.length ];
			const A = schemes.realizeVector( sc, [ a0 ], { 'stride': 1, 'lead': la } );
			const B = schemes.realizeVector( sc, [ b0 ], { 'stride': 1, 'lead': lb } );
			const C = schemes.realizeVector( scr, [ 0.0 ], { 'stride': 1, 'lead': lc } );
			const Sr = schemes.realizeVector( sc, [ Z( 0.0, 0.0 ) ], { 'stride': 1, 'lead': ls } );
			zrotg( A.data, A.args[ 1 ], B.data, B.args[ 1 ], C.data, C.args[ 1 ], Sr.data, Sr.args[ 1 ] );
			const c = C.read( 0 );
			const s = Sr.read( 0 );
			const r = A.read( 0 );
