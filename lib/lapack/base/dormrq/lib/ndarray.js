...
*/
function dormrq( side, trans, M, N, K, A, strideA1, strideA2, offsetA, TAU, strideTAU, offsetTAU, C, strideC1, strideC2, offsetC, WORK, strideWork, offsetWork ) { // eslint-disable-line max-len, max-params
	...
	// The UNBLOCKED path (K<=NB) needs only `ldwork` (nw = N for left, M for
	// right); the BLOCKED path (K>NB) additionally stores the block reflector T in
	// a trailing WORK segment, so it consumes `ldwork*NB + (NB+1)*NB` elements
	// (NB=32). Advertising the bare unblocked minimum for a blocked call would read
	// past the buffer. See test/harness/LEARNINGS.md (dormrq / dormqr entries).
	const nb = 32;
	const ldwork = ( side === 'left' ) ? Math.max( 1, N ) : Math.max( 1, M );
	const minWork = ( nb >= K ) ? ldwork : ( ( ldwork * nb ) + ( ( nb + 1 ) * nb ) );
