...
import dtplqt from './../lib/dtplqt.js';
...
test( 'dtplqt: column-major dispatch', function t() {
	const A = new Float64Array( [ 2.0, 0.5, 0.25, 0.0, 3.0, 0.75, 0.0, 0.0, 4.0 ] );  // 3x3 col-major lower
	const B = new Float64Array( [ 1.0, 0.3, 0.7, 0.5, 1.1, 0.4, 0.25, 0.6, 1.2, 0.1, 0.2, 0.9 ] );  // 3x4 col-major
	const T = new Float64Array( 2 * 3 );
	const W = new Float64Array( 2 * 3 );
	const info = dtplqt( 'column-major', 3, 4, 0, 2, A, 3, B, 3, T, 2, W );
...
test( 'dtplqt: row-major dispatch', function t() {
	...
	const A = new Float64Array( [ 2.0, 0.0, 0.0, 0.5, 3.0, 0.0, 0.25, 0.75, 4.0 ] );  // 3x3 row-major (lower part)
	const B = new Float64Array( [ 1.0, 0.5, 0.25, 0.1, 0.3, 1.1, 0.6, 0.2, 0.7, 0.4, 1.2, 0.9 ] );  // 3x4 row-major
	const T = new Float64Array( 2 * 3 );
	const W = new Float64Array( 2 * 3 );
	const info = dtplqt( 'row-major', 3, 4, 0, 2, A, 3, B, 4, T, 2, W );
	assert.strictEqual( info, 0, 'INFO=0' );
});
