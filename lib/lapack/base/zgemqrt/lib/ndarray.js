...
function zgemqrt( side, trans, M, N, K, nb, V, strideV1, strideV2, offsetV, T, strideT1, strideT2, offsetT, C, strideC1, strideC2, offsetC, WORK, strideWork, offsetWork ) {
	...
	// Caller owns WORK; assert it is large enough (logically ldwork-by-nb, ldwork = max(1,N) for side='left' or max(1,M) for side='right') so an undersized buffer is a loud RangeError, not a silent NaN. Only the non-quick-return path (M>0 && N>0 && K>0) touches WORK.
	if ( M > 0 && N > 0 && K > 0 ) {
		need = ( ( side === 'left' ) ? Math.max( 1, N ) : Math.max( 1, M ) ) * nb;
