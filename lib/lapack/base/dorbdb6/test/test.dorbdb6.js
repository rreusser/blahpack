...
import dorbdb6 from './../lib/dorbdb6.js';
...
test( 'dorbdb6 (column-major): basic projection', function t() {
	...
	const Q1 = new Float64Array( [ 1.0, 0.0 ] );
	const Q2 = new Float64Array( [ 0.0, 0.0 ] );
	const X1 = new Float64Array( [ 3.0, 4.0 ] );
	const X2 = new Float64Array( [ 5.0, 6.0 ] );
	const WORK = new Float64Array( 1 );

	const info = dorbdb6( 'column-major', 2, 2, 1, X1, 1, X2, 1, Q1, 2, Q2, 2, WORK, 1 );
...
test( 'dorbdb6 (row-major): basic projection', function t() {
	...
	const info = dorbdb6( 'row-major', 2, 2, 1, X1, 1, X2, 1, Q1, 1, Q2, 1, WORK, 1 );
	assert.strictEqual( info, 0, 'info' );
	assert.ok( Math.abs( X1[ 0 ] - 0.0 ) < 1e-14, 'X1[0]' );
	assert.ok( Math.abs( X1[ 1 ] - 4.0 ) < 1e-14, 'X1[1]' );
});
