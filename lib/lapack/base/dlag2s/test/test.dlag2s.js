...
import dlag2s from './../lib/dlag2s.js';
...
test( 'dlag2s column-major converts a 2x2 matrix', function t() {
	const A = new Float64Array( [ 1.5, 2.5, -3.75, 4.125 ] );
	const SA = new Float64Array( 4 );
	const info = dlag2s( 'column-major', 2, 2, A, 2, SA, 2 );
...
test( 'dlag2s row-major converts a 2x3 matrix', function t() {
	...
	const info = dlag2s( 'row-major', 2, 3, A, 3, SA, 3 );
	assert.strictEqual( info, 0, 'info' );
...
test( 'dlag2s returns info=1 on overflow', function t() {
	const A = new Float64Array( [ 1.0, 1.0e40, 3.0, 4.0 ] );
	const SA = new Float64Array( 4 );
	const info = dlag2s( 'column-major', 2, 2, A, 2, SA, 2 );
