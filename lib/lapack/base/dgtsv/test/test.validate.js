...
function runResidual( N, nrhs ) {
	...
	var DL = schemes.realizeVector( sc, subDiag( A0, N ), { 'stride': 1 } );
	var D = schemes.realizeVector( sc, mainDiag( A0, N ), { 'stride': 1 } );
	var DU = schemes.realizeVector( sc, superDiag( A0, N ), { 'stride': 1 } );
	var Br = schemes.dense.realize( sc, B0, { 'part': 'full' }, schemes.dense.layouts()[ 0 ] );

	var info = dgtsv( N, nrhs, DL.data, DL.args[ 0 ], DL.args[ 1 ], D.data, D.args[ 0 ], D.args[ 1 ], DU.data, DU.args[ 0 ], DU.args[ 1 ], Br.data, Br.args[ 0 ], Br.args[ 1 ], Br.args[ 2 ] );
	if ( info !== 0 ) {
...
test( 'dgtsv: bit-exact across strided vector + dense-B layouts', function t() {
	...
	checked( 'dgtsv', 'layout-invariance', function run() {
		layoutInvariant( VL, function build( vL, idx ) {
			var DL = schemes.realizeVector( sc, subDiag( A0, N ), vL );
			var D = schemes.realizeVector( sc, mainDiag( A0, N ), VL[ ( idx + 1 ) % VL.length ] );
			var DU = schemes.realizeVector( sc, superDiag( A0, N ), VL[ ( idx + 2 ) % VL.length ] );
