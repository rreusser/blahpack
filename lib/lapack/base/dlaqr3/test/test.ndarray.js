...
import dlaqr3 from './../lib/ndarray.js';
...
import _4x4_hessenberg_nw_1 from './fixtures/4x4_hessenberg_nw_1.json' with { type: 'json' };
import empty_active_block from './fixtures/empty_active_block.json' with { type: 'json' };
import empty_deflation_window from './fixtures/empty_deflation_window.json' with { type: 'json' };
...
function runDlaqr3( wantt, wantz, N, ktop, kbot, nw, Hin, iloz, ihiz ) {
	...
	const WV = new Float64Array( MAXN * MAXN );
	const nh = N;
	const nv = N;
	const H = new Float64Array( MAXN * N );
	const Z = new Float64Array( MAXN * N );
	const V = new Float64Array( MAXN * MAXN );
	const T = new Float64Array( MAXN * MAXN );
	let i;
	...
	const result = dlaqr3( wantt, wantz, N, ktop, kbot, nw, H, 1, N, 0, iloz, ihiz, Z, 1, N, 0, SR, 1, 0, SI, 1, 0, V, 1, N, 0, nh, T, 1, N, 0, nv, WV, 1, N, 0, WORK, 1, 0 ); // eslint-disable-line max-len
...
test( 'dlaqr3: 6x6 hessenberg NW=3', function t() {
	...
	const N = 6;
	const MAXN = 10;
	const Hin = extractMatrix( tc.H, N, MAXN );
	const result = runDlaqr3( true, true, N, 1, 6, 3, Hin, 1, 6 );
	assert.equal( result.ns, tc.ns, 'ns' );
	assert.equal( result.nd, tc.nd, 'nd' );
	const Hexp = extractMatrix( tc.H, N, MAXN );
...
test( 'dlaqr3: 4x4 hessenberg NW=2', function t() {
	...
	const MAXN = 10;
	const Hin = extractMatrix( tc.H, N, MAXN );
	const result = runDlaqr3( true, true, N, 1, 4, 2, Hin, 1, 4 );
	assert.equal( result.ns, tc.ns, 'ns' );
	assert.equal( result.nd, tc.nd, 'nd' );
	assertArrayClose( result.SR, new Float64Array( tc.SR ), 1e-10, 'SR' );
	assertArrayClose( result.SI, new Float64Array( tc.SI ), 1e-10, 'SI' );
...
test( 'dlaqr3: 4x4 hessenberg NW=1', function t() {
	...
	H0[ 3 + 2 * N ] = 0.5;
	H0[ 3 + 3 * N ] = 2.0;
	const result = runDlaqr3( true, false, N, 1, 4, 1, H0, 1, 4 );
	assert.equal( result.ns, tc.ns, 'ns' );
	assert.equal( result.nd, tc.nd, 'nd' );
	assertArrayClose( result.SR, new Float64Array( tc.SR ), 1e-10, 'SR' );
	assertArrayClose( result.SI, new Float64Array( tc.SI ), 1e-10, 'SI' );
...
test( 'dlaqr3: empty active block (ktop > kbot)', function t() {
	...
	const WORK = new Float64Array( 200 );
	const result = dlaqr3( true, false, N, 3, 2, 2, H, 1, N, 0, 1, 4, Z, 1, N, 0, SR, 1, 0, SI, 1, 0, V, 1, N, 0, N, T, 1, N, 0, N, WV, 1, N, 0, WORK, 1, 0, 200 ); // eslint-disable-line max-len
	assert.equal( result.ns, tc.ns, 'ns' );
	assert.equal( result.nd, tc.nd, 'nd' );
});
...
test( 'dlaqr3: empty deflation window (nw < 1)', function t() {
	...
	const tc = empty_deflation_window;
	const N = 4;
	const H = new Float64Array( N * N );
	const Z = new Float64Array( N * N );
	const SR = new Float64Array( N );
	...
	const WV = new Float64Array( N * N );
	const WORK = new Float64Array( 200 );
	const result = dlaqr3( true, false, N, 1, 4, 0, H, 1, N, 0, 1, 4, Z, 1, N, 0, SR, 1, 0, SI, 1, 0, V, 1, N, 0, N, T, 1, N, 0, N, WV, 1, N, 0, WORK, 1, 0, 200 ); // eslint-disable-line max-len
	assert.equal( result.ns, tc.ns, 'ns' );
	assert.equal( result.nd, tc.nd, 'nd' );
});
