...
//
// Memory-bound symmetric rank-1 update: one read + one write per stored
// triangle element for two flops, so the ceiling is streaming bandwidth.
...
for ( const n of [ 500, 2000 ] ) {
	for ( const layout of [ 'col', 'row' ] ) {
		for ( const uplo of [ 'upper', 'lower' ] ) {
			...
			// ~half the matrix (the stored triangle) at 2 flops per element:
