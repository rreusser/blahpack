/**
* Property-based validation for dgetrf2, following the /blahpack-validate process.
*
...
test( 'dgetrf2: P*A = L*U reconstruction (size sweep x all layouts)', function t() {
	var pairs = [];
	...
	pairs.forEach( function eachPair( p ) {
		...
		POSROW.forEach( function eachLayout( layout ) {
			...
			dgetrf2( m, n, R.data, R.args[ 0 ], R.args[ 1 ], R.args[ 2 ], IPIV, 1, 0 );
			if ( minMN === 0 ) {
			...
			checked( 'dgetrf2', 'reconstruct', function run() {
				check.assertReconstruct( sc, LU, PA0, { 'label': 'dgetrf2 m='+m+' n='+n } );
			});
		});
	});
});
...
test( 'dgetrf2: bit-exact within storage-order family (col / row)', function t() {
...
function runInvariance( variants, fam ) {
	...
	var minMN = 40;
	var SEED = 0xF00D;
	checked( 'dgetrf2', 'layout-invariance', function run() {
		...
		}, { 'label': 'dgetrf2 layout invariance '+fam+'-major' } );
	});
}
