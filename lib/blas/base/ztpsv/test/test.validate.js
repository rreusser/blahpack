/**
* Property-based validation for ztpsv, following the /blahpack-validate process.
*
...
* large N. The `conjugate-transpose` flag exercises the `A**H*x = b` path;
* check.assertResidual's trans code 'c' conjugates the referenced triangle.
*
* The packed solve has a SINGLE accumulation order fixed by the algorithm
* (forward/backward substitution) — it does not branch on stride sign — so
* layout invariance holds across ALL packed AP layouts and vector layouts with
* NO kernel-form family split.
*/
...
test( 'ztpsv: triangular packed solve residual (uplo x trans x diag x N sweep)', function t() {
		...
		TRANS.forEach( function eachTrans( tr ) {
			...
			DIAG.forEach( function eachDiag( diag ) {
				...
				SIZES.forEach( function eachN( N ) {
					...
					ztpsv( uplo, trans, diag, N, AP.data, AP.args[ 0 ], AP.args[ 1 ], X.data, X.args[ 0 ], X.args[ 1 ] );
					var got = [];
					for ( i = 0; i < N; i++ ) {
						got.push( X.read( i ) );
					}
					checked( 'ztpsv', 'residual', function run() {
						check.assertResidual( sc, A, got, b, { 'trans': code, 'factor': 100, 'label': 'ztpsv '+uplo+' '+trans+' '+diag+' N='+N } );
					});
				});
			});
		});
...
test( 'ztpsv: output is bit-exact across storage layouts (packed AP + strided/negative vectors)', function t() {
	var N = 9;
	var SEED = 0xF44E;
	...
	UPLO.forEach( function eachUplo( uplo ) {
		TRANS.forEach( function eachTrans( tr ) {
			...
			DIAG.forEach( function eachDiag( diag ) {
				var unit = ( diag === 'unit' );
				checked( 'ztpsv', 'layout-invariance', function run() {
					...
					}, { 'label': 'ztpsv '+uplo+' '+trans+' '+diag+' layout invariance' } );
				});
			});
		});
	});
