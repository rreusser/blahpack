...
const NB = 32; // hardcoded block size in the zungql/zungqr sub-kernels
	...
	UPLO.forEach( function eachUplo( uplo ) {
		SWEEP.forEach( function eachN( N ) {
			ALL_LAYOUTS.forEach( function eachLayout( layout, li ) {
				...
				const tauR = schemes.realizeVector( sc, poison( Math.max( N - 1, 0 ) ), VEC_LAYOUTS[ ( li + 2 ) % VEC_LAYOUTS.length ] );

				const out = factor( uplo, N, Ar, dR, eR, tauR );

				const Wo = schemes.realizeVector( sc, poison( workLen( N ) ), TIGHT_VEC );
				...
				const label = 'zungtr ' + uplo + ' N=' + N + ' layout=' + li;
...
function runInvariance( variants, fam, uplo ) {
	...
	const tF = schemes.realizeVector( sc, poison( N - 1 ), TIGHT_VEC );
	...
	checked( 'zungtr', 'layout-invariance', function run() {
		layoutInvariant( variants, function build( layout, i ) {
			...
			const Tr = schemes.realizeVector( sc, taus, VEC_LAYOUTS[ i % VEC_LAYOUTS.length ] );
			const Wo = schemes.realizeVector( sc, poison( workLen( N ) ), TIGHT_VEC );
...
test( 'zungtr: advertised WORK minimum suffices on the blocked path (Step 4c)', function t() {
	UPLO.forEach( function eachUplo( uplo ) {
		...
		const SEED = 0xB10C;
		const label = 'zungtr WORK-min ' + uplo + ' N=' + N;
