...
import zlantp from './../lib/ndarray.js';


// FIXTURES //

const fixtureDir = path.join( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures' );
const rawLines = readFileSync( path.join( fixtureDir, 'zlantp.jsonl' ), 'utf8' ).trim().split( '\n' ); // eslint-disable-line node/no-sync, max-len
...
function buildAP( pairs ) {
	const raw = new Float64Array( pairs );
...
function runCase( name, norm, uplo, diag, N, AP ) {
	...
	const got = zlantp( norm, uplo, diag, N, AP, 1, 0, WORK, 1, 0 );
