...
function dlag2( A, strideA1, strideA2, offsetA, B, strideB1, strideB2, offsetB, safmin ) {
	let wscale, scale1, scale2, wsmall, abi22, shift, discr, wsize, diff, wbig;
	let wdet, as11, as12, as22, sum, wr1, wr2, b11, b12, b22, wi, c4, c5, pp;
	let ss, r;

	const rtmin = Math.sqrt( safmin );
	const rtmax = 1.0 / rtmin;
	const safmax = 1.0 / safmin;

	// Scale A:
	const anorm = Math.max(
		Math.abs( A[ offsetA ] ) + Math.abs( A[ offsetA + strideA1 ] ),
		Math.abs( A[ offsetA + strideA2 ] ) + Math.abs( A[ offsetA + strideA1 + strideA2 ] ),
		safmin
	);
	const ascale = 1.0 / anorm;
	const a11 = ascale * A[ offsetA ];
	const a21 = ascale * A[ offsetA + strideA1 ];
	const a12 = ascale * A[ offsetA + strideA2 ];
	const a22 = ascale * A[ offsetA + strideA1 + strideA2 ];

	// Perturb B if necessary to ensure non-singularity:
	b11 = B[ offsetB ];
	b12 = B[ offsetB + strideB2 ];
	b22 = B[ offsetB + strideB1 + strideB2 ];
	const bmin = rtmin * Math.max( Math.abs( b11 ), Math.abs( b12 ), Math.abs( b22 ), rtmin );
	...
	// Scale B:
	const bnorm = Math.max( Math.abs( b11 ), Math.abs( b12 ) + Math.abs( b22 ), safmin );
	const bsize = Math.max( Math.abs( b11 ), Math.abs( b22 ) );
	const bscale = 1.0 / bsize;
	b11 *= bscale;
	b12 *= bscale;
	b22 *= bscale;

	// Compute larger eigenvalue (AS is A shifted by -SHIFT*B):
	const binv11 = 1.0 / b11;
	const binv22 = 1.0 / b22;
	const s1 = a11 * binv11;
	const s2 = a22 * binv22;
	...
	const c1 = bsize * ( safmin * Math.max( 1.0, ascale ) );
	const c2 = safmin * Math.max( 1.0, bnorm );
	const c3 = bsize * safmin;
	...
	const wabs = Math.abs( wr1 ) + Math.abs( wi );
	wsize = Math.max(
		safmin,
		c1,
		FUZZY1 * ( (wabs * c2) + c3 ),
