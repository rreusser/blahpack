...
import zsyconvf from './../lib/index.js';
...
test( 'main export returns 0 on quick-return for N=0', function t() {
	...
	const info = zsyconvf( 'column-major', 'upper', 'convert', 0, A, 1, E, 1, IPIV, 1, 0 ); // eslint-disable-line max-len
...
test( 'ndarray method returns 0 on quick-return for N=0', function t() {
	...
	const E = new Complex128Array( 0 );
	const IPIV = new Int32Array( 0 );
	const info = zsyconvf.ndarray( 'upper', 'convert', 0, A, 1, 1, 0, E, 1, 0, IPIV, 1, 0 ); // eslint-disable-line max-len
	assert.strictEqual( info, 0, 'returns 0' );
});
...
test( 'main export accepts lower uplo and revert way', function t() {
	...
	const info = zsyconvf( 'column-major', 'lower', 'revert', 2, A, 2, E, 1, IPIV, 1, 0 ); // eslint-disable-line max-len
	assert.strictEqual( info, 0, 'returns 0' );
});
