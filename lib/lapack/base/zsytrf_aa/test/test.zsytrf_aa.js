...
import zsytrfAa from './../lib/zsytrf_aa.js';
...
test( 'zsytrfAa has expected arity', function t() {
	assert.strictEqual( zsytrfAa.length, 8, 'has expected arity' );
});
...
test( 'zsytrfAa returns 0 for N=0', function t() {
	const info = zsytrfAa( 'column-major', 'lower', 0, new Complex128Array( 0 ), 1, new Int32Array( 0 ) );
...
test( 'zsytrfAa works for N=1 column-major', function t() {
	...
	const info = zsytrfAa( 'column-major', 'lower', 1, A, 1, ipiv, null, 1 );
...
test( 'zsytrfAa works for row-major (4x4 lower)', function t() {
	...
	const info = zsytrfAa( 'row-major', 'lower', 4, A, 4, ipiv, null, 1 );
	assert.equal( info, 0, 'info=0' );
});
