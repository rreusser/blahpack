/**
* Property-based validation for zgerc, following the /blahpack-validate process.
*
* Step 0 classification: `z` -> complex scalar; `ge` -> general dense
* (schemes.dense, logical.general); conjugated rank-1 update
* `A := alpha*x*y**H + A` validated by direct residual against an independent
* oracle `A(i,j) += alpha*x_i*conj(y_j)`.
*/

import test from 'node:test';

// from lib/<pkg>/base/<routine>/test/ the repo root is five levels up:
import { RNG, scalar as S, logical, schemes, check, layoutInvariant, SIZES } from '../../../../../test/harness/index.js';
import { checked } from '../../../../../test/harness/ledger.js';
import zgerc from './../lib/ndarray.js';

const sc = S.complex; // z-routine
const CONJ = true; // A := alpha*x*y**H + A (conjugates y)
const LogicalMatrix = logical.LogicalMatrix;

// Read the updated matrix back out of physical storage into a LogicalMatrix.
function readMatrix( R, m, n ) {
	const G = new LogicalMatrix( sc, m, n );
	let i, j;
	for ( j = 0; j < n; j++ ) {
		for ( i = 0; i < m; i++ ) {
			G.set( i, j, R.read( i, j ) );
		}
	}
	return G;
}

// Independent oracle: expected(i,j) = A0(i,j) + alpha*x_i*(conj?)y_j.
function expected( A0, alpha, x, y, m, n ) {
	const E = new LogicalMatrix( sc, m, n );
	let yj, i, j;
	for ( j = 0; j < n; j++ ) {
		yj = CONJ ? sc.conj( y[ j ] ) : y[ j ];
		for ( i = 0; i < m; i++ ) {
			E.set( i, j, sc.add( A0.get( i, j ), sc.mul( alpha, sc.mul( x[ i ], yj ) ) ) );
		}
	}
	return E;
}

function values( rng, n ) {
	const v = [];
	let i;
	for ( i = 0; i < n; i++ ) {
		v.push( sc.random( rng ) );
	}
	return v;
}

// Steps 2-3-5: residual across a full (M,N) size sweep incl. rectangular + 0.
test( 'zgerc: rank-1 update residual (size sweep MxN, incl. alpha=0)', function t() {
	SIZES.forEach( function eachM( M ) {
		SIZES.forEach( function eachN( N ) {
			const rng = new RNG( 0x100 + ( M * 100 ) + N ); // reproducible
			const A0 = logical.general( sc, rng, M, N );
			const x = values( rng, M );
			const y = values( rng, N );
			const alpha = ( ( ( M * 7 ) + N ) % 11 === 0 ) ? sc.zero : sc.random( rng );

			const R = schemes.dense.realize( sc, A0, { 'part': 'full' }, schemes.dense.layouts()[ 0 ] );
			const X = schemes.realizeVector( sc, x, { 'stride': 1 } );
			const Y = schemes.realizeVector( sc, y, { 'stride': 1 } );
			zgerc( M, N, sc.apiScalar( alpha ), X.data, X.args[ 0 ], X.args[ 1 ], Y.data, Y.args[ 0 ], Y.args[ 1 ], R.data, R.args[ 0 ], R.args[ 1 ], R.args[ 2 ] );

			const got = readMatrix( R, M, N );
			const exp = expected( A0, alpha, x, y, M, N );
			checked( 'zgerc', 'residual', function run() {
				check.assertReconstruct( sc, got, exp, { 'label': 'zgerc M='+M+' N='+N } );
			});
		});
	});
});

// Step 4: layout-invariance fuzz — output must be bit-exact across storage
// layouts (matrix AND strided/negative vectors).
test( 'zgerc: bit-exact across storage layouts', function t() {
	const M = 7;
	const N = 5;
	const SEED = 0xF00D;
	const aLayouts = schemes.dense.layouts();
	const vLayouts = schemes.vectorLayouts();
	checked( 'zgerc', 'layout-invariance', function run() {
		layoutInvariant( aLayouts, function build( aL, idx ) {
			const rng = new RNG( SEED ); // identical values every variant
			const A0 = logical.general( sc, rng, M, N );
			const x = values( rng, M );
			const y = values( rng, N );
			const alpha = sc.random( rng );

			const R = schemes.dense.realize( sc, A0, { 'part': 'full' }, aL );
			const vL = vLayouts[ idx % vLayouts.length ];
			const X = schemes.realizeVector( sc, x, vL );
			const Y = schemes.realizeVector( sc, y, vL );
			zgerc( M, N, sc.apiScalar( alpha ), X.data, X.args[ 0 ], X.args[ 1 ], Y.data, Y.args[ 0 ], Y.args[ 1 ], R.data, R.args[ 0 ], R.args[ 1 ], R.args[ 2 ] );
			return check.flattenLogical( sc, readMatrix( R, M, N ) );
		}, { 'label': 'zgerc layout invariance' } );
	});
});
