/**
* Property-based validation for dgbtrf, following the /blahpack-validate process.
*
...
* validate via FACTOR + SOLVE RESIDUAL: factor with dgbtrf, solve with the
...
*
* Pivoting caveat: dgbtrf does an `idamax` PIVOT SEARCH over a band sub-column,
...
* also reaches optimized dgemm/dger, so bit-exact layout invariance holds only
...
test( 'dgbtrf: banded LU factor+solve residual (N x (kl,ku) x nrhs)', function t() {
...
// dgemm/dger reorder across the col<->row flip).
...
test( 'dgbtrf: solution bit-exact within storage-order family (col / row)', function t() {
...
function runInvariance( fam, bandLayouts, denseLayouts ) {
	...
	checked( 'dgbtrf', 'layout-invariance', function run() {
		...
		}, { 'label': 'dgbtrf '+fam+'-major layout invariance' } );
	});
}
