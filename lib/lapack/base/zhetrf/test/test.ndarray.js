...
test( 'zhetrf: N=0 quick return', function t() {
	...
	const info = zhetrf( 'upper', 0, A, 1, 1, 0, IPIV, 1, 0 );
	assert.equal( info, 0 );
});
...
test( 'zhetrf: upper_4x4 (fixture, falls through to zhetf2 since N < NB)', function t() {
	...
	const info = zhetrf( 'upper', n, A, 1, n, 0, IPIV, 1, 0 );
...
test( 'zhetrf: lower_4x4 (fixture)', function t() {
	...
	const info = zhetrf( 'lower', n, A, 1, n, 0, IPIV, 1, 0 );
...
test( 'zhetrf: upper N=1', function t() {
	...
	const info = zhetrf( 'upper', 1, A, 1, 1, 0, IPIV, 1, 0 );
...
test( 'zhetrf: lower N=1', function t() {
	...
	const info = zhetrf( 'lower', 1, A, 1, 1, 0, IPIV, 1, 0 );
