...
test( 'dlamrg: both forward, 3+3', function t() {
	...
	const tc = fwd_fwd_3_3;
	const a = new Float64Array( [ 1.0, 3.0, 5.0, 2.0, 4.0, 6.0 ] );
...
test( 'dlamrg: first forward, second backward, 3+3', function t() {
	...
	const tc = fwd_bwd_3_3;
	const a = new Float64Array( [ 1.0, 3.0, 5.0, 6.0, 4.0, 2.0 ] );
	const idx = new Int32Array( 6 );
	dlamrg( 3, 3, a, 1, 0, 1, -1, idx, 1, 0 );
	for ( i = 0; i < 6; i++ ) {
		assert.strictEqual( idx[ i ], tc.index[ i ], 'index[' + i + ']' );
...
test( 'dlamrg: first backward, second forward, 3+2', function t() {
	...
	const tc = bwd_fwd_3_2;
	const a = new Float64Array( [ 5.0, 3.0, 1.0, 2.0, 4.0 ] );
...
test( 'dlamrg: both backward, 2+3', function t() {
	...
	const tc = bwd_bwd_2_3;
	const a = new Float64Array( [ 4.0, 2.0, 6.0, 3.0, 1.0 ] );
	const idx = new Int32Array( 5 );
...
test( 'dlamrg: n1=1, n1=1', function t() {
	...
	const tc = n1_n1;
	const a = new Float64Array( [ 3.0, 1.0 ] );
	const idx = new Int32Array( 2 );
...
test( 'dlamrg: equal elements', function t() {
	...
	const tc = equal;
	const a = new Float64Array( [ 2.0, 2.0, 2.0, 2.0 ] );
...
test( 'dlamrg: n1=4, n2=1', function t() {
	...
	const tc = n4_n1;
	const a = new Float64Array( [ 1.0, 3.0, 5.0, 7.0, 4.0 ] );
	const idx = new Int32Array( 5 );
