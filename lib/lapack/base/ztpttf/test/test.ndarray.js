import __imp0 from './fixtures/n1_n_l.json' with { type: 'json' };
import __imp1 from './fixtures/n1_c_u.json' with { type: 'json' };
...
import ztpttf from './../lib/ndarray.js';
...
function runTest( transr, uplo, N, caseName ) {
	...
	const nt = ( N * ( N + 1 ) ) / 2;
	const AP = new Complex128Array( nt );
	const APv = reinterpret( AP, 0 );
	for ( i = 0; i < 2 * nt; i += 1 ) {
		APv[ i ] = tc.AP[ i ];
	}
	const actual = new Complex128Array( nt );
	const expected = new Float64Array( tc.ARF );

	const info = ztpttf( transr, uplo, N, AP, 1, 0, actual, 1, 0 );
	assert.equal( info, 0, 'info should be 0' );
	const actualv = reinterpret( actual, 0 );
	for ( i = 0; i < 2 * nt; i += 1 ) {
...
test( 'ztpttf: N=0 quick return', function t() {
	...
	const ARF = new Complex128Array( 0 );
	const info = ztpttf( 'no-transpose', 'lower', 0, AP, 1, 0, ARF, 1, 0 );
	assert.equal( info, 0, 'info should be 0 for N=0' );
});

test( 'ztpttf: N=1, no-transpose, lower', function t() {
	const tc = fixtures.n1_N_L;
	...
	const info = ztpttf( 'no-transpose', 'lower', 1, AP, 1, 0, ARF, 1, 0 );
	assert.equal( info, 0, 'info should be 0' );
	const ARFv = reinterpret( ARF, 0 );
...
test( 'ztpttf: N=1, conjugate-transpose, upper', function t() {
	...
	APv[ 1 ] = -3.0;
	const ARF = new Complex128Array( 1 );
	const info = ztpttf( 'conjugate-transpose', 'upper', 1, AP, 1, 0, ARF, 1, 0 );
	assert.equal( info, 0, 'info should be 0' );
	const ARFv = reinterpret( ARF, 0 );
	assert.equal( ARFv[ 0 ], tc.ARF[ 0 ], 'ARF[0] real' );
	assert.equal( ARFv[ 1 ], tc.ARF[ 1 ], 'ARF[0] imag' );
