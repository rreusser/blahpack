...
function runTest( name, vect, uplo, N, ka, kb, ldab, ldbb, ldx, tol ) {
	...
	const AB = new Complex128Array( tc.AB_in );
	const BB = new Complex128Array( tc.BB );
	const Xm = new Complex128Array( N * ldx );
	const WORK = new Complex128Array( N );
	const RWORK = new Float64Array( N );

	const info = zhbgst( vect, uplo, N, ka, kb, AB, 1, ldab, 0, BB, 1, ldbb, 0, Xm, 1, ldx, 0, WORK, 1, 0, RWORK, 1, 0 );

	assert.equal( info, tc.info, 'info' );

	const av = new Float64Array( reinterpret( AB, 0 ) );
...
test( 'zhbgst: n_zero quick return', function t() {
	...
	const AB = new Complex128Array( 2 );
	const BB = new Complex128Array( 1 );
	const X = new Complex128Array( 1 );

	const info = zhbgst( 'none', 'upper', 0, 1, 0, AB, 1, 2, 0, BB, 1, 1, 0, X, 1, 1, 0, WORK, 1, 0, RWORK, 1, 0 );
...
test( 'zhbgst: upper_n10_ka2_kb2_vect (multiple inner kb-loop iterations)', function t() {
	...
	const tc = upperN10Ka2Kb2Vect;
	const AB = new Complex128Array( tc.AB_in );
	const BB = new Complex128Array( tc.BB );
	const Xm = new Complex128Array( 10 * 10 );
	const WORK = new Complex128Array( 10 );
	const RWORK = new Float64Array( 10 );

	const info = zhbgst( 'update', 'upper', 10, 2, 2, AB, 1, 3, 0, BB, 1, 3, 0, Xm, 1, 10, 0, WORK, 1, 0, RWORK, 1, 0 );
...
test( 'zhbgst: lower_n10_ka2_kb2_vect (multiple inner kb-loop iterations)', function t() {
	...
	const info = zhbgst( 'update', 'lower', 10, 2, 2, AB, 1, 3, 0, BB, 1, 3, 0, Xm, 1, 10, 0, WORK, 1, 0, RWORK, 1, 0 );
	assert.strictEqual( info, 0, 'info' );
	const av = new Float64Array( reinterpret( AB, 0 ) );
	for ( let i = 0; i < av.length; i++ ) {
		assert.ok( isFinite( av[ i ] ), 'AB[' + i + '] is finite' );
...
test( 'zhbgst: upper_n12_ka3_kb3_vect (KA == KB == 3)', function t() {
	...
	const tc = upperN12Ka3Kb3Vect;
	const AB = new Complex128Array( tc.AB_in );
	const BB = new Complex128Array( tc.BB );
	const Xm = new Complex128Array( 12 * 12 );
	const WORK = new Complex128Array( 12 );
	const RWORK = new Float64Array( 12 );

	const info = zhbgst( 'update', 'upper', 12, 3, 3, AB, 1, 4, 0, BB, 1, 4, 0, Xm, 1, 12, 0, WORK, 1, 0, RWORK, 1, 0 );
	assert.strictEqual( info, 0, 'info' );
	const av = new Float64Array( reinterpret( AB, 0 ) );
	for ( let i = 0; i < av.length; i++ ) {
		assert.ok( isFinite( av[ i ] ), 'AB[' + i + '] is finite' );
...
test( 'zhbgst: lower_n12_ka3_kb3_vect (KA == KB == 3)', function t() {
	...
	const info = zhbgst( 'update', 'lower', 12, 3, 3, AB, 1, 4, 0, BB, 1, 4, 0, Xm, 1, 12, 0, WORK, 1, 0, RWORK, 1, 0 );
	assert.strictEqual( info, 0, 'info' );
	const av = new Float64Array( reinterpret( AB, 0 ) );
	for ( let i = 0; i < av.length; i++ ) {
		assert.ok( isFinite( av[ i ] ), 'AB[' + i + '] is finite' );
...
test( 'zhbgst: upper_n7_ka2_kb2_vect (smallest hitting inner kb-loop)', function t() {
	...
	const tc = upperN7Ka2Kb2Vect;
	const AB = new Complex128Array( tc.AB_in );
	const BB = new Complex128Array( tc.BB );
	const Xm = new Complex128Array( 7 * 7 );
	const WORK = new Complex128Array( 7 );
	const RWORK = new Float64Array( 7 );

	const info = zhbgst( 'update', 'upper', 7, 2, 2, AB, 1, 3, 0, BB, 1, 3, 0, Xm, 1, 7, 0, WORK, 1, 0, RWORK, 1, 0 );
	assert.strictEqual( info, 0, 'info' );
...
test( 'zhbgst: lower_n7_ka2_kb2_vect (smallest hitting inner kb-loop)', function t() {
	...
	const info = zhbgst( 'update', 'lower', 7, 2, 2, AB, 1, 3, 0, BB, 1, 3, 0, Xm, 1, 7, 0, WORK, 1, 0, RWORK, 1, 0 );
	assert.strictEqual( info, 0, 'info' );
	const av = new Float64Array( reinterpret( AB, 0 ) );
	assert.equal( av.length, tc.AB.length, 'AB length matches' );
	for ( let i = 0; i < av.length; i++ ) {
