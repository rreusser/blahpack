...
import ztgsna from '../../ztgsna/lib/base.js';
...
function zggevx( balanc, jobvl, jobvr, sense, N, A, strideA1, strideA2, offsetA, B, strideB1, strideB2, offsetB, ALPHA, strideALPHA, offsetALPHA, BETA, strideBETA, offsetBETA, VL, strideVL1, strideVL2, offsetVL, VR, strideVR1, strideVR2, offsetVR, LSCALE, strideLSCALE, offsetLSCALE, RSCALE, strideRSCALE, offsetRSCALE, RCONDE, strideRCONDE, offsetRCONDE, RCONDV, strideRCONDV, offsetRCONDV ) {
	let rcondeS, rcondvS, snaJob, workVL, workVR, snaRes, anrmto, bnrmto, snaWork;
	let ilascl, ilbscl, chtemp, BWORK, IWORK, icols, abnrm, bbnrm, iIdx, jVal;
	let anrm, bnrm, info, ierr, temp, sVL1, sVL2, sVR1, sVR2, VLv, VRv, oVL;
	let oVR, ilo, ihi, jc, jr, i;
	...
	const noscl = ( balanc === 'none' ) || ( balanc === 'permute' );
	...
	const WORK = new Complex128Array( Math.max( 1, ( N * 32 ) + ( 33 * 32 ) ) );
	const TAU = new Complex128Array( N );

	// RWORK needs 6*N for zggbal, 2*N for zhgeqz, 2*N for ztgevc; 8*N is safe.
	const RWORK = new Float64Array( Math.max( 1, 8 * N ) );
	const RWORKscratch = new Float64Array( 1 );
	const Mout = [ 0 ];
	...
	const bal = zggbal( balanc, N, A, strideA1, strideA2, offsetA, B, strideB1, strideB2, offsetB, LSCALE, strideLSCALE, offsetLSCALE, RSCALE, strideRSCALE, offsetRSCALE, RWORK, 1, 0 );
	ilo = bal.ilo; // 1-based
	ihi = bal.ihi; // 1-based
	...
	ierr = zhgeqz( chtemp, ( ( ilvl ) ? 'update' : 'none' ), ( ( ilvr ) ? 'update' : 'none' ), N, ilo, ihi, A, strideA1, strideA2, offsetA, B, strideB1, strideB2, offsetB, ALPHA, strideALPHA, offsetALPHA, BETA, strideBETA, offsetBETA, VL, strideVL1, strideVL2, offsetVL, VR, strideVR1, strideVR2, offsetVR, WORK, 1, 0, RWORK, 1, 0 );
	if ( ierr !== 0 ) {
		if ( ierr > 0 && ierr <= N ) {
			info = ierr;
	...
	if ( ilv || !wantsn ) {
		...
		if ( !wantsn ) {
			...
			// Dedicated complex workspace for ztgsna's eigenvector-condition
			// (DIF) path, which copies (A,B) and runs ztgsyl: needs
			// `2*N*N + max(1,4*(N-1))` elements. zggevx's shared WORK is sized
			// for its own reductions and is not guaranteed large enough.
			snaWork = new Complex128Array( Math.max( 1, ( 2 * N * N ) + Math.max( 1, 4 * ( N - 1 ) ) ) );
			for ( iIdx = 0; iIdx < N; iIdx++ ) {
				...
				rcondeS[ 0 ] = ZERO;
				rcondvS[ 0 ] = ZERO;
				snaRes = ztgsna( snaJob, 'selected', BWORK, 1, 0, N, A, strideA1, strideA2, offsetA, B, strideB1, strideB2, offsetB, workVL, 1, N, 0, workVR, 1, N, 0, rcondeS, 1, 0, rcondvS, 1, 0, 1, 0, snaWork, 1, 0, IWORK, 1, 0 );
				if ( snaRes.info < 0 ) {
					info = N + 2;
					return finalize();
				}
