...
const lines = readFileSync( path.join( fixtureDir, 'zsytri2.jsonl' ), 'utf8' ).trim().split( '\n' ); // eslint-disable-line max-len
...
function runFixture( uplo, N, tc, tol ) {
	...
	const info = zsytri2( uplo, N, A, 1, LDA, 0, ipiv, 1, 0, work, 1, 0 );
...
test( 'zsytri2: N=1 trivial inverse (unblocked path)', function t() {
	...
	const info = zsytri2( 'upper', 1, A, 1, LDA, 0, ipiv, 1, 0, work, 1, 0 );
	assert.equal( info, tc.info, 'info' );
	const Aview = new Float64Array( A.buffer );
...
test( 'zsytri2: N=0 quick return', function t() {
	...
	const info = zsytri2( 'lower', 0, A, 1, 1, 0, ipiv, 1, 0, work, 1, 0 );
