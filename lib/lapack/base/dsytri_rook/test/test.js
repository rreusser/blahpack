...
test( 'main export inverts a 1x1 matrix', function t() {
	...
	const info = dsytriRook( 'column-major', 'upper', 1, A, 1, IPIV, 1, WORK );
...
test( 'ndarray method inverts a 1x1 matrix', function t() {
	...
	const IPIV = new Int32Array( [ 0 ] );
	const WORK = new Float64Array( 1 );
	const info = dsytriRook.ndarray( 'lower', 1, A, 1, 1, 0, IPIV, 1, 0, WORK, 1, 0 );
	assert.equal( info, 0, 'info' );
	assert.equal( A[ 0 ], 0.2, 'A[0]' );
});

test( 'main export quick-returns for N=0', function t() {
	const info = dsytriRook( 'column-major', 'upper', 0, new Float64Array( 0 ), 1, new Int32Array( 0 ), 1, new Float64Array( 0 ) ); // eslint-disable-line max-len
	assert.equal( info, 0, 'info' );
});
