...
import zungtsqr_row from './../lib/ndarray.js';


// FIXTURES //

const fixtureDir = path.join( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures' );
const fixtureRaw = readFileSync( path.join( fixtureDir, 'zungtsqr_row.jsonl' ), 'utf8' ); // eslint-disable-line node/no-sync
...
function assertOrthonormalColumns( Q, M, N, tol, msg ) {
	let expected, dotR, dotI, aR, aI, bR, bI, i, j, k;
	const view = reinterpret( Q, 0 );
	for ( j = 0; j < N; j++ ) {
		for ( i = 0; i < N; i++ ) {
			dotR = 0.0;
			dotI = 0.0;
...
function runCase( cfg ) {
	...
	const info = zungtsqr_row( M, N, mb, nb, A, 1, M, 0, T, 1, nb, 0, WORK, 1, 0 );
...
function colToRow( Acm, M, N ) {
	let idxC, idxR, i, j;
	const Arm = new Complex128Array( M * N );
	const Acmv = reinterpret( Acm, 0 );
	const Armv = reinterpret( Arm, 0 );
	for ( j = 0; j < N; j++ ) {
		for ( i = 0; i < M; i++ ) {
			idxC = ( ( j * M ) + i ) * 2;
			idxR = ( ( i * N ) + j ) * 2;
...
function colToRowT( Tcm, nbT, ncolsT ) {
	const Trm = new Complex128Array( nbT * ncolsT );
	const Tcv = reinterpret( Tcm, 0 );
	const Trv = reinterpret( Trm, 0 );
	let ii, jj;
...
test( 'zungtsqr_row: m=12, n=3, mb=5, nb=3 (NB == N column block)', function t() {
	runCase({
		...
		'A': buildA( 12, 3, function fill( i, j ) {
			...
			const a = ( ( ( ( i * 7 ) + ( j * 3 ) ) % 11 ) / 5.0 ) + 0.1;
			const b = ( ( ( ( i * 5 ) + ( j * 2 ) ) % 7 ) / 8.0 ) - 0.3;
			return [ a, b ];
		}),
		'tol': 1e-12
	});
...
test( 'zungtsqr_row: m=0, n=0 quick return', function t() {
	...
	const info = zungtsqr_row( 0, 0, 1, 1, A, 1, 1, 0, T, 1, 1, 0, WORK, 1, 0 );
...
test( 'zungtsqr_row: m=3, n=0 quick return', function t() {
	...
	const T = new Complex128Array( 1 );
	const WORK = new Complex128Array( 1 );
	const info = zungtsqr_row( 3, 0, 1, 1, A, 1, 3, 0, T, 1, 1, 0, WORK, 1, 0 );
	assert.strictEqual( info, 0, 'INFO' );
});
...
test( 'zungtsqr_row: row-major layout (M=6, N=3) verifies stride symmetry', function t() {
	...
	const Acm = buildA( 6, 3, function fill( ii, jj ) {
		if ( ii === jj ) {
			return [ 5.0 + jj, 0.1 ];
		}
		return [ 0.4 / ( Math.abs( ii - jj ) + 1 ), 0.02 * ( ii - jj ) ];
	});
	const nblk = numBlocks( 6, 3, 4 );
	const Tcm = new Complex128Array( 2 * 3 * nblk );
	WORK = new Complex128Array( 2 * 3 );
	zlatsqr( 6, 3, 4, 2, Acm, 1, 6, 0, Tcm, 1, 2, 0, WORK, 1, 0 );

	// Build row-major copies of V and T.
	const Arm = colToRow( Acm, 6, 3 );
	const Trm = colToRowT( Tcm, 2, 3 * nblk );
