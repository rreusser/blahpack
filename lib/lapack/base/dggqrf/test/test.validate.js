...
/**
* Property-based validation for dggqrf, following the /blahpack-validate process.
*
...
*   `dggqrf(N, M, P, A, TAUA, B, TAUB, WORK)` factors an N-by-M matrix A and an
*   N-by-P matrix B as
*
*       A = Q * R,        B = Q * T * Z,
*
...
import dggqrf from './../lib/ndarray.js';

const sc = S.real; // d-routine
const LogicalMatrix = logical.LogicalMatrix;
const ROUTINE = 'dggqrf';
...
function workLen( N, M, P ) {
	const kA = Math.min( N, M );
	const kB = Math.min( N, P );
	const ge = ( kA > NB ) ? ( ( M * NB ) + ( NB * NB ) ) : Math.max( 1, M );
	const or = ( kA > NB ) ? ( ( P * NB ) + ( ( NB + 1 ) * NB ) ) : Math.max( 1, P );
	const rq = ( kB > NB ) ? ( ( N * NB ) + ( NB * NB ) ) : Math.max( 1, N );
...
function fullZ_RQ( rd, taus, rows, cols, k ) {
	const Z = new LogicalMatrix( sc, cols, cols );
	let i;
	for ( i = 0; i < cols; i++ ) {
		Z.set( i, i, sc.one );
	}
...
function factor( N, M, P, layoutA, layoutB, wlen, doPoison ) {
	const kA = Math.min( N, M );
	const kB = Math.min( N, P );
	const rng = new RNG( 0x51 + ( N * 10000 ) + ( M * 100 ) + P );
	const A0 = logical.general( sc, rng, N, M );
	const B0 = logical.general( sc, rng, N, P );
	...
	dggqrf( N, M, P, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], TAr.data, TAr.args[ 0 ], TAr.args[ 1 ], Br.data, Br.args[ 0 ], Br.args[ 1 ], Br.args[ 2 ], TBr.data, TBr.args[ 0 ], TBr.args[ 1 ], work, 1, 0 );
...
function assertGGQRF( f, N, M, P, label ) {
	...
	const Q = fullQ_QR( f.Ar, f.tausA, N, kA );
	...
	const T = readR_RQ( f.Br, N, P );
	const Z = fullZ_RQ( f.Br, f.tausB, N, P, kB );
	checked( ROUTINE, 'orthonormal', function run() {
		check.assertOrthonormal( sc, Z, { 'label': label + ' Z' } );
	} );
	const TZ = ref.matmul( sc, T, Z );
	const Brec = ref.matmul( sc, Q, TZ );
	checked( ROUTINE, 'reconstruct', function run() {
...
test( 'dggqrf: A=Q*R, B=Q*T*Z, orthonormal Q & Z ((N,M,P) sweep x layouts)', function t() {
	TRIPLES.forEach( function eachTriple( tr ) {
		const N = tr[ 0 ];
		const M = tr[ 1 ];
		const P = tr[ 2 ];
		ALL_LAYOUTS.forEach( function eachLayout( layout, li ) {
			const f = factor( N, M, P, layout, ALL_LAYOUTS[ ( li + 3 ) % ALL_LAYOUTS.length ], workLen( N, M, P ), false );
...
function flatFactored( f, N, M, P ) {
	let flat = check.flattenLogical( sc, readMat( f.Ar, N, M ) );
...
function runInvariance( variants, fam, N, M, P ) {
	const kA = Math.min( N, M );
	const kB = Math.min( N, P );
	const wl = workLen( N, M, P );
	const SEED = 0xF00D + ( N * 131 ) + ( M * 17 ) + P;
	checked( ROUTINE, 'layout-invariance', function run() {
		layoutInvariant( variants, function build( layout, i ) {
			...
			dggqrf( N, M, P, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], TAr.data, TAr.args[ 0 ], TAr.args[ 1 ], Br.data, Br.args[ 0 ], Br.args[ 1 ], Br.args[ 2 ], TBr.data, TBr.args[ 0 ], TBr.args[ 1 ], W.data, W.args[ 0 ], W.args[ 1 ] );
			const tausA = [];
			const tausB = [];
			let j;
			for ( j = 0; j < kA; j++ ) {
...
test( 'dggqrf: bit-exact within storage-order family (col / row), blocked', function t() {
...
test( 'dggqrf: advertised WORK minimum suffices on the blocked path (Step 4c)', function t() {
	[ [ 64, 64, 64 ], [ 40, 64, 40 ], [ 64, 40, 64 ] ].forEach( function eachCase( c ) {
		const N = c[ 0 ];
		const M = c[ 1 ];
		const P = c[ 2 ];
		const label = ROUTINE + ' WORK-min N=' + N + ' M=' + M + ' P=' + P;

		function run( len ) {
			const f = factor( N, M, P, null, null, len, true );
			let flat = check.flattenLogical( sc, readMat( f.Ar, N, M ) );
			flat = flat.concat( check.flattenLogical( sc, readMat( f.Br, N, P ) ) );
			return flat;
		}
		...
		const f = factor( N, M, P, null, null, minLen, true );
