...
import ztrsv from '../../../../blas/base/ztrsv/lib/base.js';
...
// Machine constants
const SMLNUM = dlamch( 'safe-minimum' ) / dlamch( 'epsilon' );
const BIGNUM = ONE / SMLNUM;
const RMAX = dlamch( 'overflow' );
...
function computeTransposeSum( av, xv, oA, ox, sa1, sa2, sx, j, N, uscalRe, uscalIm, tscal, upper, conjugate, A, x, strideA1, strideA2, offsetA, strideX, offsetX, out ) {
	let csumjR = ZERO;
	let csumjI = ZERO;
	let dotResult, ar, ai, xr, xi, ur, ui, i;
...
function zlatrs( uplo, trans, diag, normin, N, A, strideA1, strideA2, offsetA, x, strideX, offsetX, scale, CNORM, strideCNORM, offsetCNORM ) {
	let jfirst, tscal, jlast, xbnd, xmax, grow, tmax, jinc, tjjs_re, tjjs_im;
	let uscal_re, uscal_im, rec, tjj, xj, jr, ji, j, i;

	const sa1 = strideA1 * 2; // Float64 strides
		...
		if ( tmax <= RMAX ) {
			...
			for ( j = 0; j < N; j++ ) {
				if ( CNORM[ offsetCNORM + j * sc ] <= RMAX ) {
					...
					const newTscal = TWO * tscal; // eslint-disable-line no-var
