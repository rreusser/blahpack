...
test( 'dpbsv: upper tridiagonal N=5, NRHS=1', function t() {
	...
	const info = dpbsv( 'upper', 5, 1, 1, AB, 1, 2, 0, B, 1, 5, 0 );
...
test( 'dpbsv: lower tridiagonal N=5, NRHS=1', function t() {
	...
	const B = new Float64Array( [ 1, 0, 0, 0, 1 ] );
	const info = dpbsv( 'lower', 5, 1, 1, AB, 1, 2, 0, B, 1, 5, 0 );
	assert.strictEqual( info, tc.info );
	assertArrayClose( toArray( B ), tc.x, 1e-10, 'x' );
});
...
test( 'dpbsv: upper tridiagonal N=5, NRHS=2', function t() {
	...
	const info = dpbsv( 'upper', 5, 1, 2, AB, 1, 2, 0, B, 1, 5, 0 );
	assert.strictEqual( info, tc.info );
	assertArrayClose( toArray( B ), tc.x, 1e-10, 'x' );
});
...
test( 'dpbsv: N=0 quick return', function t() {
	...
	const info = dpbsv( 'upper', 0, 0, 1, AB, 1, 1, 0, B, 1, 1, 0 );
	assert.strictEqual( info, tc.info );
});
...
test( 'dpbsv: NRHS=0 quick return', function t() {
	...
	const B = new Float64Array( 1 );
	const info = dpbsv( 'lower', 1, 0, 0, AB, 1, 1, 0, B, 1, 1, 0 );
	assert.strictEqual( info, tc.info );
});
...
test( 'dpbsv: N=1', function t() {
	...
	const info = dpbsv( 'upper', 1, 0, 1, AB, 1, 1, 0, B, 1, 1, 0 );
	assert.strictEqual( info, tc.info );
	assertArrayClose( toArray( B ), tc.x, 1e-10, 'x' );
});
...
test( 'dpbsv: not positive definite (INFO > 0)', function t() {
	...
	const info = dpbsv( 'lower', 2, 1, 1, AB, 1, 2, 0, B, 1, 2, 0 );
	assert.strictEqual( info, tc.info );
});
...
test( 'dpbsv: lower pentadiagonal N=4, NRHS=1', function t() {
	...
	const info = dpbsv( 'lower', 4, 2, 1, AB, 1, 3, 0, B, 1, 4, 0 );
	assert.strictEqual( info, tc.info );
	assertArrayClose( toArray( B ), tc.x, 1e-10, 'x' );
});
...
test( 'dpbsv: upper pentadiagonal N=4, NRHS=1', function t() {
	...
	const B = new Float64Array( [ 1, 2, 3, 4 ] );
	const info = dpbsv( 'upper', 4, 2, 1, AB, 1, 3, 0, B, 1, 4, 0 );
	assert.strictEqual( info, tc.info );
	assertArrayClose( toArray( B ), tc.x, 1e-10, 'x' );
});
