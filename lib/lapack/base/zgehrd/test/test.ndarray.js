...
import full4x4 from './fixtures/4x4_full.json' with { type: 'json' };
import full5x5 from './fixtures/5x5_full.json' with { type: 'json' };
import full6x6 from './fixtures/6x6_full.json' with { type: 'json' };
import full8x8 from './fixtures/8x8_full.json' with { type: 'json' };
import partial4x4 from './fixtures/4x4_partial_ilo2_ihi3.json' with { type: 'json' };
import nOne from './fixtures/n_one.json' with { type: 'json' };
import nTwo from './fixtures/n_two.json' with { type: 'json' };
...
function runTest( name, N, ilo, ihi, inputA ) {
	...
	const A = new Complex128Array( N * N ); const av = reinterpret( A, 0 ); av.set( inputA );
	const TAU = new Complex128Array( Math.max( 1, N - 1 ) );
	const WORK = new Complex128Array( Math.max( 1, N * 32 + ( NBMAX + 1 ) * NBMAX ) );
	const info = zgehrd(N, ilo, ihi, A, 1, N, 0, TAU, 1, 0, WORK, 1, 0 );
	assert.equal( info, tc.INFO, 'INFO' );
	assertArrayClose( av, new Float64Array( tc.A ), 1e-13, 'A' );
	if ( tc.TAU ) {
		const tv = reinterpret( TAU, 0 );
		const tauSlice = new Float64Array( tc.TAU.length );
		for ( let i = 0; i < tc.TAU.length; i++ ) { tauSlice[ i ] = tv[ i ]; }
...
test( 'zgehrd: n1', function () {
	...
	const info = zgehrd(1, 1, 1, A, 1, 1, 0, TAU, 1, 0, WORK, 1, 0 );
...
test( 'zgehrd: 8x8', function () {
	const N = 8;
	const inputA = new Float64Array( 2*N*N );
	for ( let idx = 1; idx <= N*N; idx++ ) { inputA[(idx-1)*2] = idx; inputA[(idx-1)*2+1] = ((idx*7+3)%11 - 5)*0.5; }
	runTest( '8x8_full', 8, 1, 8, inputA );
});
test( 'zgehrd: 40x40 blocked', function () {
	const N = 40;
	const inputA = new Float64Array( 2*N*N );
	for ( let idx = 1; idx <= N*N; idx++ ) { inputA[(idx-1)*2] = ((idx*13+7)%97)/10.0; inputA[(idx-1)*2+1] = ((idx*7+3)%53-26)/20.0; }
	const A1 = new Complex128Array( N*N ); const av1 = reinterpret( A1, 0 ); av1.set( inputA );
	const TAU1 = new Complex128Array( N-1 );
	const WORK1 = new Complex128Array( N*32 + (NBMAX+1)*NBMAX );
	const info = zgehrd(N, 1, N, A1, 1, N, 0, TAU1, 1, 0, WORK1, 1, 0 );
	...
	let trOrigR = 0, trOrigI = 0, trHessR = 0, trHessI = 0;
	for ( i = 0; i < N; i++ ) {
		trOrigR += inputA[2*(i*N+i)]; trOrigI += inputA[2*(i*N+i)+1];
		trHessR += av1[2*(i*N+i)]; trHessI += av1[2*(i*N+i)+1];
	}
	const trDiff = Math.sqrt( (trOrigR-trHessR)*(trOrigR-trHessR) + (trOrigI-trHessI)*(trOrigI-trHessI) );
	...
	const normOrig = Math.sqrt( normOrigSq );
	const normHess = Math.sqrt( normHessSq );
	const normDiff = Math.abs( normOrig - normHess ) / normOrig;
