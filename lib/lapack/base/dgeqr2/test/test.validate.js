/**
* Property-based validation for dgeqr2, following the /blahpack-validate process.
*
...
import dgeqr2 from './../lib/ndarray.js';
...
test( 'dgeqr2: A = Q*R and QᴴQ = I ((M,N) sweep x all layouts)', function t() {
	PAIRS.forEach( function eachPair( pr ) {
		...
		ALL_LAYOUTS.forEach( function eachLayout( layout, li ) {
			...
			dgeqr2( M, N, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], Tr.data, Tr.args[ 0 ], Tr.args[ 1 ], Wr.data, Wr.args[ 0 ], Wr.args[ 1 ] );
			...
			const label = 'dgeqr2 M=' + M + ' N=' + N + ' layout=' + li;

			const recon = reconstruct( Ar, taus, M, N, k );
			checked( 'dgeqr2', 'reconstruct', function run() {
				check.assertReconstruct( sc, recon, A0, { 'label': label } );
			} );

			const Q = formQ( Ar, taus, M, k );
			checked( 'dgeqr2', 'orthonormal', function run() {
...
// Step 4: layout-invariance fuzz. dgeqr2 -> dlarf -> optimized dgemv/dger, which
...
test( 'dgeqr2: bit-exact within storage-order family (col / row)', function t() {
...
function runInvariance( variants, fam ) {
	...
	const k = Math.min( M, N );
	const SEED = 0xF00D;
	checked( 'dgeqr2', 'layout-invariance', function run() {
		...
		}, { 'label': 'dgeqr2 layout invariance ' + fam + '-major' } );
	} );
}
