...
import dla_gercond from './../lib/ndarray.js';


// FIXTURES //

const fixtureDir = path.join( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures' );
const lines = readFileSync( path.join( fixtureDir, 'dla_gercond.jsonl' ), 'utf8' ).trim().split( '\n' ); // eslint-disable-line max-len, node/no-sync
...
function setup3x3() {
	...
	const A = new Float64Array( [ 2.0, 1.0, 0.0, 1.0, 3.0, 1.0, 0.0, 1.0, 4.0 ] );
...
function run3x3( trans, cmode, c, caseName ) {
	...
	const result = dla_gercond( trans, 3, obj.A, 1, 3, 0, obj.AF, 1, 3, 0, obj.IPIV, 1, 0, cmode, c, 1, 0, WORK, 1, 0, IWORK, 1, 0 ); // eslint-disable-line max-len
...
test( 'dla_gercond: N=1 edge case', function t() {
	...
	const result = dla_gercond( 'no-transpose', 1, A, 1, 1, 0, AF, 1, 1, 0, IPIV, 1, 0, 1, c, 1, 0, WORK, 1, 0, IWORK, 1, 0 ); // eslint-disable-line max-len
...
test( 'dla_gercond: N=0 returns 1.0', function t() {
	...
	const result = dla_gercond( 'no-transpose', 0, A, 1, 1, 0, AF, 1, 1, 0, IPIV, 1, 0, 1, c, 1, 0, WORK, 1, 0, IWORK, 1, 0 ); // eslint-disable-line max-len
