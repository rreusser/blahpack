...
function dgelqt( order, M, N, mb, A, LDA, T, LDT, WORK ) {
	...
	if ( WORK === null || WORK === void 0 ) {
		// See ndarray.js: the trailing `dlarfb` (side='right', rowwise) scratch
		// scales with trailing ROWS, peaking at `(M-min(mb,K))*min(mb,K)`; the
		// reference `mb*N` is a lower bound valid only for M<=N, so take the max.
		const ib0 = ( mb < k ) ? mb : k;
		const peak = ( M - ib0 ) * ib0;
		const minWork = ( k === 0 ) ? 0 : max( mb * N, peak );
