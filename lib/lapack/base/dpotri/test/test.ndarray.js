...
const ndarray = dpotri;

// FIXTURES //

import upper_3x3_chol from './fixtures/upper_3x3_chol.json' with { type: 'json' };
import upper_3x3 from './fixtures/upper_3x3.json' with { type: 'json' };
import lower_3x3_chol from './fixtures/lower_3x3_chol.json' with { type: 'json' };
...
test( 'dpotri: UPLO=U, 3x3 SPD matrix', function t() {
	const tc_chol = upper_3x3_chol;
	const tc = upper_3x3;
	const A = new Float64Array( tc_chol.chol );

	const info = dpotri( 'upper', 3, A, 1, 3, 0 );
	...
	const origA = new Float64Array([
		4, 2, 1,
		2, 5, 3,
		1, 3, 6
	]);
	const C = symmMul( 'upper', 3, A, origA );
	assertIdentity( C, 3, 1e-13, 'upper_3x3' );
});

test( 'dpotri: UPLO=L, 3x3 SPD matrix', function t() {
	const tc_chol = lower_3x3_chol;
	const tc = lower_3x3;
	const A = new Float64Array( tc_chol.chol );

	const info = dpotri( 'lower', 3, A, 1, 3, 0 );
	...
	const C = symmMul( 'lower', 3, A, origA );
...
test( 'dpotri: N=1 edge case', function t() {
	...
	const info = dpotri( 'upper', 1, A, 1, 1, 0 );
...
test( 'dpotri: N=0 quick return', function t() {
	const tc = n_zero;
	const A = new Float64Array( [ 999.0 ] );

	const info = dpotri( 'upper', 0, A, 1, 1, 0 );
...
test( 'dpotri: UPLO=U, 4x4 SPD matrix', function t() {
	...
	const origA = new Float64Array( A );
	...
	const C = symmMul( 'upper', 4, A, origA );
...
test( 'dpotri: UPLO=L, 4x4 SPD matrix', function t() {
	...
	// Verify mathematical property
	const C = symmMul( 'lower', 4, A, origA );
...
test( 'dpotri: non-zero offset', function t() {
	...
	const invA = new Float64Array( 9 );
	for ( i = 0; i < 9; i++ ) {
		invA[ i ] = A[ offset + i ];
	}
	const C = symmMul( 'upper', 3, invA, origA );
