/**
* Property-based validation for dsyrfs, following the /blahpack-validate process.
*
* Step 0 classification: `d` -> real scalar; `sy` -> symmetric INDEFINITE dense
* (schemes.dense, logical.symmetric); `rfs` (iterative refinement + error bounds)
* -> three independent properties. dsyrfs refines an approximate solution X to
* A*X = B and returns FERR (forward-error bound) and BERR (componentwise backward
* error). The Bunch-Kaufman factor AF + IPIV are produced by the already-validated
* dsytrf; the un-refined initial X by dsytrs; the TRUE solution independently by
* the trusted dsysv on fresh copies. We assert, against the ORIGINAL full
* symmetric A0:
...
import dsyrfs from './../lib/ndarray.js';
...
// (Bunch-Kaufman factor + solve; needs an N-length caller-owned WORK).
function trueSolution( A0, B0, N, nrhs, uplo ) {
	...
	const info = dsysv( uplo, N, nrhs, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], IPIV, 1, 0, Br.data, Br.args[ 0 ], Br.args[ 1 ], Br.args[ 2 ], WORK, 1, 0 );
	if ( info !== 0 ) {
		throw new Error( 'oracle dsysv failed (info='+info+'); matrix singular?' );
	}
	return Br;
}

// Steps 2-3-5: three properties across uplo flags, a size sweep, and nrhs.
test( 'dsyrfs: refinement residual + BERR + FERR bound (uplo x N x nrhs)', function t() {
...
function runProperty( uplo, N, nrhs ) {
	...
	const IPIV = new Int32Array( N );
	dsytrf( uplo, N, AFr.data, AFr.args[ 0 ], AFr.args[ 1 ], AFr.args[ 2 ], IPIV, 1, 0 );

	// B (RHS, unchanged) and X (initial un-refined solve, refined in place):
	const Br = schemes.dense.realize( sc, B0, { 'part': 'full' }, TIGHT );
	const Xr = schemes.dense.realize( sc, B0, { 'part': 'full' }, TIGHT );
	dsytrs( uplo, N, nrhs, AFr.data, AFr.args[ 0 ], AFr.args[ 1 ], AFr.args[ 2 ], IPIV, 1, 0, Xr.data, Xr.args[ 0 ], Xr.args[ 1 ], Xr.args[ 2 ] );
	...
	dsyrfs( uplo, N, nrhs, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], AFr.data, AFr.args[ 0 ], AFr.args[ 1 ], AFr.args[ 2 ], IPIV, 1, 0, Br.data, Br.args[ 0 ], Br.args[ 1 ], Br.args[ 2 ], Xr.data, Xr.args[ 0 ], Xr.args[ 1 ], Xr.args[ 2 ], FERR, 1, 0, BERR, 1, 0, WORK, 1, 0, IWORK, 1, 0 );

	const Xtrue = trueSolution( A0, B0, N, nrhs, uplo );

	const tag = 'dsyrfs '+uplo+' N='+N+' nrhs='+nrhs;

	// (a) residual: refined X remains a valid solution of A0*X = B0.
	checked( 'dsyrfs', 'residual', function run() {
		let j;
		for ( j = 0; j < nrhs; j++ ) {
	...
	checked( 'dsyrfs', 'structural', function run() {
...
// per layout together with the fixed A0 and B0; dsyrfs's internal kernels (dsymv,
// dsytrs, daxpy, dlacn2) run over identical values, so only ADDRESSING changes
// and the refined X + FERR + BERR must reproduce bit-for-bit. The fixed IPIV is
// consumed by dsytrs (no pivot search of its own), so the full col/row families
...
test( 'dsyrfs: bit-exact within storage-order family (col / row)', function t() {
	UPLOS.forEach( function eachUplo( uplo ) {
...
function runInvariance( uplo, variants, fam ) {
	...
	dsytrf( uplo, N, AF0.data, AF0.args[ 0 ], AF0.args[ 1 ], AF0.args[ 2 ], IPIV0, 1, 0 );
	...
	dsytrs( uplo, N, nrhs, AF0.data, AF0.args[ 0 ], AF0.args[ 1 ], AF0.args[ 2 ], IPIV0, 1, 0, X0r.data, X0r.args[ 0 ], X0r.args[ 1 ], X0r.args[ 2 ] );
	const Xinit = readMat( X0r, N, nrhs );

	checked( 'dsyrfs', 'layout-invariance', function run() {
		layoutInvariant( variants, function build( layout ) {
			...
			const WORK = new Float64Array( 3 * N );
			const IWORK = new Int32Array( N );
			dsyrfs( uplo, N, nrhs, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], AFr.data, AFr.args[ 0 ], AFr.args[ 1 ], AFr.args[ 2 ], IPIV0, 1, 0, Br.data, Br.args[ 0 ], Br.args[ 1 ], Br.args[ 2 ], Xr.data, Xr.args[ 0 ], Xr.args[ 1 ], Xr.args[ 2 ], FERR, 1, 0, BERR, 1, 0, WORK, 1, 0, IWORK, 1, 0 );
			...
			return out;
		}, { 'label': 'dsyrfs '+uplo+' layout invariance '+fam+'-major' } );
	});
}
