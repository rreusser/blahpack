...
function runCase( caseName, M, N, mb1, nb1, nb2, A, tol ) {
	...
	const info = zgetsqrhrt( M, N, mb1, nb1, nb2, A, 1, M, 0, Tinfo.T, 1, Tinfo.LDT, 0 );
...
test( 'zgetsqrhrt: m=0, n=0 quick return', function t() {
	...
	const info = zgetsqrhrt( 0, 0, 1, 1, 1, A, 1, 1, 0, T, 1, 1, 0 );
...
test( 'zgetsqrhrt: m=3, n=0 quick return', function t() {
	...
	const T = new Complex128Array( 1 );
	const info = zgetsqrhrt( 3, 0, 1, 1, 1, A, 1, 3, 0, T, 1, 1, 0 );
	assert.equal( info, 0, 'INFO' );
});
...
test( 'zgetsqrhrt: R diagonal is real-positive after factorization', function t() {
	...
	const info = zgetsqrhrt( 8, 3, 4, 2, 2, A, 1, 8, 0, T, 1, 2, 0 );
	assert.equal( info, 0, 'INFO' );
	const view = reinterpret( A, 0 );
	for ( i = 0; i < 3; i++ ) {
		diag = view[ ( ( ( i * 8 ) + i ) * 2 ) ];
