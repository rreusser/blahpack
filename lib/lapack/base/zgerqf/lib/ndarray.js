...
*/
function zgerqf( M, N, A, strideA1, strideA2, offsetA, TAU, strideTAU, offsetTAU, WORK, strideWork, offsetWork ) { // eslint-disable-line max-len, max-params
	var need;
	var K;
	...
	// from an out-of-bounds read. base blocks when min(M,N) > NB (needing
	// M*NB + NB*NB), otherwise runs unblocked (needing M). K===0 is a quick
	// return that touches no workspace.
	K = ( M < N ) ? M : N;
	if ( K > 0 ) {
		need = ( K > NB ) ? ( ( M * NB ) + ( NB * NB ) ) : max( 1, M );
