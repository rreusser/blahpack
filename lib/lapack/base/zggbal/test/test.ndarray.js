...
import zggbal from './../lib/ndarray.js';
...
test( 'zggbal: JOB=N, 4x4 — sets ilo=1, ihi=N, scales=1', function t() {
	...
	const result = zggbal( 'none', n, A, 1, n, 0, B, 1, n, 0, lscale, 1, 0, rscale, 1, 0, work, 1, 0 );
...
test( 'zggbal: N=0 — quick return', function t() {
	...
	const B = new Complex128Array( 2 );
	const lscale = new Float64Array( 1 );
	const rscale = new Float64Array( 1 );
	const work = new Float64Array( 6 );
	const result = zggbal( 'both', 0, A, 1, 1, 0, B, 1, 1, 0, lscale, 1, 0, rscale, 1, 0, work, 1, 0 );
	assert.strictEqual( result.info, tc.info, 'info' );
	assert.strictEqual( result.ilo, tc.ilo, 'ilo' );
	assert.strictEqual( result.ihi, tc.ihi, 'ihi' );
});
...
test( 'zggbal: N=1 — quick return with scales=1', function t() {
	...
	const rscale = new Float64Array( n );
	const work = new Float64Array( 6 * n );
	const result = zggbal( 'both', n, A, 1, 1, 0, B, 1, 1, 0, lscale, 1, 0, rscale, 1, 0, work, 1, 0 );
	assert.strictEqual( result.info, tc.info, 'info' );
	assert.strictEqual( result.ilo, tc.ilo, 'ilo' );
	assert.strictEqual( result.ihi, tc.ihi, 'ihi' );
	assertArrayClose( toArray( lscale ), tc.lscale, 'lscale' );
...
test( 'zggbal: JOB=P, 4x4 — permute only', function t() {
	...
	const rscale = new Float64Array( n );
	const work = new Float64Array( 6 * n );
	const result = zggbal( 'permute', n, A, 1, n, 0, B, 1, n, 0, lscale, 1, 0, rscale, 1, 0, work, 1, 0 );
	assert.strictEqual( result.info, tc.info, 'info' );
	assert.strictEqual( result.ilo, tc.ilo, 'ilo' );
	assert.strictEqual( result.ihi, tc.ihi, 'ihi' );
	assertArrayClose( toArray( lscale ), tc.lscale, 'lscale' );
...
test( 'zggbal: JOB=S, 3x3 — scale only', function t() {
	...
	const rscale = new Float64Array( n );
	const work = new Float64Array( 6 * n );
	const result = zggbal( 'scale', n, A, 1, n, 0, B, 1, n, 0, lscale, 1, 0, rscale, 1, 0, work, 1, 0 );
	assert.strictEqual( result.info, tc.info, 'info' );
	assert.strictEqual( result.ilo, tc.ilo, 'ilo' );
	assert.strictEqual( result.ihi, tc.ihi, 'ihi' );
	assertArrayClose( toArray( lscale ), tc.lscale, 'lscale' );
...
test( 'zggbal: JOB=B, 4x4 — both permute and scale', function t() {
	...
	const rscale = new Float64Array( n );
	const work = new Float64Array( 6 * n );
	const result = zggbal( 'both', n, A, 1, n, 0, B, 1, n, 0, lscale, 1, 0, rscale, 1, 0, work, 1, 0 );
	assert.strictEqual( result.info, tc.info, 'info' );
	assert.strictEqual( result.ilo, tc.ilo, 'ilo' );
	assert.strictEqual( result.ihi, tc.ihi, 'ihi' );
	assertArrayClose( toArray( lscale ), tc.lscale, 'lscale' );
