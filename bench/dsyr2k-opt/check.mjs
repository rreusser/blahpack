...
// uplo/trans, layouts (for A, B, and C independently), sizes with remainders,
...
// comparison at a tolerance scaled to the reduction length. The FULL C storage
// is compared (both triangles plus padding), which also proves the opposite
// triangle is never touched, and A and B are bit-compared before/after to
// prove the inputs are never written.
	...
	for ( const K of dims ) {
		for ( const uplo of [ 'upper', 'lower' ] ) {
			for ( const trans of [ 'no-transpose', 'transpose' ] ) {
				// A and B dims: N×K (no-transpose) or K×N (transpose)
					...
					for ( const lb of combos( RA, SA ) ) {
						for ( const lc of combos( N, N ) ) {
							for ( const [ alpha, beta ] of alphabeta ) {
								...
								const B = rand( sizeOf( RA, SA, lb.s1, lb.s2, ob ) );
								const A0 = A.slice();
								const B0 = B.slice();
								...
								ncases += 1;
								const desc = `${uplo} ${trans} N=${N} K=${K} A:${la.tag} B:${lb.tag} C:${lc.tag} a=${alpha} b=${beta}`;
								if ( !bitEq( A, A0 ) || !bitEq( B, B0 ) ) {
									failures += 1;
									console.log( 'FAIL (input matrix written) ' + desc );
									continue;
								}
								...
								if ( bad >= 0 ) {
									failures += 1;
									console.log( `FAIL ${desc}: C[${bad}] v0=${C0[ bad ]} v1=${C1[ bad ]}` );
								}
							}
						}
					}
