...
const NB = 32; // Hardcoded block size matching base.js
...
function zunmrq( order, side, trans, M, N, K, A, LDA, TAU, strideTAU, C, LDC, WORK, strideWork ) {
	let need, sa1, sa2, sc1, sc2, nw, ow;
	...
	if ( WORK === null ) {
		...
		// contract requires `(ldwork*NB)+((NB+1)*NB)` complex elements (NB=32),
		// where `ldwork = max(1, N)` (left) or `max(1, M)` (right). K === 0 (or
		// M/N === 0) is a quick return needing no workspace.
		if ( M > 0 && N > 0 && K > 0 ) {
			nw = ( side === 'left' ) ? max( 1, N ) : max( 1, M );
			need = ( nw * NB ) + ( ( NB + 1 ) * NB );
