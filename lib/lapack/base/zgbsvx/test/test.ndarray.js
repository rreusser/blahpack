...
import { ndarray as zgbsvxNdarray } from './../lib/index.js';
...
function denseToGB( dense, N, kl, ku ) {
	let bandRow, di, bi, i, j;

	const nRows = kl + ku + 1;
	const AB = new Float64Array( (2 * nRows) * N );
	for ( j = 0; j < N; j++ ) {
...
const KL = 1;
const KU = 1;
const A_DENSE = new Float64Array([
	// Col 0
	4,
	1,
	1,
...
test( 'zgbsvx: fact=not-factored, trans=no-transpose', function t() {
	...
	const nrhs = 1;
	const nRows = KL + KU + 1;
	const AB = denseToGB( A_DENSE, N, KL, KU );
	...
	const w = makeWork( N );
	const bData = zmatvec( A_DENSE, [ 1, 0, 1, 0, 1, 0, 1, 0 ], N );
	const B = new Complex128Array( bData );
	const X = new Complex128Array( N * nrhs );
	const result = zgbsvx( 'not-factored', 'no-transpose', N, KL, KU, nrhs, AB, 1, nRows, 0, AFB, 1, (2 * KL) + KU + 1, 0, IPIV, 1, 0, 'none', r, 1, 0, c, 1, 0, B, 1, N, 0, X, 1, N, 0, FERR, 1, 0, BERR, 1, 0, w.WORK, 1, 0, w.RWORK, 1, 0 ); // eslint-disable-line max-len
...
test( 'zgbsvx: fact=not-factored, trans=conjugate-transpose', function t() {
	...
	const BERR = new Float64Array( nrhs );
	const w = makeWork( N );
	const bData = zmatvecH( A_DENSE, [ 1, 0, 1, 0, 1, 0, 1, 0 ], N );
	const B = new Complex128Array( bData );
	const X = new Complex128Array( N * nrhs );
	const result = zgbsvx( 'not-factored', 'conjugate-transpose', N, KL, KU, nrhs, AB, 1, nRows, 0, AFB, 1, (2 * KL) + KU + 1, 0, IPIV, 1, 0, 'none', r, 1, 0, c, 1, 0, B, 1, N, 0, X, 1, N, 0, FERR, 1, 0, BERR, 1, 0, w.WORK, 1, 0, w.RWORK, 1, 0 ); // eslint-disable-line max-len
	assert.equal( result.info, 0 );
	assert.equal( result.equed, 'none' );
	const Xv = reinterpret( X, 0 );
	assertArrayClose( toArray( Xv ), [ 1, 0, 1, 0, 1, 0, 1, 0 ], 1e-12, 'x' );
...
test( 'zgbsvx: fact=equilibrate', function t() {
	...
	const AB = denseToGB( dense, n, kl, ku );
	const AFB = new Complex128Array( ( (2 * kl) + ku + 1 ) * n );
	const IPIV = new Int32Array( n );
	const r = new Float64Array( n );
	const c = new Float64Array( n );
	...
	const BERR = new Float64Array( nrhs );
	const w = makeWork( n );
	const B = new Complex128Array( [ 1e6 + 1, 0, 1.001 + 1, 0, 1 + 1e3, 0 ] );
	const X = new Complex128Array( n * nrhs );
	const result = zgbsvx( 'equilibrate', 'no-transpose', n, kl, ku, nrhs, AB, 1, nRows, 0, AFB, 1, (2 * kl) + ku + 1, 0, IPIV, 1, 0, 'none', r, 1, 0, c, 1, 0, B, 1, n, 0, X, 1, n, 0, FERR, 1, 0, BERR, 1, 0, w.WORK, 1, 0, w.RWORK, 1, 0 ); // eslint-disable-line max-len
	assert.equal( result.info, 0 );
	const Xv = reinterpret( X, 0 );
...
test( 'zgbsvx: fact=factored (reuse factorization)', function t() {
	...
	B = new Complex128Array( bData );
	X = new Complex128Array( N * nrhs );
	const result = zgbsvx( 'factored', 'no-transpose', N, KL, KU, nrhs, AB, 1, nRows, 0, AFB, 1, (2 * KL) + KU + 1, 0, IPIV, 1, 0, 'none', r, 1, 0, c, 1, 0, B, 1, N, 0, X, 1, N, 0, FERR, 1, 0, BERR, 1, 0, w.WORK, 1, 0, w.RWORK, 1, 0 ); // eslint-disable-line max-len
	const Xv = reinterpret( X, 0 );
...
test( 'zgbsvx: N=0 quick return', function t() {
	...
	const result = zgbsvx( 'not-factored', 'no-transpose', 0, 0, 0, 1, AB, 1, 1, 0, AFB, 1, 1, 0, IPIV, 1, 0, 'none', r, 1, 0, c, 1, 0, B, 1, 1, 0, X, 1, 1, 0, FERR, 1, 0, BERR, 1, 0, WORK, 1, 0, RWORK, 1, 0 ); // eslint-disable-line max-len
	assert.equal( result.info, 0 );
...
test( 'zgbsvx: multiple right-hand sides', function t() {
	...
	const BERR = new Float64Array( nrhs );
	const w = makeWork( N );
	const b1 = zmatvec( A_DENSE, [ 1, 0, 1, 0, 1, 0, 1, 0 ], N );
	const b2 = zmatvec( A_DENSE, [ 2, 1, -1, 0.5, 0.5, -0.5, 1, 1 ], N );
...
test( 'zgbsvx: fact=equilibrate, trans=conjugate-transpose', function t() {
	...
	const B = new Complex128Array( [ 1e6 + 1, 0, 1.001 + 1, 0, 1 + 1e3, 0 ] );
	const X = new Complex128Array( n * nrhs );
	const result = zgbsvx( 'equilibrate', 'conjugate-transpose', n, kl, ku, nrhs, AB, 1, nRows, 0, AFB, 1, (2 * kl) + ku + 1, 0, IPIV, 1, 0, 'none', r, 1, 0, c, 1, 0, B, 1, n, 0, X, 1, n, 0, FERR, 1, 0, BERR, 1, 0, w.WORK, 1, 0, w.RWORK, 1, 0 ); // eslint-disable-line max-len
	assert.equal( result.info, 0 );
	assert.ok( result.rcond > 0, 'rcond > 0' );
});
