...
function computeWorkSize( M, N, nrhs ) {
	...
	const mnthr = Math.round( 1.6 * minmn );
	mm = M;
	wsize = 1;
...
* @throws {RangeError} eighth argument must be greater than or equal to `max(1,nrhs)` (row-major) or `max(1,M,N)` (column-major)
* @returns {integer} info - 0 if successful, >0 if DBDSQR did not converge
*/
function dgelss( order, M, N, nrhs, A, LDA, B, LDB, S, strideS, rcond, rank, WORK, strideWork ) {
	let sa1, sa2, sb1, sb2, ow, sw, W;
	...
	if ( order === 'row-major' && LDB < max( 1, nrhs ) ) {
		throw new RangeError( format( 'invalid argument. Eighth argument must be greater than or equal to max(1,nrhs). Value: `%d`.', LDB ) );
	}
	if ( order === 'column-major' && LDB < max( 1, max( M, N ) ) ) {
		throw new RangeError( format( 'invalid argument. Eighth argument must be greater than or equal to max(1,M,N). Value: `%d`.', LDB ) );
	}
	...
	if ( WORK === null || WORK === void 0 ) {
		W = new Float64Array( computeWorkSize( M, N, nrhs ) );
		sw = 1;
		ow = 0;
	} else {
		W = WORK;
	...
	return base( M, N, nrhs, A, sa1, sa2, 0, B, sb1, sb2, 0, S, strideS, os, rcond, rank, W, sw, ow );
}
