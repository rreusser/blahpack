...
*/
function dormql( side, trans, M, N, K, A, strideA1, strideA2, offsetA, TAU, strideTAU, offsetTAU, C, strideC1, strideC2, offsetC, WORK, strideWork, offsetWork ) { // eslint-disable-line max-len, max-params
	...
	// from an out-of-bounds read. With `nw = max(1,N)` (side='left') or
	...
	// reflector T in a trailing segment and needs `nw*NB + (NB+1)*NB` (the JS
	// hardcodes NB and does NOT adapt the block size down when WORK is small, so
	// the reference unblocked minimum is NOT a safe minimum here — mirror zunmql).
	let minWork = ( side === 'left' ) ? Math.max( 1, N ) : Math.max( 1, M );
	if ( K > 32 ) {
		minWork = ( minWork * 32 ) + ( 33 * 32 );
	}
