...
function dlaein( rightv, noinit, N, H, strideH1, strideH2, offsetH, wr, wi, VR, strideVR, offsetVR, VI, strideVI, offsetVI, B, strideB1, strideB2, offsetB, WORK, strideWork, offsetWork, eps3, smlnum, bignum ) {
	let absbii, absbjj, normin, scaleA, vnorm, trans, vcrit, info, norm, temp;
	let vmax, its, rec, i1, i2, i3, xr, xi, ej, ei, w1, x, y, w, i, j;

	info = 0;

	// DLATRS scale is returned via length-1 array:
	const scale = new Float64Array( 1 );
	const dladivOut = new Float64Array( 2 );
	...
	const sVR = strideVR;
	const sVI = strideVI;
	const sWK = strideWork;
	const oH = offsetH;
	const oB = offsetB;
	const oVR = offsetVR;
	const oVI = offsetVI;
	const oWK = offsetWork;
