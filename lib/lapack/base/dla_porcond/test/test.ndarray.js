...
import dla_porcond from './../lib/ndarray.js';


// FIXTURES //

const fixtureDir = path.join( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures' );
const lines = readFileSync( path.join( fixtureDir, 'dla_porcond.jsonl' ), 'utf8' ).trim().split( '\n' ); // eslint-disable-line max-len, node/no-sync
...
const C3 = new Float64Array([ 2.0, 1.0, 0.5 ]);
...
function factorA( A, N, uplo ) {
	const AF = new Float64Array( A.length );
	let i;
...
test( 'UPLO=upper, CMODE=1', function t() {
	...
	const tc = findCase( 'uplo_U_cmode_1' );
	const AF = factorA( A3, 3, 'upper' );
	const WORK = new Float64Array( 9 );
	const IWORK = new Int32Array( 3 );

	const result = dla_porcond( 'upper', 3, A3, 1, 3, 0, AF, 1, 3, 0, 1, C3, 1, 0, WORK, 1, 0, IWORK, 1, 0 ); // eslint-disable-line max-len
	assertClose( result, tc.result, 1e-14, 'result' );
});

test( 'UPLO=lower, CMODE=1', function t() {

	const tc = findCase( 'uplo_L_cmode_1' );
	const AF = factorA( A3, 3, 'lower' );
	const WORK = new Float64Array( 9 );
	const IWORK = new Int32Array( 3 );

	const result = dla_porcond( 'lower', 3, A3, 1, 3, 0, AF, 1, 3, 0, 1, C3, 1, 0, WORK, 1, 0, IWORK, 1, 0 ); // eslint-disable-line max-len
	assertClose( result, tc.result, 1e-14, 'result' );
});
...
test( 'UPLO=upper, CMODE=0', function t() {
	...
	const tc = findCase( 'uplo_U_cmode_0' );
	const AF = factorA( A3, 3, 'upper' );
	const WORK = new Float64Array( 9 );
	const IWORK = new Int32Array( 3 );

	const result = dla_porcond( 'upper', 3, A3, 1, 3, 0, AF, 1, 3, 0, 0, C3, 1, 0, WORK, 1, 0, IWORK, 1, 0 ); // eslint-disable-line max-len
...
test( 'UPLO=lower, CMODE=0', function t() {
	...
	const tc = findCase( 'uplo_L_cmode_0' );
	const AF = factorA( A3, 3, 'lower' );
	const WORK = new Float64Array( 9 );
	const IWORK = new Int32Array( 3 );

	const result = dla_porcond( 'lower', 3, A3, 1, 3, 0, AF, 1, 3, 0, 0, C3, 1, 0, WORK, 1, 0, IWORK, 1, 0 ); // eslint-disable-line max-len
	assertClose( result, tc.result, 0.15, 'result' );
});
...
test( 'N=1 edge case', function t() {
	...
	const AF = new Float64Array([ 3.0 ]); // sqrt(9)
	const C = new Float64Array([ 2.0 ]);
	const WORK = new Float64Array( 3 );
	const IWORK = new Int32Array( 1 );

	const result = dla_porcond( 'upper', 1, A, 1, 1, 0, AF, 1, 1, 0, 1, C, 1, 0, WORK, 1, 0, IWORK, 1, 0 ); // eslint-disable-line max-len
	assertClose( result, tc.result, 1e-14, 'result' );
});
...
test( 'N=0 edge case', function t() {
	...
	const AF = new Float64Array( 0 );
	const C = new Float64Array( 0 );
	const WORK = new Float64Array( 0 );
	const IWORK = new Int32Array( 0 );

	const result = dla_porcond( 'upper', 0, A, 1, 1, 0, AF, 1, 1, 0, 1, C, 1, 0, WORK, 1, 0, IWORK, 1, 0 ); // eslint-disable-line max-len
