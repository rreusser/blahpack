...
* @param {(Float64Array|null)} WORK - caller-provided workspace (length `>= N*(NB+1)` with `NB = 32` when `N > 1`); `null` requests internal allocation
...
* @returns {integer} `0`
*/
function dsytrfAa( order, uplo, N, A, LDA, IPIV, WORK, strideWork ) {
	let sa1, sa2;
	...
	if ( WORK === null || WORK === void 0 ) {
		WORK = new Float64Array( max( 1, N * ( NB + 1 ) ) );
		strideWork = 1;
	}
	const ow = stride2offset( max( 1, N * ( NB + 1 ) ), strideWork );
