...
*/
function makeWork( N ) {
	var lwork = Math.max( 1, ( 2 * N ) + ( ( N + 1 ) * NB ) );
...
test( 'dgeqp3: 4x3 matrix (tall)', function t() {
	...
	WORK = makeWork( 3 );
	info = dgeqp3( 4, 3, A, 1, LDA, 0, JPVT, 1, 0, TAU, 1, 0, WORK, 1, 0 );
...
test( 'dgeqp3: 3x4 matrix (wide)', function t() {
	...
	TAU = new Float64Array( 3 );
	WORK = makeWork( 4 );
	info = dgeqp3( 3, 4, A, 1, LDA, 0, JPVT, 1, 0, TAU, 1, 0, WORK, 1, 0 );
	assert.equal( info, tc.info, 'info' );
...
test( 'dgeqp3: square 4x4', function t() {
	...
	WORK = makeWork( 4 );
	info = dgeqp3( 4, 4, A, 1, LDA, 0, JPVT, 1, 0, TAU, 1, 0, WORK, 1, 0 );
	assert.equal( info, tc.info, 'info' );
...
test( 'dgeqp3: N=0 (quick return)', function t() {
	...
	WORK = makeWork( 0 );
	info = dgeqp3( 3, 0, A, 1, LDA, 0, JPVT, 1, 0, TAU, 1, 0, WORK, 1, 0 );
...
test( 'dgeqp3: M=0 (quick return)', function t() {
	...
	TAU = new Float64Array( 1 );
	WORK = makeWork( 3 );
	info = dgeqp3( 0, 3, A, 1, 1, 0, JPVT, 1, 0, TAU, 1, 0, WORK, 1, 0 );
	assert.equal( info, 0, 'info' );
});
...
test( 'dgeqp3: 1x1 matrix', function t() {
	...
	TAU = new Float64Array( 1 );
	WORK = makeWork( 1 );
	info = dgeqp3( 1, 1, A, 1, LDA, 0, JPVT, 1, 0, TAU, 1, 0, WORK, 1, 0 );
	assert.equal( info, tc.info, 'info' );
...
test( 'dgeqp3: fixed column 1', function t() {
	...
	TAU = new Float64Array( 3 );
	WORK = makeWork( 3 );
	info = dgeqp3( 3, 3, A, 1, LDA, 0, JPVT, 1, 0, TAU, 1, 0, WORK, 1, 0 );
	assert.equal( info, tc.info, 'info' );
...
test( 'dgeqp3: wide 8x36 (unblocked, sminmn < NB)', function t() {
	...
	WORK = makeWork( N );
	info = dgeqp3( M, N, A, 1, BIGMN, 0, JPVT, 1, 0, TAU, 1, 0, WORK, 1, 0 );
	assert.equal( info, tc.info, 'info' );
	aOut = [];
	for ( j = 0; j < N; j++ ) {
		for ( i = 0; i < M; i++ ) {
