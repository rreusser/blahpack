...
import upper3x3_1rhs from './fixtures/upper_3x3_1rhs.json' with { type: 'json' };
import lower3x3_2rhs from './fixtures/lower_3x3_2rhs.json' with { type: 'json' };
import n1Fixture from './fixtures/n1.json' with { type: 'json' };
import upper4x4Pivot from './fixtures/upper_4x4_pivot.json' with { type: 'json' };
import lower4x4Pivot from './fixtures/lower_4x4_pivot.json' with { type: 'json' };
...
function packB( flatDoubles, N, nrhs, LDB ) {
	let idx, j, i;
	const out = new Complex128Array( N * nrhs );
...
function buildAP( flatDoubles, nn ) {
	const out = new Complex128Array( nn );
	const ov = reinterpret( out, 0 );
	let i;
	for ( i = 0; i < 2 * nn; i++ ) {
		ov[ i ] = flatDoubles[ i ];
...
function runFixtureTest( name, uplo ) {
	...
	const LDB = 6; // NMAX in Fortran test
	...
	const AP = buildAP( tc.AP_factored, nn );
	const IPIV = convertIPIV( tc.ipiv, N );

	// Pack B (LDB -> N)
	const B = packB( tc.b, N, nrhs, LDB );

	const info = zhptrs( uplo, N, nrhs, AP, 1, 0, IPIV, 1, 0, B, 1, N, 0 );
	assert.strictEqual( info, 0 );

	// Extract x values from fixture (same LDB packing)
	const xExpected = packB( tc.x, N, nrhs, LDB );
	const xExpectedV = reinterpret( xExpected, 0 );
...
test( 'zhptrs: N=1', function t() {
	...
	Bv[ 0 ] = tc.b[ 0 ];
	Bv[ 1 ] = tc.b[ 1 ];
	const IPIV = convertIPIV( tc.ipiv, 1 );
	const info = zhptrs( 'upper', 1, 1, AP, 1, 0, IPIV, 1, 0, B, 1, 1, 0 );
	assert.strictEqual( info, 0 );
	assertArrayClose( toArray( Bv ), tc.x, 1e-14, 'n1' );
});

test( 'zhptrs: N=0', function t() {
	const info = zhptrs( 'upper', 0, 1, new Complex128Array( 1 ), 1, 0, new Int32Array( 1 ), 1, 0, new Complex128Array( 1 ), 1, 1, 0 ); // eslint-disable-line max-len
	assert.strictEqual( info, 0 );
});

test( 'zhptrs: nrhs=0 returns 0', function t() {
	const info = zhptrs( 'upper', 3, 0, new Complex128Array( 6 ), 1, 0, new Int32Array( 3 ), 1, 0, new Complex128Array( 3 ), 1, 3, 0 ); // eslint-disable-line max-len
	assert.strictEqual( info, 0 );
});
