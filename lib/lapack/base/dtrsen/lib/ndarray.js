...
* @param {Float64Array} WORK - workspace (length `>= max(1, N)` for `job='none'`, `>= max(1, M*(N-M))` for `job='eigenvalues'`, `>= max(1, 2*M*(N-M))` for `job='subspace'` or `job='both'`)
* @param {integer} strideWork - stride for WORK
* @param {NonNegativeInteger} offsetWork - starting index for WORK
* @param {Int32Array} IWORK - integer workspace (length `>= 1` for `job='none'` or `job='eigenvalues'`, `>= max(1, M*(N-M))` for `job='subspace'` or `job='both'`)
...
*/
function dtrsen( job, compq, SELECT, strideSELECT, offsetSELECT, N, T, strideT1, strideT2, offsetT, Q, strideQ1, strideQ2, offsetQ, WR, strideWR, offsetWR, WI, strideWI, offsetWI, M, s, sep, WORK, strideWork, offsetWork, IWORK, strideIWork, offsetIWork ) { // eslint-disable-line max-len, max-params
	...
	// Caller owns the workspace. The exact requirement depends on the selected
	// cluster size `M` (computed inside `base` from `SELECT`), so assert only
	// the path-independent lower bound: `job='none'` always needs `max(1,N)`;
	// the condition-number paths need `>= 1` (their `M*(N-M)` requirement is
	// enforced once `M` is known).
	const need = ( job === 'none' ) ? Math.max( 1, N ) : 1;
	...
	return base( job, compq, SELECT, strideSELECT, offsetSELECT, N, T, strideT1, strideT2, offsetT, Q, strideQ1, strideQ2, offsetQ, WR, strideWR, offsetWR, WI, strideWI, offsetWI, M, s, sep, WORK, strideWork, offsetWork, IWORK, strideIWork, offsetIWork ); // eslint-disable-line max-len
}
