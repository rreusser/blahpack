...
function dstevx( jobz, range, N, d, strideD, offsetD, e, strideE, offsetE, vl, vu, il, iu, abstol, M, w, strideW, offsetW, Z, strideZ1, strideZ2, offsetZ, WORK, strideWork, offsetWork, IWORK, strideIWork, offsetIWork, IFAIL, strideIFAIL, offsetIFAIL ) { // eslint-disable-line max-len, max-params
	...
	// Zero-dim quick return: an empty matrix requires no workspace, so this
	// MUST precede the WORK/IWORK size assertions below.
	if ( N === 0 ) {
	...
	// ndarray.js never allocates; assert the caller-owned WORK (>= 5*N) and
	// IWORK (>= 5*N) are large enough so an undersized buffer is a loud
	// RangeError, not a silent NaN from an out-of-bounds read.
	const minWork = Math.max( 1, 5 * N );
