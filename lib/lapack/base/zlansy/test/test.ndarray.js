...
import zlansy from './../lib/ndarray.js';


// FIXTURES //

const fixtureFile = path.join( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures', 'zlansy.jsonl' );
...
function buildA( lda, ncols, cells ) {
	const buf = new Float64Array( 2 * lda * ncols );
	let i, idx;
...
const A_UPPER = buildA( NMAX, NMAX, [
	1, 1, 2.0, 1.0,
...
const A_LOWER = buildA( NMAX, NMAX, [
	1, 1, 2.0, 1.0,
...
test( 'zlansy: max_upper', function t() {
	...
	const tc = findCase( 'max_upper' );
	const got = zlansy( 'max', 'upper', 4, A_UPPER, 1, NMAX, 0, WORK, 1, 0 );
	assertClose( got, tc.val, 1e-12, 'zlansy: max_upper' );
});

test( 'zlansy: one_upper', function t() {
	const WORK = new Float64Array( 4 );
	const tc = findCase( 'one_upper' );
	const got = zlansy( 'one-norm', 'upper', 4, A_UPPER, 1, NMAX, 0, WORK, 1, 0 );
	assertClose( got, tc.val, 1e-12, 'zlansy: one_upper' );
});

test( 'zlansy: inf_upper', function t() {
	const WORK = new Float64Array( 4 );
	const tc = findCase( 'inf_upper' );
	const got = zlansy( 'inf-norm', 'upper', 4, A_UPPER, 1, NMAX, 0, WORK, 1, 0 );
	assertClose( got, tc.val, 1e-12, 'zlansy: inf_upper' );
});

test( 'zlansy: fro_upper', function t() {
	const WORK = new Float64Array( 4 );
	const tc = findCase( 'fro_upper' );
	const got = zlansy( 'frobenius', 'upper', 4, A_UPPER, 1, NMAX, 0, WORK, 1, 0 );
	assertClose( got, tc.val, 1e-12, 'zlansy: fro_upper' );
});

test( 'zlansy: one_lower', function t() {
	const WORK = new Float64Array( 4 );
	const tc = findCase( 'one_lower' );
	const got = zlansy( 'one-norm', 'lower', 4, A_LOWER, 1, NMAX, 0, WORK, 1, 0 );
	assertClose( got, tc.val, 1e-12, 'zlansy: one_lower' );
});

test( 'zlansy: fro_lower', function t() {
	const WORK = new Float64Array( 4 );
	const tc = findCase( 'fro_lower' );
	const got = zlansy( 'frobenius', 'lower', 4, A_LOWER, 1, NMAX, 0, WORK, 1, 0 );
...
test( 'zlansy: n0', function t() {
	...
	const got = zlansy( 'max', 'upper', 0, A_UPPER, 1, NMAX, 0, WORK, 1, 0 );
...
test( 'zlansy: max_lower (computed)', function t() {
	...
	const got = zlansy( 'max', 'lower', 4, A_LOWER, 1, NMAX, 0, WORK, 1, 0 );
	const tc = findCase( 'max_upper' );
...
test( 'zlansy: inf_lower (computed)', function t() {
	const WORK = new Float64Array( 4 );
	const got = zlansy( 'inf-norm', 'lower', 4, A_LOWER, 1, NMAX, 0, WORK, 1, 0 );
	const tc = findCase( 'inf_upper' );
...
test( 'zlansy: invalid norm returns 0 from base', function t() {
	...
	const got = zlansy( 'invalid', 'upper', 4, A_UPPER, 1, NMAX, 0, WORK, 1, 0 );
