...
import dgeqr2p from './../lib/ndarray.js';


// FIXTURES //

const fixtureDir = path.join( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures' );
const lines = readFileSync( path.join( fixtureDir, 'dgeqr2p.jsonl' ), 'utf8' ).trim().split( '\n' );
...
test( 'dgeqr2p: 3x2', function t() {
	...
	const tc = findCase( '3x2' );
	const A = new Float64Array( [ 1, 3, 5, 2, 4, 6 ] );
	const TAU = new Float64Array( 2 );
	const WORK = new Float64Array( 2 );
	const info = dgeqr2p( 3, 2, A, 1, 3, 0, TAU, 1, 0, WORK, 1, 0 );
...
test( 'dgeqr2p: 2x2', function t() {
	...
	const tc = findCase( '2x2' );
	const A = new Float64Array( [ 4, 3, 1, 2 ] );
	const TAU = new Float64Array( 2 );
	const WORK = new Float64Array( 2 );
	const info = dgeqr2p( 2, 2, A, 1, 2, 0, TAU, 1, 0, WORK, 1, 0 );
	assert.strictEqual( info, tc.INFO );
	assertArrayClose( A, tc.A, 1e-13, 'A' );
	assertArrayClose( TAU, tc.TAU, 1e-13, 'TAU' );
	assert.ok( A[ 0 ] >= 0, 'R[0,0] non-negative' );
...
test( 'dgeqr2p: n_zero', function t() {
	...
	const TAU = new Float64Array( 2 );
	const WORK = new Float64Array( 2 );
	const info = dgeqr2p( 2, 0, A, 1, 2, 0, TAU, 1, 0, WORK, 1, 0 );
	assert.strictEqual( info, tc.INFO );
});
...
test( 'dgeqr2p: m_zero', function t() {
	...
	const TAU = new Float64Array( 2 );
	const WORK = new Float64Array( 2 );
	const info = dgeqr2p( 0, 2, A, 1, 1, 0, TAU, 1, 0, WORK, 1, 0 );
	assert.strictEqual( info, tc.INFO );
});
...
test( 'dgeqr2p: 4x3', function t() {
	...
	const info = dgeqr2p( 4, 3, A, 1, 4, 0, TAU, 1, 0, WORK, 1, 0 );
	assert.strictEqual( info, tc.INFO );
	assertArrayClose( A, tc.A, 1e-13, 'A' );
	assertArrayClose( TAU, tc.TAU, 1e-13, 'TAU' );
	assert.ok( A[ 0 ] >= 0, 'R[0,0] non-negative' );
...
test( 'dgeqr2p: 3x4', function t() {
	...
	const A = new Float64Array( [ 1, 5, 9, 2, 6, 1, 3, 7, 2, 4, 8, 3 ] );
	const TAU = new Float64Array( 3 );
	const WORK = new Float64Array( 4 );
	const info = dgeqr2p( 3, 4, A, 1, 3, 0, TAU, 1, 0, WORK, 1, 0 );
	assert.strictEqual( info, tc.INFO );
	assertArrayClose( A, tc.A, 1e-13, 'A' );
	assertArrayClose( TAU, tc.TAU, 1e-13, 'TAU' );
	assert.ok( A[ 0 ] >= 0, 'R[0,0] non-negative' );
