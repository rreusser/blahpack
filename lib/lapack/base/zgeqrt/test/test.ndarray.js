...
const fixtureRaw = readFileSync( path.join( fixtureDir, 'zgeqrt.jsonl' ), 'utf8' ); // eslint-disable-line node/no-sync
...
function colmajor( rows, M, N ) {
	let i, j;
	const out = new Complex128Array( M * N );
...
function runCase( caseName, M, N, nb, rows, tol ) {
	...
	const info = zgeqrt( M, N, nb, A, 1, M, 0, T, 1, nb, 0, WORK, 1, 0 );
...
test( 'zgeqrt: m=0 quick return', function t() {
	...
	const info = zgeqrt( 0, 3, 1, A, 1, 1, 0, T, 1, 1, 0, WORK, 1, 0 );
...
test( 'zgeqrt: n=0 quick return', function t() {
	...
	const T = new Complex128Array( 0 );
	const WORK = new Complex128Array( 0 );
	const info = zgeqrt( 3, 0, 1, A, 1, 3, 0, T, 1, 1, 0, WORK, 1, 0 );
	assert.strictEqual( info, 0 );
});
...
test( 'zgeqrt: works with non-trivial offsets and strides', function t() {
	...
	const offsetT = 3;
	const offsetWork = 2;
	const A = new Complex128Array( ( LDA * N ) + offsetA );
	const T = new Complex128Array( ( LDT * K ) + offsetT );
	const WORK = new Complex128Array( ( nb * N ) + offsetWork );
	const Av = reinterpret( A, 0 );
	const Tv = reinterpret( T, 0 );
	const rows = [
		[ [ 2.0, 0.5 ], [ 1.0, -0.2 ], [ 3.0, 0.1 ] ],
	...
	const info = zgeqrt( M, N, nb, A, 1, LDA, offsetA, T, 1, LDT, offsetT, WORK, 1, offsetWork ); // eslint-disable-line max-len
	assert.strictEqual( info, 0 );

	// Pull out the M-by-N region of A.
	const Aout = new Complex128Array( M * N );
	const Aoutv = reinterpret( Aout, 0 );
	for ( j = 0; j < N; j++ ) {
		for ( i = 0; i < M; i++ ) {
	...
	const Tout = new Complex128Array( nb * K );
	const Toutv = reinterpret( Tout, 0 );
	for ( j = 0; j < K; j++ ) {
		for ( i = 0; i < nb; i++ ) {
...
test( 'zgeqrt: works on a row-major matrix layout', function t() {
	...
	const info = zgeqrt( M, N, nb, A, N, 1, 0, T, K, 1, 0, WORK, 1, 0 );
	assert.strictEqual( info, 0 );

	// Convert column-major fixture A/T to row-major for comparison.
	const Aexp = new Complex128Array( M * N );
	const Aexpv = reinterpret( Aexp, 0 );
	for ( j = 0; j < N; j++ ) {
		for ( i = 0; i < M; i++ ) {
	...
	const Texp = new Complex128Array( nb * K );
	const Texpv = reinterpret( Texp, 0 );
	for ( j = 0; j < K; j++ ) {
		for ( i = 0; i < nb; i++ ) {
...
test( 'ndarray: nb=min(M,N) accepted at boundary', function t() {
	...
	const WORK = new Complex128Array( 9 );
	const info = zgeqrt( 4, 3, 3, A, 1, 4, 0, T, 1, 3, 0, WORK, 1, 0 );
	assert.strictEqual( info, 0 );
});
