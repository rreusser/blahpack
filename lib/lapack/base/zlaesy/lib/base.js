...
function csqrt( re, im, out ) {
	...
	const t = Math.sqrt( ( Math.abs( re ) + r ) / 2.0 );
	if ( re >= 0.0 ) {
		out[ 0 ] = t;
		out[ 1 ] = im / ( 2.0 * t );
	} else {
...
function zlaesy( a, b, c ) {
	let invTabs, evscalr, evscali, evnorm, sn1sqr, sn1sqi, sn1sr, sn1si, cs1r;
	let cs1i, sn1r, sn1i, rt1r, rt1i, rt2r, rt2i, babs, tabs, tmpr, tmpi, sumr;
	let sumi, ss2r, ss2i, tz2r, tz2i, bz2r, bz2i, tzr, tzi, bzr, bzi, sr, si;
	let tr, ti, z;
