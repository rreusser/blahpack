...
import dsytrsRook from './../lib/ndarray.js';


// FIXTURES //

const fixtureDir = path.join( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures' ); // eslint-disable-line max-len
const lines = readFileSync( path.join( fixtureDir, 'dsytrs_rook.jsonl' ), 'utf8' ).trim().split( '\n' ); // eslint-disable-line node/no-sync, max-len
...
function runCase( uplo, N, nrhs, tc, bIn, tol ) {
	...
	const info = dsytrsRook( uplo, N, nrhs, A, 1, N, 0, ipiv, 1, 0, b, 1, N, 0 );
...
test( 'dsytrs_rook: 5x5_upper_2rhs', function t() {
	const tc = findCase( '5x5_upper_2rhs' );
...
test( 'dsytrs_rook: n_zero', function t() {
	...
	const info = dsytrsRook( 'lower', 0, 1, A, 1, 1, 0, ipiv, 1, 0, b, 1, 1, 0 );
	assert.equal( info, tc.info, 'info' );
});

test( 'dsytrs_rook: nrhs_zero', function t() {
	const tc = findCase( 'nrhs_zero' );
	const A = new Float64Array( 9 );
	const b = new Float64Array( 3 );
	const ipiv = new Int32Array( 3 );
	const info = dsytrsRook( 'lower', 3, 0, A, 1, 3, 0, ipiv, 1, 0, b, 1, 3, 0 );
	assert.equal( info, tc.info, 'info' );
});
...
test( 'dsytrs_rook: n_one_upper hand-computed', function t() {
	...
	const info = dsytrsRook( 'upper', 1, 1, A, 1, 1, 0, ipiv, 1, 0, b, 1, 1, 0 );
...
test( 'dsytrs_rook: ndarray with non-zero offset', function t() {
	...
	const Apad = new Float64Array( 20 );
	const bpad = new Float64Array( 6 );
	const raw = convertIpiv( tc.ipiv );
	for ( i = 0; i < 4; i++ ) {
		ipiv[ i + 2 ] = raw[ i ];
	}
	for ( i = 0; i < 16; i++ ) {
	...
	bpad[ 4 ] = 12.0;
	bpad[ 5 ] = 14.0;
	const info = dsytrsRook( 'lower', 4, 1, Apad, 1, 4, 4, ipiv, 1, 2, bpad, 1, 4, 2 );
	assert.equal( info, tc.info, 'info' );
