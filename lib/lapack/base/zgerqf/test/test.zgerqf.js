...
test( 'zgerqf throws TypeError for invalid order', function t() {
	assert.throws( function throws() {
		zgerqf( 'invalid', 2, 2, new Float64Array( 4 ), 2, new Float64Array( 4 ), 2, new Float64Array( 4 ), 2 );
	}, TypeError );
});

test( 'zgerqf throws RangeError for negative M', function t() {
	assert.throws( function throws() {
		zgerqf( 'row-major', -1, 2, new Float64Array( 4 ), 2, new Float64Array( 4 ), 2, new Float64Array( 4 ), 2 );
	}, RangeError );
});

test( 'zgerqf throws RangeError for negative N', function t() {
	assert.throws( function throws() {
		zgerqf( 'row-major', 2, -1, new Float64Array( 4 ), 2, new Float64Array( 4 ), 2, new Float64Array( 4 ), 2 );
	}, RangeError );
});
