/**
* Property-based validation for dgehd2, following the /blahpack-validate process.
*
...
* (unitarity) QᴴQ = I.
*
* dgehd2 reduces a general N×N matrix A to upper Hessenberg form H by an orthogonal
* similarity transformation Qᵀ·A·Q = H, i.e. A = Q·H·Qᵀ. On exit the upper
...
import dgehd2 from './../lib/ndarray.js';

const sc = S.real; // d-routine
const LogicalMatrix = logical.LogicalMatrix;

const SIZES = SIZES_SMALL; // 1,2,3,5,8,16,17,33,64
...
test( 'dgehd2: A = Q*H*Qᴴ and QᴴQ = I (N sweep x all layouts)', function t() {
	SIZES.forEach( function eachN( N ) {
		...
		ALL_LAYOUTS.forEach( function eachLayout( layout, li ) {
			...
			dgehd2( N, 1, N, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], Tr.data, Tr.args[ 0 ], Tr.args[ 1 ], Wr.data, Wr.args[ 0 ], Wr.args[ 1 ] );

			const taus = readTaus( Tr, k );
			const label = 'dgehd2 N=' + N + ' layout=' + li;

			const Hm = readH( Ar, N );
			const Q = formQ( Ar, taus, N );

			checked( 'dgehd2', 'orthonormal', function run() {
				check.assertOrthonormal( sc, Q, { 'label': label + ' Q' } );
			} );

			// A0 = Q·H·Qᴴ
			const recon = ref.matmul( sc, ref.matmul( sc, Q, Hm ), Q, { 'transb': 'c' } );
			checked( 'dgehd2', 'reconstruct', function run() {
...
test( 'dgehd2: bit-exact within storage-order family (col / row)', function t() {
...
function runInvariance( variants, fam ) {
	...
	const k = N - 1;
	const SEED = 0xF00D;
	checked( 'dgehd2', 'layout-invariance', function run() {
		layoutInvariant( variants, function build( layout, i ) {
			...
			return flat;
		}, { 'label': 'dgehd2 layout invariance ' + fam + '-major (N=' + N + ')' } );
	} );
}
