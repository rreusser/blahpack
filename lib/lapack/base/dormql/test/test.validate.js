...
/**
* Property-based validation for dormql, following the /blahpack-validate process.
*
...
* `ref.matmul` and compare to dormql's in-place output. Because dorgql forms Q by
* an independent code path, a systematic reflector/side/trans error in dormql
...
*
* dormql is BLOCKED (dlarft + dlarfb) whenever K > NB, falling back to the
* unblocked dorm2l for K <= NB. Its inner kernel is the optimized dgemm/dtrsm,
...
* path with a poisoned buffer (this is where the real dormql WORK under-advertise
* bug lived; see test/harness/LEARNINGS.md).
*/
...
import dormql from './../lib/ndarray.js';
...
// Drive one property case: factor an R×K panel, apply op(Q) to C0 with dormql,
...
function runCase( M, N, K, side, trans, tcode, layout ) {
	...
	dormql( side, trans, M, N, K, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], Tr.data, Tr.args[ 0 ], Tr.args[ 1 ], Cr.data, Cr.args[ 0 ], Cr.args[ 1 ], Cr.args[ 2 ], poisonedWork( sc, ormWork( side, M, N ) ), 1, 0 );
...
// Step 2/5: PROPERTY. op(Q) applied by dormql == explicit-Q oracle, swept over
// side × trans × (M,N,K) × dense layouts (all layouts for small/medium dims,
// tight only for large).
test( 'dormql: op(Q)·C matches explicit-Q oracle (side × trans × dims × layouts)', function t() {
...
// and C per layout and run ONLY dormql; assert bit-exact output WITHIN a
...
test( 'dormql: bit-exact within storage-order family (col / row)', function t() {
	var M = 48;
	var N = 40;
			...
			[ [ colLayouts, 'col' ], [ rowLayouts, 'row' ] ].forEach( function eachFam( fam ) {
				checked( NAME, 'layout-invariance', function run() {
					layoutInvariant( fam[ 0 ], function build( layout, i ) {
						...
						dormql( side, tc[ 0 ], M, N, K, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], Tr.data, Tr.args[ 0 ], Tr.args[ 1 ], Cr.data, Cr.args[ 0 ], Cr.args[ 1 ], Cr.args[ 2 ], poisonedWork( sc, ormWork( side, M, N ) ), 1, 0 );
...
// far more than the unblocked nw. The wrapper's original guard advertised only
// the unblocked `max(1,N)`/`max(1,M)` (the real dormql WORK under-advertise bug;
// see LEARNINGS.md). Derive the advertised minimum from the wrapper's own throw
...
test( 'dormql: advertised WORK minimum suffices on the blocked path (Step 4c)', function t() {
	[ [ 'left', 80, 50, 40 ], [ 'right', 50, 80, 40 ] ].forEach( function eachCase( cfg ) {
		...
		function run( len ) {
			...
			dormql( side, trans, M, N, K, s.Ar.data, s.Ar.args[ 0 ], s.Ar.args[ 1 ], s.Ar.args[ 2 ], s.Tr.data, s.Tr.args[ 0 ], s.Tr.args[ 1 ], Cr.data, Cr.args[ 0 ], Cr.args[ 1 ], Cr.args[ 2 ], poisonedWork( sc, len ), 1, 0 );
			return check.flattenLogical( sc, readFull( Cr, M, N ) );
		}
		...
		var s = setup();
		var Cr = schemes.dense.realize( sc, s.C0, { 'part': 'full' }, null );
		dormql( side, trans, M, N, K, s.Ar.data, s.Ar.args[ 0 ], s.Ar.args[ 1 ], s.Ar.args[ 2 ], s.Tr.data, s.Tr.args[ 0 ], s.Tr.args[ 1 ], Cr.data, Cr.args[ 0 ], Cr.args[ 1 ], Cr.args[ 2 ], poisonedWork( sc, minLen ), 1, 0 );
