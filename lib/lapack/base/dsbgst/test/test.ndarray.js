...
import dsbgst from './../lib/ndarray.js';
...
function runTest( name, vect, uplo, N, ka, kb, ldab, ldbb, ldx, tol ) {
	...
	const AB = new Float64Array( tc.AB_in );
	const BB = new Float64Array( tc.BB );
	const Xm = new Float64Array( N * ldx );
	const WORK = new Float64Array( 2 * N );

	const info = dsbgst( vect, uplo, N, ka, kb, AB, 1, ldab, 0, BB, 1, ldbb, 0, Xm, 1, ldx, 0, WORK, 1, 0 ); // eslint-disable-line max-len
...
test( 'dsbgst: n_zero quick return', function t() {
	...
	const info = dsbgst( 'none', 'upper', 0, 1, 0, AB, 1, 2, 0, BB, 1, 1, 0, X, 1, 1, 0, WORK, 1, 0 ); // eslint-disable-line max-len
