...
*     uses a 2x2 complex register-tiled kernel over the STORED triangle (the
*     settled complex level-3 tile geometry; see bench/zgemm-opt/GEOMETRY.md):
*     the two rank-2k product terms are FUSED into one complex accumulator per
*     cell by folding alpha into hoisted column factors t1 = alpha*B(l,j) and
*     t2 = conj(alpha)*A(l,j), so each C(i,j) is accumulated in registers across
*     the whole K loop (C touched once, row loads reused across the tile). The
*     reference recomputes an O(K) strided dot product per cell, so the tile
*     wins ~1.25-1.75x here.
* -   **trans='no-transpose'** (C := alpha*A*B^H + ...) keeps the reference
*     rank-1-update structure (verbatim). For that mode the fused two-term tile
*     recomputes the column temps once per row-tile — ~50% more FLOPs — and the
*     reference already streams C with unit stride on the common column-major
*     layout, so tiling would regress it. Dispatching by trans avoids that.
...
function zher2k( uplo, trans, N, K, alpha, A, strideA1, strideA2, offsetA, B, strideB1, strideB2, offsetB, beta, C, strideC1, strideC2, offsetC ) {
	...
	var pak; var pbk; var pc; var nb; var ii;
	var alphaR; var alphaI;
	var temp1R; var temp1I; var temp2R; var temp2I;
	var upper; var nota;
	var ar; var ak; var br; var bk; var csr; var csc;
	var sa1; var sa2; var sb1; var sb2; var sc1; var sc2;
	var ajR; var ajI; var bjR; var bjI; var aiR; var aiI; var biR; var biI;
	var Av; var Bv; var Cv; var oA; var oB; var oC;
	var ic; var ia; var ib; var i; var j; var l;
	...
	if ( nota ) {
		...
		// streams C with unit stride on column-major, which the fused two-term
		// tile cannot beat — see file header).
		if ( upper ) {
			for ( j = 0; j < N; j++ ) {
				if ( beta === 0.0 ) {
					ic = oC + ( j * sc2 );
	...
	// trans === 'conjugate-transpose': C := alpha*A^H*B + conj(alpha)*B^H*A +
	// beta*C, via the 2x2 fused two-term register tile. op(A)(i,l) = A(l,i) =
	// Av[ oA + i*ar + l*ak ]; csr conjugates the row operands (index i), csc the
	// column operands (index j). For trans=C the row operands carry the conj.
	ar = sa2;
	ak = sa1;
	br = sb2;
	bk = sb1;
	...
	// Off-diagonal cell (ii,jj): C(ii,jj) = alpha*A^H*B + conj(alpha)*B^H*A + beta*C.
