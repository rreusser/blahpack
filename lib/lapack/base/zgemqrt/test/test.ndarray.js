...
const rawLines = readFileSync( path.join( fixtureDir, 'zgemqrt.jsonl' ), 'utf8' ).trim().split( '\n' );
...
function makeComplex( flat ) {
	...
	const arr = new Complex128Array( flat.length / 2 );
...
*/
function smallVT() {
	const tc = findCase( 'qr_factors_small' );
	return {
		'V': makeComplex( tc.v ),
		'T': makeComplex( tc.t )
	};
...
*/
function blockVT() {
	const tc = findCase( 'qr_factors_block' );
	return {
		'V': makeComplex( tc.v ),
		'T': makeComplex( tc.t )
	};
...
*/
function singleVT() {
	const tc = findCase( 'qr_factors_single' );
	return {
		'V': makeComplex( tc.v ),
		'T': makeComplex( tc.t )
	};
...
test( 'zgemqrt: left, no-transpose -> Q*I (small, NB=2, K=3)', function t() {
	...
	const vt = smallVT();
	const C = eye( 4, 4, 4 );
	const WORK = new Complex128Array( 4 * 2 );
	const info = zgemqrt( 'left', 'no-transpose', 4, 4, 3, 2, vt.V, 1, 4, 0, vt.T, 1, 2, 0, C, 1, 4, 0, WORK, 1, 0 );
...
test( 'zgemqrt: left, conjugate-transpose -> Q^H*I (small)', function t() {
	...
	const tc = findCase( 'left_ctrans' );
	const vt = smallVT();
	const C = eye( 4, 4, 4 );
	const WORK = new Complex128Array( 4 * 2 );
	const info = zgemqrt( 'left', 'conjugate-transpose', 4, 4, 3, 2, vt.V, 1, 4, 0, vt.T, 1, 2, 0, C, 1, 4, 0, WORK, 1, 0 );
	assert.equal( info, tc.info );
	assertArrayClose( toArray( reinterpret( C, 0 ) ), tc.c, 1e-13, 'c' );
});
...
test( 'zgemqrt: right, no-transpose, rectangular C(3,4) (small)', function t() {
	...
	const tc = findCase( 'right_notrans_rect' );
	const vt = smallVT();
	const C = rect34();
	const WORK = new Complex128Array( 3 * 2 );
	const info = zgemqrt( 'right', 'no-transpose', 3, 4, 3, 2, vt.V, 1, 4, 0, vt.T, 1, 2, 0, C, 1, 3, 0, WORK, 1, 0 );
	assert.equal( info, tc.info );
	assertArrayClose( toArray( reinterpret( C, 0 ) ), tc.c, 1e-13, 'c' );
});
...
test( 'zgemqrt: right, conjugate-transpose, rectangular C(3,4) (small)', function t() {
	...
	const tc = findCase( 'right_ctrans_rect' );
	const vt = smallVT();
	const C = rect34();
	const WORK = new Complex128Array( 3 * 2 );
	const info = zgemqrt( 'right', 'conjugate-transpose', 3, 4, 3, 2, vt.V, 1, 4, 0, vt.T, 1, 2, 0, C, 1, 3, 0, WORK, 1, 0 );
	assert.equal( info, tc.info );
	assertArrayClose( toArray( reinterpret( C, 0 ) ), tc.c, 1e-13, 'c' );
});
...
test( 'zgemqrt: left, no-transpose on dense 4x4 (small)', function t() {
	...
	const tc = findCase( 'left_notrans_dense' );
	const vt = smallVT();
	const C = dense44();
	const WORK = new Complex128Array( 4 * 2 );
	const info = zgemqrt( 'left', 'no-transpose', 4, 4, 3, 2, vt.V, 1, 4, 0, vt.T, 1, 2, 0, C, 1, 4, 0, WORK, 1, 0 );
...
test( 'zgemqrt: left, conjugate-transpose on dense 4x4 (small)', function t() {
	...
	const tc = findCase( 'left_ctrans_dense' );
	const vt = smallVT();
	const C = dense44();
	const WORK = new Complex128Array( 4 * 2 );
	const info = zgemqrt( 'left', 'conjugate-transpose', 4, 4, 3, 2, vt.V, 1, 4, 0, vt.T, 1, 2, 0, C, 1, 4, 0, WORK, 1, 0 );
...
test( 'zgemqrt: M=0 quick return', function t() {
	...
	const info = zgemqrt( 'left', 'no-transpose', 0, 4, 0, 2, vt.V, 1, 4, 0, vt.T, 1, 2, 0, C, 1, 1, 0, WORK, 1, 0 );
	assert.equal( info, tc.info );
});
...
test( 'zgemqrt: N=0 quick return', function t() {
	...
	const C = new Complex128Array( 1 );
	const WORK = new Complex128Array( 1 );
	const info = zgemqrt( 'left', 'no-transpose', 4, 0, 0, 2, vt.V, 1, 4, 0, vt.T, 1, 2, 0, C, 1, 4, 0, WORK, 1, 0 );
	assert.equal( info, tc.info );
});

test( 'zgemqrt: K=0 quick return (C unchanged)', function t() {

	const tc = findCase( 'k_zero' );
	const vt = smallVT();
	const C = eye( 4, 4, 4 );
	const Cexp = eye( 4, 4, 4 );
	const WORK = new Complex128Array( 4 * 2 );
	const info = zgemqrt( 'left', 'no-transpose', 4, 4, 0, 2, vt.V, 1, 4, 0, vt.T, 1, 2, 0, C, 1, 4, 0, WORK, 1, 0 );
	assert.equal( info, tc.info );
...
test( 'zgemqrt: left, no-transpose (multi-block, M=10, K=8, NB=3)', function t() {
	...
	const tc = findCase( 'left_notrans_block' );
	const vt = blockVT();
	const C = new Complex128Array( 100 );
	fillCos( C, 10, 10, 10 );
	const WORK = new Complex128Array( 10 * 3 );
	const info = zgemqrt( 'left', 'no-transpose', 10, 10, 8, 3, vt.V, 1, 10, 0, vt.T, 1, 3, 0, C, 1, 10, 0, WORK, 1, 0 );
	assert.equal( info, tc.info );
...
test( 'zgemqrt: left, conjugate-transpose (multi-block)', function t() {
	...
	const tc = findCase( 'left_ctrans_block' );
	const vt = blockVT();
	const C = new Complex128Array( 100 );
	fillCos( C, 10, 10, 10 );
	const WORK = new Complex128Array( 10 * 3 );
	const info = zgemqrt( 'left', 'conjugate-transpose', 10, 10, 8, 3, vt.V, 1, 10, 0, vt.T, 1, 3, 0, C, 1, 10, 0, WORK, 1, 0 );
	assert.equal( info, tc.info );
	assertArrayClose( toArray( reinterpret( C, 0 ) ), tc.c, 1e-12, 'c' );
});
...
test( 'zgemqrt: right, no-transpose (multi-block)', function t() {
	...
	const tc = findCase( 'right_notrans_block' );
	const vt = blockVT();
	const C = new Complex128Array( 100 );
	fillCos( C, 10, 10, 10 );
	const WORK = new Complex128Array( 10 * 3 );
	const info = zgemqrt( 'right', 'no-transpose', 10, 10, 8, 3, vt.V, 1, 10, 0, vt.T, 1, 3, 0, C, 1, 10, 0, WORK, 1, 0 );
	assert.equal( info, tc.info );
	assertArrayClose( toArray( reinterpret( C, 0 ) ), tc.c, 1e-12, 'c' );
});
...
test( 'zgemqrt: right, conjugate-transpose (multi-block)', function t() {
	...
	const tc = findCase( 'right_ctrans_block' );
	const vt = blockVT();
	const C = new Complex128Array( 100 );
	fillCos( C, 10, 10, 10 );
	const WORK = new Complex128Array( 10 * 3 );
	const info = zgemqrt( 'right', 'conjugate-transpose', 10, 10, 8, 3, vt.V, 1, 10, 0, vt.T, 1, 3, 0, C, 1, 10, 0, WORK, 1, 0 );
	assert.equal( info, tc.info );
	assertArrayClose( toArray( reinterpret( C, 0 ) ), tc.c, 1e-12, 'c' );
});
...
test( 'zgemqrt: left, no-transpose (single-block, NB == K)', function t() {
	...
	const tc = findCase( 'left_notrans_single' );
	const vt = singleVT();
	const C = new Complex128Array( 25 );
	fillSin( C, 5, 5, 5 );
	const WORK = new Complex128Array( 5 * 4 );
	const info = zgemqrt( 'left', 'no-transpose', 5, 5, 4, 4, vt.V, 1, 5, 0, vt.T, 1, 4, 0, C, 1, 5, 0, WORK, 1, 0 );
	assert.equal( info, tc.info );
	assertArrayClose( toArray( reinterpret( C, 0 ) ), tc.c, 1e-12, 'c' );
});
...
test( 'zgemqrt: left, conjugate-transpose (single-block)', function t() {
	...
	const tc = findCase( 'left_ctrans_single' );
	const vt = singleVT();
	const C = new Complex128Array( 25 );
	fillSin( C, 5, 5, 5 );
	const WORK = new Complex128Array( 5 * 4 );
	const info = zgemqrt( 'left', 'conjugate-transpose', 5, 5, 4, 4, vt.V, 1, 5, 0, vt.T, 1, 4, 0, C, 1, 5, 0, WORK, 1, 0 );
	assert.equal( info, tc.info );
	assertArrayClose( toArray( reinterpret( C, 0 ) ), tc.c, 1e-12, 'c' );
});
...
test( 'zgemqrt: round trip Q^H * Q * C === C (left, multi-block)', function t() {
	...
	const vt = blockVT();
	const C = new Complex128Array( 100 );
	const Corig = new Complex128Array( 100 );
	fillCos( C, 10, 10, 10 );
	const Cview = reinterpret( C, 0 );
	const Oview = reinterpret( Corig, 0 );
	Oview.set( Cview );
	const WORK = new Complex128Array( 10 * 3 );
...
test( 'zgemqrt: validator rejects invalid side', function t() {
	...
	const V = new Complex128Array( 12 );
	const T = new Complex128Array( 6 );
