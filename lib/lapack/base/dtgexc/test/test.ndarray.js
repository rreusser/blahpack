...
import dtgexc from './../lib/ndarray.js';
const ndarrayFn = dtgexc;
...
function loadFixtures() {
	const fixtures = {};
	const lines = readFileSync( resolve( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures', 'dtgexc.jsonl' ), 'utf8' ).trim().split( '\n' );
	let i, d;
...
test( 'dtgexc: move 1x1 block forward with Q and Z', function t() {
	...
	const tc = FIXTURES.move_1x1_forward_qz;
	const N = 4;
	...
	const Q = eye( N );
	const Z = eye( N );
	const WORK = new Float64Array( 4 * N + 16 );

	// Fortran: IFST=1, ILST=4 → JS 0-based: ifst=0, ilst=3
	const r = dtgexc( true, true, N, A, 1, N, 0, B, 1, N, 0, Q, 1, N, 0, Z, 1, N, 0, 0, 3, WORK, 1, 0 );
...
test( 'dtgexc: move 1x1 block backward with Q and Z', function t() {
	...
	const tc = FIXTURES.move_1x1_backward_qz;
	const N = 4;
	...
	const r = dtgexc( true, true, N, A, 1, N, 0, B, 1, N, 0, Q, 1, N, 0, Z, 1, N, 0, 3, 0, WORK, 1, 0 );
	assert.strictEqual( r.info, tc.info, 'info' );
	assert.strictEqual( r.ifst, tc.ifst - 1, 'ifst' );
	assert.strictEqual( r.ilst, tc.ilst - 1, 'ilst' );
	assertArrayClose( toArray( A ), tc.A, 1e-12, 'A' );
...
test( 'dtgexc: move 2x2 block forward with Q and Z', function t() {
	...
	const tc = FIXTURES.move_2x2_forward_qz;
	const N = 4;
...
test( 'dtgexc: move 2x2 block backward with Q and Z', function t() {
	...
	const tc = FIXTURES.move_2x2_backward_qz;
	const N = 4;
	...
	const r = dtgexc( true, true, N, A, 1, N, 0, B, 1, N, 0, Q, 1, N, 0, Z, 1, N, 0, 2, 0, WORK, 1, 0 );
	assert.strictEqual( r.info, tc.info, 'info' );
	assert.strictEqual( r.ifst, tc.ifst - 1, 'ifst' );
	assert.strictEqual( r.ilst, tc.ilst - 1, 'ilst' );
...
test( 'dtgexc: move 1x1 forward without Q/Z', function t() {
	...
	const tc = FIXTURES.move_1x1_no_qz;
	const N = 3;
	...
	const r = dtgexc( false, false, N, A, 1, N, 0, B, 1, N, 0, Q, 1, N, 0, Z, 1, N, 0, 0, 2, WORK, 1, 0 );
	assert.strictEqual( r.info, tc.info, 'info' );
	assert.strictEqual( r.ifst, tc.ifst - 1, 'ifst' );
	assert.strictEqual( r.ilst, tc.ilst - 1, 'ilst' );
	assertArrayClose( toArray( A ), tc.A, 1e-12, 'A' );
...
test( 'dtgexc: noop when ifst equals ilst', function t() {
	...
	const tc = FIXTURES.noop_ifst_eq_ilst;
	const N = 3;
	...
	const r = dtgexc( false, false, N, A, 1, N, 0, B, 1, N, 0, Q, 1, N, 0, Z, 1, N, 0, 1, 1, WORK, 1, 0 );
	assert.strictEqual( r.info, tc.info, 'info' );
	assert.strictEqual( r.ifst, tc.ifst - 1, 'ifst' );
	assert.strictEqual( r.ilst, tc.ilst - 1, 'ilst' );
...
test( 'dtgexc: N=1 quick return', function t() {
	...
	const r = dtgexc( true, true, 1, A, 1, 1, 0, B, 1, 1, 0, Q, 1, 1, 0, Z, 1, 1, 0, 0, 0, WORK, 1, 0 );
...
test( 'dtgexc: move 1x1 forward in 6x6 matrix', function t() {
	...
	const tc = FIXTURES.move_1x1_forward_6x6;
	const N = 6;
	...
	const r = dtgexc( true, true, N, A, 1, N, 0, B, 1, N, 0, Q, 1, N, 0, Z, 1, N, 0, 1, 4, WORK, 1, 0 );
	assert.strictEqual( r.info, tc.info, 'info' );
	assert.strictEqual( r.ifst, tc.ifst - 1, 'ifst' );
	assert.strictEqual( r.ilst, tc.ilst - 1, 'ilst' );
	assertArrayClose( toArray( A ), tc.A, 1e-12, 'A' );
...
test( 'dtgexc: ifst points to second row of 2x2 block (adjusted)', function t() {
	...
	const tc = FIXTURES.ifst_second_row_2x2;
	const N = 4;
	...
	const r = dtgexc( true, true, N, A, 1, N, 0, B, 1, N, 0, Q, 1, N, 0, Z, 1, N, 0, 1, 3, WORK, 1, 0 );
	assert.strictEqual( r.info, tc.info, 'info' );
	assert.strictEqual( r.ifst, tc.ifst - 1, 'ifst' );
	assert.strictEqual( r.ilst, tc.ilst - 1, 'ilst' );
	assertArrayClose( toArray( A ), tc.A, 1e-12, 'A' );
...
test( 'dtgexc: move 1x1 forward across 2x2 block (nbnext=2)', function t() {
	...
	const tc = FIXTURES.move_1x1_fwd_across_2x2;
	const N = 4;
...
test( 'dtgexc: move 1x1 backward across 2x2 block (nbnext=2)', function t() {
	...
	const tc = FIXTURES.move_1x1_bwd_across_2x2;
	const N = 4;
...
test( 'dtgexc: move 1x1 forward with nbl=2 adjustment', function t() {
	...
	const tc = FIXTURES.move_1x1_fwd_nbl2_adjust;
	const N = 4;
	...
	const r = dtgexc( true, true, N, A, 1, N, 0, B, 1, N, 0, Q, 1, N, 0, Z, 1, N, 0, 0, 2, WORK, 1, 0 );
	assert.strictEqual( r.info, tc.info, 'info' );
	assert.strictEqual( r.ifst, tc.ifst - 1, 'ifst' );
	assert.strictEqual( r.ilst, tc.ilst - 1, 'ilst' );
	verifyOrthogonal( Q, N, 1e-12, 'Q orthogonal' );
...
test( 'dtgexc: ilst points to second row of 2x2 block', function t() {
	...
	const tc = FIXTURES.ilst_second_row_2x2;
	const N = 4;
...
test( 'dtgexc: move 2x2 forward across 2x2 in 6x6', function t() {
	...
	const tc = FIXTURES.move_2x2_fwd_across_2x2_6x6;
	const N = 6;
	...
	const Borig = new Float64Array( N * N );
	...
	const r = dtgexc( true, true, N, A, 1, N, 0, B, 1, N, 0, Q, 1, N, 0, Z, 1, N, 0, 0, 4, WORK, 1, 0 );
	assert.strictEqual( r.info, tc.info, 'info' );
	assert.strictEqual( r.ifst, tc.ifst - 1, 'ifst' );
	assert.strictEqual( r.ilst, tc.ilst - 1, 'ilst' );
	verifyOrthogonal( Q, N, 1e-12, 'Q orthogonal' );
...
test( 'dtgexc: move 2x2 backward across 2x2 in 6x6', function t() {
	...
	const tc = FIXTURES.move_2x2_bwd_across_2x2_6x6;
	const N = 6;
	...
	const r = dtgexc( true, true, N, A, 1, N, 0, B, 1, N, 0, Q, 1, N, 0, Z, 1, N, 0, 4, 0, WORK, 1, 0 );
	assert.strictEqual( r.info, tc.info, 'info' );
	assert.strictEqual( r.ifst, tc.ifst - 1, 'ifst' );
	assert.strictEqual( r.ilst, tc.ilst - 1, 'ilst' );
	verifyOrthogonal( Q, N, 1e-12, 'Q orthogonal' );
