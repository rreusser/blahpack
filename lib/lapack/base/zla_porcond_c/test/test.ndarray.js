...
import zla_porcond_c from './../lib/ndarray.js';


// FIXTURES //

const fixtureDir = path.join( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures' );
const lines = readFileSync( path.join( fixtureDir, 'zla_porcond_c.jsonl' ), 'utf8' ).trim().split( '\n' ); // eslint-disable-line node/no-sync
...
function runCase( uplo, c, capply, fixtureName ) {
	...
	const info = zpotrf( uplo, N, AF, 1, N, 0 );
	assert.equal( info, 0, fixtureName + ': zpotrf info' );
	const res = zla_porcond_c( uplo, N, A, 1, N, 0, AF, 1, N, 0, c, 1, 0, capply, WORK, 1, 0, RWORK, 1, 0 );
...
test( 'zla_porcond_c: n1_uplo_U', function t() {
	...
	const AF = new Complex128Array( [ 2.0, 0.0 ] );
	const c = new Float64Array( [ 2.0 ] );
	const WORK = new Complex128Array( 2 * N );
	const RWORK = new Float64Array( N );
	const tc = findCase( 'n1_uplo_U' );
	const res = zla_porcond_c( 'upper', N, A, 1, N, 0, AF, 1, N, 0, c, 1, 0, true, WORK, 1, 0, RWORK, 1, 0 );
...
test( 'zla_porcond_c: n0', function t() {
	...
	const res = zla_porcond_c( 'upper', 0, A, 1, 1, 0, AF, 1, 1, 0, c, 1, 0, true, WORK, 1, 0, RWORK, 1, 0 );
...
test( 'zla_porcond_c: lower anorm zero returns 0', function t() {
	...
	const WORK = new Complex128Array( 2 * N );
	const RWORK = new Float64Array( N );
	const res = zla_porcond_c( 'lower', N, A, 1, N, 0, AF, 1, N, 0, c, 1, 0, false, WORK, 1, 0, RWORK, 1, 0 );
