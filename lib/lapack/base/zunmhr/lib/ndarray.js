...
function zunmhr( side, trans, M, N, ilo, ihi, A, strideA1, strideA2, offsetA, TAU, strideTAU, offsetTAU, C, strideC1, strideC2, offsetC, WORK, strideWork, offsetWork ) { // eslint-disable-line max-len, max-params
	...
	// Caller owns the workspace. When Q is actually applied (M>0, N>0, and there
	// are reflectors, i.e. IHI-ILO !== 0), zunmqr needs `>= N` complex elements
	// for a left-side apply and `>= M` for a right-side apply. Assert a
	// sufficiently large array so an under-sized buffer is a loud RangeError.
	if ( M > 0 && N > 0 && ( ihi - ilo ) !== 0 ) {
		need = ( side === 'left' ) ? max( 1, N ) : max( 1, M );
