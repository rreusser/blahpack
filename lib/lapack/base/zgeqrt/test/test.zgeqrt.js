...
import zgeqrt from './../lib/zgeqrt.js';
...
test( 'zgeqrt accepts column-major valid call', function t() {
	const A = new Complex128Array( 12 ); // 4x3 column-major
	const T = new Complex128Array( 6 ); // 2x3
	const WORK = new Complex128Array( 12 );
	const info = zgeqrt( 'column-major', 4, 3, 2, A, 4, T, 2, WORK );
	assert.strictEqual( info, 0, 'returns 0' );
});

test( 'zgeqrt accepts row-major valid call', function t() {
	const A = new Complex128Array( 12 ); // 4x3 row-major
	const T = new Complex128Array( 6 ); // 2x3 row-major (LDT=3)
	const WORK = new Complex128Array( 12 );
	const info = zgeqrt( 'row-major', 4, 3, 2, A, 3, T, 3, WORK );
	assert.strictEqual( info, 0, 'returns 0' );
});
