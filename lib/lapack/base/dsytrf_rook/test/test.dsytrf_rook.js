...
test( 'dsytrf_rook has expected arity', function t() {
	assert.strictEqual( dsytrfRook.length, 9, 'has expected arity' );
});
...
test( 'dsytrf_rook returns 0 for N=0', function t() {
	const info = dsytrfRook( 'column-major', 'upper', 0, new Float64Array( 1 ), 1, new Int32Array( 1 ), 1 ); // eslint-disable-line max-len
	assert.strictEqual( info, 0, 'returns 0' );
});

test( 'dsytrf_rook column-major basic 2x2', function t() {
	const IPIV = new Int32Array( 2 );
	const A = new Float64Array( [ 4.0, 1.0, 1.0, 3.0 ] );
	const info = dsytrfRook( 'column-major', 'lower', 2, A, 2, IPIV, 1, null, 1 );
...
test( 'dsytrf_rook row-major basic 2x2 (transposes via swapped strides)', function t() {
	const IPIV = new Int32Array( 2 );
	const A = new Float64Array( [ 4.0, 1.0, 1.0, 3.0 ] );
	const info = dsytrfRook( 'row-major', 'lower', 2, A, 2, IPIV, 1, null, 1 );
	assert.strictEqual( info, 0, 'info' );
});
