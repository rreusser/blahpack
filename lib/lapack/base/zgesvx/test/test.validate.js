/**
* Property-based validation for zgesvx, following the /blahpack-validate process.
*
...
* equilibration + LU factor (zgetrf) + solve (zgetrs) + iterative refinement
* (zgerfs) + condition estimate (zgecon). It therefore inherits the three
...
*                  FERR ∈ [0,1).
*
* fact/equed/trans convention (confirmed from lib/base.js + lib/ndarray.js):
...
*   - rcond is returned in the result OBJECT (result.rcond). The ndarray wrapper
*     declares a vestigial `rcond` parameter it never receives (callers omit it,
*     as the reference tests do); the extra parameter's rightward shift is exactly
*     cancelled when the wrapper forwards to base(), which has NO rcond parameter.
*     So zgesvx is invoked WITHOUT an rcond array argument (see test.ndarray.js).
*   - trans in {no-transpose(one-norm), transpose/conjugate-transpose(inf-norm)}.
*     zgecon uses the same norm zgesvx picks per trans.
*   - The equilibration path (fact='equilibrate') is exercised separately below on
*     a deliberately badly-scaled matrix: even when equed != 'none' the returned X
*     still solves the ORIGINAL system (the driver un-scales X on exit).
*
...
// Build op(A0) as a LogicalMatrix: 'n' -> A0, 't' -> A0^T, 'c' -> A0^H.
...
// Independent true inverse of op(A0): solve op(A0)*Y = I with zgesv.
function opInverse( A0, N, code ) {
	const Aop = opMatrix( A0, N, code );
	const Ar = schemes.dense.realize( sc, Aop, { 'part': 'full' }, TIGHT );
	const Ir = schemes.dense.realize( sc, identity( N ), { 'part': 'full' }, TIGHT );
	const ipiv = new Int32Array( N );
	zgesv( N, N, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], ipiv, 1, 0, Ir.data, Ir.args[ 0 ], Ir.args[ 1 ], Ir.args[ 2 ] );
	return readMat( Ir, N, N );
}

// Drive zgesvx on realized copies; return physical output handles + result.
// NOTE: called WITHOUT an rcond array argument (see the header note).
function callGesvx( fact, trans, N, nrhs, A0, B0, layout ) {
	const Ar = schemes.dense.realize( sc, A0, { 'part': 'full' }, layout );
	const AFr = schemes.dense.realize( sc, zeros( N, N ), { 'part': 'full' }, layout );
	const Br = schemes.dense.realize( sc, B0, { 'part': 'full' }, layout );
	const Xr = schemes.dense.realize( sc, zeros( N, nrhs ), { 'part': 'full' }, layout );
	const ipiv = new Int32Array( N );
	const r = new Float64Array( N );
	const c = new Float64Array( N );
	const FERR = new Float64Array( nrhs );
	const BERR = new Float64Array( nrhs );
	const WORK = poisonedWork( sc, Math.max( 1, 2 * N ) ); // complex workspace
	const RWORK = new Float64Array( Math.max( 1, 2 * N ) ); // real workspace
	const res = zgesvx( fact, trans, N, nrhs, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], AFr.data, AFr.args[ 0 ], AFr.args[ 1 ], AFr.args[ 2 ], ipiv, 1, 0, 'none', r, 1, 0, c, 1, 0, Br.data, Br.args[ 0 ], Br.args[ 1 ], Br.args[ 2 ], Xr.data, Xr.args[ 0 ], Xr.args[ 1 ], Xr.args[ 2 ], FERR, 1, 0, BERR, 1, 0, WORK, 1, 0, RWORK, 1, 0 );
	return {
		'X': Xr,
		'FERR': FERR,
		'BERR': BERR,
...
test( 'zgesvx: residual + rcond + FERR/BERR bound (trans x N x nrhs)', function t() {
...
function runProperty( trans, N, nrhs ) {
	...
	const which = ( code === 'n' ) ? 'one' : 'inf'; // gesvx norm choice per trans

	const out = callGesvx( 'not-factored', trans, N, nrhs, A0, B0, TIGHT );
	const tag = 'zgesvx '+trans+' N='+N+' nrhs='+nrhs;

	// (a) residual: X solves op(A0)*X = B0 against the ORIGINAL matrix.
	checked( 'zgesvx', 'residual', function run() {
		if ( out.res.info !== 0 ) {
	...
	checked( 'zgesvx', 'property', function run() {
	...
	checked( 'zgesvx', 'structural', function run() {
...
// forces zgeequ to equilibrate (equed != 'none'), yet the returned X must still
// solve the ORIGINAL op(A0)*X = B0 (the driver un-scales X on exit).
function badlyScaled( rng, N ) {
	const M = logical.general( sc, rng, N, N );
	const pr = [];
	const pc = [];
	let i, j;
	for ( i = 0; i < N; i++ ) {
...
test( 'zgesvx: equilibration path un-scales X to the ORIGINAL system', function t() {
	let sawEquil = false;
	[ 'no-transpose', 'transpose' ].forEach( function eachTrans( trans ) {
		[ 5, 8, 16 ].forEach( function eachN( N ) {
			const rng = new RNG( 0xEC + N );
			const A0 = badlyScaled( rng, N );
			const B0 = logical.general( sc, rng, N, 2 );
			const code = transCode( trans );
			const out = callGesvx( 'equilibrate', trans, N, 2, A0, B0, TIGHT );
			if ( out.res.equed !== 'none' ) {
				sawEquil = true;
			}
			checked( 'zgesvx', 'residual', function run() {
				const tag = 'zgesvx EQUIL '+trans+' N='+N+' equed='+out.res.equed;
				if ( out.res.info !== 0 ) {
...
// Step 4: layout-invariance fuzz. zgesvx factors A from scratch (zgetrf: izamax
// pivot search => positive row stride required, schemes.dense.pivotLayouts) and
// composes zgecon/zgetrs/zgerfs whose inner zgemv/zlatrs kernels switch on a
...
// ++ FERR ++ BERR.
const PIVOT = schemes.dense.pivotLayouts();
const COL = PIVOT.filter( function isCol( L ) {
	return L.order !== 'row';
});
const ROW = PIVOT.filter( function isRow( L ) {
	return L.order === 'row';
});

test( 'zgesvx: bit-exact within storage-order family (col / row)', function t() {
	TRANS.forEach( function eachTrans( trans ) {
...
function runInvariance( trans, variants, fam ) {
	...
	checked( 'zgesvx', 'layout-invariance', function run() {
		layoutInvariant( variants, function build( layout ) {
			const out = callGesvx( 'not-factored', trans, N, nrhs, A0, B0, layout );
			...
			return flat;
		}, { 'label': 'zgesvx '+trans+' layout invariance '+fam+'-major' } );
	});
}
