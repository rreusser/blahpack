...
import zhpcon from './../lib/ndarray.js';
...
test( 'zhpcon: N=0 quick return', function t() {
	...
	rcond[ 0 ] = 0.5;
	const info = zhpcon( 'upper', 0, new Complex128Array( 0 ), 1, 0, new Int32Array( 0 ), 1, 0, 1.0, rcond, new Complex128Array( 0 ), 1, 0 );
...
test( 'zhpcon: anorm=0 returns rcond=0 (upper)', function t() {
	...
	const rcond = new Float64Array( 1 );
	rcond[ 0 ] = 0.5;
	const info = zhpcon( 'upper', N, AP, 1, 0, IPIV, 1, 0, 0.0, rcond, new Complex128Array( 2 * N ), 1, 0 );
	assert.strictEqual( info, 0 );
...
test( 'zhpcon: identity 3x3 upper (rcond=1)', function t() {
	...
	const anorm = packedNorm1HermUpper( AP, N );
	const IPIV = new Int32Array( N );
	zhptrf( 'upper', N, AP, 1, 0, IPIV, 1, 0 );
	const rcond = new Float64Array( 1 );
	const info = zhpcon( 'upper', N, AP, 1, 0, IPIV, 1, 0, anorm, rcond, new Complex128Array( 2 * N ), 1, 0 );
	assert.strictEqual( info, 0 );
...
test( 'zhpcon: identity 4x4 lower (rcond=1)', function t() {
	...
	const anorm = packedNorm1HermLower( AP, N );
	const IPIV = new Int32Array( N );
	zhptrf( 'lower', N, AP, 1, 0, IPIV, 1, 0 );
	const rcond = new Float64Array( 1 );
	const info = zhpcon( 'lower', N, AP, 1, 0, IPIV, 1, 0, anorm, rcond, new Complex128Array( 2 * N ), 1, 0 );
	assert.strictEqual( info, 0 );
	assert.ok( close( rcond[ 0 ], 1.0, 1e-9 ) );
});
...
test( 'zhpcon: HPD 3x3 upper with complex off-diagonals', function t() {
	...
	const N = 3;
	const AP = new Complex128Array( new Float64Array([
		10, 0,
		2, 1,
		8, 0,
		3, -2,
...
test( 'zhpcon: singular matrix - zero diagonal in factored D (upper)', function t() {
	...
	const rcond = new Float64Array( 1 );
	rcond[ 0 ] = 0.5;
	const info = zhpcon( 'upper', N, AP, 1, 0, IPIV, 1, 0, 1.0, rcond, new Complex128Array( 2 * N ), 1, 0 );
	assert.strictEqual( info, 0 );
	assert.strictEqual( rcond[ 0 ], 0.0 );
});
...
test( 'zhpcon: singular matrix - zero diagonal in factored D (lower)', function t() {
	...
	const rcond = new Float64Array( 1 );
	rcond[ 0 ] = 0.5;
	const info = zhpcon( 'lower', N, AP, 1, 0, IPIV, 1, 0, 1.0, rcond, new Complex128Array( 2 * N ), 1, 0 );
	assert.strictEqual( info, 0 );
	assert.strictEqual( rcond[ 0 ], 0.0 );
});
...
test( 'zhpcon: N=1 simple', function t() {
	...
	zhptrf( 'upper', N, AP, 1, 0, IPIV, 1, 0 );
	const rcond = new Float64Array( 1 );
	const info = zhpcon( 'upper', N, AP, 1, 0, IPIV, 1, 0, 5.0, rcond, new Complex128Array( 2 ), 1, 0 );
	assert.strictEqual( info, 0 );
