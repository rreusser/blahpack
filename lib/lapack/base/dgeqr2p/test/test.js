...
test( 'main export produces non-negative R diagonal', function t() {
	...
	const info = dgeqr2p( 'column-major', 3, 2, A, 3, TAU, 1, WORK, 1 );
...
test( 'ndarray method produces non-negative R diagonal', function t() {
	...
	const TAU = new Float64Array( 2 );
	const WORK = new Float64Array( 2 );
	const info = dgeqr2p.ndarray( 3, 2, A, 1, 3, 0, TAU, 1, 0, WORK, 1, 0 );
	assert.strictEqual( info, 0 );
	assert.ok( A[ 0 ] >= 0, 'R[0,0] non-negative' );
	assert.ok( A[ 4 ] >= 0, 'R[1,1] non-negative' );
});
...
test( 'ndarray method handles M=0 quick return', function t() {
	const info = dgeqr2p.ndarray( 0, 2, new Float64Array( 4 ), 1, 1, 0, new Float64Array( 2 ), 1, 0, new Float64Array( 2 ), 1, 0 ); // eslint-disable-line max-len
	assert.strictEqual( info, 0 );
});
