...
import dlaorhr_col_getrfnp2 from './../lib/ndarray.js';
const ndarrayFn = dlaorhr_col_getrfnp2;


// FIXTURES //

const fixtureDir = path.join( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures' );
const fixturePath = path.join( fixtureDir, 'dlaorhr_col_getrfnp2.jsonl' );
...
function runCase( name, M, N, input, dlen ) {
	...
	const info = dlaorhr_col_getrfnp2( M, N, a, 1, Math.max( 1, M ), 0, d, 1, 0 );
...
test( 'dlaorhr_col_getrfnp2: M=0 quick return', function t() {
	...
	const info = dlaorhr_col_getrfnp2( 0, 3, a, 1, 1, 0, d, 1, 0 );
...
test( 'dlaorhr_col_getrfnp2: N=0 quick return', function t() {
	const a = new Float64Array( [ 1.0, 2.0, 3.0 ] );
	const d = new Float64Array( 1 );
	const info = dlaorhr_col_getrfnp2( 3, 0, a, 1, 3, 0, d, 1, 0 );
	assert.equal( info, 0, 'info' );
	assert.equal( a[ 0 ], 1.0, 'a unchanged' );
});
...
test( 'dlaorhr_col_getrfnp2: row-major (3x3)', function t() {
	...
	const aRowMajor = new Float64Array( [ 0.5, -0.4, 0.2, 0.3, 0.6, -0.1, -0.2, 0.1, 0.7 ] ); // eslint-disable-line max-len
	const d = new Float64Array( 3 );

	// Row-major: strideA1 = 3 (row stride), strideA2 = 1 (col stride)
	const info = dlaorhr_col_getrfnp2( 3, 3, aRowMajor, 3, 1, 0, d, 1, 0 );
	assert.equal( info, tc.info, 'info' );

	// Transpose result for fixture comparison
	const aColMajor = new Float64Array( 9 );
	for ( i = 0; i < 3; i++ ) {
		for ( j = 0; j < 3; j++ ) {
