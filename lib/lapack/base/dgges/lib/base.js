...
import dtgsen from '../../dtgsen/lib/base.js';
...
* @param {Float64Array} WORK - caller-provided workspace of length at least max(8*N, 6*N+16, 1)
...
function dgges( jobvsl, jobvsr, sort, selctg, N, A, strideA1, strideA2, offsetA, B, strideB1, strideB2, offsetB, ALPHAR, strideALPHAR, offsetALPHAR, ALPHAI, strideALPHAI, offsetALPHAI, BETA, strideBETA, offsetBETA, VSL, strideVSL1, strideVSL2, offsetVSL, VSR, strideVSR1, strideVSR2, offsetVSR, WORK, strideWork, offsetWork, BWORK, strideBwork, offsetBwork ) {
	...
	// Partition the caller-provided WORK buffer to match the reference layout
	// (Fortran ILEFT/IRIGHT/IWRK):
	//   WORK[0..N)   : LSCALE (left balancing scale factors)
	//   WORK[N..2N)  : RSCALE (right balancing scale factors)
	//   WORK[2N..)   : TAU (during the QR phase) and general scratch (IWRK)
	const oLscale = offsetWork;
	const oRscale = offsetWork + ( N * strideWork );
	const oWrk = offsetWork + ( 2 * N * strideWork );
	...
	const anrm = dlange( 'max', N, N, A, strideA1, strideA2, offsetA, WORK, strideWork, offsetWork );
	ilascl = false;
	anrmto = 0.0;
	...
	const bnrm = dlange( 'max', N, N, B, strideB1, strideB2, offsetB, WORK, strideWork, offsetWork );
	ilbscl = false;
	bnrmto = 0.0;
	...
	const bal = dggbal( 'permute', N, A, strideA1, strideA2, offsetA, B, strideB1, strideB2, offsetB, WORK, strideWork, oLscale, WORK, strideWork, oRscale, WORK, strideWork, oWrk );
	...
	// TAU occupies the head of the IWRK region (Fortran ITAU = IWRK); the QR
	// scratch that follows starts after TAU (IWRK = ITAU + IROWS).
	const oWrkq = oWrk + ( irows * strideWork );
	...
	dgeqrf( irows, icols, B, strideB1, strideB2, oBI, WORK, strideWork, oWrk, WORK, strideWork, oWrkq );

	// Apply the orthogonal transformation to A: A(ilo:ihi, ilo:N) = Q^T * A(ilo:ihi, ilo:N)
	dormqr( 'left', 'transpose', irows, icols, irows, B, strideB1, strideB2, oBI, WORK, strideWork, oWrk, A, strideA1, strideA2, oAI, WORK, strideWork, oWrkq );
	...
	if ( ilvsl ) {
		...
		dorgqr( irows, irows, irows, VSL, strideVSL1, strideVSL2, offsetVSL + ( ( ilo - 1 ) * strideVSL1 ) + ( ( ilo - 1 ) * strideVSL2 ), WORK, strideWork, oWrk, WORK, strideWork, oWrkq );
	}
	...
	if ( wantst ) {
		...
		for ( i = 0; i < N; i++ ) {
			SELECT[ offsetBwork + ( i * strideBwork ) ] = ( selctg( ALPHAR[ offsetALPHAR + ( i * strideALPHAR ) ], ALPHAI[ offsetALPHAI + ( i * strideALPHAI ) ], BETA[ offsetBETA + ( i * strideBETA ) ] ) ) ? 1 : 0;
		}

		// Reorder eigenvalues using dtgsen (ijob=0, simplest mode)
		// M is an Int32Array output: M[0] = number of selected eigenvalues
		ierr = dtgsen( 0, ilvsl, ilvsr, SELECT, strideBwork, offsetBwork, N, A, strideA1, strideA2, offsetA, B, strideB1, strideB2, offsetB, ALPHAR, strideALPHAR, offsetALPHAR, ALPHAI, strideALPHAI, offsetALPHAI, BETA, strideBETA, offsetBETA, VSL, strideVSL1, strideVSL2, offsetVSL, VSR, strideVSR1, strideVSR2, offsetVSR, M, PL, PR, DIF, 1, 0, WORK, strideWork, oWrk, IWORK, 1, 0 );
		if ( ierr === 1 ) {
			info = N + 3;
		}
	}
	...
	if ( ilvsl ) {
		dggbak( 'permute', 'left', N, ilo, ihi, WORK, strideWork, oLscale, WORK, strideWork, oRscale, N, VSL, strideVSL1, strideVSL2, offsetVSL );
	}
	if ( ilvsr ) {
		dggbak( 'permute', 'right', N, ilo, ihi, WORK, strideWork, oLscale, WORK, strideWork, oRscale, N, VSR, strideVSR1, strideVSR2, offsetVSR );
	}
...
function finalize( info, sdim, ilascl, ilbscl, anrmto, anrm, bnrmto, bnrm, N, A, strideA1, strideA2, offsetA, B, strideB1, strideB2, offsetB, ALPHAR, strideALPHAR, offsetALPHAR, ALPHAI, strideALPHAI, offsetALPHAI, BETA, strideBETA, offsetBETA, wantst, selctg, safmax ) { // eslint-disable-line max-params
	let alphai, alphar, lastsl, lst2sl, cursl, beta, temp, ip, i;
