/**
* Property-based validation for zgetf2, following the /blahpack-validate process.
*
...
import zgetf2 from './../lib/ndarray.js';

const sc = S.complex; // z-routine
const LogicalMatrix = logical.LogicalMatrix;

// The pivot search izamax walks each sub-column with strideA1; its reference
...
function ipivRealize( n, layout ) {
	...
	const data = new Int32Array( lead + span + tail + 1 );
...
function readL( R, M, minMN ) {
	const Lm = new LogicalMatrix( sc, M, minMN );
	let i, j;
	for ( j = 0; j < minMN; j++ ) {
		for ( i = 0; i < M; i++ ) {
			if ( i > j ) {
...
function readU( R, minMN, N ) {
	const Um = new LogicalMatrix( sc, minMN, N );
	let i, j;
	for ( j = 0; j < N; j++ ) {
		for ( i = 0; i < minMN; i++ ) {
			if ( i <= j ) {
...
// below).
test( 'zgetf2: LU reconstruction P*A = L*U ((M,N) sweep x all layouts)', function t() {
	PAIRS.forEach( function eachPair( pr ) {
		...
		POSROW.forEach( function eachLayout( layout, li ) {
			...
			const ipiv = ipivRealize( minMN, { 'stride': 1, 'lead': 0, 'tail': 0 } );
			zgetf2( M, N, R.data, R.args[ 0 ], R.args[ 1 ], R.args[ 2 ], ipiv.data, ipiv.stride, ipiv.offset );

			const Lm = readL( R, M, minMN );
			const Um = readU( R, minMN, N );
			const recon = ref.matmul( sc, Lm, Um );
			const PA = applyPivots( A0, ipiv, minMN );
			checked( 'zgetf2', 'reconstruct', function run() {
				check.assertReconstruct( sc, recon, PA, { 'label': 'zgetf2 M='+M+' N='+N+' layout='+li } );
			});
		});
	});
});
...
test( 'zgetf2: bit-exact within storage-order family (col / row)', function t() {
...
function runInvariance( variants, fam ) {
	...
	const minMN = Math.min( M, N );
	const SEED = 0xF00D;
	checked( 'zgetf2', 'layout-invariance', function run() {
		layoutInvariant( variants, function build( layout, i ) {
			const rng = new RNG( SEED ); // identical values every variant
			const A0 = logical.general( sc, rng, M, N );
			const R = schemes.dense.realize( sc, A0, { 'part': 'full' }, layout );
			const ipiv = ipivRealize( minMN, IPIV_LAYOUTS[ i % IPIV_LAYOUTS.length ] );
			zgetf2( M, N, R.data, R.args[ 0 ], R.args[ 1 ], R.args[ 2 ], ipiv.data, ipiv.stride, ipiv.offset );
			return check.flattenLogical( sc, readFull( R, M, N ) );
		}, { 'label': 'zgetf2 layout invariance '+fam+'-major' } );
	});
}
