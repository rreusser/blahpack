...
*/
function zhegvx( itype, jobz, range, uplo, N, A, LDA, B, LDB, vl, vu, il, iu, abstol, out, w, strideW, Z, LDZ, WORK, strideWork, RWORK, strideRWork, IWORK, strideIWork, IFAIL, strideIFAIL ) { // eslint-disable-line max-len, max-params
	...
	if ( RWORK === null || RWORK === void 0 ) {
		const minRwork = Math.max( 1, 7 * N );
		RWORK = new Float64Array( minRwork );
		strideRWork = 1;
	}
	if ( IWORK === null || IWORK === void 0 ) {
		const minIwork = Math.max( 1, 5 * N );
	...
	const oifail = stride2offset( N, strideIFAIL );
	...
	return base( itype, jobz, range, uplo, N, A, sa1, sa2, 0, B, sb1, sb2, 0, vl, vu, il, iu, abstol, out, w, strideW, ow, Z, sz1, sz2, 0, WORK, strideWork, owork, RWORK, strideRWork, orwork, IWORK, strideIWork, oiwork, IFAIL, strideIFAIL, oifail ); // eslint-disable-line max-len
}
