...
function runReconstruct( uplo, n, layout ) {
	...
	var info = zsytrf( uplo, n, R.data, R.args[ 0 ], R.args[ 1 ], R.args[ 2 ], ipiv, 1, 0 );
	if ( info !== 0 ) {
...
function runInvariance( uplo, n ) {
	...
	var info = zsytrf( uplo, n, Rf.data, Rf.args[ 0 ], Rf.args[ 1 ], Rf.args[ 2 ], ipiv0, 1, 0 );
	if ( info !== 0 ) {
	...
	checked( 'zsytri', 'layout-invariance', function run() {
		layoutInvariant( PURE_ADDR, function build( layout ) {
			...
			var code = zsytri( uplo, n, R.data, R.args[ 0 ], R.args[ 1 ], R.args[ 2 ], ipiv, 1, 0, work, 1, 0 );
			if ( code !== 0 ) {
...
test( 'zsytri: workspace minimum (len N) suffices with a poisoned buffer', function t() {
	...
	checked( 'zsytri', 'workspace', function check_() {
		var minLen = assertWorkspaceSufficient( run, {}, 'zsytri WORK@N=33 lower' );
		if ( minLen !== N ) {
