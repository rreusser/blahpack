...
import zlasyfRk from './../lib/ndarray.js';


// FIXTURES //

const fixtureDir = path.join( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures' );
const rawLines = readFileSync( path.join( fixtureDir, 'zlasyf_rk.jsonl' ), 'utf8' ).trim().split( '\n' ); // eslint-disable-line node/no-sync, max-len
...
test( 'zlasyf_rk: lower_6x6_nb3', function t() {
	...
	const tc = findCase( 'lower_6x6_nb3' );
	const A = buildMatrix( 6, [
		[ 0, 0, 0.01, 0.01 ],
		[ 1, 0, 5.0, -1.0 ],
		[ 1, 1, 0.02, -0.02 ],
		[ 2, 0, 1.0, 1.0 ],
	...
	const e = new Complex128Array( 6 );
	const result = zlasyfRk( 'lower', 6, 3, A, 1, 6, 0, e, 1, 0, IPIV, 1, 0, W, 1, 6, 0 );
...
test( 'zlasyf_rk: upper_6x6_nb3', function t() {
	...
	const tc = findCase( 'upper_6x6_nb3' );
	const A = buildMatrix( 6, [
		[ 0, 0, 0.01, 0.01 ],
		[ 0, 1, 5.0, -1.0 ],
		[ 1, 1, 0.02, -0.02 ],
	...
	const W = new Complex128Array( 6 * 3 );
	const e = new Complex128Array( 6 );
	const result = zlasyfRk( 'upper', 6, 3, A, 1, 6, 0, e, 1, 0, IPIV, 1, 0, W, 1, 6, 0 );
	const Av = reinterpret( A, 0 );
	const Ev = reinterpret( e, 0 );
	assertArrayClose( Av, tc.A, 1e-12, 'A' );
	assertArrayClose( Ev, tc.e, 1e-12, 'e' );
...
test( 'zlasyf_rk: lower_4x4_full_nb8', function t() {
	...
	const tc = findCase( 'lower_4x4_full_nb8' );
	const A = buildMatrix( 4, [
		[ 0, 0, 4.0, 0.2 ],
		[ 1, 0, 1.0, 0.5 ],
		[ 1, 1, 3.0, -0.1 ],
		[ 2, 0, 2.0, -1.0 ],
	...
	const result = zlasyfRk( 'lower', 4, 8, A, 1, 4, 0, e, 1, 0, IPIV, 1, 0, W, 1, 4, 0 );
	const Av = reinterpret( A, 0 );
	const Ev = reinterpret( e, 0 );
	assertArrayClose( Av, tc.A, 1e-12, 'A' );
	assertArrayClose( Ev, tc.e, 1e-12, 'e' );
...
test( 'zlasyf_rk: upper_4x4_full_nb8', function t() {
	...
	const tc = findCase( 'upper_4x4_full_nb8' );
	const A = buildMatrix( 4, [
		[ 0, 0, 4.0, 0.2 ],
		[ 0, 1, 1.0, 0.5 ],
		[ 1, 1, 3.0, -0.1 ],
	...
	const W = new Complex128Array( 4 * 8 );
	const e = new Complex128Array( 4 );
	const result = zlasyfRk( 'upper', 4, 8, A, 1, 4, 0, e, 1, 0, IPIV, 1, 0, W, 1, 4, 0 );
	const Av = reinterpret( A, 0 );
	const Ev = reinterpret( e, 0 );
	assertArrayClose( Av, tc.A, 1e-12, 'A' );
	assertArrayClose( Ev, tc.e, 1e-12, 'e' );
