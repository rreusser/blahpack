...
import zgeqr2p from './../lib/ndarray.js';


// FIXTURES //

const fixtureFile = path.join( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures', 'zgeqr2p.jsonl' );
...
test( 'zgeqr2p: square 2x2 matrix', function t() {
	...
	const tc = findCase( 'square_2x2' );
	const a = new Complex128Array( [ 1, 1, 0, 1, 1, 0, 1, 1 ] );
	const tau = new Complex128Array( 2 );
	const work = new Complex128Array( 10 );
	const info = zgeqr2p( 2, 2, a, 1, 2, 0, tau, 1, 0, work, 1, 0 );
	assertClose( info, tc.info, 1e-14, 'info' );
	assertArrayClose( reinterpret( a, 0 ), tc.a, 1e-10, 'a' );
	assertArrayClose( reinterpret( tau, 0 ), tc.tau, 1e-10, 'tau' );
...
test( 'zgeqr2p: M=0 quick return', function t() {
	...
	const info = zgeqr2p( 0, 2, a, 1, 1, 0, tau, 1, 0, work, 1, 0 );
...
test( 'zgeqr2p: N=0 quick return', function t() {
	...
	const tau = new Complex128Array( 2 );
	const work = new Complex128Array( 2 );
	const info = zgeqr2p( 3, 0, a, 1, 3, 0, tau, 1, 0, work, 1, 0 );
	assertClose( info, 0, 1e-14, 'info' );
});
...
test( 'zgeqr2p: 1x1 matrix', function t() {
	...
	const work = new Complex128Array( 2 );
	const info = zgeqr2p( 1, 1, a, 1, 1, 0, tau, 1, 0, work, 1, 0 );
	assertClose( info, tc.info, 1e-14, 'info' );
	assertArrayClose( reinterpret( a, 0 ), tc.a, 1e-10, 'a' );
	assertArrayClose( reinterpret( tau, 0 ), tc.tau, 1e-10, 'tau' );
...
test( 'zgeqr2p: 4x3 tall matrix', function t() {
	...
	const a = new Complex128Array( [ 2, 1, 1, -1, 3, 0.5, 1, 0, 1, 0, 4, 2, 2, -0.5, 3, 1, 3, -1, 2, 1, 5, 0, 1, 0.5 ] ); // eslint-disable-line max-len
	const tau = new Complex128Array( 3 );
	const work = new Complex128Array( 10 );
	const info = zgeqr2p( 4, 3, a, 1, 4, 0, tau, 1, 0, work, 1, 0 );
	assertClose( info, tc.info, 1e-14, 'info' );
	assertArrayClose( reinterpret( a, 0 ), tc.a, 1e-10, 'a' );
	assertArrayClose( reinterpret( tau, 0 ), tc.tau, 1e-10, 'tau' );
...
test( 'zgeqr2p: 3x4 wide matrix', function t() {
	...
	const a = new Complex128Array( [ 1, 0.5, 2, -0.5, 3, 0.25, 4, 0, 5, 1, 6, -1, 7, 0.5, 8, 0.25, 9, -0.25, 1, 1, 2, 2, 3, 3 ] ); // eslint-disable-line max-len
	const tau = new Complex128Array( 3 );
	const work = new Complex128Array( 10 );
	const info = zgeqr2p( 3, 4, a, 1, 3, 0, tau, 1, 0, work, 1, 0 );
	assertClose( info, tc.info, 1e-14, 'info' );
	assertArrayClose( reinterpret( a, 0 ), tc.a, 1e-10, 'a' );
	assertArrayClose( reinterpret( tau, 0 ), tc.tau, 1e-10, 'tau' );
