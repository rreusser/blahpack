...
const rawLines = readFileSync( path.join( fixtureDir, 'dgemqrt.jsonl' ), 'utf8' ).trim().split( '\n' );
...
test( 'dgemqrt: left, no-transpose -> Q*I (small, NB=2, K=3)', function t() {
	...
	const info = dgemqrt( 'left', 'no-transpose', 4, 4, 3, 2, vt.V, 1, 4, 0, vt.T, 1, 2, 0, C, 1, 4, 0, WORK, 1, 0 );
...
test( 'dgemqrt: left, transpose -> Q^T*I (small)', function t() {
	...
	const C = eye( 4, 4, 4 );
	const WORK = new Float64Array( 4 * 2 );
	const info = dgemqrt( 'left', 'transpose', 4, 4, 3, 2, vt.V, 1, 4, 0, vt.T, 1, 2, 0, C, 1, 4, 0, WORK, 1, 0 );
	assert.equal( info, tc.info );
	assertArrayClose( toArray( C ), tc.c, 1e-14, 'c' );
});
...
test( 'dgemqrt: right, no-transpose, rectangular C(3,4) (small)', function t() {
	...
	const vt = smallVT();
	const C = rect34();
	const WORK = new Float64Array( 3 * 2 );
	const info = dgemqrt( 'right', 'no-transpose', 3, 4, 3, 2, vt.V, 1, 4, 0, vt.T, 1, 2, 0, C, 1, 3, 0, WORK, 1, 0 );
	assert.equal( info, tc.info );
	assertArrayClose( toArray( C ), tc.c, 1e-14, 'c' );
});
...
test( 'dgemqrt: right, transpose, rectangular C(3,4) (small)', function t() {
	...
	const C = rect34();
	const WORK = new Float64Array( 3 * 2 );
	const info = dgemqrt( 'right', 'transpose', 3, 4, 3, 2, vt.V, 1, 4, 0, vt.T, 1, 2, 0, C, 1, 3, 0, WORK, 1, 0 );
	assert.equal( info, tc.info );
	assertArrayClose( toArray( C ), tc.c, 1e-14, 'c' );
});
...
test( 'dgemqrt: M=0 quick return', function t() {
	...
	const info = dgemqrt( 'left', 'no-transpose', 0, 4, 0, 2, vt.V, 1, 4, 0, vt.T, 1, 2, 0, C, 1, 1, 0, WORK, 1, 0 );
	assert.equal( info, tc.info );
});
...
test( 'dgemqrt: N=0 quick return', function t() {
	...
	const C = new Float64Array( 1 );
	const WORK = new Float64Array( 1 );
	const info = dgemqrt( 'left', 'no-transpose', 4, 0, 0, 2, vt.V, 1, 4, 0, vt.T, 1, 2, 0, C, 1, 4, 0, WORK, 1, 0 );
	assert.equal( info, tc.info );
});
...
test( 'dgemqrt: K=0 quick return (C unchanged)', function t() {
	...
	const WORK = new Float64Array( 4 * 2 );
	const info = dgemqrt( 'left', 'no-transpose', 4, 4, 0, 2, vt.V, 1, 4, 0, vt.T, 1, 2, 0, C, 1, 4, 0, WORK, 1, 0 );
	assert.equal( info, tc.info );
...
test( 'dgemqrt: left, no-transpose (multi-block, M=10, K=8, NB=3)', function t() {
	...
	const info = dgemqrt( 'left', 'no-transpose', 10, 10, 8, 3, vt.V, 1, 10, 0, vt.T, 1, 3, 0, C, 1, 10, 0, WORK, 1, 0 );
	assert.equal( info, tc.info );
...
test( 'dgemqrt: left, transpose (multi-block)', function t() {
	...
	fillCos( C, 10, 10, 10 );
	const WORK = new Float64Array( 10 * 3 );
	const info = dgemqrt( 'left', 'transpose', 10, 10, 8, 3, vt.V, 1, 10, 0, vt.T, 1, 3, 0, C, 1, 10, 0, WORK, 1, 0 );
	assert.equal( info, tc.info );
	assertArrayClose( toArray( C ), tc.c, 1e-13, 'c' );
});
...
test( 'dgemqrt: right, no-transpose (multi-block)', function t() {
	...
	fillCos( C, 10, 10, 10 );
	const WORK = new Float64Array( 10 * 3 );
	const info = dgemqrt( 'right', 'no-transpose', 10, 10, 8, 3, vt.V, 1, 10, 0, vt.T, 1, 3, 0, C, 1, 10, 0, WORK, 1, 0 );
	assert.equal( info, tc.info );
	assertArrayClose( toArray( C ), tc.c, 1e-13, 'c' );
});
...
test( 'dgemqrt: right, transpose (multi-block)', function t() {
	...
	fillCos( C, 10, 10, 10 );
	const WORK = new Float64Array( 10 * 3 );
	const info = dgemqrt( 'right', 'transpose', 10, 10, 8, 3, vt.V, 1, 10, 0, vt.T, 1, 3, 0, C, 1, 10, 0, WORK, 1, 0 );
	assert.equal( info, tc.info );
	assertArrayClose( toArray( C ), tc.c, 1e-13, 'c' );
});
...
test( 'dgemqrt: left, no-transpose (single-block, NB == K)', function t() {
	...
	const info = dgemqrt( 'left', 'no-transpose', 5, 5, 4, 4, vt.V, 1, 5, 0, vt.T, 1, 4, 0, C, 1, 5, 0, WORK, 1, 0 );
	assert.equal( info, tc.info );
	assertArrayClose( toArray( C ), tc.c, 1e-13, 'c' );
});
...
test( 'dgemqrt: left, transpose (single-block)', function t() {
	...
	fillSin( C, 5, 5, 5 );
	const WORK = new Float64Array( 5 * 4 );
	const info = dgemqrt( 'left', 'transpose', 5, 5, 4, 4, vt.V, 1, 5, 0, vt.T, 1, 4, 0, C, 1, 5, 0, WORK, 1, 0 );
	assert.equal( info, tc.info );
	assertArrayClose( toArray( C ), tc.c, 1e-13, 'c' );
});
