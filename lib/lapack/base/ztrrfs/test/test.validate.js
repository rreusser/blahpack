/**
* Property-based validation for ztrrfs, following the /blahpack-validate process.
*
...
* properties. Unlike po/ge-rfs, the TRIANGULAR variant does NOT refine X: ztrrfs
...
* approximate X is produced by the trusted ztrtrs. Because no higher-precision
* oracle exists, the independent "true" solution is also ztrtrs on fresh copies
...
* A0 with op() selected by `trans` (N or conjugate-transpose):
...
* workspace is a real RWORK of N (not IWORK). Only the uplo triangle of A is
* realized (and, for a unit diagonal, the diagonal stays implicit); the opposite
* triangle stays poisoned, so a wrong-triangle / unit-diagonal read trips NaN.
*/
...
import ztrrfs from './../lib/ndarray.js';
import ztrtrs from '../../ztrtrs/lib/ndarray.js';
...
// trans flag -> reference transpose code (ztrrfs supports N and conjugate-T).
const TRANS = [
	[ 'no-transpose', 'n' ],
...
// Compute op(A0)*X = B0 with the trusted ztrtrs into fresh poisoned storage.
function solve( A0, B0, N, nrhs, uplo, trans, diag, unit ) {
	...
	const info = ztrtrs( uplo, trans, diag, N, nrhs, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], Xr.data, Xr.args[ 0 ], Xr.args[ 1 ], Xr.args[ 2 ] );
	if ( info !== 0 ) {
		throw new Error( 'oracle ztrtrs failed (info='+info+'); A singular?' );
	}
	return Xr;
}
...
// and nrhs.
test( 'ztrrfs: solve residual + BERR + FERR bound (uplo x trans x diag x N x nrhs)', function t() {
...
function runProperty( uplo, trans, code, diag, N, nrhs ) {
	...
	const seed = 0x100 + ( N * 40 ) + ( nrhs * 8 ) + ( uplo === 'upper' ? 0 : 4 ) + ( code === 'n' ? 0 : 2 ) + ( unit ? 1 : 0 );
	const rng = new RNG( seed ); // reproducible; log on failure
	...
	// X: the GIVEN approximate solution of op(A)*X = B via trusted ztrtrs:
	const Xsol = solve( A0, B0, N, nrhs, uplo, trans, diag, unit );
	...
	ztrrfs( uplo, trans, diag, N, nrhs, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], Br.data, Br.args[ 0 ], Br.args[ 1 ], Br.args[ 2 ], Xsol.data, Xsol.args[ 0 ], Xsol.args[ 1 ], Xsol.args[ 2 ], FERR, 1, 0, BERR, 1, 0, WORK, 1, 0, RWORK, 1, 0 );

	// Independent "true" solution (fresh copies; identical values -> actual err ~0):
	const Xtrue = solve( A0, B0, N, nrhs, uplo, trans, diag, unit );

	const tag = 'ztrrfs '+uplo+' '+trans+' '+diag+' N='+N+' nrhs='+nrhs;
	...
	checked( 'ztrrfs', 'residual', function run() {
		let j;
		for ( j = 0; j < nrhs; j++ ) {
	...
	checked( 'ztrrfs', 'structural', function run() {
...
// then re-realized per layout together with the fixed A0 and B0; ztrrfs's
// internal kernels (ztrmv, ztrsv, zaxpy, zlacn2) run over identical values, so
...
test( 'ztrrfs: bit-exact within storage-order family (col / row)', function t() {
	UPLOS.forEach( function eachUplo( uplo ) {
		TRANS.forEach( function eachTrans( tr ) {
			DIAGS.forEach( function eachDiag( diag ) {
...
function runInvariance( uplo, trans, diag, variants, fam ) {
	...
	const X0r = solve( A0, B0, N, nrhs, uplo, trans, diag, unit );
	const Xgiven = readMat( X0r, N, nrhs );

	checked( 'ztrrfs', 'layout-invariance', function run() {
		layoutInvariant( variants, function build( layout ) {
			...
			const Xr = schemes.dense.realize( sc, Xgiven, { 'part': 'full' }, layout );
			const FERR = new Float64Array( nrhs );
			const BERR = new Float64Array( nrhs );
			const WORK = new Complex128Array( 2 * N );
			const RWORK = new Float64Array( N );
			ztrrfs( uplo, trans, diag, N, nrhs, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], Br.data, Br.args[ 0 ], Br.args[ 1 ], Br.args[ 2 ], Xr.data, Xr.args[ 0 ], Xr.args[ 1 ], Xr.args[ 2 ], FERR, 1, 0, BERR, 1, 0, WORK, 1, 0, RWORK, 1, 0 );
			...
			return out;
		}, { 'label': 'ztrrfs '+uplo+' '+trans+' '+diag+' layout invariance '+fam+'-major' } );
	});
}
