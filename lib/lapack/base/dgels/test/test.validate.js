...
function runLDGuard( order, trans, M, N, nrhs ) {
	...
	assertLeadingDimGuard( function callLDB( ldb ) {
		var A = realizeLD( S.real, order, M, N, M, N, ldaReq, dVal( 0.0 ) );
		var B = realizeLD( S.real, order, p, nrhs, bRows, nrhs, ldb, dVal( 9.0 ) );
	...
	assertLeadingDimGuard( function callLDA( lda ) {
		var A = realizeLD( S.real, order, M, N, M, N, lda, dVal( 0.0 ) );
		var B = realizeLD( S.real, order, p, nrhs, bRows, nrhs, ldbReq, dVal( 9.0 ) );
...
function runWrapperMinLDB( sc, M, N, nrhs ) {
	...
	var av = dVal( 1.5 );
	var bv = dVal( 4.2 );
	var A = realizeLD( S.real, order, M, N, M, N, ldaReq, av );
	var B = realizeLD( S.real, order, M, nrhs, bRows, nrhs, ldbReq, bv );
...
function dgelsWorkRun( sc, trans, M, N, nrhs, seed ) {
	return function run( workLen ) {
		...
		var work = poisonedWork( S.real, workLen ); // NaN-filled: over-read OR read-before-write => NaN
		dgels( trans, M, N, nrhs, RA.data, RA.args[ 0 ], RA.args[ 1 ], RA.args[ 2 ], RB.data, RB.args[ 0 ], RB.args[ 1 ], RB.args[ 2 ], work, 1, 0 );
		var out = [];
		var i;
		var j;
