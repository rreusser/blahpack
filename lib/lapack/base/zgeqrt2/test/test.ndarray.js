...
const fixtureRaw = readFileSync( path.join( fixtureDir, 'zgeqrt2.jsonl' ), 'utf8' ); // eslint-disable-line node/no-sync
...
function runCase( caseName, M, N, rows, tol ) {
	...
	const info = zgeqrt2( M, N, A, 1, M, 0, T, 1, N, 0 );
...
test( 'zgeqrt2: n=0 quick return', function t() {
	...
	const info = zgeqrt2( 4, 0, A, 1, 4, 0, T, 1, 1, 0 );
...
test( 'zgeqrt2: m=0, n=0 quick return', function t() {
	const A = new Complex128Array( 0 );
	const T = new Complex128Array( 0 );
	const info = zgeqrt2( 0, 0, A, 1, 1, 0, T, 1, 1, 0 );
	assert.strictEqual( info, 0 );
});
...
test( 'zgeqrt2: works with non-trivial offsets and strides', function t() {
	...
	const tc = findCase( 'm4_n3_complex' );
	const LDA = 7;
	const LDT = 5;
	const offsetA = 4;
	const offsetT = 2;
	const A = new Complex128Array( ( LDA * 3 ) + offsetA );
	const T = new Complex128Array( ( LDT * 3 ) + offsetT );
	const Av = reinterpret( A, 0 );
	const Tv = reinterpret( T, 0 );
	const rows = [
		[ [ 2.0, 0.5 ], [ 1.0, -0.2 ], [ 3.0, 0.1 ] ],
	...
	const info = zgeqrt2( 4, 3, A, 1, LDA, offsetA, T, 1, LDT, offsetT );
	assert.strictEqual( info, 0 );

	// Pull out the 4x3 result region:
	const Aout = new Complex128Array( 12 );
	...
	const Tout = new Complex128Array( 9 );
...
test( 'zgeqrt2: works on a row-major matrix layout', function t() {
	...
	const info = zgeqrt2( M, N, A, N, 1, 0, T, N, 1, 0 );
	assert.strictEqual( info, 0 );

	// Convert column-major fixture A/T to row-major for comparison.
	const Aexp = new Float64Array( 2 * M * N );
	for ( j = 0; j < N; j++ ) {
		for ( i = 0; i < M; i++ ) {
	...
	const Texp = new Float64Array( 2 * N * N );
	for ( j = 0; j < N; j++ ) {
		for ( i = 0; i < N; i++ ) {
