...
/**
* Property-based validation for zunglq, following the /blahpack-validate process.
*
* Step 0 classification: `z` -> complex scalar; `unglq` FORMS the M-by-N unitary
* factor Q (orthonormal ROWS) from the elementary reflectors an LQ factorization
* (zgelqf) leaves in the first K rows of A. This is the BLOCKED former: it batches
* the reflectors through zlarft/zlarfb (an optimized zgemm) and falls back to the
* unblocked zungl2 for the trailing panel, producing EXACTLY the same Q. It
* CONSUMES a factorization, so it is validated as the second half of the
* composition zgelqf -> zunglq:
*
...
*   - zunglq overwrites A's M rows with Q (M-by-N);
...
// Generous WORK for the (blocked) zgelqf factor.
function gelqfWork( M ) {
	return Math.max( 1, ( M * NB ) + ( NB * NB ) );
}

// WORK the blocked zunglq consumes: zlarft stores its ib-by-ib T factor with
...
// SIZES, incl. the LARGE 48/63/64/65/100 that exercise zlarft/zlarfb) + wide
...
// Factor A0 with zgelqf at `layout`, snapshot L, then form Q in place with zunglq.
// When `poison`, zunglq's WORK is a poisoned buffer of exactly `owlen` elements.
function factorAndForm( M, N, layout, owlen, poison ) {
	...
	zunglq( M, N, K, R.data, R.args[ 0 ], R.args[ 1 ], R.args[ 2 ], TAU.data, TAU.args[ 0 ], TAU.args[ 1 ], OW, 1, 0 );
...
// (M,N) sweep (blocked + unblocked) and every dense storage layout. zunglq does
...
test( 'zunglq: orthonormal rows (Q·Qᴴ=I) and A=L*Q ((M,N) sweep x all layouts)', function t() {
...
// triggering zlarft/zlarfb). FREEZE the zgelqf factorization once, then re-realize
// the fixed reflectors + tau at each org-layout variant and run ONLY zunglq. Within
...
// the optimized zlarfb->zgemm accumulation (~1 ULP), so cross-order agreement is
...
// because base.js passes stride 1 (not strideWork) to zlarft/zlarfb, so a non-unit
...
function runInvariance( variants, fam, M, N, fr ) {
	...
	checked( ROUTINE, 'layout-invariance', function run() {
		layoutInvariant( variants, function build( layout, i ) {
			...
			zunglq( M, N, K, R.data, R.args[ 0 ], R.args[ 1 ], R.args[ 2 ], TAU.data, TAU.args[ 0 ], TAU.args[ 1 ], OW.data, OW.args[ 0 ], OW.args[ 1 ] );
			return check.flattenLogical( sc, readFull( R, M, N ) );
		}, { 'label': ROUTINE + ' layout invariance ' + fam + '-major ' + M + 'x' + N } );
	});
}

test( 'zunglq: bit-exact Q within storage-order family (col / row), blocked path', function t() {
...
test( 'zunglq: advertised WORK minimum suffices on the blocked path (poisoned)', function t() {
