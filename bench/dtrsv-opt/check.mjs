...
// sizes with all remainder classes. Block substitution reorders the
// accumulation, so the gate is the backward-error tier
// (docs/optimization-policy.md): elementwise comparison at a tolerance
// scaled to the substitution length. A is well conditioned (diagonal near
// one, off-diagonal scaled by 0.5/N) so solves stay bounded. The unread
// triangle — and the diagonal when diag='unit' — is filled with huge
// garbage so any read outside the stored triangle is caught.
...
function makeA( uplo, diag, N, sa1, sa2, oa, size ) {
	...
	for ( let i = 0; i < N; i++ ) {
		...
		for ( let j = j0; j < j1; j++ ) {
			if ( i === j ) {
				if ( diag === 'unit' ) continue; // must never be read
				A[ oa + ( i * sa1 ) + ( j * sa2 ) ] = 1.0 + ( 0.1 * Math.random() );
			} else {
				A[ oa + ( i * sa1 ) + ( j * sa2 ) ] = scale * rand();
			}
		}
	}
	return A;
