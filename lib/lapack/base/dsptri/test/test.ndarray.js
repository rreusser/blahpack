...
test( 'dsptri: 3x3_upper', function t() {
	...
	const result = factorAndInvert( 'upper', 3, ap );
...
test( 'dsptri: 3x3_lower', function t() {
	...
	const result = factorAndInvert( 'lower', 3, ap );
	assert.equal( result.info, tc.info );
	assertArrayClose( result.ap, tc.ap, 1e-14, 'ap' );
});
...
test( 'dsptri: 4x4_upper', function t() {
	...
	const result = factorAndInvert( 'upper', 4, ap );
	assert.equal( result.info, tc.info );
	assertArrayClose( result.ap, tc.ap, 1e-14, 'ap' );
});
...
test( 'dsptri: 4x4_lower', function t() {
	...
	const result = factorAndInvert( 'lower', 4, ap );
	assert.equal( result.info, tc.info );
	assertArrayClose( result.ap, tc.ap, 1e-14, 'ap' );
});
...
test( 'dsptri: n_zero', function t() {
	...
	const info = dsptri( 'upper', 0, ap, 1, 0, ipiv, 1, 0, work, 1, 0 );
...
test( 'dsptri: n_one', function t() {
	...
	const result = factorAndInvert( 'upper', 1, ap );
	assert.equal( result.info, tc.info );
	assertArrayClose( result.ap, tc.ap, 1e-14, 'ap' );
});
...
test( 'dsptri: 3x3_indef_upper', function t() {
	...
	const tc = indefUpper3x3;
	const ap = new Float64Array( [ 1.0, 2.0, 0.0, 3.0, 1.0, 1.0 ] );
	const result = factorAndInvert( 'upper', 3, ap );
	assert.equal( result.info, tc.info );
	assertArrayClose( result.ap, tc.ap, 1e-14, 'ap' );
});
...
test( 'dsptri: 3x3_indef_lower', function t() {
	...
	const tc = indefLower3x3;
	const ap = new Float64Array( [ 1.0, 2.0, 3.0, 0.0, 1.0, 1.0 ] );
	const result = factorAndInvert( 'lower', 3, ap );
	assert.equal( result.info, tc.info );
	assertArrayClose( result.ap, tc.ap, 1e-14, 'ap' );
});
