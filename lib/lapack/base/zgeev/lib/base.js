...
*/
function zgeev( jobvl, jobvr, N, A, strideA1, strideA2, offsetA, w, strideW, offsetW, VL, strideVL1, strideVL2, offsetVL, VR, strideVR1, strideVR2, offsetVR, WORK, strideWork, offsetWork, RWORK, strideRWork, offsetRWork ) {
	let tmp_mag, scalea, cscale, SELECT, tmp_re, tmp_im, info, side, nout, iwrk;
	let scl, sv1, sv2, vv, rv, oV, k, i;

	const wantvl = ( jobvl === 'compute-vectors' );
	const wantvr = ( jobvr === 'compute-vectors' );

	// Caller owns WORK; the effective workspace length is the number of complex
	// elements available from `offsetWork` (drives the `lwork - iwrk` arguments
	// passed to the block kernels). The ndarray/wrapper layers guarantee at
	// least `max( 1, 2*N )` elements.
	...
	// Handle N=1 case
	if ( N === 1 ) {
		const av = reinterpret( A, 0 ); // eslint-disable-line no-var
		const wv = reinterpret( w, 0 ); // eslint-disable-line no-var
		const oA1 = offsetA * 2; // eslint-disable-line no-var
		wv[ offsetW * 2 ] = av[ oA1 ];
		wv[ offsetW * 2 + 1 ] = av[ oA1 + 1 ];
		if ( wantvl ) {
			const vlv = reinterpret( VL, 0 ); // eslint-disable-line no-var
			vlv[ offsetVL * 2 ] = ONE;
			vlv[ offsetVL * 2 + 1 ] = ZERO;
		}
		if ( wantvr ) {
			const vrv = reinterpret( VR, 0 ); // eslint-disable-line no-var
			vrv[ offsetVR * 2 ] = ONE;
	...
	const anrm = zlange( 'max', N, N, A, strideA1, strideA2, offsetA, DUM, 1, 0 );
	scalea = false;
	cscale = ONE;
	...
	const balRes = zgebal( 'both', N, A, strideA1, strideA2, offsetA, RWORK, strideRWork, offsetRWork + ibal * strideRWork );
	...
	if ( wantvl || wantvr ) {
		...
		nout = 0;
		ztrevc3( side, 'backtransform', SELECT, 1, 0, N, A, strideA1, strideA2, offsetA, VL, strideVL1, strideVL2, offsetVL, VR, strideVR1, strideVR2, offsetVR, N, nout, WORK, strideWork, offsetWork + iwrk * strideWork, RWORK, strideRWork, offsetRWork + irwork * strideRWork );
	}
