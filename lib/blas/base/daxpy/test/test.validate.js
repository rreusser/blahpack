/**
* Property-based validation for daxpy, following the /blahpack-validate process.
*
...
import daxpy from './../lib/ndarray.js';
...
test( 'daxpy: y := alpha*x + y (size sweep, incl. N=0,1, alpha=0)', function t() {
	SIZES.forEach( function eachN( N ) {
		...
		daxpy( N, sc.apiScalar( alpha ), X.data, X.args[ 0 ], X.args[ 1 ], Y.data, Y.args[ 0 ], Y.args[ 1 ] );
		...
		checked( 'daxpy', 'property', function run() {
			check.assertFinite( sc, got, 'daxpy N='+N );
			var errSq = 0.0;
			var scaleSq = 0.0;
			var d;
			var c;
			...
			check.assertScaled( Math.sqrt( errSq ), Math.sqrt( scaleSq ), check.tol( N, 20 ), 'daxpy N='+N );
		});
	});
});
...
test( 'daxpy: bit-exact across vector layouts', function t() {
	var N = 17;
	var SEED = 0xF00D;
	var vLayouts = schemes.vectorLayouts();
	checked( 'daxpy', 'layout-invariance', function run() {
