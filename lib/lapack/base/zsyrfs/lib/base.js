...
import zsymv from '../../zsymv/lib/base.js';
...
* @param {Complex128Array} WORK - caller-owned complex workspace of at least `2*N` elements from `offsetWork` (partitioned into two contiguous N-length segments: residual/solution vector and the zlacn2 sign-vector); base.js never allocates
...
function zsyrfs( uplo, N, nrhs, A, strideA1, strideA2, offsetA, AF, strideAF1, strideAF2, offsetAF, IPIV, strideIPIV, offsetIPIV, B, strideB1, strideB2, offsetB, X, strideX1, strideX2, offsetX, FERR, strideFERR, offsetFERR, BERR, strideBERR, offsetBERR, WORK, strideWork, offsetWork, RWORK, strideRWork, offsetRWork ) { // eslint-disable-line no-unused-vars
	...
	// Partition the caller-owned complex WORK (>=2*N) into two contiguous
	// N-length segments: WRK is the residual/solution vector (WORK(0:N),
	// mirroring Fortran WORK(1:N)); V is the sign-vector workspace consumed by
	// zlacn2 (WORK(N:2N), mirroring Fortran WORK(N+1:2N)). RWK is the
	// caller-owned real RWORK (>=N). base.js never allocates problem-sized
	// workspace — the wrapper owns and sizes the buffers.
	WRK = ( offsetWork === 0 ) ? WORK : WORK.subarray( offsetWork );
	V = WORK.subarray( offsetWork + N );
	RWK = ( offsetRWork === 0 ) ? RWORK : RWORK.subarray( offsetRWork );
