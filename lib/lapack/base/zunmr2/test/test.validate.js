...
/**
* Property-based validation for zunmr2, following the /blahpack-validate process.
*
* Step 0 classification: `z` -> complex scalar; `unmr2` (UNBLOCKED apply-Q) ->
...
* (trans='no-transpose') or Qᴴ (trans='conjugate-transpose'). zunmr2 applies each
* reflector one at a time via zlarf (no blocking, no T-factor scratch — WORK=nw).
*
...
* stored ROW-wise — reflector i lives in row i, essential columns 0..NQ-K+i-1,
* implicit 1 at column NQ-K+i.
*
* Oracle (INDEPENDENT cross-validation, recorded as 'reconstruct' at L2): form
* the EXPLICIT order-NQ unitary Q from the SAME reflectors via `zungrq` (an
* independent code path). zungrq expects the K reflectors in the LAST K rows of an
* NQ-by-NQ matrix — exactly the K-by-NQ zgerqf panel dropped into the bottom K
* rows. Compute op(Q)*C0 / C0*op(Q) with the harness's naive `ref.matmul` and
* compare to zunmr2's in-place output.
*
* zunmr2's inner kernel is the optimized zgemv/zgerc (zlarf), which selects its
* summation form by operand strides: bit-exact layout invariance holds only
* WITHIN a storage-order family (col vs row); cross-order correctness is certified
* by the property swept over ALL layouts.
*/
...
// (M,N,K) triples with K <= min(M,N) (valid for BOTH sides). zunmr2 is unblocked.
const TRIPLES = [
...
// in the gerqf-factored K×NQ panel via the independent zungrq. The panel drops
...
function seedFor( M, N, K, side ) {
	return ( 0x500 + ( M * 1000 ) + ( N * 10 ) + K + ( side === 'left' ? 0 : 7 ) ) >>> 0;
}

// Drive one property case: factor a K×NQ panel, apply op(Q) to C0 with zunmr2,
...
function runCase( M, N, K, side, trans, tcode, layout ) {
	...
	zunmr2( side, trans, M, N, K, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], Tr.data, Tr.args[ 0 ], Tr.args[ 1 ], Cr.data, Cr.args[ 0 ], Cr.args[ 1 ], Cr.args[ 2 ], Wo, 1, 0 );
...
// Step 2/5: PROPERTY. op(Q) applied by zunmr2 == explicit-Q oracle, swept over
// side × trans × (M,N,K) × dense layouts (all layouts for small/medium dims,
// tight only for large).
test( 'zunmr2: op(Q)·C matches explicit-Q oracle (side × trans × dims × layouts)', function t() {
...
// Step 3: LAYOUT INVARIANCE. Freeze reflectors + tau + C0, re-realize per layout
// and run ONLY zunmr2; assert bit-exact output WITHIN a storage-order family
// (col / row). The optimized zgemv/zgerc inside zlarf reorders across the
...
test( 'zunmr2: bit-exact within storage-order family (col / row)', function t() {
	const M = 17;
	const N = 15;
	const K = 9;
	[ 'left', 'right' ].forEach( function eachSide( side ) {
			...
			[ [ colLayouts, 'col' ], [ rowLayouts, 'row' ] ].forEach( function eachFam( fam ) {
				checked( NAME, 'layout-invariance', function run() {
					layoutInvariant( fam[ 0 ], function build( layout, i ) {
						...
						const Cr = schemes.dense.realize( sc, C0, { 'part': 'full' }, layout );
						const Wo = poisonedWork( sc, ormWork( side, M, N ) );
						zunmr2( side, tc[ 0 ], M, N, K, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], Tr.data, Tr.args[ 0 ], Tr.args[ 1 ], Cr.data, Cr.args[ 0 ], Cr.args[ 1 ], Cr.args[ 2 ], Wo, 1, 0 );
...
// Step 4c: WORKSPACE CONFORMANCE. zunmr2 is unblocked (zlarf), so it consumes
// exactly nw = N (left) / M (right) WORK elements — no trailing T-factor segment.
// Verify the advertised minimum actually suffices under a poisoned buffer.
test( 'zunmr2: advertised WORK minimum suffices (unblocked, poisoned)', function t() {
	[ [ 'left', 20, 12, 8 ], [ 'right', 12, 20, 8 ] ].forEach( function eachCase( cfg ) {
		...
		function run( len ) {
			...
			zunmr2( side, trans, M, N, K, s.Ar.data, s.Ar.args[ 0 ], s.Ar.args[ 1 ], s.Ar.args[ 2 ], s.Tr.data, s.Tr.args[ 0 ], s.Tr.args[ 1 ], Cr.data, Cr.args[ 0 ], Cr.args[ 1 ], Cr.args[ 2 ], Wo, 1, 0 );
			return check.flattenLogical( sc, readFull( Cr, M, N ) );
		}
		...
		const s = setup();
		const Cr = schemes.dense.realize( sc, s.C0, { 'part': 'full' }, null );
		zunmr2( side, trans, M, N, K, s.Ar.data, s.Ar.args[ 0 ], s.Ar.args[ 1 ], s.Ar.args[ 2 ], s.Tr.data, s.Tr.args[ 0 ], s.Tr.args[ 1 ], Cr.data, Cr.args[ 0 ], Cr.args[ 1 ], Cr.args[ 2 ], poisonedWork( sc, minLen ), 1, 0 );
