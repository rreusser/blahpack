...
function runReconstruct( uplo, n, layout ) {
	...
	const factorWork = new Complex128Array( lwork );
	const work = poisonedWork( sc, Math.max( n, 1 ) ); // poisoned complex scratch (len N)

	let info = zhetrf( uplo, n, R.data, R.args[ 0 ], R.args[ 1 ], R.args[ 2 ], ipiv, 1, 0, factorWork, 1, 0, lwork );
	if ( info !== 0 ) {
	...
	const invA = readHermFull( R, n, uplo );
...
function runInvariance( uplo, n ) {
	...
	const lwork = n * NB;
	const factorWork = new Complex128Array( lwork );
	const info = zhetrf( uplo, n, Rf.data, Rf.args[ 0 ], Rf.args[ 1 ], Rf.args[ 2 ], ipiv0, 1, 0, factorWork, 1, 0, lwork );
	if ( info !== 0 ) {
	...
	checked( 'zhetri', 'layout-invariance', function run() {
		layoutInvariant( PURE_ADDR, function build( layout ) {
			...
			const code = zhetri( uplo, n, R.data, R.args[ 0 ], R.args[ 1 ], R.args[ 2 ], ipiv, 1, 0, work, 1, 0 );
			if ( code !== 0 ) {
...
test( 'zhetri: workspace minimum (len N) suffices with a poisoned buffer', function t() {
	...
	checked( 'zhetri', 'workspace', function check_() {
		const minLen = assertWorkspaceSufficient( run, {}, 'zhetri WORK@N=33 upper' );
		if ( minLen !== N ) {
