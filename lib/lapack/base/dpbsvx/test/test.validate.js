/**
* Property-based validation for dpbsvx, following the /blahpack-validate process.
*
...
* band matrix under conj=identity); `svx` (EXPERT Cholesky-solve DRIVER: optional
...
* proven for its constituent routines (dpbsv/dpbrfs residual, dpbcon rcond,
* dpbrfs FERR/BERR). We drive dpbsvx with fact='not-factored' (factor A in place,
* NO equilibration -> equed='none', so X solves the ORIGINAL A0*X=B0 directly)
* and assert, against the ORIGINAL full symmetric band A0 (both triangles
...
*       dpbsv solve of A0*X=I), and in (0,1].
...
* wrong triangle / out of band trips a NaN. NOTE base returns a plain integer
* `info` and writes `equed[0]` into the caller's array; the ndarray `rcond`
* out-array after offsetX is exercised directly.
*/
...
// 1-norm (max abs column sum) of the FULL symmetric band logical matrix
...
function invNorm1( A0, n, kd, uplo ) {
	...
	const info = dpbsv( uplo, n, kd, n, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], Br.data, Br.args[ 0 ], Br.args[ 1 ], Br.args[ 2 ] );
	if ( info !== 0 ) {
...
function trueSolution( A0, B0, N, kd, nrhs, uplo ) {
	...
	const info = dpbsv( uplo, N, kd, nrhs, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], Br.data, Br.args[ 0 ], Br.args[ 1 ], Br.args[ 2 ] );
	if ( info !== 0 ) {
		throw new Error( 'oracle dpbsv failed (info='+info+'); matrix not SPD?' );
	}
	return Br;
...
// Drive dpbsvx (fact='not-factored', no equilibration) and return the physical X
...
function drive( uplo, N, kd, nrhs, A0, B0, abLayout, bLayout ) {
	...
	const AFr = schemes.banded.realize( sc, A0, { 'part': uplo, 'k': kd }, abLayout ); // overwritten by dcopy+dpbtrf
	const Br = schemes.dense.realize( sc, B0, { 'part': 'full' }, bLayout );
	const Xr = schemes.dense.realize( sc, B0, { 'part': 'full' }, bLayout ); // overwritten by dlacpy+dpbtrs
	const equed = [ '' ];
	...
	const info = dpbsvx( 'not-factored', uplo, N, kd, nrhs, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], AFr.data, AFr.args[ 0 ], AFr.args[ 1 ], AFr.args[ 2 ], equed, s, 1, 0, Br.data, Br.args[ 0 ], Br.args[ 1 ], Br.args[ 2 ], Xr.data, Xr.args[ 0 ], Xr.args[ 1 ], Xr.args[ 2 ], rcond, FERR, 1, 0, BERR, 1, 0, WORK, 1, 0, IWORK, 1, 0 );
	return {
		'Xr': Xr,
		'rcond': rcond,
		'FERR': FERR,
...
// diagonal kd=0).
test( 'dpbsvx: expert-driver residual + rcond + FERR/BERR (uplo x N x kd x nrhs)', function t() {
...
function runProperty( uplo, N, kd, nrhs ) {
	...
	const A0 = logical.positiveDefiniteBanded( sc, rng, N, kd ); // full SPD band oracle
	const B0 = logical.general( sc, rng, N, nrhs );
	const anorm = norm1Full( A0, N );

	const d = drive( uplo, N, kd, nrhs, A0, B0, TIGHT_B, TIGHT_D );
	const tag = 'dpbsvx '+uplo+' N='+N+' kd='+kd+' nrhs='+nrhs;

	if ( d.info !== 0 ) {
		throw new Error( tag+': dpbsvx returned info='+d.info+' (expected 0 for well-conditioned SPD input)' );
	}
	if ( d.equed !== 'none' ) {
	...
	checked( 'dpbsvx', 'residual', function run() {
		let j;
		for ( j = 0; j < nrhs; j++ ) {
	...
	checked( 'dpbsvx', 'property', function run() {
	...
	checked( 'dpbsvx', 'structural', function run() {
...
// Step 4: layout-invariance fuzz. dpbsvx routes through dpbtrf (dscal/dsyr) +
// dpbtrs (dtbsv) + dpbcon (dlatbs/dlacn2) + dpbrfs (dsbmv/daxpy); none of these
// reorder their reductions with storage strides, so banded storage is a SINGLE
// addressing family (col AND row, incl negative strides — as dpbsv/dpbrfs already
// certify). Changing ONLY addressing must reproduce X ++ rcond ++ FERR ++ BERR
// bit-for-bit across ALL banded AB/AFB layouts and ALL dense B/X layouts.
const aLayouts = schemes.banded.layouts();
const bLayouts = schemes.dense.layouts();

test( 'dpbsvx: bit-exact across storage layouts (single family)', function t() {
	UPLOS.forEach( function eachUplo( uplo ) {
		runInvariance( uplo );
	});
});
...
function runInvariance( uplo ) {
	...
	const nrhs = 2;
	const SEED = 0xBEEF;
	checked( 'dpbsvx', 'layout-invariance', function run() {
		layoutInvariant( aLayouts, function build( aL, idx ) {
			...
			return out;
		}, { 'label': 'dpbsvx '+uplo+' layout invariance' } );
	});
}
