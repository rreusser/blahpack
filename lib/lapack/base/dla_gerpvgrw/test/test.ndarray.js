...
import dla_gerpvgrw from './../lib/ndarray.js';
...
test( 'dla_gerpvgrw: N=0 returns 1', function t() {
	...
	const r = dla_gerpvgrw( 0, 0, A, 1, 1, 0, AF, 1, 1, 0 );
	assert.strictEqual( r, 1.0 );
});

test( 'dla_gerpvgrw: ncols=0 returns 1', function t() {
	const A = new Float64Array( [ 1, 2, 3, 4 ] );
	const AF = new Float64Array( [ 1, 2, 3, 4 ] );
	const r = dla_gerpvgrw( 2, 0, A, 1, 2, 0, AF, 1, 2, 0 );
	assert.strictEqual( r, 1.0 );
});
...
test( 'dla_gerpvgrw: 1x1 unit ratio', function t() {
	...
	const AF = new Float64Array( [ 3 ] );
	const r = dla_gerpvgrw( 1, 1, A, 1, 1, 0, AF, 1, 1, 0 );
...
test( 'dla_gerpvgrw: 3x3 column-major basic case', function t() {
	...
	const N = 3;
	const A = new Float64Array( [ 2, 1, 1, 0, 3, 2, 0, 0, 4 ] ); // upper triangular, col-major
	const AF = new Float64Array( [ 2, 1, 1, 0, 3, 2, 0, 0, 4 ] );
	const r = dla_gerpvgrw( N, N, A, 1, N, 0, AF, 1, N, 0 );
	approxEqual( r, 1.0, 1e-12, 'rpvgrw' );
});
...
test( 'dla_gerpvgrw: 3x3 with growth in column 1', function t() {
	...
	const N = 3;
	const A = new Float64Array( [ 1, 0, 0, 5, 1, 0, 0, 0, 1 ] );
	const AF = new Float64Array( [ 1, 0, 0, 20, 1, 0, 0, 0, 1 ] );
	const r = dla_gerpvgrw( N, N, A, 1, N, 0, AF, 1, N, 0 );
...
test( 'dla_gerpvgrw: ncols < N (process subset of columns)', function t() {
	...
	const r = dla_gerpvgrw( N, 2, A, 1, N, 0, AF, 1, N, 0 );
	approxEqual( r, 1.0, 1e-12, 'rpvgrw' );
});
...
test( 'dla_gerpvgrw: large values -> ratio 0.5', function t() {
	...
	const N = 2;
	const A = new Float64Array( [ 1e10, 0, 0, 1 ] );
	const AF = new Float64Array( [ 2e10, 0, 0, 1 ] );
	const r = dla_gerpvgrw( N, 2, A, 1, N, 0, AF, 1, N, 0 );
	approxEqual( r, 0.5, 1e-12, 'rpvgrw' );
});
...
test( 'dla_gerpvgrw: zero U column in middle does not corrupt rpvgrw', function t() {
	const N = 3;
	const A = new Float64Array( [ 1, 0, 0, 4, 1, 0, 0, 0, 1 ] );

	// AF[*,1] entirely zero in upper triangle -> umax=0, branch skipped
	const AF = new Float64Array( [ 1, 0, 0, 0, 0, 0, 0, 0, 1 ] );
	const r = dla_gerpvgrw( N, N, A, 1, N, 0, AF, 1, N, 0 );
	approxEqual( r, 1.0, 1e-12, 'rpvgrw' );
});
...
test( 'dla_gerpvgrw: respects offsets and strides', function t() {
	...
	const N = 2;
	const A = new Float64Array( [ 1, 0, 0, 0, 0, 1, 0, 0 ] ); // col-major, LDA=4
	const AF = new Float64Array( [ 1, 0, 0, 0, 0, 1, 0, 0 ] );

	// strideA1=1, strideA2=4, offset=0
	const r = dla_gerpvgrw( N, N, A, 1, 4, 0, AF, 1, 4, 0 );
	approxEqual( r, 1.0, 1e-12, 'rpvgrw' );
});
...
test( 'dla_gerpvgrw: respects nonzero offsets', function t() {
	...
	const A = new Float64Array( [ 99, 99, 1, 0, 0, 1 ] );
	const AF = new Float64Array( [ 99, 99, 2, 0, 0, 4 ] );
	const r = dla_gerpvgrw( N, N, A, 1, N, 2, AF, 1, N, 2 );
...
test( 'dla_gerpvgrw: signs do not matter (uses abs)', function t() {
	const N = 2;
	const A = new Float64Array( [ -3, 0, 0, -2 ] );
	const AF = new Float64Array( [ -3, 0, 0, -2 ] );
	const r = dla_gerpvgrw( N, N, A, 1, N, 0, AF, 1, N, 0 );
	approxEqual( r, 1.0, 1e-12, 'rpvgrw' );
});
