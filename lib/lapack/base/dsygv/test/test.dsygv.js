...
test( 'dsygv has expected arity', function t() {
	assert.strictEqual( dsygv.length, 12, 'has expected arity' );
});

test( 'dsygv throws TypeError for invalid uplo', function t() {
	assert.throws( function throws() {
		dsygv( 2, 2, 'invalid', new Float64Array( 4 ), new Float64Array( 4 ), 2, new Float64Array( 4 ), 2, 2, 1, new Float64Array( 4 ), 1 );
	}, TypeError );
});

test( 'dsygv throws RangeError for negative N', function t() {
	assert.throws( function throws() {
		dsygv( 2, 2, 'upper', -1, new Float64Array( 4 ), 2, new Float64Array( 4 ), 2, 2, 1, new Float64Array( 4 ), 1 );
	}, RangeError );
});
