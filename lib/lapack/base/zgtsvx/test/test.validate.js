...
function zgtsvSolve( A0, N, k, code, Rhs ) {
	...
	const info = zgtsv( N, k, DL.data, DL.args[ 0 ], DL.args[ 1 ], D.data, D.args[ 0 ], D.args[ 1 ], DU.data, DU.args[ 0 ], DU.args[ 1 ], Br.data, Br.args[ 0 ], Br.args[ 1 ], Br.args[ 2 ] );
	if ( info !== 0 ) {
...
function opInverse( A0, N, code ) {
	const Ir = zgtsvSolve( A0, N, N, code, identity( N ) );
...
function callGtsvx( driver, fact, trans, N, nrhs, A0, B0, vlayout, dlayout ) {
	...
	const WORK = poisonedWork( sc, Math.max( 1, 2 * N ) ); // 2*N complex, poisoned
	const RWORK = new Float64Array( Math.max( 1, N ) );
	const info = driver( fact, trans, N, nrhs, DL.data, DL.args[ 0 ], DL.args[ 1 ], D.data, D.args[ 0 ], D.args[ 1 ], DU.data, DU.args[ 0 ], DU.args[ 1 ], DLF.data, DLF.args[ 0 ], DLF.args[ 1 ], DF.data, DF.args[ 0 ], DF.args[ 1 ], DUF.data, DUF.args[ 0 ], DUF.args[ 1 ], DU2.data, DU2.args[ 0 ], DU2.args[ 1 ], ipiv, 1, 0, Br.data, Br.args[ 0 ], Br.args[ 1 ], Br.args[ 2 ], Xr.data, Xr.args[ 0 ], Xr.args[ 1 ], Xr.args[ 2 ], rcond, FERR, 1, 0, BERR, 1, 0, WORK, 1, 0, RWORK, 1, 0 );
	return {
		'X': Xr,
		'rcond': rcond[ 0 ],
		'FERR': FERR,
...
function runProperty( trans, N, nrhs ) {
	...
	const out = callGtsvx( zgtsvx, 'not-factored', trans, N, nrhs, A0, B0, TIGHT, DTIGHT );
	const tag = 'zgtsvx '+trans+' N='+N+' nrhs='+nrhs;
	...
	checked( 'zgtsvx', 'structural', function run() {
		const Xtrue = zgtsvSolve( A0, N, nrhs, code, B0 );
...
test( 'zgtsvx: base.js and ndarray.js agree bit-exact on rcond (+ X/FERR/BERR)', function t() {
	TRANS.forEach( function eachTrans( trans ) {
		const N = 9;
		const nrhs = 2;
		const rng = new RNG( 0xA9 );
		const A0 = logical.tridiagonal( sc, rng, N );
		const B0 = logical.general( sc, rng, N, nrhs );
		const code = transCode( trans );

		checked( 'zgtsvx', 'exact', function run() {
			const a = callGtsvx( zgtsvxBase, 'not-factored', trans, N, nrhs, A0, B0, TIGHT, DTIGHT );
			const b = callGtsvx( zgtsvx, 'not-factored', trans, N, nrhs, A0, B0, TIGHT, DTIGHT );
			const fa = check.flattenLogical( sc, readMat( a.X, N, nrhs ) );
			const fb = check.flattenLogical( sc, readMat( b.X, N, nrhs ) );
			const tag = 'zgtsvx '+trans+' code='+code;
			let k;
...
function runInvariance( trans ) {
	...
	checked( 'zgtsvx', 'layout-invariance', function run() {
		layoutInvariant( VL, function build( vL, idx ) {
			...
			const out = callGtsvx( zgtsvx, 'not-factored', trans, N, nrhs, A0, B0, vL, dL );
