...
import illcond_5x5 from './fixtures/illcond_5x5.json' with { type: 'json' };
...
test( 'dporfs: basic_upper_3x3', function t() {
	...
	const info = dporfs( 'upper', 3, 1, sys.A, 1, 3, 0, sys.AF, 1, 3, 0, sys.B, 1, 3, 0, sys.X, 1, 3, 0, sys.FERR, 1, 0, sys.BERR, 1, 0, WORK, 1, 0, IWORK, 1, 0 );
...
test( 'dporfs: basic_lower_3x3', function t() {
	...
	const WORK = new Float64Array( 9 );
	const IWORK = new Int32Array( 3 );
	const info = dporfs( 'lower', 3, 1, sys.A, 1, 3, 0, sys.AF, 1, 3, 0, sys.B, 1, 3, 0, sys.X, 1, 3, 0, sys.FERR, 1, 0, sys.BERR, 1, 0, WORK, 1, 0, IWORK, 1, 0 );
	assert.equal( info, tc.info, 'info' );
	assertArrayClose( toArray( sys.X.subarray( 0, 3 ) ), tc.x, 1e-12, 'x' );
	assert.ok( sys.FERR[ 0 ] >= 0.0, 'FERR >= 0' );
	assert.ok( sys.BERR[ 0 ] >= 0.0, 'BERR >= 0' );
...
test( 'dporfs: multi_rhs_3x3', function t() {
	...
	const WORK = new Float64Array( 9 );
	const IWORK = new Int32Array( 3 );
	const info = dporfs( 'upper', 3, 2, sys.A, 1, 3, 0, sys.AF, 1, 3, 0, sys.B, 1, 3, 0, sys.X, 1, 3, 0, sys.FERR, 1, 0, sys.BERR, 1, 0, WORK, 1, 0, IWORK, 1, 0 );
	assert.equal( info, tc.info, 'info' );
...
test( 'dporfs: n_zero', function t() {
	...
	const info = dporfs( 'upper', 0, 1, A, 1, 1, 0, A, 1, 1, 0, A, 1, 1, 0, A, 1, 1, 0, FERR, 1, 0, BERR, 1, 0);
	assert.equal( info, tc.info, 'info' );
...
test( 'dporfs: nrhs_zero', function t() {
	...
	const FERR = new Float64Array( 1 );
	const BERR = new Float64Array( 1 );
	const info = dporfs( 'upper', 3, 0, A, 1, 3, 0, A, 1, 3, 0, A, 1, 3, 0, A, 1, 3, 0, FERR, 1, 0, BERR, 1, 0);
	assert.equal( info, tc.info, 'info' );
});

test( 'dporfs: illcond_5x5', function t() {

	const tc = illcond_5x5;
	const sys = setupSystem( 'upper', [
		100,
		10,
		1,
	...
	const info = dporfs( 'upper', 5, 1, sys.A, 1, 5, 0, sys.AF, 1, 5, 0, sys.B, 1, 5, 0, sys.X, 1, 5, 0, sys.FERR, 1, 0, sys.BERR, 1, 0, WORK, 1, 0, IWORK, 1, 0 );
	assert.equal( info, tc.info, 'info' );
...
test( 'dporfs: perturbed solution triggers iterative refinement', function t() {
	...
	X[ 1 ] -= 1e-8;
	X[ 2 ] += 1e-8;
	info = dporfs( 'upper', 3, 1, A, 1, 3, 0, AF, 1, 3, 0, B, 1, 3, 0, X, 1, 3, 0, FERR, 1, 0, BERR, 1, 0, WORK, 1, 0, IWORK, 1, 0 );
