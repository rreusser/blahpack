...
import dtpqrt from './../lib/ndarray.js';


// FIXTURES //

const fixtureDir = path.join( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures' );
const lines = readFileSync( path.join( fixtureDir, 'dtpqrt.jsonl' ), 'utf8' ).trim().split( '\n' );
...
test( 'm_zero: quick return', function t() {
	const info = dtpqrt( 0, 3, 0, 1, new Float64Array( 9 ), 1, 3, 0, new Float64Array( 0 ), 1, 1, 0, new Float64Array( 3 ), 1, 1, 0, new Float64Array( 3 ), 1, 0 );
	const tc = findCase( 'm_zero' );
	assert.strictEqual( info, tc.INFO, 'INFO' );
});

test( 'n_zero: quick return', function t() {
	const info = dtpqrt( 3, 0, 0, 2, new Float64Array( 0 ), 1, 1, 0, new Float64Array( 0 ), 1, 1, 0, new Float64Array( 0 ), 1, 1, 0, new Float64Array( 0 ), 1, 0 );
