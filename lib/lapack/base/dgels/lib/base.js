...
* @param {Float64Array} work - caller-provided workspace array of length at least `MN + max(MN,nrhs)*NB` (`MN = min(M,N)`, `NB = 32`), plus an extra `(NB+1)*NB` on the blocked path (`MN > NB`); WORK(0:MN-1) holds TAU and WORK(MN:) is factorization/application scratch
...
function dgels( trans, M, N, nrhs, A, strideA1, strideA2, offsetA, B, strideB1, strideB2, offsetB, work, strideWork, offsetWork ) {
	...
	const WORK = ( offsetWork === 0 ) ? work : work.subarray( offsetWork );
	...
	const anrm = dlange( 'max', M, N, A, strideA1, strideA2, offsetA, WORK, 1, MN );
	iascl = 0;
	...
	// Scale B
	const brow = ( tpsd ) ? N : M;
	const bnrm = dlange( 'max', brow, nrhs, B, strideB1, strideB2, offsetB, WORK, 1, MN );
	ibscl = 0;
