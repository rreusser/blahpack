...
const NB = 64; // Block size (hardcoded; Fortran uses ILAENV)
...
function dsysvx( fact, uplo, N, nrhs, A, strideA1, strideA2, offsetA, AF, strideAF1, strideAF2, offsetAF, IPIV, strideIPIV, offsetIPIV, B, strideB1, strideB2, offsetB, X, strideX1, strideX2, offsetX, rcond, FERR, strideFERR, offsetFERR, BERR, strideBERR, offsetBERR, WORK, strideWork, offsetWork, IWORK, strideIWork, offsetIWork ) { // eslint-disable-line max-len, max-params
	...
	// Compute optimal workspace size and report it in WORK[0] (overwritten by
	// the computation that follows on a real run; retained for parity with the
	// reference, which returns the optimal size to the caller).
	...
	const anorm = dlansy( 'inf-norm', uplo, N, A, strideA1, strideA2, offsetA, WORK, strideWork, offsetWork );
