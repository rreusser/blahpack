...
*
*   op(A)_X + X_op(B) = scale_C  (when isgn = +1)
_   op(A)_X - X_op(B) = scale_C  (when isgn = -1)
*
...
function ztrsyl( trana, tranb, isgn, M, N, A, strideA1, strideA2, offsetA, B, strideB1, strideB2, offsetB, C, strideC1, strideC2, offsetC, scale ) {
	let scaloc, info, suml, sumr, da11, db, j, k, l, vr, vi, ar, ai;

	const notrna = ( trana === 'no-transpose' );
	const notrnb = ( tranb === 'no-transpose' );
	...
	// Set constants
	const smlnum = SMLNUM * ( M * N ) / EPS;
	const bignum = ONE / smlnum;
	const smin = Math.max( smlnum, EPS * zlange( 'max', M, M, A, strideA1, strideA2, offsetA, DUM, 1, 0 ), EPS * zlange( 'max', N, N, B, strideB1, strideB2, offsetB, DUM, 1, 0 ) );
	const sgn = isgn;
