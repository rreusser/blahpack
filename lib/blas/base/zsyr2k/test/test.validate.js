/**
* Property-based validation for zsyr2k, following the /blahpack-validate process.
*
* Step 0 classification: `z` -> complex scalar; `sy` -> complex-symmetric result
* C (NO conjugation, one triangle referenced); `r2k` -> symmetric rank-2k update
*
...
*
* Note: zsyr2k uses the plain (non-conjugate) transpose even for complex data,
* and both alpha and beta are complex. Only the `uplo` triangle of C is
* referenced/updated; the other triangle stays poisoned. Validated as a residual
* against an independent naive-matmul oracle over uplo x trans x sizes x scalars,
* then fuzzed for bit-exact layout invariance (L3).
*/
...
import zsyr2k from './../lib/ndarray.js';

var sc = S.complex; // z-routine
var HER = false; // complex-symmetric (no conjugation)
...
test( 'zsyr2k: C = alpha*A*B^T + alpha*B*A^T + beta*C (uplo x trans x sizes x scalars)', function t() {
	[ 'upper', 'lower' ].forEach( function eachUplo( uplo ) {
		FLAGS.forEach( function eachFlag( flag ) {
			...
			PAIRS.forEach( function eachPair( pr ) {
				...
				var shp = ( knd === 'n' ) ? [ n, k ] : [ k, n ];
				var A = logical.general( sc, rng, shp[ 0 ], shp[ 1 ] );
				var B = logical.general( sc, rng, shp[ 0 ], shp[ 1 ] );
				var C0 = logical.symmetric( sc, rng, n );
				var P = products( knd, A, B );
				...
				COMBOS.forEach( function eachCombo( combo ) {
					...
					zsyr2k( uplo, fstr, n, k, sc.apiScalar( alpha ), RA.data, RA.args[ 0 ], RA.args[ 1 ], RA.args[ 2 ], RB.data, RB.args[ 0 ], RB.args[ 1 ], RB.args[ 2 ], sc.apiScalar( beta ), RC.data, RC.args[ 0 ], RC.args[ 1 ], RC.args[ 2 ] );

					var got = readTri( RC, n, uplo );
					var expected = expectedTri( n, uplo, P, C0, alpha, beta );
					var label = 'zsyr2k ' + uplo + ' ' + knd + ' n=' + n + ' k=' + k + ' ' + combo.join( '' );
					checked( 'zsyr2k', 'residual', function run() {
...
test( 'zsyr2k: bit-exact across storage layouts', function t() {
	var n = 6;
	var k = 4;
	var SEED = 0xF00D;
	...
	[ [ 'upper', 'no-transpose' ], [ 'lower', 'no-transpose' ], [ 'upper', 'transpose' ], [ 'lower', 'transpose' ] ].forEach( function eachCase( cs ) {
		var uplo = cs[ 0 ];
		var fstr = cs[ 1 ];
		var knd = ( fstr === 'no-transpose' ) ? 'n' : 't';
		checked( 'zsyr2k', 'layout-invariance', function run() {
			layoutInvariant( layouts, function build( layout, idx ) {
				...
				var RC = schemes.dense.realize( sc, C0, { 'part': uplo }, layouts[ ( idx + 2 ) % layouts.length ] );

				zsyr2k( uplo, fstr, n, k, sc.apiScalar( alpha ), RA.data, RA.args[ 0 ], RA.args[ 1 ], RA.args[ 2 ], RB.data, RB.args[ 0 ], RB.args[ 1 ], RB.args[ 2 ], sc.apiScalar( beta ), RC.data, RC.args[ 0 ], RC.args[ 1 ], RC.args[ 2 ] );

				return check.flattenLogical( sc, readTri( RC, n, uplo ) );
			}, { 'label': 'zsyr2k ' + uplo + ' ' + knd + ' layout invariance' } );
		});
	});
});
