...
/**
* Property-based validation for zunm2l, following the /blahpack-validate process.
*
...
* `ref.matmul` and compare to zunm2l's in-place output. Because zungql forms Q by
* an independent code path, a systematic reflector/side/trans error in zunm2l
...
*
* zunm2l is the UNBLOCKED (level-2) kernel: its inner dlarf calls the optimized
...
import zunm2l from './../lib/ndarray.js';
...
// zero/trivial corners; zunm2l is always unblocked so no NB threshold matters.
const TRIPLES = [
...
// Drive one property case: factor an R×K panel, apply op(Q) to C0 with zunm2l,
...
function runCase( M, N, K, side, trans, tcode, layout ) {
	...
	zunm2l( side, trans, M, N, K, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], Tr.data, Tr.args[ 0 ], Tr.args[ 1 ], Cr.data, Cr.args[ 0 ], Cr.args[ 1 ], Cr.args[ 2 ], poisonedWork( sc, ormWork( side, M, N ) ), 1, 0 );
...
// Step 2/5: PROPERTY. op(Q) applied by zunm2l == explicit-Q oracle, swept over
// side × trans × (M,N,K) × dense layouts.
test( 'zunm2l: op(Q)·C matches explicit-Q oracle (side × trans × dims × layouts)', function t() {
...
// and C per layout and run ONLY zunm2l; assert bit-exact output WITHIN a
...
test( 'zunm2l: bit-exact within storage-order family (col / row)', function t() {
	const M = 17;
	const N = 13;
			...
			[ [ colLayouts, 'col' ], [ rowLayouts, 'row' ] ].forEach( function eachFam( fam ) {
				checked( NAME, 'layout-invariance', function run() {
					layoutInvariant( fam[ 0 ], function build( layout, i ) {
						...
						const Cr = schemes.dense.realize( sc, C0, { 'part': 'full' }, layout );
						zunm2l( side, tc[ 0 ], M, N, K, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], Tr.data, Tr.args[ 0 ], Tr.args[ 1 ], Cr.data, Cr.args[ 0 ], Cr.args[ 1 ], Cr.args[ 2 ], poisonedWork( sc, ormWork( side, M, N ) ), 1, 0 );
