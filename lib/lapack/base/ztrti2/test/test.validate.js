/**
* Property-based validation for ztrti2, following the /blahpack-validate process.
*
...
import ztrti2 from './../lib/ndarray.js';
...
test( 'ztrti2: inverse reconstruction A*inv(A)=I (size sweep x uplo x diag x all layouts)', function t() {
		...
		DIAGS.forEach( function eachDiag( diag ) {
			...
			SIZES_SMALL.forEach( function eachN( n ) {
				schemes.dense.layouts().forEach( function eachLayout( layout ) {
					...
					ztrti2( uplo, diag, n, R.data, R.args[ 0 ], R.args[ 1 ], R.args[ 2 ] );
					const invA = readInv( R, n, uplo, unit, sc.one );
					const P = ref.matmul( sc, A0, invA );
					checked( 'ztrti2', 'reconstruct', function run() {
						assertInvResidual( A0, invA, P, n, 'ztrti2 '+uplo+' '+diag+' n='+n, 100 );
					});
				});
			});
		});
...
// layouts WITHIN a single storage-order family. ztrti2's inner loop calls the
// optimized ztrmv, which reorders its summation on a col<->row storage-order
// flip (~1 ULP, not a defect — see the zpotf2 LEARNINGS entry), so bit-equality
...
test( 'ztrti2: bit-exact within storage-order family (col / row)', function t() {
	UPLOS.forEach( function eachUplo( uplo ) {
		DIAGS.forEach( function eachDiag( diag ) {
...
function runInvariance( uplo, diag, variants, fam ) {
	...
	const unit = ( diag === 'unit' );
	const SEED = 0xF00D;
	checked( 'ztrti2', 'layout-invariance', function run() {
		...
		}, { 'label': 'ztrti2 '+uplo+' '+diag+' layout invariance '+fam+'-major' } );
	});
}
