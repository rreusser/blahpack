...
import dtrsyl from '../../dtrsyl/lib/base.js';
...
*/
function dtrsen( job, compq, SELECT, strideSELECT, offsetSELECT, N, T, strideT1, strideT2, offsetT, Q, strideQ1, strideQ2, offsetQ, WR, strideWR, offsetWR, WI, strideWI, offsetWI, M, s, sep, WORK, strideWork, offsetWork, IWORK, strideIWork, offsetIWork ) {
	let scalArr, liwmin, lwmin, rnorm, scale, isave, ierr, pair, swap, info;
	let kase, est, ks, kk, k;

	const wantbh = ( job === 'both' );
	const wants = ( job === 'eigenvalues' ) || wantbh;
	const wantsp = ( job === 'subspace' ) || wantbh;

	info = 0;

	// Caller owns the workspace; derive the available length so the retained
	// (never-taken) workspace-query path stays consistent.
	...
	for ( k = 0; k < N; k++ ) {
		if ( pair ) {
			...
			if ( swap ) {
				...
				if ( kk !== ks ) {
					const res = dtrexc( compq, N, T, strideT1, strideT2, offsetT, Q, strideQ1, strideQ2, offsetQ, kk, ks, WORK, strideWork, offsetWork );
					ierr = res.info;
				}
				if ( ierr === 1 || ierr === 2 ) {
					// Reordering failed
