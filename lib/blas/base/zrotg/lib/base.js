...
const SAFMIN = FLOAT64_SMALLEST_NORMAL; // 2^(-1022) ~ 2.225e-308
const SAFMAX = 1.0 / SAFMIN;           // 2^1022 ~ 4.494e+307
const RTMIN = Math.sqrt( SAFMIN );      // 2^(-511) ~ 1.491e-154
...
function zrotg( a, offsetA, b, offsetB, c, offsetC, s, offsetS ) {
	let rtmax, fsRe, fsIm, gsRe, gsIm, fRe, fIm, f2, g1, g2, h2, cc, d, u, v, w;
	...
	const f1 = Math.max( Math.abs( fRe ), Math.abs( fIm ) );
	g1 = Math.max( Math.abs( gRe ), Math.abs( gIm ) );
