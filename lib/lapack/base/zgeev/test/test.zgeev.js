...
test( 'zgeev has expected arity', function t() {
	assert.strictEqual( zgeev.length, 15, 'has expected arity' );
});

test( 'zgeev throws RangeError for negative N', function t() {
	assert.throws( function throws() {
		zgeev( 2, 2, -1, new Float64Array( 4 ), 2, 2, 1, new Float64Array( 4 ), 2, new Float64Array( 4 ), 2, new Float64Array( 4 ), 1, new Float64Array( 4 ), 1 );
	}, RangeError );
});
