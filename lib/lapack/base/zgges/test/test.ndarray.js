...
function runCase( tc, jobvsl, jobvsr ) {
	...
	const res = zgges( jobvsl, jobvsr, 'not-sorted', nosel, N, A, 1, Math.max( 1, N ), 0, B, 1, Math.max( 1, N ), 0, ALPHA, 1, 0, BETA, 1, 0, VSL, 1, Math.max( 1, N ), 0, VSR, 1, Math.max( 1, N ), 0 );
...
test( 'zgges: N=0 (degenerate)', function t() {
	...
	const res = zgges( 'no-vectors', 'no-vectors', 'not-sorted', nosel, 0, A, 1, 0, 0, B, 1, 0, 0, ALPHA, 1, 0, BETA, 1, 0, VSL, 1, 0, 0, VSR, 1, 0, 0 );
...
test( 'zgges: tiny inputs trigger A-scaling (ilascl)', function t() {
	...
	const res = zgges( 'compute-vectors', 'compute-vectors', 'not-sorted', nosel, N, A, 1, N, 0, B, 1, N, 0, ALPHA, 1, 0, BETA, 1, 0, VSL, 1, N, 0, VSR, 1, N, 0 );
	assert.equal( res.info, 0, 'info' );
});
...
test( 'zgges: sort=sorted with selctg (positive real part)', function t() {
	...
	const VSL = new Complex128Array( N * N );
	const VSR = new Complex128Array( N * N );
	const res = zgges( 'compute-vectors', 'compute-vectors', 'sorted', selPositiveReal, N, A, 1, N, 0, B, 1, N, 0, ALPHA, 1, 0, BETA, 1, 0, VSL, 1, N, 0, VSR, 1, N, 0 );
	assert.equal( res.info, 0, 'info' );
...
test( 'zgges: sort=sorted with selctg (none selected)', function t() {
	...
	const VSL = new Complex128Array( N * N );
	const VSR = new Complex128Array( N * N );
	const res = zgges( 'compute-vectors', 'compute-vectors', 'sorted', nosel, N, A, 1, N, 0, B, 1, N, 0, ALPHA, 1, 0, BETA, 1, 0, VSL, 1, N, 0, VSR, 1, N, 0 );
	assert.equal( res.info, 0, 'info' );
	assert.equal( res.sdim, 0, 'sdim' );
});
...
test( 'zgges: jobvsl=V jobvsr=N (left-only) basic', function t() {
	...
	const res = zgges( 'compute-vectors', 'no-vectors', 'not-sorted', nosel, N, A, 1, N, 0, B, 1, N, 0, ALPHA, 1, 0, BETA, 1, 0, VSL, 1, N, 0, VSR, 1, 1, 0 );
	assert.equal( res.info, 0, 'info' );
});
...
test( 'zgges: jobvsl=N jobvsr=V (right-only) basic', function t() {
	...
	const VSR = new Complex128Array( N * N );
	const res = zgges( 'no-vectors', 'compute-vectors', 'not-sorted', nosel, N, A, 1, N, 0, B, 1, N, 0, ALPHA, 1, 0, BETA, 1, 0, VSL, 1, 1, 0, VSR, 1, N, 0 );
	assert.equal( res.info, 0, 'info' );
});
