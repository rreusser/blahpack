...
const RE = S.real; // d, e, TAU real for a d-routine
const NAME = 'dormtr';
...
function formQ( Flog, tauVals, nq, uplo ) {
	...
	const Tr = schemes.realizeVector( RE, tauVals, TIGHT_VEC );
...
function reduce( uplo, nq, rng ) {
	const A0 = logical.symmetric( sc, rng, nq );
	...
	const tR = schemes.realizeVector( RE, poisonReal( Math.max( nq - 1, 0 ) ), TIGHT_VEC );
...
function runCase( M, N, side, uplo, trans, tcode, layout ) {
	...
	const Tr = schemes.realizeVector( RE, ( red.taus.length ? red.taus : poisonReal( 1 ) ), TIGHT_VEC );
...
test( 'dormtr: bit-exact within storage-order family (col / row)', function t() {
	const M = 48;
	const N = 40; // left: NQ=48 -> NQ-1=47 > NB; right: NQ=40 -> 39 > NB. blocked both sides.
	[ 'left', 'right' ].forEach( function eachSide( side ) {
		[ 'upper', 'lower' ].forEach( function eachUplo( uplo ) {
			[ [ 'no-transpose', 'n' ], [ TRANS_T, TCODE_T ] ].forEach( function eachTrans( tc ) {
				const nq = ( side === 'left' ) ? M : N;
				...
				[ [ colLayouts, 'col' ], [ rowLayouts, 'row' ] ].forEach( function eachFam( fam ) {
					checked( NAME, 'layout-invariance', function run() {
						layoutInvariant( fam[ 0 ], function build( layout, i ) {
							const Ar = schemes.dense.realize( sc, red.F, { 'part': uplo }, layout );
							const Tr = schemes.realizeVector( RE, red.taus, VEC_LAYOUTS[ i % VEC_LAYOUTS.length ] );
							const Cr = schemes.dense.realize( sc, C0, { 'part': 'full' }, layout );
							const Wo = poisonedWork( sc, ormWork( side, M, N ) );
...
test( 'dormtr: advertised WORK minimum suffices on the blocked path (Step 4c)', function t() {
	[ [ 'left', 80, 50 ], [ 'right', 50, 80 ] ].forEach( function eachCase( cfg ) {
		...
		const N = cfg[ 2 ];
		const uplo = 'lower';
		const nq = ( side === 'left' ) ? M : N; // 80 -> NQ-1=79 > NB -> blocked
		...
		function run( len ) {
			...
			const Tr = schemes.realizeVector( RE, red.taus, TIGHT_VEC );
