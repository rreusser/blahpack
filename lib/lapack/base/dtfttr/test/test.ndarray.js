...
function runFixtureTest( name, transr, uplo ) {
	...
	info = dtfttr( transr, uplo, n, arf, 1, 0, A, 1, n, 0 );
...
test( 'dtfttr: N=0 quick return', function t() {
	...
	info = dtfttr( 'no-transpose', 'lower', 0, arf, 1, 0, A, 1, 1, 0 );
...
test( 'dtfttr: N=1 quick return', function t() {
	...
	info = dtfttr( 'no-transpose', 'lower', 1, arf, 1, 0, A, 1, 1, 0 );
	assert.equal( info, 0, 'info' );
...
test( 'dtfttr: N=5, normal, lower, LDA=8', function t() {
	...
	info = dtfttr( 'no-transpose', 'lower', n, arf, 1, 0, A, 1, lda, 0 );
...
test( 'dtfttr: ARF offset', function t() {
	...
	A = new Float64Array( n * n );
	info = dtfttr( 'no-transpose', 'lower', n, arf, 1, 3, A, 1, n, 0 );
	assert.equal( info, 0, 'info' );
	expected = tc.A;
	for ( i = 0; i < expected.length; i += 1 ) {
...
test( 'dtfttr: A offset', function t() {
	...
	info = dtfttr( 'no-transpose', 'lower', n, arf, 1, 0, A, 1, n, 2 );
	assert.equal( info, 0, 'info' );
	expected = tc.A;
	for ( i = 0; i < expected.length; i += 1 ) {
