...
const TRANSRS = [ 'no-transpose', 'conjugate-transpose' ]; // RFP normal / conj-transposed layout
...
function invertRFP( A0, n, transr, uplo, unit, rfpLayout ) {
	...
	const info = ztftri( transr, uplo, diagStr( unit ), n, rfp.data, rfp.stride, rfp.offset );
	if ( info !== 0 ) {
		...
		UPLOS.forEach( function eachUplo( uplo ) {
			DIAGS.forEach( function eachDiag( diag ) {
				...
				SIZES.forEach( function eachN( n ) {
					...
					const label = 'ztftri '+transr+' '+uplo+' '+diag+' n='+n;
