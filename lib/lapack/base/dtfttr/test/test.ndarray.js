...
function runFixtureTest( name, transr, uplo ) {
	...
	const info = dtfttr( transr, uplo, n, arf, 1, 0, A, 1, n, 0 );

	assert.equal( info, tc.info, name + ' info' );

	const expected = tc.A;
	const actual = toArray( A );
...
test( 'dtfttr: N=0 quick return', function t() {
	...
	const info = dtfttr( 'no-transpose', 'lower', 0, arf, 1, 0, A, 1, 1, 0 );
...
test( 'dtfttr: N=1 quick return', function t() {
	...
	const info = dtfttr( 'no-transpose', 'lower', 1, arf, 1, 0, A, 1, 1, 0 );
	assert.equal( info, 0, 'info' );
...
test( 'dtfttr: N=5, normal, lower, LDA=8', function t() {
	...
	const info = dtfttr( 'no-transpose', 'lower', n, arf, 1, 0, A, 1, lda, 0 );
...
test( 'dtfttr: ARF offset', function t() {
	...
	const tc = n5NL;
	const n = tc.n;
	const arf = new Float64Array( [ 99.0, 88.0, 77.0 ].concat( toArray( tc.ARF ) ) );
	const A = new Float64Array( n * n );
	const info = dtfttr( 'no-transpose', 'lower', n, arf, 1, 3, A, 1, n, 0 );
	assert.equal( info, 0, 'info' );
	const expected = tc.A;
	for ( i = 0; i < expected.length; i += 1 ) {
...
test( 'dtfttr: A offset', function t() {
	...
	const info = dtfttr( 'no-transpose', 'lower', n, arf, 1, 0, A, 1, n, 2 );
	assert.equal( info, 0, 'info' );
	const expected = tc.A;
	for ( i = 0; i < expected.length; i += 1 ) {
