...
function runProperty( nm, N ) {
	var rng = new RNG( 0x5000 + N ); // reproducible
	...
	var D = schemes.realizeVector( sc, mainDiag( A0, N ), TIGHT_VEC );
	var DU = schemes.realizeVector( sc, superDiag( A0, N ), TIGHT_VEC );
	var DU2 = sc.alloc( Math.max( 1, N ) ); // second superdiagonal, length N-2
	...
	var info = dgtcon( nm.api, N, DL.data, DL.args[ 0 ], DL.args[ 1 ], D.data, D.args[ 0 ], D.args[ 1 ], DU.data, DU.args[ 0 ], DU.args[ 1 ], DU2, 1, 0, ipiv, 1, 0, anorm, rcond, WORK, 1, 0, IWORK, 1, 0 );
