...
function dormqr( side, trans, M, N, K, A, strideA1, strideA2, offsetA, TAU, strideTAU, offsetTAU, C, strideC1, strideC2, offsetC, WORK, strideWork, offsetWork ) {
	...
	// Caller owns the workspace. The unblocked path (NB >= K, NB=32) needs only
	// nw = max(1,N) elements for side='left' (max(1,M) for 'right'); the BLOCKED
	// path (NB < K) additionally stores the block reflector T in a separate
	// trailing segment, needing nw*NB + (NB+1)*NB. Advertising only the unblocked
	// nw would accept a buffer the blocked path over-reads → silent NaN.
	const nb = 32;
	const nw = ( side === 'left' ) ? Math.max( 1, N ) : Math.max( 1, M );
	const minWork = ( nb >= K ) ? nw : ( ( nw * nb ) + ( ( nb + 1 ) * nb ) );
