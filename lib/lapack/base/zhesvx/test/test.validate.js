...
function invNorm1( A0, n, uplo ) {
	...
	var info = zhesv( uplo, n, n, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], ipiv, 1, 0, Br.data, Br.args[ 0 ], Br.args[ 1 ], Br.args[ 2 ], work, 1, 0 );
	if ( info !== 0 ) {
...
function drive( uplo, N, nrhs, A0, B0, layout ) {
	...
	var AFr = schemes.dense.realize( sc, A0, { 'part': uplo }, layout ); // overwritten by zlacpy+zhetrf
	var Br = schemes.dense.realize( sc, B0, { 'part': 'full' }, layout );
	var Xr = schemes.dense.realize( sc, B0, { 'part': 'full' }, layout ); // overwritten by zlacpy+zhetrs
	var ipiv = new Int32Array( Math.max( N, 1 ) );
	...
	var info = zhesvx( 'not-factored', uplo, N, nrhs, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], AFr.data, AFr.args[ 0 ], AFr.args[ 1 ], AFr.args[ 2 ], ipiv, 1, 0, Br.data, Br.args[ 0 ], Br.args[ 1 ], Br.args[ 2 ], Xr.data, Xr.args[ 0 ], Xr.args[ 1 ], Xr.args[ 2 ], rcond, FERR, 1, 0, BERR, 1, 0, WORK, 1, 0, RWORK, 1, 0 );
	return {
		'Xr': Xr,
		'rcond': rcond,
		'FERR': FERR,
...
function runResidual( uplo, N, nrhs, layout ) {
	...
	var A0 = logical.hermitian( sc, rng, N ); // full complex Hermitian indefinite oracle
	var B0 = logical.general( sc, rng, N, nrhs );
	var d = drive( uplo, N, nrhs, A0, B0, layout );
	var tag = 'zhesvx '+uplo+' N='+N+' nrhs='+nrhs;
