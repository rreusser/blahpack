...
* @param {Complex128Array} WORK - caller-owned complex workspace of at least `max(1,2*N)` complex elements from `offsetWork` (used by zsycon; never allocated here)
* @param {integer} strideWork - stride for WORK (in complex elements)
* @param {NonNegativeInteger} offsetWork - offset for WORK (in complex elements)
* @param {Float64Array} RWORK - caller-owned real workspace of at least `max(1,N)` elements from `offsetRWork` (used by zlansy; never allocated here)
...
*/
function zsysvx( fact, uplo, N, nrhs, A, strideA1, strideA2, offsetA, AF, strideAF1, strideAF2, offsetAF, IPIV, strideIPIV, offsetIPIV, B, strideB1, strideB2, offsetB, X, strideX1, strideX2, offsetX, rcond, FERR, strideFERR, offsetFERR, BERR, strideBERR, offsetBERR, WORK, strideWork, offsetWork, RWORK, strideRWork, offsetRWork ) {
	let info;
	...
	// Compute infinity-norm of A (uses caller-owned real workspace RWORK; zlansy
	// runs before zsyrfs, so reusing RWORK here is safe)
	const anorm = zlansy( 'inf-norm', uplo, N, A, strideA1, strideA2, offsetA, RWORK, strideRWork, offsetRWork );
