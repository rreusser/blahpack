...
function zhbgv( order, jobz, uplo, N, ka, kb, AB, LDAB, BB, LDBB, w, strideW, Z, LDZ, WORK, strideWork, RWORK, strideRWork ) {
	let sbb1, sbb2, sab1, sab2, sz1, sz2;
	...
	if ( RWORK === null || RWORK === void 0 ) {
		const minRwork = Math.max( 1, 3 * N );
		RWORK = new Float64Array( minRwork );
		strideRWork = 1;
	}
	const owk = stride2offset( Math.max( 1, N ), strideWork );
	const ork = stride2offset( Math.max( 1, 3 * N ), strideRWork );
