...
function dtpmqrt( side, trans, M, N, K, l, nb, V, strideV1, strideV2, offsetV, T, strideT1, strideT2, offsetT, A, strideA1, strideA2, offsetA, B, strideB1, strideB2, offsetB, WORK, strideWork, offsetWork ) {
	...
	// from an out-of-bounds read. WORK is logically nb-by-N (`nb*N` doubles) for
	// side='left' or M-by-nb (`M*nb`) for side='right'. When M, N, or K is 0 the
	// routine is a quick return that references no workspace.
	if ( M > 0 && N > 0 && K > 0 ) {
		need = ( side === 'left' ) ? ( nb * N ) : ( M * nb );
