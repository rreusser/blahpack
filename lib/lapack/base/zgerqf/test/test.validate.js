...
/**
* Property-based validation for zgerqf, following the /blahpack-validate process.
*
...
*     (complex trait: conj is genuine), and applyHRight reads it directly as `v`.
...
function factor( M, N, layout, wlen, doPoison ) {
	...
	zgerqf( M, N, R.data, R.args[ 0 ], R.args[ 1 ], R.args[ 2 ], TAU.data, TAU.args[ 0 ], TAU.args[ 1 ], work, 1, 0 );
	const taus = [];
	let i;
	for ( i = 0; i < k; i++ ) {
...
test( 'zgerqf: RQ reconstruction A=R*Q and orthonormal Q ((M,N) sweep x all layouts)', function t() {
...
const WLAYOUTS = VLAYOUTS.filter( function unitStride( L ) {
...
test( 'zgerqf: bit-exact within storage-order family (col / row), blocked path', function t() {
...
function runInvariance( variants, fam, M, N ) {
	...
	checked( ROUTINE, 'layout-invariance', function run() {
		layoutInvariant( variants, function build( layout, i ) {
			...
			zgerqf( M, N, R.data, R.args[ 0 ], R.args[ 1 ], R.args[ 2 ], TAU.data, TAU.args[ 0 ], TAU.args[ 1 ], WORK.data, WORK.args[ 0 ], WORK.args[ 1 ] );
...
test( 'zgerqf: advertised WORK minimum suffices on the blocked path (poisoned)', function t() {
