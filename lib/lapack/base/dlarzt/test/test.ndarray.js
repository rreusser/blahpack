...
const fixtureData = readFileSync( path.join( fixtureDir, 'dlarzt.jsonl' ), 'utf8' ).trim().split( '\n' ); // eslint-disable-line node/no-sync, max-len
...
test( 'dlarzt: K=1, N=4 - single reflector', function t() {
	...
	const tc = findCase( 'k1_n4' );
	const V = new Float64Array( [ 1.0, 0.5, -0.3, 0.7 ] );
	const TAU = new Float64Array( [ 0.8 ] );
...
test( 'dlarzt: K=3, N=5 - multiple reflectors', function t() {
	...
	const TAU = new Float64Array( [ 0.5, 0.7, 0.9 ] );
...
test( 'dlarzt: K=2, N=3 - one zero tau', function t() {
	...
	const tc = findCase( 'k2_n3_zero_tau' );
	const K = 2;

	const V = new Float64Array( K * 3 );
	...
	const TAU = new Float64Array( [ 0.0, 0.6 ] );
	const T = new Float64Array( K * K );
...
test( 'dlarzt: K=1, N=1 - edge case single element', function t() {
	...
	const tc = findCase( 'k1_n1' );
	const V = new Float64Array( [ 1.0 ] );
	const TAU = new Float64Array( [ 0.3 ] );
	const T = new Float64Array( 1 );
...
test( 'dlarzt: K=2, N=4 - all zero taus', function t() {
	...
	const tc = findCase( 'k2_n4_all_zero_tau' );
	const K = 2;

	const V = new Float64Array( K * 4 );
	V[ 0 + (0 * K) ] = 1.0;
	V[ 0 + (1 * K) ] = 0.2;
	V[ 0 + (2 * K) ] = 0.3;
	V[ 0 + (3 * K) ] = 0.4;
...
test( 'dlarzt: non-unit strides for V', function t() {
	...
	const V = new Float64Array( [ 1.0, 99.0, 0.5, 99.0, -0.3, 99.0, 0.7, 99.0 ] );
	const TAU = new Float64Array( [ 0.8 ] );
	const T = new Float64Array( 1 );
...
test( 'dlarzt: non-unit stride for TAU', function t() {
	...
	// TAU with stride 2
	const TAU = new Float64Array( [ 0.8, 99.0 ] );
	const T = new Float64Array( 1 );
...
test( 'dlarzt: offset for V and TAU', function t() {
	...
	// V with offset=2
	const V = new Float64Array( [ 99.0, 99.0, 1.0, 0.5, -0.3, 0.7 ] );
	const TAU = new Float64Array( [ 99.0, 0.8 ] );
	const T = new Float64Array( 1 );
