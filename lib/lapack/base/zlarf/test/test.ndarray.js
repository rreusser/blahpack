...
import zlarf from './../lib/ndarray.js';


// FIXTURES //

import zlarfLeft3x2 from './fixtures/zlarf_left_3x2.json' with { type: 'json' };
import zlarfRight2x3 from './fixtures/zlarf_right_2x3.json' with { type: 'json' };
import zlarfTauZero from './fixtures/zlarf_tau_zero.json' with { type: 'json' };
import zlarfNZero from './fixtures/zlarf_n_zero.json' with { type: 'json' };
import zlarfLeft4x3 from './fixtures/zlarf_left_4x3.json' with { type: 'json' };
...
test( 'zlarf: left side, 3x2 matrix', function t() {
	const tc = zlarfLeft3x2;
	// v = [1+0i, 0.5+0.5i, -0.3+0.2i]
	const v = new Complex128Array( [ 1.0, 0.0, 0.5, 0.5, -0.3, 0.2 ] );
	const tau = new Complex128Array( [ 1.5, 0.3 ] );
...
test( 'zlarf: tau=0 (identity)', function t() {
	const tc = zlarfTauZero;
	const v = new Complex128Array( [ 1.0, 0.0, 0.5, 0.5 ] );
	const tau = new Complex128Array( [ 0.0, 0.0 ] );
	const C = new Complex128Array( [
		1.0, 0.0,  2.0, 1.0,
		3.0, 2.0,  4.0, 3.0
	]);
...
test( 'zlarf: n=0', function t() {
	...
	const tau = new Complex128Array( [ 1.0, 0.0 ] );
...
test( 'zlarf: v with trailing zeros (lastv reduced)', function t() {
	...
	const v = new Complex128Array( [ 1.0, 0.0, 0.5, 0.5, 0.0, 0.0 ] );
	const tau = new Complex128Array( [ 1.5, 0.3 ] );
	const C = new Complex128Array( [
		1.0, 0.0,  3.0, 1.0,  5.0, 2.0,
		2.0, 1.0,  4.0, -1.0, 6.0, 0.0
	]);
	let Cv = reinterpret( C, 0 );
	const origRow2re0 = Cv[ 4 ]; // row2, col0
	const origRow2im0 = Cv[ 5 ];
	const origRow2re1 = Cv[ 10 ]; // row2, col1
	const origRow2im1 = Cv[ 11 ];
	const work = new Complex128Array( 20 );
...
test( 'zlarf: negative strideV', function t() {
	...
	const vfwd = new Complex128Array( [ 1.0, 0.0, 0.5, 0.5, -0.3, 0.2 ] );
	const tau = new Complex128Array( [ 1.5, 0.3 ] );
	const C1 = new Complex128Array( [
		1.0, 0.0,  3.0, 1.0,  5.0, 2.0,
		2.0, 1.0,  4.0, -1.0, 6.0, 0.0
	]);
	const C2 = new Complex128Array( [
		1.0, 0.0,  3.0, 1.0,  5.0, 2.0,
		2.0, 1.0,  4.0, -1.0, 6.0, 0.0
	]);
	const work1 = new Complex128Array( 20 );
	const work2 = new Complex128Array( 20 );

	// Forward stride
	zlarf( 'left', 3, 2, vfwd, 1, 0, tau, 0, C1, 1, 3, 0, work1, 1, 0 );

	// Reversed: store v in reverse order, use strideV=-1, offsetV=2
	const vrev = new Complex128Array( [ -0.3, 0.2, 0.5, 0.5, 1.0, 0.0 ] );
	zlarf( 'left', 3, 2, vrev, -1, 2, tau, 0, C2, 1, 3, 0, work2, 1, 0 );

	// Should produce same result
	const C1v = reinterpret( C1, 0 );
	const C2v = reinterpret( C2, 0 );
	for ( let i = 0; i < C1v.length; i++ ) {
		assert.strictEqual( C2v[ i ], C1v[ i ], 'C[' + i + ']' );
	}
});

test( 'zlarf: left side, 4x3 matrix', function t() {
	const tc = zlarfLeft4x3;
	const v = new Complex128Array( [
		1.0, 0.0,  0.2, 0.3,  -0.5, 0.1,  0.4, -0.6
	]);
	const tau = new Complex128Array( [ 1.2, -0.4 ] );
	const C = new Complex128Array( [
