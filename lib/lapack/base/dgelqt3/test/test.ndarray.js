...
const raw = readFileSync( path.join( fixtureDir, 'dgelqt3.jsonl' ), 'utf8' ); // eslint-disable-line node/no-sync
...
function runCase( caseName, M, N, rows, tol ) {
	...
	const info = dgelqt3( M, N, A, 1, M, 0, T, 1, M, 0 );
...
test( 'dgelqt3: m=0 quick return', function t() {
	...
	const info = dgelqt3( 0, 0, A, 1, 1, 0, T, 1, 1, 0 );
...
test( 'dgelqt3: works with non-trivial offsets and strides', function t() {
	...
	const offsetA = 7;
	const offsetT = 3;
	const A = new Float64Array( ( LDA * 4 ) + offsetA );
	const T = new Float64Array( ( LDT * 2 ) + offsetT );
	const rows = [
		[ 2.0, 1.5, 0.5, -1.0 ],
		[ 0.7, 3.0, 1.1, 0.4 ]
	];
	...
	const info = dgelqt3( 2, 4, A, 1, LDA, offsetA, T, 1, LDT, offsetT );
	assert.strictEqual( info, 0 );

	// Pull out the 2x4 result region:
	const Aout = new Float64Array( 8 );
	for ( j = 0; j < 4; j++ ) {
		for ( i = 0; i < 2; i++ ) {
			Aout[ ( j * 2 ) + i ] = A[ offsetA + ( j * LDA ) + i ];
		}
	}
	const Tout = new Float64Array( 4 );
	for ( j = 0; j < 2; j++ ) {
		for ( i = 0; i < 2; i++ ) {
...
test( 'dgelqt3: works on a row-major matrix layout', function t() {
	...
	const info = dgelqt3( M, N, A, N, 1, 0, T, M, 1, 0 );
	assert.strictEqual( info, 0 );
	...
	const Texp = new Float64Array( M * M );
	for ( j = 0; j < M; j++ ) {
		for ( i = 0; i < M; i++ ) {
