...
import dlarfb_gett from './../lib/ndarray.js';


// FIXTURES //

const fixtureDir = path.join( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures' );
const lines = readFileSync( path.join( fixtureDir, 'dlarfb_gett.jsonl' ), 'utf8' ).trim().split( '\n' ); // eslint-disable-line node/no-sync
...
function buildCase1( storeV1 ) {
	...
	const LDT = 6;
	const LDW = 6;
	const A = new Float64Array( LDA * 4 );
	const B = new Float64Array( LDB * 4 );
	const T = new Float64Array( LDT * 2 );
	const W = new Float64Array( LDW * 4 );
...
function buildCase3( storeV1 ) {
	...
	const T = new Float64Array( LDT * 3 );
	const W = new Float64Array( LDW * 3 );
...
function buildCaseK1() {
	...
	const A = new Float64Array( LDA * 3 );
	const B = new Float64Array( LDB * 3 );
	const T = new Float64Array( LDT * 1 );
	const W = new Float64Array( LDW * 3 );
...
test( 'dlarfb_gett: basic_notident_k2_m3_n4', function t() {
	const inp = buildCase1( true );
	const tc = findCase( 'basic_notident_k2_m3_n4' );
...
test( 'dlarfb_gett: basic_ident_k2_m3_n4', function t() {
	const inp = buildCase1( false );
	const tc = findCase( 'basic_ident_k2_m3_n4' );
...
test( 'dlarfb_gett: n_eq_k_notident_k3_m4', function t() {
	const inp = buildCase3( true );
	const tc = findCase( 'n_eq_k_notident_k3_m4' );
...
test( 'dlarfb_gett: n_eq_k_ident_k3_m4', function t() {
	const inp = buildCase3( false );
	const tc = findCase( 'n_eq_k_ident_k3_m4' );
...
test( 'dlarfb_gett: m_zero_notident', function t() {
	const inp = buildCase1( true );
	const tc = findCase( 'm_zero_notident' );
	const B = new Float64Array( inp.LDB * 4 );
...
test( 'dlarfb_gett: k1_notident_m2_n3', function t() {
	const inp = buildCaseK1();
	const tc = findCase( 'k1_notident_m2_n3' );
...
test( 'dlarfb_gett: k1_ident_m2_n3', function t() {
	const inp = buildCaseK1();
	const tc = findCase( 'k1_ident_m2_n3' );
...
test( 'dlarfb_gett: quick return M=0/N=0/K=0/K>N', function t() {
	...
	const inp = buildCase1( true );
	const snap = new Float64Array( inp.A );
