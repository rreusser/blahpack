/**
* Property-based validation for ztbtrs, following the /blahpack-validate process.
*
...
* oracle, sweeping uplo x trans x diag x N x kd x nrhs, where trans includes the
* distinct complex transpose ('t') and conjugate-transpose ('c') forms. Uses the
* backward-error normalization `‖op(A)X - B‖ / (‖A‖_F‖X‖ + ‖B‖)`
* (check.assertResidual), robust for the possibly ill-conditioned unit-diagonal
* case.
*
* ztbtrs applies ztbsv per RHS column; the tbsv kernel walks a FIXED element order
* per (uplo, trans) case, so band-array and B strides change only addressing,
* never accumulation order — output is bit-exact across ALL banded layouts (single
* family) for AB and ALL dense layouts for B. Negative band strides (layouts 4-6)
* exercise the band-row-step class that bit dpbtf2 (see LEARNINGS.md, dpbtf2
* entry).
*/
...
import ztbtrs from './../lib/ndarray.js';
...
// trans flag -> reference transpose code ('t' and 'c' are distinct for complex).
const TRANS = [
	[ 'no-transpose', 'n' ],
	[ 'transpose', 't' ],
...
// the backward-error normalization keeps the unit-diagonal case honest. The
// oracle applies the SAME trans code, so 't' vs 'c' distinctly validate the
// complex transpose vs conjugate-transpose solves.
test( 'ztbtrs: triangular-banded solve residual (uplo x trans x diag x N x kd x nrhs sweep)', function t() {
				...
				NS.forEach( function eachN( N ) {
					bands( N ).forEach( function eachKd( kd ) {
						NRHS.forEach( function eachNrhs( nrhs ) {
							...
							ztbtrs( uplo, trans, diag, N, kd, nrhs, R.data, R.args[ 0 ], R.args[ 1 ], R.args[ 2 ], BR.data, BR.args[ 0 ], BR.args[ 1 ], BR.args[ 2 ] );
							const X = readSolution( BR, N, nrhs );
							checked( 'ztbtrs', 'residual', function run() {
								let j, i;
								for ( j = 0; j < nrhs; j++ ) {
									const xj = [];
									const bj = [];
									...
									check.assertResidual( sc, A0, xj, bj, { 'trans': code, 'factor': 100, 'label': 'ztbtrs '+uplo+' '+trans+' '+diag+' N='+N+' kd='+kd+' nrhs='+nrhs+' col='+j } );
								}
							});
						});
					});
...
// Step 4: layout-invariance fuzz. ztbtrs applies ztbsv per RHS column; the tbsv
...
test( 'ztbtrs: solution is bit-exact across storage layouts', function t() {
	const N = 11;
	const kd = 3;
	const nrhs = 2;
	const SEED = 0xF00D;
	...
	UPLO.forEach( function eachUplo( uplo ) {
		TRANS.forEach( function eachTrans( tr ) {
			...
			DIAG.forEach( function eachDiag( diag ) {
				const unit = ( diag === 'unit' );
				checked( 'ztbtrs', 'layout-invariance', function run() {
					...
					}, { 'label': 'ztbtrs '+uplo+' '+trans+' '+diag+' layout invariance' } );
				});
			});
		});
	});
