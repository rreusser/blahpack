...
function runReal( tc, ltran, N, Trows, xInit, keys ) {
	...
	var res = dlaqtr( ltran, true, N, T, 1, ldt, 0, b, 1, 0, 0.0, x, 1, 0, work, 1, 0 ); // eslint-disable-line max-len
	var i;
...
function runComplex( tc, ltran, N, Trows, bArr, w, xInit, keys ) {
	...
	var x = new Float64Array( xInit );
	var work = new Float64Array( Math.max( N, 1 ) );
	var res = dlaqtr( ltran, false, N, T, 1, ldt, 0, b, 1, 0, w, x, 1, 0, work, 1, 0 ); // eslint-disable-line max-len
	var i;
	assert.equal( res.info, tc.INFO, 'INFO' );
	assertClose( res.scale, tc.SCALE, 1e-13, 'SCALE' );
	for ( i = 0; i < keys.length; i++ ) {
...
test( 'dlaqtr: n0_quick', function t() {
	...
	var res = dlaqtr( false, true, 0, T, 1, 1, 0, b, 1, 0, 0.0, x, 1, 0, work, 1, 0 ); // eslint-disable-line max-len
