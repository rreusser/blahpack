...
*/
function zunmtr( side, uplo, trans, M, N, A, strideA1, strideA2, offsetA, TAU, strideTAU, offsetTAU, C, strideC1, strideC2, offsetC, WORK, strideWork, offsetWork ) { // eslint-disable-line max-len, max-params
	...
	// zunmtr forwards WORK straight to the BLOCKED zunmql/zunmqr on the (NQ-1)
	// reflectors (NQ = M for left, N for right), which store the block reflector T
	// in a trailing WORK segment and need `nw*NB + (NB+1)*NB` (NB=32, nw = N for
	// left, M for right) once `NQ-1 > NB`; the reference unblocked minimum `nw`
	// under-counts our non-adaptive blocked kernel. See test/harness/LEARNINGS.md
	// (2026-07-18 dorgtr/zungtr/dormtr/zunmtr entry).
	const NB = 32;
	const nw = ( side === 'left' ) ? Math.max( 1, N ) : Math.max( 1, M );
	const nq = ( side === 'left' ) ? M : N;
	const minWork = ( ( nq - 1 ) > NB ) ? ( ( nw * NB ) + ( ( NB + 1 ) * NB ) ) : nw;
	...
	return base( side, uplo, trans, M, N, A, strideA1, strideA2, offsetA, TAU, strideTAU, offsetTAU, C, strideC1, strideC2, offsetC, WORK, strideWork, offsetWork ); // eslint-disable-line max-len
}
