...
import zla_porfsx_extended from './../lib/zla_porfsx_extended.js';
...
function createBenchmark( N ) {
	let err_bnds_norm, err_bnds_comp, berr_out, AYB, RES, DY, YT, AF, Y0, A, B;
	let c, i;
...
function main() {
	...
	const min = 2; // matrix order 2
	const max = 6; // matrix order 64
	for ( i = min; i <= max; i++ ) {
		len = pow( 2, i );
