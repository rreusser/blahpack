...
import zlassq from './../../../../lapack/base/zlassq/lib/base.js';
import zlacn2 from './../../../../lapack/base/zlacn2/lib/base.js';
...
* -   WORK (complex) and IWORK (integer) are caller-owned. Required sizes:
*     WORK >= max( 1, 4*M*(N-M) + 1 ) complex elements; IWORK >=
*     max( 1, 2*M*(N-M), N+6 ) integer elements. The caller (wrapper/ndarray
*     layer) is responsible for allocation and size validation.
*
* @private
...
function ztgsen( ijob, wantq, wantz, SELECT, strideSELECT, offsetSELECT, N, A, strideA1, strideA2, offsetA, B, strideB1, strideB2, offsetB, ALPHA, strideALPHA, offsetALPHA, BETA, strideBETA, offsetBETA, Q, strideQ1, strideQ2, offsetQ, Z, strideZ1, strideZ2, offsetZ, M, pl, pr, DIF, strideDIF, offsetDIF, WORK, strideWork, offsetWork, IWORK, strideIWork, offsetIWork ) { // eslint-disable-line max-len, max-params, no-unused-vars
	let dscale, rdscal, result, SCALV, KASEV, ISAVE, DIFV, dsum, kase, ierr;
	let res, mn2, IWK, ijb, ia2, ib2, WK, n1, n2, ks, ia, ib, i, k;
	...
	const sq1 = strideQ1 * 2;
	const sq2 = strideQ2 * 2;
	...
	if ( wantp ) {
		...
		// Partition caller-owned workspace (contiguous, stride 1). WK holds C,F
		// plus ztgsyl scratch; IWK is ztgsyl's integer workspace.
		WK = WORK.subarray( offsetWork );
		IWK = IWORK.subarray( offsetIWork );
	...
	if ( wantd ) {
		...
		// Partition caller-owned workspace (contiguous, stride 1):
		WK = WORK.subarray( offsetWork );
		IWK = IWORK.subarray( offsetIWork );
		SCALV = new Float64Array( 1 );
		DIFV = new Float64Array( 1 );
...
function normalizeB( Av, Bv, Qv, ALPHAv, BETAv, sa1, sa2, sb1, sb2, sq1, sq2, strideALPHA, strideBETA, offsetA, offsetB, offsetQ, offsetALPHA, offsetBETA, N, safmin, wantq, A, B, Q, csA1, csA2, csB1, csB2, csQ1, csQ2 ) { // eslint-disable-line max-len, max-params, no-unused-vars
	let temp1, temp2, bkkR, bkkI, ia2, ib2, ia, ib, d, k;
