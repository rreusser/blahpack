...
*/
function dlarrj( N, d, strideD, E2, strideE2, ifirst, ilast, rtol, offset, w, strideW, WERR, strideWERR, WORK, IWORK, pivmin, spdiam ) {
	if ( WORK === null || WORK === void 0 ) {
		const minWork = Math.max( 1, 2 * N );
		WORK = new Float64Array( minWork );
	}
	if ( IWORK === null || IWORK === void 0 ) {
		const minIwork = Math.max( 1, 2 * N );
		IWORK = new Int32Array( minIwork );
	}
	const oWERR = stride2offset( N, strideWERR );
	const oE2 = stride2offset( N, strideE2 );
	const oD = stride2offset( N, strideD );
	const oW = stride2offset( N, strideW );
	if ( N < 0 ) {
		throw new RangeError( format( 'invalid argument. First argument must be a nonnegative integer. Value: `%d`.', N ) );
	}
	return base( N, d, strideD, oD, E2, strideE2, oE2, ifirst, ilast, rtol, offset, w, strideW, oW, WERR, strideWERR, oWERR, WORK, 1, 0, IWORK, 1, 0, pivmin, spdiam );
}
