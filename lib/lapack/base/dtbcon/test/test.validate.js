/**
* Property-based validation for dtbcon, following the /blahpack-validate process.
*
...
* (self-contained condition-number ESTIMATOR) -> PROPERTY: dtbcon returns
* `rcond ≈ 1/κ` with κ = ‖A‖·‖A⁻¹‖ in the chosen norm. dtbcon computes ‖A‖ itself
* (dlantb) and estimates ‖A⁻¹‖ (dlacn2 + dlatbs). We compute the TRUE value
...
* dtbtrs (independent of dtbcon's dlacn2/dlatbs estimator). Assert agreement
* within a modest factor F and rcond ∈ (0,1].
*
* NOTE: dtbcon's second workspace is an INTEGER IWORK (Int32Array, length N);
* WORK is a Float64Array of length 3N.
*/
...
import dtbcon from './../lib/ndarray.js';
import dtbtrs from '../../dtbtrs/lib/ndarray.js';
...
// INDEPENDENT ‖A⁻¹‖: solve A·X = I with dtbtrs (banded triangular solve) on a
...
// triangle, so B is a dense N x N identity that dtbtrs overwrites with A⁻¹.
function invNorm( A0, N, uplo, diag, kd, unit, which ) {
	...
	dtbtrs( uplo, 'no-transpose', diag, N, kd, N, Ab.data, Ab.args[ 0 ], Ab.args[ 1 ], Ab.args[ 2 ], B.data, B.args[ 0 ], B.args[ 1 ], B.args[ 2 ] );
	for ( j = 0; j < N; j++ ) {
		for ( i = 0; i < N; i++ ) {
...
test( 'dtbcon: rcond ≈ 1/κ vs independent truth (norm x uplo x diag x N x kd)', function t() {
...
function runProperty( nm, uplo, diag, N, kd ) {
	...
	const info = dtbcon( nm.api, uplo, diag, N, kd, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], RCOND, WORK, 1, 0, IWORK, 1, 0 );

	// Independent truth:
	const invnorm = invNorm( A0, N, uplo, diag, kd, unit, nm.which );
	const trueRcond = 1.0 / ( anorm * invnorm );

	const label = 'dtbcon ' + nm.api + ' ' + uplo + ' ' + diag + ' N=' + N + ' kd=' + kd;
	checked( 'dtbcon', 'property', function run() {
		if ( info !== 0 ) {
...
// Step 4: layout-invariance. dtbcon is self-contained (no pivot search); its
// inner dlantb/dlatbs/dlacn2 walk the band by value in a FIXED element order set
...
test( 'dtbcon: bit-exact across all storage layouts', function t() {
	NORMS.forEach( function eachNorm( nm ) {
		UPLOS.forEach( function eachUplo( uplo ) {
			DIAGS.forEach( function eachDiag( diag ) {
...
function runInvariance( nm, uplo, diag ) {
	...
	const unit = ( diag === 'unit' );
	const SEED = 0xF22E;
	checked( 'dtbcon', 'layout-invariance', function run() {
		layoutInvariant( schemes.banded.layouts(), function build( layout ) {
			...
			const WORK = new Float64Array( 3 * N );
			const IWORK = new Int32Array( N );
			dtbcon( nm.api, uplo, diag, N, kd, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], RCOND, WORK, 1, 0, IWORK, 1, 0 );
			return [ RCOND[ 0 ] ];
		}, { 'label': 'dtbcon ' + nm.api + ' ' + uplo + ' ' + diag + ' layout invariance' } );
	} );
}
