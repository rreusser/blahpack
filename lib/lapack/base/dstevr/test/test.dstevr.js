...
test( 'dstevr has expected arity', function t() {
	assert.strictEqual( dstevr.length, 23, 'has expected arity' );
});

test( 'dstevr throws RangeError for negative N', function t() {
	assert.throws( function throws() {
		dstevr( 2, 2, -1, 2, 1, 2, 1, 2, 2, 2, 2, 2, 2, 2, 1, new Float64Array( 4 ), 2, new Float64Array( 4 ), 1, new Float64Array( 4 ), 1, new Float64Array( 4 ), 1 );
	}, RangeError );
});
