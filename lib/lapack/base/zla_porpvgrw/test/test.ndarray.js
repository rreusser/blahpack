...
test( 'ndarray ncols=0 returns 1', function t() {
	const r = zla_porpvgrw.ndarray( 'upper', 0, new Complex128Array( 0 ), 1, 1, 0, new Complex128Array( 0 ), 1, 1, 0, new Float64Array( 0 ), 1, 0 );
	assert.strictEqual( r, 1.0 );
});

test( 'ndarray ncols=0 returns 1 (lower)', function t() {
	const r = zla_porpvgrw.ndarray( 'lower', 0, new Complex128Array( 0 ), 1, 1, 0, new Complex128Array( 0 ), 1, 1, 0, new Float64Array( 0 ), 1, 0 );
	assert.strictEqual( r, 1.0 );
});
...
test( 'ndarray upper 2x2 trivial (rpvgrw=1)', function t() {
	...
	// AF = same.
	const A = cArr( [ 1, 0, 0, 0, 0.5, 0, 1, 0 ] );
	const AF = cArr( [ 1, 0, 0, 0, 0.5, 0, 1, 0 ] );
	const WORK = new Float64Array( 4 );
	const r = zla_porpvgrw.ndarray( 'upper', 2, A, 1, 2, 0, AF, 1, 2, 0, WORK, 1, 0 );
...
test( 'ndarray lower 2x2 trivial (rpvgrw=1)', function t() {
	const A = cArr( [ 1, 0, 0.5, 0, 0, 0, 1, 0 ] );
	const AF = cArr( [ 1, 0, 0.5, 0, 0, 0, 1, 0 ] );
	const WORK = new Float64Array( 4 );
	const r = zla_porpvgrw.ndarray( 'lower', 2, A, 1, 2, 0, AF, 1, 2, 0, WORK, 1, 0 );
	approxEqual( r, 1.0, 1e-12, 'r' );
});
...
test( 'ndarray complex magnitudes use CABS1', function t() {
	...
	const r = zla_porpvgrw.ndarray( 'upper', 1, A, 1, 1, 0, AF, 1, 1, 0, WORK, 1, 0 );
	approxEqual( r, 1.0, 1e-12, 'r' );
...
test( 'ndarray umax=0 column is skipped', function t() {
	...
	const A = cArr( [ 0, 0, 0, 0, 0, 0, 1, 0 ] );
	const AF = cArr( [ 0, 0, 0, 0, 0, 0, 2, 0 ] );
	const WORK = new Float64Array( 4 );
	const r = zla_porpvgrw.ndarray( 'upper', 2, A, 1, 2, 0, AF, 1, 2, 0, WORK, 1, 0 );
...
test( 'ndarray lower 3x3 with strides', function t() {
	...
	const r = zla_porpvgrw.ndarray( 'lower', N, A, 1, N, 0, AF, 1, N, 0, WORK, 1, 0 );
	approxEqual( r, 1.0, 1e-12, 'r' );
});
