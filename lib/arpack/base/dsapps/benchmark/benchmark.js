...
function createBenchmark( len ) {
	...
	dg = uniform( kplusp, -5.0, 5.0, options );
	sd = uniform( kplusp, 0.1, 2.0, options );
	...
	*/
	function benchmark( b ) {
		...
		workd = new Float64Array( 2*len );
		b.tic();
		for ( i = 0; i < b.iterations; i++ ) {
			dsapps( len, kev, np, shift, 1, v, len, h, kplusp, resid, 1, q, kplusp, workd, 1 );
			if ( isnan( resid[ 0 ] ) ) {
