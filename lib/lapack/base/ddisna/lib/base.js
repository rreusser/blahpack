...
const OVERFLOW = dlamch( 'overflow' );
...
function ddisna( job, M, N, d, strideD, offsetD, SEP, strideSEP, offsetSEP ) { // eslint-disable-line max-len, max-params
	let thresh, newgap, oldgap, incr, decr, info, k, i;

	info = 0;
	const eigen = ( job === 'eigenvalues' );
	const left = ( job === 'left-vectors' );
	const right = ( job === 'right-vectors' );
	const sing = left || right;
	...
	const anorm = Math.max( Math.abs( d[ offsetD ] ), Math.abs( d[ offsetD + ((k - 1) * strideD) ] ) ); // eslint-disable-line max-len
	if ( anorm === 0.0 ) {
		thresh = EPS;
	} else {
