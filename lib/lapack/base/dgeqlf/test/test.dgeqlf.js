...
test( 'dgeqlf has expected arity', function t() {
	assert.strictEqual( dgeqlf.length, 8, 'has expected arity' );
});

test( 'dgeqlf throws TypeError for invalid order', function t() {
	assert.throws( function throws() {
		dgeqlf( 'invalid', 2, 2, new Float64Array( 4 ), 2, new Float64Array( 4 ), 2, new Float64Array( 4 ) );
	}, TypeError );
});

test( 'dgeqlf throws RangeError for negative M', function t() {
	assert.throws( function throws() {
		dgeqlf( 'row-major', -1, 2, new Float64Array( 4 ), 2, new Float64Array( 4 ), 2, new Float64Array( 4 ) );
	}, RangeError );
});

test( 'dgeqlf throws RangeError for negative N', function t() {
	assert.throws( function throws() {
		dgeqlf( 'row-major', 2, -1, new Float64Array( 4 ), 2, new Float64Array( 4 ), 2, new Float64Array( 4 ) );
	}, RangeError );
});

test( 'dgeqlf throws RangeError for row-major LDA < N', function t() {
	assert.throws( function throws() {
		dgeqlf( 'row-major', 3, 5, new Float64Array( 25 ), 3, new Float64Array( 3 ), 1, new Float64Array( 5 ) );
	}, RangeError );
});

test( 'dgeqlf throws RangeError for column-major LDA < M', function t() {
	assert.throws( function throws() {
		dgeqlf( 'column-major', 5, 3, new Float64Array( 25 ), 3, new Float64Array( 3 ), 1, new Float64Array( 5 ) );
	}, RangeError );
});
...
test( 'dgeqlf column-major 3x3 matches fixture', function t() {
	...
	const info = dgeqlf( 'column-major', 3, 3, A, 3, TAU, 1, WORK );
...
test( 'dgeqlf column-major 4x3 matches fixture', function t() {
	...
	const TAU = new Float64Array( 3 );
	const WORK = new Float64Array( 3 );
	const info = dgeqlf( 'column-major', 4, 3, A, 4, TAU, 1, WORK );
	assert.equal( info, tc.INFO );
	assertArrayClose( A, tc.A, 1e-13, 'A' );
	assertArrayClose( TAU, tc.TAU, 1e-13, 'TAU' );
});
...
test( 'dgeqlf row-major 3x3 matches transposed fixture', function t() {
	...
	const TAU = new Float64Array( 3 );
	const WORK = new Float64Array( 3 );
	const info = dgeqlf( 'row-major', M, N, A, N, TAU, 1, WORK );
	assert.equal( info, tc.INFO );
	for ( j = 0; j < N; j++ ) {
		for ( i = 0; i < M; i++ ) {
