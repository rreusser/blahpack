...
test( 'dspcon: N=0 quick return', function t() {
	...
	rcond[ 0 ] = 0.5;
	const info = dspcon( 'upper', 0, new Float64Array( 0 ), 1, 0, new Int32Array( 0 ), 1, 0, 1.0, rcond, new Float64Array( 0 ), 1, 0, new Int32Array( 0 ), 1, 0 );
...
test( 'dspcon: anorm=0 returns rcond=0 (upper)', function t() {
	const N = 3;
	const AP = new Float64Array( ( N * ( N + 1 ) ) / 2 );
	const IPIV = new Int32Array( N );
	const rcond = new Float64Array( 1 );
	rcond[ 0 ] = 0.5;
	const info = dspcon( 'upper', N, AP, 1, 0, IPIV, 1, 0, 0.0, rcond, new Float64Array( 2 * N ), 1, 0, new Int32Array( N ), 1, 0 );
	assert.strictEqual( info, 0 );
...
test( 'dspcon: identity 3x3 upper (rcond=1)', function t() {
	...
	const anorm = packedNorm1Upper( AP, N );
	const IPIV = new Int32Array( N );
	dsptrf( 'upper', N, AP, 1, 0, IPIV, 1, 0 );
	const rcond = new Float64Array( 1 );
	const info = dspcon( 'upper', N, AP, 1, 0, IPIV, 1, 0, anorm, rcond, new Float64Array( 2 * N ), 1, 0, new Int32Array( N ), 1, 0 );
	assert.strictEqual( info, 0 );
...
test( 'dspcon: identity 4x4 lower (rcond=1)', function t() {
	...
	const anorm = packedNorm1Lower( AP, N );
	const IPIV = new Int32Array( N );
	dsptrf( 'lower', N, AP, 1, 0, IPIV, 1, 0 );
	const rcond = new Float64Array( 1 );
	const info = dspcon( 'lower', N, AP, 1, 0, IPIV, 1, 0, anorm, rcond, new Float64Array( 2 * N ), 1, 0, new Int32Array( N ), 1, 0 );
	assert.strictEqual( info, 0 );
	assert.ok( close( rcond[ 0 ], 1.0, 1e-9 ) );
});
...
test( 'dspcon: singular matrix - zero diagonal in factored D (upper)', function t() {
	...
	const IPIV = new Int32Array( N ); // All zeros >= 0 means 1x1 pivots (kp=0 for all).
	const rcond = new Float64Array( 1 );
	rcond[ 0 ] = 0.5;
	const info = dspcon( 'upper', N, AP, 1, 0, IPIV, 1, 0, 1.0, rcond, new Float64Array( 2 * N ), 1, 0, new Int32Array( N ), 1, 0 );
	assert.strictEqual( info, 0 );
	assert.strictEqual( rcond[ 0 ], 0.0 );
});
...
test( 'dspcon: singular matrix - zero diagonal in factored D (lower)', function t() {
	...
	const rcond = new Float64Array( 1 );
	rcond[ 0 ] = 0.5;
	const info = dspcon( 'lower', N, AP, 1, 0, IPIV, 1, 0, 1.0, rcond, new Float64Array( 2 * N ), 1, 0, new Int32Array( N ), 1, 0 );
	assert.strictEqual( info, 0 );
	assert.strictEqual( rcond[ 0 ], 0.0 );
});
...
test( 'dspcon: N=1 simple', function t() {
	...
	dsptrf( 'upper', N, AP, 1, 0, IPIV, 1, 0 );
	const rcond = new Float64Array( 1 );
	const info = dspcon( 'upper', N, AP, 1, 0, IPIV, 1, 0, 5.0, rcond, new Float64Array( 2 ), 1, 0, new Int32Array( 1 ), 1, 0 );
	assert.strictEqual( info, 0 );
