...
* @param {Complex128Array} WORK - caller-owned workspace of at least `MN + max(MN,nrhs)*NB` complex elements (`MN = min(M,N)`, `NB = 32`), plus `(NB+1)*NB` more on the blocked path (`MN > NB`); base.js never allocates
...
*/
function zgels( trans, M, N, nrhs, A, strideA1, strideA2, offsetA, B, strideB1, strideB2, offsetB, WORK, strideWork, offsetWork ) { // eslint-disable-line max-len
	let scllen, iascl, ibscl, info, bi, i, j;

	const MN = Math.min( M, N );

	const tpsd = ( trans === 'conjugate-transpose' );
	...
	// Partition the caller-owned WORK: W[0:MN] holds TAU (the Householder scalars),
	// W[MN:] is factorization/application scratch. base.js never allocates — the
	// caller (wrapper/ndarray) owns and sizes the buffer (enables batch reuse).
	const W = ( offsetWork === 0 ) ? WORK : WORK.subarray( offsetWork );

	// Scale A if max element is outside [smlnum, bignum]
	const anrm = zlange( 'max', M, N, A, strideA1, strideA2, offsetA, null, 1, 0 );
	iascl = 0;
	...
	// Scale B
	const brow = ( tpsd ) ? N : M;
	const bnrm = zlange( 'max', brow, nrhs, B, strideB1, strideB2, offsetB, null, 1, 0 );
	ibscl = 0;
	...
	if ( M >= N ) {
		...
		zgeqrf( M, N, A, strideA1, strideA2, offsetA, W, 1, 0, W, 1, MN );

		if ( !tpsd ) {
			// Case 1: Least squares: minimize || B - A*X ||

			// B := Q^H * B
			zunmqr( 'left', 'conjugate-transpose', M, nrhs, N, A, strideA1, strideA2, offsetA, W, 1, 0, B, strideB1, strideB2, offsetB, W, 1, MN );
			...
			// B := Q * B
			zunmqr( 'left', 'no-transpose', M, nrhs, N, A, strideA1, strideA2, offsetA, W, 1, 0, B, strideB1, strideB2, offsetB, W, 1, MN );

			scllen = M;
		}
	} else {
		// M < N: LQ factorization of A
		zgelqf( M, N, A, strideA1, strideA2, offsetA, W, 1, 0, W, 1, MN );
		...
		if ( !tpsd ) {
			...
			// B := Q^H * B
			zunmlq( 'left', 'conjugate-transpose', N, nrhs, M, A, strideA1, strideA2, offsetA, W, 1, 0, B, strideB1, strideB2, offsetB, W, 1, MN );
			...
			// B := Q * B
			zunmlq( 'left', 'no-transpose', N, nrhs, M, A, strideA1, strideA2, offsetA, W, 1, 0, B, strideB1, strideB2, offsetB, W, 1, MN );
