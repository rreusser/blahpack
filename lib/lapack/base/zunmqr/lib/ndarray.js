...
function zunmqr( side, trans, M, N, K, A, strideA1, strideA2, offsetA, TAU, strideTAU, offsetTAU, C, strideC1, strideC2, offsetC, WORK, strideWork, offsetWork ) {
	...
	// from an out-of-bounds read. The unblocked path (NB >= K, NB=32) calls
	// zunm2r and needs only nw = max(1,N) for side='left' (max(1,M) for 'right')
	// elements; the blocked path additionally holds the block reflector T, for
	// nw*NB + (NB+1)*NB. K===0 is a base quick return that uses no workspace.
	if ( K > 0 ) {
		nb = 32;
		nw = ( side === 'left' ) ? Math.max( 1, N ) : Math.max( 1, M );
		need = ( nb >= K ) ? nw : ( ( nw * nb ) + ( ( nb + 1 ) * nb ) );
