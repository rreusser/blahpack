...
const raw = readFileSync( path.join( fixtureDir, 'zgeqrt3.jsonl' ), 'utf8' ); // eslint-disable-line node/no-sync
...
function runCase( caseName, M, N, rows, tol ) {
	...
	const info = zgeqrt3( M, N, A, 1, M, 0, T, 1, N, 0 );
...
test( 'zgeqrt3: n=0 quick return', function t() {
	...
	const info = zgeqrt3( 0, 0, A, 1, 1, 0, T, 1, 1, 0 );
...
test( 'zgeqrt3: works with non-trivial offsets and strides', function t() {
	...
	const tc = findCase( 'm4_n2' );
	const LDA = 7;
	const LDT = 4;
	const offsetA = 5;
	const offsetT = 3;
	const A = new Complex128Array( ( LDA * 2 ) + offsetA );
	...
	const info = zgeqrt3( 4, 2, A, 1, LDA, offsetA, T, 1, LDT, offsetT );
	assert.strictEqual( info, 0 );
...
test( 'zgeqrt3: works on a row-major matrix layout', function t() {
	...
	const info = zgeqrt3( M, N, A, N, 1, 0, T, N, 1, 0 );
	assert.strictEqual( info, 0 );
