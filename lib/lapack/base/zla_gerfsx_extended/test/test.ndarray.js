...
const lines = readFileSync( path.join( fixtureDir, 'zla_gerfsx_extended.jsonl' ), 'utf8' ).trim().split( '\n' ); // eslint-disable-line node/no-sync
const fixture = lines.map( parse );

// Matrix A used in the notrans and conjtrans cases (column-major):
const A_NOTRANS_DATA = [
	4.0,
	1.0,
	1.0,
	0.0,
...
function runCase( trans, colequ, n_norms, rcond, ithresh, ignore_cwise, Ypre ) {
	...
	const A = new Complex128Array( A_NOTRANS_DATA );
	const B = new Complex128Array( B_NOTRANS_DATA );
	const fs = factorAndSolve( 3, A, B, trans );
	...
	const Y_TAIL = new Complex128Array( 3 );
	const Y = Ypre || fs.Y;
	const info = zla_gerfsx_extended( 2, trans, 3, 1, A, 1, 3, 0, fs.AF, 1, 3, 0, fs.IPIV, 1, 0, colequ, C, 1, 0, B, 1, 3, 0, Y, 1, 3, 0, BERR_OUT, 1, 0, n_norms, ERRS_N, 1, 1, 0, ERRS_C, 1, 1, 0, RES, 1, 0, AYB, 1, 0, DY, 1, 0, Y_TAIL, 1, 0, rcond, ithresh, 0.5, 0.25, ignore_cwise );
	return {
		'Y': Y,
		'BERR_OUT': BERR_OUT,
		'ERRS_N': ERRS_N,
...
test( 'zla_gerfsx_extended: notrans_basic', function t() {
	const tc = findCase( 'notrans_basic' );
	const r = runCase( 'no-transpose', true, 2, 1e-3, 10, false, null );
...
test( 'zla_gerfsx_extended: conjtrans_basic', function t() {
	const tc = findCase( 'conjtrans_basic' );
	const r = runCase( 'conjugate-transpose', false, 2, 1e-3, 10, false, null );
	assert.equal( r.info, 0 );
	assertArrayClose( toArr( r.Y ), tc.Y, 1e-13, 'Y' );
	assertArrayClose( r.BERR_OUT, tc.BERR_OUT, 1e-12, 'BERR_OUT' );
	assertArrayClose( r.ERRS_N, tc.ERRS_N, 1e-12, 'ERRS_N' );
...
test( 'zla_gerfsx_extended: no_equ (2x2, N_NORMS=1)', function t() {
	...
	const tc = findCase( 'no_equ' );
	const fs = factorAndSolve( 2, A, B, 'no-transpose' );
	const C = new Float64Array( [ 1.0, 1.0 ] );
	const BERR_OUT = new Float64Array( 1 );
	const ERRS_N = new Float64Array( 3 );
	const ERRS_C = new Float64Array( 3 );
	const RES = new Complex128Array( 2 );
	const AYB = new Float64Array( 2 );
	const DY = new Complex128Array( 2 );
	const Y_TAIL = new Complex128Array( 2 );
	const info = zla_gerfsx_extended( 2, 'no-transpose', 2, 1, A, 1, 2, 0, fs.AF, 1, 2, 0, fs.IPIV, 1, 0, false, C, 1, 0, B, 1, 2, 0, fs.Y, 1, 2, 0, BERR_OUT, 1, 0, 1, ERRS_N, 1, 1, 0, ERRS_C, 1, 1, 0, RES, 1, 0, AYB, 1, 0, DY, 1, 0, Y_TAIL, 1, 0, 1e-2, 10, 0.5, 0.25, false );
	assert.equal( info, 0 );
...
test( 'zla_gerfsx_extended: n_zero (quick return)', function t() {
	const BERR_OUT = new Float64Array( [ 42.0 ] );
	const ERRS_N = new Float64Array( [ 99.0, 99.0, 99.0 ] );
	const ERRS_C = new Float64Array( [ 88.0, 88.0, 88.0 ] );
	const info = zla_gerfsx_extended( 2, 'no-transpose', 0, 0, new Complex128Array( 1 ), 1, 1, 0, new Complex128Array( 1 ), 1, 1, 0, new Int32Array( 1 ), 1, 0, false, new Float64Array( 1 ), 1, 0, new Complex128Array( 1 ), 1, 1, 0, new Complex128Array( 1 ), 1, 1, 0, BERR_OUT, 1, 0, 2, ERRS_N, 1, 1, 0, ERRS_C, 1, 1, 0, new Complex128Array( 1 ), 1, 0, new Float64Array( 1 ), 1, 0, new Complex128Array( 1 ), 1, 0, new Complex128Array( 1 ), 1, 0, 1e-3, 10, 0.5, 0.25, false );
	assert.equal( info, 0 );
...
test( 'zla_gerfsx_extended: ignore_cwise', function t() {
	const tc = findCase( 'ignore_cwise' );
	const r = runCase( 'no-transpose', true, 1, 1e-3, 10, true, null );
	assert.equal( r.info, 0 );
	assertArrayClose( toArr( r.Y ), tc.Y, 1e-13, 'Y' );
	assertArrayClose( r.BERR_OUT, tc.BERR_OUT, 1e-12, 'BERR_OUT' );
	assertArrayClose( r.ERRS_N, tc.ERRS_N, 1e-12, 'ERRS_N' );
...
test( 'zla_gerfsx_extended: rcond=0 triggers precision promotion', function t() {
	const r = runCase( 'no-transpose', true, 2, 0.0, 10, false, null );
	assert.equal( r.info, 0 );
});

test( 'zla_gerfsx_extended: ithresh=1 single-iteration', function t() {
	const r = runCase( 'no-transpose', true, 2, 1e-3, 1, false, null );
	assert.equal( r.info, 0 );
});
...
test( 'zla_gerfsx_extended: yk=0 with nonzero dyk', function t() {
	...
	const A = new Complex128Array( A_NOTRANS_DATA );
	const B = new Complex128Array( B_NOTRANS_DATA );
	const fs = factorAndSolve( 3, A, B, 'no-transpose' );
	const fv = reinterpret( fs.Y, 0 );
	fv[ 0 ] = 0.0;
	fv[ 1 ] = 0.0;
	const r = runCase( 'no-transpose', true, 2, 1e-3, 10, false, fs.Y );
	assert.equal( r.info, 0 );
});

test( 'zla_gerfsx_extended: transpose trans_type', function t() {
	const r = runCase( 'transpose', true, 2, 1e-3, 10, false, null );
	assert.equal( r.info, 0 );
});
...
test( 'zla_gerfsx_extended: rcond=0 ithresh=100 forces EXTRA_Y path', function t() {
	...
	const r = runCase( 'no-transpose', true, 2, 0.0, 100, false, null );
	assert.equal( r.info, 0 );
});
...
test( 'zla_gerfsx_extended: large rthresh forces NOPROG state', function t() {
	...
	const B = new Complex128Array( B_NOTRANS_DATA );
	const fs = factorAndSolve( 3, A, B, 'no-transpose' );
	const info = zla_gerfsx_extended( 2, 'no-transpose', 3, 1, A, 1, 3, 0, fs.AF, 1, 3, 0, fs.IPIV, 1, 0, false, C, 1, 0, B, 1, 3, 0, fs.Y, 1, 3, 0, BERR_OUT, 1, 0, 2, ERRS_N, 1, 1, 0, ERRS_C, 1, 1, 0, RES, 1, 0, AYB, 1, 0, DY, 1, 0, Y_TAIL, 1, 0, 0.0, 100, 2.0, 1e-18, false );
	assert.equal( info, 0 );
});
...
test( 'zla_gerfsx_extended: zero Y triggers HUGEVAL dx_x branch', function t() {
	...
	const B = new Complex128Array( B_NOTRANS_DATA );
	const fs = factorAndSolve( 3, A, B, 'no-transpose' );
	const yv = reinterpret( fs.Y, 0 );
	for ( i = 0; i < 6; i++ ) {
		yv[ i ] = 0.0;
	}
	const info = zla_gerfsx_extended( 2, 'no-transpose', 3, 1, A, 1, 3, 0, fs.AF, 1, 3, 0, fs.IPIV, 1, 0, false, C, 1, 0, B, 1, 3, 0, fs.Y, 1, 3, 0, BERR_OUT, 1, 0, 2, ERRS_N, 1, 1, 0, ERRS_C, 1, 1, 0, RES, 1, 0, AYB, 1, 0, DY, 1, 0, Y_TAIL, 1, 0, 1e-3, 10, 0.5, 0.25, false );
	assert.equal( info, 0 );
});
...
test( 'zla_gerfsx_extended: nrhs=2 multi-rhs', function t() {
	...
	const ERRS_C = new Float64Array( 2 * 3 );
	const ERRS_N = new Float64Array( 2 * 3 );
	const Y_TAIL = new Complex128Array( 3 );
	const tc = findCase( 'notrans_basic' );
	const A = new Complex128Array( A_NOTRANS_DATA );
	const B = new Complex128Array([
	...
	const fs1 = factorAndSolve( 3, A, new Complex128Array( B_NOTRANS_DATA ), 'no-transpose' );
	const Y = new Complex128Array( 6 );
	const yv = reinterpret( Y, 0 );
	const fv = reinterpret( fs1.Y, 0 );
	const AYB = new Float64Array( 3 );
	const DY = new Complex128Array( 3 );
	const RES = new Complex128Array( 3 );
	const C = new Float64Array( [ 1.0, 1.0, 1.0 ] );
	...
	const info = zla_gerfsx_extended( 2, 'no-transpose', 3, 2, A, 1, 3, 0, fs1.AF, 1, 3, 0, fs1.IPIV, 1, 0, true, C, 1, 0, B, 1, 3, 0, Y, 1, 3, 0, BERR_OUT, 1, 0, 2, ERRS_N, 1, 2, 0, ERRS_C, 1, 2, 0, RES, 1, 0, AYB, 1, 0, DY, 1, 0, Y_TAIL, 1, 0, 1e-3, 10, 0.5, 0.25, false );
	assert.equal( info, 0 );
