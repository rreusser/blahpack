...
import dlatrz from './../lib/ndarray.js';
const ndarrayFn = dlatrz;


// FIXTURES //

const FIXTURE_DIR = path.join( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures' );
const LINES = readFileSync( path.join( FIXTURE_DIR, 'dlatrz.jsonl' ), 'utf8' ).trim().split( '\n' ); // eslint-disable-line node/no-sync
...
test( 'dlatrz: 3x5 with l=2', function t() {
	...
	const expected = findCase( '3x5_l2' );
	const lda = 8;
	const input = [
		4.0,
		0.0,
...
test( 'dlatrz: 4x6 with l=2', function t() {
	...
	const expected = findCase( '4x6_l2' );
	const lda = 8;
	const input = [
		5.0,
		0.0,
...
test( 'dlatrz: square 3x3 (M === N, TAU set to zero)', function t() {
	...
	const expected = findCase( 'square_3x3' );
	const lda = 8;
	const input = [
		3.0,
		0.0,
	...
	const A = buildA( input, lda, 3, 3 );
	const TAU = new Float64Array([ 7.0, 7.0, 7.0 ]);
	const WORK = new Float64Array( 3 );
...
test( 'dlatrz: M=0 quick return', function t() {
	...
	const TAU = new Float64Array([ 9.0 ]);
	const A = new Float64Array( 8 * 3 );
...
test( 'dlatrz: 1x3 with l=2', function t() {
	...
	const expected = findCase( '1x3_l2' );
	const lda = 8;
	const input = [
		3.0,
		1.0,
...
test( 'dlatrz: 2x4 with l=2', function t() {
	...
	const expected = findCase( '2x4_l2' );
	const lda = 8;
	const input = [
		2.0,
		0.0,
...
test( 'dlatrz: non-zero offsets and row-major layout', function t() {
	...
	const sa2 = 1;
	const offsetA = 2;
	const A = new Float64Array( offsetA + ( 3 * sa1 ) );
	const input = [
		4.0,
		0.0,
		0.0,
