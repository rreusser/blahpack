...
test( 'dpotrs: lower_single_rhs', function t() {
	...
	const info = dpotrs( 'lower', 3, 1, A, 1, 3, 0, B, 1, 3, 0 );
...
test( 'dpotrs: upper_single_rhs', function t() {
	...
	const info = dpotrs( 'upper', 3, 1, A, 1, 3, 0, B, 1, 3, 0 );
	assert.equal( info, tc.info );
	assertArrayClose( toArray( B ), tc.x, 1e-14, 'x' );
});
...
test( 'dpotrs: lower_multi_rhs', function t() {
	...
	const A = new Float64Array( [ 4, 2, 1, 2, 5, 3, 1, 3, 9 ] );
	const B = new Float64Array( [ 1, 0, 0, 0, 1, 0 ] );
	dpotrf( 'lower', 3, A, 1, 3, 0 );
	const info = dpotrs( 'lower', 3, 2, A, 1, 3, 0, B, 1, 3, 0 );
	assert.equal( info, tc.info );
	assertArrayClose( toArray( B ), tc.x, 1e-14, 'x' );
});
...
test( 'dpotrs: n_zero', function t() {
	...
	const info = dpotrs( 'lower', 0, 1, A, 1, 1, 0, B, 1, 1, 0 );
	assert.equal( info, tc.info );
});
...
test( 'dpotrs: nrhs_zero', function t() {
	...
	const info = dpotrs( 'lower', 3, 0, A, 1, 3, 0, B, 1, 3, 0 );
	assert.equal( info, tc.info );
});
...
test( 'dpotrs: one_by_one', function t() {
	...
	const info = dpotrs( 'lower', 1, 1, A, 1, 1, 0, B, 1, 1, 0 );
	assert.equal( info, tc.info );
	assertArrayClose( toArray( B ), tc.x, 1e-14, 'x' );
});
...
test( 'dpotrs: upper_multi_rhs_3', function t() {
	...
	dpotrf( 'upper', 3, A, 1, 3, 0 );
	const info = dpotrs( 'upper', 3, 3, A, 1, 3, 0, B, 1, 3, 0 );
	assert.equal( info, tc.info );
	assertArrayClose( toArray( B ), tc.x, 1e-14, 'x' );
});
