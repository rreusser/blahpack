...
import dlamswlq from './../lib/ndarray.js';


// FIXTURES //

const fixtureDir = path.join( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures' );
const fixtureRaw = readFileSync( path.join( fixtureDir, 'dlamswlq.jsonl' ), 'utf8' ); // eslint-disable-line node/no-sync
...
function pack( buf, mr, cols, LD ) {
	let i, j;
	const out = new Float64Array( mr * cols );
	for ( j = 0; j < cols; j++ ) {
		for ( i = 0; i < mr; i++ ) {
...
function runApplyCase( cfg ) {
	const factor = findCase( cfg.factorName );
	const nblk = numBlocks( cfg.Xq, cfg.K, cfg.nb );
	const TC = nblk * cfg.K;
	const Aco = unpack( factor.A, cfg.K, cfg.Xq, cfg.K );
	const Tco = unpack( factor.T, cfg.mb, TC, cfg.mb );
	const Cco = buildC( cfg.cM, cfg.cN, cfg.fill );
	const lwk = ( cfg.side === 'left' ) ? Math.max( 1, cfg.nb ) * cfg.mb : Math.max( 1, cfg.cM ) * cfg.mb;
	const work = new Float64Array( lwk );
	const info = dlamswlq( cfg.side, cfg.trans, cfg.cM, cfg.cN, cfg.K, cfg.mb, cfg.nb, Aco, 1, cfg.K, 0, Tco, 1, cfg.mb, 0, Cco, 1, cfg.cM, 0, work, 1, 0 );
	const tc = findCase( cfg.applyName );
...
test( 'dlamswlq: M=0 quick return', function t() {
	const info = dlamswlq( 'left', 'no-transpose', 0, 4, 0, 1, 1, buf( 16 ), 1, 4, 0, buf( 16 ), 1, 4, 0, buf( 16 ), 1, 4, 0, buf( 16 ), 1, 0 );
	assert.equal( info, 0, 'M=0 returns 0' );
});

test( 'dlamswlq: N=0 quick return', function t() {
	const info = dlamswlq( 'left', 'no-transpose', 4, 0, 0, 1, 1, buf( 16 ), 1, 4, 0, buf( 16 ), 1, 4, 0, buf( 16 ), 1, 4, 0, buf( 16 ), 1, 0 );
	assert.equal( info, 0, 'N=0 returns 0' );
});

test( 'dlamswlq: K=0 quick return', function t() {
	const info = dlamswlq( 'left', 'no-transpose', 4, 4, 0, 1, 1, buf( 16 ), 1, 4, 0, buf( 16 ), 1, 4, 0, buf( 16 ), 1, 4, 0, buf( 16 ), 1, 0 );
