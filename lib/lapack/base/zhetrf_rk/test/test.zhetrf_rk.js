...
import zhetrfrk from './../lib/zhetrf_rk.js';
...
test( 'zhetrf_rk has expected arity', function t() {
	assert.strictEqual( zhetrfrk.length, 11, 'has expected arity' );
});
...
test( 'zhetrf_rk: column-major lower factorization succeeds and N=0 is a quick return', function t() {
	...
	const A = new Complex128Array( [ 4, 0, 1, -2, 3, 1, 0.5, -0.5, 0, 0, 5, 0, 2, -1, 1, 2, 0, 0, 0, 0, 7, 0, 3, 0, 0, 0, 0, 0, 0, 0, 6, 0 ] );
	const e = new Complex128Array( 4 );
	const ipiv = new Int32Array( 4 );
	info = zhetrfrk( 'column-major', 'lower', 4, A, 4, e, 1, ipiv, 1, null, 1 );
	assert.equal( info, 0, 'info=0' );

	// N = 0 quick return
	info = zhetrfrk( 'column-major', 'lower', 0, new Complex128Array( 0 ), 1, new Complex128Array( 0 ), 1, new Int32Array( 0 ), 1, null, 1 );
