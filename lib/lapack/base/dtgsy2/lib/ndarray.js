...
function dtgsy2( trans, ijob, M, N, A, strideA1, strideA2, offsetA, B, strideB1, strideB2, offsetB, C, strideC1, strideC2, offsetC, D, strideD1, strideD2, offsetD, E, strideE1, strideE2, offsetE, F, strideF1, strideF2, offsetF, scale, rdsum, rdscal, IWORK, strideIWork, offsetIWork, pq ) { // eslint-disable-line max-len, max-params
	...
	// Caller owns IWORK; assert it is large enough so an undersized buffer is a
	// loud RangeError, not a silent NaN/out-of-bounds write. IWORK holds the
	// M+N+2 block-boundary indices for (A,D) and (B,E). (M+N+2 >= 2 for all
	// valid M,N >= 0, so no separate zero-dim quick return is required — the
	// bookkeeping indices are written even for an empty solve.)
	if ( !IWORK || ( IWORK.length - offsetIWork ) < ( M + N + 2 ) ) {
		throw new RangeError( format( 'invalid argument. IWORK array must have at least %d elements from offset %d. Provided length: %d.', M + N + 2, offsetIWork, ( IWORK ) ? IWORK.length : 0 ) );
	}
