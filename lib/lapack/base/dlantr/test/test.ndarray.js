...
import dlantr from './../lib/ndarray.js';


// FIXTURES //

const fixtureDir = path.join( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures' );
const rawLines = readFileSync( path.join( fixtureDir, 'dlantr.jsonl' ), 'utf8' ).trim().split( '\n' ); // eslint-disable-line node/no-sync, max-len
...
//   [ 0   0   9  -3 ]
//   [ 0   0   0   4 ]
const UPPER_4X4 = new Float64Array([
	1, 0, 0, 0,
	-4, 5, 0, 0,
	7, -8, 9, 0,
	-2, 6, -3, 4
...
//   [-4   7  -2   3 ]
const LOWER_4X4 = new Float64Array([
	2, -3, 1, -4,
	0, 6, -5, 7,
	0, 0, 8, -2,
	0, 0, 0, 3
...
//   [ 0   0 ]
const RECT_UPPER_MN = new Float64Array([
	1, 0, 0,
	-4, 5, 0
]);
...
//   [ 0   5  -8   6 ]
const RECT_UPPER_NM = new Float64Array([
	1, 0,
	-4, 5,
	7, -8,
	-2, 6
...
//   [-4   7 ]
const RECT_LOWER_MN = new Float64Array([
	2, -3, 1, -4,
	0, 6, -5, 7
]);

// 1x1
const ONE_BY_ONE = new Float64Array([ 5.0 ]);
...
function runCase( name, norm, uplo, diag, M, N, A, lda ) {
	...
	const got = dlantr( norm, uplo, diag, M, N, A, 1, lda, 0, WORK, 1, 0 );
