...
import zhetf2rook from './../../zhetf2_rook/lib/base.js';
import zlahefrook from './../../zlahef_rook/lib/base.js';
...
*/
function zhetrfRook( uplo, N, A, strideA1, strideA2, offsetA, IPIV, strideIPIV, offsetIPIV, WORK, strideWork, offsetWork ) {
	let ldwork, result, iinfo, ipidx, info, nb, kb, k, j;
	...
	if ( uplo === 'upper' ) {
		...
		while ( k >= 1 ) {
			if ( k > nb ) {
				...
				result = zlahefrook( 'upper', k, nb, A, strideA1, strideA2, offsetA, IPIV, strideIPIV, offsetIPIV, WORK, strideWork, ldwork * strideWork, offsetWork );
				kb = result.kb;
				iinfo = result.info;
			} else {
		...
		while ( k <= N ) {
			if ( k <= N - nb ) {
				...
				result = zlahefrook( 'lower', N - k + 1, nb, A, strideA1, strideA2, offsetA + ( ( k - 1 ) * strideA1 ) + ( ( k - 1 ) * strideA2 ), IPIV, strideIPIV, offsetIPIV + ( ( k - 1 ) * strideIPIV ), WORK, strideWork, ldwork * strideWork, offsetWork );
				kb = result.kb;
				iinfo = result.info;
			} else {
