...
import zsytrsRook from './../lib/zsytrs_rook.js';
...
test( 'zsytrs_rook column-major small solve', function t() {
	...
	const info = zsytrsRook( 'column-major', 'upper', 1, 1, A, 1, ipiv, 1, b, 1 );
...
test( 'zsytrs_rook row-major small solve', function t() {
	...
	const b = new Complex128Array( [ 1.0, 1.0 ] );
	const ipiv = new Int32Array( [ 0 ] );
	const info = zsytrsRook( 'row-major', 'upper', 1, 1, A, 1, ipiv, 1, b, 1 );
	assert.equal( info, 0, 'info' );
});
