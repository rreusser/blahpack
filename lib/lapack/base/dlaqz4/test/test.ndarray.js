...
import dlaqz4 from './../lib/ndarray.js';


// FIXTURES //

const fixtureDir = path.join( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures' );
const lines = readFileSync( path.join( fixtureDir, 'dlaqz4.jsonl' ), 'utf8' ).trim().split( '\n' );
...
function eye( N, NMAX ) {
	let i;
	const M = new Float64Array( NMAX * NMAX );
	for ( i = 0; i < N; i++ ) {
		M[ i + ( i * NMAX ) ] = 1.0;
	}
	return M;
...
function runCase( caseName, opts ) {
	const NMAX = 12;
	const pencil = buildPencil( opts.N, NMAX );
	const Q = eye( opts.N, NMAX );
	const Z = eye( opts.N, NMAX );
	const QC = eye( opts.N, NMAX );
	const ZC = eye( opts.N, NMAX );
	const WORK = new Float64Array( NMAX * NMAX );
	const info = dlaqz4( opts.ilschur, opts.ilq, opts.ilz, opts.N, opts.ilo - 1, opts.ihi - 1, opts.nshifts, opts.nblockDesired, opts.SR, 1, 0, opts.SI, 1, 0, opts.SS, 1, 0, pencil.A, 1, NMAX, 0, pencil.B, 1, NMAX, 0, Q, 1, NMAX, 0, Z, 1, NMAX, 0, QC, 1, NMAX, 0, ZC, 1, NMAX, 0, WORK, 1, 0 );
...
test( 'dlaqz4: returns info=-8 when nblockDesired < nshifts+1', function t() {
	...
	const ZC = new Float64Array( 16 );
	const WORK = new Float64Array( 16 );
	const info = dlaqz4( true, true, true, 4, 0, 3, 4, 4, new Float64Array( 4 ), 1, 0, new Float64Array( 4 ), 1, 0, new Float64Array( 4 ), 1, 0, A, 1, 4, 0, B, 1, 4, 0, Q, 1, 4, 0, Z, 1, 4, 0, QC, 1, 4, 0, ZC, 1, 4, 0, WORK, 1, 0 );
