...
test( 'dsytrf_rk has expected arity', function t() {
	assert.strictEqual( dsytrfrk.length, 11, 'has expected arity' );
});
...
test( 'dsytrf_rk: row-major / column-major produce equivalent factorizations', function t() {
	...
	const Acm = new Float64Array( [ 4, 1, -2, 0.5, 0, -3, 1, 2, 0, 0, 5, -1, 0, 0, 0, 2 ] ); // eslint-disable-line max-len
	const Arm = new Float64Array( [ 4, 0, 0, 0, 1, -3, 0, 0, -2, 1, 5, 0, 0.5, 2, -1, 2 ] ); // eslint-disable-line max-len
	const ecm = new Float64Array( 4 );
	const erm = new Float64Array( 4 );
	const ipcm = new Int32Array( 4 );
	const iprm = new Int32Array( 4 );
	const info1 = dsytrfrk( 'column-major', 'lower', 4, Acm, 4, ecm, 1, ipcm, 1, null, 1 );
	const info2 = dsytrfrk( 'row-major', 'lower', 4, Arm, 4, erm, 1, iprm, 1, null, 1 );
