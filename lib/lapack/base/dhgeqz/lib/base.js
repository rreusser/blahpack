...
import dlanhs from '../../dlanhs/lib/base.js';
import dlag2 from '../../dlag2/lib/base.js';
import dlapy2 from '../../dlapy2/lib/base.js';
import dlapy3 from '../../dlapy3/lib/base.js';
...
import drot from '../../../../blas/base/drot/lib/ndarray.js';
...
const ZERO = 0.0;
const ONE = 1.0;
const HALF = 0.5; // eslint-disable-line no-unused-vars
const SAFETY = 100.0;

const SAFMIN = dlamch( 'safe-minimum' );
const SAFMAX = ONE / SAFMIN; // eslint-disable-line no-unused-vars
const ULP = dlamch( 'epsilon' ) * dlamch( 'base' );

// Scratch arrays reused across calls:
const GIVENSBUF = new Float64Array( 3 ); // for dlartg output [c, s, r]
const TAUBUF = new Float64Array( 1 );    // for dlarfg tau output
const ALPHABUF = new Float64Array( 1 );  // for dlarfg alpha in/out
...
*/
function dhgeqz( job, compq, compz, N, ilo, ihi, H, strideH1, strideH2, offsetH, T, strideT1, strideT2, offsetT, ALPHAR, strideALPHAR, offsetALPHAR, ALPHAI, strideALPHAI, offsetALPHAI, BETA, strideBETA, offsetBETA, Q, strideQ1, strideQ2, offsetQ, Z, strideZ1, strideZ2, offsetZ, WORK, strideWork, offsetWork ) {
	let ilschr, ifirst, ifrstm, ilastm, istart, eshift, ascale, bscale, ilazro;
	let ilazr2, ilpivt, ilast, iiter, maxit, jiter, ad11l, ad21l, ad12l, ad22l;
	let ad32l, temp2, tempr, tempi, scale, s1inv, done, atol, btol, info, ad11;
	let ad21, ad12, ad22, u12l, temp, c11r, c11i, c22r, c22i, wabs, lag2, ilq;
	let ilz, u12, tau, wr2, a11, a12, a21, a22, b11, b22, c12, c21, szr, szi;
	let sqr, sqi, a1r, a1i, a2r, a2i, b1r, b1i, b1a, b2r, b2i, b2a, w11, w12;
	let w21, w22, jch, svd, t2, t3, v1, v2, v3, s1, s2, wr, wi, t1, cz, cq, an;
	let bn, cr, sr, cl, sl, u1, u2, vs, jc, jr, c, s, j;
	...
	const in_ = ihi - ilo + 1;
	const anorm = dlanhs( 'frobenius', in_, H, strideH1, strideH2, offsetH + ( ilo * strideH1 ) + ( ilo * strideH2 ), WORK, strideWork, offsetWork );
	const bnorm = dlanhs( 'frobenius', in_, T, strideT1, strideT2, offsetT + ( ilo * strideT1 ) + ( ilo * strideT2 ), WORK, strideWork, offsetWork );
