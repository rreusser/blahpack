...
import dla_syrpvgrw from './../lib/index.js';
...
FIXTURES.forEach( function each( fx ) {
	test( 'ndarray fixture: ' + fx.name, function t() {
		...
		const rpvgrw = dla_syrpvgrw.ndarray( ( fx.N === 0 ) ? 'upper' : ( ( fx.name.indexOf( 'upper' ) >= 0 ) ? 'upper' : 'lower' ), N, info, A, 1, N, 0, AF, 1, N, 0, IPIV, 1, 0, WORK, 1, 0 );
...
test( 'ndarray N=0 returns 1', function t() {
	const rpvgrw = dla_syrpvgrw.ndarray( 'upper', 0, 0, new Float64Array( 0 ), 1, 1, 0, new Float64Array( 0 ), 1, 1, 0, new Int32Array( 0 ), 1, 0, new Float64Array( 0 ), 1, 0 );
...
test( 'ndarray respects strides and offsets (lower)', function t() {
	...
	const rpvgrw = dla_syrpvgrw.ndarray( 'lower', 2, 0, A, 1, 4, 0, AF, 1, 4, 0, IPIV, 1, 0, WORK, 1, 0 );
...
test( 'ndarray triggers diagonal update branch (lower 2x2 pivot, AF[k,k] dominant)', function t() {
	...
	const N = 3;
	const A = new Float64Array( [ 1, 1, 1, 0, 1, 1, 0, 0, 1 ] );

	// AF: column 0 has AF[0,0]=10 (dominant), AF[1,0]=0.1, AF[2,0]=0.1

	//     Column 1 has AF[1,1]=0.5, AF[2,1]=0.1
	const AF = new Float64Array( [ 10, 0.1, 0.1, 0, 0.5, 0.1, 0, 0, 1 ] );
	const IPIV = new Int32Array( [ -2, -2, 2 ] ); // 2x2 at k=0,1; 1x1 at k=2 (kp=2 0-based)
	const WORK = new Float64Array( 6 );
	const rpvgrw = dla_syrpvgrw.ndarray( 'lower', N, 0, A, 1, N, 0, AF, 1, N, 0, IPIV, 1, 0, WORK, 1, 0 );
...
test( 'ndarray rpvgrw < 1 when factor grows (lower)', function t() {
	...
	const AF = new Float64Array( [ 10, 0, 0, 1 ] );
