...
import dlatps from './../lib/ndarray.js';

import fxUpperNNonunit from './fixtures/upper_n_nonunit.json' with { type: 'json' };
import fxLowerNNonunit from './fixtures/lower_n_nonunit.json' with { type: 'json' };
import fxUpperTNonunit from './fixtures/upper_t_nonunit.json' with { type: 'json' };
import fxLowerTNonunit from './fixtures/lower_t_nonunit.json' with { type: 'json' };
import fxUpperNUnit from './fixtures/upper_n_unit.json' with { type: 'json' };
import fxLowerTUnit from './fixtures/lower_t_unit.json' with { type: 'json' };
import fxUpperNNorminY from './fixtures/upper_n_normin_y.json' with { type: 'json' };
import fxIdentity from './fixtures/identity.json' with { type: 'json' };
...
test( 'dlatps.ndarray: upper, no-transpose, non-unit (3x3)', function t() {
	const AP = new Float64Array( [ 2.0, 1.0, 3.0, 1.0, 2.0, 4.0 ] );
	const x = new Float64Array( [ 1.0, 2.0, 3.0 ] );
	const scale = new Float64Array( 1 );
	const CNORM = new Float64Array( 3 );
	const info = dlatps( 'upper', 'no-transpose', 'non-unit', 'no', 3, AP, 1, 0, x, 1, 0, scale, CNORM, 1, 0 );
...
test( 'dlatps.ndarray: lower, no-transpose, non-unit (3x3)', function t() {
	const AP = new Float64Array( [ 2.0, 1.0, 1.0, 3.0, 2.0, 4.0 ] );
	const x = new Float64Array( [ 1.0, 2.0, 3.0 ] );
	const scale = new Float64Array( 1 );
	const CNORM = new Float64Array( 3 );
	const info = dlatps( 'lower', 'no-transpose', 'non-unit', 'no', 3, AP, 1, 0, x, 1, 0, scale, CNORM, 1, 0 );
...
test( 'dlatps.ndarray: upper, transpose, non-unit (3x3)', function t() {
	...
	const scale = new Float64Array( 1 );
	const CNORM = new Float64Array( 3 );
	const info = dlatps( 'upper', 'transpose', 'non-unit', 'no', 3, AP, 1, 0, x, 1, 0, scale, CNORM, 1, 0 );
...
test( 'dlatps.ndarray: lower, transpose, non-unit (3x3)', function t() {
	...
	const scale = new Float64Array( 1 );
	const CNORM = new Float64Array( 3 );
	const info = dlatps( 'lower', 'transpose', 'non-unit', 'no', 3, AP, 1, 0, x, 1, 0, scale, CNORM, 1, 0 );
...
test( 'dlatps.ndarray: upper, no-transpose, unit diagonal (3x3)', function t() {
	const AP = new Float64Array( [ 99.0, 1.0, 99.0, 1.0, 2.0, 99.0 ] );
	const x = new Float64Array( [ 1.0, 2.0, 3.0 ] );
	const scale = new Float64Array( 1 );
	const CNORM = new Float64Array( 3 );
	const info = dlatps( 'upper', 'no-transpose', 'unit', 'no', 3, AP, 1, 0, x, 1, 0, scale, CNORM, 1, 0 );
...
test( 'dlatps.ndarray: lower, transpose, unit diagonal (3x3)', function t() {
	const AP = new Float64Array( [ 99.0, 1.0, 2.0, 99.0, 3.0, 99.0 ] );
	const x = new Float64Array( [ 6.0, 5.0, 4.0 ] );
	const scale = new Float64Array( 1 );
	const CNORM = new Float64Array( 3 );
	const info = dlatps( 'lower', 'transpose', 'unit', 'no', 3, AP, 1, 0, x, 1, 0, scale, CNORM, 1, 0 );
...
test( 'dlatps.ndarray: upper, no-transpose, non-unit, normin=yes (4x4)', function t() {
	const AP = new Float64Array( [ 3.0, 1.0, 4.0, 2.0, 1.0, 2.0, 1.0, 2.0, 1.0, 5.0 ] );
	const x = new Float64Array( [ 1.0, 1.0, 1.0, 1.0 ] );
	const scale = new Float64Array( 1 );
	const CNORM = new Float64Array( [ 0.0, 1.0, 3.0, 4.0 ] );
	const info = dlatps( 'upper', 'no-transpose', 'non-unit', 'yes', 4, AP, 1, 0, x, 1, 0, scale, CNORM, 1, 0 );
...
test( 'dlatps.ndarray: identity matrix passes through unchanged', function t() {
	...
	const x = new Float64Array( [ 7.0, 8.0, 9.0 ] );
	const scale = new Float64Array( 1 );
	const CNORM = new Float64Array( 3 );
	const info = dlatps( 'upper', 'no-transpose', 'non-unit', 'no', 3, AP, 1, 0, x, 1, 0, scale, CNORM, 1, 0 );
...
test( 'dlatps.ndarray: N=0 quick return', function t() {
	...
	const info = dlatps( 'upper', 'no-transpose', 'non-unit', 'no', 0, AP, 1, 0, x, 1, 0, scale, CNORM, 1, 0 );
...
test( 'dlatps.ndarray: N=1', function t() {
	...
	const scale = new Float64Array( 1 );
	const CNORM = new Float64Array( 1 );
	const info = dlatps( 'upper', 'no-transpose', 'non-unit', 'no', 1, AP, 1, 0, x, 1, 0, scale, CNORM, 1, 0 );
...
test( 'dlatps.ndarray: lower, no-transpose, unit diagonal (3x3)', function t() {
	...
	const scale = new Float64Array( 1 );
	const CNORM = new Float64Array( 3 );
	const info = dlatps( 'lower', 'no-transpose', 'unit', 'no', 3, AP, 1, 0, x, 1, 0, scale, CNORM, 1, 0 );
...
test( 'dlatps.ndarray: upper, transpose, unit diagonal (3x3)', function t() {
	...
	const scale = new Float64Array( 1 );
	const CNORM = new Float64Array( 3 );
	const info = dlatps( 'upper', 'transpose', 'unit', 'no', 3, AP, 1, 0, x, 1, 0, scale, CNORM, 1, 0 );
	assert.strictEqual( info, 0 );
	assert.strictEqual( scale[ 0 ], 1.0 );
...
test( 'dlatps.ndarray: near-overflow triggers careful path (tiny diagonal)', function t() {
	...
	const AP = new Float64Array( [ 1.0e-300, 1.0, 1.0e-300, 0.5, 1.0, 1.0e-300 ] );
	const x = new Float64Array( [ 1.0, 2.0, 3.0 ] );
	const scale = new Float64Array( 1 );
	const CNORM = new Float64Array( 3 );
	const info = dlatps( 'upper', 'no-transpose', 'non-unit', 'no', 3, AP, 1, 0, x, 1, 0, scale, CNORM, 1, 0 );
...
test( 'dlatps.ndarray: large CNORM triggers tscal scaling', function t() {
	...
	const AP = new Float64Array( [ 2.0, 1.0e308, 3.0, 1.0e308, 1.0e308, 4.0 ] );
...
test( 'dlatps.ndarray: lower, transpose, non-unit careful path (tiny diagonal)', function t() {
	const AP = new Float64Array( [ 1.0e-300, 1.0, 0.5, 1.0e-300, 1.0, 1.0e-300 ] );
	const x = new Float64Array( [ 1.0, 2.0, 3.0 ] );
	const scale = new Float64Array( 1 );
	const CNORM = new Float64Array( 3 );
	const info = dlatps( 'lower', 'transpose', 'non-unit', 'no', 3, AP, 1, 0, x, 1, 0, scale, CNORM, 1, 0 );
...
test( 'dlatps.ndarray: zero-diagonal forces unit-vector substitution', function t() {
	...
	const AP = new Float64Array( [ 0.0, 1.0, 1.0 ] );
	const x = new Float64Array( [ 1.0, 1.0 ] );
	const scale = new Float64Array( 1 );
	const CNORM = new Float64Array( 2 );
	const info = dlatps( 'upper', 'no-transpose', 'non-unit', 'no', 2, AP, 1, 0, x, 1, 0, scale, CNORM, 1, 0 );
	assert.strictEqual( info, 0 );
...
test( 'dlatps.ndarray: huge initial xmax triggers pre-scaling', function t() {
	...
	const AP = new Float64Array( [ 1.0e-300, 1.0e-300, 1.0e-300 ] );
	const x = new Float64Array( [ 1.0e308, 1.0e308 ] );
	const scale = new Float64Array( 1 );
	const CNORM = new Float64Array( 2 );
	const info = dlatps( 'upper', 'no-transpose', 'non-unit', 'no', 2, AP, 1, 0, x, 1, 0, scale, CNORM, 1, 0 );
	assert.strictEqual( info, 0 );
...
test( 'dlatps.ndarray: unit upper, careful path (huge off-diagonal)', function t() {
	...
	const AP = new Float64Array( [ 99.0, 1.0e150, 99.0, 1.0e150, 1.0e150, 99.0 ] );
	const x = new Float64Array( [ 1.0, 2.0, 3.0 ] );
	const scale = new Float64Array( 1 );
	const CNORM = new Float64Array( 3 );
	const info = dlatps( 'upper', 'no-transpose', 'unit', 'no', 3, AP, 1, 0, x, 1, 0, scale, CNORM, 1, 0 );
...
test( 'dlatps.ndarray: unit lower, careful path (huge off-diagonal)', function t() {
	const AP = new Float64Array( [ 99.0, 1.0e150, 1.0e150, 99.0, 1.0e150, 99.0 ] );
	const x = new Float64Array( [ 1.0, 2.0, 3.0 ] );
	const scale = new Float64Array( 1 );
	const CNORM = new Float64Array( 3 );
	const info = dlatps( 'lower', 'no-transpose', 'unit', 'no', 3, AP, 1, 0, x, 1, 0, scale, CNORM, 1, 0 );
...
test( 'dlatps.ndarray: huge x on careful path forces per-step rec scaling', function t() {
	...
	const AP = new Float64Array( [ 1.0e-200, 1.0, 1.0e-200, 1.0, 1.0, 1.0e-200 ] );
	const x = new Float64Array( [ 1.0e150, 1.0e150, 1.0e150 ] );
	const scale = new Float64Array( 1 );
	const CNORM = new Float64Array( 3 );
	const info = dlatps( 'upper', 'no-transpose', 'non-unit', 'no', 3, AP, 1, 0, x, 1, 0, scale, CNORM, 1, 0 );
	assert.strictEqual( info, 0 );
...
test( 'dlatps.ndarray: zero diagonal in transpose path forces e_j', function t() {
	...
	const AP = new Float64Array( [ 1.0, 1.0, 0.0 ] );
	const x = new Float64Array( [ 2.0, 3.0 ] );
	const scale = new Float64Array( 1 );
	const CNORM = new Float64Array( 2 );
	const info = dlatps( 'upper', 'transpose', 'non-unit', 'no', 2, AP, 1, 0, x, 1, 0, scale, CNORM, 1, 0 );
	assert.strictEqual( info, 0 );
