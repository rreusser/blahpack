...
const lines = readFileSync( path.join( fixtureDir, 'zhetrd_he2hb.jsonl' ), 'utf8' ).trim().split( '\n' ); // eslint-disable-line node/no-sync
...
test( 'zhetrd_he2hb: upper, N=8, KD=3 (blocked path)', function t() {
	...
	const tc = findCase( 'upper_n8_kd3' );
	const N = 8;
	const kd = 3;
	const A = buildHermitian( N, 20, 0.5 );
	const AB = new Complex128Array( ( kd + 1 ) * N );
	const TAU = new Complex128Array( N - kd );

	const info = zhetrd_he2hb( 'upper', N, kd, A, 1, N, 0, AB, 1, kd + 1, 0, TAU, 1, 0, allocWork( N, kd ), 1, 0 );
...
test( 'zhetrd_he2hb: lower, N=8, KD=3 (blocked path)', function t() {
	...
	const tc = findCase( 'lower_n8_kd3' );
	const N = 8;
	const kd = 3;
	const A = buildHermitian( N, 20, 0.5 );
	const AB = new Complex128Array( ( kd + 1 ) * N );
	...
	const info = zhetrd_he2hb( 'lower', N, kd, A, 1, N, 0, AB, 1, kd + 1, 0, TAU, 1, 0, allocWork( N, kd ), 1, 0 );
	assert.equal( info, 0, 'info' );
	assertClose( reinterpret( A, 0 ), tc.A, 1e-12, 'A' );
	assertClose( reinterpret( AB, 0 ), tc.AB, 1e-12, 'AB' );
	assertClose( reinterpret( TAU, 0 ), tc.TAU, 1e-12, 'TAU' );
...
test( 'zhetrd_he2hb: upper, N=2, KD=1 (quick return)', function t() {
	...
	const tc = findCase( 'upper_n2_kd1_quick' );
	const N = 2;
	const kd = 1;
	const A = new Complex128Array( [ 4.0, 0.0, 1.0, -1.0, 1.0, 1.0, 5.0, 0.0 ] );
	const AB = new Complex128Array( ( kd + 1 ) * N );
	const TAU = new Complex128Array( Math.max( 1, N - kd ) );
...
test( 'zhetrd_he2hb: lower, N=2, KD=1 (quick return)', function t() {
	...
	const tc = findCase( 'lower_n2_kd1_quick' );
	const N = 2;
	const kd = 1;
	const A = new Complex128Array( [ 4.0, 0.0, 1.0, -1.0, 1.0, 1.0, 5.0, 0.0 ] );
	const AB = new Complex128Array( ( kd + 1 ) * N );
...
test( 'zhetrd_he2hb: N=0 quick return', function t() {
	...
	const info = zhetrd_he2hb( 'upper', 0, 1, A, 1, 1, 0, AB, 1, 2, 0, TAU, 1, 0, allocWork( 0, 1 ), 1, 0 );
	assert.equal( info, tc.info, 'info' );
});

test( 'zhetrd_he2hb: upper, N=12, KD=4 (multiple panels)', function t() {

	const tc = findCase( 'upper_n12_kd4' );
	const N = 12;
	const kd = 4;
	const A = buildHermitian( N, 30, 0.3 );
	const AB = new Complex128Array( ( kd + 1 ) * N );
	const TAU = new Complex128Array( N - kd );
...
test( 'zhetrd_he2hb: lower, N=12, KD=4 (multiple panels)', function t() {
	...
	const tc = findCase( 'lower_n12_kd4' );
	const N = 12;
	const kd = 4;
	const A = buildHermitian( N, 30, 0.3 );
	const AB = new Complex128Array( ( kd + 1 ) * N );
...
test( 'zhetrd_he2hb: lower, N=8, KD=3 with caller-provided WORK', function t() {
	...
	const AB = new Complex128Array( ( kd + 1 ) * N );
	const TAU = new Complex128Array( N - kd );
	const WORK = new Complex128Array( ( N * kd ) + ( N * nb ) + ( 2 * kd * kd ) );

	const info = zhetrd_he2hb( 'lower', N, kd, A, 1, N, 0, AB, 1, kd + 1, 0, TAU, 1, 0, WORK, 1, 0 );
	assert.equal( info, 0, 'info' );
	assertClose( reinterpret( A, 0 ), tc.A, 1e-12, 'A' );
	assertClose( reinterpret( AB, 0 ), tc.AB, 1e-12, 'AB' );
	assertClose( reinterpret( TAU, 0 ), tc.TAU, 1e-12, 'TAU' );
