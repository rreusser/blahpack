...
import dsyconRook from './../lib/ndarray.js';


// FIXTURES //

const fixtureDir = path.join( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures' ); // eslint-disable-line max-len
const lines = readFileSync( path.join( fixtureDir, 'dsycon_rook.jsonl' ), 'utf8' ).trim().split( '\n' ); // eslint-disable-line node/no-sync, max-len
...
function runCase( uplo, N, tc, tol ) {
	...
	const info = dsyconRook( uplo, N, A, 1, N, 0, ipiv, 1, 0, tc.anorm, rcond, work, 1, 0, iwork, 1, 0 ); // eslint-disable-line max-len
...
test( 'dsycon_rook: N=0 returns rcond=1', function t() {
	...
	const rcond = new Float64Array( 1 );
	const info = dsyconRook( 'upper', 0, A, 1, 1, 0, ipiv, 1, 0, 0.0, rcond, work, 1, 0, iwork, 1, 0 );
...
test( 'dsycon_rook: anorm=0 returns rcond=0', function t() {
	...
	const rcond = new Float64Array( 1 );
	const info = dsyconRook( 'upper', 2, A, 1, 2, 0, ipiv, 1, 0, 0.0, rcond, work, 1, 0, iwork, 1, 0 );
	assert.strictEqual( info, 0 );
...
test( 'dsycon_rook: singular D returns rcond=0 (upper)', function t() {
	...
	const iwork = new Int32Array( 2 );
	const rcond = new Float64Array( 1 );
	const info = dsyconRook( 'upper', 2, A, 1, 2, 0, ipiv, 1, 0, 1.0, rcond, work, 1, 0, iwork, 1, 0 );
	assert.strictEqual( info, 0 );
	assert.strictEqual( rcond[ 0 ], 0.0 );
});
...
test( 'dsycon_rook: singular D returns rcond=0 (lower)', function t() {
	...
	const iwork = new Int32Array( 2 );
	const rcond = new Float64Array( 1 );
	const info = dsyconRook( 'lower', 2, A, 1, 2, 0, ipiv, 1, 0, 1.0, rcond, work, 1, 0, iwork, 1, 0 );
	assert.strictEqual( info, 0 );
	assert.strictEqual( rcond[ 0 ], 0.0 );
});

test( 'dsycon_rook: invalid uplo throws', function t() {
	let rcond, iwork, ipiv, work, A;
