...
import dla_wwaddw from './../lib/dla_wwaddw.js';
...
test( 'dla_wwaddw quick return for N=0 leaves arrays untouched', function t() {
	...
	const y = new Float64Array( [ 0.1, 0.2 ] );
	const w = new Float64Array( [ 5.0, 6.0 ] );
...
test( 'dla_wwaddw returns the X array', function t() {
	...
	const w = new Float64Array( [ 0.5, 0.5, 0.5 ] );
	const out = dla_wwaddw( 3, x, 1, y, 1, w, 1 );
...
test( 'dla_wwaddw preserves (X+Y)+W invariant', function t() {
	let after, rel, i;
	const n = 4;
	const x = new Float64Array( [ 1.0e8, 2.0e8, 3.0e8, 4.0e8 ] );
	const y = new Float64Array( [ 1.0e-4, 2.0e-4, 3.0e-4, 4.0e-4 ] );
	const w = new Float64Array( [ 7.0, 8.0, 9.0, 10.0 ] );
...
test( 'dla_wwaddw with negative stride', function t() {
	const x = new Float64Array( [ 1.0, 2.0, 3.0 ] );
	const y = new Float64Array( [ 0.1, 0.2, 0.3 ] );
	const w = new Float64Array( [ 10.0, 20.0, 30.0 ] );
