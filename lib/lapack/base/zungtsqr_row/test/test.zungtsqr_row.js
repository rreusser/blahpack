...
test( 'the function returns 0 for the M=0 quick-return case', function t() {
	const info = zungtsqr_row( 'column-major', 0, 0, 1, 1, new Complex128Array( 1 ), 1, new Complex128Array( 1 ), 1, new Complex128Array( 1 ) );
	assert.strictEqual( info, 0, 'returns 0' );
});

test( 'the function returns 0 for the N=0 quick-return case', function t() {
	const info = zungtsqr_row( 'column-major', 3, 0, 1, 1, new Complex128Array( 3 ), 3, new Complex128Array( 1 ), 1, new Complex128Array( 1 ) );
	assert.strictEqual( info, 0, 'returns 0' );
});

test( 'the function produces orthonormal columns in column-major layout', function t() {
	let expected, WORK, info, dotR, dotI, aR, aI, bR, bI, i, j, k;

	const A = new Complex128Array( 24 ); // 8 x 3
	...
	const T = new Complex128Array( 2 * 3 * 5 ); // nb * N * nblk
