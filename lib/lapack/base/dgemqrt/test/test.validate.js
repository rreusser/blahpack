...
const PAIRS = [ [ 1, 1 ], [ 2, 2 ], [ 3, 2 ], [ 4, 4 ], [ 5, 3 ], [ 8, 3 ], [ 8, 5 ], [ 16, 5 ], [ 17, 8 ] ];
...
function readV( Ard, L, K ) {
	const V = new LogicalMatrix( sc, L, K );
	let i, j;
	for ( j = 0; j < K; j++ ) {
		for ( i = 0; i < L; i++ ) {
...
function fullQ( Ard, Tread, L, K, nb ) {
	const Q = new LogicalMatrix( sc, L, L );
	const V = readV( Ard, L, K );
	let i, j;
	for ( j = 0; j < L; j++ ) {
		for ( i = 0; i < L; i++ ) {
...
function factorize( L, K, nb, seed, layout ) {
	...
	const A0 = logical.general( sc, rng, L, K );
	...
	PAIRS.forEach( function eachPair( pr ) {
		...
		nbsFor( K ).forEach( function eachNb( nb ) {
			ALL_LAYOUTS.forEach( function eachLayout( layout, li ) {
				const f = factorize( L, K, nb, 0x2000 + ( L * 1000 ) + ( K * 10 ) + nb, layout );
				const Q = fullQ( f.Ar, f.Tr.read, L, K, nb );
				SIDES.forEach( function eachSide( side ) {
					TRANS.forEach( function eachTrans( trans ) {
						FREE.forEach( function eachFree( fr ) {
							const rows = ( side === 'left' ) ? L : fr;
							const cols = ( side === 'left' ) ? fr : L;
							const rngC = new RNG( 0x9000 + ( counter++ ) );
							const C0 = logical.general( sc, rngC, rows, cols );
							...
							const ldwork = ( side === 'left' ) ? Math.max( 1, cols ) : Math.max( 1, rows );
							const Wr = poisonedWork( sc, ldwork * nb );
							...
							const label = 'dgemqrt L=' + L + ' K=' + K + ' nb=' + nb + ' side=' + side + ' trans=' + trans + ' free=' + fr + ' layout=' + li;
							const got = readFull( Cr, rows, cols );
							const exp = expected( side, trans, Q, C0 );
...
function runInvariance( variants, fam, side, trans ) {
	const L = 17;
	const K = 8;
	const nb = 3; // multiple blocks: ceil(8/3) = 3
	const fr = 5;
	const rows = ( side === 'left' ) ? L : fr;
	const cols = ( side === 'left' ) ? fr : L;
	const ldwork = ( side === 'left' ) ? Math.max( 1, cols ) : Math.max( 1, rows );
	...
	checked( 'dgemqrt', 'layout-invariance', function run() {
		layoutInvariant( variants, function build( layout, i ) {
			...
			const f = factorize( L, K, nb, SEED, layout );
			const cLayout = variants[ ( i + 1 ) % variants.length ];
			const rngC = new RNG( SEED ^ 0x555 );
			const C0 = logical.general( sc, rngC, rows, cols );
			const Cr = schemes.dense.realize( sc, C0, { 'part': 'full' }, cLayout );
			const Wr = poisonedWork( sc, ldwork * nb );
...
test( 'dgemqrt: advertised WORK minimum suffices (Step 4c)', function t() {
	...
	CASES.forEach( function eachCase( c ) {
		...
		const rows = ( side === 'left' ) ? L : fr;
		const cols = ( side === 'left' ) ? fr : L;
		const SEED = 0xB10C + ( L * 7 ) + K;
		const label = 'dgemqrt WORK-min side=' + side + ' trans=' + trans + ' L=' + L + ' K=' + K + ' nb=' + nb + ' free=' + fr;

		function run( len ) {
			const f = factorize( L, K, nb, SEED, null );
			const rngC = new RNG( SEED ^ 0x555 );
			const C0 = logical.general( sc, rngC, rows, cols );
