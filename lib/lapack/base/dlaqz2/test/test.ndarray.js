...
import dlaqz2 from './../lib/ndarray.js';


// FIXTURES //

const fixtureDir = path.join( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures' );
const lines = readFileSync( path.join( fixtureDir, 'dlaqz2.jsonl' ), 'utf8' ).trim().split( '\n' ); // eslint-disable-line node/no-sync
...
test( 'dlaqz2: normal_k1_qz (ilq=ilz=true)', function t() {
	const tc = findCase( 'normal_k1_qz' );
	const A = initA( N );
	const B = initB( N );
	const Q = eye( N );
	const Z = eye( N );
...
test( 'dlaqz2: normal_k2_noqz (ilq=ilz=false)', function t() {
	const tc = findCase( 'normal_k2_noqz' );
	const A = initA( N );
	const B = initB( N );
	const Q = eye( N );
	const Z = eye( N );
...
test( 'dlaqz2: edge_k3_ihi5 (k+2 == ihi, ilq=ilz=true)', function t() {
	const tc = findCase( 'edge_k3_ihi5' );
	const A = initA( N );
	const B = initB( N );
	const Q = eye( N );
	const Z = eye( N );
...
test( 'dlaqz2: normal_narrow_qonly (narrow istopm, ilq=true, ilz=false)', function t() {
	const tc = findCase( 'normal_narrow_qonly' );
	const A = initA( N );
	const B = initB( N );
	const Q = eye( N );
	const Z = eye( N );
