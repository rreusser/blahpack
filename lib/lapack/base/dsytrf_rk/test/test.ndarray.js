...
import dsytrfrk from './../lib/ndarray.js';


// FIXTURES //

const fixtureDir = path.join( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures' );
const lines = readFileSync( path.join( fixtureDir, 'dsytrf_rk.jsonl' ), 'utf8' ).trim().split( '\n' ); // eslint-disable-line node/no-sync, max-len
const FIXTURES = lines.map( function parse( line ) {
...
function build50Lower() {
	const A = new Float64Array( 50 * 50 );
	let i, j;
	for ( j = 1; j <= 50; j++ ) {
		for ( i = j; i <= 50; i++ ) {
...
function build33Lower() {
	const A = new Float64Array( 33 * 33 );
	let i, j;
	for ( j = 1; j <= 33; j++ ) {
		for ( i = j; i <= 33; i++ ) {
...
function runCase( caseName, uplo, N, A, tol ) {
	...
	const info = dsytrfrk( uplo, N, A, 1, N, 0, e, 1, 0, ipiv, 1, 0, new Float64Array( ( N*32 > 0 ) ? N*32 : 1 ), 1, 0 );
...
test( 'dsytrf_rk: 4x4_lower', function t() {
	const A = new Float64Array( [ 4, 2, 1, 0, 0, 5, 2, 1, 0, 0, 6, 3, 0, 0, 0, 8 ] ); // eslint-disable-line max-len
	runCase( '4x4_lower', 'lower', 4, A, 1e-13 );
});

test( 'dsytrf_rk: 4x4_upper', function t() {
	const A = new Float64Array( [ 4, 0, 0, 0, 2, 5, 0, 0, 1, 2, 6, 0, 0, 1, 3, 8 ] ); // eslint-disable-line max-len
	runCase( '4x4_upper', 'upper', 4, A, 1e-13 );
});

test( 'dsytrf_rk: 4x4_indef_lower (forces 2x2 pivots)', function t() {
	const A = new Float64Array( [ 0, 1, 2, 3, 0, 0, 4, 5, 0, 0, 0, 6, 0, 0, 0, 0 ] ); // eslint-disable-line max-len
	runCase( '4x4_indef_lower', 'lower', 4, A, 1e-13 );
});

test( 'dsytrf_rk: 4x4_indef_upper (forces 2x2 pivots)', function t() {
	const A = new Float64Array( [ 0, 0, 0, 0, 1, 0, 0, 0, 2, 4, 0, 0, 3, 5, 6, 0 ] ); // eslint-disable-line max-len
...
test( 'dsytrf_rk: n=0 quick return (via base)', function t() {
	...
	const info = dsytrfrk( 'lower', 0, A, 1, 1, 0, e, 1, 0, ipiv, 1, 0, new Float64Array( 1 ), 1, 0 );
...
test( 'dsytrf_rk: 5x5_lower (1x1 + 2x2 pivots)', function t() {
	const A = new Float64Array( [ 4, 1, -2, 0.5, 1.5, 0, -3, 1, 2, 0, 0, 0, 5, -1, 0.5, 0, 0, 0, 2, 1, 0, 0, 0, 0, -4 ] ); // eslint-disable-line max-len
	runCase( '5x5_lower', 'lower', 5, A, 1e-13 );
});

test( 'dsytrf_rk: 5x5_upper (1x1 + 2x2 pivots)', function t() {
	const A = new Float64Array( [ 4, 0, 0, 0, 0, 1, -3, 0, 0, 0, -2, 1, 5, 0, 0, 0.5, 2, -1, 2, 0, 1.5, 0, 0.5, 1, -4 ] ); // eslint-disable-line max-len
