...
import zunbdb1 from './../lib/ndarray.js';


// FIXTURES //

const fixtureDir = path.join( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures' );
const lines = readFileSync( path.join( fixtureDir, 'zunbdb1.jsonl' ), 'utf8' ).trim().split( '\n' ); // eslint-disable-line node/no-sync
...
function runFixtureCase( name, M, P, Q, hasPhi ) {
	...
	const info = zunbdb1( M, P, Q, X11, 1, LDX_PAD, 0, X21, 1, LDX_PAD, 0, THETA, 1, 0, PHI, 1, 0, TAUP1, 1, 0, TAUP2, 1, 0, TAUQ1, 1, 0, WORK, 1, 0 );
...
test( 'zunbdb1.ndarray: q0_quick_return — loop body never executes', function t() {
	...
	const info = zunbdb1( 4, 2, 0, X11, 1, LDX_PAD, 0, X21, 1, LDX_PAD, 0, THETA, 1, 0, PHI, 1, 0, TAUP1, 1, 0, TAUP2, 1, 0, TAUQ1, 1, 0, WORK, 1, 0 );
...
test( 'zunbdb1.ndarray: returns info=0 when called with M=Q=0 (degenerate)', function t() {
	...
	const info = zunbdb1( 0, 0, 0, X11, 1, 1, 0, X21, 1, 1, 0, THETA, 1, 0, PHI, 1, 0, TAUP1, 1, 0, TAUP2, 1, 0, TAUQ1, 1, 0, WORK, 1, 0 );
	assert.strictEqual( info, 0, 'info' );
});
