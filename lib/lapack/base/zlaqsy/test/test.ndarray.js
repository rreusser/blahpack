...
test( 'zlaqsy: upper_equilibrate', function t() {
	...
	const equed = zlaqsy( 'upper', 3, A, 1, 3, 0, s, 1, 0, 0.05, 16.0 );
...
test( 'zlaqsy: lower_equilibrate', function t() {
	...
	const s = new Float64Array( [ 0.5, 1.0/3.0, 0.25 ] );
	const equed = zlaqsy( 'lower', 3, A, 1, 3, 0, s, 1, 0, 0.05, 16.0 );
	assert.equal( equed, 'yes', 'equed' );
	const view = reinterpret( A, 0 );
	assertArrayClose( toArray( view ), tc.a, 1e-14, 'a' );
});
...
test( 'zlaqsy: no_equilibrate', function t() {
	...
	const equed = zlaqsy( 'upper', 3, A, 1, 3, 0, s, 1, 0, 0.5, 16.0 );
...
test( 'zlaqsy: n_zero', function t() {
	...
	const equed = zlaqsy( 'upper', 0, A, 1, 1, 0, s, 1, 0, 1.0, 1.0 );
	assert.equal( equed, 'none', 'equed' );
});
...
test( 'zlaqsy: n_one_upper', function t() {
	...
	const A = new Complex128Array( [ 100.0, 50.0 ] );
	const s = new Float64Array( [ 0.1 ] );
	const equed = zlaqsy( 'upper', 1, A, 1, 1, 0, s, 1, 0, 0.01, 100.0 );
	assert.equal( equed, 'yes', 'equed' );
	const view = reinterpret( A, 0 );
	assertArrayClose( toArray( view ), tc.a, 1e-14, 'a' );
});
...
test( 'zlaqsy: small_amax', function t() {
	...
	const equed = zlaqsy( 'upper', 2, A, 1, 2, 0, s, 1, 0, 1.0, 1e-300 );
	assert.equal( equed, 'yes', 'equed' );
	const view = reinterpret( A, 0 );
	assertArrayClose( toArray( view ), tc.a, 1e-14, 'a' );
});
