...
test( 'dpprfs: multi_rhs_3x3', function t() {
	...
	assert.equal( info, tc.info, 'info' );
	const xOut = new Float64Array( X.buffer );
...
test( 'dpprfs: n_zero', function t() {
	...
	const info = dpprfs( 'upper', 0, 1, AP, 1, 0, AFP, 1, 0, B, 1, 1, 0, X, 1, 1, 0, FERR, 1, 0, BERR, 1, 0, WORK, 1, 0, IWORK, 1, 0 ); // eslint-disable-line max-len
	assert.equal( info, tc.info, 'info' );
...
test( 'dpprfs: nrhs_zero', function t() {
	...
	const WORK = new Float64Array( 9 );
	const IWORK = new Int32Array( 3 );
	const info = dpprfs( 'upper', 3, 0, AP, 1, 0, AFP, 1, 0, B, 1, 3, 0, X, 1, 3, 0, FERR, 1, 0, BERR, 1, 0, WORK, 1, 0, IWORK, 1, 0 ); // eslint-disable-line max-len
	assert.equal( info, tc.info, 'info' );
});
