...
test( 'dgecon: N=0 quick return', function t() {
	...
	const info = dgecon( 'one-norm', 0, new Float64Array( 0 ), 1, 1, 0, 0, rcond, new Float64Array( 0 ), 1, 0, new Int32Array( 0 ), 1, 0 );
...
test( 'dgecon: anorm=0 returns rcond=0', function t() {
	...
	const rcond = new Float64Array( 1 );
	rcond[ 0 ] = 0.5;
	const info = dgecon( 'one-norm', N, A, 1, N, 0, 0.0, rcond, work, 1, 0, iwork, 1, 0 );
	assert.strictEqual( info, 0 );
...
test( 'dgecon: identity 3x3 (well-conditioned, rcond=1)', function t() {
	...
	A[ 4 ] = 1.0;
	A[ 8 ] = 1.0;
	const anorm = norm1( A, N );
	const IPIV = new Int32Array( N );
	const work = new Float64Array( 4 * N );
	const iwork = new Int32Array( N );
	const rcond = new Float64Array( 1 );
	dgetrf( N, N, A, 1, N, 0, IPIV, 1, 0 );
	const info = dgecon( 'one-norm', N, A, 1, N, 0, anorm, rcond, work, 1, 0, iwork, 1, 0 );
	assert.strictEqual( info, 0 );
...
test( 'dgecon: identity 4x4 inf-norm', function t() {
	...
	const anorm = normInf( A, N );
	const IPIV = new Int32Array( N );
	const work = new Float64Array( 4 * N );
	const iwork = new Int32Array( N );
	const rcond = new Float64Array( 1 );
	dgetrf( N, N, A, 1, N, 0, IPIV, 1, 0 );
	const info = dgecon( 'inf-norm', N, A, 1, N, 0, anorm, rcond, work, 1, 0, iwork, 1, 0 );
	assert.strictEqual( info, 0 );
	assert.ok( close( rcond[ 0 ], 1.0, 1e-9 ) );
});
...
test( 'dgecon: well-conditioned 3x3 (1-norm)', function t() {
	const N = 3;
	const A = new Float64Array( [ 2.0, 1.0, 0.0, 1.0, 2.0, 1.0, 0.0, 1.0, 2.0 ] );
	const anorm = norm1( A, N );
	const IPIV = new Int32Array( N );
	const work = new Float64Array( 4 * N );
	const iwork = new Int32Array( N );
...
test( 'dgecon: anorm = NaN returns info=-5, rcond=NaN', function t() {
	...
	const rcond = new Float64Array( 1 );
	dgetrf( N, N, A, 1, N, 0, IPIV, 1, 0 );
	const info = dgecon( 'one-norm', N, A, 1, N, 0, NaN, rcond, work, 1, 0, iwork, 1, 0 );
...
test( 'dgecon: anorm = +Infinity returns info=-5', function t() {
	...
	const rcond = new Float64Array( 1 );
	dgetrf( N, N, A, 1, N, 0, IPIV, 1, 0 );
	const info = dgecon( 'one-norm', N, A, 1, N, 0, Infinity, rcond, work, 1, 0, iwork, 1, 0 );
	assert.strictEqual( info, -5 );
});
