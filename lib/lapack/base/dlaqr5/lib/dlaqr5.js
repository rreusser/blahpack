...
/**
* Performs a single small-bulge multishift QR sweep on a real upper Hessenberg matrix.
*
...
* @param {Float64Array} SR - real parts of the shifts (length `nshfts`)
* @param {integer} strideSR - `SR` stride length
* @param {Float64Array} SI - imaginary parts of the shifts (length `nshfts`)
* @param {integer} strideSI - `SI` stride length
* @param {Float64Array} H - upper Hessenberg matrix; overwritten
...
* @param {Float64Array} Z - Schur vectors; updated when `wantz` is true
* @param {PositiveInteger} LDZ - leading dimension of `Z`
* @param {Float64Array} V - workspace of bulge data (`3 x nshfts/2`)
* @param {PositiveInteger} LDV - leading dimension of `V`
* @param {Float64Array} U - accumulator workspace (`(2*nshfts+1) x (2*nshfts+1)`)
...
* @param {Float64Array} WV - vertical workspace
...
* @param {Float64Array} WH - horizontal workspace
* @param {PositiveInteger} LDWH - leading dimension of `WH`
* @throws {RangeError} if a numerical argument does not satisfy constraints
* @returns {void}
*/
function dlaqr5( wantt, wantz, kacc22, N, ktop, kbot, nshfts, SR, strideSR, SI, strideSI, H, LDH, iloz, ihiz, Z, LDZ, V, LDV, U, LDU, nv, WV, LDWV, nh, WH, LDWH ) {
	if ( N < 0 ) {
		throw new RangeError( format( 'invalid argument. Fourth argument must be a nonnegative integer. Value: `%d`.', N ) );
	}
	if ( LDH < max( 1, N ) ) {
		throw new RangeError( format( 'invalid argument. Thirteenth argument must be greater than or equal to max(1,N). Value: `%d`.', LDH ) );
	}
	if ( LDZ < max( 1, N ) ) {
		throw new RangeError( format( 'invalid argument. Seventeenth argument must be greater than or equal to max(1,N). Value: `%d`.', LDZ ) );
	}
	const osr = stride2offset( nshfts, strideSR );
	const osi = stride2offset( nshfts, strideSI );
	return base( wantt, wantz, kacc22, N, ktop, kbot, nshfts, SR, strideSR, osr, SI, strideSI, osi, H, 1, LDH, 0, iloz, ihiz, Z, 1, LDZ, 0, V, 1, LDV, 0, U, 1, LDU, 0, nv, WV, 1, LDWV, 0, nh, WH, 1, LDWH, 0 );
}
