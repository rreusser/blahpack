...
test( 'ndarray returns a positive condition number for an SPD matrix', function t() { // eslint-disable-line max-len
	...
	const A = new Float64Array([ 4.0, 1.0, 0.5, 1.0, 5.0, 1.0, 0.5, 1.0, 6.0 ]);
	const AF = new Float64Array( A );
	dpotrf( 'upper', 3, AF, 1, 3, 0 );
	const C = new Float64Array([ 2.0, 1.0, 0.5 ]);
	const WORK = new Float64Array( 9 );
	const IWORK = new Int32Array( 3 );
	const result = dla_porcond.ndarray( 'upper', 3, A, 1, 3, 0, AF, 1, 3, 0, 1, C, 1, 0, WORK, 1, 0, IWORK, 1, 0 ); // eslint-disable-line max-len
