/**
* Property-based validation for ztrtrs, following the /blahpack-validate process.
*
...
* diagonal = 1) as the independent oracle. For complex A, `trans` includes
* 'conjugate-transpose' (A**H), exercised distinctly from 'transpose' (A**T).
* `check.assertResidual` normalizes by ‖A‖·‖x‖ + ‖b‖ (backward error), so an
* ill-conditioned unit-diagonal triangle is still validated correctly.
*/
...
test( 'ztrtrs: triangular solve residual (uplo x trans x diag x N x nrhs)', function t() {
...
function runResidual( uplo, trans, diag, N, nrhs ) {
	...
	ztrtrs( uplo, trans, diag, N, nrhs, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], Br.data, Br.args[ 0 ], Br.args[ 1 ], Br.args[ 2 ] );

	const code = transCode( trans );
	checked( 'ztrtrs', 'residual', function run() {
		let j;
		for ( j = 0; j < nrhs; j++ ) {
			check.assertResidual( sc, A0, readCol( Br, N, j ), logicalCol( B0, N, j ), {
				'trans': code,
				'factor': 100,
				'label': 'ztrtrs '+uplo+' '+trans+' '+diag+' N='+N+' nrhs='+nrhs+' col='+j
			});
		}
	});
}
...
test( 'ztrtrs: bit-exact across all storage layouts', function t() {
	...
	combos.forEach( function eachCombo( c ) {
		...
		const diag = c[ 2 ];
		const unit = ( diag === 'unit' );
		checked( 'ztrtrs', 'layout-invariance', function run() {
			...
			}, { 'label': 'ztrtrs '+uplo+' '+trans+' '+diag+' layout invariance' } );
		});
	});
});
