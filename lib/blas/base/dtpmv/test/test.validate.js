/**
* Property-based validation for dtpmv, following the /blahpack-validate process.
*
...
import dtpmv from './../lib/ndarray.js';
...
// Scaled residual assertion mirroring dtrmv / dspmv.
...
test( 'dtpmv: triangular packed matrix-vector residual (uplo x trans x diag x N sweep)', function t() {
		...
		TRANS.forEach( function eachTrans( tr ) {
			...
			DIAG.forEach( function eachDiag( diag ) {
				...
				SIZES.forEach( function eachN( N ) {
					...
					dtpmv( uplo, trans, diag, N, AP.data, AP.args[ 0 ], AP.args[ 1 ], X.data, X.args[ 0 ], X.args[ 1 ] );
					var expected = ref.matvec( sc, A, x, { 'trans': code } );
					var got = [];
					for ( i = 0; i < N; i++ ) {
						got.push( X.read( i ) );
					}
					checked( 'dtpmv', 'residual', function run() {
						assertResidual( got, expected, 'dtpmv '+uplo+' '+trans+' '+diag+' N='+N, N );
					});
				});
			});
		});
...
// family split is required (unlike the dense dtrmv dot/axpy choice).
test( 'dtpmv: output is bit-exact across storage layouts (packed AP + strided/negative vectors)', function t() {
	var N = 9;
	var SEED = 0xF11E;
	...
	UPLO.forEach( function eachUplo( uplo ) {
		TRANS.forEach( function eachTrans( tr ) {
			...
			DIAG.forEach( function eachDiag( diag ) {
				var unit = ( diag === 'unit' );
				checked( 'dtpmv', 'layout-invariance', function run() {
					...
					}, { 'label': 'dtpmv '+uplo+' '+trans+' '+diag+' layout invariance' } );
				});
			});
		});
	});
