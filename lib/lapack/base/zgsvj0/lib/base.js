...
*/
function zgsvj0( jobv, M, N, A, strideA1, strideA2, offsetA, d, strideD, offsetD, sva, strideSVA, offsetSVA, mv, V, strideV1, strideV2, offsetV, eps, sfmin, tol, nsweep, work, strideWork, offsetWork ) {
	let convergedSweep, pskipped, aapqMag, bailjbc, ijblsk, iswrot, mxaapq;
	let mxsinj, newDpI, newDpR, notrot, swband, thsign, aapp0, aapq1, aapqI;
	let aapqR, apoaq, aqoap, ompqI, ompqR, rotok, rsvec, temp1, theta, aapp;
	let aaqq, dotZ, info, ap0, aq0, dp0, dq0, dqI, dqR, ibr, igl, ir1, jbc, jgl;
	let mvl, nbl, res, vp0, vq0, cs, sn, i, p, q, t;

	const applv = ( jobv === 'apply-v' );
	rsvec = ( jobv === 'compute-v' );

	// Determine the available workspace length from the caller-provided buffer
	...
	const rootsfmin = Math.sqrt( sfmin );
	const small = sfmin / eps;
	const big = 1.0 / sfmin;
	const rootbig = 1.0 / rootsfmin;
	const bigtheta = 1.0 / rooteps;
	const roottol = Math.sqrt( tol );

	const emptsw = ( N * ( N - 1 ) ) / 2;
	notrot = 0;
	...
	const blskip = kbl * kbl;
	const rowskip = ( kbl > 5 ) ? 5 : kbl;
	const lkahead = 1;
	pskipped = 0;
	iswrot = 0;
	mxaapq = 0.0;
	...
	const sScratch = new Float64Array( 2 );
