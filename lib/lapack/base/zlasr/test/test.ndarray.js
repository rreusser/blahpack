...
import zlasr from './../lib/index.js';
...
function extractAll( A, M, N, LDA ) {
	const Av = ( A instanceof Complex128Array ) ? reinterpret( A, 0 ) : A;
	const result = [];
	let row, col;
	for ( row = 0; row < M; row++ ) {
		for ( col = 0; col < N; col++ ) {
...
test( 'returns early when M=0', function t() {
	const A = new Complex128Array( [ 1, 2, 3, 4, 5, 6, 7, 8 ] );
	const c = new Float64Array( [ 0.5 ] );
	const s = new Float64Array( [ 0.5 ] );
	const expected = new Float64Array( reinterpret( A, 0 ) );
...
test( 'left, variable pivot, forward: 2x2 with 45-degree rotation', function t() {
	...
	const c = new Float64Array( [ cos45 ] );
	const s = new Float64Array( [ sin45 ] );
...
test( 'left, variable pivot, forward: 3x2 with complex entries', function t() {
	...
	const c = new Float64Array( [ 0.6, 0.8 ] );
	const s = new Float64Array( [ 0.8, 0.6 ] );
...
test( 'works with non-unit stride for c and s', function t() {
	...
	const c = new Float64Array( [ 0.6, 999, 0.8 ] );
	const s = new Float64Array( [ 0.8, 999, 0.6 ] );
...
test( 'works with offset for c, s, and A', function t() {
	...
	const junk = 2; // in complex elements
	const A = new Complex128Array( junk + LDA * N );
	let Av = reinterpret( A, 0 );
	...
	const c = new Float64Array( [ 999, 0.6 ] );
	const s = new Float64Array( [ 999, 0.8 ] );

	base( 'left', 'variable', 'forward', M, N, c, 1, 1, s, 1, 1, A, 1, LDA, junk );

	// Extract from offset (using Float64 view starting at junk*2 doubles)
	Av = reinterpret( A, 0 );
	const result = extractAll( new Float64Array( Av.buffer, Av.byteOffset + junk * 2 * 8 ), M, N, LDA );
	const expected = new Float64Array([
		0.6, 0, 0.8, 0,
		-0.8, 0, 0.6, 0
	]);
...
test( 'skips rotation when c=1 and s=0', function t() {
	...
	const original = new Float64Array( reinterpret( A, 0 ) );

	// Both rotations are identity
	const c = new Float64Array( [ 1, 1 ] );
	const s = new Float64Array( [ 0, 0 ] );
...
test( 'handles 1x1 matrix (no rotations to apply)', function t() {
	let A = new Complex128Array( [ 3, 4 ] ); // (3+4i)
	const c = new Float64Array( 0 );
	const s = new Float64Array( 0 );
	const expected = new Float64Array( [ 3, 4 ] );
...
test( 'left, variable pivot, forward: 3x3 complex, two rotations', function t() {
	...
	const c = new Float64Array( [ 0.8, 0.5 ] );
	const s = new Float64Array( [ 0.6, Math.sqrt( 0.75 ) ] );
	const sq75 = Math.sqrt( 0.75 );
	...
	//            = (-sq75-0.3, -0.4)
	const r20re = -0.5 + 0.6 * sq75;
	const r20im = 0.8 * sq75;
	const r10re = -sq75 - 0.3;
	const r10im = -0.4;
	...
	//            = (1.2, -2*sq75-0.3)
	const r21re = -2.4 * sq75;
	const r21im = -1 + 0.6 * sq75;
	const r11re = 1.2;
	const r11im = -2 * sq75 - 0.3;
	...
	//            = (4*sq75-0.6, 0.8)
	const r22re = 2 + 1.2 * sq75;
	const r22im = -1.6 * sq75;
	const r12re = 4 * sq75 - 0.6;
	const r12im = 0.8;
...
test( 'skips rotation when c=1, s=0 for L/V/B', function t() {
	...
	const N = 2;
	const LDA = M;
	const A = buildMatrix( M, N, [ [ 1, 1 ], [ 2, 2 ], [ 3, 3 ], [ 4, 4 ] ] );
	const original = new Float64Array( reinterpret( A, 0 ) );
...
test( 'accepts lowercase side, pivot, direct', function t() {
	...
	const A1 = buildMatrix( M, N, [ [ 1, 2 ], [ 3, 4 ], [ 5, 6 ], [ 7, 8 ] ] );
	const A2 = buildMatrix( M, N, [ [ 1, 2 ], [ 3, 4 ], [ 5, 6 ], [ 7, 8 ] ] );
...
test( 'returns A', function t() {
	const A = buildMatrix( 2, 2, [ [ 1, 0 ], [ 0, 0 ], [ 0, 0 ], [ 1, 0 ] ] );
	const c = new Float64Array( [ 1 ] );
	const s = new Float64Array( [ 0 ] );
	const result = base( 'left', 'variable', 'forward', 2, 2, c, 1, 0, s, 1, 0, A, 1, 2, 0 );
