...
* `x(i) * (alpha*y(j))` with the identical floating-point expression, and the
* reference `y(j) !== 0` column guard is preserved, so this only reschedules
* memory: it is verified bit-identically against the reference kernel
* (`bench/zger-opt/`). See `DIFFERENCES.md`.
*
* @private
