...
test( 'dtpcon: N=0 quick return (rcond=1)', function t() {
	const rcond = new Float64Array( 1 );
	rcond[ 0 ] = 0.5;
	const info = dtpcon( 'one-norm', 'upper', 'non-unit', 0, new Float64Array( 0 ), 1, 0, rcond, new Float64Array( 0 ), 1, 0, new Int32Array( 0 ), 1, 0 );
...
test( 'dtpcon: zero matrix returns rcond=0', function t() {
	...
	const info = dtpcon( 'one-norm', 'upper', 'non-unit', N, AP, 1, 0, rcond, work, 1, 0, iwork, 1, 0 );
	assert.strictEqual( info, 0 );
...
test( 'dtpcon: identity 3x3 upper non-unit (inf-norm, rcond=1)', function t() {
	...
	const work = new Float64Array( 3 * N );
	const iwork = new Int32Array( N );
	const info = dtpcon( 'inf-norm', 'upper', 'non-unit', N, AP, 1, 0, rcond, work, 1, 0, iwork, 1, 0 );
	assert.strictEqual( info, 0 );
	assert.ok( close( rcond[ 0 ], 1.0, 1e-9 ) );
});
...
test( 'dtpcon: upper unit 4x4 (one-norm)', function t() {
	...
	const work = new Float64Array( 3 * N );
	const iwork = new Int32Array( N );
	const info = dtpcon( 'one-norm', 'upper', 'unit', N, AP, 1, 0, rcond, work, 1, 0, iwork, 1, 0 );
	assert.strictEqual( info, 0 );
...
test( 'dtpcon: upper unit 4x4 (inf-norm)', function t() {
	...
	const work = new Float64Array( 3 * N );
	const iwork = new Int32Array( N );
	const info = dtpcon( 'inf-norm', 'upper', 'unit', N, AP, 1, 0, rcond, work, 1, 0, iwork, 1, 0 );
	assert.strictEqual( info, 0 );
	assert.ok( rcond[ 0 ] > 0.0 && rcond[ 0 ] <= 1.0 );
});
...
test( 'dtpcon: lower non-unit 4x4 diagonal (one-norm)', function t() {
	...
	A[ 10 ] = 2.0;
	A[ 15 ] = 1.0;
	const AP = packLower( A, N );
	const rcond = new Float64Array( 1 );
	const work = new Float64Array( 3 * N );
	const iwork = new Int32Array( N );
	const info = dtpcon( 'one-norm', 'lower', 'non-unit', N, AP, 1, 0, rcond, work, 1, 0, iwork, 1, 0 );
	assert.strictEqual( info, 0 );
...
test( 'dtpcon: lower non-unit 4x4 diagonal (inf-norm)', function t() {
	...
	const work = new Float64Array( 3 * N );
	const iwork = new Int32Array( N );
	const info = dtpcon( 'inf-norm', 'lower', 'non-unit', N, AP, 1, 0, rcond, work, 1, 0, iwork, 1, 0 );
	assert.strictEqual( info, 0 );
	assert.ok( close( rcond[ 0 ], 0.25, 1e-9 ) );
});
...
test( 'dtpcon: lower unit 4x4 (one-norm)', function t() {
	...
	const work = new Float64Array( 3 * N );
	const iwork = new Int32Array( N );
	const info = dtpcon( 'one-norm', 'lower', 'unit', N, AP, 1, 0, rcond, work, 1, 0, iwork, 1, 0 );
	assert.strictEqual( info, 0 );
	assert.ok( rcond[ 0 ] > 0.0 && rcond[ 0 ] <= 1.0 );
});
...
test( 'dtpcon: lower unit 4x4 (inf-norm)', function t() {
	...
	const work = new Float64Array( 3 * N );
	const iwork = new Int32Array( N );
	const info = dtpcon( 'inf-norm', 'lower', 'unit', N, AP, 1, 0, rcond, work, 1, 0, iwork, 1, 0 );
	assert.strictEqual( info, 0 );
	assert.ok( rcond[ 0 ] > 0.0 && rcond[ 0 ] <= 1.0 );
});
