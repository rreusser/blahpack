 ...
 *
 * -   WORK is caller-owned (at least N*NB elements).
 *
 *
...
function dorgql( M, N, K, A, strideA1, strideA2, offsetA, TAU, strideTAU, offsetTAU, WORK, strideWork, offsetWork ) {
	...
	// Caller owns WORK; assert it is large enough (blocked path needs N*NB for the
	// T factor + dlarfb scratch; the unblocked dorg2l needs N) so an under-sized
	// buffer is a loud RangeError, not a silent NaN from an out-of-bounds read.
	const minWork = ( K > NB ) ? ( N * NB ) : N;
