...
import zgemlqt from './../lib/zgemlqt.js';
...
test( 'zgemlqt: row-major succeeds (K=0 quick return on row-major path)', function t() {
	...
	const V = new Complex128Array( 16 );
	const T = new Complex128Array( 16 );
	const C = new Complex128Array( 16 );
	const info = zgemlqt( 'row-major', 'left', 'no-transpose', 4, 4, 0, 1, V, 4, T, 1, C, 4, WORK, 1 );
	assert.equal( info, 0 );
});
...
test( 'zgemlqt: column-major succeeds (Q*I via small fixture)', function t() {
	...
	const f = findCase( 'lq_factors_small' );
	const V = toComplex( f.v );
	const T = toComplex( f.t );
	const C = eye( 4, 4, 4 );
	const WORK = new Complex128Array( 4 * 2 );
	const info = zgemlqt( 'column-major', 'left', 'no-transpose', 4, 4, 3, 2, V, 3, T, 2, C, 4, WORK, 1 );
