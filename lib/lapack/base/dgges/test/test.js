...
function assertMatrixColumnsClose( actual, expected, n, tol, msg ) {
	let matchPos, matchNeg, relErr, j, i;
	for ( j = 0; j < n; j++ ) {
...
function runTest( tc ) {
	...
	const n = tc.n;
	const jobvsl = ( tc.jobvsl === 'V' ) ? 'compute-vectors' : 'no-vectors';
	const jobvsr = ( tc.jobvsr === 'V' ) ? 'compute-vectors' : 'no-vectors';
	...
	const VSL = new Float64Array( ( tc.jobvsl === 'V' ) ? n * n : 1 );
	const VSR = new Float64Array( ( tc.jobvsr === 'V' ) ? n * n : 1 );
	const WORK = new Float64Array( Math.max( 8 * n, ( 6 * n ) + 16, 1 ) );
	const BWORK = new Uint8Array( Math.max( 1, n ) );

	const result = dgges( jobvsl, jobvsr, 'not-sorted', noop, n, A, 1, n, 0, B, 1, n, 0, ALPHAR, 1, 0, ALPHAI, 1, 0, BETA, 1, 0, VSL, 1, n, 0, VSR, 1, n, 0, WORK, 1, 0, BWORK, 1, 0 ); // eslint-disable-line max-len
...
test( 'dgges: N=0 quick return', function t() {
	const result = dgges( 'compute-vectors', 'compute-vectors', 'not-sorted', noop, 0, new Float64Array( 0 ), 1, 1, 0, new Float64Array( 0 ), 1, 1, 0, new Float64Array( 0 ), 1, 0, new Float64Array( 0 ), 1, 0, new Float64Array( 0 ), 1, 0, new Float64Array( 0 ), 1, 1, 0, new Float64Array( 0 ), 1, 1, 0, new Float64Array( 1 ), 1, 0, new Uint8Array( 1 ), 1, 0 ); // eslint-disable-line max-len
...
test( 'dgges: sorted eigenvalues with selection function', function t() {
	...
	const result = dgges( 'compute-vectors', 'compute-vectors', 'sorted', selectPositive, n, A, 1, n, 0, B, 1, n, 0, ALPHAR, 1, 0, ALPHAI, 1, 0, BETA, 1, 0, VSL, 1, n, 0, VSR, 1, n, 0, new Float64Array( Math.max( 8 * n, ( 6 * n ) + 16 ) ), 1, 0, new Uint8Array( n ), 1, 0 ); // eslint-disable-line max-len
	assert.equal( result.info, 0, 'info' );
...
test( 'dgges: sorted with no eigenvalues selected', function t() {
	...
	const VSL = new Float64Array( n * n );
	const VSR = new Float64Array( n * n );
	const result = dgges( 'compute-vectors', 'compute-vectors', 'sorted', noop, n, A, 1, n, 0, B, 1, n, 0, ALPHAR, 1, 0, ALPHAI, 1, 0, BETA, 1, 0, VSL, 1, n, 0, VSR, 1, n, 0, new Float64Array( Math.max( 8 * n, ( 6 * n ) + 16 ) ), 1, 0, new Uint8Array( n ), 1, 0 ); // eslint-disable-line max-len
	assert.equal( result.info, 0, 'info' );
...
test( 'dgges: sorted with complex conjugate eigenvalue pair', function t() {
	...
	const VSL = new Float64Array( n * n );
	const VSR = new Float64Array( n * n );
	const result = dgges( 'compute-vectors', 'compute-vectors', 'sorted', selectAll, n, A, 1, n, 0, B, 1, n, 0, ALPHAR, 1, 0, ALPHAI, 1, 0, BETA, 1, 0, VSL, 1, n, 0, VSR, 1, n, 0, new Float64Array( Math.max( 8 * n, ( 6 * n ) + 16 ) ), 1, 0, new Uint8Array( n ), 1, 0 ); // eslint-disable-line max-len
	assert.equal( result.info, 0, 'info' );
...
test( 'dgges: small-norm matrix triggers scaling', function t() {
	...
	const VSL = new Float64Array( n * n );
	const VSR = new Float64Array( n * n );
	const result = dgges( 'compute-vectors', 'compute-vectors', 'not-sorted', noop, n, A, 1, n, 0, B, 1, n, 0, ALPHAR, 1, 0, ALPHAI, 1, 0, BETA, 1, 0, VSL, 1, n, 0, VSR, 1, n, 0, new Float64Array( Math.max( 8 * n, ( 6 * n ) + 16 ) ), 1, 0, new Uint8Array( n ), 1, 0 ); // eslint-disable-line max-len
	assert.equal( result.info, 0, 'info' );
	const eig0 = ALPHAR[ 0 ] / BETA[ 0 ];
	const eig1 = ALPHAR[ 1 ] / BETA[ 1 ];
	const eigs = [ eig0, eig1 ].sort();
