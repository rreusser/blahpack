...
import zhetrsRook from './../lib/zhetrs_rook.js';
...
test( 'zhetrs_rook column-major small solve', function t() {
	...
	const info = zhetrsRook( 'column-major', 'upper', 1, 1, A, 1, ipiv, 1, b, 1 );
...
test( 'zhetrs_rook row-major small solve', function t() {
	...
	const b = new Complex128Array( [ 1.0, 1.0 ] );
	const ipiv = new Int32Array( [ 0 ] );
	const info = zhetrsRook( 'row-major', 'upper', 1, 1, A, 1, ipiv, 1, b, 1 );
	assert.equal( info, 0, 'info' );
});
