...
import zlange from './../lib/ndarray.js';


// FIXTURES //

const fixtureFile = path.join( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures', 'zlange.jsonl' );
...
// 2x2 complex matrix
const A_2X2 = buildA( LDA, 4, [
	1, 1, 1.0, 2.0, 2, 1, 3.0, 4.0,
	1, 2, 5.0, 6.0, 2, 2, 7.0, 8.0
]);

// 1x1 complex matrix
const A_1X1 = buildA( LDA, 1, [ 1, 1, 3.0, 4.0 ] );
...
function runCase( name, norm, M, N, A ) {
	const WORK = new Float64Array( Math.max( M, 1 ) );
	const tc = findCase( name );
	const got = zlange( norm, M, N, A, 1, LDA, 0, WORK, 1, 0 );
...
test( 'zlange: 1x1 max norm', function t() {
	...
	// |3+4i| = 5
	const got = zlange( 'max', 1, 1, A_1X1, 1, LDA, 0, WORK, 1, 0 );
	assertClose( got, 5.0, 1e-12, 'zlange: 1x1 max' );
});

test( 'zlange: 1x1 one-norm', function t() {
	const WORK = new Float64Array( 1 );
	const got = zlange( 'one-norm', 1, 1, A_1X1, 1, LDA, 0, WORK, 1, 0 );
	assertClose( got, 5.0, 1e-12, 'zlange: 1x1 one' );
});

test( 'zlange: 1x1 inf-norm', function t() {
	const WORK = new Float64Array( 1 );
	const got = zlange( 'inf-norm', 1, 1, A_1X1, 1, LDA, 0, WORK, 1, 0 );
