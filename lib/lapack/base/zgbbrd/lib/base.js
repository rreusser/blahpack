...
function zgbbrd( vect, M, N, ncc, kl, ku, AB, strideAB1, strideAB2, offsetAB, d, strideD, offsetD, e, strideE, offsetE, Q, strideQ1, strideQ2, offsetQ, PT, stridePT1, stridePT2, offsetPT, C, strideC1, strideC2, offsetC, WORK, strideWork, offsetWork, RWORK, strideRWork, offsetRWork ) {
	let rbArr, inca, abst, kuml, kumu, newR, newI, kb1, klm, kun, ml0, mu0, nrt;
	let ipR, ipS, rbR, rbI, rbV, csR, csI, kb, nr, j1, j2, ml, mu, kk, ip, tR;
	let tI, t0, t1, oi, i, j, l;

	const wantb = ( vect === 'both' );
	const wantq = ( vect === 'q-only' ) || wantb;
	const wantpt = ( vect === 'p-only' ) || wantb;
	const wantc = ( ncc > 0 );
	const klu1 = kl + ku + 1;
	...
	const sAB1 = strideAB1 * 2;
	const sAB2 = strideAB2 * 2;
	const oAB = offsetAB * 2;

	const WORKv = reinterpret( WORK, 0 );
	const sWORK = strideWork * 2;
	const oWORK = offsetWork * 2;

	const sD = strideD;
	const sE = strideE;
	const sRW = strideRWork;
	const oRW = offsetRWork;

	// Scratch for zlartg RA output
	const raArr = new Complex128Array( 1 );
	const raV = reinterpret( raArr, 0 );

	// Scratch for zrot S argument (real 2-element [re, im])
	const svec = new Float64Array( 2 );
