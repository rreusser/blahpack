...
function dsyev( jobz, uplo, N, A, strideA1, strideA2, offsetA, w, strideW, offsetW, WORK, strideWork, offsetWork ) {
	...
	// from an out-of-bounds read. WORK is partitioned into E (N), TAU (N), and a
	// scratch segment (>= N-1), so the minimum requirement is max( 1, 3*N-1 ).
	const need = Math.max( 1, ( 3 * N ) - 1 );
