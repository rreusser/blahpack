...
import dgeqrfp from './../lib/ndarray.js';


// FIXTURES //

import fixture3x3 from './fixtures/3x3.json' with { type: 'json' };
import fixture4x3 from './fixtures/4x3.json' with { type: 'json' };
import fixtureNZero from './fixtures/n_zero.json' with { type: 'json' };
import fixtureLarge from './fixtures/large_65x65.json' with { type: 'json' };
...
test( 'dgeqrfp: 3x3', function t() {
	...
	const info = dgeqrfp( 3, 3, A, 1, 3, 0, TAU, 1, 0, WORK, 1, 0 );
...
test( 'dgeqrfp: 4x3', function t() {
	...
	const TAU = new Float64Array( 3 );
	const WORK = new Float64Array( 3 * 32 );
	const info = dgeqrfp( 4, 3, A, 1, 4, 0, TAU, 1, 0, WORK, 1, 0 );
	assert.strictEqual( info, tc.INFO );
...
test( 'dgeqrfp: N=0', function t() {
	...
	const tc = fixtureNZero;
	const A = new Float64Array( 9 );
	const TAU = new Float64Array( 3 );
	const WORK = new Float64Array( 32 );
	const info = dgeqrfp( 3, 0, A, 1, 3, 0, TAU, 1, 0, WORK, 1, 0 );
	assert.strictEqual( info, tc.INFO );
});
...
test( 'dgeqrfp: M=0', function t() {
	...
	const TAU = new Float64Array( 3 );
	const WORK = new Float64Array( 32 );
	const info = dgeqrfp( 0, 3, A, 1, 3, 0, TAU, 1, 0, WORK, 1, 0 );
	assert.strictEqual( info, 0 );
});

test( 'dgeqrfp: throws RangeError when WORK is null', function t() {
	assert.throws( function() {
		dgeqrfp( 3, 3, new Float64Array( 9 ), 1, 3, 0, new Float64Array( 3 ), 1, 0, null, 1, 0 );
	}, RangeError );
});

test( 'dgeqrfp: throws RangeError when WORK is too small', function t() {
	assert.throws( function() {
		dgeqrfp( 3, 3, new Float64Array( 9 ), 1, 3, 0, new Float64Array( 3 ), 1, 0, new Float64Array( 1 ), 1, 0 );
	}, RangeError );
});
...
test( 'dgeqrfp: large 65x65 (blocked path)', function t() {
	...
	const info = dgeqrfp( M, N, A, 1, LDA, 0, TAU, 1, 0, WORK, 1, 0 );
	assert.strictEqual( info, tc.INFO );
