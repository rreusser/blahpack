...
//
// These rank-1 updates are memory-bound (one read + one write of A per
// element for two flops), so the ceiling is streaming bandwidth, not
// arithmetic. `alpha` is kept tiny so repeated in-place updates cannot grow
// A without bound across timing iterations.
...
for ( const n of [ 500, 2000 ] ) {
	for ( const layout of [ 'col', 'row' ] ) {
		...
		// A is read once and written once per call:
		const gbs = ( ms ) => ( 16 * n * n ) / ( ms * 1e6 );
		console.log(
			( n + 'x' + n + ' ' + layout ).padEnd( 34 ) +
