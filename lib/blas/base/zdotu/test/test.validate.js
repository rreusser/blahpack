...
// L2 property: got == sum_i x_i*y_i (no conjugation), validated against an independent inline oracle across a size sweep (incl N=0,1).
test( 'zdotu: unconjugated dot product equals sum_i x_i*y_i (property)', function t() {
	checked( 'zdotu', 'property', function run() {
		SIZES.forEach( function eachN( N ) {
			...
			const got = val( zdotu( N, X.data, X.args[ 0 ], X.args[ 1 ], Y.data, Y.args[ 0 ], Y.args[ 1 ] ) );

			// Independent oracle: naive ascending sum of x_i*y_i:
			let exp = sc.zero;
			for ( i = 0; i < N; i++ ) {
				exp = sc.add( exp, sc.mul( xv[ i ], yv[ i ] ) );
			}
			check.assertFinite( sc, [ got ], 'zdotu N='+N );
			check.assertScaled( sc.abs( sc.sub( got, exp ) ), sc.abs( exp ), check.tol( N, 20 ), 'zdotu N='+N );
		});
	});
});

// L3 layout invariance: output is bit-exact across all vector layouts (padded / gapped / negative strides).
test( 'zdotu: output is bit-exact across storage layouts', function t() {
	const N = 17;
	const SEED = 0x2D0D;
	const vL = schemes.vectorLayouts();
	checked( 'zdotu', 'layout-invariance', function run() {
