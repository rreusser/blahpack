...
function zunmbr( vect, side, trans, M, N, K, A, strideA1, strideA2, offsetA, TAU, strideTAU, offsetTAU, C, strideC1, strideC2, offsetC, WORK, strideWork, offsetWork ) {
	...
	// Caller owns the workspace; assert it is sufficiently large so an under-sized (or non-array) buffer is a loud RangeError, not a silent NaN. ZUNMBR (unblocked) needs N (side='left') or M (side='right') complex elements.
	need = ( side === 'left' ) ? N : M;
