...
function dgtsvSolve( A0, N, k, code, Rhs ) {
	...
	var info = dgtsv( N, k, DL.data, DL.args[ 0 ], DL.args[ 1 ], D.data, D.args[ 0 ], D.args[ 1 ], DU.data, DU.args[ 0 ], DU.args[ 1 ], Br.data, Br.args[ 0 ], Br.args[ 1 ], Br.args[ 2 ] );
	if ( info !== 0 ) {
...
function opInverse( A0, N, code ) {
	var Ir = dgtsvSolve( A0, N, N, code, identity( N ) );
...
function runProperty( trans, N, nrhs ) {
	...
	var out = callGtsvx( 'not-factored', trans, N, nrhs, A0, B0, TIGHT, DTIGHT );
	var tag = 'dgtsvx '+trans+' N='+N+' nrhs='+nrhs;
	...
	checked( 'dgtsvx', 'structural', function run() {
		var Xtrue = dgtsvSolve( A0, N, nrhs, code, B0 );
...
function runInvariance( trans ) {
	...
	checked( 'dgtsvx', 'layout-invariance', function run() {
		layoutInvariant( VL, function build( vL, idx ) {
			...
			var out = callGtsvx( 'not-factored', trans, N, nrhs, A0, B0, vL, dL );
