...
import dgeqrt3 from './../lib/dgeqrt3.js';
...
test( 'dgeqrt3 succeeds with column-major layout', function t() {
	const A = new Float64Array( [ 2.0, 1.0, 0.5, 0.5, 3.0, 1.5 ] ); // 3x2 col-major
	const T = new Float64Array( 4 );
	const info = dgeqrt3( 'column-major', 3, 2, A, 3, T, 2 );
	assert.strictEqual( info, 0, 'returns 0' );
});

test( 'dgeqrt3 succeeds with row-major layout', function t() {
	const A = new Float64Array( [ 2.0, 0.5, 1.0, 3.0, 0.5, 1.5 ] ); // 3x2 row-major
	const T = new Float64Array( 4 );
	const info = dgeqrt3( 'row-major', 3, 2, A, 2, T, 2 );
	assert.strictEqual( info, 0, 'returns 0' );
});
