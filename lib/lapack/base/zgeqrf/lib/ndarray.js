...
function zgeqrf( M, N, A, strideA1, strideA2, offsetA, TAU, strideTAU, offsetTAU, WORK, strideWork, offsetWork ) {
	...
	// NaN from an out-of-bounds read. With NB=32, the blocked path (min(M,N)>NB)
	// needs `N*NB + NB*NB` complex elements; otherwise `N` suffice.
	const need = ( Math.min( M, N ) > 32 ) ? ( ( N*32 ) + ( 32*32 ) ) : N;
