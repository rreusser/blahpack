...
const rawLines = readFileSync( path.join( fixtureDir, 'dlasyf_rk.jsonl' ), 'utf8' ).trim().split( '\n' ); // eslint-disable-line node/no-sync, max-len
...
test( 'dlasyf_rk: 6x6_lower_nb3', function t() {
	...
	const tc = findCase( '6x6_lower_nb3' );
	...
	const W = new Float64Array( 18 );
	const E = new Float64Array( 6 );
	const result = dlasyfRk( 'lower', 6, 3, A, 1, 6, 0, E, 1, 0, ipiv, 1, 0, W, 1, 6, 0 );
...
test( 'dlasyf_rk: 6x6_upper_nb3', function t() {
	...
	const tc = findCase( '6x6_upper_nb3' );
	const A = new Float64Array( 36 );
	A[ 0 ] = 2;
	A[ 6 ] = -1;
	A[ 7 ] = 3;
	...
	const W = new Float64Array( 18 );
	const E = new Float64Array( 6 );
	const result = dlasyfRk( 'upper', 6, 3, A, 1, 6, 0, E, 1, 0, ipiv, 1, 0, W, 1, 6, 0 );
	assertArrayClose( A, tc.a, 1e-13, 'a' );
	assertArrayClose( E, tc.e, 1e-13, 'e' );
	assert.equal( result.info, tc.info, 'info' );
	assert.equal( result.kb, tc.kb, 'kb' );
...
test( 'dlasyf_rk: 6x6_indef_lower_nb3', function t() {
	...
	const tc = findCase( '6x6_indef_lower_nb3' );
	const A = new Float64Array( 36 );
	A[ 0 ] = 0;
	A[ 1 ] = 3;
	A[ 2 ] = 1;
...
test( 'dlasyf_rk: 6x6_indef_upper_nb3', function t() {
	...
	const tc = findCase( '6x6_indef_upper_nb3' );
	const A = new Float64Array( 36 );
	A[ 0 ] = 0;
	A[ 6 ] = 3;
	A[ 7 ] = 0;
...
test( 'dlasyf_rk: 4x4_lower_full_nb8', function t() {
	...
	const tc = findCase( '4x4_lower_full_nb8' );
	...
	const result = dlasyfRk( 'lower', 4, 8, A, 1, 4, 0, E, 1, 0, ipiv, 1, 0, W, 1, 4, 0 );
	assertArrayClose( A, tc.a, 1e-13, 'a' );
	assertArrayClose( E, tc.e, 1e-13, 'e' );
	assert.equal( result.info, tc.info, 'info' );
	assert.equal( result.kb, tc.kb, 'kb' );
...
test( 'dlasyf_rk: 4x4_upper_full_nb8', function t() {
	...
	const tc = findCase( '4x4_upper_full_nb8' );
	const A = new Float64Array( 16 );
	A[ 0 ] = 4;
	A[ 4 ] = 1;
	A[ 5 ] = 3;
	...
	const W = new Float64Array( 32 );
	const E = new Float64Array( 4 );
	const result = dlasyfRk( 'upper', 4, 8, A, 1, 4, 0, E, 1, 0, ipiv, 1, 0, W, 1, 4, 0 );
	assertArrayClose( A, tc.a, 1e-13, 'a' );
	assertArrayClose( E, tc.e, 1e-13, 'e' );
	assert.equal( result.info, tc.info, 'info' );
	assert.equal( result.kb, tc.kb, 'kb' );
