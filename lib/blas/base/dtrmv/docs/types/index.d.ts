...
interface Routine {
	...
	*/
	...
	/**
	* Performs one of the matrix-vector operations `x := A*x` or `x := A**T*x` where `A` is an `N` by `N` upper or lower triangular matrix using alternative indexing semantics.
	*
	* @param uplo - specifies whether the upper or lower triangular part is referenced
	* @param trans - specifies whether the matrix should be transposed
	* @param diag - specifies whether the matrix is unit triangular
