...
* @param {Complex128Array} WORK - caller-owned complex workspace of at least `2*N` elements; never allocated internally
...
* @param {Float64Array} RWORK - caller-owned real workspace of at least `N` elements; never allocated internally
...
function zsprfs( uplo, N, nrhs, AP, strideAP, offsetAP, AFP, strideAFP, offsetAFP, IPIV, strideIPIV, offsetIPIV, B, strideB1, strideB2, offsetB, X, strideX1, strideX2, offsetX, FERR, strideFERR, offsetFERR, BERR, strideBERR, offsetBERR, WORK, strideWork, offsetWork, RWORK, strideRWork, offsetRWork ) { // eslint-disable-line max-len, max-params
	...
	// an empty problem requires no workspace (base fills FERR/BERR with zeros).
	if ( N === 0 || nrhs === 0 ) {
	...
	// WORK holds 2*N complex elements; RWORK holds N real elements.
