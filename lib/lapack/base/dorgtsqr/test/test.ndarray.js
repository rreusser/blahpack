...
import dlatsqr from './../../dlatsqr/lib/base.js';
import dorgtsqr from './../lib/ndarray.js';


// FIXTURES //

const fixtureDir = path.join( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures' );
const fixtureRaw = readFileSync( path.join( fixtureDir, 'dorgtsqr.jsonl' ), 'utf8' ); // eslint-disable-line node/no-sync
...
function runCase( cfg ) {
	let info;
	const fc = findCase( cfg.name );
	const nblk = numBlocks( cfg.M, cfg.N, cfg.mb );
	const TC = nblk * cfg.N;
	const nblocal = ( cfg.nb < cfg.N ) ? cfg.nb : cfg.N;
	const Aco = buildA( cfg.M, cfg.N, cfg.fill );
	const Tco = new Float64Array( cfg.nb * TC );

	// Factor A with dlatsqr to populate the reflectors and T factors.
	const work = new Float64Array( cfg.nb * cfg.N );
	...
	const Qp = pack( Aco, cfg.M, cfg.N, cfg.M );
...
test( 'dorgtsqr: m0_n0_quickreturn — quick return for M=0,N=0', function t() {
	...
	const info = dorgtsqr( 0, 0, 4, 1, A, 1, 1, 0, T, 1, 1, 0, W, 1, 0 );
...
test( 'dorgtsqr: m5_n0_quickreturn — quick return for N=0', function t() {
	...
	const T = new Float64Array( 4 );
	const W = new Float64Array( 4 );
	const info = dorgtsqr( 5, 0, 4, 1, A, 1, 5, 0, T, 1, 1, 0, W, 1, 0 );
	assert.equal( info, 0, 'returns 0' );
});
...
test( 'dorgtsqr: nb > N — clamped internally so the result still matches', function t() {
	...
	let info;
	const Aco = buildA( 10, 2, fill2 );
	const Tco = new Float64Array( 2 * numBlocks( 10, 2, 4 ) * 2 );
	const work = new Float64Array( 2 * 2 );
