/**
* Property-based validation for zungbr, following the /blahpack-validate process.
*
* Step 0 classification: `d` -> real scalar; `or`/`gbr` -> BLOCKED formation of the
* orthogonal factor Q or Pᴴ from a bidiagonal reduction (zgebrd). zgebrd reduces a
* general M0-by-N0 matrix A0 to bidiagonal B by Qᴴ·A0·P = B, i.e. A0 = Q·B·Pᴴ,
* leaving the Householder reflectors (H for Q, G for P) in A0 and their scalar
* factors in TAUQ/TAUP. zungbr CONSUMES that factorization:
...
*   - VECT='P' (from TAUP): forms Pᴴ. The original was K-by-N0, K = M0 (its row
*     count). If K < N0 it delegates to dorglq(M,N,K); if K >= N0 it shifts one row
*     down and forms Pᴴ(2:n,2:n) via dorglq(N-1,N-1,N-1) — Pᴴ is N0-by-N0.
*
* K convention (confirmed from zungbr.f): VECT='Q' -> K = N0; VECT='P' -> K = M0.
*
* Oracles (INDEPENDENT of the reflector algebra):
*   (a) orthonormality — the formed Q has orthonormal columns and the formed Pᴴ has
*       orthonormal rows/cols (it is unitary/orthogonal, up to the economy shape);
*   (b) reconstruction — with BOTH Q and Pᴴ formed from the SAME zgebrd factor plus
*       the bidiagonal B (from d,e), A0 = Q · B · Pᴴ. This cross-checks zungbr
*       against the reduction it inverts (mirrors the zgebrd A=Q·B·Pᴴ validation but
*       with zungbr-FORMED Q and Pᴴ instead of hand-rolled reflector products).
*
* Because zungbr returns Pᴴ directly (complex: unitary), the complex ZLACGV
* conjugation subtlety is handled INTERNALLY by the zunglq/zgebrd pair — the
* reconstruction uses Pᴴ with NO manual conjugation (real zgebrd is unaffected
* either way).
*
* Economy shapes used here (mn = min(M0,N0)):
*   upper (M0>=N0): Q is M0-by-N0 (dorgqr(M0,N0,N0)); Pᴴ is N0-by-N0
*                   (zungbr('P',N0,N0,M0)); B core is N0-by-N0 UPPER bidiagonal.
*   lower (M0<N0):  Q is M0-by-M0 (zungbr('Q',M0,M0,N0)); Pᴴ is M0-by-N0
...
import zungbr from './../lib/ndarray.js';
import zgebrd from '../../zgebrd/lib/ndarray.js';

var sc = S.complex; // z-routine
var LogicalMatrix = logical.LogicalMatrix;
var NB = 32; // hardcoded default block size in the org/gebrd kernels
...
var WORK_LAYOUTS = [ { 'stride': 1, 'lead': 0, 'tail': 0 }, { 'stride': 1, 'lead': 3, 'tail': 2 }, { 'stride': 1, 'lead': 1, 'tail': 0 }, { 'stride': 1, 'lead': 5, 'tail': 4 } ];


// HELPERS //

// zgebrd WORK (blocked path needs (M+N)*NB; unblocked needs max(M,N)).
function workLenBrd( M, N ) {
	return Math.max( 1, ( M + N ) * NB, M, N );
}

// A generously-sized WORK for a zungbr call of shape (Mo,No): the internal blocked
...
function readBlock( Ar, Mr, Nr ) {
	var F = new LogicalMatrix( sc, Mr, Nr );
	var i;
	var j;
	for ( j = 0; j < Nr; j++ ) {
		for ( i = 0; i < Mr; i++ ) {
...
function ctranspose( Mtx ) {
	var T = new LogicalMatrix( sc, Mtx.cols, Mtx.rows );
	var i;
	var j;
...
function buildB( dvals, evals, mn, upper ) {
	var B = new LogicalMatrix( sc, mn, mn );
	var i;
	for ( i = 0; i < mn; i++ ) {
...
// zungbr call-shape (Mo,No,Ko) for forming Q from an (M0,N0) zgebrd factor.
...
// zungbr call-shape (Mo,No,Ko) for forming Pᴴ from an (M0,N0) zgebrd factor.
function pShape( M0, N0 ) {
	if ( M0 >= N0 ) {
		return { 'Mo': N0, 'No': N0, 'Ko': M0 }; // Pᴴ (N0-by-N0)
	}
	return { 'Mo': M0, 'No': N0, 'Ko': M0 }; // economy Pᴴ (M0-by-N0)
}


// DRIVER: freeze one zgebrd factorization at tight col-major. //
...
function freezeFactor( M0, N0, seed ) {
	...
	var Wr = schemes.realizeVector( sc, poison( workLenBrd( M0, N0 ) ), TIGHT_VEC );
...
// Form Q (or Pᴴ) by re-realizing the frozen reflectors + tau at the given layout
// and running ONLY zungbr.
function formFactor( vect, F, M0, N0, layout, tauLayout, workLayout ) {
	var sh = ( vect === 'apply-Q' ) ? qShape( M0, N0 ) : pShape( M0, N0 );
	var tau = ( vect === 'apply-Q' ) ? F.tauq : F.taup;
	var Ar = schemes.dense.realize( sc, F.Frozen, { 'part': 'full' }, layout );
	var Tr = schemes.realizeVector( sc, tau, tauLayout || TIGHT_VEC );
	var Wr = schemes.realizeVector( sc, poison( workLenOrg( sh.Mo, sh.No ) ), workLayout || TIGHT_VEC );
	zungbr( vect, sh.Mo, sh.No, sh.Ko, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], Tr.data, Tr.args[ 0 ], Tr.args[ 1 ], Wr.data, Wr.args[ 0 ], Wr.args[ 1 ] );
	return readBlock( Ar, sh.Mo, sh.No );
}


// Steps 2/3/5: orthonormality of Q and Pᴴ + reconstruction A0 = Q·B·Pᴴ across the
// (M0,N0) sweep (upper + lower) and every dense storage layout.
test( 'zungbr: Q,Pᴴ orthonormal and A0 = Q·B·Pᴴ ((M0,N0) sweep x all layouts)', function t() {
	PAIRS.forEach( function eachPair( pr ) {
		var M0 = pr[ 0 ];
		var N0 = pr[ 1 ];
		var upper = ( M0 >= N0 );
		ALL_LAYOUTS.forEach( function eachLayout( layout, li ) {
			var F = freezeFactor( M0, N0, 0x100 + ( M0 * 100 ) + N0 );
			var label = 'zungbr M0=' + M0 + ' N0=' + N0 + ' (' + ( upper ? 'upper' : 'lower' ) + ') layout=' + li;

			var Q = formFactor( 'apply-Q', F, M0, N0, layout, null, null );
			var Pt = formFactor( 'apply-P', F, M0, N0, layout, null, null );

			checked( 'zungbr', 'orthonormal', function run() {
				assertOrtho( Q, label + ' Q' );
			} );
			checked( 'zungbr', 'orthonormal', function run() {
				assertOrtho( Pt, label + ' Pᴴ' );
			} );

			var B = buildB( F.dvals, F.evals, F.mn, upper );
			var recon = refMatmul( sc, refMatmul( sc, Q, B, {} ), Pt, {} );
			checked( 'zungbr', 'reconstruct', function run() {
...
// zungbr. dlarft/dlarfb -> dgemm/dtrmm pick their summation form from operand
...
test( 'zungbr: bit-exact within storage-order family (col / row), Q + Pᴴ, upper + lower', function t() {
	...
	[ [ 40, 36 ], [ 36, 40 ] ].forEach( function eachShape( pr ) {
		...
		var F = freezeFactor( M0, N0, 0xF00D + M0 + N0 );
	...
	function runFamily( variants, fam, F, M0, N0, vect, tag ) {
		var vlab = ( vect === 'apply-Q' ) ? 'Q' : 'Pᴴ';
		checked( 'zungbr', 'layout-invariance', function run() {
			layoutInvariant( variants, function build( layout, i ) {
				var G = formFactor( vect, F, M0, N0, layout, VEC_LAYOUTS[ i % VEC_LAYOUTS.length ], WORK_LAYOUTS[ i % WORK_LAYOUTS.length ] );
				return check.flattenLogical( sc, G );
			}, { 'label': 'zungbr layout invariance ' + fam + '-major ' + vlab + ' ' + tag + ' (M0=' + M0 + ' N0=' + N0 + ')' } );
		} );
	}
} );


// Step 4c: WORKSPACE CONFORMANCE (plain assertion, NOT `checked`). zungbr's wrapper
...
test( 'zungbr: advertised WORK minimum suffices on the blocked path (Step 4c)', function t() {
	...
	[
		...
		{ 'M0': 64, 'N0': 80, 'vect': 'apply-P' }, // lower Pᴴ -> dorglq(64,80,64)
		{ 'M0': 80, 'N0': 64, 'vect': 'apply-P' }  // upper Pᴴ -> dorglq(63,63,63)
	].forEach( function eachCase( cs ) {
		var M0 = cs.M0;
		var N0 = cs.N0;
		var vect = cs.vect;
		var vlab = ( vect === 'apply-Q' ) ? 'Q' : 'Pᴴ';
		var label = 'zungbr WORK-min ' + vlab + ' M0=' + M0 + ' N0=' + N0;
		var sh = ( vect === 'apply-Q' ) ? qShape( M0, N0 ) : pShape( M0, N0 );
		var F = freezeFactor( M0, N0, 0xB10C + ( M0 * 7 ) + N0 );

		function run( len ) {
			var Ar = schemes.dense.realize( sc, F.Frozen, { 'part': 'full' }, null );
			var Tr = schemes.realizeVector( sc, ( vect === 'apply-Q' ) ? F.tauq : F.taup, TIGHT_VEC );
			var Wo = poisonedWork( sc, len );
			zungbr( vect, sh.Mo, sh.No, sh.Ko, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], Tr.data, Tr.args[ 0 ], Tr.args[ 1 ], Wo, 1, 0 );
