...
import dorbdb2 from './../lib/ndarray.js';
...
function runFixtureCase( name, M, P, Q, hasPhi, hasTaup1 ) {
	...
	const info = dorbdb2( M, P, Q, X11, 1, LDX_PAD, 0, X21, 1, LDX_PAD, 0, THETA, 1, 0, PHI, 1, 0, TAUP1, 1, 0, TAUP2, 1, 0, TAUQ1, 1, 0, WORK, 1, 0 );
...
test( 'dorbdb2.ndarray: p0_only_second_loop (P=0 — first loop never runs; only second loop runs)', function t() {
	...
	const tc = findCase( 'p0_only_second_loop' );
	const X11 = new Float64Array( 1 );
	const X21 = buildBuffer( tc.X21in, 6, 3 );
	...
	const info = dorbdb2( 6, 0, 3, X11, 1, 1, 0, X21, 1, LDX_PAD, 0, THETA, 1, 0, PHI, 1, 0, TAUP1, 1, 0, TAUP2, 1, 0, TAUQ1, 1, 0, WORK, 1, 0 );
...
test( 'dorbdb2.ndarray: q0_quick_return — both loops never execute', function t() {
	...
	const info = dorbdb2( 4, 0, 0, X11, 1, 1, 0, X21, 1, LDX_PAD, 0, THETA, 1, 0, PHI, 1, 0, TAUP1, 1, 0, TAUP2, 1, 0, TAUQ1, 1, 0, WORK, 1, 0 );
	assert.strictEqual( info, 0, 'info' );
});
