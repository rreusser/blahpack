/**
* Property-based validation for zporfs, following the /blahpack-validate process.
*
...
* three independent properties. zporfs refines an approximate solution X to
...
* zpotrf; the un-refined initial X by zpotrs; the TRUE solution independently by
* the trusted zposv. We assert, against the ORIGINAL full Hermitian A0:
...
import zporfs from './../lib/ndarray.js';
import zpotrf from '../../zpotrf/lib/ndarray.js';
import zpotrs from '../../zpotrs/lib/ndarray.js';
...
test( 'zporfs: refinement residual + BERR + FERR bound (uplo x N x nrhs)', function t() {
...
function runProperty( uplo, N, nrhs ) {
	...
	// A = original HPD (uplo triangle); AF = its Cholesky factor:
	const Ar = schemes.dense.realize( sc, A0, { 'part': uplo }, TIGHT );
	const AFr = schemes.dense.realize( sc, A0, { 'part': uplo }, TIGHT );
	zpotrf( uplo, N, AFr.data, AFr.args[ 0 ], AFr.args[ 1 ], AFr.args[ 2 ] );

	// B (RHS, unchanged) and X (initial un-refined solve, refined in place):
	const Br = schemes.dense.realize( sc, B0, { 'part': 'full' }, TIGHT );
	const Xr = schemes.dense.realize( sc, B0, { 'part': 'full' }, TIGHT );
	zpotrs( uplo, N, nrhs, AFr.data, AFr.args[ 0 ], AFr.args[ 1 ], AFr.args[ 2 ], Xr.data, Xr.args[ 0 ], Xr.args[ 1 ], Xr.args[ 2 ] );
	...
	zporfs( uplo, N, nrhs, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], AFr.data, AFr.args[ 0 ], AFr.args[ 1 ], AFr.args[ 2 ], Br.data, Br.args[ 0 ], Br.args[ 1 ], Br.args[ 2 ], Xr.data, Xr.args[ 0 ], Xr.args[ 1 ], Xr.args[ 2 ], FERR, 1, 0, BERR, 1, 0, WORK, 1, 0, RWORK, 1, 0 );

	const Xtrue = trueSolution( A0, B0, N, nrhs, uplo );

	const tag = 'zporfs '+uplo+' N='+N+' nrhs='+nrhs;

	// (a) residual: refined X remains a valid solution of A0*X = B0.
	checked( 'zporfs', 'residual', function run() {
		let j;
		for ( j = 0; j < nrhs; j++ ) {
	...
	checked( 'zporfs', 'structural', function run() {
...
// fixed A0 and B0; zporfs's internal kernels (zhemv, zpotrs, zaxpy, zlacn2) run
...
test( 'zporfs: bit-exact within storage-order family (col / row)', function t() {
	UPLOS.forEach( function eachUplo( uplo ) {
...
function runInvariance( uplo, variants, fam ) {
	...
	zpotrf( uplo, N, AF0.data, AF0.args[ 0 ], AF0.args[ 1 ], AF0.args[ 2 ] );
	...
	zpotrs( uplo, N, nrhs, AF0.data, AF0.args[ 0 ], AF0.args[ 1 ], AF0.args[ 2 ], X0r.data, X0r.args[ 0 ], X0r.args[ 1 ], X0r.args[ 2 ] );
	const Xinit = readMat( X0r, N, nrhs );

	checked( 'zporfs', 'layout-invariance', function run() {
		layoutInvariant( variants, function build( layout ) {
			...
			return out;
		}, { 'label': 'zporfs '+uplo+' layout invariance '+fam+'-major' } );
	});
}
