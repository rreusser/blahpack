...
const raw = readFileSync( path.join( fixtureDir, 'zgelqt.jsonl' ), 'utf8' ); // eslint-disable-line node/no-sync
...
function runCase( caseName, M, N, mb, rows, tol ) {
	...
	const info = zgelqt( M, N, mb, A, 1, M, 0, T, 1, mb, 0, WORK, 0 );
...
test( 'zgelqt: M=0 quick return', function t() {
	...
	const info = zgelqt( 0, 4, 1, A, 1, 1, 0, T, 1, 1, 0, WORK, 0 );
...
test( 'zgelqt: N=0 quick return', function t() {
	...
	const T = new Complex128Array( 4 );
	const WORK = new Complex128Array( 4 );
	const info = zgelqt( 4, 0, 1, A, 1, 4, 0, T, 1, 1, 0, WORK, 0 );
	assert.strictEqual( info, 0 );
});
...
test( 'zgelqt: works with non-trivial offsets and strides (column-major)', function t() {
	...
	const T = new Complex128Array( ( LDT * N ) + offsetT );
	const WORK = new Complex128Array( mb * N );
	...
	const info = zgelqt( M, N, mb, A, 1, LDA, offsetA, T, 1, LDT, offsetT, WORK, 0 );
	assert.strictEqual( info, 0 );

	// Compare against the compact column-major fixture (M-by-N for A; mb-by-K for T).
	Aview = reinterpret( A, 0 );
	const Tview = reinterpret( T, 0 );
	const Aexp = tc.A;
	const Texp = tc.T;
	for ( j = 0; j < N; j++ ) {
		for ( i = 0; i < M; i++ ) {
...
test( 'zgelqt: works on a row-major matrix layout', function t() {
	...
	const info = zgelqt( M, N, mb, A, N, 1, 0, T, k, 1, 0, WORK, 0 );
	assert.strictEqual( info, 0 );
	...
	const Texp = new Float64Array( 2 * mb * k );
	for ( j = 0; j < k; j++ ) {
		for ( i = 0; i < mb; i++ ) {
