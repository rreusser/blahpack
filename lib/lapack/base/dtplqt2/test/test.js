...
import dtplqt2 from './../lib/base.js';


// FIXTURES //

const fixtureDir = path.join( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures' ); // eslint-disable-line max-len
const lines = readFileSync( path.join( fixtureDir, 'dtplqt2.jsonl' ), 'utf8' ).trim().split( '\n' ); // eslint-disable-line node/no-sync
...
function loadMat( packed, M, N, LD ) {
	const out = new Float64Array( LD * Math.max( 1, N ) );
	let i, j;
	for ( j = 0; j < N; j++ ) {
		for ( i = 0; i < M; i++ ) {
...
function runCase( name, M, N, l, Ain, Bin ) {
	...
	const LD = 6;
	const A = loadMat( Ain, M, M, LD );
	const B = loadMat( Bin, M, N, LD );
	const T = new Float64Array( LD * Math.max( 1, M ) );

	const info = dtplqt2( M, N, l, A, 1, LD, 0, B, 1, LD, 0, T, 1, LD, 0 );
...
test( 'dtplqt2: m_zero (quick return)', function t() {
	...
	const info = dtplqt2( 0, 3, 0, A, 1, 6, 0, B, 1, 6, 0, T, 1, 6, 0 );
	assert.equal( info, 0 );
});
...
test( 'dtplqt2: n_zero (quick return)', function t() {
	...
	const info = dtplqt2( 3, 0, 0, A, 1, 6, 0, B, 1, 6, 0, T, 1, 6, 0 );
	assert.equal( info, 0 );
});
