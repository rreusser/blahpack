...
test( 'zgeqp3: 4x3 matrix', function t() {
	...
	info = zgeqp3( 4, 3, A, 1, LDA, 0, JPVT, 1, 0, TAU, 1, 0, WORK, 1, 0, RWORK, 1, 0 ); // eslint-disable-line max-len
...
test( 'zgeqp3: 3x4 matrix', function t() {
	...
	WORK = new Complex128Array( 200 );
	RWORK = new Float64Array( 2 * LDA );
	info = zgeqp3( 3, 4, A, 1, LDA, 0, JPVT, 1, 0, TAU, 1, 0, WORK, 1, 0, RWORK, 1, 0 ); // eslint-disable-line max-len
	assertClose( info, tc.info, 1e-14, 'info' );
	assertArrayClose( toArray( Av.subarray( 0, tc.a.length ) ), tc.a, 1e-10, 'a' );
	assertArrayClose( toArray( reinterpret( TAU, 0 ) ), tc.tau, 1e-10, 'tau' );
	assert.deepStrictEqual( toArray( JPVT ), tc.jpvt, 'jpvt' );
...
test( 'zgeqp3: rank-deficient 3x3', function t() {
	...
	WORK = new Complex128Array( 200 );
	RWORK = new Float64Array( 2 * LDA );
	info = zgeqp3( 3, 3, A, 1, LDA, 0, JPVT, 1, 0, TAU, 1, 0, WORK, 1, 0, RWORK, 1, 0 ); // eslint-disable-line max-len
...
test( 'zgeqp3: N=0 quick return', function t() {
	...
	info = zgeqp3( 3, 0, A, 1, 3, 0, JPVT, 1, 0, TAU, 1, 0, WORK, 1, 0, RWORK, 1, 0 ); // eslint-disable-line max-len
	assertClose( info, tc.info, 1e-14, 'info' );
});
...
test( 'zgeqp3: M=0 quick return', function t() {
	...
	WORK = new Complex128Array( 10 );
	RWORK = new Float64Array( 10 );
	info = zgeqp3( 0, 3, A, 1, 1, 0, JPVT, 1, 0, TAU, 1, 0, WORK, 1, 0, RWORK, 1, 0 ); // eslint-disable-line max-len
	assertClose( info, tc.info, 1e-14, 'info' );
});
...
test( 'zgeqp3: 1x1 matrix', function t() {
	...
	WORK = new Complex128Array( 200 );
	RWORK = new Float64Array( 2 * LDA );
	info = zgeqp3( 1, 1, A, 1, LDA, 0, JPVT, 1, 0, TAU, 1, 0, WORK, 1, 0, RWORK, 1, 0 ); // eslint-disable-line max-len
	assertClose( info, tc.info, 1e-14, 'info' );
	assertArrayClose( toArray( Av.subarray( 0, tc.a.length ) ), tc.a, 1e-10, 'a' );
	assertArrayClose( toArray( reinterpret( TAU, 0 ) ), tc.tau, 1e-10, 'tau' );
	assert.deepStrictEqual( toArray( JPVT ), tc.jpvt, 'jpvt' );
...
test( 'zgeqp3: large matrix 35x36 (blocked zlaqps path, sminmn > 32)', function t() { // eslint-disable-line max-len
	...
	info = zgeqp3( M, N, A, 1, BLDA, 0, JPVT, 1, 0, TAU, 1, 0, WORK, 1, 0, RWORK, 1, 0 ); // eslint-disable-line max-len
	assert.equal( info, 0, 'info' );
	prevMag = Infinity;
	minmn = Math.min( M, N );
	for ( i = 0; i < minmn; i++ ) {
