...
import Complex64Array from '@stdlib/array/complex64/lib/index.js';
import reinterpret64 from '@stdlib/strided/base/reinterpret-complex64/lib/index.js';
import zlat2c from './../lib/ndarray.js';


// FIXTURES //

const fixtureDir = path.join( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures' );
const lines = readFileSync( path.join( fixtureDir, 'zlat2c.jsonl' ), 'utf8' ).trim().split( '\n' ); // eslint-disable-line node/no-sync
...
function assertClose( actual, expected, tol, msg ) {
	...
	const relErr = d / Math.max( Math.abs( expected ), 1.0 );
...
function toComplex128( vals ) {
	let i;
	const out = new Complex128Array( vals.length / 2 );
...
test( 'zlat2c: upper 3x3', function t() {
	...
	const A = buildA3();
	const SA = new Complex64Array( 9 );
	const info = zlat2c( 'upper', 3, A, 1, 3, 0, SA, 1, 3, 0 );
...
test( 'zlat2c: lower 3x3', function t() {
	...
	const A = buildA3();
	const SA = new Complex64Array( 9 );
	const info = zlat2c( 'lower', 3, A, 1, 3, 0, SA, 1, 3, 0 );
	assert.equal( info, tc.info, 'info' );
	assertArrayClose( flat64( SA ), tc.sa, 1e-6, 'sa' );
});
...
test( 'zlat2c: n=0 quick return', function t() {
	...
	const A = buildA3();
	const SA = new Complex64Array( 9 );
	const info = zlat2c( 'upper', 0, A, 1, 3, 0, SA, 1, 3, 0 );
	assert.equal( info, tc.info, 'info' );
	assertArrayClose( flat64( SA ), tc.sa, 1e-6, 'sa' );
});
...
test( 'zlat2c: n=1', function t() {
	...
	const A = buildA3();
	const SA = new Complex64Array( 9 );
	const info = zlat2c( 'upper', 1, A, 1, 3, 0, SA, 1, 3, 0 );
	assert.equal( info, tc.info, 'info' );
	assertArrayClose( flat64( SA ), tc.sa, 1e-6, 'sa' );
});

test( 'zlat2c: upper overflow in real part', function t() {
	const tc = findCase( 'overflow_upper_real' );
...
test( 'zlat2c: upper overflow in imaginary part (negative)', function t() {
	const tc = findCase( 'overflow_upper_imag' );
	const A = new Complex128Array( 9 );
	A.set( [ 1.0, 0.5 ], 0 );
	A.set( [ 0.0, -1.0e300 ], 3 );
	A.set( [ 2.0, 0.0 ], 4 );
...
test( 'zlat2c: lower 4x4', function t() {
	...
	const A = buildA4();
	const SA = new Complex64Array( 16 );
	const info = zlat2c( 'lower', 4, A, 1, 4, 0, SA, 1, 4, 0 );
	assert.equal( info, tc.info, 'info' );
	assertArrayClose( flat64( SA ), tc.sa, 1e-6, 'sa' );
});
...
test( 'zlat2c: row-major via swapped strides (upper)', function t() {
	...
	const A = toComplex128( vals );
	const SA = new Complex64Array( 9 );
	const info = zlat2c( 'upper', 3, A, 3, 1, 0, SA, 3, 1, 0 );
