...
*/
function ztfttr( transr, uplo, N, ARF, strideARF, offsetARF, A, strideA1, strideA2, offsetA ) {
	let nisodd, np1x2, nx2, ij, n1, n2, da, k, p, i, j, l;

	// Reinterpret Complex128Arrays as Float64Arrays:
	const Fv = reinterpret( ARF, 0 );
	const Av = reinterpret( A, 0 );

	// Convert complex-element strides/offsets to Float64 strides/offsets:
	const sarf = strideARF * 2;
	const sa1 = strideA1 * 2;
	const sa2 = strideA2 * 2;
	const oARF = offsetARF * 2;
	const oA = offsetA * 2;
	...
	const normalTransr = ( transr === 'no-transpose' );
