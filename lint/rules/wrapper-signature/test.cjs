...
// Tests for the wrapper-signature rule, driven by fixtures. The virtual path is
// `<routine>.js` (the strided wrapper) so the rule selects the file.
//
// Run:  node --test lint/rules/wrapper-signature/test.cjs
...
var RULE_ID = 'blahpack/wrapper-signature';

var linter = new Linter( { 'configType': 'flat', 'cwd': ROOT } );
var config = {
	'plugins': { 'blahpack': { 'rules': { 'wrapper-signature': rule } } },
...
function lintFixture( file ) {
	...
	// The strided wrapper lives at `.../base/<routine>/lib/<routine>.js`.
	var virtual = path.join( ROOT, 'lib', 'lapack', 'base', routine, 'lib', routine + '.js' );
