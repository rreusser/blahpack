/**
* Property-based validation for dormhr, following the /blahpack-validate process.
*
* Step 0 classification: `d` -> real scalar; `ormhr` (BLOCKED apply-Q) -> overwrite
* the M-by-N matrix C with op(Q)*C (side='left') or C*op(Q) (side='right'), where
* Q = H(ilo)…H(ihi-1) is the orthogonal factor of a Hessenberg reduction
* (reflectors + TAU as returned by DGEHRD), and op(Q) is Q (trans='no-transpose')
* or Qᵀ (trans='transpose'). ILO/IHI are 1-BASED; Q has order nq = M (left) or N
* (right), and we validate the full reduction ilo=1, ihi=nq (nh = nq-1 reflectors).
*
* Oracle (INDEPENDENT cross-validation, recorded as 'reconstruct'): form the
* EXPLICIT orthogonal Q from the SAME reflectors via `dorghr` (a separately
* validated routine), then compute op(Q)*C0 / C0*op(Q) with the harness's naive
* `ref.matmul` and compare to dormhr's in-place output. Because dorghr forms Q by
* an independent code path, a systematic reflector/side/trans error in dormhr would
* disagree with the oracle.
*
* dormhr delegates to the BLOCKED dormqr (dlarft + dlarfb, whenever nh > NB=32),
* whose optimized dgemm/dgemv selects its summation form by comparing operand
...
* path with a poisoned buffer (dormhr forwards WORK straight to dormqr, which stores
...
import dormhr from './../lib/ndarray.js';
import dgehrd from '../../dgehrd/lib/ndarray.js';
import dorghr from '../../dorghr/lib/ndarray.js';
...
// WORK dgehrd needs (full reduction ilo=1,ihi=nq): blocked nq*NB + LDT*NB, else nq.
function gehrdWork( nq ) {
	return ( nq > NB ) ? ( ( nq * NB ) + ( LDT * NB ) ) : Math.max( 1, nq );
}

// WORK dorghr advertises: nh*NB (nh = nq-1), else 1.
function orghrWork( nq ) {
	return Math.max( 1, ( nq - 1 ) * NB );
}

// Generous WORK superset for the dormhr apply (covers the blocked dormqr need
...
// Factor an nq×nq matrix with dgehrd. Returns { Fac (LogicalMatrix of the
// reflectors + H), taus }.
function factor( nq, seed ) {
	const k = Math.max( 0, nq - 1 );
	const rng = new RNG( seed );
	const A0 = logical.general( sc, rng, nq, nq );
	const Ar = schemes.dense.realize( sc, A0, { 'part': 'full' }, null );
	const Tr = schemes.realizeVector( sc, poison( k ), TIGHT_VEC );
	dgehrd( nq, 1, nq, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], Tr.data, Tr.args[ 0 ], Tr.args[ 1 ], poisonedWork( sc, gehrdWork( nq ) ), 1, 0 );
	return {
...
// Form the EXPLICIT nq×nq orthogonal Q from a dgehrd factor via the independent
// dorghr. Returns Q as a LogicalMatrix.
function formQ( Fac, taus, nq ) {
	const Qr = schemes.dense.realize( sc, Fac, { 'part': 'full' }, null );
	const Tr = schemes.realizeVector( sc, taus, TIGHT_VEC );
	dorghr( nq, 1, nq, Qr.data, Qr.args[ 0 ], Qr.args[ 1 ], Qr.args[ 2 ], Tr.data, Tr.args[ 0 ], Tr.args[ 1 ], poisonedWork( sc, orghrWork( nq ) ), 1, 0 );
...
// Drive one property case: apply op(Q) to C0 with dormhr and compare to the
// explicit-Q oracle. Returns { got, expected, label }.
function runCase( M, N, side, trans, tcode, layout ) {
	const nq = ( side === 'left' ) ? M : N;
	const active = ( M > 0 && N > 0 && nq > 1 ); // nq<=1 -> nh=0 -> Q = I
	const rng = new RNG( seedFor( M, N, side ) );
	let f, expected, Q;
	...
	const Ar = schemes.dense.realize( sc, f.Fac, { 'part': 'full' }, layout );
	const Tr = schemes.realizeVector( sc, ( f.taus.length ? f.taus : poison( 0 ) ), TIGHT_VEC );

	const C0 = logical.general( sc, rng, M, N );
	const Cr = schemes.dense.realize( sc, C0, { 'part': 'full' }, layout );

	dormhr( side, trans, M, N, 1, nq, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], Tr.data, Tr.args[ 0 ], Tr.args[ 1 ], Cr.data, Cr.args[ 0 ], Cr.args[ 1 ], Cr.args[ 2 ], poisonedWork( sc, ormWork( side, M, N ) ), 1, 0 );
...
// Step 2/5: PROPERTY. op(Q) applied by dormhr == explicit-Q (dorghr) oracle,
// swept over side × trans × (M,N) × dense layouts (all layouts for small/medium
// dims, tight only for large).
test( 'dormhr: op(Q)·C matches explicit-Q oracle (side × trans × dims × layouts)', function t() {
		...
		[ 'left', 'right' ].forEach( function eachSide( side ) {
			[ [ 'no-transpose', 'n' ], [ TRANS_T, TCODE_T ] ].forEach( function eachTrans( tc ) {
				layouts.forEach( function eachLayout( layout, li ) {
					const r = runCase( M, N, side, tc[ 0 ], tc[ 1 ], layout );
...
// tight), then re-realize A and C per layout and run ONLY dormhr; assert bit-exact
// output WITHIN a storage-order family (col / row). The optimized dgemv/dgemm
// inside dlarfb reorders across the col<->row flip, so cross-family equality is
...
test( 'dormhr: bit-exact within storage-order family (col / row), blocked', function t() {
	[ [ 'left', 48, 40 ], [ 'right', 40, 48 ] ].forEach( function eachCase( cfg ) {
		const side = cfg[ 0 ];
		const M = cfg[ 1 ];
		const N = cfg[ 2 ];
		const nq = ( side === 'left' ) ? M : N; // 48 -> nh=47 > NB=32 -> blocked
		const f = factor( nq, seedFor( M, N, side ) );
		const rng = new RNG( seedFor( M, N, side ) + 1 );
		const C0 = logical.general( sc, rng, M, N );
			...
			[ [ colLayouts, 'col' ], [ rowLayouts, 'row' ] ].forEach( function eachFam( famv ) {
				checked( NAME, 'layout-invariance', function run() {
					layoutInvariant( famv[ 0 ], function build( layout, i ) {
						const Ar = schemes.dense.realize( sc, f.Fac, { 'part': 'full' }, layout );
						const Tr = schemes.realizeVector( sc, f.taus, VEC_LAYOUTS[ i % VEC_LAYOUTS.length ] );
						const Cr = schemes.dense.realize( sc, C0, { 'part': 'full' }, layout );
						dormhr( side, tc[ 0 ], M, N, 1, nq, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], Tr.data, Tr.args[ 0 ], Tr.args[ 1 ], Cr.data, Cr.args[ 0 ], Cr.args[ 1 ], Cr.args[ 2 ], poisonedWork( sc, ormWork( side, M, N ) ), 1, 0 );
...
// Step 4c: WORKSPACE CONFORMANCE (plain assertion, NOT `checked`). dormhr forwards
// WORK straight to the BLOCKED dormqr, which stores the block reflector T in a
...
test( 'dormhr: advertised WORK minimum suffices on the blocked path (Step 4c)', function t() {
	[ [ 'left', 48, 40 ], [ 'right', 40, 48 ] ].forEach( function eachCase( cfg ) {
		const side = cfg[ 0 ];
		const M = cfg[ 1 ];
		const N = cfg[ 2 ];
		const nq = ( side === 'left' ) ? M : N; // nh = nq-1 = 47 > NB -> blocked
		...
		function run( len ) {
			const Ar = schemes.dense.realize( sc, f.Fac, { 'part': 'full' }, null );
			const Tr = schemes.realizeVector( sc, f.taus, TIGHT_VEC );
			const Cr = schemes.dense.realize( sc, C0, { 'part': 'full' }, null );
			dormhr( side, trans, M, N, 1, nq, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], Tr.data, Tr.args[ 0 ], Tr.args[ 1 ], Cr.data, Cr.args[ 0 ], Cr.args[ 1 ], Cr.args[ 2 ], poisonedWork( sc, len ), 1, 0 );
			return check.flattenLogical( sc, readFull( Cr, M, N ) );
		}
		...
		const Tr = schemes.realizeVector( sc, f.taus, TIGHT_VEC );
		const Cr = schemes.dense.realize( sc, C0, { 'part': 'full' }, null );
		dormhr( side, trans, M, N, 1, nq, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], Tr.data, Tr.args[ 0 ], Tr.args[ 1 ], Cr.data, Cr.args[ 0 ], Cr.args[ 1 ], Cr.args[ 2 ], poisonedWork( sc, minLen ), 1, 0 );
		const Q = formQ( f.Fac, f.taus, nq );
