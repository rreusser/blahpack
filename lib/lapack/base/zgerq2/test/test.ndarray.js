...
function runZgerq2( M, N, aFlat ) {
	const WORK = new Complex128Array( Math.max( 1, M ) );
	...
	const info = zgerq2( M, N, A, 1, M, 0, TAU, 1, 0, WORK, 1, 0 );
...
test( 'zgerq2: M=0 (quick return)', function t() {
	...
	const info = zgerq2( 0, 3, A, 1, 1, 0, TAU, 1, 0, WORK, 1, 0 );
...
test( 'zgerq2: N=0 (quick return)', function t() {
	...
	const TAU = new Complex128Array( 1 );
	const WORK = new Complex128Array( 1 );
	const info = zgerq2( 3, 0, A, 1, 3, 0, TAU, 1, 0, WORK, 1, 0 );
	assert.equal( info, tc.info );
});
