...
function dtprfb( side, trans, direct, storev, M, N, K, l, V, strideV1, strideV2, offsetV, T, strideT1, strideT2, offsetT, A, strideA1, strideA2, offsetA, B, strideB1, strideB2, offsetB, WORK, strideWork1, strideWork2, offsetWork ) {
	...
	// from an out-of-bounds read. WORK is a logical K-by-N block for side='left'
	// or M-by-K for side='right'; account for the (possibly strided) 2D extent.
	const rows = ( side === 'left' ) ? K : M;
	const cols = ( side === 'left' ) ? N : K;
	const need = ( ( rows - 1 ) * strideWork1 ) + ( ( cols - 1 ) * strideWork2 ) + 1;
