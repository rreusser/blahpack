...
const lines = readFileSync( path.join( fixtureDir, 'zlaein.jsonl' ), 'utf8' ).trim().split( '\n' ); // eslint-disable-line node/no-sync
...
const NMAX = 5;
const DLAMCH_S = 2.2250738585072014e-308;
const DLAMCH_P = 2.220446049250313e-16;
const SMLNUM = DLAMCH_S / DLAMCH_P;
const EPS3 = 1.0e-4;
const TOL = 1.0e-12;
...
function makeMatrix( entries ) {
	let idx, r, c, i;
	const M = new Complex128Array( NMAX * NMAX );
...
function makeV( pairs ) {
	let i;
	const v = new Complex128Array( NMAX );
...
function runCase( H, N, wre, wim, rightv, noinit, vInit ) {
	let i;
	const v = vInit || new Complex128Array( NMAX );
	const B = new Complex128Array( NMAX * NMAX );
	const rwork = new Float64Array( NMAX );
	const w = new Complex128( wre, wim );
	const info = zlaein( rightv, noinit, N, H, 1, NMAX, 0, w, v, 1, 0, B, 1, NMAX, 0, rwork, 1, 0, EPS3, SMLNUM ); // eslint-disable-line max-len
	const view = reinterpret( v, 0 );
	const out = [];
	for ( i = 0; i < 2 * N; i++ ) {
		out.push( view[ i ] );
...
test( 'zlaein: right_noinit_3x3', function t() {
	const tc = findCase( 'right_noinit_3x3' );
	const H = makeMatrix( H3 );
	const res = runCase( H, 3, 3.9, -0.95, true, true );
	assert.equal( res.info, tc.info, 'info' );
	assertArrayClose( res.v, tc.v, TOL, 'v' );
} );

test( 'zlaein: left_noinit_3x3', function t() {
	const tc = findCase( 'left_noinit_3x3' );
	const H = makeMatrix( H3 );
	const res = runCase( H, 3, 3.9, -0.95, false, true );
	assert.equal( res.info, tc.info, 'info' );
	assertArrayClose( res.v, tc.v, TOL, 'v' );
} );

test( 'zlaein: right_init_3x3', function t() {
	const tc = findCase( 'right_init_3x3' );
	const H = makeMatrix( H3 );
	const v = makeV( [ [ 0.1, 0.0 ], [ 0.2, 0.1 ], [ 0.9, -0.2 ] ] );
	const res = runCase( H, 3, 3.9, -0.95, true, false, v );
	assert.equal( res.info, tc.info, 'info' );
	assertArrayClose( res.v, tc.v, TOL, 'v' );
} );

test( 'zlaein: right_noinit_4x4', function t() {
	const tc = findCase( 'right_noinit_4x4' );
	const H = makeMatrix( H4 );
	const res = runCase( H, 4, 3.95, 0.45, true, true );
	assert.equal( res.info, tc.info, 'info' );
	assertArrayClose( res.v, tc.v, TOL, 'v' );
} );

test( 'zlaein: left_noinit_4x4', function t() {
	const tc = findCase( 'left_noinit_4x4' );
	const H = makeMatrix( H4 );
	const res = runCase( H, 4, 3.95, 0.45, false, true );
	assert.equal( res.info, tc.info, 'info' );
	assertArrayClose( res.v, tc.v, TOL, 'v' );
} );

test( 'zlaein: right_noinit_2x2', function t() {
	const tc = findCase( 'right_noinit_2x2' );
	const H = makeMatrix( H2 );
	const res = runCase( H, 2, 2.45, 0.28, true, true );
	assert.equal( res.info, tc.info, 'info' );
	assertArrayClose( res.v, tc.v, TOL, 'v' );
} );

test( 'zlaein: right_near_eig_diag', function t() {
	const tc = findCase( 'right_near_eig_diag' );
	const H = makeMatrix( HD );
	const res = runCase( H, 3, 2.0001, 0.0, true, true );
	assert.equal( res.info, tc.info, 'info' );
	assertArrayClose( res.v, tc.v, TOL, 'v' );
} );

test( 'zlaein: left_near_eig_diag', function t() {
	const tc = findCase( 'left_near_eig_diag' );
	const H = makeMatrix( HD );
	const res = runCase( H, 3, 2.0001, 0.0, false, true );
	assert.equal( res.info, tc.info, 'info' );
	assertArrayClose( res.v, tc.v, TOL, 'v' );
} );
...
test( 'zlaein: right with zero subdiagonal (reducible H)', function t() {
	...
	const B = new Complex128Array( NMAX * NMAX );
	const rwork = new Float64Array( NMAX );
	const w = new Complex128( 3.0, 0.0 );
	const info = zlaein( true, true, 3, H, 1, NMAX, 0, w, v, 1, 0, B, 1, NMAX, 0, rwork, 1, 0, EPS3, SMLNUM ); // eslint-disable-line max-len
	const res = reinterpret( v, 0 );
...
test( 'zlaein: left with zero subdiagonal (reducible H)', function t() {
	...
	const rwork = new Float64Array( NMAX );
	const w = new Complex128( 3.0, 0.0 );
	const info = zlaein( false, true, 3, H, 1, NMAX, 0, w, v, 1, 0, B, 1, NMAX, 0, rwork, 1, 0, EPS3, SMLNUM ); // eslint-disable-line max-len
	const res = reinterpret( v, 0 );
	assert.ok( info === 0 || info === 1, 'info is 0 or 1' );
	for ( i = 0; i < 6; i++ ) {
		assert.ok( Number.isFinite( res[ i ] ), 'v[' + i + '] finite' );
...
test( 'zlaein: N<=0 returns 0 immediately', function t() {
	...
	const B = new Complex128Array( NMAX * NMAX );
	const rwork = new Float64Array( NMAX );
	const w = new Complex128( 1.0, 0.0 );
	const info = zlaein( true, true, 0, H, 1, NMAX, 0, w, v, 1, 0, B, 1, NMAX, 0, rwork, 1, 0, EPS3, SMLNUM ); // eslint-disable-line max-len
