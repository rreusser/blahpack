...
import zhetri2x from './../lib/ndarray.js';


// FIXTURES //

const fixtureDir = path.join( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures' ); // eslint-disable-line max-len
const rawLines = readFileSync( path.join( fixtureDir, 'zhetri2x.jsonl' ), 'utf8' ).trim().split( '\n' ); // eslint-disable-line max-len
...
function buildA( aReals, N ) {
	let i;
	const arr = new Complex128Array( FIXTURE_LDA * N );
	const buf = new Float64Array( arr.buffer, arr.byteOffset, arr.length * 2 );
...
function runFixture( uplo, N, nb, caseName ) {
	...
	const A = buildA( tc.a_factored, N );
	const ipiv = convertIpiv( tc.ipiv );
	const ldwork = N + nb + 1;
	const work = new Complex128Array( ldwork * ( nb + 3 ) );

	const info = zhetri2x( uplo, N, A, 1, FIXTURE_LDA, 0, ipiv, 1, 0, work, 1, 0, nb );
...
test( 'zhetri2x: N=1 lower (trivial)', function t() {
	...
	const nb = 2;
	const ldwork = 1 + nb + 1;
	const arr = new Complex128Array( FIXTURE_LDA * 1 );
	const buf = new Float64Array( arr.buffer, arr.byteOffset, arr.length * 2 );
	buf[ 0 ] = 5.0;
	buf[ 1 ] = 0.0;
	const ipiv = new Int32Array( [ 0 ] );
	const work = new Complex128Array( ldwork * ( nb + 3 ) );
	const info = zhetri2x( 'lower', 1, arr, 1, FIXTURE_LDA, 0, ipiv, 1, 0, work, 1, 0, nb );
...
test( 'zhetri2x: N=0 quick return', function t() {
	...
	const info = zhetri2x( 'lower', 0, A, 1, 1, 0, ipiv, 1, 0, work, 1, 0, 2 );
...
test( 'zhetri2x: singular D returns positive info', function t() {
	...
	const nb = 2;
	const ldwork = 2 + nb + 1;
	const arr = new Complex128Array( 2 * 2 );
	const buf = new Float64Array( arr.buffer, arr.byteOffset, arr.length * 2 );
	...
	const work = new Complex128Array( ldwork * ( nb + 3 ) );
	const info = zhetri2x( 'lower', 2, arr, 1, 2, 0, ipiv, 1, 0, work, 1, 0, nb );
