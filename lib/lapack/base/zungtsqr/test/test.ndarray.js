...
import zungtsqr from './../lib/ndarray.js';


// FIXTURES //

const fixtureDir = path.join( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures' );
const rawLines = readFileSync( path.join( fixtureDir, 'zungtsqr.jsonl' ), 'utf8' ).trim().split( '\n' );
...
function runCase( name ) {
	...
	const A = makeComplex( tc.V ); // M-by-N (column-major, contiguous)
	const T = makeComplex( tc.T ); // LDT-by-TCOLS (column-major, contiguous)
	const nbloc = ( tc.NB < tc.N ) ? tc.NB : tc.N;
	const WORK = new Complex128Array( ( tc.M + nbloc ) * tc.N );

	const info = zungtsqr( tc.M, tc.N, tc.MB, tc.NB, A, 1, tc.M, 0, T, 1, tc.LDT, 0, WORK, 1, 0 );
...
test( 'zungtsqr quick-return: M=0', function t() {
	...
	const info = zungtsqr( 0, 0, 4, 2, A, 1, 1, 0, T, 1, 1, 0, WORK, 1, 0 );
...
test( 'zungtsqr quick-return: N=0', function t() {
	...
	const A = new Complex128Array( 1 );
	const T = new Complex128Array( 1 );
	const info = zungtsqr( 8, 0, 4, 2, A, 1, 1, 0, T, 1, 1, 0, WORK, 1, 0 );
	assert.strictEqual( info, 0, 'N=0 -> info 0' );
});

test( 'zungtsqr orthonormality: Q^H * Q = I', function t() {
	let dotI, dotR, idxI, idxJ, dot, i, j, k;

	// Use the basic_blocked fixture and verify Q^H*Q == I directly from JS output.
	const tc = findCase( 'basic_blocked' );
	const A = makeComplex( tc.V );
	const T = makeComplex( tc.T );
	const nbloc = ( tc.NB < tc.N ) ? tc.NB : tc.N;
	const WORK = new Complex128Array( ( tc.M + nbloc ) * tc.N );
...
test( 'zungtsqr custom strides/offsets via ndarray (offsetWork)', function t() {
	...
	const tc = findCase( 'tall_narrow_nb1' );
	const A = makeComplex( tc.V );
	const T = makeComplex( tc.T );
	const nbloc = ( tc.NB < tc.N ) ? tc.NB : tc.N;
	const pad = 7;
	const WORK = new Complex128Array( pad + ( ( tc.M + nbloc ) * tc.N ) );

	const info = zungtsqr( tc.M, tc.N, tc.MB, tc.NB, A, 1, tc.M, 0, T, 1, tc.LDT, 0, WORK, 1, pad );
