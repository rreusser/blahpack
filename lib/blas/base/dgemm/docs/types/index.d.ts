...
interface Routine {
	...
	*/
	...
	/**
	* Performs one of the matrix-matrix operations `C = alpha*op(A)*op(B) + beta*C` where op(X) is one of `op(X) = X`, `op(X) = X^T`, or `op(X) = X^H` using alternative indexing semantics.
	*
	* @param transa - specifies the operation for matrix `A`
	* @param transb - specifies the operation for matrix `B`
	* @param M - number of rows
