/**
* Property-based validation for zgeru, following the /blahpack-validate process.
*
* Step 0 classification: `z` -> complex scalar; `ge` -> general dense
* (schemes.dense, logical.general); unconjugated rank-1 update
* `A := alpha*x*y**T + A` validated by direct residual against an independent
* oracle `A(i,j) += alpha*x_i*y_j`.
*/
...
import zgeru from './../lib/ndarray.js';

const sc = S.complex; // z-routine
const CONJ = false; // A := alpha*x*y**T + A (does NOT conjugate y)
...
test( 'zgeru: rank-1 update residual (size sweep MxN, incl. alpha=0)', function t() {
	SIZES.forEach( function eachM( M ) {
		SIZES.forEach( function eachN( N ) {
			const rng = new RNG( 0x100 + ( M * 100 ) + N ); // reproducible
			const A0 = logical.general( sc, rng, M, N );
			const x = values( rng, M );
			const y = values( rng, N );
			const alpha = ( ( ( M * 7 ) + N ) % 11 === 0 ) ? sc.zero : sc.random( rng );

			const R = schemes.dense.realize( sc, A0, { 'part': 'full' }, schemes.dense.layouts()[ 0 ] );
			const X = schemes.realizeVector( sc, x, { 'stride': 1 } );
			const Y = schemes.realizeVector( sc, y, { 'stride': 1 } );
			zgeru( M, N, sc.apiScalar( alpha ), X.data, X.args[ 0 ], X.args[ 1 ], Y.data, Y.args[ 0 ], Y.args[ 1 ], R.data, R.args[ 0 ], R.args[ 1 ], R.args[ 2 ] );

			const got = readMatrix( R, M, N );
			const exp = expected( A0, alpha, x, y, M, N );
			checked( 'zgeru', 'residual', function run() {
				check.assertReconstruct( sc, got, exp, { 'label': 'zgeru M='+M+' N='+N } );
			});
		});
	});
});
...
test( 'zgeru: bit-exact across storage layouts', function t() {
	const M = 7;
	const N = 5;
	const SEED = 0xF00D;
	const aLayouts = schemes.dense.layouts();
	const vLayouts = schemes.vectorLayouts();
	checked( 'zgeru', 'layout-invariance', function run() {
		layoutInvariant( aLayouts, function build( aL, idx ) {
			...
			const R = schemes.dense.realize( sc, A0, { 'part': 'full' }, aL );
