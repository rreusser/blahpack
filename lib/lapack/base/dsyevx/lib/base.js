...
import dormtr from '../../dormtr/lib/base.js';
...
*/
function dsyevx( jobz, range, uplo, N, A, strideA1, strideA2, offsetA, vl, vu, il, iu, abstol, out, w, strideW, offsetW, Z, strideZ1, strideZ2, offsetZ, WORK, strideWork, offsetWork, IWORK, strideIWork, offsetIWork, IFAIL, strideIFAIL, offsetIFAIL ) {
	let lwork, iscale, abstll, indwkn, nsplit, indibl, indisp, indiwo, sigma;
	let indee, order, itmp1, info, imax, Mout, tmp1, test, vll, vuu, jj, i, j;
	let M;
	...
	const anrm = dlansy( 'max', uplo, N, A, strideA1, strideA2, offsetA, WORK, strideWork, offsetWork );
	sigma = 1.0;
	...
	if ( M === 0 && info === 0 ) {
		...
		if ( wantz && M > 0 ) {
			...
			// Caller owns the workspace; derive the usable WORK length from the
			// provided array (replaces the former scalar `lwork` parameter).
			lwork = Math.floor( ( WORK.length - offsetWork ) / Math.abs( strideWork ) );
			indwkn = inde;
			dormtr( 'left', uplo, 'no-transpose', N, M, A, strideA1, strideA2, offsetA, WORK, strideWork, indtau, Z, strideZ1, strideZ2, offsetZ, WORK, strideWork, indwkn);
		}
	}
