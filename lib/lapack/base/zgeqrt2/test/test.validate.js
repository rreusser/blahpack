/**
* Property-based validation for zgeqrt2, following the /blahpack-validate process.
*
* Step 0 classification: `z` -> complex scalar; `ge` -> general dense; `qrt2`
* (UNBLOCKED compact-WY QR) -> reconstruction A = Q*R AND orthonormality of Q.
*
* zgeqrt2 computes, for a general complex M-by-N matrix A (M >= N), the compact-WY QR
...
import zgeqrt2 from './../lib/ndarray.js';
...
// dgemv/dger/dtrmv inside zgeqrt2 reorder across the col<->row flip).
...
test( 'zgeqrt2: bit-exact within storage-order family (col / row)', function t() {
...
function runInvariance( variants, fam ) {
	...
	const K = Math.min( M, N );
	const SEED = 0xF00D;
	checked( 'zgeqrt2', 'layout-invariance', function run() {
		...
		}, { 'label': 'zgeqrt2 layout invariance ' + fam + '-major (M=' + M + ' N=' + N + ')' } );
	} );
}
