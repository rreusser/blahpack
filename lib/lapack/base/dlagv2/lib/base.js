...
function dlagv2( A, strideA1, strideA2, offsetA, B, strideB1, strideB2, offsetB, alphar, strideALPHAR, offsetALPHAR, alphai, strideALPHAI, offsetALPHAI, beta, strideBETA, offsetBETA ) { // eslint-disable-line max-len
	let scale1, eig, svd, csl, snl, csr, snr, wi, h1, h2, h3, qq, rr;
	...
	// Scale A:
	const anorm = Math.max(Math.abs( A[ offsetA ] ) + Math.abs( A[ offsetA + strideA1 ] ), Math.abs( A[ offsetA + strideA2 ] ) + Math.abs( A[ offsetA + strideA1 + strideA2 ] ), SAFMIN);
	...
	// Scale B:
	const bnorm = Math.max(Math.abs( B[ offsetB ] ), Math.abs( B[ offsetB + strideB2 ] ) + Math.abs( B[ offsetB + strideB1 + strideB2 ] ), SAFMIN);
	const bscale = 1.0 / bnorm;
	B[ offsetB ] *= bscale;
