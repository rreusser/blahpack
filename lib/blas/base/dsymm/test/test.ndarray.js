...
test( 'dsymm: beta_zero', function t() {
	...
	const B = new Float64Array([ 2.0, 3.0, 4.0, 5.0 ]);
	const C = new Float64Array([ 999.0, 999.0, 999.0, 999.0 ]);
...
test( 'dsymm: alpha_zero_beta_zero (zeros C)', function t() {
	...
	const A = new Float64Array( 4 );
	const B = new Float64Array( 4 );
	const C = new Float64Array([ 99.0, 88.0, 77.0, 66.0 ]);
...
test( 'dsymm: left_lower_nonzero_beta', function t() {
	const tc = left_lower_nonzero_beta;
	const A = new Float64Array([
		2.0,
		1.0,
		3.0,
...
test( 'dsymm: right_upper_nonzero_beta', function t() {
	const tc = right_upper_nonzero_beta;
	const A = new Float64Array([
		2.0,
		0.0,
		0.0,
...
test( 'ndarray: tiled paths, all side x uplo x layout, tiles + remainders', function t() {
	let expected, layout, shape, side, uplo, msg, sa1, sa2, sb1, sb2, sc1, sc2;
	let ka, M, N, A, B, C, beta, i, s, u, w, q, z;
	const sides = [ 'left', 'right' ];
	const uplos = [ 'upper', 'lower' ];
	const layouts = [ 'col', 'row' ];

	// 12x12 gives whole tiles; 9x7 forces row and column remainders:
	const shapes = [ [ 12, 12 ], [ 9, 7 ] ];
...
test( 'ndarray: K-panel cache blocking (K > KC) and diagonal clamps', function t() {
	let expected, uplo, side, msg, sa1, sa2, sb1, sb2, ka, A, B, C, M, N, i, s;
	let u;
	const sides = [ 'left', 'right' ];
	const uplos = [ 'upper', 'lower' ];
