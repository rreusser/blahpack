...
import zlaWwaddw from './../lib/index.js';


// TESTS //

test( 'main export is a function', function t1() {
	assert.strictEqual( typeof zlaWwaddw, 'function', 'main export is a function' ); // eslint-disable-line max-len
});

test( 'main export has an ndarray method', function t2() {
	assert.strictEqual( typeof zlaWwaddw.ndarray, 'function', 'has ndarray method' ); // eslint-disable-line max-len
});
...
test( 'main export: ndarray interface with offsets', function t4() {
	const x = new Complex128Array( [ 999, 999, 1, 2, 3, 4 ] );
	const y = new Complex128Array( [ 999, 999, 0.1, 0.2, 0.3, 0.4 ] );
	const w = new Complex128Array( [ 999, 999, 10, 20, 30, 40 ] );
