...
function makeWork( M, N ) {
	...
	var wsz = Math.max( 1, 5 * minmn, ( 3 * minmn ) + maxmn, ( 2 * maxmn * maxmn ) + ( 3 * maxmn ) + maxmn );
...
test( 'dgesvd: quick return for M=0', function t() {
	...
	var work = new Float64Array( 1 ); // M=0 triggers quick return before work is used
	var info = dgesvd( 'none', 'none', 0, 3, A, 1, 1, 0, s, 1, 0, U, 1, 1, 0, VT, 1, 1, 0, work, 1, 0 );
...
test( 'dgesvd: quick return for N=0', function t() {
	...
	var U = new Float64Array( 1 );
	var VT = new Float64Array( 1 );
	var work = new Float64Array( 1 ); // N=0 triggers quick return before work is used
	var info = dgesvd( 'none', 'none', 3, 0, A, 1, 3, 0, s, 1, 0, U, 1, 1, 0, VT, 1, 1, 0, work, 1, 0 );
	assert.equal( info, 0 );
});
...
test( 'dgesvd: 1x1 edge case', function t() {
	...
	var U = new Float64Array( 1 );
	var VT = new Float64Array( 1 );
	var work = makeWork( M, N );
	var info = dgesvd( 'all-columns', 'all-rows', M, N, A, 1, 1, 0, s, 1, 0, U, 1, 1, 0, VT, 1, 1, 0, work, 1, 0 );
	assert.equal( info, tc.info );
...
test( 'dgesvd: 4x3 full SVD (JOBU=A, JOBVT=A) — Path 9', function t() {
	...
	var work = makeWork( M, N );
	var info = dgesvd( 'all-columns', 'all-rows', M, N, A, 1, M, 0, s, 1, 0, U, 1, M, 0, VT, 1, N, 0, work, 1, 0 );
	assert.equal( info, 0 );
...
test( 'dgesvd: 4x3 compact SVD (JOBU=S, JOBVT=S) — Path 6', function t() {
	...
	var work = makeWork( M, N );
	var info = dgesvd( 'economy', 'economy', M, N, A, 1, M, 0, s, 1, 0, U, 1, M, 0, VT, 1, minmn, 0, work, 1, 0 );
	assert.equal( info, 0 );
...
test( 'dgesvd: 4x3 JOBU=O, JOBVT=S — Path 3', function t() {
	...
	var work = makeWork( M, N );
	var info = dgesvd( 'overwrite', 'economy', M, N, A, 1, M, 0, s, 1, 0, U, 1, 1, 0, VT, 1, minmn, 0, work, 1, 0 );
	assert.equal( info, 0 );
...
test( 'dgesvd: 4x3 JOBU=N, JOBVT=N — Path 1 (values only)', function t() {
	...
	var work = makeWork( M, N );
	var info = dgesvd( 'none', 'none', M, N, A, 1, M, 0, s, 1, 0, U, 1, 1, 0, VT, 1, 1, 0, work, 1, 0 );
	assert.equal( info, 0 );
...
test( 'dgesvd: 4x3 JOBU=S, JOBVT=O — Path 5', function t() {
	...
	var work = makeWork( M, N );
	var info = dgesvd( 'economy', 'overwrite', M, N, A, 1, M, 0, s, 1, 0, U, 1, M, 0, VT, 1, 1, 0, work, 1, 0 );
	assert.equal( info, 0 );
...
test( 'dgesvd: 4x3 JOBU=A, JOBVT=S — Path 9', function t() {
	...
	var work = makeWork( M, N );
	var info = dgesvd( 'all-columns', 'economy', M, N, A, 1, M, 0, s, 1, 0, U, 1, M, 0, VT, 1, minmn, 0, work, 1, 0 );
	assert.equal( info, 0 );
...
test( 'dgesvd: 3x4 JOBU=N, JOBVT=S — Path 4t', function t() {
	...
	var work = makeWork( M, N );
	var info = dgesvd( 'none', 'economy', M, N, A, 1, M, 0, s, 1, 0, U, 1, 1, 0, VT, 1, minmn, 0, work, 1, 0 );
	assert.equal( info, 0 );
...
test( 'dgesvd: 3x4 JOBU=S, JOBVT=N — Path 1t (with U)', function t() {
	...
	var work = makeWork( M, N );
	var info = dgesvd( 'economy', 'none', M, N, A, 1, M, 0, s, 1, 0, U, 1, M, 0, VT, 1, 1, 0, work, 1, 0 );
	assert.equal( info, 0 );
...
test( 'dgesvd: 4x3 JOBU=N, JOBVT=A — Path 1 (with VT)', function t() {
	...
	var work = makeWork( M, N );
	var info = dgesvd( 'none', 'all-rows', M, N, A, 1, M, 0, s, 1, 0, U, 1, 1, 0, VT, 1, N, 0, work, 1, 0 );
	assert.equal( info, 0 );
...
test( 'dgesvd: 3x4 JOBU=A, JOBVT=N — Path 1t (with full U)', function t() {
	...
	var work = makeWork( M, N );
	var info = dgesvd( 'all-columns', 'none', M, N, A, 1, M, 0, s, 1, 0, U, 1, M, 0, VT, 1, 1, 0, work, 1, 0 );
	assert.equal( info, 0 );
...
test( 'dgesvd: 4x3 JOBU=O, JOBVT=N — Path 2', function t() {
	...
	var work = makeWork( M, N );
	var info = dgesvd( 'overwrite', 'none', M, N, A, 1, M, 0, s, 1, 0, U, 1, 1, 0, VT, 1, 1, 0, work, 1, 0 );
	assert.equal( info, 0 );
...
test( 'dgesvd: 4x3 JOBU=A, JOBVT=O — Path 8', function t() {
	...
	var work = makeWork( M, N );
	var info = dgesvd( 'all-columns', 'overwrite', M, N, A, 1, M, 0, s, 1, 0, U, 1, M, 0, VT, 1, 1, 0, work, 1, 0 );
	assert.equal( info, 0 );
...
test( 'dgesvd: 3x4 JOBU=S, JOBVT=A — Path 9t', function t() {
	...
	var work = makeWork( M, N );
	var info = dgesvd( 'economy', 'all-rows', M, N, A, 1, M, 0, s, 1, 0, U, 1, M, 0, VT, 1, N, 0, work, 1, 0 );
	assert.equal( info, 0 );
...
test( 'dgesvd: 3x4 JOBU=O, JOBVT=A — Path 8t', function t() {
	...
	var work = makeWork( M, N );
	var info = dgesvd( 'overwrite', 'all-rows', M, N, A, 1, M, 0, s, 1, 0, U, 1, 1, 0, VT, 1, N, 0, work, 1, 0 );
	assert.equal( info, 0 );
...
test( 'dgesvd: 4x3 JOBU=N, JOBVT=O — Path 1 (VT overwrite)', function t() {
	...
	var work = makeWork( M, N );
	var info = dgesvd( 'none', 'overwrite', M, N, A, 1, M, 0, s, 1, 0, U, 1, 1, 0, VT, 1, 1, 0, work, 1, 0 );
	assert.equal( info, 0 );
