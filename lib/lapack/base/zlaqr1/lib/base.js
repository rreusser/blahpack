...
* 3-by-3 matrix (H - s1_I)_(H - s2*I), scaling to avoid overflows and
* most underflows.
*
...
function zlaqr1( N, H, strideH1, strideH2, offsetH, s1, s2, v, strideV, offsetV ) {
	let h21sR, h21sI, h31sR, h31sI, tR, tI, uR, uI, aR, aI, bR, bI, s;
	...
	const sv = reinterpret( v, 0 );
	const sh1 = strideH1 * 2;
	const sh2 = strideH2 * 2;

	// oH is the Float64 base offset
	const oH = offsetH * 2; // eslint-disable-line no-var
	const ov = offsetV * 2; // eslint-disable-line no-var
	const svv = strideV * 2; // eslint-disable-line no-var
