...
* tolerance.
*
* FACTOR SOURCE — a deliberate deviation. The natural packed factor, zhptrf, is
* BROKEN on its UPPER-triangle path (it produces a wrong Hermitian packed factor;
* its lower path is correct). See test/harness/LEARNINGS.md, "zhptrf upper path".
* zhptri itself is CORRECT for BOTH uplo (verified: fed a valid factor it
* reconstructs to ~1e-13). To validate zhptri INDEPENDENTLY of that broken
* dependency, the Bunch-Kaufman factor + IPIV are produced by the VALIDATED DENSE
* Hermitian factorization `zhetrf` and re-packed into packed storage. That is a
* mathematically valid packed factor of the same A0 (a Bunch-Kaufman factor is not
* unique, but any valid factor+IPIV inverts to inv(A0)). When zhptrf's upper path
* is fixed, switch the factor source back to zhptrf.
*
* The factor is produced ONCE (dense, tight col-major) and only the INVERSE
...
import zhetrf from '../../zhetrf/lib/ndarray.js'; // validated factor source (zhptrf upper is buggy)
...
// the VALIDATED DENSE zhetrf (tight col-major) and read the uplo triangle back as
// a LogicalMatrix, so the packed inverse can be re-run at arbitrary packed layouts
// off a fixed, correct factor. (zhptrf's upper path is broken — see the file
// header and LEARNINGS.) Returns null on a rare singular draw.
var TIGHT = { 'stride': 1, 'lead': 0, 'tail': 0 };
var DENSE_TIGHT = { 'order': 'col', 'sgn1': 1, 'sgn2': 1, 'g': 1, 'ldaExtra': 0, 'lead': 0, 'tail': 0 };
...
function factorOne( A0, n, uplo ) {
	var R = schemes.dense.realize( sc, A0, { 'part': uplo }, DENSE_TIGHT );
	var ipiv = new Int32Array( Math.max( n, 1 ) );
	var lwork = Math.max( 1, 64 * n );
	var info = zhetrf( uplo, n, R.data, R.args[ 0 ], R.args[ 1 ], R.args[ 2 ], ipiv, 1, 0, poisonedWork( sc, lwork ), 1, 0, lwork );
	if ( info !== 0 ) {
		return null; // rare singular draw
	}
	return {
		'factor': readTri( R, n, uplo ), // dense readTri: R.read(i,j) yields the value
		'ipiv': ipiv
	};
}
...
function invertAt( F, n, uplo, layout ) {
	...
	var info = zhptri( uplo, n, R.data, R.args[ 0 ], R.args[ 1 ], ipiv, 1, 0, work, 1, 0 );
	if ( info !== 0 ) {
...
function runInvariance( uplo, n ) {
	...
	if ( F === null ) {
		throw new Error( 'zhptri invariance setup: zhetrf returned a singular factor' );
	}
...
test( 'zhptri: workspace minimum (len N) suffices with a poisoned buffer', function t() {
	...
	if ( F === null ) {
		throw new Error( 'zhptri workspace setup: zhetrf returned a singular factor' );
	}
	...
	checked( 'zhptri', 'workspace', function check_() {
		var minLen = assertWorkspaceSufficient( run, {}, 'zhptri WORK@N=33 upper' );
		if ( minLen !== N ) {
