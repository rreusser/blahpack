...
//
// Memory-bound Hermitian rank-1 update `A += alpha*x*x^H`: one read + one
// write of the stored triangle per complex element, four flops each. `alpha`
// is kept tiny so repeated in-place updates stay bounded.
...
for ( const n of [ 100, 500, 2000 ] ) {
	for ( const layout of [ 'col', 'row' ] ) {
		for ( const uplo of [ 'upper', 'lower' ] ) {
			...
			const x = randz( n );
			const [ a, b ] = race( [
				{ fn: () => v0( uplo, n, ALPHA, x, 1, 0, A0, sa1, sa2, 0 ) },
				{ fn: () => v1( uplo, n, ALPHA, x, 1, 0, A1, sa1, sa2, 0 ) }
			] );
			// ~half the matrix (the stored triangle) at 4 flops/complex element:
