...
import dgetsqrhrt from './../lib/ndarray.js';


// FIXTURES //

const fixtureDir = path.join( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures' );
const lines = readFileSync( path.join( fixtureDir, 'dgetsqrhrt.jsonl' ), 'utf8' ).trim().split( '\n' ); // eslint-disable-line node/no-sync
...
function packT( T, LDT, nb2, tcols ) {
	const out = new Float64Array( nb2 * tcols );
	let i, j;
	for ( j = 0; j < tcols; j += 1 ) {
		for ( i = 0; i < nb2; i += 1 ) {
...
function runFixture( tc, M, N, mb1, nb1, nb2, A ) {
	...
	const nb2local = Math.min( nb2, N );
	const tcols = tcolsFor( N, nb2local );
	const T = new Float64Array( nb2local * tcols );
	const WORK = new Float64Array( 1024 );
	const info = dgetsqrhrt( M, N, mb1, nb1, nb2, A, 1, M, 0, T, 1, nb2local, 0, WORK, 1, 0 );
...
test( 'dgetsqrhrt: m8_n3_mb4_nb1_2_nb2_2 (basic blocked TSQR + HRT)', function t() {
	const tc = findCase( 'm8_n3_mb4_nb1_2_nb2_2' );
	const A = buildSinCosMatrix( 8, 3, 1, 3, 0.5, 2, -1, 4.0 );
	runFixture( tc, 8, 3, 4, 2, 2, A );
});

test( 'dgetsqrhrt: m10_n2_mb4_nb1_2_nb2_1', function t() {
	const tc = findCase( 'm10_n2_mb4_nb1_2_nb2_1' );
	const A = buildSinCosMatrix( 10, 2, 2, 1, 0.3, 1, -2, 6.0 );
	runFixture( tc, 10, 2, 4, 2, 1, A );
});

test( 'dgetsqrhrt: m4_n4_mb6_square (single-block TSQR fall-through)', function t() {
	const tc = findCase( 'm4_n4_mb6_square' );
	const M = 4;
	const N = 4;
	const A = new Float64Array( M * N );
	let i, j;
...
test( 'dgetsqrhrt: m0_n0_quickreturn', function t() {
	...
	const tc = findCase( 'm0_n0_quickreturn' );
	const info = dgetsqrhrt( 0, 0, 1, 1, 1, new Float64Array( 0 ), 1, 1, 0, new Float64Array( 0 ), 1, 1, 0, new Float64Array( 1 ), 1, 0 );
	assert.strictEqual( info, tc.INFO, 'INFO matches' );
});

test( 'dgetsqrhrt: m6_n6_mb7_nb1_3_nb2_4 (square N=6 with mb1 just > N)', function t() {
	const tc = findCase( 'm6_n6_mb7_nb1_3_nb2_4' );
	const M = 6;
	const N = 6;
	const A = new Float64Array( M * N );
	let i, j;
...
test( 'dgetsqrhrt: A = Q*R round-trip', function t() {
	let jbn, bb, jb, w2, c, i, j, k, s, t, u, w, y;
	...
	const nb2local = Math.min( nb2, N );
	const tcols = tcolsFor( N, nb2local );
	const Aorig = new Float64Array( M * N );
	const A = new Float64Array( M * N );
	const R = new Float64Array( N * N );
	const V = new Float64Array( M * N );
	const T = new Float64Array( nb2local * tcols );
	const WORK = new Float64Array( 1024 );
	const QR = new Float64Array( M * N );
	const numBlocks = Math.ceil( N / nb2 );
	...
	const info = dgetsqrhrt( M, N, 5, 2, nb2, A, 1, M, 0, T, 1, nb2local, 0, WORK, 1, 0 );
	assert.strictEqual( info, 0, 'INFO returned 0' );
