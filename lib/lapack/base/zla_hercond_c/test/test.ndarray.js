...
import zla_hercond_c from './../lib/ndarray.js';


// FIXTURES //

const fixtureDir = path.join( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures' );
const lines = readFileSync( path.join( fixtureDir, 'zla_hercond_c.jsonl' ), 'utf8' ).trim().split( '\n' ); // eslint-disable-line node/no-sync
...
function runCase( uplo, c, capply, fixtureName ) {
	...
	const info = zhetrf( uplo, N, AF, 1, N, 0, IPIV, 1, 0 );
	assert.equal( info, 0, fixtureName + ': zhetrf info' );
	const res = zla_hercond_c( uplo, N, A, 1, N, 0, AF, 1, N, 0, IPIV, 1, 0, c, 1, 0, capply, WORK, 1, 0, RWORK, 1, 0 );
...
test( 'zla_hercond_c: n1_uplo_U', function t() {
	...
	const res = zla_hercond_c( 'upper', N, A, 1, N, 0, AF, 1, N, 0, IPIV, 1, 0, c, 1, 0, true, WORK, 1, 0, RWORK, 1, 0 );
...
test( 'zla_hercond_c: n0', function t() {
	...
	const res = zla_hercond_c( 'upper', 0, A, 1, 1, 0, AF, 1, 1, 0, IPIV, 1, 0, c, 1, 0, true, WORK, 1, 0, RWORK, 1, 0 );
...
test( 'zla_hercond_c: lower anorm zero returns 0', function t() {
	...
	const WORK = new Complex128Array( 2 * N );
	const RWORK = new Float64Array( N );
	const res = zla_hercond_c( 'lower', N, A, 1, N, 0, AF, 1, N, 0, IPIV, 1, 0, c, 1, 0, false, WORK, 1, 0, RWORK, 1, 0 );
