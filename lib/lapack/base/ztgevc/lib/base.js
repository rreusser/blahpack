...
function ztgevc( side, howmny, SELECT, strideSELECT, offsetSELECT, N, S, strideS1, strideS2, offsetS, P, strideP1, strideP2, offsetP, VL, strideVL1, strideVL2, offsetVL, VR, strideVR1, strideVR2, offsetVR, mm, M, WORK, strideWork, offsetWork, RWORK, strideRWork, offsetRWork ) {
	let acoefa, acoeff, bcoefa, sumaRe, sumaIm, sumbRe, sumbIm, ilcomp, anorm;
	let bnorm, scale, sbeta, dmin, temp, xmax, ieig, isrc, ibeg, iend, idx2;
	let sjjr, sjji, pjjr, pjji, big, lsa, lsb, idx, srr, sri, prr, pri, je, jr;
	let im, sr, si, pr, pi, dr, di, j, i;

	const compl = ( side === 'left' || side === 'both' );
	const compr = ( side === 'right' || side === 'both' );
	const ilback = ( howmny === 'backtransform' );
	const ilall = ( howmny === 'all' || howmny === 'backtransform' );
	...
	const sS1 = strideS1 * 2;
	const sS2 = strideS2 * 2;
	const oS = offsetS * 2;
	const sP1 = strideP1 * 2;
	const sP2 = strideP2 * 2;
	const oP = offsetP * 2;
	const sVL1 = strideVL1 * 2;
	const sVL2 = strideVL2 * 2;
	const oVL = offsetVL * 2;
	const sVR1 = strideVR1 * 2;
	const sVR2 = strideVR2 * 2;
	const oVR = offsetVR * 2;

	// Machine constants
	const safmin = dlamch( 'Safe minimum' );
	big = ONE / safmin;
	const ulp = dlamch( 'Epsilon' ) * dlamch( 'Base' );
	const small = safmin * N / ulp;
	big = ONE / small;
	const bignum = ONE / ( safmin * N );
	...
	const ascale = ONE / Math.max( anorm, safmin );
	const bscale = ONE / Math.max( bnorm, safmin );

	// Temporary complex scalars
	const salpha = new Float64Array( 2 );
	const bcoeff = new Float64Array( 2 );
	const sum = new Float64Array( 2 );
	const d = new Float64Array( 2 );
	const ca = new Float64Array( 2 );
	const cb = new Float64Array( 2 );
