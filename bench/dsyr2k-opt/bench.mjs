...
// Also prints the shipped register-tiled dgemm on the same size as a roofline
// reference (dsyr2k does about the same flops as the same-size dgemm: two
// rank-k products over half the matrix).
...
for ( const n of [ 128, 256, 512 ] ) {
	...
	for ( const layout of [ 'col', 'row' ] ) {
		// A and B are n×k in both trans modes here (n=k), C is n×n
		...
		for ( const uplo of [ 'upper', 'lower' ] ) {
			for ( const trans of [ 'no-transpose', 'transpose' ] ) {
				const [ a, b ] = race( [
					{ fn: () => v0( uplo, trans, n, k, 1.0, A, sa1, sa2, 0, B, sa1, sa2, 0, 0.0, C, sc1, sc2, 0 ) },
					{ fn: () => v1( uplo, trans, n, k, 1.0, A, sa1, sa2, 0, B, sa1, sa2, 0, 0.0, C, sc1, sc2, 0 ) }
				] );
				const gf = ( ms ) => ( 2 * n * ( n + 1 ) * k ) / ( ms * 1e6 );
				console.log(
