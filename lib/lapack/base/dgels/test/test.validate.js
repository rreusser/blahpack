...
function oracleSolve( sc, A, b, d ) {
	...
	if ( d.tall ) {
		...
		const CtC = new logical.LogicalMatrix( sc, d.q, d.q );
		for ( i = 0; i < d.q; i++ ) {
			for ( j = 0; j < d.q; j++ ) {
				s = 0.0;
	...
	const CCt = new logical.LogicalMatrix( sc, d.p, d.p );
	for ( i = 0; i < d.p; i++ ) {
		for ( j = 0; j < d.p; j++ ) {
			s = 0.0;
...
const PAIRS = [ [ 1, 1 ], [ 2, 3 ], [ 3, 2 ], [ 5, 3 ], [ 3, 5 ], [ 8, 5 ], [ 16, 7 ], [ 7, 16 ], [ 17, 17 ], [ 32, 32 ], [ 33, 20 ], [ 20, 33 ], [ 40, 33 ], [ 33, 40 ], [ 64, 33 ], [ 33, 64 ], [ 64, 64 ], [ 65, 40 ] ];
const NRHS = [ 1, 3 ];
...
test( 'dgels: least-squares optimality / min-norm feasibility (property vs independent oracle)', function t() {
	...
	const lays = [ schemes.dense.layouts()[ 0 ], schemes.dense.layouts()[ 2 ] ];
	let ti, pi, ri, li;
...
function runProperty( sc, trans, M, N, nrhs, lay ) {
	const d = opDesc( trans, M, N );
	const ldb = Math.max( M, N );
	const rng = new RNG( 0x5000 + ( M * 131 ) + ( N * 7 ) + ( ( trans === 'transpose' ) ? 3 : 0 ) + nrhs );
	const A = logical.general( sc, rng, M, N );
	const B = makeB( sc, rng, ldb, d.p, nrhs );
	const RA = schemes.dense.realize( sc, A, { 'part': 'full' }, lay );
	const RB = schemes.dense.realize( sc, B, { 'part': 'full' }, lay );
	const work = new Float64Array( workLen( M, N, nrhs ) );
	const info = dgels( trans, M, N, nrhs, RA.data, RA.args[ 0 ], RA.args[ 1 ], RA.args[ 2 ], RB.data, RB.args[ 0 ], RB.args[ 1 ], RB.args[ 2 ], work, 1, 0 );
	if ( info !== 0 ) {
...
function validateColumn( sc, A, b, x, d, label ) {
	...
	checked( 'dgels', 'residual', function property() {
		const Cx = ref.matvec( sc, A, x, { 'trans': d.cApply } );
		const r = [];
		let i;
		...
		const nA = frob( sc, A );
		if ( d.tall ) {
			const g = ref.matvec( sc, A, r, { 'trans': d.ctApply } );
...
test( 'dgels: layout invariance (bit-exact within storage-order family; unblocked + blocked)', function t() {
	const sc = S.real;
	const sizes = [ [ 5, 3 ], [ 3, 5 ], [ 4, 4 ], [ 8, 5 ], [ 5, 8 ], [ 40, 33 ], [ 33, 40 ], [ 64, 64 ] ];
	let ti, si;
	for ( ti = 0; ti < TRANS.length; ti++ ) {
...
function runInvariance( sc, trans, M, N, nrhs, variants, fam ) {
	...
	checked( 'dgels', 'layout-invariance', function fuzz() {
		layoutInvariant( variants, function run( layout, idx ) {
			...
			const RB = schemes.dense.realize( sc, B, { 'part': 'full' }, variants[ ( idx + 2 ) % variants.length ] );
			const work = new Float64Array( workLen( M, N, nrhs ) );
			dgels( trans, M, N, nrhs, RA.data, RA.args[ 0 ], RA.args[ 1 ], RA.args[ 2 ], RB.data, RB.args[ 0 ], RB.args[ 1 ], RB.args[ 2 ], work, 1, 0 );
			const out = new logical.LogicalMatrix( sc, ldb, nrhs );
			let i, j;
			for ( j = 0; j < nrhs; j++ ) {
				for ( i = 0; i < ldb; i++ ) {
...
const ORDERS = [ 'column-major', 'row-major' ];
const LDPAIRS = [ [ 3, 5 ], [ 5, 3 ], [ 4, 4 ], [ 2, 6 ], [ 7, 2 ] ]; // include M<N (output grows)
...
function runLDGuard( order, trans, M, N, nrhs ) {
	const p = ( trans === 'no-transpose' ) ? M : N; // referenced input rows of B
	const bRows = Math.max( M, N );                  // B's full (output) row extent
	const ldaReq = requiredLD( order, M, N );
	const ldbReq = requiredLD( order, bRows, nrhs );
	const lbl = order + ' ' + trans + ' ' + M + 'x' + N;
	...
	assertLeadingDimGuard( function callLDB( ldb ) {
		const A = realizeLD( S.real, order, M, N, M, N, ldaReq, dVal( 0.0 ) );
		const B = realizeLD( S.real, order, p, nrhs, bRows, nrhs, ldb, dVal( 9.0 ) );
	...
	assertLeadingDimGuard( function callLDA( lda ) {
		const A = realizeLD( S.real, order, M, N, M, N, lda, dVal( 0.0 ) );
		const B = realizeLD( S.real, order, p, nrhs, bRows, nrhs, ldbReq, dVal( 9.0 ) );
...
function runWrapperMinLDB( sc, M, N, nrhs ) {
	...
	const bRows = Math.max( M, N ); // = N here
	const ldbReq = requiredLD( order, bRows, nrhs );
	const ldaReq = requiredLD( order, M, N );
	const av = dVal( 1.5 );
	const bv = dVal( 4.2 );
	const A = realizeLD( S.real, order, M, N, M, N, ldaReq, av );
	const B = realizeLD( S.real, order, M, nrhs, bRows, nrhs, ldbReq, bv );

	// logical A and input b for the independent oracle/property:
	const Alog = new logical.LogicalMatrix( sc, M, N );
	let i, j;
	for ( j = 0; j < N; j++ ) {
		for ( i = 0; i < M; i++ ) {
			Alog.set( i, j, av( i, j ) );
	...
	const info = dgelsWrap( order, trans, M, N, nrhs, A.data, ldaReq, B.data, ldbReq, null, 1 );
	if ( info !== 0 ) {
...
function dgelsWorkRun( sc, trans, M, N, nrhs, seed ) {
	return function run( workLen ) {
		...
		const work = poisonedWork( S.real, workLen ); // NaN-filled: over-read OR read-before-write => NaN
		dgels( trans, M, N, nrhs, RA.data, RA.args[ 0 ], RA.args[ 1 ], RA.args[ 2 ], RB.data, RB.args[ 0 ], RB.args[ 1 ], RB.args[ 2 ], work, 1, 0 );
		const out = [];
		let i, j;
		for ( j = 0; j < nrhs; j++ ) {
...
test( 'dgels: advertised WORK minimum suffices on the blocked path (min(M,N) > NB)', function t() {
	...
	for ( k = 0; k < cases.length; k++ ) {
		const c = cases[ k ];
		const lbl = 'dgels WORK sufficiency ' + c[ 0 ] + ' ' + c[ 1 ] + 'x' + c[ 2 ] + ' nrhs=' + c[ 3 ];
