...
import zlarzb from './../lib/ndarray.js';


// VARIABLES //

const fixtureDir = path.join( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures' );
const lines = readFileSync( path.join( fixtureDir, 'zlarzb.jsonl' ), 'utf8' ).trim().split( '\n' ); // eslint-disable-line node/no-sync
...
function buildV( K, L ) {
	...
	const V = new Complex128Array( K * L );
...
test( 'zlarzb: SIDE=left, TRANS=no-transpose, M=5 N=4 K=2 L=3', function t() {
	...
	const T = buildT( K );
	const C = buildC( M, N );
	const WORK = new Complex128Array( N * K );
...
test( 'zlarzb: SIDE=left, TRANS=conjugate-transpose, M=5 N=4 K=2 L=3', function t() {
	...
	const tc = findCase( 'left_ctrans_m5_n4_k2_l3' );
	const M = 5;
	const N = 4;
	const K = 2;
	const L = 3;
...
test( 'zlarzb: SIDE=right, TRANS=no-transpose, M=4 N=5 K=2 L=3', function t() {
	...
	const T = buildT( K );
	const C = buildC( M, N );
	const WORK = new Complex128Array( M * K );
...
test( 'zlarzb: SIDE=right, TRANS=conjugate-transpose, M=4 N=5 K=2 L=3', function t() {
	...
	const tc = findCase( 'right_ctrans_m4_n5_k2_l3' );
	const M = 4;
	const N = 5;
	const K = 2;
	const L = 3;
...
test( 'zlarzb: SIDE=left, TRANS=no-transpose, L=0', function t() {
	...
	const K = 2;
	const L = 0;
	const V = new Complex128Array( K * 1 );
	const T = new Complex128Array( K * K );
	const Tv = reinterpret( T, 0 );
	Tv[ 0 ] = 0.6;
	Tv[ 1 ] = 0.1;
...
test( 'zlarzb: SIDE=right, TRANS=conjugate-transpose, K=1', function t() {
	...
	const tc = findCase( 'right_ctrans_k1' );
	const M = 3;
	const N = 4;
	const K = 1;
	const L = 2;
