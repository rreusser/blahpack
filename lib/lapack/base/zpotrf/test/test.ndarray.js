...
const ndarray = zpotrf;
import zpotrf2 from './../../zpotrf2/lib/base.js';
...
function randomHPD( N ) {
	...
	let br, bi, cr, ci, ar, ai, i, j, k, idx;

	// Build B with deterministic values
	const B = new Float64Array( N * N * 2 ); // interleaved
	for ( i = 0; i < N * N; i++ ) {
...
test( 'zpotrf: lower_3x3', function t() {
	...
	const info = zpotrf( 'lower', 3, A, 1, 3, 0 );
...
test( 'zpotrf: upper_3x3', function t() {
	...
	const info = zpotrf( 'upper', 3, A, 1, 3, 0 );
	assert.equal( info, tc.info );
	assertArrayClose( Array.from( reinterpret( A, 0 ) ), tc.a, 1e-14, 'a' );
});
...
test( 'zpotrf: lower_4x4', function t() {
	...
	const info = zpotrf( 'lower', 4, A, 1, 4, 0 );
	assert.equal( info, tc.info );
	assertArrayClose( Array.from( reinterpret( A, 0 ) ), tc.a, 1e-14, 'a' );
});
...
test( 'zpotrf: upper_4x4', function t() {
	...
	const info = zpotrf( 'upper', 4, A, 1, 4, 0 );
	assert.equal( info, tc.info );
	assertArrayClose( Array.from( reinterpret( A, 0 ) ), tc.a, 1e-14, 'a' );
});
...
test( 'zpotrf: n_zero', function t() {
	...
	const info = zpotrf( 'lower', 0, A, 1, 1, 0 );
	assert.equal( info, tc.info );
});
...
test( 'zpotrf: large lower (blocked path) matches zpotrf2', function t() {
	...
	const N = 80;
	const A1 = randomHPD( N );
	const A2 = new Complex128Array( A1 );
	const info1 = zpotrf( 'lower', N, A1, 1, N, 0 );
	const info2 = zpotrf2( 'lower', N, A2, 1, N, 0 );
...
test( 'zpotrf: large upper (blocked path) matches zpotrf2', function t() {
	...
	const A1 = randomHPD( N );
	const A2 = new Complex128Array( A1 );
	const info1 = zpotrf( 'upper', N, A1, 1, N, 0 );
	const info2 = zpotrf2( 'upper', N, A2, 1, N, 0 );
	assert.equal( info1, 0 );
	assert.equal( info2, 0 );
...
test( 'zpotrf: large not-posdef (blocked path)', function t() {
	...
	const idx = ( ( N - 1 ) * N + ( N - 1 ) ) * 2;
	Av[ idx ] = -1000.0;
	const info = zpotrf( 'lower', N, A, 1, N, 0 );
	assert.ok( info > 0 );
});
...
test( 'zpotrf: large not-posdef upper (blocked path)', function t() {
	...
	const idx = ( ( N - 1 ) * N + ( N - 1 ) ) * 2;
	Av[ idx ] = -1000.0;
	const info = zpotrf( 'upper', N, A, 1, N, 0 );
	assert.ok( info > 0 );
});
