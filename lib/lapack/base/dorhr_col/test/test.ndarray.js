...
import dorhr_col from './../lib/ndarray.js';
const ndarrayFn = dorhr_col;


// FIXTURES //

const fixtureDir = path.join( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures' );
const fixturePath = path.join( fixtureDir, 'dorhr_col.jsonl' );
...
function runCase( name, M, N, nb ) {
	...
	const a = new Float64Array( tc.q_in );
	const ldt = Math.min( nb, N );
	const t = new Float64Array( ldt * N );
	const d = new Float64Array( N );
	const info = dorhr_col( M, N, nb, a, 1, Math.max( 1, M ), 0, t, 1, Math.max( 1, ldt ), 0, d, 1, 0 ); // eslint-disable-line max-len
...
test( 'dorhr_col: M=0 N=0 quick return', function t() {
	...
	const info = dorhr_col( 0, 0, 1, a, 1, 1, 0, t, 1, 1, 0, d, 1, 0 );
...
test( 'dorhr_col: N=0 quick return', function t() {
	...
	const t = new Float64Array( 1 );
	const d = new Float64Array( 1 );
	const info = dorhr_col( 4, 0, 1, a, 1, 4, 0, t, 1, 1, 0, d, 1, 0 );
	assert.equal( info, 0, 'info' );
});
