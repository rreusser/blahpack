...
function zgemlqt( side, trans, M, N, K, mb, V, strideV1, strideV2, offsetV, T, strideT1, strideT2, offsetT, C, strideC1, strideC2, offsetC, WORK, strideWork, offsetWork ) {
	...
	// out-of-bounds read. ZGEMLQT needs WORK of length N*mb (side='left') or
	// M*mb (side='right'); the M/N/K===0 cases are quick returns that touch no
	// workspace.
	if ( M > 0 && N > 0 && K > 0 ) {
		need = ( ( side === 'left' ) ? N : M ) * mb;
