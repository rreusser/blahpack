...
import dormr3 from './../lib/index.js';
...
test( 'main: quick return when M=0', function t() {
	...
	const info = dormr3( 'column-major', 'left', 'no-transpose', 0, 2, 0, 0, A, 2, TAU, 1, C, 1, WORK, 1 ); // eslint-disable-line max-len
...
test( 'main: quick return when N=0', function t() {
	...
	const C = new Float64Array( 1 );
	const WORK = new Float64Array( 1 );
	const info = dormr3( 'column-major', 'left', 'no-transpose', 2, 0, 0, 0, A, 2, TAU, 1, C, 2, WORK, 1 ); // eslint-disable-line max-len
	assert.strictEqual( info, 0, 'returns INFO=0' );
});
...
test( 'main: quick return when K=0 leaves C unchanged', function t() {
	...
	const A = new Float64Array( 4 );
	const TAU = new Float64Array( 2 );
	const C = new Float64Array([ 1, 2, 3, 4 ]);
	const Cexpected = new Float64Array([ 1, 2, 3, 4 ]);
	const WORK = new Float64Array( 2 );
	const info = dormr3( 'column-major', 'left', 'no-transpose', 2, 2, 0, 0, A, 2, TAU, 1, C, 2, WORK, 1 ); // eslint-disable-line max-len
	assert.strictEqual( info, 0, 'returns INFO=0' );
...
test( 'main: ndarray method is the ndarray wrapper', function t() {
	...
	const WORK = new Float64Array( 2 );
	const info = dormr3.ndarray( 'left', 'no-transpose', 2, 2, 0, 0, A, 1, 2, 0, TAU, 1, 0, C, 1, 2, 0, WORK, 1, 0 ); // eslint-disable-line max-len
	assert.strictEqual( info, 0, 'returns INFO=0' );
});
