...
import zlasyfRook from './../lib/ndarray.js';


// FIXTURES //

const fixtureDir = path.join( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures' );
const rawLines = readFileSync( path.join( fixtureDir, 'zlasyf_rook.jsonl' ), 'utf8' ).trim().split( '\n' ); // eslint-disable-line node/no-sync, max-len
...
function runCase( name, uplo, N, nb, entries ) {
	...
	const A = buildMatrix( N, entries );
	const IPIV = new Int32Array( N );
	const W = new Complex128Array( N * Math.max( nb, 1 ) );
	const result = zlasyfRook( uplo, N, nb, A, 1, N, 0, IPIV, 1, 0, W, 1, N, 0 );
