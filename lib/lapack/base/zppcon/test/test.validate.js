...
//
// Both `upper` and `lower` are now BIT-EXACT across the family. (An earlier ~1 ULP
// `lower` offset-dependence — its second solve is a CONJUGATE-TRANSPOSE `Lᴴ·x=y` —
// was initially assumed a benign reorder, but was later traced to a REAL strideAP
// bug in the shared solver `zlatps`: it dropped the stride multiply on its packed
// base pointers. That has since been FIXED; see test/harness/LEARNINGS.md. Any
// difference here now is a genuine offset/stride-base bug.)
const PURE_ADDR = [
...
function runInvariance( uplo ) {
	...
	checked( 'zppcon', 'layout-invariance', function run() {
		const vals = rcondsAcrossLayouts( uplo, Lfac, N, anorm );
		let i;
		if ( uplo === 'upper' ) {
			...
			// lower: now BIT-EXACT too (the zlatps strideAP bug it depended on is fixed).
			for ( i = 1; i < vals.length; i++ ) {
				if ( !Object.is( vals[ i ], vals[ 0 ] ) ) {
					throw new Error( 'zppcon lower pure-addressing invariance: variant '+i+' '+vals[ i ]+' != '+vals[ 0 ] );
				}
			}
		}
	});
