/**
* Property-based validation for zunmqr, following the /blahpack-validate process.
*
* Step 0 classification: `z` -> complex scalar; `unmqr` (BLOCKED apply-Q, NB=32)
...
* factorization (reflectors + TAU as returned by ZGEQRF), and op(Q) is Q
...
* `ref.matmul` and compare to zunmqr's in-place output. Because zungqr forms Q by
* an independent code path, a systematic reflector/side/trans error in zunmqr
* would disagree with the oracle. The conjugate-transpose sweep exercises the
* conjugation of TAU (Qᴴ) and the reflector inner products.
*
* zunmqr is BLOCKED (zlarft + zlarfb) whenever K > NB, falling back to the
* unblocked zunm2r for K <= NB. Its inner kernel is the optimized
* zgemm/ztrsm/zgemv, which selects its summation form by comparing operand
...
* zgeqrf col/row entries.) Step 4c additionally probes the advertised WORK
...
import zunmqr from './../lib/ndarray.js';
...
// zero/trivial corners; zunmqr is unblocked so no NB threshold matters here, but
...
// Drive one property case: factor an R×K panel, apply op(Q) to C0 with zunmqr,
...
function runCase( M, N, K, side, trans, tcode, layout ) {
	...
	zunmqr( side, trans, M, N, K, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], Tr.data, Tr.args[ 0 ], Tr.args[ 1 ], Cr.data, Cr.args[ 0 ], Cr.args[ 1 ], Cr.args[ 2 ], Wo, 1, 0 );
...
// Step 2/5: PROPERTY. op(Q) applied by zunmqr == explicit-Q oracle, swept over
// side × trans × (M,N,K) × dense layouts (all layouts for small/medium dims,
// tight only for large).
test( 'zunmqr: op(Q)·C matches explicit-Q oracle (side × trans × dims × layouts)', function t() {
...
// zunmqr; assert bit-exact output WITHIN a storage-order family (col / row). The
...
test( 'zunmqr: bit-exact within storage-order family (col / row)', function t() {
	var M = 48;
	var N = 40;
			...
			[ [ colLayouts, 'col' ], [ rowLayouts, 'row' ] ].forEach( function eachFam( fam ) {
				checked( NAME, 'layout-invariance', function run() {
					layoutInvariant( fam[ 0 ], function build( layout, i ) {
						...
						zunmqr( side, tc[ 0 ], M, N, K, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], Tr.data, Tr.args[ 0 ], Tr.args[ 1 ], Cr.data, Cr.args[ 0 ], Cr.args[ 1 ], Cr.args[ 2 ], Wo, 1, 0 );
...
test( 'zunmqr: advertised WORK minimum suffices on the blocked path (Step 4c)', function t() {
	[ [ 'left', 80, 50, 40 ], [ 'right', 50, 80, 40 ] ].forEach( function eachCase( cfg ) {
		...
		function run( len ) {
			...
			zunmqr( side, trans, M, N, K, s.Ar.data, s.Ar.args[ 0 ], s.Ar.args[ 1 ], s.Ar.args[ 2 ], s.Tr.data, s.Tr.args[ 0 ], s.Tr.args[ 1 ], Cr.data, Cr.args[ 0 ], Cr.args[ 1 ], Cr.args[ 2 ], Wo, 1, 0 );
			return check.flattenLogical( sc, readFull( Cr, M, N ) );
		}
		...
		var s = setup();
		var Cr = schemes.dense.realize( sc, s.C0, { 'part': 'full' }, null );
		zunmqr( side, trans, M, N, K, s.Ar.data, s.Ar.args[ 0 ], s.Ar.args[ 1 ], s.Ar.args[ 2 ], s.Tr.data, s.Tr.args[ 0 ], s.Tr.args[ 1 ], Cr.data, Cr.args[ 0 ], Cr.args[ 1 ], Cr.args[ 2 ], poisonedWork( sc, minLen ), 1, 0 );
