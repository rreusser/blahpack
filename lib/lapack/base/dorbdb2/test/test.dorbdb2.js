...
import dorbdb2 from './../lib/dorbdb2.js';


// FIXTURES //

const fixtureDir = path.join( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures' );
const lines = readFileSync( path.join( fixtureDir, 'dorbdb2.jsonl' ), 'utf8' ).trim().split( '\n' ); // eslint-disable-line node/no-sync
...
function buildColumnMajor( packed, P, Q ) {
	...
	const buf = new Float64Array( LDX_PAD * Q );
	for ( j = 0; j < Q; j++ ) {
		for ( i = 0; i < P; i++ ) {
...
function buildRowMajor( packed, P, Q ) {
	...
	const buf = new Float64Array( P * LDX_PAD );
	for ( j = 0; j < Q; j++ ) {
		for ( i = 0; i < P; i++ ) {
...
test( 'dorbdb2 has expected arity', function t() {
	assert.strictEqual( dorbdb2.length, 19, 'has expected arity' );
});

test( 'dorbdb2 throws TypeError for invalid order', function t() {
	const X = new Float64Array( 16 );
	assert.throws( function throws() {
		dorbdb2( 'invalid', 8, 2, 2, X, 2, X, 6, X, 1, X, 1, X, 1, X, 1, X, 1, X );
	}, TypeError );
});
...
test( 'dorbdb2 throws RangeError for negative M', function t() {
	const X = new Float64Array( 16 );
	assert.throws( function throws() {
		dorbdb2( 'column-major', -1, 0, 0, X, 1, X, 1, X, 1, X, 1, X, 1, X, 1, X, 1, X );
	}, RangeError );
});
...
test( 'dorbdb2 throws RangeError for invalid Q', function t() {
	const X = new Float64Array( 16 );
	assert.throws( function throws() {
		dorbdb2( 'column-major', 8, 0, -1, X, 1, X, 1, X, 1, X, 1, X, 1, X, 1, X, 1, X );
	}, RangeError );
});
...
test( 'dorbdb2 throws RangeError when P > Q', function t() {
	const X = new Float64Array( 16 );
	assert.throws( function throws() {
		dorbdb2( 'column-major', 10, 4, 2, X, 4, X, 6, X, 1, X, 1, X, 1, X, 1, X, 1, X );
	}, RangeError );
});
...
test( 'dorbdb2 throws RangeError for column-major LDX11 < P', function t() {
	const X = new Float64Array( 16 );
	assert.throws( function throws() {
		dorbdb2( 'column-major', 8, 2, 2, X, 1, X, 6, X, 1, X, 1, X, 1, X, 1, X, 1, X );
	}, RangeError );
});
...
test( 'dorbdb2 throws RangeError for column-major LDX21 < M-P', function t() {
	const X = new Float64Array( 16 );
	assert.throws( function throws() {
		dorbdb2( 'column-major', 8, 2, 2, X, 2, X, 1, X, 1, X, 1, X, 1, X, 1, X, 1, X );
	}, RangeError );
});
...
test( 'dorbdb2 throws RangeError for row-major LDX11 < Q', function t() {
	const X = new Float64Array( 16 );
	assert.throws( function throws() {
		dorbdb2( 'row-major', 8, 2, 2, X, 1, X, 2, X, 1, X, 1, X, 1, X, 1, X, 1, X );
	}, RangeError );
});
...
test( 'dorbdb2 throws RangeError for row-major LDX21 < Q', function t() {
	const X = new Float64Array( 16 );
	assert.throws( function throws() {
		dorbdb2( 'row-major', 8, 2, 2, X, 2, X, 1, X, 1, X, 1, X, 1, X, 1, X, 1, X );
	}, RangeError );
});
...
test( 'dorbdb2 column-major end-to-end against the m10_p3_q5 fixture', function t() {
	...
	const X11 = buildColumnMajor( tc.X11in, 3, 5 );
	const X21 = buildColumnMajor( tc.X21in, 7, 5 );

	const THETA = new Float64Array( LDX_PAD );
	const PHI = new Float64Array( LDX_PAD );
	const TAUP1 = new Float64Array( LDX_PAD );
	const TAUP2 = new Float64Array( LDX_PAD );
	const TAUQ1 = new Float64Array( LDX_PAD );
	const WORK = new Float64Array( LDX_PAD * LDX_PAD );

	const info = dorbdb2( 'column-major', 10, 3, 5, X11, LDX_PAD, X21, LDX_PAD, THETA, 1, PHI, 1, TAUP1, 1, TAUP2, 1, TAUQ1, 1, WORK );
...
test( 'dorbdb2 row-major end-to-end against the m10_p3_q5 fixture', function t() {
	...
	const X11 = buildRowMajor( tc.X11in, 3, 5 );
	const X21 = buildRowMajor( tc.X21in, 7, 5 );
	...
	const info = dorbdb2( 'row-major', 10, 3, 5, X11, LDX_PAD, X21, LDX_PAD, THETA, 1, PHI, 1, TAUP1, 1, TAUP2, 1, TAUQ1, 1, WORK );
	assert.strictEqual( info, 0, 'info' );
