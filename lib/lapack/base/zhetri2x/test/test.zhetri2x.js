...
import zhetri2x from './../lib/zhetri2x.js';
...
test( 'zhetri2x performs trivial inversion (N=1, column-major, lower)', function t() {
	...
	const info = zhetri2x( 'column-major', 'lower', 1, A, 1, IPIV, 1, 0, WORK, 1, nb );
...
test( 'zhetri2x supports row-major layout (LDA swap)', function t() {
	...
	const IPIV = new Int32Array( [ 0 ] );
	const WORK = new Complex128Array( ldwork * ( nb + 3 ) );
	const info = zhetri2x( 'row-major', 'upper', 1, A, 1, IPIV, 1, 0, WORK, 1, nb );
	assert.strictEqual( info, 0, 'info' );
