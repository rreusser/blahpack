...
import zlantr from '../../zlantr/lib/base.js';
...
function ztrcon( norm, uplo, diag, N, A, strideA1, strideA2, offsetA, RCOND, WORK, strideWork, offsetWork, RWORK, strideRWork, offsetRWork ) {
	let ainvnm, normin, xnorm, scale, kase1, ISAVE, KASE, EST, xv, sw, ow, ix;
	...
	RCOND[ 0 ] = 0.0;
	const smlnum = SMLNUM * Math.max( 1, N );

	// Determine norm type
	const onenrm = ( norm === 'one-norm' );
	const upper = ( uplo === 'upper' );

	const zlantrUplo = ( upper ) ? 'upper' : 'lower';
	const zlantrDiag = ( diag === 'unit' ) ? 'unit' : 'non-unit';

	// Compute norm of A
	const anorm = zlantr( norm, zlantrUplo, zlantrDiag, N, N, A, strideA1, strideA2, offsetA, RWORK, strideRWork, offsetRWork );
