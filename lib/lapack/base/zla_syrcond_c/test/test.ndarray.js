...
import zla_syrcond_c from './../lib/ndarray.js';


// FIXTURES //

const fixtureDir = path.join( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures' );
const lines = readFileSync( path.join( fixtureDir, 'zla_syrcond_c.jsonl' ), 'utf8' ).trim().split( '\n' ); // eslint-disable-line node/no-sync
...
function runCase( uplo, c, capply, fixtureName ) {
	...
	const info = zsytrf( uplo, N, AF, 1, N, 0, IPIV, 1, 0 );
	assert.equal( info, 0, fixtureName + ': zsytrf info' );
	const res = zla_syrcond_c( uplo, N, A, 1, N, 0, AF, 1, N, 0, IPIV, 1, 0, c, 1, 0, capply, WORK, 1, 0, RWORK, 1, 0 );
	const tc = findCase( fixtureName );
...
test( 'zla_syrcond_c: n1_upper', function t() {
	...
	const AF = new Complex128Array( [ 5.0, 2.0 ] );
	const IPIV = new Int32Array( [ 0 ] );
	const c = new Float64Array( [ 2.0 ] );
	const WORK = new Complex128Array( 2 * N );
	const RWORK = new Float64Array( N );
	const tc = findCase( 'n1_upper' );
	const res = zla_syrcond_c( 'upper', N, A, 1, N, 0, AF, 1, N, 0, IPIV, 1, 0, c, 1, 0, true, WORK, 1, 0, RWORK, 1, 0 );
...
test( 'zla_syrcond_c: n0 returns 1', function t() {
	...
	const res = zla_syrcond_c( 'upper', 0, A, 1, 1, 0, AF, 1, 1, 0, IPIV, 1, 0, c, 1, 0, true, WORK, 1, 0, RWORK, 1, 0 );
...
test( 'zla_syrcond_c: anorm zero returns 0 (upper)', function t() {
	...
	const AF = new Complex128Array( 4 );
	const IPIV = new Int32Array( N );
...
test( 'zla_syrcond_c: anorm zero returns 0 (lower, nocapply)', function t() {
	...
	const WORK = new Complex128Array( 2 * N );
	const RWORK = new Float64Array( N );
	const res = zla_syrcond_c( 'lower', N, A, 1, N, 0, AF, 1, N, 0, IPIV, 1, 0, c, 1, 0, false, WORK, 1, 0, RWORK, 1, 0 );
	assert.equal( res, 0.0, 'anorm=0 returns 0' );
});
