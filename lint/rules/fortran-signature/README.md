...
```
Fortran source ──extract_metadata.py──▶ data/routines.json ─┐
                                                            ├─▶ gen_fortran_signatures.js
data/fortran-signatures.supplemental.json ──────────────────┘        │
   (routines whose source is not vendored)                           ▼
                                              data/fortran-signatures.json   (clean cache)
                                                                     │
                                     derive.cjs  (Fortran args → signature model)
                                                                     │
                                              rule.cjs  (compare to base.js / ndarray.js)
```

The rule reads only the clean cache (`data/fortran-signatures.json`) via
`lint/lib/fortran-data.cjs`. The cache is regenerable — `node
bin/gen_fortran_signatures.js` reproduces it byte-for-byte — and holds Fortran
facts only, no JavaScript interpretation.
...
| `data/fortran-signatures.json` (repo root `data/`) | The clean, flat, Fortran-only signature cache the rule reads. Regenerated by `bin/gen_fortran_signatures.js`. |
| `data/fortran-signatures.supplemental.json` | Reference Fortran signatures for routines not vendored in `data/` (ARPACK family + a few others), folded into the cache by the generator. |
| `data/fortran-signatures.supplemental.build.cjs` | Authoring script that emits the supplemental file from transcribed reference signatures. |
...
is to add the reference Fortran signature to
`data/fortran-signatures.supplemental.json` (re-run
`bin/gen_fortran_signatures.js`) — the same faithful derivation then applies.
Ingesting
