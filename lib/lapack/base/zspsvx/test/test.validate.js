...
function invNorm1( A0, n, uplo ) {
	...
	const info = zspsv( uplo, n, n, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], ipiv, 1, 0, Br.data, Br.args[ 0 ], Br.args[ 1 ], Br.args[ 2 ] );
	if ( info !== 0 ) {
...
function drive( impl, uplo, N, nrhs, A0, B0, apLayout, bLayout ) {
	...
	const AFr = schemes.packed.realize( sc, A0, { 'part': uplo }, apLayout ); // overwritten by zcopy+zsptrf
	const Br = schemes.dense.realize( sc, B0, { 'part': 'full' }, bLayout );
	const Xr = schemes.dense.realize( sc, B0, { 'part': 'full' }, bLayout ); // overwritten by zlacpy+zsptrs
	const IPIV = new Int32Array( Math.max( N, 1 ) );
	...
	const info = impl( 'not-factored', uplo, N, nrhs, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], AFr.data, AFr.args[ 0 ], AFr.args[ 1 ], IPIV, 1, 0, Br.data, Br.args[ 0 ], Br.args[ 1 ], Br.args[ 2 ], Xr.data, Xr.args[ 0 ], Xr.args[ 1 ], Xr.args[ 2 ], rcond, FERR, 1, 0, BERR, 1, 0, WORK, 1, 0, RWORK, 1, 0 );
	return {
		'Xr': Xr,
		'rcond': rcond,
		'FERR': FERR,
...
function runProperty( uplo, N, nrhs ) {
	...
	const d = drive( zspsvx, uplo, N, nrhs, A0, B0, TIGHTP, TIGHTD );
	const tag = 'zspsvx '+uplo+' N='+N+' nrhs='+nrhs;
	...
	checked( 'zspsvx', 'property', function run() {
		...
		const db = drive( zspsvxBase, uplo, N, nrhs, A0, B0, TIGHTP, TIGHTD );
...
const ALL_DENSE = schemes.dense.layouts();
	...
	UPLOS.forEach( function eachUplo( uplo ) {
		POSITIVE_PACKED.forEach( function eachAP( apL ) {
			ALL_DENSE.forEach( function eachB( bL ) {
				const rng = new RNG( 0x2A00 + N );
				const A0 = logical.symmetric( sc, rng, N );
				const B0 = logical.general( sc, rng, N, nrhs );
				const d = drive( zspsvx, uplo, N, nrhs, A0, B0, apL, bL );
				const tag = 'zspsvx cross-layout '+uplo+' apStride='+apL.stride+' bOrder='+bL.order+' sgn1='+bL.sgn1;
				if ( d.info !== 0 ) {
...
const DENSE_PURE = schemes.dense.pureAddrLayouts();
...
function runInvariance( uplo ) {
	...
	const nrhs = 2;
	const SEED = 0xBEEF;
	const nVariants = Math.max( PACKED_PURE.length, DENSE_PURE.length );
	const variants = [];
	let k;
	...
	checked( 'zspsvx', 'layout-invariance', function run() {
		layoutInvariant( variants, function build( idx ) {
			...
			const A0 = logical.symmetric( sc, rng, N );
			const B0 = logical.general( sc, rng, N, nrhs );
			const d = drive( zspsvx, uplo, N, nrhs, A0, B0, PACKED_PURE[ idx % PACKED_PURE.length ], DENSE_PURE[ idx % DENSE_PURE.length ] );
