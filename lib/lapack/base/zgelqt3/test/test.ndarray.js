...
const raw = readFileSync( path.join( fixtureDir, 'zgelqt3.jsonl' ), 'utf8' ); // eslint-disable-line node/no-sync
...
function runCase( caseName, M, N, rows, tol ) {
	...
	const T = new Complex128Array( M * M );
	const info = zgelqt3( M, N, A, 1, M, 0, T, 1, M, 0 );
...
test( 'zgelqt3: m=0 quick return', function t() {
	...
	const info = zgelqt3( 0, 0, A, 1, 1, 0, T, 1, 1, 0 );
...
test( 'zgelqt3: works with non-trivial offsets and strides', function t() {
	...
	const tc = findCase( 'm2_n4' );
	const LDA = 5;
	const LDT = 4;
	const offsetA = 7;
	const offsetT = 3;
	const A = new Complex128Array( ( LDA * 4 ) + offsetA );
	const T = new Complex128Array( ( LDT * 2 ) + offsetT );
	const rows = [
		[ [ 2.0, 0.1 ], [ 1.5, -0.2 ], [ 0.5, 0.3 ], [ -1.0, 0.4 ] ],
		[ [ 0.7, -0.3 ], [ 3.0, 0.5 ], [ 1.1, -0.4 ], [ 0.4, 0.2 ] ]
	];
	...
	const info = zgelqt3( 2, 4, A, 1, LDA, offsetA, T, 1, LDT, offsetT );
	assert.strictEqual( info, 0 );
...
test( 'zgelqt3: works on a row-major matrix layout', function t() {
	...
	const info = zgelqt3( M, N, A, N, 1, 0, T, M, 1, 0 );
	assert.strictEqual( info, 0 );
