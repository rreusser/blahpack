...
test( 'dlarfy has expected arity', function t() {
	assert.strictEqual( dlarfy.length, 9, 'has expected arity' );
});

test( 'dlarfy throws TypeError for invalid order', function t() {
	assert.throws( function throws() {
		dlarfy( 'invalid', 'upper', 2, new Float64Array( 4 ), 2, 2, new Float64Array( 4 ), 2, new Float64Array( 4 ) );
	}, TypeError );
});

test( 'dlarfy throws TypeError for invalid uplo', function t() {
	assert.throws( function throws() {
		dlarfy( 'row-major', 'invalid', 2, new Float64Array( 4 ), 2, 2, new Float64Array( 4 ), 2, new Float64Array( 4 ) );
	}, TypeError );
});

test( 'dlarfy throws RangeError for negative N', function t() {
	assert.throws( function throws() {
		dlarfy( 'row-major', 'upper', -1, new Float64Array( 4 ), 2, 2, new Float64Array( 4 ), 2, new Float64Array( 4 ) );
	}, RangeError );
});
