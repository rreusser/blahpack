...
function ztgsyl( trans, ijob, M, N, A, strideA1, strideA2, offsetA, B, strideB1, strideB2, offsetB, C, strideC1, strideC2, offsetC, D, strideD1, strideD2, offsetD, E, strideE1, strideE2, offsetE, F, strideF1, strideF2, offsetF, scale, dif, WORK, strideWork, offsetWork, IWORK, strideIWork, offsetIWork ) { // eslint-disable-line max-len, max-params
	...
	// The complex WORK buffer is only referenced on the non-transpose path with
	// `ijob` 1 or 2 (two packed M-by-N copies); otherwise it is unused. Assert
	// its size only on that path so valid no-work callers do not throw.
	if ( trans === 'no-transpose' && ( ijob === 1 || ijob === 2 ) ) {
		const minWork = 2 * M * N;
