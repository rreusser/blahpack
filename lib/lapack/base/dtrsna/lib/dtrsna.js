...
*/
function dtrsna( order, job, howmny, SELECT, strideSELECT, N, T, LDT, VL, LDVL, VR, LDVR, s, strideS, SEP, strideSEP, mm, WORK, LDWORK, IWORK, strideIWork, offsetIWork ) { // eslint-disable-line max-len, max-params
	let st1, st2, svl1, svl2, svr1, svr2, swork1, swork2;
	...
	if ( order === 'column-major' && LDVL < max( 1, N ) ) {
		throw new RangeError( format( 'invalid argument. Tenth argument must be greater than or equal to max(1,N). Value: `%d`.', LDVL ) );
	}
	...
	if ( order === 'column-major' && LDVR < max( 1, N ) ) {
		throw new RangeError( format( 'invalid argument. Twelfth argument must be greater than or equal to max(1,N). Value: `%d`.', LDVR ) );
	}
	...
	if ( order === 'column-major' && LDWORK < max( 1, N ) ) {
		throw new RangeError( format( 'invalid argument. Twentieth argument must be greater than or equal to max(1,N). Value: `%d`.', LDWORK ) );
	}
	...
	if ( WORK === null || WORK === void 0 ) {
		const minWork = Math.max( 1, LDWORK, N + 6 );
		WORK = new Float64Array( minWork );
	}
	if ( IWORK === null || IWORK === void 0 ) {
		const minIwork = Math.max( 1, 2 * (N - 1) );
		IWORK = new Int32Array( minIwork );
		strideIWork = 1;
	}
	return base( job, howmny, SELECT, strideSELECT, 0, N, T, st1, st2, 0, VL, svl1, svl2, 0, VR, svr1, svr2, 0, s, strideS, 0, SEP, strideSEP, 0, mm, WORK, swork1, swork2, 0, IWORK, strideIWork, offsetIWork ); // eslint-disable-line max-len
}
