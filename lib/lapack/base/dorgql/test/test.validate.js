...
/**
* Property-based validation for dorgql, following the /blahpack-validate process.
*
* Step 0 classification: `d` -> real scalar; `or`/`gql` -> BLOCKED formation of
* the orthonormal factor Q (M x N economy, N <= M) from the Householder reflectors
* that `dgeqlf`/`dgeql2` leave in the LAST K columns of A (with tau in TAU). dorgql
...
*       every (M,N).
*
* dorgql is the UNBLOCKED kernel (dlarf -> dgemv/dger); the storage col<->row flip
...
import dorgql from './../lib/ndarray.js';
import dgeqlf from '../../dgeqlf/lib/ndarray.js';

const sc = S.real; // d-routine
const BLOCKED = true; // dorgql is the BLOCKED kernel (dlarft + dlarfb -> dgemm/dtrmm), falling back to dorg2l
const LogicalMatrix = logical.LogicalMatrix;
const NB = 32; // geqlf hardcoded block size (governs the geqlf WORK length only)
const ROUTINE = 'dorgql';
...
test( ROUTINE+': QᴴQ = I and A = Q*L ((M,N) sweep x all layouts)', function t() {
	PAIRS.forEach( function eachPair( pr ) {
		...
		ALL_LAYOUTS.forEach( function eachLayout( layout, li ) {
			...
			dorgql( M, N, K, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], Tr.data, Tr.args[ 0 ], Tr.args[ 1 ], Wo.data, Wo.args[ 0 ], Wo.args[ 1 ] );
...
function runInvariance( variants, fam ) {
	...
	checked( ROUTINE, 'layout-invariance', function run() {
		layoutInvariant( variants, function build( layout, i ) {
			...
			dorgql( MI, NI, K, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], Tr.data, Tr.args[ 0 ], Tr.args[ 1 ], Wo.data, Wo.args[ 0 ], Wo.args[ 1 ] );
...
// Step 4c: WORKSPACE conformance (plain assertion, NOT `checked`). dorgql hardcodes
...
test( ROUTINE+': advertised WORK minimum suffices on the blocked path (Step 4c)', function t() {
	...
	function run( len ) {
		...
		dorgql( M, N, K, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], Tr.data, Tr.args[ 0 ], Tr.args[ 1 ], Wo, 1, 0 );
