...
import zlacon from './../lib/ndarray.js';
...
test( 'zlacon: throws RangeError for negative N', function t() {
	const V = makeComplex128Array( 1 );
	const X = makeComplex128Array( 1 );
...
test( 'zlacon: N=1 quick return', function t() {
	...
	// A = [3+4i] (|.|=5)
	const A = [ 3.0, 4.0 ];
	const V = makeComplex128Array( N );
	const X = makeComplex128Array( N );
	const EST = new Float64Array( 1 );
	const KASE = new Int32Array( 1 );
...
test( 'zlacon: with non-unit strides', function t() {
	...
	const sV = 2;
	const sX = 2;
	const V = makeComplex128Array( N * sV );
	const X = makeComplex128Array( N * sX );
	const xv = reinterpret( X );
	const EST = new Float64Array( 1 );
	const KASE = new Int32Array( 1 );
