...
import dsytri3x from './../lib/ndarray.js';
...
function runFixture( uplo, N, nb, tc ) {
	...
	const info = dsytri3x( uplo, N, A, 1, N, 0, e, 1, 0, ipiv, 1, 0, work, 1, 0, nb );
...
test( 'dsytri_3x: N=1 lower trivial inverse', function t() {
	...
	const ldwork = 1 + 1 + 1;
	const work = new Float64Array( ldwork * 4 );
	const info = dsytri3x( 'lower', 1, A, 1, 1, 0, e, 1, 0, ipiv, 1, 0, work, 1, 0, 1 );
	assert.equal( info, tc.info, 'info' );
...
test( 'dsytri_3x: N=0 quick return', function t() {
	...
	const info = dsytri3x( 'lower', 0, A, 1, 1, 0, e, 1, 0, ipiv, 1, 0, work, 1, 0, 1 );
