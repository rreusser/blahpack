...
import zlarfb_gett from './../lib/ndarray.js';


// FIXTURES //

const fixtureDir = path.join( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures' );
const lines = readFileSync( path.join( fixtureDir, 'zlarfb_gett.jsonl' ), 'utf8' ).trim().split( '\n' ); // eslint-disable-line node/no-sync
...
function makeB( M, N ) {
	let i, j;
	const B = new Complex128Array( Math.max( M, 1 ) * N );
...
function runCase( ident, M, N, K ) {
	...
	const A = makeA( K, N );
	const B = makeB( M, N );
	const LDB = Math.max( M, 1 );
	const LDW = K;
	const WORK = new Complex128Array( LDW * Math.max( K, N - K ) );
...
test( 'zlarfb_gett: not-identity, N > K', function t() {
	const tc = findCase( 'notident_nbig' );
	const out = runCase( 'not-identity', 4, 5, 3 );
	assertArrayClose( out.A, tc.A, 1e-13, 'A' );
	assertArrayClose( out.B, tc.B, 1e-13, 'B' );
});

test( 'zlarfb_gett: not-identity, N = K', function t() {
	const tc = findCase( 'notident_nequal' );
	const out = runCase( 'not-identity', 4, 3, 3 );
	assertArrayClose( out.A, tc.A, 1e-13, 'A' );
	assertArrayClose( out.B, tc.B, 1e-13, 'B' );
});

test( 'zlarfb_gett: identity, N > K', function t() {
	const tc = findCase( 'ident_nbig' );
	const out = runCase( 'identity', 4, 5, 3 );
	assertArrayClose( out.A, tc.A, 1e-13, 'A' );
	assertArrayClose( out.B, tc.B, 1e-13, 'B' );
});

test( 'zlarfb_gett: identity, N = K', function t() {
	const tc = findCase( 'ident_nequal' );
	const out = runCase( 'identity', 4, 3, 3 );
	assertArrayClose( out.A, tc.A, 1e-13, 'A' );
	assertArrayClose( out.B, tc.B, 1e-13, 'B' );
});

test( 'zlarfb_gett: not-identity, M = 0', function t() {
	const tc = findCase( 'notident_m0' );
	const out = runCase( 'not-identity', 0, 5, 3 );
	assertArrayClose( out.A, tc.A, 1e-13, 'A' );
});

test( 'zlarfb_gett: identity, M = 0', function t() {
	const tc = findCase( 'ident_m0' );
	const out = runCase( 'identity', 0, 5, 3 );
	assertArrayClose( out.A, tc.A, 1e-13, 'A' );
});
...
test( 'zlarfb_gett: quick return N = 0', function t() {
	...
	const A = makeA( 3, 3 );
	const B = makeB( 2, 3 );
	const Abefore = reinterpret( A, 0 ).slice();
