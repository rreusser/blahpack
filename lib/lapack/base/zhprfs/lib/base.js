...
import zhpmv from '../../../../blas/base/zhpmv/lib/base.js';
...
* @param {Complex128Array} WORK - caller-owned complex workspace of at least `2*N` elements (two contiguous N-length segments); base.js never allocates
...
function zhprfs( uplo, N, nrhs, AP, strideAP, offsetAP, AFP, strideAFP, offsetAFP, IPIV, strideIPIV, offsetIPIV, B, strideB1, strideB2, offsetB, X, strideX1, strideX2, offsetX, FERR, strideFERR, offsetFERR, BERR, strideBERR, offsetBERR, WORK, strideWork, offsetWork, RWORK, strideRWork, offsetRWork ) { // eslint-disable-line max-len, max-params
	let lstres, count, xk, re, kk, ik, s, i, j, k, p;
	...
	// Partition the caller-owned workspace; base.js never allocates a
	// problem-sized buffer. WORK holds two contiguous N-length complex
	// segments: WRK = WORK[0:N] (residual / main work vector) and
	// V = WORK[N:2N] (the ZLACN2 sign vector). RWORK holds one N-length real
	// segment (RWK). The caller (wrapper / ndarray) owns and sizes the buffers.
	const WRK = ( offsetWork === 0 ) ? WORK : WORK.subarray( offsetWork );
	const V = WRK.subarray( N );
