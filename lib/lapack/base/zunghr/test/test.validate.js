...
var NB = 32; // hardcoded block size in the wrappers
var LDT = 65; // NBMAX+1 leading dim of the gehrd block reflector T

var SIZES = SIZES_SMALL; // 1,2,3,5,8,16,17,33,64 (33/64 straddle+clear the block threshold)
...
test( 'zunghr: QᴴQ = I and A = Q*H*Qᴴ (N sweep x all layouts)', function t() {
	SIZES.forEach( function eachN( N ) {
		...
		ALL_LAYOUTS.forEach( function eachLayout( layout, li ) {
			...
			var Wf = schemes.realizeVector( sc, poison( gehrdWork( N ) ), TIGHT_VEC );
			...
			var Wo = schemes.realizeVector( sc, poison( orghrWork( N ) ), TIGHT_VEC );
			zunghr( N, 1, N, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], Tr.data, Tr.args[ 0 ], Tr.args[ 1 ], Wo.data, Wo.args[ 0 ], Wo.args[ 1 ] );

			var Q = readFull( Ar, N, N );
			var label = 'zunghr N=' + N + ' layout=' + li;
...
function runInvariance( variants, fam ) {
	var N = 64; // nh = 63 > NB=32 -> blocked (zlarft/zlarfb reached)
	var k = N - 1;
	var SEED = 0xF00D;
	...
	checked( 'zunghr', 'layout-invariance', function run() {
		layoutInvariant( variants, function build( layout, i ) {
			...
			var Wo = schemes.realizeVector( sc, poison( orghrWork( N ) ), WORK_LAYOUTS[ i % WORK_LAYOUTS.length ] );
			zunghr( N, 1, N, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], Tr.data, Tr.args[ 0 ], Tr.args[ 1 ], Wo.data, Wo.args[ 0 ], Wo.args[ 1 ] );
