...
import zlaqr0 from '../../zlaqr0/lib/base.js';
...
* H = Z _ T _ Z^H, where T is an upper triangular matrix (the Schur form)
...
*/
function zhseqr( job, compz, N, ilo, ihi, H, strideH1, strideH2, offsetH, w, strideW, offsetW, Z, strideZ1, strideZ2, offsetZ, WORK, strideWork, offsetWork ) {
	let info, kbot, WORKL, HL;
	...
	if ( N > Math.max( NTINY, NMIN ) ) {
		info = zlaqr0( wantt, wantz, N, ilo, ihi, H, strideH1, strideH2, offsetH, w, strideW, offsetW, ilo, ihi, Z, strideZ1, strideZ2, offsetZ, WORK, strideWork, offsetWork );
	} else {
		...
		if ( info > 0 ) {
			kbot = info;
			if ( N >= NL ) {
				info = zlaqr0( wantt, wantz, N, ilo, kbot, H, strideH1, strideH2, offsetH, w, strideW, offsetW, ilo, ihi, Z, strideZ1, strideZ2, offsetZ, WORK, strideWork, offsetWork );
			} else {
				HL = new Complex128Array( 2401 ); // NL*NL, NL=49: fixed-size local embedding matrix (caller WORK too small for the N<NL fallback)
				WORKL = new Complex128Array( 49 ); // NL=49: fixed-size local workspace for the embedded NL-problem
				zlacpy( 'full', N, N, H, strideH1, strideH2, offsetH, HL, 1, NL, 0 );
				const HLv = reinterpret( HL, 0 ); // eslint-disable-line no-var
				HLv[ ( N + ( N - 1 ) * NL ) * 2 ] = 0.0;
				HLv[ ( N + ( N - 1 ) * NL ) * 2 + 1 ] = 0.0;
				zlaset( 'full', NL, NL - N, CZERO, CZERO, HL, 1, NL, N * NL );
				info = zlaqr0( wantt, wantz, NL, ilo, kbot, HL, 1, NL, 0, w, strideW, offsetW, ilo, ihi, Z, strideZ1, strideZ2, offsetZ, WORKL, 1, 0 );
				if ( wantt || info !== 0 ) {
