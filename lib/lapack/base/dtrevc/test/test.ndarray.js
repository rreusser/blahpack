...
import dtrevc from './../lib/ndarray.js';


// FIXTURES //

const fixtureDir = path.join( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures' );
const lines = readFileSync( path.join( fixtureDir, 'dtrevc.jsonl' ), 'utf8' ).trim().split( '\n' );
...
test( 'dtrevc: right eigenvectors, all, 4x4', function t() {
	...
	const info = dtrevc( 'right', 'all', SELECT, 1, 0, N, T, 1, N, 0, VL, 1, N, 0, VR, 1, N, 0, N, 0, WORK, 1, 0 );
...
test( 'dtrevc: left eigenvectors, all, 4x4', function t() {
	...
	const SELECT = new Uint8Array( N );
	const WORK = new Float64Array( 3 * N );
	const info = dtrevc( 'left', 'all', SELECT, 1, 0, N, T, 1, N, 0, VL, 1, N, 0, VR, 1, N, 0, N, 0, WORK, 1, 0 );
	assert.strictEqual( info, tc.info, 'info' );
...
test( 'dtrevc: both eigenvectors, all, 4x4', function t() {
	...
	const SELECT = new Uint8Array( N );
	const WORK = new Float64Array( 3 * N );
	const info = dtrevc( 'both', 'all', SELECT, 1, 0, N, T, 1, N, 0, VL, 1, N, 0, VR, 1, N, 0, N, 0, WORK, 1, 0 );
	assert.strictEqual( info, tc.info, 'info' );
	assertArrayClose( toArray( VR ), tc.VR, 1e-12, 'VR' );
	assertArrayClose( toArray( VL ), tc.VL, 1e-12, 'VL' );
});
...
test( 'dtrevc: right eigenvectors, selected, 4x4', function t() {
	...
	const T = buildT4();
	const VL = new Float64Array( N * N );
	const VR = new Float64Array( N * tc.M );
	const SELECT = new Uint8Array( N );
	SELECT[ 0 ] = 1;
	SELECT[ 1 ] = 0;
	SELECT[ 2 ] = 1;
	SELECT[ 3 ] = 1;
	const WORK = new Float64Array( 3 * N );
	const info = dtrevc( 'right', 'selected', SELECT, 1, 0, N, T, 1, N, 0, VL, 1, N, 0, VR, 1, N, 0, N, 0, WORK, 1, 0 );
	assert.strictEqual( info, tc.info, 'info' );
	assertArrayClose( toArray( VR ), tc.VR, 1e-12, 'VR' );
});
...
test( 'dtrevc: right backtransform, 4x4', function t() {
	...
	const SELECT = new Uint8Array( N );
	const WORK = new Float64Array( 3 * N );
	const info = dtrevc( 'right', 'backtransform', SELECT, 1, 0, N, T, 1, N, 0, VL, 1, N, 0, VR, 1, N, 0, N, 0, WORK, 1, 0 );
	assert.strictEqual( info, tc.info, 'info' );
	assertArrayClose( toArray( VR ), tc.VR, 1e-12, 'VR' );
});
...
test( 'dtrevc: right eigenvectors, all real, 4x4', function t() {
	...
	const tc = findCase( 'right all real 4x4' );
	const N = 4;
	const T = buildT4Real();
	const VL = new Float64Array( N * N );
	const VR = new Float64Array( N * N );
...
test( 'dtrevc: right eigenvectors, N=1', function t() {
	...
	const info = dtrevc( 'right', 'all', SELECT, 1, 0, 1, T, 1, 1, 0, VL, 1, 1, 0, VR, 1, 1, 0, 1, 0, WORK, 1, 0 );
	assert.strictEqual( info, tc.info, 'info' );
	assertArrayClose( toArray( VR ), tc.VR, 1e-12, 'VR' );
});
...
test( 'dtrevc: left selected complex, 4x4', function t() {
	...
	const tc = findCase( 'left selected complex 4x4' );
	const N = 4;
	const T = buildT4();
	const VL = new Float64Array( N * tc.M );
	const VR = new Float64Array( N * N );
	const SELECT = new Uint8Array( N );
	SELECT[ 0 ] = 0;
	SELECT[ 1 ] = 0;
	...
	SELECT[ 3 ] = 1;
	const WORK = new Float64Array( 3 * N );
	const info = dtrevc( 'left', 'selected', SELECT, 1, 0, N, T, 1, N, 0, VL, 1, N, 0, VR, 1, N, 0, N, 0, WORK, 1, 0 );
	assert.strictEqual( info, tc.info, 'info' );
	assertArrayClose( toArray( VL ), tc.VL, 1e-12, 'VL' );
});
...
test( 'dtrevc: left backtransform, 4x4', function t() {
	...
	const SELECT = new Uint8Array( N );
	const WORK = new Float64Array( 3 * N );
	const info = dtrevc( 'left', 'backtransform', SELECT, 1, 0, N, T, 1, N, 0, VL, 1, N, 0, VR, 1, N, 0, N, 0, WORK, 1, 0 );
	assert.strictEqual( info, tc.info, 'info' );
	assertArrayClose( toArray( VL ), tc.VL, 1e-12, 'VL' );
});
...
test( 'dtrevc: right eigenvectors, all, 6x6 mixed', function t() {
	...
	const tc = findCase( 'right all 6x6 mixed' );
	const N = 6;
	const T = buildT6();
	const VL = new Float64Array( N * N );
	const VR = new Float64Array( N * N );
...
test( 'dtrevc: left eigenvectors, all, 6x6 mixed', function t() {
	...
	const tc = findCase( 'left all 6x6 mixed' );
	const N = 6;
	const T = buildT6();
	const VL = new Float64Array( N * N );
	const VR = new Float64Array( N * N );
...
test( 'dtrevc: both backtransform, 6x6', function t() {
	...
	const tc = findCase( 'both backtransform 6x6' );
	const N = 6;
	const T = buildT6();
	const VL = new Float64Array( N * N );
	const VR = new Float64Array( N * N );
	...
	const SELECT = new Uint8Array( N );
	const WORK = new Float64Array( 3 * N );
	const info = dtrevc( 'both', 'backtransform', SELECT, 1, 0, N, T, 1, N, 0, VL, 1, N, 0, VR, 1, N, 0, N, 0, WORK, 1, 0 );
	assert.strictEqual( info, tc.info, 'info' );
	assertArrayClose( toArray( VR ), tc.VR, 1e-12, 'VR' );
	assertArrayClose( toArray( VL ), tc.VL, 1e-12, 'VL' );
});
...
test( 'dtrevc: right selected real, 6x6', function t() {
	...
	const tc = findCase( 'right selected real 6x6' );
	const N = 6;
	const T = buildT6();
	const VL = new Float64Array( N * N );
	const VR = new Float64Array( N * tc.M );
...
test( 'dtrevc: left selected complex first, 6x6', function t() {
	...
	const tc = findCase( 'left selected complex first 6x6' );
	const N = 6;
	const T = buildT6();
	const VL = new Float64Array( N * tc.M );
	const VR = new Float64Array( N * N );
...
test( 'dtrevc: selected last real eigenvalue (N=3, select only last)', function t() {
	...
	T[ 2 + 2*N ] = 3.0;
	const VL = new Float64Array( N * N );
	const VR = new Float64Array( N * 1 );
	const SELECT = new Uint8Array( N );
	SELECT[ 0 ] = 0;
	SELECT[ 1 ] = 0;
	SELECT[ 2 ] = 1;
	const WORK = new Float64Array( 3 * N );
	const info = dtrevc( 'right', 'selected', SELECT, 1, 0, N, T, 1, N, 0, VL, 1, N, 0, VR, 1, N, 0, 1, 0, WORK, 1, 0 );
...
test( 'dtrevc: left selected with last eigenvalue, 3x3', function t() {
	...
	SELECT[ 2 ] = 1;
	const WORK = new Float64Array( 3 * N );
	const info = dtrevc( 'left', 'selected', SELECT, 1, 0, N, T, 1, N, 0, VL, 1, N, 0, VR, 1, N, 0, 1, 0, WORK, 1, 0 );
	assert.strictEqual( info, 0, 'info' );
...
test( 'dtrevc: N=0 quick return', function t() {
	...
	const info = dtrevc( 'right', 'all', SELECT, 1, 0, 0, T, 1, 0, 0, VL, 1, 0, 0, VR, 1, 0, 0, 0, 0, WORK, 1, 0 );
	assert.strictEqual( info, 0, 'info' );
});
