...
test( 'dposv: lower_3x3', function t() {
	...
	const info = dposv( 'lower', 3, 1, A, 1, 3, 0, B, 1, 3, 0 );
...
test( 'dposv: upper_3x3', function t() {
	...
	const A = new Float64Array( [ 4, 2, 1, 2, 5, 3, 1, 3, 9 ] );
	const B = new Float64Array( [ 1, 2, 3 ] );
	const info = dposv( 'upper', 3, 1, A, 1, 3, 0, B, 1, 3, 0 );
	assert.equal( info, tc.info );
	assertArrayClose( toArray( B ), tc.x, 1e-14, 'x' );
});
...
test( 'dposv: n_zero', function t() {
	...
	const info = dposv( 'lower', 0, 1, A, 1, 1, 0, B, 1, 1, 0 );
	assert.equal( info, tc.info );
});
...
test( 'dposv: multi_rhs', function t() {
	...
	const info = dposv( 'lower', 3, 2, A, 1, 3, 0, B, 1, 3, 0 );
	assert.equal( info, tc.info );
	assertArrayClose( toArray( B ), tc.x, 1e-14, 'x' );
});
...
test( 'dposv: nrhs_zero', function t() {
	...
	const info = dposv( 'lower', 3, 0, A, 1, 3, 0, B, 1, 3, 0 );
	assert.equal( info, tc.info );
});
