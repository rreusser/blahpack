...
import zlags2 from '../../zlags2/lib/base.js';
import zlapll from '../../zlapll/lib/base.js';
...
const ZERO = 0.0;
const ONE = 1.0;
const HUGENUM = 1.7976931348623157e+308;
const CZERO = new Complex128( 0.0, 0.0 );
const CONE = new Complex128( 1.0, 0.0 );
const lartgOut = new Float64Array( 3 );
const ssminArr = new Float64Array( 1 );

// Scratch arrays for passing complex sines to zrot:
const snuArr = new Float64Array( 2 );
const snvArr = new Float64Array( 2 );
const snqArr = new Float64Array( 2 );
const conjSnuArr = new Float64Array( 2 );
const conjSnvArr = new Float64Array( 2 );
...
function ztgsja( jobu, jobv, jobq, M, p, N, K, l, A, strideA1, strideA2, offsetA, B, strideB1, strideB2, offsetB, tola, tolb, ALPHA, strideALPHA, offsetALPHA, BETA, strideBETA, offsetBETA, U, strideU1, strideU2, offsetU, V, strideV1, strideV2, offsetV, Q, strideQ1, strideQ2, offsetQ, WORK, strideWork, offsetWork, ncycle ) {
	let kcycle, upper, error, gamma, info, rot, a1, a2, a3, b1, b2, b3, ia, ib;
	let i, j;

	// Decode and test the input parameters
	const initu = ( jobu === 'initialize' );
	const wantu = initu || ( jobu === 'compute-vectors' );

	const initv = ( jobv === 'initialize' );
	const wantv = initv || ( jobv === 'compute-vectors' );

	const initq = ( jobq === 'initialize' );
	const wantq = initq || ( jobq === 'compute-vectors' );
