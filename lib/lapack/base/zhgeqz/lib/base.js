...
import zlanhs from '../../zlanhs/lib/base.js';
...
*/
function zhgeqz( job, compq, compz, N, ilo, ihi, H, strideH1, strideH2, offsetH, T, strideT1, strideT2, offsetT, ALPHA, strideALPHA, offsetALPHA, BETA, strideBETA, offsetBETA, Q, strideQ1, strideQ2, offsetQ, Z, strideZ1, strideZ2, offsetZ, WORK, strideWork, offsetWork, RWORK, strideRWork, offsetRWork ) {
	let ilschr, ilazr2, ilazro, ifirst, ifrstm, ilastm, istart, signbc, ctemp2;
	let ctemp3, eshift, ascale, bscale, safmin;
	const ALPHAv = reinterpret( ALPHA, 0 );
	let shift, abi22, abi12, ctemp, iiter, ilast, jiter, tempr, temp2;
	const BETAv = reinterpret( BETA, 0 );
	let done, absb, atol, btol, info, temp, ad11, ad12, ad21, ad22, u12, ilq;
	let ilz, ulp, sh1, sh2, sq1, sq2, st1, st2, sz1, sz2, rq1, rz1, cArr, sArr;
	let rArr, fv, gv, sv, rv;
	...
	const anorm = zlanhs( 'frobenius', in0, H, strideH1, strideH2, offsetH + (( ilo - 1 ) * strideH1) + (( ilo - 1 ) * strideH2), RWORK, strideRWork, offsetRWork );
	const bnorm = zlanhs( 'frobenius', in0, T, strideT1, strideT2, offsetT + (( ilo - 1 ) * strideT1) + (( ilo - 1 ) * strideT2), RWORK, strideRWork, offsetRWork );
	...
	*/
	function handleZeroTdiag() {
		...
		// CTEMP = H(ILAST, ILAST)
		const idx1 = oH + (ilast * sh1) + (ilast * sh2);
		fv[ 0 ] = Hv[ idx1 ];
		fv[ 1 ] = Hv[ idx1 + 1 ];

		// G = H(ILAST, ILAST-1)
		const idx2 = oH + (ilast * sh1) + (( ilast - 1 ) * sh2);
		gv[ 0 ] = Hv[ idx2 ];
		gv[ 1 ] = Hv[ idx2 + 1 ];
	...
	*/
	function deflateAndExtract() {
		let idx;
		const ab = cabs( Tv, oT + (ilast * st1) + (ilast * st2) );
		if ( ab > safmin ) {
	...
	*/
	function computeWilkinsonShift() {
		let divr, divi, idx, t1r, t1i, t2r, t2i, tr, ti;
	...
	*/
	function doQZSweep() {
		let idx1, idx2, v1r, v1i, v2r, v2i, sr, si, p1, p2, p3, p4, jj, kk, mn;
...
function csqrt( out, z ) {
	...
	const w = Math.sqrt( ( Math.abs( re ) + r ) * 0.5 );
	if ( re >= 0.0 ) {
		out[ 0 ] = w;
		out[ 1 ] = im / ( 2.0 * w );
	} else {
