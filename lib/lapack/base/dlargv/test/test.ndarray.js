...
test( 'dlargv: basic (mixed cases in one call)', function t() {
	const tc = basic;
	const x = new Float64Array( [ 3.0, 0.0, 5.0, 1.0 ] );
	const y = new Float64Array( [ 0.0, 4.0, 3.0, 2.0 ] );
...
test( 'dlargv: all f=0', function t() {
	...
	const y = new Float64Array( [ 5.0, 6.0, 7.0 ] );
	const c = new Float64Array( 3 );
	dlargv( 3, x, 1, 0, y, 1, 0, c, 1, 0 );
	assertArrayClose( x, tc.x, 1e-14, 'x' );
	assertArrayClose( y, tc.y, 1e-14, 'y' );
...
test( 'dlargv: non-unit strides', function t() {
	const tc = stride;
	const x = new Float64Array( [ 3.0, 0.0, 0.0, 0.0, 5.0, 0.0 ] );
	const y = new Float64Array( [ 4.0, 0.0, 0.0, 7.0, 0.0, 0.0, 12.0, 0.0, 0.0 ] );
...
test( 'dlargv: negative values', function t() {
	const tc = negative;
	const x = new Float64Array( [ -3.0, -5.0, 4.0 ] );
	const y = new Float64Array( [ 4.0, -12.0, -3.0 ] );
	const c = new Float64Array( 3 );
	dlargv( 3, x, 1, 0, y, 1, 0, c, 1, 0 );
	assertArrayClose( x, tc.x, 1e-14, 'x' );
	assertArrayClose( y, tc.y, 1e-14, 'y' );
...
test( 'dlargv: equal magnitudes', function t() {
	const tc = equal_mag;
	const x = new Float64Array( [ 1.0, -1.0, 3.0 ] );
	const y = new Float64Array( [ 1.0, 1.0, -3.0 ] );
	const c = new Float64Array( 3 );
	dlargv( 3, x, 1, 0, y, 1, 0, c, 1, 0 );
	assertArrayClose( x, tc.x, 1e-14, 'x' );
	assertArrayClose( y, tc.y, 1e-14, 'y' );
...
test( 'dlargv: large values', function t() {
	...
	const x = new Float64Array( [ 1.0e200, 1.0e-200 ] );
	const y = new Float64Array( [ 1.0e200, 1.0e-200 ] );
...
test( 'dlargv: offset support', function t() {
	...
	const x = new Float64Array( [ 999.0, 4.0, 0.0 ] );
	const y = new Float64Array( [ 999.0, 3.0, 5.0 ] );
	const c = new Float64Array( [ 999.0, 0.0, 0.0 ] );
...
test( 'dlargv: mathematical property (rotation zeros out y)', function t() {
	let residual, ci, si, i;

	const x = new Float64Array( [ 3.0, 7.0, -2.0, 0.0, 0.0 ] );
	const y = new Float64Array( [ 4.0, -1.0, 5.0, 8.0, 0.0 ] );
	const c = new Float64Array( 5 );
	const xOrig = new Float64Array( x );
	const yOrig = new Float64Array( y );
