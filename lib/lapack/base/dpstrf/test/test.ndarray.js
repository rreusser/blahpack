...
import upper_large from './fixtures/upper_large.json' with { type: 'json' };
...
function runTestCase( uplo, N, Ain, tc, tolerance ) {
	...
	const tol = ( tolerance === void 0 ) ? 1e-14 : tolerance;
	const A = new Float64Array( Ain );
	const PIV = new Int32Array( N );
	const RANK = new Int32Array( 1 );
	const WORK = new Float64Array( 2 * N );

	const info = dpstrf( uplo, N, A, 1, N, 0, PIV, 1, 0, RANK, -1.0, WORK );
...
test( 'dpstrf: n_zero', function t() {
	...
	const PIV = new Int32Array( 0 );
	const A = new Float64Array( 0 );
	const info = dpstrf( 'upper', 0, A, 1, 0, 0, PIV, 1, 0, RANK, -1.0, WORK );
