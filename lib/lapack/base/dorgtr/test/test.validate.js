...
var RE = S.real; // d, e, TAU are real for a d-routine
var LogicalMatrix = logical.LogicalMatrix;
var NB = 32; // hardcoded block size in the dorgql/dorgqr sub-kernels
	...
	UPLO.forEach( function eachUplo( uplo ) {
		SWEEP.forEach( function eachN( N ) {
			ALL_LAYOUTS.forEach( function eachLayout( layout, li ) {
				...
				var tauR = schemes.realizeVector( RE, poisonReal( Math.max( N - 1, 0 ) ), VEC_LAYOUTS[ ( li + 2 ) % VEC_LAYOUTS.length ] );

				var out = factor( uplo, N, Ar, dR, eR, tauR );

				// Form Q in place: A <- Q (N x N).
				var Wo = schemes.realizeVector( RE, poisonReal( workLen( N ) ), TIGHT_VEC );
				dorgtr( uplo, N, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], tauR.data, tauR.args[ 0 ], tauR.args[ 1 ], Wo.data, Wo.args[ 0 ], Wo.args[ 1 ] );

				var Q = readFull( Ar, N );
				var label = 'dorgtr ' + uplo + ' N=' + N + ' layout=' + li;
				...
				var recon = ref.matmul( sc, ref.matmul( sc, Q, T ), Q, { 'transb': 't' } );
...
function runInvariance( variants, fam, uplo ) {
	...
	var tF = schemes.realizeVector( RE, poisonReal( N - 1 ), TIGHT_VEC );
	...
	checked( 'dorgtr', 'layout-invariance', function run() {
		layoutInvariant( variants, function build( layout, i ) {
			...
			var Tr = schemes.realizeVector( RE, taus, VEC_LAYOUTS[ i % VEC_LAYOUTS.length ] );
			var Wo = schemes.realizeVector( RE, poisonReal( workLen( N ) ), TIGHT_VEC );
...
test( 'dorgtr: advertised WORK minimum suffices on the blocked path (Step 4c)', function t() {
	UPLO.forEach( function eachUplo( uplo ) {
		...
		var SEED = 0xB10C;
		var label = 'dorgtr WORK-min ' + uplo + ' N=' + N;
		...
		function run( len ) {
			...
			var Tr = schemes.realizeVector( RE, taus, TIGHT_VEC );
