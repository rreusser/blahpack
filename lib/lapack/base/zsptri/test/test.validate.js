...
function factorOne( A0, n, uplo ) {
	...
	const info = zsptrf( uplo, n, R.data, R.args[ 0 ], R.args[ 1 ], ipiv, 1, 0 );
	if ( info !== 0 ) {
...
function invertAt( F, n, uplo, layout ) {
	...
	const info = zsptri( uplo, n, R.data, R.args[ 0 ], R.args[ 1 ], ipiv, 1, 0, work, 1, 0 );
	if ( info !== 0 ) {
...
test( 'zsptri: workspace minimum (len N) suffices with a poisoned buffer', function t() {
	...
	checked( 'zsptri', 'workspace', function check_() {
		const minLen = assertWorkspaceSufficient( run, {}, 'zsptri WORK@N=33 upper' );
		if ( minLen !== N ) {
