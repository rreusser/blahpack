...
import dsyr from '../../../../blas/base/dsyr/lib/base.js';
...
function dpbtf2( uplo, N, kd, AB, strideAB1, strideAB2, offsetAB ) {
	...
	// NOTE: the Fortran reference uses KLD = MAX(1, LDAB-1). That MAX guard is

	// only correct for the reference's tight column-major storage (sa1=1,

	// sa2=LDAB => sa2-sa1 = LDAB-1 > 0). Under general strides the diagonal band

	// step (AB(r,c) -> AB(r-1,c+1), flat delta sa2-sa1) can be negative (e.g.

	// row-major, where sa1 >> sa2) and clamping it to 1 corrupts the DSCAL/DSYR

	// strides. The MAX(1,...) only ever mattered when LDAB=1, but then kd=0 =>

	// kn=0 and neither DSCAL nor DSYR is called, so the genuine signed step is

	// always correct here. (See test/harness/LEARNINGS.md 2026-07-17 dpbtf2.)
	const kld = sa2 - sa1;
