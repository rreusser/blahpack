...
import zlaLinBerr from './../lib/zla_lin_berr.js';
...
test( 'zlaLinBerr has expected arity', function t() {
	assert.strictEqual( zlaLinBerr.length, 8, 'has expected arity' );
});

test( 'zlaLinBerr throws RangeError for negative N', function t() {
	let threw = false;
	try {
		zlaLinBerr( -1, 2, 2, new Complex128Array( 4 ), 1, new Float64Array( 4 ), 1, new Float64Array( 2 ) ); // eslint-disable-line max-len
	} catch ( err ) {
