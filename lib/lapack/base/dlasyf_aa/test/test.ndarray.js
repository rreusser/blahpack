...
import dlasyfAa from './../lib/ndarray.js';


// FIXTURES //

const fixtureDir = path.join( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures' );
const raw = readFileSync( path.join( fixtureDir, 'dlasyf_aa.jsonl' ), 'utf8' ); // eslint-disable-line node/no-sync
...
test( 'dlasyfAa: lower, M=0 quick return', function t() {
	...
	const info = dlasyfAa( 'lower', 1, 0, 1, A, 1, 1, 0, IPIV, 1, 0, H, 1, 1, 0, WORK, 1, 0 ); // eslint-disable-line max-len
...
test( 'dlasyfAa: upper, M=0 quick return', function t() {
	...
	const H = new Float64Array( 1 );
	const A = new Float64Array( 1 );
	const info = dlasyfAa( 'upper', 1, 0, 1, A, 1, 1, 0, IPIV, 1, 0, H, 1, 1, 0, WORK, 1, 0 ); // eslint-disable-line max-len
	assert.equal( info, 0, 'M=0 returns 0' );
	assert.equal( A[ 0 ], 0.0, 'M=0 leaves A unchanged' );
});
...
test( 'dlasyfAa: lower, zero pivot triggers dlaset', function t() {
	...
	A[ 24 ] = 1.0;
	const IPIV = new Int32Array( 5 );
	const H = new Float64Array( 15 );
	const WORK = new Float64Array( 5 );
	const info = dlasyfAa( 'lower', 1, 5, 3, A, 1, 5, 0, IPIV, 1, 0, H, 1, 5, 0, WORK, 1, 0 ); // eslint-disable-line max-len
...
test( 'dlasyfAa: upper, zero pivot triggers dlaset', function t() {
	...
	const H = new Float64Array( 15 );
	const WORK = new Float64Array( 5 );
	const info = dlasyfAa( 'upper', 1, 5, 3, A, 1, 5, 0, IPIV, 1, 0, H, 1, 5, 0, WORK, 1, 0 ); // eslint-disable-line max-len
	assert.equal( info, 0, 'returns 0' );
});
