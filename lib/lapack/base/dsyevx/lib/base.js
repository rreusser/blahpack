...
function dsyevx( jobz, range, uplo, N, A, strideA1, strideA2, offsetA, vl, vu, il, iu, abstol, out, w, strideW, offsetW, Z, strideZ1, strideZ2, offsetZ, WORK, strideWork, offsetWork, IWORK, strideIWork, offsetIWork, IFAIL, strideIFAIL, offsetIFAIL ) {
	let iscale, abstll, indwkn, nsplit, indibl, indisp, indiwo, sigma;
	...
	if ( M === 0 && info === 0 ) {
		...
		if ( wantz && M > 0 ) {
			...
			// Fortran: INDWKN = INDE, LLWRKN = LWORK - INDWKN + 1. The caller
			// owns the workspace (its usable length is `WORK.length - offsetWork`),
			// so no explicit length is threaded to dormtr below.
			indwkn = inde;
