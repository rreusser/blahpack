...
import dlarrk from './../lib/ndarray.js';
...
test( 'dlarrk ndarray: stride support', function t() {
	...
	const d = new Float64Array( [ 2.0, 999.0, 2.0, 999.0, 2.0 ] );
	const e2 = new Float64Array( [ 1.0, 999.0, 1.0 ] );
	const w = new Float64Array( 1 );
	const werr = new Float64Array( 1 );
	const info = dlarrk( 3, 2, 0.0, 4.0, d, 2, 0, e2, 2, 0, 1.0e-300, 1.0e-12, w, werr ); // eslint-disable-line max-len
...
test( 'dlarrk ndarray: offset support', function t() {
	...
	const d = new Float64Array( [ 999.0, 2.0 ] );
	const e2 = new Float64Array( [ 999.0 ] );
	const w = new Float64Array( 1 );
	const werr = new Float64Array( 1 );
	const info = dlarrk( 1, 1, 0.0, 4.0, d, 1, 1, e2, 1, 0, 1.0e-300, 1.0e-12, w, werr ); // eslint-disable-line max-len
	assert.equal( info, 0, 'info' );
	assertClose( w[ 0 ], 2.0, 1e-10, 'w' );
});
