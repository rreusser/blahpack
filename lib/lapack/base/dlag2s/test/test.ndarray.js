...
import dlag2s from './../lib/ndarray.js';
const ndarrayFn = dlag2s;


// FIXTURES //

const fixtureDir = path.join( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures' );
const lines = readFileSync( path.join( fixtureDir, 'dlag2s.jsonl' ), 'utf8' ).trim().split( '\n' ); // eslint-disable-line node/no-sync
...
test( 'dlag2s: basic_3x3 (column-major)', function t() {
	const tc = findCase( 'basic_3x3' );
	const SA = new Float64Array( 9 );
	const A = new Float64Array( 9 );
	let i;
	for ( i = 0; i < 9; i++ ) {
		A[ i ] = ( i + 1 ) * 0.5;
	}
	const info = dlag2s( 3, 3, A, 1, 3, 0, SA, 1, 3, 0 );
	assert.strictEqual( info, tc.info, 'info' );
	assertArrayClose( SA, tc.sa, 0.0, 'sa' );
});

test( 'dlag2s: rect_2x4 (column-major)', function t() {
	const tc = findCase( 'rect_2x4' );
	const SA = new Float64Array( 8 );
	const A = new Float64Array( 8 );
	let i;
	for ( i = 0; i < 8; i++ ) {
		A[ i ] = ( i + 1 ) - 4.5;
	}
	const info = dlag2s( 2, 4, A, 1, 2, 0, SA, 1, 2, 0 );
	assert.strictEqual( info, tc.info, 'info' );
	assertArrayClose( SA, tc.sa, 0.0, 'sa' );
});

test( 'dlag2s: rect_4x2 (column-major)', function t() {
	const tc = findCase( 'rect_4x2' );
	const SA = new Float64Array( 8 );
	const A = new Float64Array( 8 );
	let i;
	for ( i = 0; i < 8; i++ ) {
	...
	const info = dlag2s( 4, 2, A, 1, 4, 0, SA, 1, 4, 0 );
	assert.strictEqual( info, tc.info, 'info' );
	assertArrayClose( SA, tc.sa, 0.0, 'sa' );
});

test( 'dlag2s: padded_lda (LDA=4, M=2, N=3)', function t() {
	const tc = findCase( 'padded_lda' );
	const SA = new Float64Array( 12 );
	...
	A[ 8 ] = 5.0;
	A[ 9 ] = -6.5;
	const info = dlag2s( 2, 3, A, 1, 4, 0, SA, 1, 4, 0 );
	assert.strictEqual( info, tc.info, 'info' );
...
test( 'dlag2s: m_zero leaves SA untouched and returns 0', function t() {
	...
	const SA = new Float64Array( 3 );
	const A = new Float64Array( 3 );
	SA[ 0 ] = 99.0;
	const info = dlag2s( 0, 3, A, 1, 1, 0, SA, 1, 1, 0 );
	assert.strictEqual( info, tc.info, 'info' );
...
test( 'dlag2s: scalar 1x1 rounds to single precision', function t() {
	const tc = findCase( 'scalar_1x1' );
	const SA = new Float64Array( 1 );
	const A = new Float64Array( 1 );
	A[ 0 ] = 3.14159265358979;
	const info = dlag2s( 1, 1, A, 1, 1, 0, SA, 1, 1, 0 );
	assert.strictEqual( info, tc.info, 'info' );
...
test( 'dlag2s: row-major via ndarray wrapper', function t() {
	...
	A[ 4 ] = -5.5;
	A[ 5 ] = 6.75;
	const info = ndarrayFn( 2, 3, A, 3, 1, 0, SA, 3, 1, 0 );
...
test( 'dlag2s: offsets are honored', function t() {
	const SA = new Float64Array( 10 );
	const A = new Float64Array( 10 );
	A[ 4 ] = 1.25;
	A[ 5 ] = -2.5;
	A[ 6 ] = 3.75;
	A[ 7 ] = -4.5;
	const info = dlag2s( 2, 2, A, 1, 2, 4, SA, 1, 2, 4 );
	assert.strictEqual( info, 0, 'info' );
