...
const ALPHA = ( 1.0 + Math.sqrt( 17.0 ) ) / 8.0;
...
* JS: `(i-1 + j*(j-1)/2) * sap`, 0-based Float64 position (re part), where
* `sap = strideAP*2` accounts for the packed complex element stride.
*
* @private
* @param {integer} i - 1-based row index
* @param {integer} j - 1-based column index
* @param {integer} sap - Float64 stride between packed complex elements (strideAP*2)
* @returns {integer} 0-based Float64 offset for real part
*/
function iupp( i, j, sap ) {
	return ( ( i - 1 ) + ( ( j * ( j - 1 ) / 2 )|0 ) ) * sap;
}
...
* JS: `(i-1 + (2*N-j)*(j-1)/2) * sap`, 0-based Float64 position (re part), where
* `sap = strideAP*2` accounts for the packed complex element stride.
*
* @private
* @param {integer} i - 1-based row index
...
* @returns {integer} 0-based Float64 offset for real part
*/
function ilow( i, j, N, sap ) {
	return ( ( i - 1 ) + ( ( ( (2 * N) - j ) * ( j - 1 ) / 2 )|0 ) ) * sap;
}
...
function zhptrf( uplo, N, AP, strideAP, offsetAP, IPIV, strideIPIV, offsetIPIV ) {
	let absakk, colmax, rowmax, kstep, wkm1R, wkm1I, wkp1R, wkp1I, info, imax;
	let jmax, d12R, d12I, d21R, d21I, wkR, wkI, knc, kpc, npp, d11, d22, tR, tI;
	let tt, r1, kk, kp, kc, kx, p1, p2, p3, p4, d, k, i, j;
		...
		while ( k >= 1 ) {
			...
			if ( Math.max( absakk, colmax ) === 0.0 ) {
				...
				if ( kp === kk ) {
					...
					// kx tracks the Fortran running index KX = KPC + KP - 1.
					kx = kpc + kp - 1;
					for ( j = kp + 1; j <= kk - 1; j++ ) {
						kx = kx + j - 1;
		...
		while ( k <= N ) {
			...
			if ( Math.max( absakk, colmax ) === 0.0 ) {
				...
				if ( kstep === 1 ) {
					...
					for ( j = k + 2; j <= N; j++ ) {
						p1 = oAP + ilow( j, k, N, sap );
						p2 = oAP + ilow( j, k + 1, N, sap );
						...
						for ( i = j; i <= N; i++ ) {
							...
							p3 = oAP + ilow( i, j, N, sap );
							p4 = oAP + ilow( i, k, N, sap );
							tR = oAP + ilow( i, k + 1, N, sap );
