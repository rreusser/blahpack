...
import dtzrzf from './../lib/dtzrzf.js';
...
test( 'dtzrzf returns 0 for M=0 (quick return)', function t() {
	const info = dtzrzf( 'column-major', 0, 5, new Float64Array( 5 ), 1, new Float64Array( 1 ), 1, new Float64Array( 1 ), 1 );
...
test( 'dtzrzf accepts row-major layout', function t() {
	...
	const info = dtzrzf( 'row-major', 2, 4, A, 4, TAU, 1, WORK, 1 );
	assert.strictEqual( info, 0, 'INFO' );
