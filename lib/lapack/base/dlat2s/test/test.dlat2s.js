...
import dlat2s from './../lib/dlat2s.js';
...
test( 'dlat2s column-major upper converts values', function t() {
	...
	const info = dlat2s( 'column-major', 'upper', 3, A, 3, SA, 3 );
...
test( 'dlat2s row-major lower converts values', function t() {
	const A = new Float64Array( [ 1.0, 0.0, 0.0, 2.0, 3.0, 0.0, 4.0, 5.0, 6.0 ] );
	const SA = new Float32Array( 9 );
	const info = dlat2s( 'row-major', 'lower', 3, A, 3, SA, 3 );
	assert.strictEqual( info, 0 );
...
test( 'dlat2s returns info=1 on overflow', function t() {
	const A = new Float64Array( [ 1.0, 2.0, 1e300, 4.0 ] );
	const SA = new Float32Array( 4 );
	const info = dlat2s( 'column-major', 'upper', 2, A, 2, SA, 2 );
