...
import dlaqps from './../lib/ndarray.js';
...
test( 'dlaqps: basic_4x3_nb2', function t() {
	...
	const A = makeMatrix( MAXM, 4, 3, [
		[ 1, 2, 0, 1 ],
		[ 0, 1, 3, 2 ],
		[ 3, 0, 1, 2 ]
	]);
	const JPVT = new Int32Array( [ 1, 2, 3, 0, 0, 0, 0, 0 ] );
	const VN1 = new Float64Array( 8 );
	const VN2 = new Float64Array( 8 );
	const norms = computeNorms( A, MAXM, 4, 3, 0 );
	VN1[ 0 ] = norms[ 0 ];
	VN1[ 1 ] = norms[ 1 ];
	VN1[ 2 ] = norms[ 2 ];
	VN2[ 0 ] = norms[ 0 ];
	VN2[ 1 ] = norms[ 1 ];
	VN2[ 2 ] = norms[ 2 ];
	const TAU = new Float64Array( 8 );
	const AUXV = new Float64Array( 8 );
	const F = new Float64Array( MAXN * 8 );
	const kb = dlaqps( 4, 3, 0, 2, A, 1, MAXM, 0, JPVT, 1, 0, TAU, 1, 0, VN1, 1, 0, VN2, 1, 0, AUXV, 1, 0, F, 1, MAXN, 0 ); // eslint-disable-line max-len
...
test( 'dlaqps: rect_6x4_nb3', function t() {
	...
	const A = makeMatrix( MAXM, 6, 4, [
		[ 2, 1, 0, 3, 1, 0 ],
		[ 1, 0, 2, 1, 0, 3 ],
		[ 0, 3, 1, 0, 2, 1 ],
		[ 1, 2, 0, 1, 3, 0 ]
	]);
	const JPVT = new Int32Array( [ 1, 2, 3, 4, 0, 0, 0, 0 ] );
	const VN1 = new Float64Array( 8 );
	const VN2 = new Float64Array( 8 );
	const norms = computeNorms( A, MAXM, 6, 4, 0 );
	for ( i = 0; i < 4; i++ ) { VN1[ i ] = norms[ i ]; VN2[ i ] = norms[ i ]; }
	const TAU = new Float64Array( 8 );
	const AUXV = new Float64Array( 8 );
	const F = new Float64Array( MAXN * 8 );
	const kb = dlaqps( 6, 4, 0, 3, A, 1, MAXM, 0, JPVT, 1, 0, TAU, 1, 0, VN1, 1, 0, VN2, 1, 0, AUXV, 1, 0, F, 1, MAXN, 0 ); // eslint-disable-line max-len
	assert.equal( kb, tc.kb );
...
test( 'dlaqps: nb_1', function t() {
	...
	const VN1 = new Float64Array( 8 );
	const VN2 = new Float64Array( 8 );
	const norms = computeNorms( A, MAXM, 3, 3, 0 );
	for ( i = 0; i < 3; i++ ) { VN1[ i ] = norms[ i ]; VN2[ i ] = norms[ i ]; }
	const TAU = new Float64Array( 8 );
	const AUXV = new Float64Array( 8 );
	const F = new Float64Array( MAXN * 8 );
	const kb = dlaqps( 3, 3, 0, 1, A, 1, MAXM, 0, JPVT, 1, 0, TAU, 1, 0, VN1, 1, 0, VN2, 1, 0, AUXV, 1, 0, F, 1, MAXN, 0 ); // eslint-disable-line max-len
	assert.equal( kb, tc.kb );
	assertArrayClose( extractFlat( A, MAXM * 3 ), tc.a, 1e-13, 'a' );
...
test( 'dlaqps: offset_1', function t() {
	...
	const MAXM = 8;
	const tc = offset_1;
	const A = makeMatrix( MAXM, 4, 2, [
		[ 5, 0, 0, 0 ],
		[ 1, 2, 1, 3 ]
	]);
	const JPVT = new Int32Array( [ 1, 2, 0, 0, 0, 0, 0, 0 ] );
	const VN1 = new Float64Array( 8 );
	const VN2 = new Float64Array( 8 );
	const norms = computeNorms( A, MAXM, 4, 2, 1 );
	for ( i = 0; i < 2; i++ ) { VN1[ i ] = norms[ i ]; VN2[ i ] = norms[ i ]; }
	const TAU = new Float64Array( 8 );
	const AUXV = new Float64Array( 8 );
	const F = new Float64Array( MAXN * 8 );
	const kb = dlaqps( 4, 2, 1, 2, A, 1, MAXM, 0, JPVT, 1, 0, TAU, 1, 0, VN1, 1, 0, VN2, 1, 0, AUXV, 1, 0, F, 1, MAXN, 0 ); // eslint-disable-line max-len
	assert.equal( kb, tc.kb );
...
test( 'dlaqps: collinear_norm_recomp', function t() {
	...
	const VN1 = new Float64Array( 8 );
	const VN2 = new Float64Array( 8 );
	const norms = computeNorms( A, MAXM, 6, 3, 0 );
	for ( i = 0; i < 3; i++ ) { VN1[ i ] = norms[ i ]; VN2[ i ] = norms[ i ]; }
	const TAU = new Float64Array( 8 );
	const AUXV = new Float64Array( 8 );
	const F = new Float64Array( MAXN * 8 );
	const kb = dlaqps( 6, 3, 0, 2, A, 1, MAXM, 0, JPVT, 1, 0, TAU, 1, 0, VN1, 1, 0, VN2, 1, 0, AUXV, 1, 0, F, 1, MAXN, 0 ); // eslint-disable-line max-len
	assert.equal( kb, tc.kb );
	assertArrayClose( extractFlat( A, MAXM * 3 ), tc.a, 1e-13, 'a' );
	assertArrayClose( Array.prototype.slice.call( TAU, 0, kb ), tc.tau.slice( 0, kb ), 1e-13, 'tau' ); // eslint-disable-line max-len
	assert.deepStrictEqual( Array.prototype.slice.call( JPVT, 0, 3 ), tc.jpvt );
