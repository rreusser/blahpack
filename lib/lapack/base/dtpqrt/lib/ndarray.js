...
function dtpqrt( M, N, l, nb, A, strideA1, strideA2, offsetA, B, strideB1, strideB2, offsetB, T, strideT1, strideT2, offsetT, WORK, strideWork, offsetWork ) {
	...
	// from an out-of-bounds read. WORK holds at least `nb*N` reals; the
	// M===0/N===0 case is a quick return that needs no workspace.
	if ( M > 0 && N > 0 && ( !WORK || ( WORK.length - offsetWork ) < ( nb*N ) ) ) {
		throw new RangeError( format( 'invalid argument. WORK array must have at least %d elements from offset %d. Provided length: %d.', nb*N, offsetWork, ( WORK ) ? WORK.length : 0 ) );
	}
