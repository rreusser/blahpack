...
import zungtr_4x4_l from './fixtures/zungtr_4x4_l.json' with { type: 'json' };
import zungtr_4x4_u from './fixtures/zungtr_4x4_u.json' with { type: 'json' };
import zungtr_3x3_l from './fixtures/zungtr_3x3_l.json' with { type: 'json' };
import zungtr_1x1 from './fixtures/zungtr_1x1.json' with { type: 'json' };
...
function zhetrdThenZungtr( uplo, N, AHerm ) {
	let WORK = new Complex128Array( 256 );
	const TAU = new Complex128Array( Math.max( N - 1, 1 ) );
	...
	const info = zungtr( uplo, N, Q, 1, N, 0, TAU, 1, 0, WORK, 1, 0 );
...
test( 'zungtr: 4x4 lower', function t() {
	const tc = zungtr_4x4_l;
	const N = 4;
	const AHerm = buildHerm4x4();
	const result = zhetrdThenZungtr( 'lower', N, AHerm );
	// Fixture has LDA=N=4, so dense and fixture layout match
	const expected = extractFixtureQ( tc.Q, N, N, N );
...
test( 'zungtr: 4x4 upper', function t() {
	const tc = zungtr_4x4_u;
	const N = 4;
	const AHerm = buildHerm4x4();
	const result = zhetrdThenZungtr( 'upper', N, AHerm );
	const expected = extractFixtureQ( tc.Q, N, N, N );
...
test( 'zungtr: 3x3 lower', function t() {
	const tc = zungtr_3x3_l;
	const N = 3;
	const LDA_FIXTURE = 4; // Fortran test uses LDA=NMAX=4
	const AHerm = buildHerm3x3();
	const result = zhetrdThenZungtr( 'lower', N, AHerm );
	const expected = extractFixtureQ( tc.Q, N, N, LDA_FIXTURE );
...
test( 'zungtr: 1x1', function t() {
	const tc = zungtr_1x1;
	const N = 1;
	const AHerm = new Float64Array([ 5, 0 ]);
	const result = zhetrdThenZungtr( 'lower', N, AHerm );
...
test( 'zungtr: N=0 quick return (lower)', function t() {
	...
	const info = zungtr( 'lower', 0, A, 1, 1, 0, TAU, 1, 0, WORK, 1, 0 );
...
test( 'zungtr: N=0 quick return (upper)', function t() {
	...
	const info = zungtr( 'upper', 0, A, 1, 1, 0, TAU, 1, 0, WORK, 1, 0 );
	assert.equal( info, 0, 'info' );
});
...
test( 'zungtr: Q is unitary (4x4 lower)', function t() {
	...
	const result = zhetrdThenZungtr( 'lower', N, AHerm );
	const Qv = result.Qv;
	let reKI, imKI, reKJ, imKJ, sumRe, sumIm, i, j, k;
...
test( 'zungtr: N=2 edge case (lower)', function t() {
	const N = 2;
	const AHerm = new Float64Array([ 3, 0, 1, 1, 1, -1, 5, 0 ]);
	const result = zhetrdThenZungtr( 'lower', N, AHerm );
	const Qv = result.Qv;
	let reKI, imKI, reKJ, imKJ, sumRe, sumIm, i, j, k;
