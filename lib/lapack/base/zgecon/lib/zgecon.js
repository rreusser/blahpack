...
* @param {(Float64Array|null)} RWORK - workspace (>= `2*N` reals); auto-allocated when `null`
...
*/
function zgecon( order, norm, N, A, LDA, anorm, rcond, WORK, strideWork, RWORK, strideRWork ) {
	let sa1, sa2;
	...
	if ( RWORK === null || RWORK === void 0 ) {
		RWORK = new Float64Array( max( 1, 2*N ) );
		strideRWork = 1;
	}
	const ow = stride2offset( 2*N, strideWork );
	const or = stride2offset( 2*N, strideRWork );
	return base( norm, N, A, sa1, sa2, 0, anorm, rcond, WORK, strideWork, ow, RWORK, strideRWork, or );
}
