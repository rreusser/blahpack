...
import zla_herpvgrw from './../lib/index.js';
...
FIXTURES.forEach( function each( fx ) {
	test( 'ndarray fixture: ' + fx.name, function t() {
		...
		const rpvgrw = zla_herpvgrw.ndarray( ( fx.name.indexOf( 'upper' ) >= 0 ) ? 'upper' : 'lower', N, info, A, 1, N, 0, AF, 1, N, 0, IPIV, 1, 0, WORK, 1, 0 );
...
test( 'ndarray N=0 returns 1', function t() {
	const rpvgrw = zla_herpvgrw.ndarray( 'upper', 0, 0, new Complex128Array( 0 ), 1, 1, 0, new Complex128Array( 0 ), 1, 1, 0, new Int32Array( 0 ), 1, 0, new Float64Array( 0 ), 1, 0 );
...
test( 'ndarray triggers diagonal update branch (lower 2x2 pivot, AF[k,k] dominant)', function t() {
	...
	const rpvgrw = zla_herpvgrw.ndarray( 'lower', N, 0, A, 1, N, 0, AF, 1, N, 0, IPIV, 1, 0, WORK, 1, 0 );
