...
import dsytri2x from './../lib/ndarray.js';


// FIXTURES //

const fixtureDir = path.join( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures' ); // eslint-disable-line max-len
const lines = readFileSync( path.join( fixtureDir, 'dsytri2x.jsonl' ), 'utf8' ).trim().split( '\n' ); // eslint-disable-line max-len
...
function runFixture( uplo, N, nb, tc ) {
	...
	const ldwork = N + nb + 1;
	const work = new Float64Array( ldwork * ( nb + 3 ) );
	const info = dsytri2x( uplo, N, A, 1, N, 0, ipiv, 1, 0, work, 1, 0, nb );
...
test( 'dsytri2x: N=1 lower', function t() {
	...
	const ldwork = 1 + nb + 1;
	const work = new Float64Array( ldwork * ( nb + 3 ) );
	const info = dsytri2x( 'lower', 1, A, 1, 1, 0, ipiv, 1, 0, work, 1, 0, nb );
	assert.equal( info, tc.info, 'info' );
...
test( 'dsytri2x: N=0 quick return', function t() {
	...
	const info = dsytri2x( 'lower', 0, A, 1, 1, 0, ipiv, 1, 0, work, 1, 0, nb );
