...
function dggrqf( M, p, N, A, strideA1, strideA2, offsetA, TAUA, strideTAUA, offsetTAUA, B, strideB1, strideB2, offsetB, TAUB, strideTAUB, offsetTAUB, work, strideWork, offsetWork ) { // eslint-disable-line max-len, max-params
	...
	// Blocked minimum: this driver forwards ONE shared work buffer to blocked
	// dgerqf(M,N), dormrq('R',P,N,kA), and dgeqrf(P,N), each of which (NB=32) stores a
	// trailing block-reflector T. The reference unblocked lower bound max(1,M,P,N)
	// UNDER-counts our non-adaptive path, so advertise the max of the three sub-kernels'
	// blocked needs.
	const minWork = Math.max( 1, M, p, N,
		( Math.min( M, N ) > 32 ) ? ( ( M * 32 ) + ( 32 * 32 ) ) : M, // dgerqf(M,N)
		( Math.min( M, N ) > 32 ) ? ( ( p * 32 ) + ( 33 * 32 ) ) : p, // dormrq('R',P,N,kA)
		( Math.min( p, N ) > 32 ) ? ( ( N * 32 ) + ( 32 * 32 ) ) : N  // dgeqrf(P,N)
	);
