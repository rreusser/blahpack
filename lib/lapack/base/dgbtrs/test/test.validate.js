/**
* Property-based validation for dgbtrs, following the /blahpack-validate process.
*
...
* already-validated dgbtrf), then we check `op(A0)*X = B0` against the ORIGINAL band
* matrix A0. The residual property is independent of dgbtrf's correctness — a wrong
...
* superdiagonals. Input to dgbtrf is realized with `schemes.banded {kl,ku,luFill}`,
...
import dgbtrs from './../lib/ndarray.js';
...
// Positive-first-dimension-stride banded layouts only: dgbtrf's inner idamax
// pivot search walks the band-array first dimension and is out of contract for a
// negative first-dim stride (see LEARNINGS.md getrf/getf2 family). dgbtrs itself
// carries no pivot search, but we keep AB on the same positive-sgn1 layouts so the
// frozen factor is realized exactly as dgbtrf would accept it.
...
// with dgbtrf, returning the physical realization R and its 0-based IPIV.
function factor( A0, N, kl, ku, layout ) {
	var R = schemes.banded.realize( sc, A0.copy(), { 'kl': kl, 'ku': ku, 'luFill': true }, layout );
	var ipiv = new Int32Array( N );
	dgbtrf( N, N, kl, ku, R.data, R.args[ 0 ], R.args[ 1 ], R.args[ 2 ], ipiv, 1, 0 );
...
// dgbtrs expects (called below with the ORIGINAL kl, ku).
...
// per (N,kl,ku) at a tight positive-sgn1 layout (dgbtrs reads AB read-only, so the
// same factor serves every trans/nrhs), solve with dgbtrs, then verify
// op(A0)*X = B0 per RHS column against the ORIGINAL band matrix.
test( 'dgbtrs: banded LU solve residual (trans x (kl,ku) x N x nrhs)', function t() {
...
function runResidual( N, kl, ku ) {
	var rng = new RNG( 0x2000 + ( N * 100 ) + ( kl * 10 ) + ku ); // reproducible
	var A0 = logical.banded( sc, rng, N, N, kl, ku );

	var aLayout = BANDED_POS[ 0 ]; // tight col-major
	var f = factor( A0, N, kl, ku, aLayout );
	...
	TRANS.forEach( function eachTrans( trans ) {
		...
		NRHS.forEach( function eachNrhs( nrhs ) {
			...
			dgbtrs( trans, N, kl, ku, nrhs, f.R.data, f.R.args[ 0 ], f.R.args[ 1 ], f.R.args[ 2 ], f.ipiv, 1, 0, Br.data, Br.args[ 0 ], Br.args[ 1 ], Br.args[ 2 ] );

			checked( 'dgbtrs', 'residual', function run() {
				var j;
				for ( j = 0; j < nrhs; j++ ) {
					check.assertResidual( sc, A0, readCol( Br, N, j ), logicalCol( B0, N, j ), {
						'trans': code,
						'factor': 100,
						'label': 'dgbtrs '+trans+' N='+N+' kl='+kl+' ku='+ku+' nrhs='+nrhs+' col='+j
					});
				}
			});
		});
...
// Step 4: layout-invariance fuzz. dgbtrs consumes an already-computed factor, so
...
// ONLY dgbtrs (isolating its addressing from the pivoting factor, which is out of
// contract for a negative band-array first-dim stride; see LEARNINGS.md). dgbtrs's
...
test( 'dgbtrs: bit-exact within a single storage-order family for AB and B (col / row)', function t() {
	TRANS.forEach( function eachTrans( trans ) {
...
function runInvariance( trans, abFamily, bFamily, fam ) {
	...
	var f = factor( A0, N, kl, ku, BANDED_POS[ 0 ] );
	var Fac = captureFactor( f.R, N, kl, ku );
	var ipiv = f.ipiv;
	...
	checked( 'dgbtrs', 'layout-invariance', function run() {
		layoutInvariant( variants, function build( v ) {
			var Ar = realizeFactor( Fac, kl, ku, v.a );
			var Br = schemes.dense.realize( sc, B0, { 'part': 'full' }, v.b );
			dgbtrs( trans, N, kl, ku, nrhs, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], ipiv, 1, 0, Br.data, Br.args[ 0 ], Br.args[ 1 ], Br.args[ 2 ] );
			return check.flattenLogical( sc, readB( Br, N, nrhs ) );
		}, { 'label': 'dgbtrs '+trans+' '+fam+'-major (AB) layout invariance' } );
	});
}
