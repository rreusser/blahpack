...
test( 'zgges: column-major basic 2x2 no-vectors', function t() {
	...
	const res = zgges( 'column-major', 'no-vectors', 'no-vectors', 'not-sorted', nosel, N, A, N, B, N, ALPHA, BETA, VSL, N, VSR, N );
...
test( 'zgges: row-major basic 2x2 with vectors', function t() {
	...
	const VSL = new Complex128Array( N * N );
	const VSR = new Complex128Array( N * N );
	const res = zgges( 'row-major', 'compute-vectors', 'compute-vectors', 'not-sorted', nosel, N, A, N, B, N, ALPHA, BETA, VSL, N, VSR, N );
	assert.equal( res.info, 0, 'info' );
});
