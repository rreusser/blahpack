...
import zhetrs2 from '../../zhetrs2/lib/base.js';
...
*   A = U _ D _ U^H  (if uplo = 'U'), or
*   A = L _ D _ L^H  (if uplo = 'L'),
*
...
*/
function zhesv( uplo, N, nrhs, A, strideA1, strideA2, offsetA, IPIV, strideIPIV, offsetIPIV, B, strideB1, strideB2, offsetB, WORK, strideWork, offsetWork ) {
	...
	// Factor A
	const info = zhetrf( uplo, N, A, strideA1, strideA2, offsetA, IPIV, strideIPIV, offsetIPIV );

	if ( info === 0 ) {
		// Solve using the factorization. The caller owns WORK; when it provides
		// fewer than N elements we fall back to zhetrs (which needs none),
		// otherwise we use the faster zhetrs2 (which uses the workspace).
		if ( ( WORK.length - offsetWork ) < N ) {
