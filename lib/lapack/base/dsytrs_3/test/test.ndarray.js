...
import dsytrs3 from './../lib/ndarray.js';


// FIXTURES //

const fixtureDir = path.join( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures' ); // eslint-disable-line max-len
const lines = readFileSync( path.join( fixtureDir, 'dsytrs_3.jsonl' ), 'utf8' ).trim().split( '\n' ); // eslint-disable-line node/no-sync, max-len
...
function runFixture( uplo, N, nrhs, bRhs, tc ) {
	...
	const b = new Float64Array( bRhs );

	const info = dsytrs3( uplo, N, nrhs, A, 1, N, 0, e, 1, 0, ipiv, 1, 0, b, 1, N, 0 ); // eslint-disable-line max-len
...
test( 'dsytrs3: N=0 quick return', function t() {
	...
	const info = dsytrs3( 'lower', 0, 1, A, 1, 1, 0, e, 1, 0, ipiv, 1, 0, b, 1, 1, 0 );
...
test( 'dsytrs3: NRHS=0 quick return', function t() {
	...
	const info = dsytrs3( 'lower', 3, 0, A, 1, 3, 0, e, 1, 0, ipiv, 1, 0, b, 1, 3, 0 );
	assert.equal( info, 0, 'info' );
});
