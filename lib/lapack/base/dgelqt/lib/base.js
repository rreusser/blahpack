...
import dgelqt3 from './../../dgelqt3/lib/base.js';
...
* -   `WORK` is treated as a logically 2D scratch buffer for the trailing-matrix update; matching the Fortran reference, the per-panel leading dimension equals the number of rows of the trailing block. Its size therefore scales with the trailing ROW count and peaks at the first block; allocate at least `max(mb*N, (M-min(mb,N))*min(mb,N))` entries. The reference figure `mb*N` is a lower bound valid only when `M <= N`; tall (`M > N`) inputs need the larger `(M-min(mb,N))*min(mb,N)`.
*
* @private
