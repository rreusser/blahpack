/**
* Property-based validation for dsprfs, following the /blahpack-validate process.
*
* Step 0 classification: `d` -> real scalar; `sp` -> symmetric INDEFINITE PACKED
* (schemes.packed, logical.symmetric); `rfs` (iterative refinement + error bounds)
* -> three independent properties. dsprfs refines an approximate solution X to
* A*X = B for a symmetric indefinite A held in PACKED storage, returning FERR
* (forward-error bound) and BERR (componentwise backward error). The Bunch-Kaufman
* packed factor AFP + IPIV are produced by the already-validated dsptrf; the
* un-refined initial X by dsptrs; the TRUE solution independently by the trusted
* dspsv on fresh copies. We assert, against the ORIGINAL full symmetric A0:
...
function trueSolution( A0, B0, N, nrhs, uplo ) {
	...
	if ( info !== 0 ) {
		throw new Error( 'oracle dspsv failed (info='+info+'); matrix singular?' );
	}
	return Br;
}

// Steps 2-3-5: three properties across uplo flags, a size sweep, and nrhs.
test( 'dsprfs: refinement residual + BERR + FERR bound (uplo x N x nrhs)', function t() {
...
function runProperty( uplo, N, nrhs ) {
	...
	const IPIV = new Int32Array( N );
	dsptrf( uplo, N, AFr.data, AFr.args[ 0 ], AFr.args[ 1 ], IPIV, 1, 0 );

	// B (RHS, unchanged) and X (initial un-refined solve, refined in place):
	const Br = schemes.dense.realize( sc, B0, { 'part': 'full' }, TIGHT );
	const Xr = schemes.dense.realize( sc, B0, { 'part': 'full' }, TIGHT );
	dsptrs( uplo, N, nrhs, AFr.data, AFr.args[ 0 ], AFr.args[ 1 ], IPIV, 1, 0, Xr.data, Xr.args[ 0 ], Xr.args[ 1 ], Xr.args[ 2 ] );
	...
	dsprfs( uplo, N, nrhs, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], AFr.data, AFr.args[ 0 ], AFr.args[ 1 ], IPIV, 1, 0, Br.data, Br.args[ 0 ], Br.args[ 1 ], Br.args[ 2 ], Xr.data, Xr.args[ 0 ], Xr.args[ 1 ], Xr.args[ 2 ], FERR, 1, 0, BERR, 1, 0, WORK, 1, 0, IWORK, 1, 0 );

	const Xtrue = trueSolution( A0, B0, N, nrhs, uplo );

	const tag = 'dsprfs '+uplo+' N='+N+' nrhs='+nrhs;

	// (a) residual: refined X remains a valid solution of A0*X = B0.
	checked( 'dsprfs', 'residual', function run() {
		let j;
		for ( j = 0; j < nrhs; j++ ) {
	...
	checked( 'dsprfs', 'structural', function run() {
...
// the fixed A0 and the fixed initial X (dense, tight); dsprfs's internal kernels
// (dspmv, dsptrs, daxpy, dlacn2) run over identical values, so ONLY packed
// ADDRESSING changes and the refined X + FERR + BERR must reproduce bit-for-bit.
// The fixed IPIV is consumed by dsptrs (no pivot search of its own), so all packed
...
test( 'dsprfs: bit-exact across the packed layout family', function t() {
	UPLOS.forEach( function eachUplo( uplo ) {
		runInvariance( uplo );
	});
});
...
function runInvariance( uplo ) {
	...
	dsptrf( uplo, N, AF0.data, AF0.args[ 0 ], AF0.args[ 1 ], IPIV0, 1, 0 );
	...
	dsptrs( uplo, N, nrhs, AF0.data, AF0.args[ 0 ], AF0.args[ 1 ], IPIV0, 1, 0, X0r.data, X0r.args[ 0 ], X0r.args[ 1 ], X0r.args[ 2 ] );
	const Xinit = readMat( X0r, N, nrhs );

	checked( 'dsprfs', 'layout-invariance', function run() {
		layoutInvariant( packedLayouts, function build( layout ) {
			...
			const WORK = new Float64Array( 3 * N );
			const IWORK = new Int32Array( N );
			dsprfs( uplo, N, nrhs, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], AFr.data, AFr.args[ 0 ], AFr.args[ 1 ], IPIV0, 1, 0, Br.data, Br.args[ 0 ], Br.args[ 1 ], Br.args[ 2 ], Xr.data, Xr.args[ 0 ], Xr.args[ 1 ], Xr.args[ 2 ], FERR, 1, 0, BERR, 1, 0, WORK, 1, 0, IWORK, 1, 0 );
			...
			return out;
		}, { 'label': 'dsprfs '+uplo+' packed layout invariance' } );
	});
}
