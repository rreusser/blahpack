...
function zstein( N, d, strideD, offsetD, e, strideE, offsetE, M, w, strideW, offsetW, IBLOCK, strideIBLOCK, offsetIBLOCK, ISPLIT, strideISPLIT, offsetISPLIT, Z, strideZ1, strideZ2, offsetZ, WORK, strideWork, offsetWork, IWORK, strideIWork, offsetIWork, IFAIL, strideIFAIL, offsetIFAIL ) { // eslint-disable-line max-len, max-params
	...
	// Caller owns the workspaces; assert they are sufficiently large arrays so an
	// under-sized (or non-array) buffer is a loud RangeError, not a silent NaN.
	// WORK (5*N) and IWORK (N) are only touched when N > 0 && M > 0 (otherwise a
	// quick return).
	if ( N > 0 && M > 0 ) {
		need = 5 * N;
		...
		if ( !IWORK || ( IWORK.length - offsetIWork ) < N ) {
			throw new RangeError( format( 'invalid argument. IWORK array must have at least %d elements from offset %d. Provided length: %d.', N, offsetIWork, ( IWORK ) ? IWORK.length : 0 ) ); // eslint-disable-line max-len
		}
	}
