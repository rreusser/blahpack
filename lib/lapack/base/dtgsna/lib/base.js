...
import dgemv from '../../../../blas/base/dgemv/lib/ndarray.js';
...
*/
function dtgsna( job, howmny, SELECT, strideSELECT, offsetSELECT, N, A, strideA1, strideA2, offsetA, B, strideB1, strideB2, offsetB, VL, strideVL1, strideVL2, offsetVL, VR, strideVR1, strideVR2, offsetVR, s, strideS, offsetS, DIF, strideDIF, offsetDIF, mm, M, WORK, strideWork, offsetWork, IWORK, strideIWork, offsetIWork ) {
	let alphai, alphar, alprqt, result, lwmin, tmprr, tmpri, tmpii, tmpir;
	let uhavi, uhbvi, root1, root2, pair, rnrm, lnrm, uhav, uhbv, cond, lag2;
	let beta, ierr, ifst, ilst, ks, c1, c2, n1, n2, iz, m, k, i;
	...
	// Caller owns the workspace; derive the available length from the provided array.
	...
	const scaleArr = new Float64Array( 1 );
	const difArr = new Float64Array( 1 );
	const dummyQ = new Float64Array( 1 );
	const dummyZ = new Float64Array( 1 );
	...
	for ( k = 0; k < N; k++ ) {
		...
		if ( wantdf ) {
			...
			result = dtgexc( false, false, N, WORK, 1, N, offsetWork, WORK, 1, N, offsetWork + ( N * N ), dummyQ, 1, 1, 0, dummyZ, 1, 1, 0, ifst, ilst, WORK, 1, offsetWork + ( 2 * N * N ) );
			...
			if ( ierr > 0 ) {
				...
				if ( n2 === 0 ) {
					...
					scaleArr[ 0 ] = ZERO;
					difArr[ 0 ] = ZERO;
					dtgsyl('no-transpose', DIFDRI, n2, n1, WORK, 1, N, offsetWork + ( N * n1 ) + n1, WORK, 1, N, offsetWork, WORK, 1, N, offsetWork + n1, WORK, 1, N, offsetWork + ( N * n1 ) + n1 + i, WORK, 1, N, offsetWork + i, WORK, 1, N, offsetWork + n1 + i, scaleArr, difArr, WORK, 1, offsetWork + iz, IWORK, strideIWork, offsetIWork);
