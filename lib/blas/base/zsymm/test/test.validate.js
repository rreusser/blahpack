/**
* Property-based validation for zsymm, following the /blahpack-validate process.
*
* Step 0 classification: `z` -> complex scalar; `sy` -> complex-symmetric dense operand A
* (schemes.dense, logical.symmetric); `mm` (matrix-matrix) -> residual property
...
import zsymm from './../lib/ndarray.js';
...
test( 'zsymm: C = alpha*A*B + beta*C residual (side x uplo x shape sweep)', function t() {
	[ 'left', 'right' ].forEach( function eachSide( side ) {
		[ 'upper', 'lower' ].forEach( function eachUplo( uplo ) {
			CASES.forEach( function eachCase( shape, idx ) {
				...
				const A = logical.symmetric( sc, rng, k );
				...
				checked( 'zsymm', 'residual', function run() {
					check.assertReconstruct( sc, gotC, expected, { 'label': 'zsymm '+side+' '+uplo+' '+M+'x'+N } );
				});
			});
		});
	});
...
test( 'zsymm: bit-exact across storage layouts', function t() {
	const M = 6;
	const N = 5;
	const SEED = 0xF00D;
	[ 'left', 'right' ].forEach( function eachSide( side ) {
		[ 'upper', 'lower' ].forEach( function eachUplo( uplo ) {
			...
			checked( 'zsymm', 'layout-invariance', function run() {
				...
				}, { 'label': 'zsymm '+side+' '+uplo+' layout invariance' } );
			});
		});
	});
});
