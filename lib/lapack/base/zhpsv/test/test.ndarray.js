...
import zhpsv from './../lib/ndarray.js';
...
import singularFixture from './fixtures/singular.json' with { type: 'json' };
...
function hermPackedMatMul( APdata, Xv, N, nrhs, uplo ) {
	let are, aim, xre, xim, kk, rh, i, j;

	// Unpack to full dense
	const Afull = new Float64Array( 2 * N * N );
	kk = 0;
	if ( uplo === 'upper' ) {
		for ( j = 0; j < N; j++ ) {
			for ( i = 0; i <= j; i++ ) {
...
function runFixtureTest( name, uplo ) {
	...
	const B = buildComplex( tc.b, N * nrhs );
	const bV = new Float64Array( tc.b );

	// Allocate IPIV
	const IPIV = new Int32Array( N );

	// Call zhpsv: column-major B with strideB1=1, strideB2=N
	const info = zhpsv( uplo, N, nrhs, AP, 1, 0, IPIV, 1, 0, B, 1, N, 0 );
	...
	const Ax = hermPackedMatMul( tc.AP, Bv, N, nrhs, uplo );
...
test( 'zhpsv: N=0 quick return', function t() {
	...
	const info = zhpsv( 'upper', 0, 1, AP, 1, 0, IPIV, 1, 0, B, 1, 1, 0 );
...
test( 'zhpsv: NRHS=0 quick return', function t() {
	...
	const B = new Complex128Array( 1 );
	const IPIV = new Int32Array( 1 );
	const info = zhpsv( 'upper', 1, 0, AP, 1, 0, IPIV, 1, 0, B, 1, 1, 0 );
	assert.strictEqual( info, 0 );
});

test( 'zhpsv: singular matrix returns info > 0', function t() {

	const tc = fixtures[ 'singular' ];
	const N = tc.n;
	const nn = ( N * ( N + 1 ) ) / 2;
	const AP = buildComplex( tc.AP, nn );
	const B = buildComplex( tc.b, N * tc.nrhs );
	const IPIV = new Int32Array( N );
	const info = zhpsv( 'upper', N, tc.nrhs, AP, 1, 0, IPIV, 1, 0, B, 1, N, 0 );
...
test( 'zhpsv: upper 4x4 with 2x2 pivots', function t() {
	...
	const tc = fixtures[ 'upper_4x4_pivot' ];
	const N = tc.n;
	const nn = ( N * ( N + 1 ) ) / 2;
	const AP = buildComplex( tc.AP, nn );
	const B = buildComplex( tc.b, N * tc.nrhs );
