/**
* Property-based validation for zung2r, following the /blahpack-validate process.
*
* Step 0 classification: `z` -> complex scalar; `or`/`g2r` -> UNBLOCKED formation of
* the orthonormal factor Q (M x N, N <= M) from the Householder reflectors that
* `zgeqrf`/`zgeqr2` leave in the lower trapezoid of A (with tau in TAU). zung2r
...
*
* zung2r is the UNBLOCKED kernel (dlarf -> dgemv/dger per reflector); the storage
...
import zung2r from './../lib/ndarray.js';
import zgeqrf from '../../zgeqrf/lib/ndarray.js';

const sc = S.complex; // z-routine
const BLOCKED = false; // zung2r is the unblocked kernel
...
test( 'zung2r: QᴴQ = I and A = Q*R ((M,N) sweep x all layouts)', function t() {
	PAIRS.forEach( function eachPair( pr ) {
		...
		ALL_LAYOUTS.forEach( function eachLayout( layout, li ) {
			...
			zung2r( M, N, K, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], Tr.data, Tr.args[ 0 ], Tr.args[ 1 ], Wo.data, Wo.args[ 0 ], Wo.args[ 1 ] );

			const Q = readFull( Ar, M, N );
			const label = 'zung2r M=' + M + ' N=' + N + ' layout=' + li;

			checked( 'zung2r', 'orthonormal', function run() {
				check.assertOrthonormal( sc, Q, { 'label': label + ' Q' } );
			} );

			const QR = ref.matmul( sc, Q, R );
			checked( 'zung2r', 'reconstruct', function run() {
...
test( 'zung2r: bit-exact within storage-order family (col / row)', function t() {
...
function runInvariance( variants, fam ) {
	const MI = 48;
	const NI = 40; // K = 40 (unblocked here regardless)
	const K = NI;
	const SEED = 0xF00D;
	...
	checked( 'zung2r', 'layout-invariance', function run() {
		layoutInvariant( variants, function build( layout, i ) {
			...
			zung2r( MI, NI, K, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], Tr.data, Tr.args[ 0 ], Tr.args[ 1 ], Wo.data, Wo.args[ 0 ], Wo.args[ 1 ] );
			return check.flattenLogical( sc, readFull( Ar, MI, NI ) );
		}, { 'label': 'zung2r layout invariance ' + fam + '-major (M=' + MI + ' N=' + NI + ')' } );
	} );
}
