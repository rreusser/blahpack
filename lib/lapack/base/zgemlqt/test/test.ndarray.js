...
import zgemlqt from './../lib/ndarray.js';


// FIXTURES //

const fixtureDir = path.join( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures' );
const rawLines = readFileSync( path.join( fixtureDir, 'zgemlqt.jsonl' ), 'utf8' ).trim().split( '\n' );
...
*/
function smallVT() {
	const tc = findCase( 'lq_factors_small' );
	return {
		'V': toComplex( tc.v ),
		'T': toComplex( tc.t )
	};
...
*/
function rsmallVT() {
	const tc = findCase( 'lq_factors_rsmall' );
	return {
		'V': toComplex( tc.v ),
		'T': toComplex( tc.t )
	};
...
*/
function blockVT() {
	const tc = findCase( 'lq_factors_block' );
	return {
		'V': toComplex( tc.v ),
		'T': toComplex( tc.t )
	};
...
*/
function singleVT() {
	const tc = findCase( 'lq_factors_single' );
	return {
		'V': toComplex( tc.v ),
		'T': toComplex( tc.t )
	};
...
test( 'zgemlqt: left, no-transpose -> Q*I (small, MB=2, K=3)', function t() {
	...
	const info = zgemlqt( 'left', 'no-transpose', 4, 4, 3, 2, vt.V, 1, 3, 0, vt.T, 1, 2, 0, C, 1, 4, 0, WORK, 1, 0 );
...
test( 'zgemlqt: left, conjugate-transpose -> Q^H*I (small)', function t() {
	...
	const C = eye( 4, 4, 4 );
	const WORK = new Complex128Array( 4 * 2 );
	const info = zgemlqt( 'left', 'conjugate-transpose', 4, 4, 3, 2, vt.V, 1, 3, 0, vt.T, 1, 2, 0, C, 1, 4, 0, WORK, 1, 0 );
	assert.equal( info, tc.info );
	assertComplexClose( C, tc.c, 1e-13, 'c' );
});
...
test( 'zgemlqt: right, no-transpose, rectangular C(4,3) (small)', function t() {
	...
	const info = zgemlqt( 'right', 'no-transpose', 4, 3, 3, 2, vt.V, 1, 3, 0, vt.T, 1, 2, 0, C, 1, 4, 0, WORK, 1, 0 );
	assert.equal( info, tc.info );
	assertComplexClose( C, tc.c, 1e-13, 'c' );
});
...
test( 'zgemlqt: right, conjugate-transpose, rectangular C(4,3) (small)', function t() {
	...
	const C = rect43();
	const WORK = new Complex128Array( 4 * 2 );
	const info = zgemlqt( 'right', 'conjugate-transpose', 4, 3, 3, 2, vt.V, 1, 3, 0, vt.T, 1, 2, 0, C, 1, 4, 0, WORK, 1, 0 );
	assert.equal( info, tc.info );
	assertComplexClose( C, tc.c, 1e-13, 'c' );
});
...
test( 'zgemlqt: M=0 quick return', function t() {
	...
	const info = zgemlqt( 'left', 'no-transpose', 0, 4, 0, 2, vt.V, 1, 3, 0, vt.T, 1, 2, 0, C, 1, 1, 0, WORK, 1, 0 );
	assert.equal( info, tc.info );
});
...
test( 'zgemlqt: N=0 quick return', function t() {
	...
	const C = new Complex128Array( 1 );
	const WORK = new Complex128Array( 1 );
	const info = zgemlqt( 'left', 'no-transpose', 4, 0, 0, 2, vt.V, 1, 3, 0, vt.T, 1, 2, 0, C, 1, 4, 0, WORK, 1, 0 );
	assert.equal( info, tc.info );
});
...
test( 'zgemlqt: K=0 quick return (C unchanged)', function t() {
	...
	const WORK = new Complex128Array( 4 * 2 );
	const info = zgemlqt( 'left', 'no-transpose', 4, 4, 0, 2, vt.V, 1, 3, 0, vt.T, 1, 2, 0, C, 1, 4, 0, WORK, 1, 0 );
	assert.equal( info, tc.info );
...
test( 'zgemlqt: left, no-transpose (multi-block, M=10, K=8, MB=3)', function t() {
	...
	const info = zgemlqt( 'left', 'no-transpose', 10, 10, 8, 3, vt.V, 1, 8, 0, vt.T, 1, 3, 0, C, 1, 10, 0, WORK, 1, 0 );
	assert.equal( info, tc.info );
...
test( 'zgemlqt: left, conjugate-transpose (multi-block)', function t() {
	...
	fillCLL( C, 10, 10, 10 );
	const WORK = new Complex128Array( 10 * 3 );
	const info = zgemlqt( 'left', 'conjugate-transpose', 10, 10, 8, 3, vt.V, 1, 8, 0, vt.T, 1, 3, 0, C, 1, 10, 0, WORK, 1, 0 );
	assert.equal( info, tc.info );
	assertComplexClose( C, tc.c, 1e-12, 'c' );
});
...
test( 'zgemlqt: right, no-transpose (multi-block)', function t() {
	...
	fillCLL( C, 10, 10, 10 );
	const WORK = new Complex128Array( 10 * 3 );
	const info = zgemlqt( 'right', 'no-transpose', 10, 10, 8, 3, vt.V, 1, 8, 0, vt.T, 1, 3, 0, C, 1, 10, 0, WORK, 1, 0 );
	assert.equal( info, tc.info );
	assertComplexClose( C, tc.c, 1e-12, 'c' );
});
...
test( 'zgemlqt: right, conjugate-transpose (multi-block)', function t() {
	...
	fillCLL( C, 10, 10, 10 );
	const WORK = new Complex128Array( 10 * 3 );
	const info = zgemlqt( 'right', 'conjugate-transpose', 10, 10, 8, 3, vt.V, 1, 8, 0, vt.T, 1, 3, 0, C, 1, 10, 0, WORK, 1, 0 );
	assert.equal( info, tc.info );
	assertComplexClose( C, tc.c, 1e-12, 'c' );
});
...
test( 'zgemlqt: left, no-transpose (single-block, MB == K)', function t() {
	...
	const info = zgemlqt( 'left', 'no-transpose', 5, 5, 4, 4, vt.V, 1, 4, 0, vt.T, 1, 4, 0, C, 1, 5, 0, WORK, 1, 0 );
	assert.equal( info, tc.info );
	assertComplexClose( C, tc.c, 1e-12, 'c' );
});
...
test( 'zgemlqt: left, conjugate-transpose (single-block)', function t() {
	...
	fillSingleC( C, 5, 5, 5 );
	const WORK = new Complex128Array( 5 * 4 );
	const info = zgemlqt( 'left', 'conjugate-transpose', 5, 5, 4, 4, vt.V, 1, 4, 0, vt.T, 1, 4, 0, C, 1, 5, 0, WORK, 1, 0 );
	assert.equal( info, tc.info );
	assertComplexClose( C, tc.c, 1e-12, 'c' );
});
