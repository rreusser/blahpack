...
import dtpqrt2 from './../lib/ndarray.js';


// FIXTURES //

const fixtureDir = path.join( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures' );
const lines = readFileSync( path.join( fixtureDir, 'dtpqrt2.jsonl' ), 'utf8' ).trim().split( '\n' ); // eslint-disable-line node/no-sync
...
function buildInputs( name ) {
	...
	const A = new Float64Array( def.N * def.N );
	const B = new Float64Array( def.M * def.N );
	const T = new Float64Array( def.N * def.N );
...
function colToRow( src, m, n ) {
	let i, j;
	const dst = new Float64Array( m * n );
	for ( j = 0; j < n; j++ ) {
		for ( i = 0; i < m; i++ ) {
			dst[ ( i * n ) + j ] = src[ i + ( j * m ) ];
		}
...
[ 'm4_n3_l0', 'm4_n3_l3', 'm3_n3_l2', 'm3_n3_l3', 'm3_n1_l1', 'm2_n4_l2' ].forEach( function each( name ) {
	test( 'column-major: ' + name, function t() {
		...
		const info = dtpqrt2( inp.M, inp.N, inp.L, inp.A, 1, inp.lda, 0, inp.B, 1, inp.ldb, 0, inp.T, 1, inp.ldt, 0 );
	...
	test( 'row-major: ' + name, function t() {
		const inp = buildInputs( name );
		const tc = findCase( name );
		const A = colToRow( inp.A, inp.N, inp.N );
		const B = colToRow( inp.B, inp.M, inp.N );
		const T = colToRow( inp.T, inp.N, inp.N );
		const info = dtpqrt2( inp.M, inp.N, inp.L, A, inp.N, 1, 0, B, inp.N, 1, 0, T, inp.N, 1, 0 );
		assert.strictEqual( info, tc.INFO );
...
test( 'column-major: M=0 quick return', function t() {
	const info = dtpqrt2( 0, 3, 0, new Float64Array( 9 ), 1, 3, 0, new Float64Array( 0 ), 1, 0, 0, new Float64Array( 9 ), 1, 3, 0 );
	assert.strictEqual( info, 0 );
});

test( 'column-major: N=0 quick return', function t() {
	const info = dtpqrt2( 3, 0, 0, new Float64Array( 0 ), 1, 0, 0, new Float64Array( 0 ), 1, 0, 0, new Float64Array( 0 ), 1, 0, 0 );
	assert.strictEqual( info, 0 );
});
