...
/**
* Property-based validation for zungql, following the /blahpack-validate process.
*
* Step 0 classification: `z` -> complex scalar; `un`/`gql` -> BLOCKED formation of
* the orthonormal factor Q (M x N economy, N <= M) from the Householder reflectors
* that `zgeqlf`/`dgeql2` leave in the LAST K columns of A (with tau in TAU). zungql
...
*       every (M,N).
*
* zungql is the UNBLOCKED kernel (dlarf -> dgemv/dger); the storage col<->row flip
...
const BLOCKED = true; // zungql is the BLOCKED kernel (zlarft + zlarfb), falling back to zung2l
const LogicalMatrix = logical.LogicalMatrix;
const NB = 32; // geqlf hardcoded block size (governs the geqlf WORK length only)
const ROUTINE = 'zungql';
...
// WORK length zgeqlf needs (blocked: N*NB + NB*NB in a SEPARATE trailing segment;
// else max(1,N)).
...
test( ROUTINE+': QᴴQ = I and A = Q*L ((M,N) sweep x all layouts)', function t() {
	PAIRS.forEach( function eachPair( pr ) {
		...
		ALL_LAYOUTS.forEach( function eachLayout( layout, li ) {
			...
			const Tr = schemes.realizeVector( sc, poison( Math.max( N, 1 ) ), TIGHT_VEC );
			const Wf = schemes.realizeVector( sc, poison( workLenFactor( M, N ) ), TIGHT_VEC );

			// Factor: A <- reflectors + L.
			zgeqlf( M, N, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], Tr.data, Tr.args[ 0 ], Tr.args[ 1 ], Wf.data, Wf.args[ 0 ], Wf.args[ 1 ] );

			// Save the economy L (bottom N x N) BEFORE org overwrites A with Q.
			const L = readL( Ar, M, N );

			// Form Q in place: A <- Q (M x N).
			const Wo = schemes.realizeVector( sc, poison( workLenOrg( M, N ) ), TIGHT_VEC );
			zungql( M, N, K, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], Tr.data, Tr.args[ 0 ], Tr.args[ 1 ], Wo.data, Wo.args[ 0 ], Wo.args[ 1 ] );

			const Q = readFull( Ar, M, N );
			const label = ROUTINE+' M=' + M + ' N=' + N + ' layout=' + li;

			checked( ROUTINE, 'orthonormal', function run() {
				check.assertOrthonormal( sc, Q, { 'label': label + ' Q' } );
			} );

			const QL = ref.matmul( sc, Q, L );
...
function runInvariance( variants, fam ) {
	const MI = 48;
	const NI = 40; // K = 40 > NB=32 -> BLOCKED path (dlarft/dlarfb reached)
	const K = NI;
	const SEED = 0xF00D;

	// Freeze the factorization once (tight col-major).
	const rng = new RNG( SEED );
	const A0 = logical.general( sc, rng, MI, NI );
	const Af = schemes.dense.realize( sc, A0, { 'part': 'full' }, null );
	const Tf = schemes.realizeVector( sc, poison( NI ), TIGHT_VEC );
	const Wf = schemes.realizeVector( sc, poison( workLenFactor( MI, NI ) ), TIGHT_VEC );
	zgeqlf( MI, NI, Af.data, Af.args[ 0 ], Af.args[ 1 ], Af.args[ 2 ], Tf.data, Tf.args[ 0 ], Tf.args[ 1 ], Wf.data, Wf.args[ 0 ], Wf.args[ 1 ] );
	const Frozen = readFull( Af, MI, NI );
	const taus = [];
	let ti;
	for ( ti = 0; ti < NI; ti++ ) {
	...
	checked( ROUTINE, 'layout-invariance', function run() {
		layoutInvariant( variants, function build( layout, i ) {
			...
			const Wo = schemes.realizeVector( sc, poison( workLenOrg( MI, NI ) ), WORK_LAYOUTS[ i % WORK_LAYOUTS.length ] );
			zungql( MI, NI, K, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], Tr.data, Tr.args[ 0 ], Tr.args[ 1 ], Wo.data, Wo.args[ 0 ], Wo.args[ 1 ] );
...
// Step 4c: WORKSPACE conformance (plain assertion, NOT `checked`). zungql hardcodes
// NB and stores the block-reflector T factor (leading dim N) plus dlarfb scratch in
// the caller-provided WORK, so a copied reference LWORK could UNDER-count (see the
// dgerqf/zungqr/dorglq LEARNINGS on the blocked-former WORK-under-count class). Derive
// the advertised minimum from the wrapper's own throw boundary, then run at exactly
// that length with a POISONED WORK on the BLOCKED path (M=N=80, K=80 > NB) and require
// finite Q (no NaN leak past WORK) AND orthonormality.
test( ROUTINE+': advertised WORK minimum suffices on the blocked path (Step 4c)', function t() {
	const M = 80;
	const N = 80;
	const K = N; // K = 80 > NB=32 -> blocked
	const SEED = 0xB10C;
	...
	const A0 = logical.general( sc, rng, M, N );
	const Af = schemes.dense.realize( sc, A0, { 'part': 'full' }, null );
	const Tf = schemes.realizeVector( sc, poison( N ), TIGHT_VEC );
	const Wf = schemes.realizeVector( sc, poison( workLenFactor( M, N ) ), TIGHT_VEC );
	zgeqlf( M, N, Af.data, Af.args[ 0 ], Af.args[ 1 ], Af.args[ 2 ], Tf.data, Tf.args[ 0 ], Tf.args[ 1 ], Wf.data, Wf.args[ 0 ], Wf.args[ 1 ] );
	const Frozen = readFull( Af, M, N );
	const taus = [];
	let ti;
	for ( ti = 0; ti < N; ti++ ) {
		taus.push( Tf.read( ti ) );
	...
	function run( len ) {
		...
		zungql( M, N, K, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], Tr.data, Tr.args[ 0 ], Tr.args[ 1 ], Wo, 1, 0 );
