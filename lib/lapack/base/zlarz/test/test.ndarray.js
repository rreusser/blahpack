...
const FIXTURE_PATH = resolve( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures', 'zlarz.jsonl' );
const RAW = readFileSync( FIXTURE_PATH, 'utf8' ); // eslint-disable-line node/no-sync
...
*/
function buildC4x3() {
	const out = new Complex128Array( 12 );
...
*/
function buildC3x2() {
	const out = new Complex128Array( 6 );
	const v = reinterpret( out, 0 );
	v[ 0 ] = 1.0;
	v[ 1 ] = 0.0;
	v[ 2 ] = 2.0;
...
*/
function buildV2() {
	const out = new Complex128Array( 2 );
	const v = reinterpret( out, 0 );
	v[ 0 ] = 0.5;
	v[ 1 ] = 0.2;
	v[ 2 ] = -0.3;
...
function buildTau( re, im ) {
	const out = new Complex128Array( 1 );
	const v = reinterpret( out, 0 );
	v[ 0 ] = re;
	v[ 1 ] = im;
	return out;
...
test( 'ndarray: returns C unchanged when M=0', function t() {
	...
	const out = ndarray( 'left', 0, 1, 0, v, 1, 0, tau, 0, C, 1, 1, 0, WORK, 1, 0 );
...
test( 'ndarray: returns C unchanged when N=0', function t() {
	...
	const C = new Complex128Array( 1 );
	const v = new Complex128Array( 1 );
	const out = ndarray( 'left', 1, 0, 0, v, 1, 0, tau, 0, C, 1, 1, 0, WORK, 1, 0 );
	assert.strictEqual( out, C, 'returns C' );
});

test( 'ndarray: left 4x3 L=2 matches fixture (column-major)', function t() {
	const WORK = new Complex128Array( 3 );
	const tau = buildTau( 1.2, -0.4 );
	const tc = findCase( 'zlarz_left_4x3_l2' );
	const C = buildC4x3();
	const v = buildV2();
...
test( 'ndarray: left 4x3 L=2 row-major strides matches fixture', function t() {
	...
	const tau = buildTau( 1.2, -0.4 );
	const tc = findCase( 'zlarz_left_4x3_l2' );
	const rowIn = permuteColToRow( toArray( reinterpret( buildC4x3(), 0 ) ), 4, 3 );
	const expected = permuteColToRow( tc.C, 4, 3 );
	const C = new Complex128Array( rowIn );
	const v = buildV2();
...
test( 'ndarray: left with padded buffers, sentinels untouched', function t() {
	...
	Cv[ 0 ] = 13.0;
	Cv[ 1 ] = -13.0;
	const base = toArray( reinterpret( buildC4x3(), 0 ) );
	for ( i = 0; i < 24; i++ ) {
		Cv[ 2 + i ] = base[ i ];
	}
	Cv[ 26 ] = 17.0;
	...
	const full = toArray( reinterpret( C, 0 ) );
...
test( 'ndarray: right with non-unit strideV and offsetV', function t() {
	const WORK = new Complex128Array( 3 );
	const tau = buildTau( 0.8, 0.5 );
	const tc = findCase( 'zlarz_right_3x4_l2' );
	const C = buildC4x3();
	...
	const vfull = toArray( reinterpret( v, 0 ) );
...
test( 'ndarray: tau = 0 is a no-op (left)', function t() {
	const WORK = new Complex128Array( 2 );
	const tau = buildTau( 0.0, 0.0 );
	const tc = findCase( 'zlarz_tau_zero' );
	const C = buildC3x2();
	const v = buildV2();
...
test( 'ndarray: L=0 on left only updates first row', function t() {
	const WORK = new Complex128Array( 2 );
	const tau = buildTau( 1.0, 0.3 );
	const tc = findCase( 'zlarz_l_zero_left' );
	const C = buildC3x2();
	const v = buildV2();
...
test( 'ndarray: left 3x2 L=3 (L == M)', function t() {
	const WORK = new Complex128Array( 2 );
	const tau = buildTau( 0.9, 0.2 );
	const tc = findCase( 'zlarz_left_3x2_l3' );
	const C = buildC3x2();
	const v = buildV3();
...
test( 'ndarray: right 2x3 L=3 (L == N)', function t() {
	const WORK = new Complex128Array( 2 );
	const tau = buildTau( 1.1, -0.3 );
	const tc = findCase( 'zlarz_right_2x3_l3' );
	const C = buildC3x2();
	const v = buildV3();
