...
import zlaein from './../lib/ndarray.js';


// FIXTURES //

const fixtureDir = path.join( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures' );
const rawLines = readFileSync( path.join( fixtureDir, 'zlaein.jsonl' ), 'utf8' ).trim().split( '\n' ); // eslint-disable-line node/no-sync, max-len
...
function buildV( N, entries ) {
	const arr = new Complex128Array( N );
	const buf = viewBuf( arr );
	let i, t;
	for ( i = 0; i < entries.length; i++ ) {
		t = entries[ i ];
...
const EPS3 = 1.0e-4;
const SMLNUM = 2.2250738585072014e-308 / 2.220446049250313e-16;

function runCase( name, rightv, noinit, N, H, w, vInit ) {
	const v = vInit || new Complex128Array( N );
	const B = new Complex128Array( N * N );
	const RWORK = new Float64Array( N );
	let buf, i;

	const info = zlaein( rightv, noinit, N, H, 1, N, 0, w, v, 1, 0, B, 1, N, 0, RWORK, 1, 0, EPS3, SMLNUM );
...
test( 'zlaein: right_init_3x3 (with initial vector)', function t() {
	const v = buildV( 3, [ [ 1, 0.1, 0.0 ], [ 2, 0.2, 0.1 ], [ 3, 0.9, -0.2 ] ] );
