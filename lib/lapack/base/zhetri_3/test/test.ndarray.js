...
import zhetri3 from './../lib/ndarray.js';


// FIXTURES //

const fixtureDir = path.join( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures' ); // eslint-disable-line max-len
const lines = readFileSync( path.join( fixtureDir, 'zhetri_3.jsonl' ), 'utf8' ).trim().split( '\n' ); // eslint-disable-line node/no-sync, max-len
...
function runFixture( uplo, N, tc ) {
	...
	const info = zhetri3( uplo, N, A, 1, LDA, 0, e, 1, 0, ipiv, 1, 0, work, 1, 0 ); // eslint-disable-line max-len
...
test( 'zhetri_3: N=1 lower trivial inverse', function t() {
	...
	const A = makeComplex( [ 5.0, 0.0 ] );
	const e = makeComplex( [ 0.0, 0.0 ] );
	const ipiv = new Int32Array( [ 0 ] );
	const lwork = ( 1 + NB + 1 ) * ( NB + 3 );
	const work = new Complex128Array( lwork );
	const info = zhetri3( 'lower', 1, A, 1, 1, 0, e, 1, 0, ipiv, 1, 0, work, 1, 0 ); // eslint-disable-line max-len
	assert.equal( info, tc.info, 'info' );
...
test( 'zhetri_3: N=0 quick return', function t() {
	...
	const info = zhetri3( 'lower', 0, A, 1, 1, 0, e, 1, 0, ipiv, 1, 0, work, 1, 0 ); // eslint-disable-line max-len
...
test( 'zhetri_3: validation throws on invalid uplo', function t() {
	...
	assert.throws( function fn() {
		zhetri3( 'invalid', 2, A, 1, 2, 0, e, 1, 0, ipiv, 1, 0, work, 1, 0 );
	}, TypeError );
});
