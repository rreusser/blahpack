...
import dsyconvf from './../lib/ndarray.js';


// FIXTURES //

const fixtureDir = path.join( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures' );
const lines = readFileSync( path.join( fixtureDir, 'dsyconvf.jsonl' ), 'utf8' ).trim().split( '\n' ); // eslint-disable-line node/no-sync,max-len
...
function assertIpivEqual( actual, expectedFortran, msg ) {
	let i;
	const expected = toIpiv( expectedFortran );
	assert.equal( actual.length, expected.length, msg + ': length mismatch' );
	for ( i = 0; i < expected.length; i++ ) {
...
test( 'dsyconvf: upper_convert (1x1 pivots)', function upperConvert() {
	...
	const IPIV = toIpiv( tc.ipiv_trf );
	const info = dsyconvf( 'upper', 'convert', N, A, 1, N, 0, E, 1, 0, IPIV, 1, 0 );
	assert.equal( info, 0 );
...
test( 'dsyconvf: upper_revert (1x1 pivots)', function upperRevert() {
	const tcC = findCase( 'upper_convert' );
	const tc = findCase( 'upper_revert' );
	const N = 5;
	const A = new Float64Array( tcC.a_converted );
	const E = new Float64Array( tcC.e );
	const IPIV = toIpiv( tcC.ipiv_converted );
	const info = dsyconvf( 'upper', 'revert', N, A, 1, N, 0, E, 1, 0, IPIV, 1, 0 );
	assert.equal( info, 0 );
...
test( 'dsyconvf: lower_convert (1x1 pivots)', function lowerConvert() {
	...
	const E = new Float64Array( N );
	const IPIV = toIpiv( tc.ipiv_trf );
	const info = dsyconvf( 'lower', 'convert', N, A, 1, N, 0, E, 1, 0, IPIV, 1, 0 );
	assert.equal( info, 0 );
	assertArrayClose( A, tc.a_converted, 1e-14, 'a_converted' );
	assertArrayClose( E, tc.e, 1e-14, 'e' );
	assertIpivEqual( IPIV, tc.ipiv_converted, 'ipiv_converted' );
...
test( 'dsyconvf: lower_revert (1x1 pivots)', function lowerRevert() {
	const tcC = findCase( 'lower_convert' );
	const tc = findCase( 'lower_revert' );
	const N = 5;
	const A = new Float64Array( tcC.a_converted );
	const E = new Float64Array( tcC.e );
	const IPIV = toIpiv( tcC.ipiv_converted );
	const info = dsyconvf( 'lower', 'revert', N, A, 1, N, 0, E, 1, 0, IPIV, 1, 0 );
	assert.equal( info, 0 );
	assertArrayClose( A, tc.a_reverted, 1e-14, 'a_reverted' );
	assertIpivEqual( IPIV, tc.ipiv_reverted, 'ipiv_reverted' );
});
...
test( 'dsyconvf: n0_upper_convert quick return', function n0UpperConvert() {
	...
	const E = new Float64Array( [ 42.0 ] );
	const IPIV = new Int32Array( [ 7 ] );
	const info = dsyconvf( 'upper', 'convert', 0, A, 1, 1, 0, E, 1, 0, IPIV, 1, 0 );
	assert.equal( info, 0 );
...
test( 'dsyconvf: n0_lower_revert quick return', function n0LowerRevert() {
	...
	const E = new Float64Array( [ 42.0 ] );
	const IPIV = new Int32Array( [ 7 ] );
	const info = dsyconvf( 'lower', 'revert', 0, A, 1, 1, 0, E, 1, 0, IPIV, 1, 0 );
	assert.equal( info, 0 );
	assert.equal( A[ 0 ], 1.0, 'A untouched' );
	assert.equal( E[ 0 ], 42.0, 'E untouched' );
	assert.equal( IPIV[ 0 ], 7, 'IPIV untouched' );
...
test( 'dsyconvf: upper_2x2_revert (2x2 pivot blocks)', function upper2x2Revert() {
	const tcC = findCase( 'upper_2x2_convert' );
	const tc = findCase( 'upper_2x2_revert' );
	const N = 5;
	const A = new Float64Array( tcC.a_converted );
	const E = new Float64Array( tcC.e );
	const IPIV = toIpiv( tcC.ipiv_converted );
...
test( 'dsyconvf: lower_2x2_revert (2x2 pivot block at rows 0-1)', function lower2x2Revert() {
	const tcC = findCase( 'lower_2x2_convert' );
	const tc = findCase( 'lower_2x2_revert' );
	const N = 3;
	const A = new Float64Array( tcC.a_converted );
	const E = new Float64Array( tcC.e );
	const IPIV = toIpiv( tcC.ipiv_converted );
...
test( 'dsyconvf: upper 1x1 convert with explicit swap (ip != i)', function upperConvert1x1Swap() {
	...
	const E = new Float64Array( N );
	const IPIV = new Int32Array( [ 0, 3, 2, 3 ] );
...
test( 'dsyconvf: lower 1x1 swap (convert then revert, 4x4)', function lowerSwap1x1() {
	let info;
	const N = 4;
	const A = new Float64Array( [ 1, 10, 20, 30, 0, 2, 21, 31, 0, 0, 3, 32, 0, 0, 0, 4 ] );
	const orig = new Float64Array( A );
	const E = new Float64Array( N );
	const IPIV = new Int32Array( [ 0, 3, 2, 3 ] );
...
test( 'dsyconvf: lower 2x2 interchange at position (1,2) with non-trivial pivot', function lowerSwap2x2() {
	...
	const N = 4;
	const A = new Float64Array( [ 1, 10, 20, 30, 0, 2, 21, 31, 0, 0, 3, 32, 0, 0, 0, 4 ] );
	const origCol0 = [ A[ 0 ], A[ 1 ], A[ 2 ], A[ 3 ] ];
	const E = new Float64Array( N );
	const IPIV = new Int32Array( [ 0, ~3, ~3, 3 ] );
	const info = dsyconvf( 'lower', 'convert', N, A, 1, N, 0, E, 1, 0, IPIV, 1, 0 );
	assert.equal( info, 0 );
...
test( 'dsyconvf: stride and offset support', function stridesOffsets() {
	...
	A[ 4 ] = 30;
	A[ 5 ] = 40;
	const E = new Float64Array( [ 0, 0, 0 ] );
	const IPIV = new Int32Array( [ -1, -1 ] );
	const info = dsyconvf( 'upper', 'convert', N, A, 1, LDA, offsetA, E, 1, 0, IPIV, 1, 0 );
	assert.equal( info, 0 );
...
test( 'dsyconvf: upper 2x2 convert with explicit swap (ip != i-1)', function upperConvertSwap() {
	...
	const E = new Float64Array( N );
	const IPIV = new Int32Array( [ 0, ~0, ~0, 3, 4 ] );
...
test( 'dsyconvf: lower revert 2x2 at rows (1,2) triggers swap branch', function lowerRevert2x2Swap() {
	...
	A[ offsetA - 3 ] = 777;
	const E = new Float64Array( N );
	const IPIV = new Int32Array( [ 0, 1, -1, 3 ] );
	const before = new Float64Array( A );
	const info = dsyconvf( 'lower', 'revert', N, A, 1, LDA, offsetA, E, 1, 0, IPIV, 1, 0 );
	assert.equal( info, 0 );
