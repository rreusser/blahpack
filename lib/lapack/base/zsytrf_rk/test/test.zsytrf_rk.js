...
import zsytrfrk from './../lib/zsytrf_rk.js';
...
test( 'zsytrf_rk has expected arity', function t() {
	assert.strictEqual( zsytrfrk.length, 11, 'has expected arity' );
});
...
test( 'zsytrf_rk: row-major / column-major produce equivalent factorizations', function t() {
	...
	const Acm = new Complex128Array( [ 4, 0.2, 1, -0.3, -2, 0.1, 0.5, 0, 0, 0, -3, 0.5, 1, 0.4, 2, -0.1, 0, 0, 0, 0, 5, -0.2, -1, 0.3, 0, 0, 0, 0, 0, 0, 2, 0.5 ] ); // eslint-disable-line max-len
	const Arm = new Complex128Array( [ 4, 0.2, 0, 0, 0, 0, 0, 0, 1, -0.3, -3, 0.5, 0, 0, 0, 0, -2, 0.1, 1, 0.4, 5, -0.2, 0, 0, 0.5, 0, 2, -0.1, -1, 0.3, 2, 0.5 ] ); // eslint-disable-line max-len
	const ecm = new Complex128Array( 4 );
	const erm = new Complex128Array( 4 );
	const ipcm = new Int32Array( 4 );
	const iprm = new Int32Array( 4 );
	const info1 = zsytrfrk( 'column-major', 'lower', 4, Acm, 4, ecm, 1, ipcm, 1, null, 1 );
	const info2 = zsytrfrk( 'row-major', 'lower', 4, Arm, 4, erm, 1, iprm, 1, null, 1 );
	assert.equal( info1, info2, 'info matches' );
	assert.equal( info1, 0, 'info=0' );
	const av = reinterpret( ecm, 0 );
	const rv = reinterpret( erm, 0 );
	for ( i = 0; i < 4; i++ ) {
