...
test( 'dlarrf returns integer info for N=0 quick return', function t() {
	...
	const d = new Float64Array( 0 );
	const s = new Float64Array( 1 );
	const info = dlarrf( 0, d, 1, d, 1, d, 1, 1, 1, d, 1, d, 1, d, 1, 1.0, 1.0, 1.0, PIVMIN, s, d, 1, d, 1, d, 1 );
...
test( 'dlarrf wrapper: tridiag_4x4_full_cluster matches fixture', function t() {
	...
	const N = 4;
	const info = dlarrf( N, d, 1, l, 1, ld, 1, 1, 4, w, 1, wgap, 1, werr, 1, 4.0, 1.0, 1.0, PIVMIN, sigma, dplus, 1, lplus, 1, work, 1 );
...
test( 'dlarrf wrapper: tridiag_2x2 matches fixture', function t() {
	...
	const N = 2;
	const info = dlarrf( N, d, 1, l, 1, ld, 1, 1, 2, w, 1, wgap, 1, werr, 1, 2.0, 1.0, 1.0, PIVMIN, sigma, dplus, 1, lplus, 1, work, 1 );
	assert.equal( info, tc.info, 'info' );
	assertClose( sigma[ 0 ], tc.sigma, TOL, 'sigma' );
	assertArrayClose( copyToArray( dplus, N ), tc.dplus, TOL, 'dplus' );
	assertArrayClose( copyToArray( lplus, N - 1 ), tc.lplus, TOL, 'lplus' );
...
test( 'dlarrf wrapper: tight_cluster_6x6 matches fixture (exercises RRR1 path)', function t() {
	...
	const N = 6;
	const info = dlarrf( N, d, 1, l, 1, ld, 1, 3, 5, w, 1, wgap, 1, werr, 1, 6.0, 1.0, 4.0, PIVMIN, sigma, dplus, 1, lplus, 1, work, 1 );
	assert.equal( info, tc.info, 'info' );
	assertClose( sigma[ 0 ], tc.sigma, TOL, 'sigma' );
	assertArrayClose( copyToArray( dplus, N ), tc.dplus, TOL, 'dplus' );
	assertArrayClose( copyToArray( lplus, N - 1 ), tc.lplus, TOL, 'lplus' );
...
test( 'dlarrf: tight cluster with large d and tiny spdiam exercises RRR1 branch', function t() {
	...
	const N = 3;
	const info = dlarrf( N, d, 1, l, 1, ld, 1, 1, 3, w, 1, wgap, 1, werr, 1, 0.5, 1.0, 1.0, PIVMIN, sigma, dplus, 1, lplus, 1, work, 1 );
...
test( 'dlarrf: pivmin clamping triggered when dplus collapses toward zero', function t() {
	...
	const info = dlarrf( N, d, 1, l, 1, ld, 1, 1, 2, w, 1, wgap, 1, werr, 1, 2.0, 1.0, 1.0, BIGPIV, sigma, dplus, 1, lplus, 1, work, 1 );
	assert.ok( info === 0 || info === 1, 'info is 0 or 1' );
	assert.equal( typeof sigma[ 0 ], 'number', 'sigma is a number' );
});
...
test( 'dlarrf: tiny spdiam forces growth retry and exercises fallback paths', function t() {
	...
	const N = 3;
	const info = dlarrf( N, d, 1, l, 1, ld, 1, 1, 3, w, 1, wgap, 1, werr, 1, 0.001, 1.0, 1.0, PIVMIN, sigma, dplus, 1, lplus, 1, work, 1 );
	assert.ok( info === 0 || info === 1, 'info is 0 or 1' );
	assert.equal( typeof sigma[ 0 ], 'number', 'sigma is a number' );
});
...
test( 'dlarrf: case where right shift is preferred over left', function t() {
	...
	const N = 4;
	const info = dlarrf( N, d, 1, l, 1, ld, 1, 1, 3, w, 1, wgap, 1, werr, 1, 4.0, 0.0001, 1.0, PIVMIN, sigma, dplus, 1, lplus, 1, work, 1 );
	assert.ok( info === 0 || info === 1, 'info is 0 or 1' );
	assert.equal( typeof sigma[ 0 ], 'number', 'sigma is a number' );
});
...
test( 'dlarrf wrapper: tridiag_5x5_subset_cluster matches fixture', function t() {
	...
	const N = 5;
	const info = dlarrf( N, d, 1, l, 1, ld, 1, 2, 4, w, 1, wgap, 1, werr, 1, 5.0, 0.95, 0.95, PIVMIN, sigma, dplus, 1, lplus, 1, work, 1 );
	assert.equal( info, tc.info, 'info' );
	assertClose( sigma[ 0 ], tc.sigma, TOL, 'sigma' );
	assertArrayClose( copyToArray( dplus, N ), tc.dplus, TOL, 'dplus' );
	assertArrayClose( copyToArray( lplus, N - 1 ), tc.lplus, TOL, 'lplus' );
