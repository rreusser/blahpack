...
import dsytf2rk from '../../dsytf2_rk/lib/base.js';
import dlasyfrk from '../../dlasyf_rk/lib/base.js';
...
* @param {Float64Array} WORK - caller-owned workspace; base.js never allocates. Minimum length from `offsetWork` is `N*NB` (with `NB = 32`) when `N > NB` (blocked path); unused otherwise.
...
*/
function dsytrfrk( uplo, N, A, strideA1, strideA2, offsetA, e, strideE, offsetE, IPIV, strideIPIV, offsetIPIV, WORK, strideWork, offsetWork ) {
	...
	if ( uplo === 'upper' ) {
		...
		while ( k >= 1 ) {
			if ( k > nb ) {
				...
				result = dlasyfrk( 'upper', k, nb, A, strideA1, strideA2, offsetA, e, strideE, offsetE, IPIV, strideIPIV, offsetIPIV, WORK, strideWork, ldwork * strideWork, offsetWork );
				kb = result.kb;
				iinfo = result.info;
			} else {
		...
		while ( k <= N ) {
			if ( k <= N - nb ) {
				...
				result = dlasyfrk( 'lower', N - k + 1, nb, A, strideA1, strideA2, offsetA + ( ( k - 1 ) * strideA1 ) + ( ( k - 1 ) * strideA2 ), e, strideE, offsetE + ( ( k - 1 ) * strideE ), IPIV, strideIPIV, offsetIPIV + ( ( k - 1 ) * strideIPIV ), WORK, strideWork, ldwork * strideWork, offsetWork );
				kb = result.kb;
				iinfo = result.info;
			} else {
