...
import dlansy from './../lib/ndarray.js';


// FIXTURES //

const fixtureFile = path.join( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures', 'dlansy.jsonl' );
...
function buildA( lda, ncols, cells ) {
	const out = new Float64Array( lda * ncols );
	let i;
...
// 1x1 with -5.5
const A_1X1 = buildA( 5, 5, [ 1, 1, -5.5 ] );

// 5x5 upper
const A5_UPPER = buildA( 5, 5, [
	1, 1, 1.0,
	1, 2, 2.0, 2, 2, 3.0,
...
function runCase( name, norm, uplo, N, A ) {
	...
	const got = dlansy( norm, uplo, N, A, 1, LDA, 0, WORK, 1, 0 );
...
test( 'dlansy: N=0 quick return via wrapper', function t() {
	...
	const got = dlansy( 'max', 'upper', 0, A4_UPPER, 1, LDA, 0, WORK, 1, 0 );
