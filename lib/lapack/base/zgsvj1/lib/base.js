...
*/
function zgsvj1( jobv, M, N, n1, A, strideA1, strideA2, offsetA, d, strideD, offsetD, sva, strideSVA, offsetSVA, mv, V, strideV1, strideV2, offsetV, eps, sfmin, tol, nsweep, work, strideWork, offsetWork ) { // eslint-disable-line max-len
	let convergedSweep, pskipped, negAapq, bailjbc, notrot, mxaapq, mxsinj;
	let iswrot, thsign, ijblsk, swband, apoaq, aqoap, aapp0, aapqR, aapqI;
	let ompqR, ompqI, rotok, rsvec, theta, temp1, aapq1, aapp, aaqq, aapq, info;
	let nblc, nblr, mvl, ibr, jbc, igl, jgl, ap0, aq0, vp0, vq0, dp0, dq0, dpR;
	let dpI, dqR, dqI, res, cs, sn, p, q, t, i;
	...
	if ( !( rsvec || applv || jobv === 'no-v' ) ) {
		...
		info = -24;
	} else if ( ( work.length - offsetWork ) < M ) {
		info = -28;
	}
	if ( info !== 0 ) {
		return info;
