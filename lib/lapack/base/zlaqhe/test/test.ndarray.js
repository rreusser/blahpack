...
test( 'zlaqhe: no_equil', function t() {
	const tc = no_equil;
	const A = makeA3();
	const equed = zlaqhe( 'upper', 3, A, 1, 3, 0, new Float64Array( [ 1, 1, 1 ] ), 1, 0, 1.0, 16.0 );
...
test( 'zlaqhe: n_zero', function t() {
	...
	const equed = zlaqhe( 'upper', 0, A, 1, 1, 0, new Float64Array( 1 ), 1, 0, 1.0, 1.0 );
	assert.equal( equed, 'none', 'equed' );
});
...
test( 'zlaqhe: n_one', function t() {
	...
	const equed = zlaqhe( 'upper', 1, A, 1, 1, 0, new Float64Array( [ 0.5 ] ), 1, 0, 0.01, 4.0 );
	assert.equal( equed, 'yes', 'equed' );
	assertArrayClose( Array.from( reinterpret( A, 0 ) ), tc.a, 1e-14, 'a' );
});
