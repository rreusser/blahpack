...
import dsyconv from './../lib/ndarray.js';
...
test( 'dsyconv: upper_convert (all 1x1 pivots)', function t() {
	...
	const info = dsyconv( 'upper', 'convert', N, A, 1, N, 0, IPIV, 1, 0, E, 1, 0 );
...
test( 'dsyconv: upper_revert (all 1x1 pivots)', function t() {
	...
	const N = 4;
	const A = new Float64Array( tcConv.a_converted );
	const IPIV = convertIPIV( tcConv.ipiv_trf );
	const E = new Float64Array( tcConv.e );
	const info = dsyconv( 'upper', 'revert', N, A, 1, N, 0, IPIV, 1, 0, E, 1, 0 );
	assert.equal( info, 0, 'info' );
...
test( 'dsyconv: lower_convert (all 1x1 pivots)', function t() {
	...
	const IPIV = convertIPIV( tc.ipiv_trf );
	const E = new Float64Array( N );
	const info = dsyconv( 'lower', 'convert', N, A, 1, N, 0, IPIV, 1, 0, E, 1, 0 );
	assert.equal( info, 0, 'info' );
	assertArrayClose( toArray( A ), tc.a_converted, 1e-14, 'a_converted' );
	assertArrayClose( toArray( E ), tc.e, 1e-14, 'e' );
});
...
test( 'dsyconv: lower_revert (all 1x1 pivots)', function t() {
	...
	const IPIV = convertIPIV( tcConv.ipiv_trf );
	const E = new Float64Array( tcConv.e );
	const info = dsyconv( 'lower', 'revert', N, A, 1, N, 0, IPIV, 1, 0, E, 1, 0 );
	assert.equal( info, 0, 'info' );
	assertArrayClose( toArray( A ), tcRev.a_reverted, 1e-14, 'a_reverted' );
});
...
test( 'dsyconv: n1_upper', function t() {
	...
	const info = dsyconv( 'upper', 'convert', 1, A, 1, 1, 0, IPIV, 1, 0, E, 1, 0 );
	assert.equal( info, 0, 'info' );
	assertArrayClose( toArray( A ), tc.a_converted, 1e-14, 'a_converted' );
	assertArrayClose( toArray( E ), tc.e, 1e-14, 'e' );
});
...
test( 'dsyconv: n1_lower', function t() {
	...
	const IPIV = convertIPIV( tc.ipiv );
	const E = new Float64Array( 1 );
	const info = dsyconv( 'lower', 'convert', 1, A, 1, 1, 0, IPIV, 1, 0, E, 1, 0 );
	assert.equal( info, 0, 'info' );
	assertArrayClose( toArray( A ), tc.a_converted, 1e-14, 'a_converted' );
	assertArrayClose( toArray( E ), tc.e, 1e-14, 'e' );
});
...
test( 'dsyconv: round-trip upper convert then revert restores A', function t() {
	...
	const tc = upper_2x2_convert;
	const N = 4;
	const Aorig = new Float64Array( tc.a_factored );
	const A = new Float64Array( tc.a_factored );
	const IPIV = convertIPIV( tc.ipiv_trf );
	const E = new Float64Array( N );
