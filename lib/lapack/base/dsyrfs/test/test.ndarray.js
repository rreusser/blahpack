...
test( 'dsyrfs: upper_3x3 - single RHS', function t() {
	...
	const result = solveAndRefine( 'upper', 3, 1, A, b );
	assert.equal( result.info, 0 );
	const resid = residualCheck( 'upper', 3, A, result.x, b );
...
test( 'dsyrfs: lower_3x3 - single RHS', function t() {
	...
	const b = new Float64Array([ 1.0, 2.0, 3.0 ]);
	const result = solveAndRefine( 'lower', 3, 1, A, b );
	assert.equal( result.info, 0 );
	const resid = residualCheck( 'lower', 3, A, result.x, b );
	assert.ok( resid < 1e-14, 'residual should be near machine epsilon, got ' + resid ); // eslint-disable-line max-len
	assert.ok( result.berr[ 0 ] < 1e-13, 'berr should be tiny, got ' + result.berr[ 0 ] ); // eslint-disable-line max-len
	assert.ok( result.ferr[ 0 ] < 1e-12, 'ferr should be small, got ' + result.ferr[ 0 ] ); // eslint-disable-line max-len
});
...
test( 'dsyrfs: multi_rhs - two right-hand sides', function t() {
	...
	const result = solveAndRefine( 'upper', 3, 2, A, b );
	...
	assert.equal( result.info, 0 );
	const resid1 = residualCheck( 'upper', 3, A, x1, b1 );
	const resid2 = residualCheck( 'upper', 3, A, x2, b2 );
...
test( 'dsyrfs: N=0 quick return', function t() {
	...
	const info = dsyrfs( 'upper', 0, 1, A, 1, 1, 0, AF, 1, 1, 0, IPIV, 1, 0, B, 1, 1, 0, X, 1, 1, 0, FERR, 1, 0, BERR, 1, 0, WORK, 1, 0, IWORK, 1, 0 ); // eslint-disable-line max-len
	assert.equal( info, 0 );
...
test( 'dsyrfs: NRHS=0 quick return', function t() {
	...
	const info = dsyrfs( 'upper', 3, 0, A, 1, 3, 0, AF, 1, 3, 0, IPIV, 1, 0, B, 1, 3, 0, X, 1, 3, 0, FERR, 1, 0, BERR, 1, 0, WORK, 1, 0, IWORK, 1, 0 ); // eslint-disable-line max-len
	assert.equal( info, 0 );
});
...
test( 'dsyrfs: 1x1 system', function t() {
	...
	const result = solveAndRefine( 'upper', 1, 1, A, b );
	assert.equal( result.info, 0 );
...
test( 'dsyrfs: perturbed solution triggers iterative refinement', function t() {
	...
	assert.equal( info, 0 );
	const resid = residualCheck( 'upper', 3, A, X, b );
