...
test( 'dgbcon: N=0 quick return (rcond=1)', function t() {
	...
	const info = dgbcon( 'one-norm', 0, 0, 0, new Float64Array( 0 ), 1, 1, 0, new Int32Array( 0 ), 1, 0, 0.0, rcond, new Float64Array( 0 ), 1, 0, new Int32Array( 0 ), 1, 0 );
...
test( 'dgbcon: anorm=0 returns rcond=0', function t() {
	...
	const info = dgbcon( 'one-norm', N, kl, ku, AB, 1, LDAB, 0, IPIV, 1, 0, 0.0, rcond, work, 1, 0, iwork, 1, 0 );
	assert.strictEqual( info, 0 );
...
test( 'dgbcon: KL=0 KU=0 (diagonal) 4x4', function t() {
	...
	const AB = bandStore( Adense, N, kl, ku );
	const LDAB = (2 * kl) + ku + 1;
	const IPIV = new Int32Array( N );
	...
	const work = new Float64Array( 3 * N );
	const iwork = new Int32Array( N );
	const info = dgbcon( 'one-norm', N, kl, ku, AB, 1, LDAB, 0, IPIV, 1, 0, anorm, rcond, work, 1, 0, iwork, 1, 0 );
	assert.strictEqual( info, 0 );
...
test( 'dgbcon: KL=1 KU=1 (tridiagonal) 5x5 inf-norm', function t() {
	...
	const work = new Float64Array( 3 * N );
	const iwork = new Int32Array( N );
	const info = dgbcon( 'inf-norm', N, kl, ku, AB, 1, LDAB, 0, IPIV, 1, 0, anorm, rcond, work, 1, 0, iwork, 1, 0 );
	assert.strictEqual( info, 0 );
...
test( 'dgbcon: KL=0 KU=0 N=1 (degenerate diagonal)', function t() {
	...
	const work = new Float64Array( 3 * N );
	const iwork = new Int32Array( N );
	const info = dgbcon( 'one-norm', N, kl, ku, AB, 1, LDAB, 0, IPIV, 1, 0, 5.0, rcond, work, 1, 0, iwork, 1, 0 );
	assert.strictEqual( info, 0 );
