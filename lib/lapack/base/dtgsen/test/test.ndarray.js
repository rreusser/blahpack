...
import dtgsen from './../lib/ndarray.js';


// FIXTURES //

const fixtureDir = path.join( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures' );
const rawLines = readFileSync( path.join( fixtureDir, 'dtgsen.jsonl' ), 'utf8' ).trim().split( '\n' ); // eslint-disable-line node/no-sync, max-len
...
function runCase( name, ijob, N, A, B, Q, Z, SELECT ) {
	...
	const ALPHAR = new Float64Array( Math.max( N, 1 ) );
	const ALPHAI = new Float64Array( Math.max( N, 1 ) );
	const BETA = new Float64Array( Math.max( N, 1 ) );
	...
	const info = dtgsen( ijob, true, true, SELECT, 1, 0, N,
		A, 1, ld, 0,
		B, 1, ld, 0,
...
test( 'dtgsen: n1_trivial', function t() {
	const A = buildMat( 1, 1, [ [ 1, 1, 5 ] ] );
	const B = buildMat( 1, 1, [ [ 1, 1, 2 ] ] );
...
test( 'dtgsen: negative_b_diag (sign-flip path)', function t() {
	const A = buildMat( 3, 3, [ [ 1, 1, 2 ], [ 1, 2, 0.5 ], [ 1, 3, 0.3 ], [ 2, 2, 3 ], [ 2, 3, 0.4 ], [ 3, 3, 1 ] ] );
	const B = buildMat( 3, 3, [ [ 1, 1, -1 ], [ 1, 2, 0.2 ], [ 1, 3, 0.1 ], [ 2, 2, 1.5 ], [ 2, 3, 0.3 ], [ 3, 3, -2 ] ] );
...
test( 'dtgsen: returns -1 for invalid ijob', function t() {
	...
	const WORK = new Float64Array( 500 );
	const IWORK = new Int32Array( 500 );
	const rv = dtgsen( -1, true, true, sel, 1, 0, 1, A, 1, 1, 0, B, 1, 1, 0, ALPHAR, 1, 0, ALPHAI, 1, 0, BETA, 1, 0, identity( 1 ), 1, 1, 0, identity( 1 ), 1, 1, 0, M, [ 0 ], [ 0 ], DIF, 1, 0, WORK, 1, 0, IWORK, 1, 0 );
...
test( 'dtgsen: returns -5 for negative N', function t() {
	...
	const WORK = new Float64Array( 500 );
	const IWORK = new Int32Array( 500 );
	const rv = dtgsen( 0, true, true, sel, 1, 0, -1, A, 1, 1, 0, B, 1, 1, 0, ALPHAR, 1, 0, ALPHAI, 1, 0, BETA, 1, 0, identity( 1 ), 1, 1, 0, identity( 1 ), 1, 1, 0, M, [ 0 ], [ 0 ], DIF, 1, 0, WORK, 1, 0, IWORK, 1, 0 );
