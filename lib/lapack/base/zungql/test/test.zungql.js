...
import zungql from './../lib/zungql.js';
...
test( 'zungql: column-major 3x3 produces orthonormal Q', function t() {
	...
	const info = zungql( 'column-major', M, N, K, A, LDA, TAU, 1, Wbig, 1 );
...
test( 'zungql: row-major path executes (N=1 trivial Q)', function t() {
	...
	const info = zungql( 'row-major', 1, 1, 0, new Complex128Array( [ 0, 0 ] ), 1, new Complex128Array( 1 ), 1, new Complex128Array( 32 ), 1 );
	assert.strictEqual( info, 0, 'info' );
});
