...
function dggevx( balanc, jobvl, jobvr, sense, N, A, strideA1, strideA2, offsetA, B, strideB1, strideB2, offsetB, ALPHAR, strideALPHAR, offsetALPHAR, ALPHAI, strideALPHAI, offsetALPHAI, BETA, strideBETA, offsetBETA, VL, strideVL1, strideVL2, offsetVL, VR, strideVR1, strideVR2, offsetVR, LSCALE, strideLSCALE, offsetLSCALE, RSCALE, strideRSCALE, offsetRSCALE, RCONDE, strideRCONDE, offsetRCONDE, RCONDV, strideRCONDV, offsetRCONDV, work, strideWork, offsetWork, iwork, strideIwork, offsetIwork, bwork, strideBwork, offsetBwork ) { // eslint-disable-line max-params
	...
	oTAU = offsetWork;                   // TAU lives at [offsetWork .. offsetWork+N-1]
	oSCR = offsetWork + N;              // scratch starts right after TAU
	lwork = work.length - oSCR;         // remaining elements for scratch
	...
	rcondeS = new Float64Array( 2 );	rcondvS = new Float64Array( 2 );	MCNT = new Int32Array( 1 );

	// Scale A if max element outside range [SMLNUM, BIGNUM]
	anrm = dlange( 'max', N, N, A, strideA1, strideA2, offsetA, work, 1, oSCR );
	ilascl = false;
	anrmto = 0.0;
	...
	bnrm = dlange( 'max', N, N, B, strideB1, strideB2, offsetB, work, 1, oSCR );
	ilbscl = false;
	bnrmto = 0.0;
	...
	bal = dggbal( balanc, N, A, strideA1, strideA2, offsetA, B, strideB1, strideB2, offsetB, LSCALE, strideLSCALE, offsetLSCALE, RSCALE, strideRSCALE, offsetRSCALE, work, 1, oSCR );
	ilo = bal.ilo; // 1-based
	ihi = bal.ihi; // 1-based
	...
	ierr = dhgeqz( chtemp, compvl, compvr, N, ilo - 1, ihi - 1, A, strideA1, strideA2, offsetA, B, strideB1, strideB2, offsetB, ALPHAR, strideALPHAR, offsetALPHAR, ALPHAI, strideALPHAI, offsetALPHAI, BETA, strideBETA, offsetBETA, VL, strideVL1, strideVL2, offsetVL, VR, strideVR1, strideVR2, offsetVR, work, 1, oSCR, lwork );
	if ( ierr !== 0 ) {
		if ( ierr > 0 && ierr <= N ) {
			info = ierr;
	...
	if ( ilv || !wantsn ) {
		...
		if ( !wantsn ) {
			...
			for ( iIdx = 0; iIdx < N; iIdx++ ) {
				...
				rcondvS[ 1 ] = ZERO;
				MCNT[ 0 ] = 0;
				dtgsna( snaJob, 'selected', bwork, 1, offsetBwork, N, A, strideA1, strideA2, offsetA, B, strideB1, strideB2, offsetB, workVL, 1, N, 0, workVR, 1, N, 0, rcondeS, 1, 0, rcondvS, 1, 0, mm, MCNT, SNSWORK, 1, 0, iwrk1Len, iwork, 1, offsetIwork );
