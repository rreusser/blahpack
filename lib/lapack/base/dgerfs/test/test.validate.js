/**
* Property-based validation for dgerfs, following the /blahpack-validate process.
*
* Step 0 classification: `d` -> real scalar; `ge` -> general dense (schemes.dense,
* logical.general); `rfs` (iterative refinement + error bounds) -> three
* independent properties. dgerfs refines an approximate solution X to
...
* already-validated dgetrf; the un-refined initial X by dgetrs; the TRUE solution
* independently by the trusted dgesv applied to op(A0). We assert, against the
...
import dgetrs from '../../dgetrs/lib/ndarray.js';
...
var NRHS = [ 1, 2 ];
var TIGHT = schemes.dense.layouts()[ 0 ]; // tight col-major (a valid pivot layout)
...
test( 'dgerfs: refinement residual + BERR + FERR bound (trans x N x nrhs)', function t() {
...
function runProperty( trans, N, nrhs ) {
	...
	var A0 = logical.general( sc, rng, N, N ); // general dense oracle
	...
	var AFr = schemes.dense.realize( sc, A0, { 'part': 'full' }, TIGHT );
	var ipiv = new Int32Array( N );
	dgetrf( N, N, AFr.data, AFr.args[ 0 ], AFr.args[ 1 ], AFr.args[ 2 ], ipiv, 1, 0 );
	...
	dgetrs( trans, N, nrhs, AFr.data, AFr.args[ 0 ], AFr.args[ 1 ], AFr.args[ 2 ], ipiv, 1, 0, Xr.data, Xr.args[ 0 ], Xr.args[ 1 ], Xr.args[ 2 ] );

	var FERR = new Float64Array( nrhs );
	var BERR = new Float64Array( nrhs );
	var WORK = poisonedWork( sc, 3 * N ); // poisoned: OOB read -> NaN
	var IWORK = new Int32Array( N );
	dgerfs( trans, N, nrhs, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], AFr.data, AFr.args[ 0 ], AFr.args[ 1 ], AFr.args[ 2 ], ipiv, 1, 0, Br.data, Br.args[ 0 ], Br.args[ 1 ], Br.args[ 2 ], Xr.data, Xr.args[ 0 ], Xr.args[ 1 ], Xr.args[ 2 ], FERR, 1, 0, BERR, 1, 0, WORK, 1, 0, IWORK, 1, 0 );

	var Xtrue = trueSolution( A0, B0, N, nrhs, code );

	var tag = 'dgerfs '+trans+' N='+N+' nrhs='+nrhs;

	// (a) residual: refined X remains a valid solution of op(A0)*X = B0.
	checked( 'dgerfs', 'residual', function run() {
		var j;
		for ( j = 0; j < nrhs; j++ ) {
	...
	checked( 'dgerfs', 'structural', function run() {
		var xcol;
		var tcol;
		var berrCap;
		var eActual;
...
// Step 4: layout-invariance fuzz. dgerfs consumes an already-computed
...
// re-realizes those FIXED values at every storage layout and runs only dgerfs.
// Its inner residual kernel dgemv has a col<->row fast-path switch that
...
test( 'dgerfs: bit-exact within storage-order family (col / row)', function t() {
	TRANS.forEach( function eachTrans( trans ) {
...
function runInvariance( trans, variants, fam ) {
	...
	var AF0 = schemes.dense.realize( sc, A0, { 'part': 'full' }, TIGHT );
	var ipiv = new Int32Array( N );
	dgetrf( N, N, AF0.data, AF0.args[ 0 ], AF0.args[ 1 ], AF0.args[ 2 ], ipiv, 1, 0 );
	var Lfac = readFac( AF0, N );

	var X0r = schemes.dense.realize( sc, B0, { 'part': 'full' }, TIGHT );
	dgetrs( trans, N, nrhs, AF0.data, AF0.args[ 0 ], AF0.args[ 1 ], AF0.args[ 2 ], ipiv, 1, 0, X0r.data, X0r.args[ 0 ], X0r.args[ 1 ], X0r.args[ 2 ] );
	var Xinit = readMat( X0r, N, nrhs );

	checked( 'dgerfs', 'layout-invariance', function run() {
		layoutInvariant( variants, function build( layout ) {
			var Ar = schemes.dense.realize( sc, A0, { 'part': 'full' }, layout );
			var AFr = schemes.dense.realize( sc, Lfac, { 'part': 'full' }, layout );
			var Br = schemes.dense.realize( sc, B0, { 'part': 'full' }, layout );
			var Xr = schemes.dense.realize( sc, Xinit, { 'part': 'full' }, layout );
			var FERR = new Float64Array( nrhs );
			var BERR = new Float64Array( nrhs );
			var WORK = poisonedWork( sc, 3 * N );
			var IWORK = new Int32Array( N );
			dgerfs( trans, N, nrhs, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], AFr.data, AFr.args[ 0 ], AFr.args[ 1 ], AFr.args[ 2 ], ipiv, 1, 0, Br.data, Br.args[ 0 ], Br.args[ 1 ], Br.args[ 2 ], Xr.data, Xr.args[ 0 ], Xr.args[ 1 ], Xr.args[ 2 ], FERR, 1, 0, BERR, 1, 0, WORK, 1, 0, IWORK, 1, 0 );
			...
			return out;
		}, { 'label': 'dgerfs '+trans+' layout invariance '+fam+'-major' } );
	});
}
