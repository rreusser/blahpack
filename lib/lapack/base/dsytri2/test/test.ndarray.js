...
import dsytri2 from './../lib/ndarray.js';


// FIXTURES //

const fixtureDir = path.join( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures' ); // eslint-disable-line max-len
const lines = readFileSync( path.join( fixtureDir, 'dsytri2.jsonl' ), 'utf8' ).trim().split( '\n' ); // eslint-disable-line max-len
...
function runFixture( uplo, N, tc, tol ) {
	...
	const A = new Float64Array( tc.a_factored.length );
	for ( i = 0; i < tc.a_factored.length; i++ ) {
		A[ i ] = tc.a_factored[ i ];
	}
	const ipiv = convertIpiv( tc.ipiv );
	const work = makeWork( N );
	const info = dsytri2( uplo, N, A, 1, LDA, 0, ipiv, 1, 0, work, 1, 0 );
...
test( 'dsytri2: N=1 trivial inverse (unblocked path)', function t() {
	...
	const info = dsytri2( 'upper', 1, A, 1, LDA, 0, ipiv, 1, 0, work, 1, 0 );
	assert.equal( info, tc.info, 'info' );
...
test( 'dsytri2: N=0 quick return', function t() {
	...
	const info = dsytri2( 'lower', 0, A, 1, 1, 0, ipiv, 1, 0, work, 1, 0 );
