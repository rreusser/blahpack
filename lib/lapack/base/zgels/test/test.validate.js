...
const PAIRS = [ [ 1, 1 ], [ 2, 3 ], [ 3, 2 ], [ 5, 3 ], [ 3, 5 ], [ 8, 5 ], [ 16, 7 ], [ 7, 16 ], [ 17, 17 ], [ 33, 20 ], [ 20, 33 ], [ 40, 33 ], [ 33, 40 ], [ 64, 64 ] ];
const NRHS = [ 1, 3 ];
...
function runProperty( trans, M, N, nrhs ) {
	...
	const rng = new RNG( 0x5000 + ( M * 131 ) + ( N * 7 ) + ( ( trans === 'no-transpose' ) ? 0 : 3 ) + nrhs );
	const A = logical.general( sc, rng, M, N );
	const B = makeB( rng, ldb, d.p, nrhs );
	...
	const lwork = bigLwork( M, N, nrhs );
	const WORK = sc.alloc( lwork );
	const info = zgels( trans, M, N, nrhs, RA.data, RA.args[ 0 ], RA.args[ 1 ], RA.args[ 2 ], RB.data, RB.args[ 0 ], RB.args[ 1 ], RB.args[ 2 ], WORK, 1, 0 );
	if ( info !== 0 ) {
...
test( 'zgels: layout invariance (bit-exact within storage-order family)', function t() {
	const sizes = [ [ 5, 3 ], [ 3, 5 ], [ 4, 4 ], [ 40, 33 ], [ 33, 40 ] ];
	let ti, si;
	for ( ti = 0; ti < TRANS.length; ti++ ) {
