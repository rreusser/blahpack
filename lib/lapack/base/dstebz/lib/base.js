...
import dlaebz from './../../dlaebz/lib/base.js';
...
* @param {Float64Array} WORK - caller-owned contiguous workspace of length 4*N; base.js never allocates. Partitioned to match the reference DSTEBZ: E2 (squared off-diagonals) occupies WORK[0:N] and the dlaebz AB/C interval buffers occupy WORK[N:4*N].
* @param {integer} strideWork - stride for WORK
* @param {NonNegativeInteger} offsetWork - starting index for WORK
* @param {Int32Array} IWORK - caller-owned contiguous workspace of length 3*N; base.js never allocates. Holds the dlaebz NAB (and NVAL, range=`'index'`) count buffers, matching the reference DSTEBZ.
...
function dstebz( range, order, N, vl, vu, il, iu, abstol, d, strideD, offsetD, e, strideE, offsetE, M, nsplit, w, strideW, offsetW, IBLOCK, strideIBLOCK, offsetIBLOCK, ISPLIT, strideISPLIT, offsetISPLIT, WORK, strideWork, offsetWork, IWORK, strideIWork, offsetIWork ) { // eslint-disable-line max-len, max-params
	let nabWork, ncnvrg, toofew, irange, iorder, ibegin, idiscl, idiscu, pivmin;
	let abWork, bnorm, tnorm, iinfo, iwoff, wkill, atoli, itmax, itmp1, jdisc;
	let cWork, iout, info, ioff, iend, tmp1, tmp2, mout, nwl, nwu, wlu, wul;
	let in_, nsp, gl, gu, im, wl, wu, ib, ie, iw, jb, je, m, j;
	...
	const safemn = dlamch( 'safe-minimum' );
	const ulp = dlamch( 'precision' );
	const rtoli = ulp * RELFAC;
	const nb = 0;
	...
	if ( irange === 3 ) {
		...
		// Set up 2 search intervals for dlaebz IJOB=3. Partition the caller-
		// owned WORK/IWORK to match the reference DSTEBZ layout: E2 occupies
		// WORK[0:N]; AB (2x2 col-major) starts at WORK[N]; C (length 2) at
		// WORK[N+2*MMAX]=WORK[N+4]; NAB (2x2 col-major) at IWORK[0]; NVAL
		// (length 2) at IWORK[4]. The dlaebz WORK/IWORK scratch (unused here
		// since NB=0) reuses the output arrays W and IBLOCK, as in the
		// reference. base.js never allocates problem-sized workspace.
		abWork = WORK.subarray( offsetWork + N );
		cWork = WORK.subarray( offsetWork + N + 4 );
		nabWork = IWORK.subarray( offsetIWork );
		const nvalWork = IWORK.subarray( offsetIWork + 4 );
		...
		iinfo = dlaebz( 3, itmax, N, 2, 2, nb, atoli, rtoli, pivmin, d, strideD, offsetD, e, strideE, offsetE, WORK, strideWork, offsetWork, nvalWork, 1, 0, abWork, 1, 2, 0, cWork, 1, 0, mout, nabWork, 1, 2, 0, w, strideW, offsetW, IBLOCK, strideIBLOCK, offsetIBLOCK );
	...
	for ( jb = 0; jb < nsp; jb++ ) {
		...
		if ( in_ === 1 ) {
			...
			// Partition the caller-owned WORK/IWORK for the dlaebz calls to
			// match the reference DSTEBZ layout: E2 occupies WORK[0:N]; AB
			// (in_ x 2 col-major) starts at WORK[N]; C (length in_) at
			// WORK[N+2*in_]; NAB (in_ x 2 col-major) at IWORK[0]. The dlaebz
			// WORK/IWORK scratch (unused here since NB=0) reuses the tails of
			// the output arrays W and IBLOCK, as in the reference. base.js
			// never allocates problem-sized workspace.
			abWork = WORK.subarray( offsetWork + N );
			cWork = WORK.subarray( offsetWork + N + (2 * in_) );
			nabWork = IWORK.subarray( offsetIWork );
			...
			iinfo = dlaebz( 1, 0, in_, in_, 1, nb, atoli, rtoli, pivmin, d, strideD, offsetD + (ibegin * strideD), e, strideE, offsetE + (ibegin * strideE), WORK, strideWork, offsetWork + (ibegin * strideWork), idumma, 1, 0, abWork, 1, in_, 0, cWork, 1, 0, mout, nabWork, 1, in_, 0, w, strideW, offsetW + (m * strideW), IBLOCK, strideIBLOCK, offsetIBLOCK + (m * strideIBLOCK) );
			...
			iinfo = dlaebz( 2, itmax, in_, in_, 1, nb, atoli, rtoli, pivmin, d, strideD, offsetD + (ibegin * strideD), e, strideE, offsetE + (ibegin * strideE), WORK, strideWork, offsetWork + (ibegin * strideWork), idumma, 1, 0, abWork, 1, in_, 0, cWork, 1, 0, mout, nabWork, 1, in_, 0, w, strideW, offsetW + (m * strideW), IBLOCK, strideIBLOCK, offsetIBLOCK + (m * strideIBLOCK) );
