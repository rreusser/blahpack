/**
* Property-based validation for zptsvx, following the /blahpack-validate process.
*
* Step 0 classification: `z` -> complex scalar; `pt` -> Hermitian POSITIVE DEFINITE
* TRIDIAGONAL (logical.tridiagonalPositiveDefinite); `svx` (EXPERT tridiagonal-solve
* DRIVER: factor (zpttrf) + condition estimate (zptcon) + solve (zpttrs) + iterative
* refinement (zptrfs) + error bounds, all in one call) -> a COMPOSITE of the three
* properties already proven for its constituent routines. We drive zptsvx with
* fact='not-factored' (factor A into DF/EF, NO equilibration -> X solves the ORIGINAL
* A0*X=B0 directly) and assert, against the ORIGINAL full Hermitian tridiagonal A0:
...
*       reciprocal condition number (anorm from A0, ‖A0⁻¹‖₁ from a trusted zptsv
...
*   e(i) = A0.get(i+1,i)  (SUB-diagonal,  COMPLEX Complex128Array, length N-1)
* CONFIRMED from base.js: `d`/`DF` are Float64Array (the HPD diagonal is real);
* `e`/`EF`/`B`/`X`/`WORK` are Complex128; `RWORK`/`FERR`/`BERR`/`rcond` are real. There
* is NO trans/uplo param (the matrix is Hermitian). anorm/invnorm are 1-norms of the
* FULL logical matrix.
*
...
import zptsvx from './../lib/ndarray.js';
import zptsvxBase from './../lib/base.js';
import zptsv from '../../zptsv/lib/ndarray.js';
...
// Main diagonal as REAL numbers (d is Float64Array for zptsvx).
...
// 1-norm (max abs column sum) of the FULL Hermitian tridiagonal logical matrix.
...
// Independent ‖A0⁻¹‖₁: solve A0·X = I with the trusted zptsv on fresh d/e; X =
...
function invNorm1( A0, n ) {
	...
	const info = zptsv( n, n, D.data, D.args[ 0 ], D.args[ 1 ], E.data, E.args[ 0 ], E.args[ 1 ], Br.data, Br.args[ 0 ], Br.args[ 1 ], Br.args[ 2 ] );
	if ( info !== 0 ) {
		throw new Error( 'oracle zptsv failed (info='+info+'); matrix not HPD?' );
	}
	let mx = 0.0;
	let s, i, j;
	for ( j = 0; j < n; j++ ) {
...
// Independent TRUE solution: solve A0*X = B0 on fresh copies with trusted zptsv.
function trueSolution( A0, B0, N, nrhs ) {
	const D = schemes.realizeVector( S.real, mainDiagReal( A0, N ), TIGHTV );
	const E = schemes.realizeVector( sc, subDiag( A0, N ), TIGHTV );
	const Br = schemes.dense.realize( sc, B0, { 'part': 'full' }, TIGHTB );
	const info = zptsv( N, nrhs, D.data, D.args[ 0 ], D.args[ 1 ], E.data, E.args[ 0 ], E.args[ 1 ], Br.data, Br.args[ 0 ], Br.args[ 1 ], Br.args[ 2 ] );
	if ( info !== 0 ) {
		throw new Error( 'oracle zptsv failed (info='+info+'); matrix not HPD?' );
	}
	return Br;
...
function drive( N, nrhs, A0, B0, layout, impl ) {
	...
	const EF = new Complex128Array( Math.max( N - 1, 1 ) );
	...
	const WORK = new Complex128Array( Math.max( N, 1 ) );
	const RWORK = new Float64Array( Math.max( N, 1 ) );
	const info = fn( 'not-factored', N, nrhs, D.data, D.args[ 0 ], D.args[ 1 ], E.data, E.args[ 0 ], E.args[ 1 ], DF, 1, 0, EF, 1, 0, Br.data, Br.args[ 0 ], Br.args[ 1 ], Br.args[ 2 ], Xr.data, Xr.args[ 0 ], Xr.args[ 1 ], Xr.args[ 2 ], rcond, FERR, 1, 0, BERR, 1, 0, WORK, 1, 0, RWORK, 1, 0 );
	return {
		'Xr': Xr,
		'rcond': rcond,
		'FERR': FERR,
...
test( 'zptsvx: expert-driver residual + rcond + FERR/BERR (N x nrhs)', function t() {
...
function runProperty( N, nrhs ) {
	...
	const tag = 'zptsvx N='+N+' nrhs='+nrhs;

	if ( d.info !== 0 ) {
		throw new Error( tag+': zptsvx returned info='+d.info+' (expected 0 for well-conditioned HPD input)' );
	}

	// (a) residual: X solves the ORIGINAL A0*X = B0.
	checked( 'zptsvx', 'residual', function run() {
		let j;
		for ( j = 0; j < nrhs; j++ ) {
	...
	checked( 'zptsvx', 'property', function run() {
		...
		const db = drive( N, nrhs, A0, B0, TIGHT, zptsvxBase );
	...
	checked( 'zptsvx', 'structural', function run() {
...
// invariance family (as for zptsv). Only ADDRESSING changes: the flattened
// X ++ rcond ++ FERR ++ BERR must reproduce bit-for-bit.
const VL = schemes.vectorLayouts();
const DENSE = schemes.dense.layouts();

test( 'zptsvx: bit-exact across strided vector + dense layouts (single family)', function t() {
	const N = 12;
	const nrhs = 2;
	const SEED = 0xF00D;
	checked( 'zptsvx', 'layout-invariance', function run() {
		layoutInvariant( VL, function build( vL, idx ) {
			...
			return out;
		}, { 'label': 'zptsvx layout invariance' } );
	});
});
