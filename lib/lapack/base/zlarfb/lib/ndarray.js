...
function zlarfb( side, trans, direct, storev, M, N, K, V, strideV1, strideV2, offsetV, T, strideT1, strideT2, offsetT, C, strideC1, strideC2, offsetC, WORK, strideWork1, strideWork2, offsetWork ) {
	...
	// from an out-of-bounds read. WORK is an ldw-by-K matrix where ldw is `N`
	// for a left apply and `M` for a right apply.
	if ( K > 0 ) {
		ldw = ( side === 'left' ) ? N : M;
		need = ( ( ldw - 1 ) * strideWork1 ) + ( ( K - 1 ) * strideWork2 ) + 1;
