...
import zlansp from './../lib/ndarray.js';


// FIXTURES //

const fixtureFile = path.join( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures', 'zlansp.jsonl' );
...
FIXTURES.forEach( function build( fx ) {
	test( 'zlansp: ' + fx.name, function t() {
		...
		const result = zlansp( args.norm, args.uplo, args.N, AP, 1, 0, WORK, 1, 0 );
