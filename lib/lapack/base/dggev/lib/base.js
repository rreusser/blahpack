...
const BIGNUM_BASE = ONE / SMLNUM_BASE; // eslint-disable-line no-unused-vars
...
function dggev( jobvl, jobvr, N, A, strideA1, strideA2, offsetA, B, strideB1, strideB2, offsetB, ALPHAR, strideALPHAR, offsetALPHAR, ALPHAI, strideALPHAI, offsetALPHAI, BETA, strideBETA, offsetBETA, VL, strideVL1, strideVL2, offsetVL, VR, strideVR1, strideVR2, offsetVR, WORK, strideWork, offsetWork ) { // eslint-disable-line max-len
	let ilascl, ilbscl, anrmto, bnrmto, chtemp, compvl, compvr, oVLdst, icols;
	let oBsrc, ierr, info, oVLI;
	...
	const oLSCALE = offsetWork;                              // WORK[0..N-1]
	const oRSCALE = offsetWork + ( N * strideWork );         // WORK[N..2N-1]
	const oTAU = offsetWork + ( 2 * N * strideWork );     // WORK[2N..3N-1] (TAU, carved from scratch)
	const oWRK = offsetWork + ( 3 * N * strideWork );     // WORK[3N..] (QR scratch, after TAU)
	const SELECT = new Uint8Array( 1 ); // unused but required by dtgevc

	// Scale A if max element outside range [SMLNUM, BIGNUM]
	const anrm = dlange( 'max', N, N, A, strideA1, strideA2, offsetA, WORK, strideWork, oWRK );
	ilascl = false;
	anrmto = 0.0;
	...
	const bnrm = dlange( 'max', N, N, B, strideB1, strideB2, offsetB, WORK, strideWork, oWRK );
	ilbscl = false;
	bnrmto = 0.0;
	...
	const bal = dggbal( 'permute', N, A, strideA1, strideA2, offsetA, B, strideB1, strideB2, offsetB, WORK, strideWork, oLSCALE, WORK, strideWork, oRSCALE, WORK, strideWork, offsetWork + ( 2 * N * strideWork ) );
	...
	ierr = dhgeqz( chtemp, compvl, compvr, N, ilo - 1, ihi - 1, A, strideA1, strideA2, offsetA, B, strideB1, strideB2, offsetB, ALPHAR, strideALPHAR, offsetALPHAR, ALPHAI, strideALPHAI, offsetALPHAI, BETA, strideBETA, offsetBETA, VL, strideVL1, strideVL2, offsetVL, VR, strideVR1, strideVR2, offsetVR, WORK, strideWork, oTAU );
	if ( ierr !== 0 ) {
		if ( ierr > 0 && ierr <= N ) {
			info = ierr;
