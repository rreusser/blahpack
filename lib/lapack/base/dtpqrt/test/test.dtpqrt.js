...
import dtpqrt from './../lib/dtpqrt.js';
...
test( 'dtpqrt: column-major dispatch', function t() {
	const A = new Float64Array( [ 2.0, 0.0, 0.0, 0.5, 3.0, 0.0, 0.25, 0.75, 4.0 ] );  // 3x3 col-major upper
	const B = new Float64Array( [ 1.0, 0.3, 0.7, 0.2, 0.5, 1.1, 0.4, 0.3, 0.25, 0.6, 1.2, 0.4 ] );  // 4x3 col-major
	const T = new Float64Array( 2 * 3 );
	const W = new Float64Array( 2 * 3 );
	const info = dtpqrt( 'column-major', 4, 3, 0, 2, A, 3, B, 4, T, 2, W );
...
test( 'dtpqrt: row-major dispatch', function t() {
	...
	const A = new Float64Array( [ 2.0, 0.5, 0.25, 0.0, 3.0, 0.75, 0.0, 0.0, 4.0 ] );  // 3x3 row-major upper
	const B = new Float64Array( [ 1.0, 0.5, 0.25, 0.3, 1.1, 0.6, 0.7, 0.4, 1.2, 0.2, 0.3, 0.4 ] );  // 4x3 row-major
	const T = new Float64Array( 2 * 3 );
	const W = new Float64Array( 2 * 3 );
	const info = dtpqrt( 'row-major', 4, 3, 0, 2, A, 3, B, 3, T, 3, W );
	assert.strictEqual( info, 0, 'INFO=0' );
});
