...
*/
function dlatrs3( order, uplo, trans, diag, normin, N, nrhs, A, LDA, X, LDX, SCALE, strideSCALE, CNORM, strideCNORM, work ) {
	let sa1, sa2, sx1, sx2;
	...
	if ( work === null || work === void 0 ) {
		const nba = Math.ceil( N / 8 );
		const minWork = Math.max( 1, 2 * nba * Math.max( nba, Math.min( nrhs, 32)));
		work = new Float64Array( minWork );
	}
	return base( uplo, trans, diag, normin, N, nrhs, A, sa1, sa2, 0, X, sx1, sx2, 0, SCALE, strideSCALE, 0, CNORM, strideCNORM, 0, work, 1, 0 );
}
