...
import zlartg from './../../../../lapack/base/zlartg/lib/base.js';
import zlassq from './../../../../lapack/base/zlassq/lib/base.js';
import zrot from './../../../../lapack/base/zrot/lib/base.js';
...
function ztgex2( wantq, wantz, N, A, strideA1, strideA2, offsetA, B, strideB1, strideB2, offsetB, Q, strideQ1, strideQ2, offsetQ, Z, strideZ1, strideZ2, offsetZ, j1 ) { // eslint-disable-line max-len, max-params
	let strong, scale, weak, sum, res, Sv, Tv, sa, sb, oa, ob, sv, ij, i;
	...
	const S = new Complex128Array( LDST * LDST );
	const T = new Complex128Array( LDST * LDST );
	...
	const g = new Complex128Array( 1 );
	const c = new Float64Array( 1 );
	const s = new Complex128Array( 1 );
	const cdum = new Complex128Array( 1 );

	// Float64Array(2) buffers for zrot sine arguments
	const szArr = new Float64Array( 2 );
	const sqArr = new Float64Array( 2 );
	const negConjSZ = new Float64Array( 2 );
	const negSQ = new Float64Array( 2 );
	const conjSQ = new Float64Array( 2 );
	...
	const thresha = Math.max( TWENTY * eps * sa, smlnum );
	const threshb = Math.max( TWENTY * eps * sb, smlnum );
	...
	const fr = ( (Sv[6] * Tv[0]) - (Sv[7] * Tv[1]) ) - ( (Tv[6] * Sv[0]) - (Tv[7] * Sv[1]) );
	const fi = ( (Sv[6] * Tv[1]) + (Sv[7] * Tv[0]) ) - ( (Tv[6] * Sv[1]) + (Tv[7] * Sv[0]) );

	// g = S(2,2)*T(1,2) - T(2,2)*S(1,2)
	const gr = ( (Sv[6] * Tv[4]) - (Sv[7] * Tv[5]) ) - ( (Tv[6] * Sv[4]) - (Tv[7] * Sv[5]) );
	const gi = ( (Sv[6] * Tv[5]) + (Sv[7] * Tv[4]) ) - ( (Tv[6] * Sv[5]) + (Tv[7] * Sv[4]) );
