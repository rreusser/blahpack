...
import dlagv2 from '../../dlagv2/lib/base.js';
...
import dorgr2 from '../../dorgr2/lib/base.js';
import dorm2r from '../../dorm2r/lib/base.js';
import dormr2 from '../../dormr2/lib/base.js';
import drot from '../../../../blas/base/drot/lib/base.js';
import dscal from '../../../../blas/base/dscal/lib/base.js';
import dtgsy2 from '../../dtgsy2/lib/base.js';
...
const SMLNUM = dlamch( 'safe-minimum' ) / EPS;
...
* @param {Float64Array} WORK - caller-owned workspace of at least `max( 1, N*(n1+n2), (n1+n2)*(n1+n2)*2 )` elements; base.js never allocates
...
*/
function dtgex2( wantq, wantz, N, A, strideA1, strideA2, offsetA, B, strideB1, strideB2, offsetB, Q, strideQ1, strideQ2, offsetQ, Z, strideZ1, strideZ2, offsetZ, j1, n1, n2, WORK, strideWork, offsetWork ) { // eslint-disable-line max-len
	let dscale, strong, ddum, dsum, weak, info, res, sa, sb, f, g;
	...
	// Fixed-size 4-by-4 scratch arrays (column-major, stride=LDST=4). These do
	// NOT scale with the problem — the routine only ever swaps 1-by-1/2-by-2
	// diagonal blocks (m <= 4) — so they are algorithm-internal bookkeeping,
	// not caller-owned workspace. Sizes are literals (LDST*LDST=16, LDST=4,
	// LDST+2=6) so the workspace-allocation policy treats them as fixed scratch.
	const li = new Float64Array( 16 );
	const ir = new Float64Array( 16 );
	const s = new Float64Array( 16 );
	const t = new Float64Array( 16 );
	const scpy = new Float64Array( 16 );
	const tcpy = new Float64Array( 16 );
	const ircop = new Float64Array( 16 );
	const licop = new Float64Array( 16 );
	const taul = new Float64Array( 4 );
	const taur = new Float64Array( 4 );
	const ai = new Float64Array( 2 );
	const ar = new Float64Array( 2 );
	const be = new Float64Array( 2 );
	const iwork = new Int32Array( 6 );
	const out = new Float64Array( 3 ); // for dlartg output

	// Problem-sized workspace is caller-owned. `wk` is a view into WORK at
	// offsetWork (workspace is contiguous, stride 1); base.js never allocates.
	const wk = ( offsetWork === 0 ) ? WORK : WORK.subarray( offsetWork );
	...
	dscale = res.scl;
	dsum = res.sumsq;
	const dnormb = dscale * sqrt( dsum );

	// Thresholds
	const thresha = max( TWENTY * EPS * dnorma, SMLNUM );
	const threshb = max( TWENTY * EPS * dnormb, SMLNUM );
	...
	return dtgex2General( wantq, wantz, N, A, strideA1, strideA2, offsetA, B, strideB1, strideB2, offsetB, Q, strideQ1, strideQ2, offsetQ, Z, strideZ1, strideZ2, offsetZ, j1, n1, n2, m, li, ir, s, t, scpy, tcpy, ircop, licop, taul, taur, ai, ar, be, iwork, wk, thresha, threshb ); // eslint-disable-line max-len
}
...
*/
function dtgex2General( wantq, wantz, N, A, strideA1, strideA2, offsetA, B, strideB1, strideB2, offsetB, Q, strideQ1, strideQ2, offsetQ, Z, strideZ1, strideZ2, offsetZ, j1, n1, n2, m, li, ir, s, t, scpy, tcpy, ircop, licop, taul, taur, ai, ar, be, iwork, wk, thresha, threshb ) { // eslint-disable-line max-len
	let linfo, lagv, res, i;

	// Scalars for dtgsy2
	const scale = new Float64Array( 1 );
	const idum = new Int32Array( 1 );
	...
	const dscale = new Float64Array( 1 );
	const dsum = new Float64Array( 1 );
	dscale[ 0 ] = ZERO;
	dsum[ 0 ] = ONE;
	...
	const brqa21 = dscale[ 0 ] * sqrt( dsum[ 0 ] );
	...
	const bqra21 = dscale[ 0 ] * sqrt( dsum[ 0 ] );
	...
	const sa = res.scl * sqrt( res.sumsq );
	...
	dsum[ 0 ] = ONE;
	res = dlassq( m * m, wk, 1, m * m, dscale[ 0 ], dsum[ 0 ] );
	const sb = res.scl * sqrt( res.sumsq );

	const strong = sa <= thresha && sb <= threshb;
