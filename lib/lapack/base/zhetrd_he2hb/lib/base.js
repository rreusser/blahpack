...
import zhemm from '../../../../blas/base/zhemm/lib/base.js';
import zher2k from '../../../../blas/base/zher2k/lib/base.js';
...
const NEG_HALF = new Complex128( -0.5, 0.0 );
...
function zhetrd_he2hb( uplo, N, kd, A, strideA1, strideA2, offsetA, AB, strideAB1, strideAB2, offsetAB, TAU, strideTAU, offsetTAU, WORK, strideWork, offsetWork ) {
	let ldS2, ldW, pn, pk, lk, i, j;

	const upper = ( uplo === 'upper' );
	const sa1 = strideA1;
	const sa2 = strideA2;
	const sab1 = strideAB1;
	const sab2 = strideAB2;
	...
	const ow = offsetWork;
	const tpos = ow;
	const wpos = tpos + ( (kd * kd) * sw );
	const s1pos = wpos + ( (N * kd) * sw );
	const s2pos = s1pos + ( (kd * kd) * sw );
