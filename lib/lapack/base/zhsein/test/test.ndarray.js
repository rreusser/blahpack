...
import zhseinBase from './../lib/base.js';


// FIXTURES //

const fixtureDir = path.join( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures' );
const rawLines = readFileSync( path.join( fixtureDir, 'zhsein.jsonl' ), 'utf8' ).trim().split( '\n' ); // eslint-disable-line node/no-sync, max-len
...
function buildH( N, ld, entries ) {
	const arr = new Complex128Array( ld * N );
	const buf = viewBuf( arr );
	let i, t, idx;
...
function buildW( entries ) {
	const arr = new Complex128Array( entries.length );
	const buf = viewBuf( arr );
	let i;
	for ( i = 0; i < entries.length; i++ ) {
		buf[ i * 2 ] = entries[ i ][ 0 ];
...
function runRight( name, N, H, w, SELECT ) {
	const ld = N;
	const VL = new Complex128Array( ld * N );
	const VR = new Complex128Array( ld * N );
	...
	const res = zhsein( 'right', 'no', 'no', SELECT, 1, 0, N, H, 1, ld, 0, w, 1, 0,
...
function runLeft( name, N, H, w, SELECT ) {
	...
	const res = zhsein( 'left', 'no', 'no', SELECT, 1, 0, N, H, 1, ld, 0, w, 1, 0,
		VL, 1, ld, 0, VR, 1, ld, 0, N, M, WORK, 1, 0, RWORK, 1, 0,
		IFAILL, 1, 0, IFAILR, 1, 0 );
...
function runBoth( name, N, H, w, SELECT, eigsrc ) {
	...
	const res = zhseinBase( 'both', eigsrc || 'no', 'no', SELECT, 1, 0, N, H, 1, ld, 0, w, 1, 0,
		VL, 1, ld, 0, VR, 1, ld, 0, N, M, WORK, 1, 0, RWORK, 1, 0,
		IFAILL, 1, 0, IFAILR, 1, 0 );
...
test( 'zhsein: both_4x4 (SIDE=both via base)', function t() {
	...
	const w = buildW( [ [ 1.0, 0.5 ], [ 2.0, -0.5 ], [ 3.0, 1.0 ], [ 4.0, 0.5 ] ] );
...
test( 'zhsein: both_fromqr_block_5x5 (eigsrc=qr)', function t() {
	...
	const w = buildW( [ [ 2.0, 0.5 ], [ 3.0, -0.5 ], [ 4.0, 1.0 ], [ 5.0, -1.0 ], [ 6.0, 0.5 ] ] );
...
test( 'zhsein: invalid eigsrc returns -2 via base', function t() {
	...
	const res = zhseinBase( 'right', 'bogus', 'no', sel, 1, 0, 1, H, 1, 1, 0, w, 1, 0,
...
test( 'zhsein: invalid initv returns -3 via base', function t() {
	...
	const IFAILL = new Int32Array( 1 );
	const IFAILR = new Int32Array( 1 );
	const res = zhseinBase( 'right', 'no', 'bogus', sel, 1, 0, 1, H, 1, 1, 0, w, 1, 0,
		VL, 1, 1, 0, VR, 1, 1, 0, 1, [ 0 ], WORK, 1, 0, RWORK, 1, 0,
		IFAILL, 1, 0, IFAILR, 1, 0 );
...
test( 'zhsein: negative N returns -5 via base', function t() {
	...
	const IFAILL = new Int32Array( 1 );
	const IFAILR = new Int32Array( 1 );
	const res = zhseinBase( 'right', 'no', 'no', sel, 1, 0, -1, H, 1, 1, 0, w, 1, 0,
		VL, 1, 1, 0, VR, 1, 1, 0, 1, [ 0 ], WORK, 1, 0, RWORK, 1, 0,
		IFAILL, 1, 0, IFAILR, 1, 0 );
