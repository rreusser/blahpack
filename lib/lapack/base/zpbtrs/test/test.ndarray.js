...
test( 'zpbtrs: upper_single_rhs (UPLO=U, N=3, KD=1, NRHS=1)', function t() {
	...
	const finfo = zpbtrf( 'upper', 3, 1, AB, 1, 2, 0 );
	assert.equal( finfo, 0 );
	// b = A*[1, 1+i, 2]
	const B = new Complex128Array( [ 4, 2, 10, 2, 13, 3 ] );
	const info = zpbtrs( 'upper', 3, 1, 1, AB, 1, 2, 0, B, 1, 3, 0 );
...
test( 'zpbtrs: lower_single_rhs (UPLO=L, N=3, KD=1, NRHS=1)', function t() {
	...
	const finfo = zpbtrf( 'lower', 3, 1, AB, 1, 2, 0 );
	assert.equal( finfo, 0 );
	const B = new Complex128Array( [ 4, 2, 10, 2, 13, 3 ] );
	const info = zpbtrs( 'lower', 3, 1, 1, AB, 1, 2, 0, B, 1, 3, 0 );
	assert.equal( info, tc.info );
	assertArrayClose( Array.from( reinterpret( B, 0 ) ), tc.B, 1e-13, 'B' );
});
...
test( 'zpbtrs: n_zero (N=0 quick return)', function t() {
	...
	const info = zpbtrs( 'upper', 0, 1, 1, AB, 1, 2, 0, B, 1, 1, 0 );
	assert.equal( info, 0 );
});
...
test( 'zpbtrs: nrhs_zero (NRHS=0 quick return)', function t() {
	...
	const info = zpbtrs( 'upper', 3, 1, 0, AB, 1, 2, 0, B, 1, 3, 0 );
	assert.equal( info, 0 );
});

test( 'zpbtrs: upper_two_rhs (UPLO=U, N=3, KD=1, NRHS=2)', function t() {
	const tc = upper_two_rhs;
	const AB = new Complex128Array( [
		0, 0, 4, 0,
		1, 1, 5, 0,
		2, -1, 6, 0
	...
	const info = zpbtrs( 'upper', 3, 1, 2, AB, 1, 2, 0, B, 1, 3, 0 );
	assert.equal( info, tc.info );
	assertArrayClose( Array.from( reinterpret( B, 0 ) ), tc.B, 1e-13, 'B' );
});

test( 'zpbtrs: lower_two_rhs (UPLO=L, N=3, KD=1, NRHS=2)', function t() {
	const tc = lower_two_rhs;
	const AB = new Complex128Array( [
		4, 0, 1, -1,
		5, 0, 2, 1,
		6, 0, 0, 0
	...
	const info = zpbtrs( 'lower', 3, 1, 2, AB, 1, 2, 0, B, 1, 3, 0 );
	assert.equal( info, tc.info );
	assertArrayClose( Array.from( reinterpret( B, 0 ) ), tc.B, 1e-13, 'B' );
});
