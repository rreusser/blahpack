...
import zla_heamv from './../../zla_heamv/lib/base.js';
...
const HUGEVAL = Number.POSITIVE_INFINITY;
...
function zla_porfsx_extended( prec_type, uplo, N, nrhs, A, strideA1, strideA2, offsetA, AF, strideAF1, strideAF2, offsetAF, colequ, c, strideC, offsetC, B, strideB1, strideB2, offsetB, Y, strideY1, strideY2, offsetY, BERR_OUT, strideBERR_OUT, offsetBERR_OUT, n_norms, ERR_BNDS_NORM, strideERR_BNDS_NORM1, strideERR_BNDS_NORM2, offsetERR_BNDS_NORM, ERR_BNDS_COMP, strideERR_BNDS_COMP1, strideERR_BNDS_COMP2, offsetERR_BNDS_COMP, RES, strideRES, offsetRES, AYB, strideAYB, offsetAYB, DY, strideDY, offsetDY, Y_TAIL, strideY_TAIL, offsetY_TAIL, rcond, ithresh, rthresh, dz_ub, ignore_cwise ) {
	let y_prec_state, prevnormdx, final_dx_x, final_dz_z, incr_prec, prev_dz_z;
	let dxratmax, dzratmax, x_state, z_state, normdx, normy, normx, dxrat;
	let dzrat, ymin, info, dx_x, dz_z, cnt, dyk, ci, yk, i, j;
	...
	const Y_TAILv = reinterpret( Y_TAIL, 0 );
	const DYv = reinterpret( DY, 0 );
	const Bv = reinterpret( B, 0 );
	const sy1 = strideY1 * 2;
	const sy2 = strideY2 * 2;
	const sb1 = strideB1 * 2;
	const sb2 = strideB2 * 2;
	const oY = offsetY * 2;
	const oB = offsetB * 2;
	const oT = offsetY_TAIL * 2;
	const sT = strideY_TAIL * 2;
	const oD = offsetDY * 2;
	const sD = strideDY * 2;
	...
	for ( j = 0; j < nrhs; j++ ) {
		...
		zla_lin_berr( N, N, 1, RES, strideRES, 0, offsetRES, AYB, strideAYB, 0, offsetAYB, BERR_OUT, strideBERR_OUT, offsetBERR_OUT + ( j * strideBERR_OUT ) );
	}
