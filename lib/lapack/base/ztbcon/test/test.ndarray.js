...
test( 'ztbcon: N=0 quick return (rcond=1)', function t() {
	const rcond = new Float64Array( 1 );
	rcond[ 0 ] = 0.5;
	const info = ztbcon( 'one-norm', 'upper', 'non-unit', 0, 0, new Complex128Array( 0 ), 1, 1, 0, rcond, new Complex128Array( 0 ), 1, 0, new Float64Array( 0 ), 1, 0 );
...
test( 'ztbcon: zero matrix returns rcond=0', function t() {
	...
	const info = ztbcon( 'one-norm', 'upper', 'non-unit', N, KD, AB, 1, KD+1, 0, rcond, work, 1, 0, rwork, 1, 0 );
	assert.strictEqual( info, 0 );
...
test( 'ztbcon: identity 3x3 KD=0 upper non-unit (one-norm)', function t() {
	...
	const AB = packUpperBandReal( A, N, KD );
	const rcond = new Float64Array( 1 );
	const work = new Complex128Array( 2 * N );
	const rwork = new Float64Array( N );
	const info = ztbcon( 'one-norm', 'upper', 'non-unit', N, KD, AB, 1, KD+1, 0, rcond, work, 1, 0, rwork, 1, 0 );
...
test( 'ztbcon: identity 3x3 KD=0 upper non-unit (inf-norm)', function t() {
	...
	const work = new Complex128Array( 2 * N );
	const rwork = new Float64Array( N );
	const info = ztbcon( 'inf-norm', 'upper', 'non-unit', N, KD, AB, 1, KD+1, 0, rcond, work, 1, 0, rwork, 1, 0 );
	assert.strictEqual( info, 0 );
	assert.ok( close( rcond[ 0 ], 1.0, 1e-9 ) );
});
...
test( 'ztbcon: upper unit KD=1 4x4 (one-norm)', function t() {
	...
	const work = new Complex128Array( 2 * N );
	const rwork = new Float64Array( N );
	const info = ztbcon( 'one-norm', 'upper', 'unit', N, KD, AB, 1, KD+1, 0, rcond, work, 1, 0, rwork, 1, 0 );
	assert.strictEqual( info, 0 );
...
test( 'ztbcon: upper unit KD=1 4x4 (inf-norm)', function t() {
	...
	const work = new Complex128Array( 2 * N );
	const rwork = new Float64Array( N );
	const info = ztbcon( 'inf-norm', 'upper', 'unit', N, KD, AB, 1, KD+1, 0, rcond, work, 1, 0, rwork, 1, 0 );
	assert.strictEqual( info, 0 );
	assert.ok( rcond[ 0 ] > 0.0 && rcond[ 0 ] <= 1.0 );
});
...
test( 'ztbcon: lower non-unit diagonal KD=0 4x4 (one-norm)', function t() {
	...
	const AB = packLowerBandReal( A, N, KD );
	const rcond = new Float64Array( 1 );
	const work = new Complex128Array( 2 * N );
	const rwork = new Float64Array( N );
	const info = ztbcon( 'one-norm', 'lower', 'non-unit', N, KD, AB, 1, KD+1, 0, rcond, work, 1, 0, rwork, 1, 0 );
	assert.strictEqual( info, 0 );
...
test( 'ztbcon: lower non-unit diagonal KD=0 4x4 (inf-norm)', function t() {
	...
	const work = new Complex128Array( 2 * N );
	const rwork = new Float64Array( N );
	const info = ztbcon( 'inf-norm', 'lower', 'non-unit', N, KD, AB, 1, KD+1, 0, rcond, work, 1, 0, rwork, 1, 0 );
	assert.strictEqual( info, 0 );
	assert.ok( close( rcond[ 0 ], 0.25, 1e-9 ) );
});
...
test( 'ztbcon: lower unit KD=1 4x4 (one-norm)', function t() {
	...
	const work = new Complex128Array( 2 * N );
	const rwork = new Float64Array( N );
	const info = ztbcon( 'one-norm', 'lower', 'unit', N, KD, AB, 1, KD+1, 0, rcond, work, 1, 0, rwork, 1, 0 );
	assert.strictEqual( info, 0 );
	assert.ok( rcond[ 0 ] > 0.0 && rcond[ 0 ] <= 1.0 );
});
...
test( 'ztbcon: lower unit KD=1 4x4 (inf-norm)', function t() {
	...
	const work = new Complex128Array( 2 * N );
	const rwork = new Float64Array( N );
	const info = ztbcon( 'inf-norm', 'lower', 'unit', N, KD, AB, 1, KD+1, 0, rcond, work, 1, 0, rwork, 1, 0 );
	assert.strictEqual( info, 0 );
	assert.ok( rcond[ 0 ] > 0.0 && rcond[ 0 ] <= 1.0 );
});
...
test( 'ztbcon: well-conditioned upper KD=2 5x5 complex (one-norm)', function t() {
	...
	const work = new Complex128Array( 2 * N );
	const rwork = new Float64Array( N );
	const info = ztbcon( 'one-norm', 'upper', 'non-unit', N, KD, AB, 1, LDAB, 0, rcond, work, 1, 0, rwork, 1, 0 );
	assert.strictEqual( info, 0 );
	assert.ok( rcond[ 0 ] > 0.0 && rcond[ 0 ] <= 1.0 );
});
...
test( 'ztbcon: well-conditioned lower KD=2 5x5 complex (inf-norm)', function t() {
	...
	const work = new Complex128Array( 2 * N );
	const rwork = new Float64Array( N );
	const info = ztbcon( 'inf-norm', 'lower', 'non-unit', N, KD, AB, 1, LDAB, 0, rcond, work, 1, 0, rwork, 1, 0 );
	assert.strictEqual( info, 0 );
	assert.ok( rcond[ 0 ] > 0.0 && rcond[ 0 ] <= 1.0 );
});
