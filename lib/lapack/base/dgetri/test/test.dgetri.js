...
test( 'dgetri has expected arity', function t() {
	assert.strictEqual( dgetri.length, 8, 'has expected arity' );
});

test( 'dgetri throws TypeError for invalid order', function t() {
	assert.throws( function throws() {
		dgetri( 'invalid', new Float64Array( 4 ), new Float64Array( 4 ), 2, new Float64Array( 4 ), 1, new Float64Array( 4 ), 1 );
	}, TypeError );
});

test( 'dgetri throws RangeError for negative N', function t() {
	assert.throws( function throws() {
		dgetri( 'row-major', -1, new Float64Array( 4 ), 2, new Float64Array( 4 ), 1, new Float64Array( 4 ), 1 );
	}, RangeError );
});
