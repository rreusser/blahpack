...
const TIGHT_B = BANDED_POS[ 0 ];               // tight col-major band storage
const TIGHT_D = schemes.dense.layouts()[ 0 ];  // tight col-major dense (B / X)
...
function trueSolution( A0, B0, N, kl, ku, nrhs, code ) {
	const op = opMatrix( A0, N, kl, ku, code );
	const Ar = schemes.banded.realize( sc, op.M, { 'kl': op.kl, 'ku': op.ku, 'luFill': true }, TIGHT_B );
	const Br = schemes.dense.realize( sc, B0, { 'part': 'full' }, TIGHT_D );
	const ipiv = new Int32Array( N );
	const info = zgbsv( N, op.kl, op.ku, nrhs, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], ipiv, 1, 0, Br.data, Br.args[ 0 ], Br.args[ 1 ], Br.args[ 2 ] );
	if ( info !== 0 ) {
...
function opInverse( A0, N, kl, ku, code ) {
	const op = opMatrix( A0, N, kl, ku, code );
	const Ar = schemes.banded.realize( sc, op.M, { 'kl': op.kl, 'ku': op.ku, 'luFill': true }, TIGHT_B );
	const Ir = schemes.dense.realize( sc, identity( N ), { 'part': 'full' }, TIGHT_D );
	const ipiv = new Int32Array( N );
	const info = zgbsv( N, op.kl, op.ku, N, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], ipiv, 1, 0, Ir.data, Ir.args[ 0 ], Ir.args[ 1 ], Ir.args[ 2 ] );
	if ( info !== 0 ) {
...
function callSvx( fact, trans, N, kl, ku, nrhs, A0, B0, abLayout, bLayout ) {
	const Ar = schemes.banded.realize( sc, A0, { 'kl': kl, 'ku': ku, 'luFill': false }, abLayout );
	const AFr = schemes.banded.realize( sc, zeros( N, N ), { 'kl': kl, 'ku': ku, 'luFill': true }, abLayout );
	const Br = schemes.dense.realize( sc, B0, { 'part': 'full' }, bLayout );
	const Xr = schemes.dense.realize( sc, zeros( N, nrhs ), { 'part': 'full' }, bLayout );
	const ipiv = new Int32Array( N );
	const r = new Float64Array( Math.max( 1, N ) );
	const c = new Float64Array( Math.max( 1, N ) );
	const FERR = new Float64Array( nrhs );
	const BERR = new Float64Array( nrhs );
	const WORK = poisonedWork( sc, Math.max( 1, 2 * N ) );
	const RWORK = new Float64Array( Math.max( 1, N ) );
	const res = zgbsvx( fact, trans, N, kl, ku, nrhs, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], AFr.data, AFr.args[ 0 ], AFr.args[ 1 ], AFr.args[ 2 ], ipiv, 1, 0, 'none', r, 1, 0, c, 1, 0, Br.data, Br.args[ 0 ], Br.args[ 1 ], Br.args[ 2 ], Xr.data, Xr.args[ 0 ], Xr.args[ 1 ], Xr.args[ 2 ], FERR, 1, 0, BERR, 1, 0, WORK, 1, 0, RWORK, 1, 0 );
	return {
		'X': Xr,
		'FERR': FERR,
		'BERR': BERR,
...
function runProperty( trans, N, kl, ku, nrhs ) {
	...
	const out = callSvx( 'not-factored', trans, N, kl, ku, nrhs, A0, B0, TIGHT_B, TIGHT_D );
	const tag = 'zgbsvx '+trans+' N='+N+' kl='+kl+' ku='+ku+' nrhs='+nrhs;
	...
	checked( 'zgbsvx', 'property', function run() {
		const inv = opInverse( A0, N, kl, ku, code );
		const trueRcond = 1.0 / ( norm1( inv.op ) * norm1( inv.inv ) );
...
function badlyScaledBand( rng, N, kl, ku ) {
	const M = logical.banded( sc, rng, N, N, kl, ku );
	const pr = [];
	const pc = [];
	let i, j;
	for ( i = 0; i < N; i++ ) {
...
test( 'zgbsvx: equilibration path un-scales X to the ORIGINAL system', function t() {
	...
	[ 'no-transpose', 'transpose' ].forEach( function eachTrans( trans ) {
		[ 5, 8, 16 ].forEach( function eachN( N ) {
			...
			const A0 = badlyScaledBand( rng, N, kl, ku );
			const B0 = logical.general( sc, rng, N, 2 );
			const code = transCode( trans );
			const out = callSvx( 'equilibrate', trans, N, kl, ku, 2, A0, B0, TIGHT_B, TIGHT_D );
			if ( out.res.equed !== 'none' ) {
				sawEquil = true;
			}
			checked( 'zgbsvx', 'residual', function run() {
				const tag = 'zgbsvx EQUIL '+trans+' N='+N+' equed='+out.res.equed;
				if ( out.res.info !== 0 ) {
...
function runInvariance( trans, abFamily, dFamily, fam ) {
	...
	checked( 'zgbsvx', 'layout-invariance', function run() {
		layoutInvariant( variants, function build( v ) {
			...
			const A0 = logical.banded( sc, rng, N, N, kl, ku );
			const B0 = logical.general( sc, rng, N, nrhs );
			const out = callSvx( 'not-factored', trans, N, kl, ku, nrhs, A0, B0, v.ab, v.d );
