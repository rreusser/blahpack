/**
* Property-based validation for dgeqrt2, following the /blahpack-validate process.
*
* Step 0 classification: `d` -> real scalar; `ge` -> general dense; `qrt2`
* (UNBLOCKED compact-WY QR) -> reconstruction A = Q*R AND orthonormality of Q.
*
* dgeqrt2 computes, for a general M-by-N matrix A (M >= N), the compact-WY QR
...
test( 'dgeqrt2: A = Q*R and QᴴQ = I ((M,N) sweep x all layouts)', function t() {
	PAIRS.forEach( function eachPair( pr ) {
		...
		ALL_LAYOUTS.forEach( function eachLayout( layout, li ) {
			...
			dgeqrt2( M, N, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], Tr.data, Tr.args[ 0 ], Tr.args[ 1 ], Tr.args[ 2 ] );

			const label = 'dgeqrt2 M=' + M + ' N=' + N + ' layout=' + li;

			const recon = reconstruct( Ar, Tr.read, M, N, K );
			checked( 'dgeqrt2', 'reconstruct', function run() {
				check.assertReconstruct( sc, recon, A0, { 'label': label, 'factor': 100 } );
			} );

			const Q = formQ( Ar, Tr.read, M, K );
			checked( 'dgeqrt2', 'orthonormal', function run() {
...
// dgemv/dger/dtrmv inside dgeqrt2 reorder across the col<->row flip).
...
test( 'dgeqrt2: bit-exact within storage-order family (col / row)', function t() {
...
function runInvariance( variants, fam ) {
	...
	const K = Math.min( M, N );
	const SEED = 0xF00D;
	checked( 'dgeqrt2', 'layout-invariance', function run() {
		...
		}, { 'label': 'dgeqrt2 layout invariance ' + fam + '-major (M=' + M + ' N=' + N + ')' } );
	} );
}
