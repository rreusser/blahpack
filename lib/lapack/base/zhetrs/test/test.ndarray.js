...
import upper4x4_1rhs from './fixtures/upper_4x4_1rhs.json' with { type: 'json' };
import lower4x4_2rhs from './fixtures/lower_4x4_2rhs.json' with { type: 'json' };
...
function runFixtureTest( name, uplo ) {
	...
	const A = packComplex( tc.A_factored, N, LDA );
	const IPIV = convertIPIV( tc.ipiv, N );

	// Pack B
	const B = packB( tc.b, N, nrhs, LDA );

	// Pack original A for verification
	const Aorig = packComplex( tc.A_orig, N, LDA );

	const info = zhetrs( uplo, N, nrhs, A, 1, N, 0, IPIV, 1, 0, B, 1, N, 0 );
	assert.strictEqual( info, 0 );

	// Verify: A_orig * x should equal b for each RHS
	const Bv = reinterpret( B, 0 );
	let x1, Ax, x1v, b_orig, rr;
	for ( rr = 0; rr < nrhs; rr++ ) {
		x1 = new Complex128Array( N );
		Ax = new Complex128Array( N );
		x1v = reinterpret( x1, 0 );
		for ( let jj = 0; jj < 2 * N; jj++ ) {
			x1v[ jj ] = Bv[ jj + rr * 2 * N ];
		}
		...
		if ( nrhs > 1 ) {
			...
			const bov = reinterpret( b_orig, 0 );
			for ( let kk = 0; kk < 2 * N; kk++ ) {
...
test( 'zhetrs: N=0', function t() {
	const info = zhetrs( 'upper', 0, 1, new Complex128Array( 1 ), 1, 1, 0, new Int32Array( 1 ), 1, 0, new Complex128Array( 1 ), 1, 1, 0 );
	assert.strictEqual( info, 0 );
});
...
test( 'zhetrs: N=1', function t() {
	...
	const info = zhetrs( 'upper', 1, 1, A, 1, 1, 0, IPIV, 1, 0, B, 1, 1, 0 );
	assert.strictEqual( info, 0 );
...
test( 'zhetrs: nrhs=0 returns 0', function t() {
	const info = zhetrs( 'upper', 3, 0, new Complex128Array( 9 ), 1, 3, 0, new Int32Array( 3 ), 1, 0, new Complex128Array( 3 ), 1, 3, 0 );
	assert.strictEqual( info, 0 );
});
