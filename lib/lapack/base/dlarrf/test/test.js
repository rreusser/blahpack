...
import dlarrf from './../lib/index.js';
...
test( 'main export computes a valid RRR for a 4x4 cluster', function t() {
	...
	const N = 4;
	const info = dlarrf( N, d, 1, l, 1, ld, 1, 1, 4, w, 1, wgap, 1, werr, 1, 4.0, 1.0, 1.0, PIVMIN, sigma, dplus, 1, lplus, 1, work, 1 ); // eslint-disable-line max-len
	assert.strictEqual( info, 0, 'info is 0 on success' );
	assert.ok( sigma[ 0 ] >= 0.9 && sigma[ 0 ] <= 1.0, 'sigma is in the lower gap' ); // eslint-disable-line max-len
	assert.ok( dplus[ 0 ] > 0, 'dplus[0] is positive' );
	assert.ok( Math.abs( lplus[ 0 ] ) < 1.0, 'lplus[0] is a reasonable magnitude' ); // eslint-disable-line max-len
});
...
test( 'ndarray computes a valid RRR for a 4x4 cluster', function t() {
	...
	const w = new Float64Array( [ 0.95, 1.95, 2.95, 4.05 ] );
	const N = 4;
	const info = dlarrf.ndarray( N, d, 1, 0, l, 1, 0, ld, 1, 0, 1, 4, w, 1, 0, wgap, 1, 0, werr, 1, 0, 4.0, 1.0, 1.0, PIVMIN, sigma, dplus, 1, 0, lplus, 1, 0, work, 1, 0 ); // eslint-disable-line max-len
	assert.strictEqual( info, 0, 'info is 0 on success' );
	assert.ok( sigma[ 0 ] >= 0.9 && sigma[ 0 ] <= 1.0, 'sigma is in the lower gap' ); // eslint-disable-line max-len
