...
function zlaqps( M, N, offset, nb, A, strideA1, strideA2, offsetA, JPVT, strideJPVT, offsetJPVT, TAU, strideTAU, offsetTAU, VN1, strideVN1, offsetVN1, VN2, strideVN2, offsetVN2, AUXV, strideAUXV, offsetAUXV, F, strideF1, strideF2, offsetF ) {
	let negTauK, lsticc, itemp, temp2, temp, tauK, pvt, rk, k, j;
	...
	const oA = offsetA * 2; // Float64 offset
	const oF = offsetF * 2; // Float64 offset

	const lastrk = Math.min( M, N + offset );
	lsticc = 0;
	k = 0;
	const tol3z = Math.sqrt( dlamch( 'Epsilon' ) );
	const akk = new Float64Array( 2 );
