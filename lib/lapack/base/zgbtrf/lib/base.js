...
const NB = 32; // Block size (Fortran uses ILAENV)
const NBMAX = 64;
const LDWORK = NBMAX + 1;
const TEMP = new Float64Array( 2 );
...
function zgbtrf( M, N, kl, ku, AB, strideAB1, strideAB2, offsetAB, IPIV, strideIPIV, offsetIPIV ) {
	let ONE_NEG, info, idx, id2, km, jp, jb, jm, ju, i2, i3, j2, j3, k2, nw, ip;
	let ii, jj, tr, ti, i, j;
	...
	const WORK13 = new Complex128Array( LDWORK * NBMAX );
	const WORK31 = new Complex128Array( LDWORK * NBMAX );
	const W13v = reinterpret( WORK13, 0 );
	const W31v = reinterpret( WORK31, 0 );
	...
	for ( j = 0; j < minMN; j += NB ) {
		...
		if ( j + jb < N ) {
			...
			// `ju - j + 1` counts columns j..ju inclusive (Fortran: J2 =
			// MIN( JU-J+1, KV ) - JB; J3 = MAX( 0, JU-J-KV+1 )).
