...
import zlangb from './../lib/ndarray.js';


// FIXTURES //

const fixtureFile = path.join( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures', 'zlangb.jsonl' );
...
function buildAB( ldab, ncols, cells ) {
	let idx, i;
	const out = new Complex128Array( ldab * ncols );
	const view = new Float64Array( out.buffer );
...
function runCase( name, norm, N, KL, KU, AB, ldab ) {
	...
	const got = zlangb( norm, N, KL, KU, AB, 1, ldab, 0, WORK, 1, 0 );
...
// 1x1 (3+4i)
const AB_1X1 = buildAB( 1, 1, [ 1, 1, 3.0, 4.0 ] );
