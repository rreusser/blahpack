...
import zlat2c from './../lib/zlat2c.js';
...
test( 'zlat2c converts a 2x2 upper (column-major)', function t() {
	...
	A.set( [ 3.0, 4.0 ], 2 );
	A.set( [ 5.0, 6.0 ], 3 );
	const SA = new Complex64Array( 4 );
	const info = zlat2c( 'column-major', 'upper', 2, A, 2, SA, 2 );
...
test( 'zlat2c converts a 2x2 lower (row-major)', function t() {
	...
	A.set( [ 5.0, 6.0 ], 3 );
	const SA = new Complex64Array( 4 );
	const info = zlat2c( 'row-major', 'lower', 2, A, 2, SA, 2 );
	assert.equal( info, 0, 'info' );
});
