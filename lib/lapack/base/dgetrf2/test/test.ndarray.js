...
function ipivTo0Based( ipiv1 ) {
	const out = new Int32Array( ipiv1.length );
	let i;
...
test( 'dgetrf2: 3x3 non-singular matrix', function t() {
	...
	const tc = _3x3;
	const a = new Float64Array( [ 2, 4, 8, 1, 3, 7, 1, 3, 9 ] );
	const ipiv = new Int32Array( 3 );
	const info = dgetrf2( 3, 3, a, 1, 3, 0, ipiv, 1, 0 );
...
test( 'dgetrf2: 4x3 tall matrix', function t() {
	...
	const tc = _4x3;
	const a = new Float64Array( [ 2, 0, 1, 0, 1, 3, 0, 1, 0, 1, 4, 2 ] );
	const ipiv = new Int32Array( 3 );
	const info = dgetrf2( 4, 3, a, 1, 4, 0, ipiv, 1, 0 );
	assertArrayClose( a, tc.a, 1e-14, 'a' );
	assert.equal( info, tc.info, 'info' );
...
test( 'dgetrf2: 3x4 wide matrix', function t() {
	...
	const tc = _3x4;
	const a = new Float64Array( [ 1, 4, 7, 2, 5, 8, 3, 6, 9, 10, 11, 12 ] );
	const ipiv = new Int32Array( 3 );
	const info = dgetrf2( 3, 4, a, 1, 3, 0, ipiv, 1, 0 );
	assertArrayClose( a, tc.a, 1e-14, 'a' );
	assert.equal( info, tc.info, 'info' );
	assert.deepEqual( ipiv, ipivTo0Based( tc.ipiv ), 'ipiv' );
});
...
test( 'dgetrf2: singular matrix (info > 0)', function t() {
	...
	const tc = singular;
	const a = new Float64Array( [ 1, 0, 0, 0, 0, 0, 0, 0, 1 ] );
	const ipiv = new Int32Array( 3 );
	const info = dgetrf2( 3, 3, a, 1, 3, 0, ipiv, 1, 0 );
	assertArrayClose( a, tc.a, 1e-14, 'a' );
...
test( 'dgetrf2: N=0 quick return', function t() {
	...
	const info = dgetrf2( 3, 0, a, 1, 3, 0, ipiv, 1, 0 );
	assert.equal( info, 0 );
});
...
test( 'dgetrf2: M=0 quick return', function t() {
	...
	const ipiv = new Int32Array( 1 );
	const info = dgetrf2( 0, 3, a, 1, 1, 0, ipiv, 1, 0 );
	assert.equal( info, 0 );
});
...
test( 'dgetrf2: 1x1 non-singular', function t() {
	...
	const ipiv = new Int32Array( 1 );
	const info = dgetrf2( 1, 1, a, 1, 1, 0, ipiv, 1, 0 );
	assertArrayClose( a, tc.a, 1e-14, 'a' );
	assert.equal( info, tc.info, 'info' );
	assert.deepEqual( ipiv, ipivTo0Based( tc.ipiv ), 'ipiv' );
});
...
test( 'dgetrf2: Nx1 column vector', function t() {
	...
	const ipiv = new Int32Array( 1 );
	const info = dgetrf2( 3, 1, a, 1, 3, 0, ipiv, 1, 0 );
	assertArrayClose( a, tc.a, 1e-14, 'a' );
	assert.equal( info, tc.info, 'info' );
	assert.deepEqual( ipiv, ipivTo0Based( tc.ipiv ), 'ipiv' );
});
...
test( 'dgetrf2: 1xN row vector', function t() {
	...
	const tc = row_vector;
	const a = new Float64Array( [ 2, 3, 7 ] );
	const ipiv = new Int32Array( 1 );
	const info = dgetrf2( 1, 3, a, 1, 1, 0, ipiv, 1, 0 );
	assertArrayClose( a, tc.a, 1e-14, 'a' );
	assert.equal( info, tc.info, 'info' );
	assert.deepEqual( ipiv, ipivTo0Based( tc.ipiv ), 'ipiv' );
});
...
test( 'dgetrf2: Nx1 column with subnormal pivot (sfmin path)', function t() {
	...
	const a = new Float64Array( [ 5e-324, 1.0, 2.0 ] );
	const ipiv = new Int32Array( 1 );
	const info = dgetrf2( 3, 1, a, 1, 3, 0, ipiv, 1, 0 );
	assert.equal( info, 0, 'info should be 0' );
	const a2 = new Float64Array( [ 1e-310, 5e-311, 2e-311 ] );
	const ipiv2 = new Int32Array( 1 );
	const info2 = dgetrf2( 3, 1, a2, 1, 3, 0, ipiv2, 1, 0 );
