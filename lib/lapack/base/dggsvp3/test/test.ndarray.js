...
test( 'dggsvp3: basic_4x3_3x3_UVQ', function t() {
	...
	K = [ 0 ];
	l = [ 0 ];
	info = dggsvp3( 'compute-U', 'compute-V', 'compute-Q', 4, 3, 3, A, 1, 4, 0, B, 1, 3, 0, 1e-8, 1e-8, K, l, U, 1, 4, 0, V, 1, 3, 0, Q, 1, 3, 0, IWORK, 1, 0, TAU, 1, 0, WORK, 1, 0 ); // eslint-disable-line max-len
...
test( 'dggsvp3: basic_4x3_3x3_NNN', function t() {
	...
	K = [ 0 ];
	l = [ 0 ];
	info = dggsvp3( 'none', 'none', 'none', 4, 3, 3, A, 1, 4, 0, B, 1, 3, 0, 1e-8, 1e-8, K, l, U, 1, 1, 0, V, 1, 1, 0, Q, 1, 1, 0, IWORK, 1, 0, TAU, 1, 0, WORK, 1, 0 ); // eslint-disable-line max-len
	assert.equal( info, tc.info );
	assert.equal( K[ 0 ], tc.K );
	assert.equal( l[ 0 ], tc.L );
	assertArrayClose( extractMatrix( A, 0, 1, 4, 4, 3 ), tc.A, 1e-12, 'A' );
...
test( 'dggsvp3: m_zero', function t() {
	...
	K = [ 0 ];
	l = [ 0 ];
	info = dggsvp3( 'compute-U', 'compute-V', 'compute-Q', 0, 2, 2, A, 1, 1, 0, B, 1, 2, 0, 1e-8, 1e-8, K, l, U, 1, 1, 0, V, 1, 2, 0, Q, 1, 2, 0, IWORK, 1, 0, TAU, 1, 0, WORK, 1, 0 ); // eslint-disable-line max-len
	assert.equal( info, tc.info );
	assert.equal( K[ 0 ], tc.K );
	assert.equal( l[ 0 ], tc.L );
});
...
test( 'dggsvp3: n_zero', function t() {
	...
	K = [ 0 ];
	l = [ 0 ];
	info = dggsvp3( 'compute-U', 'compute-V', 'compute-Q', 3, 2, 0, A, 1, 3, 0, B, 1, 2, 0, 1e-8, 1e-8, K, l, U, 1, 3, 0, V, 1, 2, 0, Q, 1, 1, 0, IWORK, 1, 0, TAU, 1, 0, WORK, 1, 0 ); // eslint-disable-line max-len
	assert.equal( info, tc.info );
	assert.equal( K[ 0 ], tc.K );
	assert.equal( l[ 0 ], tc.L );
});
...
test( 'dggsvp3: p_zero', function t() {
	...
	K = [ 0 ];
	l = [ 0 ];
	info = dggsvp3( 'compute-U', 'compute-V', 'compute-Q', 2, 0, 2, A, 1, 2, 0, B, 1, 1, 0, 1e-8, 1e-8, K, l, U, 1, 2, 0, V, 1, 1, 0, Q, 1, 2, 0, IWORK, 1, 0, TAU, 1, 0, WORK, 1, 0 ); // eslint-disable-line max-len
	assert.equal( info, tc.info );
	assert.equal( K[ 0 ], tc.K );
	assert.equal( l[ 0 ], tc.L );
});
...
test( 'dggsvp3: rank_deficient_B', function t() {
	...
	K = [ 0 ];
	l = [ 0 ];
	info = dggsvp3( 'compute-U', 'compute-V', 'compute-Q', 3, 3, 3, A, 1, 3, 0, B, 1, 3, 0, 1e-8, 1e-8, K, l, U, 1, 3, 0, V, 1, 3, 0, Q, 1, 3, 0, IWORK, 1, 0, TAU, 1, 0, WORK, 1, 0 ); // eslint-disable-line max-len
	assert.equal( info, tc.info );
	assert.equal( K[ 0 ], tc.K );
	assert.equal( l[ 0 ], tc.L );
...
test( 'dggsvp3: wide_2x5_UVQ', function t() {
	...
	K = [ 0 ];
	l = [ 0 ];
	info = dggsvp3( 'compute-U', 'compute-V', 'compute-Q', 2, 2, 5, A, 1, 2, 0, B, 1, 2, 0, 1e-8, 1e-8, K, l, U, 1, 2, 0, V, 1, 2, 0, Q, 1, 5, 0, IWORK, 1, 0, TAU, 1, 0, WORK, 1, 0 ); // eslint-disable-line max-len
	assert.equal( info, tc.info );
	assert.equal( K[ 0 ], tc.K );
	assert.equal( l[ 0 ], tc.L );
...
test( 'dggsvp3: tall_B_3x5x3', function t() {
	...
	K = [ 0 ];
	l = [ 0 ];
	info = dggsvp3( 'compute-U', 'compute-V', 'compute-Q', 3, 5, 3, A, 1, 3, 0, B, 1, 5, 0, 1e-8, 1e-8, K, l, U, 1, 3, 0, V, 1, 5, 0, Q, 1, 3, 0, IWORK, 1, 0, TAU, 1, 0, WORK, 1, 0 ); // eslint-disable-line max-len
	assert.equal( info, tc.info );
	assert.equal( K[ 0 ], tc.K );
	assert.equal( l[ 0 ], tc.L );
	assertArrayClose( extractMatrix( A, 0, 1, 3, 3, 3 ), tc.A, 1e-12, 'A' );
