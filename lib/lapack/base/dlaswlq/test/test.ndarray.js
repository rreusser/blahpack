...
import dlaswlq from './../lib/ndarray.js';


// FIXTURES //

const fixtureDir = path.join( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures' );
const fixtureRaw = readFileSync( path.join( fixtureDir, 'dlaswlq.jsonl' ), 'utf8' ); // eslint-disable-line node/no-sync
...
function runCase( cfg ) {
	...
	const nblk = numBlocks( M, N, nb );
	const T = new Float64Array( mb * M * nblk );
	const WORK = new Float64Array( mb * M );
	const info = dlaswlq( M, N, mb, nb, A, 1, M, 0, T, 1, mb, 0, WORK, 1, 0 );
...
test( 'dlaswlq: M=0 quick return', function t() {
	...
	const info = dlaswlq( 0, 0, 2, 4, A, 1, 1, 0, T, 1, 1, 0, WORK, 1, 0 );
...
test( 'dlaswlq: M=0, N>0 quick return', function t() {
	...
	const T = new Float64Array( 0 );
	const WORK = new Float64Array( 0 );
	const info = dlaswlq( 0, 3, 1, 4, A, 1, 1, 0, T, 1, 1, 0, WORK, 1, 0 );
	assert.strictEqual( info, 0, 'returns 0' );
});
...
test( 'dlaswlq: works with non-trivial offsets', function t() {
	...
	const oT = 7;
	const oWORK = 3;
	const tc = findCase( 'm3_n8_mb2_nb4' );
	const nblk = numBlocks( M, N, nb );
	const A = new Float64Array( ( M * N ) + oA );
	const T = new Float64Array( ( mb * M * nblk ) + oT );
	const WORK = new Float64Array( ( mb * M ) + oWORK );
	...
	const info = dlaswlq( M, N, mb, nb, A, 1, M, oA, T, 1, mb, oT, WORK, 1, oWORK );
