...
test( 'zheconRook computes rcond=1 for N=0 (column-major)', function t() {
	...
	const info = zheconRook( 'column-major', 'upper', 0, new Complex128Array( 1 ), 1, new Int32Array( 1 ), 1, 0, 0.0, rcond, new Complex128Array( 2 ), 1 ); // eslint-disable-line max-len
...
test( 'zheconRook computes rcond=1 for N=0 (row-major)', function t() {
	const rcond = new Float64Array( 1 );
	const info = zheconRook( 'row-major', 'upper', 0, new Complex128Array( 1 ), 1, new Int32Array( 1 ), 1, 0, 0.0, rcond, new Complex128Array( 2 ), 1 ); // eslint-disable-line max-len
	assert.strictEqual( info, 0 );
	assert.strictEqual( rcond[ 0 ], 1.0 );
});
