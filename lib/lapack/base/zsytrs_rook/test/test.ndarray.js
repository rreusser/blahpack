...
import zsytrsRook from './../lib/ndarray.js';


// FIXTURES //

const fixtureDir = path.join( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures' ); // eslint-disable-line max-len
const lines = readFileSync( path.join( fixtureDir, 'zsytrs_rook.jsonl' ), 'utf8' ).trim().split( '\n' ); // eslint-disable-line node/no-sync, max-len
...
function runCase( uplo, tc ) {
	...
	const info = zsytrsRook( uplo, N, nrhs, A, 1, lda, 0, IPIV, 1, 0, B, 1, lda, 0 );
...
test( 'zsytrs_rook: n0', function t() {
	...
	const info = zsytrsRook( 'upper', 0, 1, A, 1, 1, 0, ipiv, 1, 0, b, 1, 1, 0 );
...
test( 'zsytrs_rook: nrhs=0 quick return', function t() {
	...
	const info = zsytrsRook( 'lower', 3, 0, A, 1, 3, 0, ipiv, 1, 0, b, 1, 3, 0 );
	assert.equal( info, 0, 'info' );
});
...
test( 'zsytrs_rook: ndarray non-zero offset (upper_4x4)', function t() {
	...
	const info = zsytrsRook( 'upper', tc.n, tc.nrhs, Ac, 1, lda, 2, IPIV, 1, 2, Bc, 1, lda, 2 );
	assert.equal( info, tc.info, 'info' );
