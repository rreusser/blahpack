...
import zsytrs2 from '../../zsytrs2/lib/base.js';
...
* @param {Complex128Array} WORK - caller-owned workspace; base.js never allocates. Minimum length from `offsetWork` is `N` (used by the zsytrs2 solve when `N > 0` and `nrhs > 0`).
...
*/
function zsysv( uplo, N, nrhs, A, strideA1, strideA2, offsetA, IPIV, strideIPIV, offsetIPIV, B, strideB1, strideB2, offsetB, WORK, strideWork, offsetWork ) {
	...
	const info = zsytrf( uplo, N, A, strideA1, strideA2, offsetA, IPIV, strideIPIV, offsetIPIV );

	if ( info === 0 ) {
		// Solve using the factorization (zsytrs2 needs an N-length workspace).
		zsytrs2( uplo, N, nrhs, A, strideA1, strideA2, offsetA, IPIV, strideIPIV, offsetIPIV, B, strideB1, strideB2, offsetB, WORK, strideWork, offsetWork );
	}
