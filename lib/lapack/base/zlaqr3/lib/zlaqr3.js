...
* @param {(Complex128Array|null)} WORK - workspace array (length `>= max(1, 2*nw)`); `null` requests internal allocation
* @param {integer} strideWork - stride length for `WORK`
* @throws {RangeError} if a numerical argument does not satisfy constraints
* @returns {Object} `{ ns, nd }`
*/
function zlaqr3( wantt, wantz, N, ktop, kbot, nw, H, LDH, iloz, ihiz, Z, LDZ, ns, nd, SH, strideSH, V, LDV, nhp, T, LDT, nvp, WV, LDWV, WORK, strideWork ) {
	...
	const swv1 = 1;
	const swv2 = LDWV;
	const osh = stride2offset( N, strideSH );
	if ( WORK === null || WORK === void 0 ) {
		WORK = new Complex128Array( max( 1, 2*nw ) );
		strideWork = 1;
	}
	...
	return base( wantt, wantz, N, ktop, kbot, nw, H, sh1, sh2, 0, iloz, ihiz, Z, sz1, sz2, 0, ns, nd, SH, strideSH, osh, V, sv1, sv2, 0, nhp, T, st1, st2, 0, nvp, WV, swv1, swv2, 0, WORK, strideWork, owork );
}
