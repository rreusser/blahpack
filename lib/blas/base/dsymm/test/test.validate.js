/**
* Property-based validation for dsymm, following the /blahpack-validate process.
*
* Step 0 classification: `d` -> real scalar; `sy` -> symmetric dense operand A
...
test( 'dsymm: C = alpha*A*B + beta*C residual (side x uplo x shape sweep)', function t() {
	[ 'left', 'right' ].forEach( function eachSide( side ) {
		[ 'upper', 'lower' ].forEach( function eachUplo( uplo ) {
			CASES.forEach( function eachCase( shape, idx ) {
				...
				checked( 'dsymm', 'residual', function run() {
					check.assertReconstruct( sc, gotC, expected, { 'label': 'dsymm '+side+' '+uplo+' '+M+'x'+N } );
				});
			});
		});
	});
...
test( 'dsymm: bit-exact across storage layouts', function t() {
	const M = 6;
	const N = 5;
	const SEED = 0xF00D;
	[ 'left', 'right' ].forEach( function eachSide( side ) {
		[ 'upper', 'lower' ].forEach( function eachUplo( uplo ) {
			...
			checked( 'dsymm', 'layout-invariance', function run() {
				...
				}, { 'label': 'dsymm '+side+' '+uplo+' layout invariance' } );
			});
		});
	});
});
