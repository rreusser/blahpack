...
import zlatsqr from './../lib/ndarray.js';


// FIXTURES //

const fixtureDir = path.join( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures' );
const fixtureRaw = readFileSync( path.join( fixtureDir, 'zlatsqr.jsonl' ), 'utf8' ); // eslint-disable-line node/no-sync
...
function runCase( cfg ) {
	...
	const info = zlatsqr( M, N, mb, nb, A, 1, M, 0, T, 1, nb, 0, WORK, 1, 0 );
...
test( 'zlatsqr: m=9, n=2, mb=4, nb=1 (single-reflector block size with TSQR)', function t() {
	runCase({
		...
		'A': buildA( 9, 2, function fill( i, j ) {
			let re = ( ( ( ( i * 5 ) + ( j * 7 ) ) % 13 ) / 4.0 ) + 0.5;
			const im = ( ( ( ( i * 3 ) + ( j * 2 ) ) % 5 ) / 6.0 ) - 0.1;
			if ( i === j ) {
				re += 4.0;
			}
			return [ re, im ];
...
test( 'zlatsqr: m=20, n=4, mb=8, nb=4 (tall blocked TSQR)', function t() {
	runCase({
		...
		'A': buildA( 20, 4, function fill( i, j ) {
			let re = ( ( ( ( i * 11 ) + ( j * 5 ) ) % 17 ) / 6.0 ) + 0.2;
			const im = ( ( ( ( i * 4 ) + ( j * 9 ) ) % 13 ) / 7.0 ) - 0.2;
			if ( i === j ) {
				re += 6.0;
			}
			return [ re, im ];
...
test( 'zlatsqr: M=0 quick return', function t() {
	...
	const info = zlatsqr( 0, 0, 4, 1, A, 1, 1, 0, T, 1, 1, 0, WORK, 1, 0 );
...
test( 'zlatsqr: N=0 quick return', function t() {
	...
	const T = new Complex128Array( 0 );
	const WORK = new Complex128Array( 0 );
	const info = zlatsqr( 3, 0, 4, 1, A, 1, 3, 0, T, 1, 1, 0, WORK, 1, 0 );
	assert.strictEqual( info, 0, 'returns 0' );
});
...
test( 'zlatsqr: works with non-trivial offsets', function t() {
	...
	const oT = 7;
	const oWORK = 3;
	const tc = findCase( 'm8_n3_mb4_nb2' );
	const nblk = numBlocks( M, N, mb );
	const A = new Complex128Array( ( M * N ) + oA );
	const T = new Complex128Array( ( nb * N * nblk ) + oT );
	const WORK = new Complex128Array( ( nb * N ) + oWORK );
	...
	const info = zlatsqr( M, N, mb, nb, A, 1, M, oA, T, 1, nb, oT, WORK, 1, oWORK );
