...
import dgetsqrhrt from './../lib/dgetsqrhrt.js';
...
test( 'dgetsqrhrt computes a QR factorization on a small matrix (column-major)', function t() {
	const A = new Float64Array( [ 2.0, 1.0, 3.0, 1.0, 1.0, 4.0, 2.0, 3.0, 3.0, 2.0, 5.0, 1.0 ] ); // eslint-disable-line max-len
	const T = new Float64Array( 6 );
	const W = new Float64Array( 256 );
	const info = dgetsqrhrt( 'column-major', 4, 3, 4, 2, 2, A, 4, T, 2, W );
...
test( 'dgetsqrhrt computes a QR factorization on a small matrix (row-major)', function t() {
	...
	const A = new Float64Array( [ 2.0, 1.0, 3.0, 1.0, 4.0, 2.0, 3.0, 2.0, 5.0, 1.0, 3.0, 1.0 ] ); // eslint-disable-line max-len
	const T = new Float64Array( 6 );
	const W = new Float64Array( 256 );
	const info = dgetsqrhrt( 'row-major', 4, 3, 4, 2, 2, A, 3, T, 3, W );
	assert.strictEqual( info, 0, 'INFO returned 0' );
});
