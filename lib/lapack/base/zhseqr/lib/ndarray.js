...
* @param {Complex128Array} WORK - caller-provided workspace array (minimum `max(1,N)` complex elements from `offsetWork`; larger improves performance via zlaqr0)
...
*/
function zhseqr( job, compz, N, ilo, ihi, H, strideH1, strideH2, offsetH, w, strideW, offsetW, Z, strideZ1, strideZ2, offsetZ, WORK, strideWork, offsetWork ) { // eslint-disable-line max-len, max-params
	...
	// Zero-dimension quick return: an empty matrix must not require a workspace
	// buffer, so this precedes the WORK size assertion.
	if ( N === 0 ) {
		return 0;
	}
	// Caller owns WORK; assert it is large enough so an under-sized buffer is a
	// loud RangeError, not a silent NaN from an out-of-bounds read. The minimum
	// is max(1,N) complex elements (the small-N embedded path uses a fixed
	// NL-element internal scratch buffer, so the caller only needs N here).
	...
	return base( job, compz, N, ilo, ihi, H, strideH1, strideH2, offsetH, w, strideW, offsetW, Z, strideZ1, strideZ2, offsetZ, WORK, strideWork, offsetWork ); // eslint-disable-line max-len
}
