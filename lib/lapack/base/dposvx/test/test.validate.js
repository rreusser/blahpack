/**
* Property-based validation for dposvx, following the /blahpack-validate process.
*
...
* proven for its constituent routines (dposv/dporfs residual, dpocon rcond,
* dporfs FERR/BERR). We drive dposvx with fact='not-factored' (factor A in place,
* NO equilibration -> equed='none', so X solves the ORIGINAL A0*X=B0 directly)
* and assert, against the ORIGINAL full symmetric A0:
...
*       reciprocal condition number (anorm from A0, ‖A0⁻¹‖₁ from a trusted dposv
...
*       trusted dposv.
* Only the uplo triangle of A is realized; the opposite stays poisoned, so a read
* of the wrong triangle trips a NaN. Note this file EXERCISES the documented
* ndarray signature (explicit `rcond` out-array after offsetX) which base.js had
* silently failed to implement — see test/harness/LEARNINGS.md 2026-07-18.
*/
...
function invNorm1( A0, n, uplo ) {
	...
	var info = dposv( uplo, n, n, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], Br.data, Br.args[ 0 ], Br.args[ 1 ], Br.args[ 2 ] );
	if ( info !== 0 ) {
...
function trueSolution( A0, B0, N, nrhs, uplo ) {
	...
	var info = dposv( uplo, N, nrhs, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], Br.data, Br.args[ 0 ], Br.args[ 1 ], Br.args[ 2 ] );
	if ( info !== 0 ) {
		throw new Error( 'oracle dposv failed (info='+info+'); matrix not SPD?' );
	}
	return Br;
...
// Drive dposvx (fact='not-factored', no equilibration) and return the physical
// X reader plus the rcond/FERR/BERR outputs.
function drive( uplo, N, nrhs, A0, B0, layout ) {
	var Ar = schemes.dense.realize( sc, A0, { 'part': uplo }, layout );
	var AFr = schemes.dense.realize( sc, A0, { 'part': uplo }, layout ); // overwritten by dlacpy+dpotrf
	var Br = schemes.dense.realize( sc, B0, { 'part': 'full' }, layout );
	var Xr = schemes.dense.realize( sc, B0, { 'part': 'full' }, layout ); // overwritten by dlacpy+dpotrs
	...
	var out = dposvx( 'not-factored', uplo, N, nrhs, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], AFr.data, AFr.args[ 0 ], AFr.args[ 1 ], AFr.args[ 2 ], 'none', s, 1, 0, Br.data, Br.args[ 0 ], Br.args[ 1 ], Br.args[ 2 ], Xr.data, Xr.args[ 0 ], Xr.args[ 1 ], Xr.args[ 2 ], rcond, FERR, 1, 0, BERR, 1, 0, WORK, 1, 0, IWORK, 1, 0 );
	return {
		'Xr': Xr,
		'rcond': rcond,
		'FERR': FERR,
...
// and nrhs.
test( 'dposvx: expert-driver residual + rcond + FERR/BERR (uplo x N x nrhs)', function t() {
...
function runProperty( uplo, N, nrhs ) {
	...
	var A0 = logical.positiveDefinite( sc, rng, N ); // full symmetric/SPD oracle
	var B0 = logical.general( sc, rng, N, nrhs );
	var anorm = norm1Full( A0, N );

	var d = drive( uplo, N, nrhs, A0, B0, TIGHT );
	var tag = 'dposvx '+uplo+' N='+N+' nrhs='+nrhs;

	if ( d.result.info !== 0 ) {
		throw new Error( tag+': dposvx returned info='+d.result.info+' (expected 0 for well-conditioned SPD input)' );
	}
	if ( d.result.equed !== 'none' ) {
		throw new Error( tag+': expected equed=none for fact=not-factored, got '+d.result.equed );
	}

	// (a) residual: X solves the ORIGINAL A0*X = B0 (no equilibration).
	checked( 'dposvx', 'residual', function run() {
		var j;
		for ( j = 0; j < nrhs; j++ ) {
	...
	checked( 'dposvx', 'property', function run() {
		var trueRcond = 1.0 / ( anorm * invNorm1( A0, N, uplo ) );
		assertRcond( d.rcond[ 0 ], trueRcond, tag );
		// The driver echoes rcond in the return object too:
		if ( d.result.rcond !== d.rcond[ 0 ] ) {
			throw new Error( tag+': return-object rcond '+d.result.rcond+' disagrees with out-array rcond '+d.rcond[ 0 ] );
		}
	});

	// (c) BERR tiny and >= 0; FERR a valid, not-absurdly-loose forward-error bound.
	checked( 'dposvx', 'structural', function run() {
...
// Step 4: layout-invariance fuzz. dposvx composes a Cholesky factor + BLAS solves
// + condition estimate + refinement; a storage-order flip can legitimately reorder
// those fast paths, so bit-equality is asserted within a single storage-order
// family (col / row split, as for dposv/dporfs). Cross-order correctness is
// certified by the swept residual above. All values are identical across variants
// (same seed), so only ADDRESSING changes: the flattened X ++ rcond ++ FERR ++ BERR
...
test( 'dposvx: bit-exact within storage-order family (col / row)', function t() {
	UPLOS.forEach( function eachUplo( uplo ) {
...
function runInvariance( uplo, variants, fam ) {
	...
	var nrhs = 2;
	var SEED = 0xBEEF;
	checked( 'dposvx', 'layout-invariance', function run() {
		layoutInvariant( variants, function build( layout ) {
			...
			return out;
		}, { 'label': 'dposvx '+uplo+' layout invariance '+fam+'-major' } );
	});
}
