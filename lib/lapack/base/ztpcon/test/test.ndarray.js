...
function packUpperFromReal( diagAndUpper, N ) {
	const AP = new Complex128Array( N * (N+1) / 2 );
	const view = reinterpret( AP, 0 );
	let k = 0;
	let j, i;
	for ( j = 0; j < N; j++ ) {
		for ( i = 0; i <= j; i++ ) {
...
test( 'ztpcon: N=0 quick return (rcond=1)', function t() {
	const rcond = new Float64Array( 1 );
	rcond[ 0 ] = 0.5;
	const info = ztpcon( 'one-norm', 'upper', 'non-unit', 0, new Complex128Array( 0 ), 1, 0, rcond, new Complex128Array( 0 ), 1, 0, new Float64Array( 0 ), 1, 0 );
...
test( 'ztpcon: zero matrix returns rcond=0', function t() {
	...
	const info = ztpcon( 'one-norm', 'upper', 'non-unit', N, AP, 1, 0, rcond, work, 1, 0, rwork, 1, 0 );
	assert.strictEqual( info, 0 );
...
test( 'ztpcon: identity 3x3 upper non-unit (one-norm)', function t() {
	...
	const AP = packUpperFromReal( A, N );
	const rcond = new Float64Array( 1 );
	const work = new Complex128Array( 2 * N );
	const rwork = new Float64Array( N );
	const info = ztpcon( 'one-norm', 'upper', 'non-unit', N, AP, 1, 0, rcond, work, 1, 0, rwork, 1, 0 );
...
test( 'ztpcon: identity 3x3 upper non-unit (inf-norm)', function t() {
	...
	const work = new Complex128Array( 2 * N );
	const rwork = new Float64Array( N );
	const info = ztpcon( 'inf-norm', 'upper', 'non-unit', N, AP, 1, 0, rcond, work, 1, 0, rwork, 1, 0 );
	assert.strictEqual( info, 0 );
	assert.ok( close( rcond[ 0 ], 1.0, 1e-9 ) );
});
...
test( 'ztpcon: upper unit 4x4 (one-norm)', function t() {
	...
	const work = new Complex128Array( 2 * N );
	const rwork = new Float64Array( N );
	const info = ztpcon( 'one-norm', 'upper', 'unit', N, AP, 1, 0, rcond, work, 1, 0, rwork, 1, 0 );
	assert.strictEqual( info, 0 );
...
test( 'ztpcon: upper unit 4x4 (inf-norm)', function t() {
	...
	const work = new Complex128Array( 2 * N );
	const rwork = new Float64Array( N );
	const info = ztpcon( 'inf-norm', 'upper', 'unit', N, AP, 1, 0, rcond, work, 1, 0, rwork, 1, 0 );
	assert.strictEqual( info, 0 );
	assert.ok( rcond[ 0 ] > 0.0 && rcond[ 0 ] <= 1.0 );
});
...
test( 'ztpcon: lower non-unit diagonal 4x4 (one-norm)', function t() {
	...
	const AP = packLowerFromReal( A, N );
	const rcond = new Float64Array( 1 );
	const work = new Complex128Array( 2 * N );
	const rwork = new Float64Array( N );
	const info = ztpcon( 'one-norm', 'lower', 'non-unit', N, AP, 1, 0, rcond, work, 1, 0, rwork, 1, 0 );
	assert.strictEqual( info, 0 );
...
test( 'ztpcon: lower non-unit diagonal 4x4 (inf-norm)', function t() {
	...
	const work = new Complex128Array( 2 * N );
	const rwork = new Float64Array( N );
	const info = ztpcon( 'inf-norm', 'lower', 'non-unit', N, AP, 1, 0, rcond, work, 1, 0, rwork, 1, 0 );
	assert.strictEqual( info, 0 );
	assert.ok( close( rcond[ 0 ], 0.25, 1e-9 ) );
});
...
test( 'ztpcon: lower unit 4x4 (one-norm)', function t() {
	...
	const work = new Complex128Array( 2 * N );
	const rwork = new Float64Array( N );
	const info = ztpcon( 'one-norm', 'lower', 'unit', N, AP, 1, 0, rcond, work, 1, 0, rwork, 1, 0 );
	assert.strictEqual( info, 0 );
	assert.ok( rcond[ 0 ] > 0.0 && rcond[ 0 ] <= 1.0 );
});
...
test( 'ztpcon: lower unit 4x4 (inf-norm)', function t() {
	...
	const work = new Complex128Array( 2 * N );
	const rwork = new Float64Array( N );
	const info = ztpcon( 'inf-norm', 'lower', 'unit', N, AP, 1, 0, rcond, work, 1, 0, rwork, 1, 0 );
	assert.strictEqual( info, 0 );
	assert.ok( rcond[ 0 ] > 0.0 && rcond[ 0 ] <= 1.0 );
});
