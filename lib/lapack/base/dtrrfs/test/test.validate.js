...
function solve( A0, B0, N, nrhs, uplo, trans, diag, unit ) {
	...
	const info = dtrtrs( uplo, trans, diag, N, nrhs, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], Xr.data, Xr.args[ 0 ], Xr.args[ 1 ], Xr.args[ 2 ] );
	if ( info !== 0 ) {
...
function runProperty( uplo, trans, code, diag, N, nrhs ) {
	...
	const tag = 'dtrrfs '+uplo+' '+trans+' '+diag+' N='+N+' nrhs='+nrhs;
