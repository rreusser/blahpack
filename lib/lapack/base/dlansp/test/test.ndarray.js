...
import dlansp from './../lib/ndarray.js';


// FIXTURES //

const fixtureFile = path.join( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures', 'dlansp.jsonl' );
...
// 3x3 packed:
const DATA_3x3_U = [ 2.0, 3.0, 5.0, -1.0, 2.0, 7.0 ];
const DATA_3x3_L = [ 2.0, 3.0, -1.0, 5.0, 2.0, 7.0 ];

// 4x4 packed:
const DATA_4x4_U = [ 2.0, 3.0, 5.0, -1.0, 2.0, 7.0, 4.0, -6.0, 1.0, 8.0 ];
const DATA_4x4_L = [ 2.0, 3.0, -1.0, 4.0, 5.0, 2.0, -6.0, 7.0, 1.0, 8.0 ];
...
FIXTURES.forEach( function build( fx ) {
	test( 'dlansp: ' + fx.name, function t() {
		...
		const AP = new Float64Array( args.data );
		const WORK = new Float64Array( Math.max( args.N, 1 ) );
		const result = dlansp( args.norm, args.uplo, args.N, AP, 1, 0, WORK, 1, 0 );
