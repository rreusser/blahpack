...
import zlanht from './../lib/ndarray.js';


// FIXTURES //

const fixtureFile = path.join( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures', 'zlanht.jsonl' );
...
const D_N1 = new Float64Array( [ -3.0 ] );
const E_EMPTY = new Complex128Array( 0 );

// N=2: d = [3, -4], e = [(1+2i)]
const D_N2 = new Float64Array( [ 3.0, -4.0 ] );
const E_N2 = buildCArr( [ 1.0, 2.0 ] );

// N=5: d = [2, -4, 6, -1, 3], e = [(1+2i), (-2+3i), (3-1i), (5+4i)]
const D_N5 = new Float64Array( [ 2.0, -4.0, 6.0, -1.0, 3.0 ] );
const E_N5 = buildCArr( [ 1.0, 2.0, -2.0, 3.0, 3.0, -1.0, 5.0, 4.0 ] );

// N=4: d = [10, -20, 30, -40], e = [(5+6i), (7+8i), (9+10i)]
const D_N4 = new Float64Array( [ 10.0, -20.0, 30.0, -40.0 ] );
const E_N4 = buildCArr( [ 5.0, 6.0, 7.0, 8.0, 9.0, 10.0 ] );
...
function runCase( name, norm, N, D, E ) {
	...
	const got = zlanht( norm, N, D, 1, 0, E, 1, 0 );
