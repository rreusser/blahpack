...
function runReconstruct( uplo, n, layout ) {
	...
	var work = poisonedWork( sc, Math.max( n, 1 ) ); // poisoned scratch (len N)

	var info = dsytrf( uplo, n, R.data, R.args[ 0 ], R.args[ 1 ], R.args[ 2 ], ipiv, 1, 0 );
	if ( info !== 0 ) {
...
function runInvariance( uplo, n ) {
	...
	var info = dsytrf( uplo, n, Rf.data, Rf.args[ 0 ], Rf.args[ 1 ], Rf.args[ 2 ], ipiv0, 1, 0 );
	if ( info !== 0 ) {
	...
	checked( 'dsytri', 'layout-invariance', function run() {
		layoutInvariant( PURE_ADDR, function build( layout ) {
			...
			var code = dsytri( uplo, n, R.data, R.args[ 0 ], R.args[ 1 ], R.args[ 2 ], ipiv, 1, 0, work, 1, 0 );
			if ( code !== 0 ) {
...
test( 'dsytri: workspace minimum (len N) suffices with a poisoned buffer', function t() {
	...
	checked( 'dsytri', 'workspace', function check_() {
		var minLen = assertWorkspaceSufficient( run, {}, 'dsytri WORK@N=33 upper' );
		if ( minLen !== N ) {
