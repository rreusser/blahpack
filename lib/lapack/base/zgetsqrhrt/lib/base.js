...
function zgetsqrhrt( M, N, mb1, nb1, nb2, A, strideA1, strideA2, offsetA, T, strideT1, strideT2, offsetT ) {
	...
	const lw1 = nb1local * N;
	const lw2 = nb1local * Math.max( nb1local, N - nb1local );

	const Twork = new Complex128Array( lwt );
	const WORK = new Complex128Array( ( lw1 > lw2 ) ? lw1 : lw2 );
	const Rwork = new Complex128Array( N * N );
