...
*/
function zhesv( uplo, N, nrhs, A, strideA1, strideA2, offsetA, IPIV, strideIPIV, offsetIPIV, B, strideB1, strideB2, offsetB, WORK, strideWork, offsetWork ) { // eslint-disable-line max-len, max-params
	var need;
	...
	// Caller owns WORK. base() selects zhetrs (no workspace) vs. the faster
	// zhetrs2 (needs N) from `WORK.length`, so the hard minimum is a single
	// element (the reference requires `LWORK >= 1`); N === 0 quick-returns
