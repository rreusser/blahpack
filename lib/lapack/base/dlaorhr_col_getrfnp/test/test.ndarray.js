...
import dlaorhr_col_getrfnp from './../lib/ndarray.js';
const ndarrayFn = dlaorhr_col_getrfnp;


// FIXTURES //

const fixtureDir = path.join( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures' );
const fixturePath = path.join( fixtureDir, 'dlaorhr_col_getrfnp.jsonl' );
...
function runCase( name, M, N, input, dlen ) {
	...
	const d = new Float64Array( dlen );
	const info = dlaorhr_col_getrfnp( M, N, a, 1, Math.max( 1, M ), 0, d, 1, 0 );
...
test( 'dlaorhr_col_getrfnp: 20x20 (matches Fortran sin-built input)', function t() {
	let i, j;
	const tc = findCase( '20x20' );
	const a = new Float64Array( 400 );
	for ( j = 1; j <= 20; j++ ) {
		for ( i = 1; i <= 20; i++ ) {
	...
	const d = new Float64Array( 20 );
	const info = dlaorhr_col_getrfnp( 20, 20, a, 1, 20, 0, d, 1, 0 );
	assert.equal( info, tc.info, 'info' );
	assertArrayClose( a, tc.a, 1e-12, 'a' );
	assertArrayClose( d, tc.d, 1e-14, 'd' );
});
...
test( 'dlaorhr_col_getrfnp: M=0 quick return', function t() {
	...
	const info = dlaorhr_col_getrfnp( 0, 3, a, 1, 1, 0, d, 1, 0 );
...
test( 'dlaorhr_col_getrfnp: N=0 quick return', function t() {
	const a = new Float64Array( [ 1.0, 2.0, 3.0 ] );
	const d = new Float64Array( 1 );
	const info = dlaorhr_col_getrfnp( 3, 0, a, 1, 3, 0, d, 1, 0 );
	assert.equal( info, 0, 'info' );
	assert.equal( a[ 0 ], 1.0, 'a unchanged' );
});
...
test( 'dlaorhr_col_getrfnp: blocked path with N>NB (40x40)', function t() {
	...
	const d = new Float64Array( 40 );
	const info = dlaorhr_col_getrfnp( 40, 40, a, 1, 40, 0, d, 1, 0 );
	assert.equal( info, 0, 'info' );
