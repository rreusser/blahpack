...
test( 'ztfttr: N=0 quick return', function t() {
	...
	var info = ztfttr( 'no-transpose', 'lower', 0, ARF, 1, 0, A, 1, 1, 0 );
...
test( 'ztfttr: N=1 normal (no conjugation)', function t() {
	...
	var A = new Complex128Array( 1 );
	var Av;
	var info = ztfttr( 'no-transpose', 'lower', 1, ARF, 1, 0, A, 1, 1, 0 );
	assert.equal( info, tc.info );
	Av = reinterpret( A, 0 );
...
test( 'ztfttr: N=1 conjugate-transpose', function t() {
	...
	var A = new Complex128Array( 1 );
	var Av;
	var info = ztfttr( 'conjugate-transpose', 'lower', 1, ARF, 1, 0, A, 1, 1, 0 );
	assert.equal( info, tc.info );
	Av = reinterpret( A, 0 );
	assertArrayClose( Array.from( Av ), tc.A, 1e-14, 'A' );
});
...
test( 'ztfttr: N=5, TRANSR=no-transpose, UPLO=lower (odd, normal, lower)', function t() {
	...
	var Av;
	var info = ztfttr( 'no-transpose', 'lower', N, ARF, 1, 0, A, 1, N, 0 );
	assert.equal( info, tc.info );
	Av = reinterpret( A, 0 );
	assertArrayClose( Array.from( Av ), tc.A, 1e-14, 'A' );
});
...
test( 'ztfttr: N=5, TRANSR=no-transpose, UPLO=upper (odd, normal, upper)', function t() {
	...
	var A = new Complex128Array( N * N );
	var Av;
	var info = ztfttr( 'no-transpose', 'upper', N, ARF, 1, 0, A, 1, N, 0 );
	assert.equal( info, tc.info );
	Av = reinterpret( A, 0 );
	assertArrayClose( Array.from( Av ), tc.A, 1e-14, 'A' );
});
...
test( 'ztfttr: N=5, TRANSR=conjugate-transpose, UPLO=lower (odd, conj-trans, lower)', function t() {
	...
	var A = new Complex128Array( N * N );
	var Av;
	var info = ztfttr( 'conjugate-transpose', 'lower', N, ARF, 1, 0, A, 1, N, 0 );
	assert.equal( info, tc.info );
	Av = reinterpret( A, 0 );
	assertArrayClose( Array.from( Av ), tc.A, 1e-14, 'A' );
});
...
test( 'ztfttr: N=5, TRANSR=conjugate-transpose, UPLO=upper (odd, conj-trans, upper)', function t() {
	...
	var A = new Complex128Array( N * N );
	var Av;
	var info = ztfttr( 'conjugate-transpose', 'upper', N, ARF, 1, 0, A, 1, N, 0 );
	assert.equal( info, tc.info );
	Av = reinterpret( A, 0 );
	assertArrayClose( Array.from( Av ), tc.A, 1e-14, 'A' );
});
