...
import zlanhe from './../lib/ndarray.js';


// FIXTURES //

const fixtureFile = path.join( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures', 'zlanhe.jsonl' );
...
function parseName( name ) {
	const parts = name.split( '_' ); // [zlanhe, ...]
	const rest = parts.slice( 1 );
	let N, uplo, norm, entries;

	if ( rest[ 0 ] === 'n' && rest[ 1 ] === 'zero' ) {
		return { 'N': 0, 'uplo': 'upper', 'norm': 'max', 'entries': []};
	}

	const sizeOrNorm = rest[ 0 ];
...
FIXTURES.forEach( function build( fx ) {
	test( 'zlanhe: ' + fx.name, function t() {
		...
		const N = args.N;
		const A = ( N === 0 ) ? new Complex128Array( 0 ) : buildMatrix( N, args.entries );
		const WORK = new Float64Array( Math.max( N, 1 ) );

		// strideA1 = 1, strideA2 = N (column-major, lda = N)
		const result = zlanhe( args.norm, args.uplo, N, A, 1, Math.max( N, 1 ), 0, WORK, 1, 0 );
