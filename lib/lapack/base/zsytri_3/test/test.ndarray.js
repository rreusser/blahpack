...
function runFixture( uplo, N, tc ) {
	...
	info = zsytri3( uplo, N, A, 1, LDA, 0, e, 1, 0, ipiv, 1, 0, work, 1, 0 ); // eslint-disable-line max-len
...
test( 'zsytri_3: N=1 lower trivial inverse', function t() {
	...
	lwork = ( 1 + NB + 1 ) * ( NB + 3 );
	work = new Complex128Array( lwork );
	info = zsytri3( 'lower', 1, A, 1, 1, 0, e, 1, 0, ipiv, 1, 0, work, 1, 0 ); // eslint-disable-line max-len
	assert.equal( info, tc.info, 'info' );
...
test( 'zsytri_3: N=0 quick return', function t() {
	...
	info = zsytri3( 'lower', 0, A, 1, 1, 0, e, 1, 0, ipiv, 1, 0, work, 1, 0 ); // eslint-disable-line max-len
