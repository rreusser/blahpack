...
* @param {(Complex128Array|null)} WORK - complex workspace; auto-allocated when `null`
* @param {integer} strideWork - stride length for `WORK`
* @param {(Int32Array|null)} IWORK - integer workspace; auto-allocated when `null`
...
*/
function ztgsna( order, job, howmny, SELECT, strideSELECT, N, A, LDA, B, LDB, VL, LDVL, VR, LDVR, s, strideS, DIF, strideDIF, mm, M, WORK, strideWork, IWORK, strideIWork, offsetIWork ) { // eslint-disable-line max-len, max-params
	let svl1, svl2, svr1, svr2, sa1, sa2, sb1, sb2;
	...
	if ( WORK === null || WORK === void 0 ) {
		const minWork = ( wantdf && N > 1 ) ? ( ( 2 * N * N ) + Math.max( 1, 4 * ( N - 1 ) ) ) : Math.max( 1, N );
		WORK = new Complex128Array( minWork );
		strideWork = 1;
	}
	if ( IWORK === null || IWORK === void 0 ) {
		const minIwork = Math.max( 1, N + 6 );
		IWORK = new Int32Array( minIwork );
		strideIWork = 1;
	}
	return base( job, howmny, SELECT, strideSELECT, 0, N, A, sa1, sa2, 0, B, sb1, sb2, 0, VL, svl1, svl2, 0, VR, svr1, svr2, 0, s, strideS, 0, DIF, strideDIF, 0, mm, M, WORK, strideWork, 0, IWORK, strideIWork, offsetIWork ); // eslint-disable-line max-len
}
