...
function zungtr( uplo, N, A, strideA1, strideA2, offsetA, TAU, strideTAU, offsetTAU, WORK, strideWork, offsetWork, lwork ) { // eslint-disable-line max-len, max-params
	...
	if ( N > 1 && ( !WORK || ( WORK.length - offsetWork ) < ( N - 1 ) ) ) {
		throw new RangeError( format( 'invalid argument. WORK array must have at least %d elements from offset %d. Provided length: %d.', N - 1, offsetWork, ( WORK ) ? WORK.length : 0 ) );
	}
