...
* @param {(Float64Array|null)} RWORK - real workspace of at least `N` elements; if `null`, allocated internally
* @param {integer} strideRWork - strideRWork
* @throws {TypeError} if a string argument is not a valid option
* @throws {RangeError} if a numerical argument does not satisfy constraints
* @returns {integer} info status code
*/
function zhesvx( fact, uplo, N, nrhs, A, LDA, AF, LDAF, IPIV, strideIPIV, B, LDB, X, LDX, rcond, FERR, strideFERR, BERR, strideBERR, WORK, strideWork, RWORK, strideRWork ) { // eslint-disable-line max-len, max-params
	...
	// The wrapper is the ONLY allocation site: WORK is complex (max(1,2*N)),
	// RWORK is real (N). base.js and ndarray.js never allocate.
