...
function factor( M, N, layout, wlen, poison ) {
	...
	const work = ( poison ) ? poisonedWork( sc, wlen ) : schemes.realizeVector( sc, new Array( wlen ).fill( sc.zero ), { 'stride': 1, 'lead': 0, 'tail': 0 } ).data;
...
function runInvariance( variants, fam, M, N ) {
	...
	checked( ROUTINE, 'layout-invariance', function run() {
		layoutInvariant( variants, function build( layout, i ) {
			...
			const A0 = logical.general( sc, rng, M, N );
			const R = schemes.dense.realize( sc, A0, { 'part': 'full' }, layout );
			const TAU = schemes.realizeVector( sc, new Array( k ).fill( sc.zero ), TAULAYOUTS[ i % TAULAYOUTS.length ] );
