...
function zlaic1( job, j, x, strideX, offsetX, sest, w, strideW, offsetW, gamma, sestpr, s, c ) {
	let cosineR, cosineI, sineR, sineI, norma, zeta1, zeta2, test, tmp, scl, cr;
	let s1, s2, tR, tI, b, t;

	const gammaR = real( gamma );
	const gammaI = imag( gamma );

	// alpha = zdotc( j, x, 1, w, 1 ) — conjugate dot product
	const alpha = zdotc( j, x, strideX, offsetX, w, strideW, offsetW );
	const alphaR = real( alpha );
	const alphaI = imag( alpha );

	const absalp = cmplx.abs( alpha );
	const absgam = cmplx.abs( gamma );
	const absest = abs( sest );
