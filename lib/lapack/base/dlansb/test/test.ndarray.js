...
import dlansb from './../lib/ndarray.js';


// FIXTURES //

const fixtureFile = path.join( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures', 'dlansb.jsonl' );
...
function buildAB( ldab, ncols, cells ) {
	const out = new Float64Array( ldab * ncols );
	let i;
...
// 1x1 K=0
const AB_1X1 = buildAB( LDAB, 1, [ 1, 1, 5.0 ] );
...
function runCase( name, norm, uplo, N, K, AB ) {
	...
	const got = dlansb( norm, uplo, N, K, AB, 1, LDAB, 0, WORK, 1, 0 );
