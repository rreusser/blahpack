...
import zlar2v from '../../zlar2v/lib/base.js';
import zlargv from '../../zlargv/lib/base.js';
import zlartg from '../../zlartg/lib/base.js';
import zlartv from '../../zlartv/lib/base.js';
...
const tempArr = new Complex128Array( 1 ); // scratch for zlartg result r
const tempv = reinterpret( tempArr, 0 );
const sn = new Float64Array( 2 );        // scratch for zrot sine argument
const conjSn = new Float64Array( 2 );    // scratch for conjugated sine
...
function zhbtrd( vect, uplo, N, kd, AB, strideAB1, strideAB2, offsetAB, d, strideD, offsetD, e, strideE, offsetE, Q, strideQ1, strideQ2, offsetQ, WORK, strideWork, offsetWork ) {
	let iqaend, j1end, iqend, jend, jinc, last, lend, abst, ibl, iqb, nrt, nr;
	let nq, i2, j1, j2, tr, ti, wr, wi, i, j, k, l, p;

	const initq = ( vect === 'initialize' );
	const wantq = initq || ( vect === 'update' );
	const upper = ( uplo === 'upper' );
	const kd1 = kd + 1;
	const kdm1 = kd - 1;
	const incx = strideAB2 - strideAB1; // Fortran: LDAB - 1 (in complex elements)
	...
	const inca = kd1 * strideAB2;

	const kdn = Math.min( N - 1, kd );
