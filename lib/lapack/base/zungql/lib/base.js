...
import zung2l from '../../zung2l/lib/base.js';
...
const NB = 32;  // Block size (LAPACK default for ZUNGQL)
...
* block-reflector T factor and the zlarfb scratch.
*
* @private
...
* @param {Complex128Array} WORK - caller-owned workspace of at least `N*NB` elements (`NB = 32`)
...
function zungql( M, N, K, A, strideA1, strideA2, offsetA, TAU, strideTAU, offsetTAU, WORK, strideWork, offsetWork ) {
	...
	if ( kk > 0 ) {
		...
		// zlarfb scratch (offset ib), leading dimension ldwork = N.
		...
		for ( i = K - kk; i < K; i += nb ) {
			...
			if ( N - K + i > 0 ) {
				...
				zlarft(
					...
					WORK, strideWork, ldwork, offsetWork
				);
				...
				zlarfb(
					...
					WORK, strideWork, ldwork, offsetWork,
					A, strideA1, strideA2, offsetA,
					WORK, strideWork, ldwork, offsetWork + ib
				);
			}
