...
import zsyconvf from './../lib/ndarray.js';


// FIXTURES //

const fixtureDir = path.join( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures' );
const raw = fs.readFileSync( path.join( fixtureDir, 'zsyconvf.jsonl' ), 'utf8' ); // eslint-disable-line node/no-sync
...
function assertIPIVEq( jsIPIV, fortranIPIV, msg ) {
	const expected = convertIPIV( fortranIPIV );
	let i;
...
function runConvert( uplo, tc, N ) {
	...
	const info = zsyconvf( uplo, 'convert', N, A, 1, N, 0, E, 1, 0, IPIV, 1, 0 );
...
function runRevert( uplo, tcConv, tcRev, N ) {
	...
	const IPIV = convertIPIV( tcConv.ipiv_conv );
	const E = new Complex128Array( tcConv.e );
	const info = zsyconvf( uplo, 'revert', N, A, 1, N, 0, E, 1, 0, IPIV, 1, 0 );
	assert.equal( info, 0, 'info' );
...
test( 'zsyconvf: n1_upper', function t() {
	...
	const info = zsyconvf( 'upper', 'convert', 1, A, 1, 1, 0, E, 1, 0, IPIV, 1, 0 );
	assert.equal( info, 0, 'info' );
	assertArrayClose( toArray( reinterpret( A, 0 ) ), tc.a_converted, 1e-14, 'a_converted' );
	assertArrayClose( toArray( reinterpret( E, 0 ) ), tc.e, 1e-14, 'e' );
});
...
test( 'zsyconvf: n1_lower', function t() {
	...
	const IPIV = convertIPIV( tc.ipiv );
	const E = new Complex128Array( 1 );
	const info = zsyconvf( 'lower', 'convert', 1, A, 1, 1, 0, E, 1, 0, IPIV, 1, 0 );
	assert.equal( info, 0, 'info' );
	assertArrayClose( toArray( reinterpret( A, 0 ) ), tc.a_converted, 1e-14, 'a_converted' );
	assertArrayClose( toArray( reinterpret( E, 0 ) ), tc.e, 1e-14, 'e' );
});
...
test( 'zsyconvf: quick return N=0', function t() {
	...
	const info = zsyconvf( 'upper', 'convert', 0, A, 1, 1, 0, E, 1, 0, IPIV, 1, 0 );
	assert.equal( info, 0, 'info' );
});
...
test( 'zsyconvf: upper round-trip with artificial 1x1 swap', function t() {
	...
	const IPIV = new Int32Array( [ 0, 0, 1, 3 ] );
	const N = 4;
	const A0 = new Complex128Array( 16 );
	const A = new Complex128Array( 16 );
	const E = new Complex128Array( 4 );
...
test( 'zsyconvf: upper 2x2 pivot at non-trailing position with swap', function t() {
	...
	const info = zsyconvf( 'upper', 'convert', N, A, 1, N, 0, E, 1, 0, IPIV, 1, 0 );
	assert.equal( info, 0, 'convert info' );
});
...
test( 'zsyconvf: lower 2x2 pivot at non-leading position with swap', function t() {
	...
	const info = zsyconvf( 'lower', 'convert', N, A, 1, N, 0, E, 1, 0, IPIV, 1, 0 );
	assert.equal( info, 0, 'convert info' );
});
...
test( 'zsyconvf: lower revert 2x2 pivot at non-leading position', function t() {
	...
	const IPIV = new Int32Array( [ 0, 1, 2, -1 ] );
	const N = 4;
	const A = new Complex128Array( 16 );
	const E = new Complex128Array( 4 );
	const Av = reinterpret( A, 0 );
	...
	const info = zsyconvf( 'lower', 'revert', N, A, 1, N, 0, E, 1, 0, IPIV, 1, 0 );
	assert.equal( info, 0, 'info' );
});
...
test( 'zsyconvf: upper revert VALUE path with 2x2', function t() {
	...
	const IPIV = new Int32Array( [ 0, -2, 2, 3 ] );
	const N = 4;
	const A = new Complex128Array( 16 );
	const E = new Complex128Array( 4 );
	...
	Ev[ 4 ] = 7.5;
	Ev[ 5 ] = -1.25;
	const info = zsyconvf( 'upper', 'revert', N, A, 1, N, 0, E, 1, 0, IPIV, 1, 0 );
	assert.equal( info, 0, 'info' );
	const Av = reinterpret( A, 0 );
	const ia = ( 1 + ( 2 * N ) ) * 2;
