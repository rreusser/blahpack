...
import dlasyfAa from './../../dlasyf_aa/lib/base.js';
...
* @param {Float64Array} WORK - caller-owned workspace; base.js never allocates. Minimum length from `offsetWork` is `N*(NB+1)` (with `NB = 32`) when `N > 1`; unused otherwise.
...
function dsytrfAa( uplo, N, A, strideA1, strideA2, offsetA, IPIV, strideIPIV, offsetIPIV, WORK, strideWork, offsetWork ) {
	let alpha, jb, k1, k2, mj, nj, J1, j2, j3, j;
	...
	// WORK layout (caller-owned): H is N rows × NB cols (N*NB doubles) at
	...
	if ( uplo === 'upper' ) {
		...
		dcopy( N, A, sa2, offsetA, WORK, strideWork, offsetWork );
		...
		while ( j < N ) {
			...
			dlasyfAa(uplo, 2 - k1, N - j, jb, A, sa1, sa2, offsetA + ( ( ( j === 0 ) ? 0 : ( j - 1 ) ) * sa1 ) + ( j * sa2 ), IPIV, strideIPIV, offsetIPIV + ( j * strideIPIV ), WORK, strideWork, N * strideWork, offsetWork, WORK, strideWork, offsetWork + ( N * nb * strideWork ));
			...
			if ( j < N ) {
				...
				if ( J1 > 1 || jb > 1 ) {
					...
					dcopy(N - j, A, sa2, offsetA + ( ( j - 2 ) * sa1 ) + ( j * sa2 ), WORK, strideWork, offsetWork + ( ( jb + ( jb * N ) ) * strideWork ));
					dscal( N - j, alpha, WORK, strideWork, offsetWork + ( ( jb + ( jb * N ) ) * strideWork ) );
					...
					for ( j2 = j; j2 < N; j2 += NB ) {
						...
						for ( mj = nj - 1; mj >= 1; mj -= 1 ) {
							...
							dgemv('no-transpose', mj, jb + 1, -1.0, WORK, strideWork, N * strideWork, offsetWork + ( ( ( j3 - J1 + 1 ) + ( k1 * N ) ) * strideWork ), A, sa1, offsetA + ( ( J1 - k2 - 1 ) * sa1 ) + ( j3 * sa2 ), 1.0, A, sa2, offsetA + ( j3 * sa1 ) + ( j3 * sa2 ));
							j3 += 1;
						}
						...
						dgemm('transpose', 'transpose', nj, N - j3, jb + 1, -1.0, A, sa1, sa2, offsetA + ( ( J1 - k2 - 1 ) * sa1 ) + ( j2 * sa2 ), WORK, strideWork, N * strideWork, offsetWork + ( ( ( j3 - J1 + 1 ) + ( k1 * N ) ) * strideWork ), 1.0, A, sa1, sa2, offsetA + ( j2 * sa1 ) + ( j3 * sa2 ));
					}
				...
				dcopy( N - j, A, sa2, offsetA + ( j * sa1 ) + ( j * sa2 ), WORK, strideWork, offsetWork );
			}
		}
		return 0;
	}
	...
	dcopy( N, A, sa1, offsetA, WORK, strideWork, offsetWork );
	...
	while ( j < N ) {
		...
		dlasyfAa(uplo, 2 - k1, N - j, jb, A, sa1, sa2, offsetA + ( j * sa1 ) + ( ( ( j === 0 ) ? 0 : ( j - 1 ) ) * sa2 ), IPIV, strideIPIV, offsetIPIV + ( j * strideIPIV ), WORK, strideWork, N * strideWork, offsetWork, WORK, strideWork, offsetWork + ( N * nb * strideWork ));
		...
		if ( j < N ) {
			if ( J1 > 1 || jb > 1 ) {
				...
				dcopy(N - j, A, sa1, offsetA + ( j * sa1 ) + ( ( j - 2 ) * sa2 ), WORK, strideWork, offsetWork + ( ( jb + ( jb * N ) ) * strideWork ));
				dscal( N - j, alpha, WORK, strideWork, offsetWork + ( ( jb + ( jb * N ) ) * strideWork ) );
				...
				for ( j2 = j; j2 < N; j2 += NB ) {
					...
					for ( mj = nj - 1; mj >= 1; mj -= 1 ) {
						...
						dgemv('no-transpose', mj, jb + 1, -1.0, WORK, strideWork, N * strideWork, offsetWork + ( ( ( j3 - J1 + 1 ) + ( k1 * N ) ) * strideWork ), A, sa2, offsetA + ( j3 * sa1 ) + ( ( J1 - k2 - 1 ) * sa2 ), 1.0, A, sa1, offsetA + ( j3 * sa1 ) + ( j3 * sa2 ));
						j3 += 1;
					}

					// DGEMM( 'No transpose', 'Transpose', N-J3+1, NJ, JB+1, -1, WORK(J3-J1+1+K1*N), N, A(J2, J1-K2), LDA, 1, A(J3, J2), LDA )
					dgemm('no-transpose', 'transpose', N - j3, nj, jb + 1, -1.0, WORK, strideWork, N * strideWork, offsetWork + ( ( ( j3 - J1 + 1 ) + ( k1 * N ) ) * strideWork ), A, sa1, sa2, offsetA + ( j2 * sa1 ) + ( ( J1 - k2 - 1 ) * sa2 ), 1.0, A, sa1, sa2, offsetA + ( j3 * sa1 ) + ( j2 * sa2 ));
				}
			...
			dcopy( N - j, A, sa1, offsetA + ( j * sa1 ) + ( j * sa2 ), WORK, strideWork, offsetWork );
		}
	}
	return 0;
}
