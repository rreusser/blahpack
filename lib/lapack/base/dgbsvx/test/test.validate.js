...
var TIGHT_B = BANDED_POS[ 0 ];              // tight positive-sgn1 band storage
var TIGHT_D = schemes.dense.layouts()[ 0 ]; // tight col-major (B / X / oracle)
...
function opBandSolve( A0, N, kl, ku, code, rhsLogical, nrhs ) {
	var Aop = ( code === 'n' ) ? A0 : transposeLogical( A0, N );
	var akl = ( code === 'n' ) ? kl : ku;
	var aku = ( code === 'n' ) ? ku : kl;
	var Ar = schemes.banded.realize( sc, Aop, { 'kl': akl, 'ku': aku, 'luFill': true }, TIGHT_B );
	var Br = schemes.dense.realize( sc, rhsLogical, { 'part': 'full' }, TIGHT_D );
	var ipiv = new Int32Array( N );
	var info = dgbsv( N, akl, aku, nrhs, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], ipiv, 1, 0, Br.data, Br.args[ 0 ], Br.args[ 1 ], Br.args[ 2 ] );
	if ( info !== 0 ) {
...
function callGbsvx( mod, fact, trans, N, kl, ku, nrhs, A0, B0, abLayout, dLayout ) {
	var Ar = schemes.banded.realize( sc, A0, { 'kl': kl, 'ku': ku }, abLayout );                        // AB input (kl+ku+1 rows)
	var AFr = schemes.banded.realize( sc, zeros( N, N ), { 'kl': kl, 'ku': ku, 'luFill': true }, abLayout ); // AFB (top KL rows poisoned)
	...
	var WORK = poisonedWork( sc, Math.max( 1, 3 * N ) );
	var IWORK = new Int32Array( Math.max( N, 1 ) );
	var res = mod( fact, trans, N, kl, ku, nrhs, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], AFr.data, AFr.args[ 0 ], AFr.args[ 1 ], AFr.args[ 2 ], ipiv, 1, 0, 'none', r, 1, 0, c, 1, 0, Br.data, Br.args[ 0 ], Br.args[ 1 ], Br.args[ 2 ], Xr.data, Xr.args[ 0 ], Xr.args[ 1 ], Xr.args[ 2 ], FERR, 1, 0, BERR, 1, 0, WORK, 1, 0, IWORK, 1, 0 );
	return {
		'X': Xr,
		'FERR': FERR,
		'BERR': BERR,
...
function runProperty( trans, N, kl, ku, nrhs ) {
	var rng = new RNG( 0x100 + ( N * 1000 ) + ( kl * 100 ) + ( ku * 10 ) + nrhs ); // reproducible
	var A0 = logical.banded( sc, rng, N, N, kl, ku );
	var B0 = logical.general( sc, rng, N, nrhs );
	var code = transCode( trans );
	var which = ( code === 'n' ) ? 'one' : 'inf'; // gbsvx norm choice per trans

	var out = callGbsvx( dgbsvx, 'not-factored', trans, N, kl, ku, nrhs, A0, B0, TIGHT_B, TIGHT_D );
	var tag = 'dgbsvx '+trans+' N='+N+' kl='+kl+' ku='+ku+' nrhs='+nrhs;
	...
	checked( 'dgbsvx', 'property', function run() {
		...
		var Ainv = opInverse( A0, N, kl, ku, code );
	...
	checked( 'dgbsvx', 'property', function run() {
		TRANS.forEach( function eachTrans( trans ) {
			[ 5, 9, 16 ].forEach( function eachN( N ) {
				bands( N ).forEach( function eachBand( b ) {
					var rng = new RNG( 0xA5 + ( N * 100 ) + ( b[ 0 ] * 10 ) + b[ 1 ] );
					var A0 = logical.banded( sc, rng, N, N, b[ 0 ], b[ 1 ] );
					var B0 = logical.general( sc, rng, N, 2 );
					var oB = callGbsvx( dgbsvxBase, 'not-factored', trans, N, b[ 0 ], b[ 1 ], 2, A0, B0, TIGHT_B, TIGHT_D );
					var oN = callGbsvx( dgbsvx, 'not-factored', trans, N, b[ 0 ], b[ 1 ], 2, A0, B0, TIGHT_B, TIGHT_D );
...
function runInvariance( trans, abFamily, dFamily, fam ) {
	...
	checked( 'dgbsvx', 'layout-invariance', function run() {
		layoutInvariant( variants, function build( v ) {
			...
			var A0 = logical.banded( sc, rng, N, N, kl, ku );
			var B0 = logical.general( sc, rng, N, nrhs );
			var out = callGbsvx( dgbsvx, 'not-factored', trans, N, kl, ku, nrhs, A0, B0, v.a, v.d );
