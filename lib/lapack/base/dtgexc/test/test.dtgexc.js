/* eslint-disable no-restricted-syntax, stdlib/first-unit-test, max-len, no-mixed-operators */

// MODULES //

import test from 'node:test';
import assert from 'node:assert/strict';
import Float64Array from '@stdlib/array/float64/lib/index.js';
import dtgexc from './../lib/dtgexc.js';


// TESTS //

test( 'dtgexc is a function', function t() {
	assert.strictEqual( typeof dtgexc, 'function', 'is a function' );
});

test( 'dtgexc has expected arity', function t() {
	assert.strictEqual( dtgexc.length, 16, 'has expected arity' );
});

test( 'dtgexc throws TypeError for invalid order', function t() {
	assert.throws( function throws() {
		dtgexc( 'invalid', 2, 2, 2, new Float64Array( 4 ), 2, new Float64Array( 4 ), 2, new Float64Array( 4 ), 2, new Float64Array( 4 ), 2, 2, 2, new Float64Array( 4 ), 2 );
	}, TypeError );
});

test( 'dtgexc throws RangeError for negative N', function t() {
	assert.throws( function throws() {
		dtgexc( 'row-major', true, true, -1, new Float64Array( 4 ), 2, new Float64Array( 4 ), 2, new Float64Array( 4 ), 2, new Float64Array( 4 ), 2, 0, 0, new Float64Array( 20 ), 1 );
	}, RangeError );
});

test( 'dtgexc performs column-major swap correctly', function t() {

	const N = 3;
	const A = new Float64Array([
		1.0,
		0.0,
		0.0,
		0.5,
		2.0,
		0.0,
		0.3,
		0.4,
		3.0
	]);
	const B = new Float64Array([
		1.0,
		0.0,
		0.0,
		0.2,
		1.5,
		0.0,
		0.1,
		0.3,
		2.0
	]);
	const Q = new Float64Array( N * N );
	const Z = new Float64Array( N * N );
	const WORK = new Float64Array( 4 * N + 16 );

	const r = dtgexc( 'column-major', false, false, N, A, N, B, N, Q, N, Z, N, 0, 2, WORK, 1 );
	assert.strictEqual( r.info, 0, 'info is zero' );
	assert.strictEqual( r.ilst, 2, 'ilst' );
});

test( 'dtgexc performs row-major swap correctly', function t() {

	const N = 3;

	// Row-major: each row is contiguous
	const A = new Float64Array([
		1.0,
		0.5,
		0.3,
		0.0,
		2.0,
		0.4,
		0.0,
		0.0,
		3.0
	]);
	const B = new Float64Array([
		1.0,
		0.2,
		0.1,
		0.0,
		1.5,
		0.3,
		0.0,
		0.0,
		2.0
	]);
	const Q = new Float64Array( N * N );
	const Z = new Float64Array( N * N );
	const WORK = new Float64Array( 4 * N + 16 );

	const r = dtgexc( 'row-major', false, false, N, A, N, B, N, Q, N, Z, N, 0, 2, WORK, 1 );
	assert.strictEqual( r.info, 0, 'info is zero' );
	assert.strictEqual( r.ilst, 2, 'ilst' );
});
