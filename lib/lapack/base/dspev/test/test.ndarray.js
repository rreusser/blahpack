...
import dspev from './../lib/ndarray.js';

// FIXTURES //

import jobz_v_uplo_u_4x4 from './fixtures/jobz_v_uplo_u_4x4.json' with { type: 'json' };
import jobz_v_uplo_l_4x4 from './fixtures/jobz_v_uplo_l_4x4.json' with { type: 'json' };
import jobz_n_uplo_u_4x4 from './fixtures/jobz_n_uplo_u_4x4.json' with { type: 'json' };
import jobz_n_uplo_l_4x4 from './fixtures/jobz_n_uplo_l_4x4.json' with { type: 'json' };
import jobz_v_uplo_l_3x3 from './fixtures/jobz_v_uplo_l_3x3.json' with { type: 'json' };
import jobz_v_uplo_u_3x3 from './fixtures/jobz_v_uplo_u_3x3.json' with { type: 'json' };
import n1_jobz_v from './fixtures/n1_jobz_v.json' with { type: 'json' };
...
import jobz_n_uplo_l_3x3 from './fixtures/jobz_n_uplo_l_3x3.json' with { type: 'json' };
...
test( 'dspev: JOBZ=V, UPLO=U, 4x4', function t() {
	...
	const tc = jobz_v_uplo_u_4x4;
	const Afull = unpackUpper( ( AP = packedUpper4x4(), AP ), 4 );
	const W = new Float64Array( 4 );
	const Z = new Float64Array( 16 );
	const WORK = new Float64Array( 200 );

	const info = dspev( 'compute-vectors', 'upper', 4, AP, 1, 0, W, 1, 0, Z, 1, 4, 0, WORK, 1, 0 ); // eslint-disable-line max-len
...
test( 'dspev: JOBZ=V, UPLO=L, 4x4', function t() {
	...
	const tc = jobz_v_uplo_l_4x4;
	const AP = packedLower4x4();
	const Afull = unpackLower( packedLower4x4(), 4 );
	const W = new Float64Array( 4 );
	const Z = new Float64Array( 16 );
	const WORK = new Float64Array( 200 );

	const info = dspev( 'compute-vectors', 'lower', 4, AP, 1, 0, W, 1, 0, Z, 1, 4, 0, WORK, 1, 0 ); // eslint-disable-line max-len
	assert.equal( info, tc.info );
	assertArrayClose( toArray( W ), tc.w, 1e-13, 'eigenvalues' );
	assertOrthogonal( Z, 4, 1e-13, 'orthogonality' );
	assertEigendecomp( Afull, Z, W, 4, 1e-12, 'eigendecomp' );
...
test( 'dspev: JOBZ=N, UPLO=U, 4x4', function t() {
	...
	const tc = jobz_n_uplo_u_4x4;
	const AP = packedUpper4x4();
	const W = new Float64Array( 4 );
	const Z = new Float64Array( 1 );
	const WORK = new Float64Array( 200 );

	const info = dspev( 'no-vectors', 'upper', 4, AP, 1, 0, W, 1, 0, Z, 1, 1, 0, WORK, 1, 0 ); // eslint-disable-line max-len
	assert.equal( info, tc.info );
	assertArrayClose( toArray( W ), tc.w, 1e-13, 'eigenvalues' );
});
...
test( 'dspev: JOBZ=N, UPLO=L, 4x4', function t() {
	...
	const tc = jobz_n_uplo_l_4x4;
	const AP = packedLower4x4();
	const W = new Float64Array( 4 );
	const Z = new Float64Array( 1 );
	const WORK = new Float64Array( 200 );

	const info = dspev( 'no-vectors', 'lower', 4, AP, 1, 0, W, 1, 0, Z, 1, 1, 0, WORK, 1, 0 ); // eslint-disable-line max-len
	assert.equal( info, tc.info );
	assertArrayClose( toArray( W ), tc.w, 1e-13, 'eigenvalues' );
});
...
test( 'dspev: JOBZ=V, UPLO=L, 3x3', function t() {
	...
	const tc = jobz_v_uplo_l_3x3;
	const AP = packedLower3x3();
	const Afull = unpackLower( packedLower3x3(), 3 );
	const W = new Float64Array( 3 );
	const Z = new Float64Array( 9 );
	const WORK = new Float64Array( 200 );

	const info = dspev( 'compute-vectors', 'lower', 3, AP, 1, 0, W, 1, 0, Z, 1, 3, 0, WORK, 1, 0 ); // eslint-disable-line max-len
	assert.equal( info, tc.info );
	assertArrayClose( toArray( W ), tc.w, 1e-13, 'eigenvalues' );
...
test( 'dspev: JOBZ=V, UPLO=U, 3x3', function t() {
	...
	const tc = jobz_v_uplo_u_3x3;
	const AP = packedUpper3x3();
	const Afull = unpackUpper( packedUpper3x3(), 3 );
	const W = new Float64Array( 3 );
	const Z = new Float64Array( 9 );
	const WORK = new Float64Array( 200 );

	const info = dspev( 'compute-vectors', 'upper', 3, AP, 1, 0, W, 1, 0, Z, 1, 3, 0, WORK, 1, 0 ); // eslint-disable-line max-len
	assert.equal( info, tc.info );
	assertArrayClose( toArray( W ), tc.w, 1e-13, 'eigenvalues' );
	assertOrthogonal( Z, 3, 1e-13, 'orthogonality' );
	assertEigendecomp( Afull, Z, W, 3, 1e-12, 'eigendecomp' );
...
test( 'dspev: N=1, JOBZ=V', function t() {
	...
	const tc = n1_jobz_v;
	const AP = new Float64Array( [ 3.5 ] );
	const W = new Float64Array( 1 );
	const Z = new Float64Array( 1 );
	const WORK = new Float64Array( 10 );

	const info = dspev( 'compute-vectors', 'lower', 1, AP, 1, 0, W, 1, 0, Z, 1, 1, 0, WORK, 1, 0 ); // eslint-disable-line max-len
	assert.equal( info, tc.info );
...
test( 'dspev: N=1, JOBZ=N', function t() {
	...
	const tc = n1_jobz_n;
	const AP = new Float64Array( [ 7.25 ] );
	const W = new Float64Array( 1 );
	const Z = new Float64Array( 1 );
	const WORK = new Float64Array( 10 );

	const info = dspev( 'no-vectors', 'upper', 1, AP, 1, 0, W, 1, 0, Z, 1, 1, 0, WORK, 1, 0 ); // eslint-disable-line max-len
	assert.equal( info, tc.info );
	assertClose( W[ 0 ], tc.w1, 1e-15, 'w1' );
});
...
test( 'dspev: N=0', function t() {
	...
	const info = dspev( 'compute-vectors', 'lower', 0, AP, 1, 0, W, 1, 0, Z, 1, 1, 0, WORK, 1, 0 ); // eslint-disable-line max-len
	assert.equal( info, tc.info );
});
...
test( 'dspev: diagonal 4x4', function t() {
	...
	const AP = new Float64Array( [ 3, 0, 0, 0, 1, 0, 0, 4, 0, 2 ] );
	const Afull = unpackLower( new Float64Array( AP ), 4 );
	const W = new Float64Array( 4 );
	const Z = new Float64Array( 16 );
	const WORK = new Float64Array( 200 );
...
test( 'dspev: JOBZ=N, UPLO=L, 3x3', function t() {
	...
	const tc = jobz_n_uplo_l_3x3;
	const AP = packedLower3x3();
	const W = new Float64Array( 3 );
	const Z = new Float64Array( 1 );
	const WORK = new Float64Array( 200 );

	const info = dspev( 'no-vectors', 'lower', 3, AP, 1, 0, W, 1, 0, Z, 1, 1, 0, WORK, 1, 0 ); // eslint-disable-line max-len
	assert.equal( info, tc.info );
	assertArrayClose( toArray( W ), tc.w, 1e-13, 'eigenvalues' );
});
...
test( 'dspev: scaling path (tiny)', function t() {
	...
	const AP = new Float64Array( [ 5*scale, 1*scale, 2*scale, 4*scale, 1*scale, 6*scale ] ); // eslint-disable-line max-len
	const Afull = unpackLower( new Float64Array( AP ), 3 );
	const W = new Float64Array( 3 );
	const Z = new Float64Array( 9 );
	const WORK = new Float64Array( 200 );
...
test( 'dspev: scaling path (tiny, eigenvalues only)', function t() {
	...
	const scale = 1e-154;
	const AP = new Float64Array( [ 4*scale, 1*scale, 5*scale ] );
	const W = new Float64Array( 2 );
	const Z = new Float64Array( 1 );
	const WORK = new Float64Array( 200 );

	const info = dspev( 'no-vectors', 'lower', 2, AP, 1, 0, W, 1, 0, Z, 1, 1, 0, WORK, 1, 0 ); // eslint-disable-line max-len
	assert.equal( info, 0 );
	const expected0 = ( 4.5 - Math.sqrt( 1.25 ) ) * scale;
	const expected1 = ( 4.5 + Math.sqrt( 1.25 ) ) * scale;
