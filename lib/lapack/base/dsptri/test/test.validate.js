...
function factorOne( A0, n, uplo ) {
	...
	const info = dsptrf( uplo, n, R.data, R.args[ 0 ], R.args[ 1 ], ipiv, 1, 0 );
	if ( info !== 0 ) {
...
function invertAt( F, n, uplo, layout ) {
	...
	const info = dsptri( uplo, n, R.data, R.args[ 0 ], R.args[ 1 ], ipiv, 1, 0, work, 1, 0 );
	if ( info !== 0 ) {
...
test( 'dsptri: workspace minimum (len N) suffices with a poisoned buffer', function t() {
	...
	checked( 'dsptri', 'workspace', function check_() {
		const minLen = assertWorkspaceSufficient( run, {}, 'dsptri WORK@N=33 upper' );
		if ( minLen !== N ) {
