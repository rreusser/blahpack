...
/**
* Property-based validation for dgerq2, following the /blahpack-validate process.
*
...
*
* RQ REFLECTOR CONVENTION (from data/lapack-3.12.0/SRC/dgerq2.f, restricted to
...
*     part), with an implicit 1 at column p_i and zeros beyond. dgerqf/dgerq2
*     store conj(v) there (dgerq2 conjugates the row before/after via the complex
...
*   - Each H(i) = I - tau_i * vv * vvᴴ, and (dgerq2.f Further Details)
...
* random general A suffices at every (M,N); dgerq2 is unblocked so there is no
...
import dgerq2 from './../lib/ndarray.js';

const sc = S.real; // d-routine
const LogicalMatrix = logical.LogicalMatrix;
const ROUTINE = 'dgerq2';


// HELPERS //

// Unblocked WORK need: dgerq2's dlarf('Right', ...) scratch is at most M.
function workLen( M ) {
	return Math.max( 1, M );
}
...
// C * H(i)ᴴ = C - conj(tau) (C vv) vvᴴ (for real, conj is the identity).
...
function factor( M, N, layout, wlen, doPoison ) {
	...
	dgerq2( M, N, R.data, R.args[ 0 ], R.args[ 1 ], R.args[ 2 ], TAU.data, TAU.args[ 0 ], TAU.args[ 1 ], work, 1, 0 );
	const taus = [];
	let i;
	for ( i = 0; i < k; i++ ) {
...
test( 'dgerq2: RQ reconstruction A=R*Q and orthonormal Q ((M,N) sweep x all layouts)', function t() {
...
// Step 4 (L3): layout-invariance fuzz. dgerq2 -> dlarf -> dgemv/dger, whose
...
test( 'dgerq2: bit-exact within storage-order family (col / row)', function t() {
...
function runInvariance( variants, fam, M, N ) {
	...
	checked( ROUTINE, 'layout-invariance', function run() {
		layoutInvariant( variants, function build( layout, i ) {
			...
			dgerq2( M, N, R.data, R.args[ 0 ], R.args[ 1 ], R.args[ 2 ], TAU.data, TAU.args[ 0 ], TAU.args[ 1 ], WORK.data, WORK.args[ 0 ], WORK.args[ 1 ] );
...
test( 'dgerq2: advertised WORK minimum suffices (poisoned)', function t() {
