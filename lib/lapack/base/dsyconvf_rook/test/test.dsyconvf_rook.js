...
import dsyconvf_rook from './../lib/dsyconvf_rook.js';
...
test( 'dsyconvf_rook column-major success path', function t() {
	...
	const A = new Float64Array( [ 4.0, 0.0, 0.0, 0.0, 2.0, 5.0, 0.0, 0.0, 1.0, 3.0, 6.0, 0.0, 0.0, 1.0, 2.0, 7.0 ] );
	const e = new Float64Array( 4 );
	const IPIV = new Int32Array( [ 0, 1, 2, 3 ] );
	const info = dsyconvf_rook( 'column-major', 'upper', 'convert', 4, A, 4, e, 1, IPIV, 1, 0 );
...
test( 'dsyconvf_rook row-major success path', function t() {
	...
	const e = new Float64Array( 4 );
	const IPIV = new Int32Array( [ 0, 1, 2, 3 ] );
	const info = dsyconvf_rook( 'row-major', 'lower', 'convert', 4, A, 4, e, 1, IPIV, 1, 0 );
	assert.strictEqual( info, 0 );
});
