...
import dlamtsqr from './../lib/ndarray.js';


// FIXTURES //

const fixtureDir = path.join( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures' );
const fixtureRaw = readFileSync( path.join( fixtureDir, 'dlamtsqr.jsonl' ), 'utf8' ); // eslint-disable-line node/no-sync
...
function runApplyCase( cfg ) {
	...
	const nblk = numBlocks( cfg.Mq, cfg.K, cfg.mb );
	const TC = nblk * cfg.K;
	const Aco = unpack( factor.A, cfg.Mq, cfg.K, cfg.Mq );
	const Tco = unpack( factor.T, cfg.nb, TC, cfg.nb );
	const Cco = buildC( cfg.cM, cfg.cN, cfg.fill );
	const lwk = ( cfg.side === 'left' ) ? cfg.cN * cfg.nb : cfg.mb * cfg.nb;
	const work = new Float64Array( lwk );
	const info = dlamtsqr( cfg.side, cfg.trans, cfg.cM, cfg.cN, cfg.K, cfg.mb, cfg.nb, Aco, 1, cfg.Mq, 0, Tco, 1, cfg.nb, 0, Cco, 1, cfg.cM, 0, work, 1, 0 );
...
test( 'dlamtsqr: M=0 quick return', function t() {
	const info = dlamtsqr( 'left', 'no-transpose', 0, 4, 0, 4, 1, buf( 16 ), 1, 4, 0, buf( 16 ), 1, 4, 0, buf( 16 ), 1, 4, 0, buf( 16 ), 1, 0 );
	assert.equal( info, 0, 'M=0 returns 0' );
});

test( 'dlamtsqr: N=0 quick return', function t() {
	const info = dlamtsqr( 'left', 'no-transpose', 4, 0, 0, 4, 1, buf( 16 ), 1, 4, 0, buf( 16 ), 1, 4, 0, buf( 16 ), 1, 4, 0, buf( 16 ), 1, 0 );
	assert.equal( info, 0, 'N=0 returns 0' );
});

test( 'dlamtsqr: K=0 quick return', function t() {
	const info = dlamtsqr( 'left', 'no-transpose', 4, 4, 0, 4, 1, buf( 16 ), 1, 4, 0, buf( 16 ), 1, 4, 0, buf( 16 ), 1, 4, 0, buf( 16 ), 1, 0 );
