...
// Scratch arrays
const DLARTG_OUT = new Float64Array( 3 );
...
function dgeev( jobvl, jobvr, N, A, strideA1, strideA2, offsetA, WR, strideWR, offsetWR, WI, strideWI, offsetWI, VL, strideVL1, strideVL2, offsetVL, VR, strideVR1, strideVR2, offsetVR, work, strideWork, offsetWork ) { // eslint-disable-line max-params
	let scalea, cscale, info, side, scl, cs, sn, nout, k, i;
	...
	const oScale = offsetWork;
	const oTau = offsetWork + N;
	const oScratch = offsetWork + 2 * N;
	const lworkScratch = work.length - oScratch;
	const SCALE = work.subarray( oScale, oScale + N );
	const SELECT = new Uint8Array( 1 ); // unused but required by dtrevc3

	// Scale A if max element outside range [SMLNUM, BIGNUM]
	const anrm = dlange( 'max', N, N, A, strideA1, strideA2, offsetA, work, 1, 0 );
	scalea = false;
	cscale = 0.0;
	...
	const bal = dgebal( 'both', N, A, strideA1, strideA2, offsetA, work, 1, oScale );
	const ilo = bal.ilo;
	const ihi = bal.ihi;
	...
	dgehrd( N, ilo, ihi, A, strideA1, strideA2, offsetA, work, 1, oTau, work, 1, oScratch );
