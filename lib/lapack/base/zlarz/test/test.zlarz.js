...
import zlarz from './../lib/zlarz.js';
...
test( 'zlarz throws RangeError for LDC too small (column-major)', function t() {
	...
	const tau = new Complex128Array( 1 );
	const C = new Complex128Array( 12 );
	const v = new Complex128Array( 2 );
	assert.throws( function throws() {
...
test( 'zlarz column-major left matches ndarray', function t() {
	const WORK2 = new Complex128Array( 3 );
	const WORK = new Complex128Array( 3 );
	const tau2 = buildTau( 1.2, -0.4 );
	const tau = buildTau( 1.2, -0.4 );
	const C2 = buildC4x3();
	const v2 = buildV2();
	const C = buildC4x3();
	const v = buildV2();
	const out = zlarz( 'column-major', 'left', 4, 3, 2, v, 1, tau, 0, C, 4, WORK, 1 );
...
test( 'zlarz right side column-major returns C', function t() {
	...
	const C = buildC4x3();
	const v = buildV2();
	const out = zlarz( 'column-major', 'right', 3, 4, 2, v, 1, tau, 0, C, 3, WORK, 1 );
	assert.strictEqual( out, C, 'returns C' );
});
...
test( 'zlarz returns C unchanged when M=0', function t() {
	...
	const out = zlarz( 'column-major', 'left', 0, 1, 0, v, 1, tau, 0, C, 1, WORK, 1 );
	assert.strictEqual( out, C, 'returns C' );
});
