...
import dlarmm from '../../dlarmm/lib/base.js';
import dlatrs from '../../dlatrs/lib/base.js';


// VARIABLES //

// DLAMCH('S') / DLAMCH('O'): safe minimum and overflow threshold
const SMLNUM = 2.2250738585072014e-308;
const BIGNUM = 1.0 / SMLNUM;
const RMAX = 1.7976931348623157e+308; // DLAMCH('Overflow')
...
function dlatrs3( uplo, trans, diag, normin, N, nrhs, A, strideA1, strideA2, offsetA, X, strideX1, strideX2, offsetX, SCALE, strideSCALE, offsetSCALE, CNORM, strideCNORM, offsetCNORM, work, strideWork, offsetWork ) { // eslint-disable-line max-params, no-unused-vars
	let scaloc, scamin, ifirst, jfirst, nrhsK, ilast, jlast, rscal, anrm, bnrm;
	let iinc, jinc, tmax, rhs, scl, i1, i2, ii, j1, j2, k1, k2, kk, i, j, k;

	const sa1 = strideA1;
	const sa2 = strideA2;
	const sx1 = strideX1;
	const sx2 = strideX2;
	const ss = strideSCALE;
	...
	const nba = Math.max( 1, ( ( N + NB ) - 1 ) / NB | 0 );
	const nbx = Math.max( 1, ( ( nrhs + NBRHS ) - 1 ) / NBRHS | 0 );
	...
	const lds = nba;
	const awrk = offsetWork + lscale;
	const oW = awrk + ( nba * nba );      // dlange scratch (8 elements)
	const oxnrm = oW + NB;                // XNRM cache (NBRHS elements)

	// Single-element scratch for dlatrs's scalar output (SCALOC).
	const scaleBlock = new Float64Array( 1 );
