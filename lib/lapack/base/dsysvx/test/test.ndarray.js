...
test( 'dsysvx: fact_n_upper', function t() {
	...
	info = callDsysvx( 'not-factored', 'upper', 3, 1, A, AF, IPIV, B, X, rcond, FERR, BERR, WORK, IWORK ); // eslint-disable-line max-len
...
test( 'dsysvx: fact_n_lower', function t() {
	...
	WORK = new Float64Array( 100 );
	IWORK = new Int32Array( 3 );
	info = callDsysvx( 'not-factored', 'lower', 3, 1, A, AF, IPIV, B, X, rcond, FERR, BERR, WORK, IWORK ); // eslint-disable-line max-len
	assert.equal( info, tc.info );
	assertArrayClose( toArray( X ), tc.x, 1e-12, 'x' );
	assertClose( rcond[ 0 ], tc.rcond, 1e-6, 'rcond' );
});
...
test( 'dsysvx: fact_f_upper (pre-factored)', function t() {
	...
	info = callDsysvx( 'factored', 'upper', 3, 1, A, AF, IPIV, B, X, rcond, FERR, BERR, WORK, IWORK ); // eslint-disable-line max-len
	assert.equal( info, tc.info );
	assertArrayClose( toArray( X ), tc.x, 1e-12, 'x' );
	assertClose( rcond[ 0 ], tc.rcond, 1e-6, 'rcond' );
});
...
test( 'dsysvx: n_zero', function t() {
	...
	info = callDsysvx( 'not-factored', 'upper', 0, 1, A, AF, IPIV, B, X, rcond, FERR, BERR, WORK, IWORK ); // eslint-disable-line max-len
	assert.equal( info, tc.info );
});
...
test( 'dsysvx: singular', function t() {
	...
	info = callDsysvx( 'not-factored', 'upper', 2, 1, A, AF, IPIV, B, X, rcond, FERR, BERR, WORK, IWORK ); // eslint-disable-line max-len
	assert.equal( info, tc.info );
...
test( 'dsysvx: multi_rhs', function t() {
	...
	WORK = new Float64Array( 100 );
	IWORK = new Int32Array( 3 );
	info = callDsysvx( 'not-factored', 'upper', 3, 2, A, AF, IPIV, B, X, rcond, FERR, BERR, WORK, IWORK ); // eslint-disable-line max-len
	assert.equal( info, tc.info );
	assertArrayClose( toArray( X ), tc.x, 1e-12, 'x' );
	assertClose( rcond[ 0 ], tc.rcond, 1e-6, 'rcond' );
});
