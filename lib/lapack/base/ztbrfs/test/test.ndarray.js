...
import ztbtrs from './../../ztbtrs/lib/base.js';
import ztbrfs from './../lib/ndarray.js';
...
import lower_kd2_conj_trans from './fixtures/lower_kd2_conj_trans.json' with { type: 'json' };
...
function runCase( uplo, trans, diagType, N, nrhs, kd, ABdata, ldab, Bdata, ldb ) { // eslint-disable-line max-len
	...
	const AB = new Complex128Array( ABdata );
	...
	const info = ztbrfs( uplo, trans, diagType, N, kd, nrhs, AB, 1, ldab, 0, B, 1, ldb, 0, X, 1, ldb, 0, FERR, 1, 0, BERR, 1, 0, WORK, 1, 0, RWORK, 1, 0 ); // eslint-disable-line max-len
...
test( 'ztbrfs: upper_no_trans (upper, no-transpose, non-unit, N=3, KD=1)', function t() { // eslint-disable-line max-len
	...
	const ldab = 2;
	const ldb = 3;
	const ABdata = new Float64Array( 2 * ldab * 3 );
	...
	const Bdata = new Float64Array( 2 * ldb * 1 );
	setAB( Bdata, ldb, 0, 0, 4.0, 1.0 );
	setAB( Bdata, ldb, 1, 0, 6.0, 0.0 );
	setAB( Bdata, ldb, 2, 0, 5.0, -1.0 );
	const res = runCase( 'upper', 'no-transpose', 'non-unit', 3, 1, 1, ABdata, ldab, Bdata, ldb ); // eslint-disable-line max-len
...
test( 'ztbrfs: lower_no_trans (lower, no-transpose, non-unit, N=3, KD=1)', function t() { // eslint-disable-line max-len
	...
	const res = runCase( 'lower', 'no-transpose', 'non-unit', 3, 1, 1, ABdata, ldab, Bdata, ldb ); // eslint-disable-line max-len
	assert.equal( res.info, tc.info );
	assertArrayClose( getCol( res.xv, ldb, 3, 0 ), tc.x, 1e-12, 'x' );
	assertArrayClose( res.berr, tc.berr, 1e-12, 'berr' );
	assertArrayClose( res.ferr, tc.ferr, 1e-6, 'ferr' );
...
test( 'ztbrfs: upper_conj_trans (upper, conjugate-transpose, non-unit, N=3, KD=1)', function t() { // eslint-disable-line max-len
	...
	const res = runCase( 'upper', 'conjugate-transpose', 'non-unit', 3, 1, 1, ABdata, ldab, Bdata, ldb ); // eslint-disable-line max-len
	assert.equal( res.info, tc.info );
	assertArrayClose( getCol( res.xv, ldb, 3, 0 ), tc.x, 1e-12, 'x' );
	assertArrayClose( res.berr, tc.berr, 1e-12, 'berr' );
	assertArrayClose( res.ferr, tc.ferr, 1e-6, 'ferr' );
...
test( 'ztbrfs: lower_conj_trans (lower, conjugate-transpose, non-unit, N=3, KD=1)', function t() { // eslint-disable-line max-len
	...
	const res = runCase( 'lower', 'conjugate-transpose', 'non-unit', 3, 1, 1, ABdata, ldab, Bdata, ldb ); // eslint-disable-line max-len
	assert.equal( res.info, tc.info );
	assertArrayClose( getCol( res.xv, ldb, 3, 0 ), tc.x, 1e-12, 'x' );
	assertArrayClose( res.berr, tc.berr, 1e-12, 'berr' );
	assertArrayClose( res.ferr, tc.ferr, 1e-6, 'ferr' );
...
test( 'ztbrfs: upper_unit_diag (upper, no-transpose, unit, N=3, KD=1)', function t() { // eslint-disable-line max-len
	...
	const res = runCase( 'upper', 'no-transpose', 'unit', 3, 1, 1, ABdata, ldab, Bdata, ldb ); // eslint-disable-line max-len
	assert.equal( res.info, tc.info );
	assertArrayClose( getCol( res.xv, ldb, 3, 0 ), tc.x, 1e-12, 'x' );
	assertArrayClose( res.berr, tc.berr, 1e-12, 'berr' );
	assertArrayClose( res.ferr, tc.ferr, 1e-6, 'ferr' );
...
test( 'ztbrfs: lower_unit_diag (lower, no-transpose, unit, N=3, KD=1)', function t() { // eslint-disable-line max-len
	...
	const res = runCase( 'lower', 'no-transpose', 'unit', 3, 1, 1, ABdata, ldab, Bdata, ldb ); // eslint-disable-line max-len
	assert.equal( res.info, tc.info );
	assertArrayClose( getCol( res.xv, ldb, 3, 0 ), tc.x, 1e-12, 'x' );
	assertArrayClose( res.berr, tc.berr, 1e-12, 'berr' );
	assertArrayClose( res.ferr, tc.ferr, 1e-6, 'ferr' );
...
test( 'ztbrfs: n_zero (N=0 quick return)', function t() {
	...
	FERR[ 0 ] = 999.0;
	BERR[ 0 ] = 999.0;
	const info = ztbrfs( 'upper', 'no-transpose', 'non-unit', 0, 0, 1, AB, 1, 1, 0, B, 1, 1, 0, X, 1, 1, 0, FERR, 1, 0, BERR, 1, 0, WORK, 1, 0, RWORK, 1, 0 ); // eslint-disable-line max-len
...
test( 'ztbrfs: nrhs_zero (NRHS=0 quick return)', function t() {
	...
	const tc = nrhs_zero;
	const AB = new Complex128Array( 9 );
	...
	const FERR = new Float64Array( 1 );
	const BERR = new Float64Array( 1 );
	const info = ztbrfs( 'upper', 'no-transpose', 'non-unit', 3, 1, 0, AB, 1, 3, 0, B, 1, 3, 0, X, 1, 3, 0, FERR, 1, 0, BERR, 1, 0, WORK, 1, 0, RWORK, 1, 0 ); // eslint-disable-line max-len
	assert.equal( info, tc.info );
});
...
test( 'ztbrfs: multi_rhs (upper, no-transpose, non-unit, N=3, KD=1, NRHS=2)', function t() { // eslint-disable-line max-len
	...
	setAB( ABdata, ldab, 0, 2, 2.0, -1.0 );
	setAB( ABdata, ldab, 1, 2, 5.0, -1.0 );
	const Bdata = new Float64Array( 2 * ldb * 2 );
	setAB( Bdata, ldb, 0, 0, 4.0, 1.0 );
	setAB( Bdata, ldb, 1, 0, 6.0, 0.0 );
	setAB( Bdata, ldb, 2, 0, 5.0, -1.0 );
	...
	const res = runCase( 'upper', 'no-transpose', 'non-unit', 3, 2, 1, ABdata, ldab, Bdata, ldb ); // eslint-disable-line max-len
	assert.equal( res.info, tc.info );
...
test( 'ztbrfs: upper_kd2 (upper, no-transpose, non-unit, N=4, KD=2)', function t() { // eslint-disable-line max-len
	...
	const ldab = 3;
	const ldb = 4;
	const ABdata = new Float64Array( 2 * ldab * 4 );
	...
	const res = runCase( 'upper', 'no-transpose', 'non-unit', 4, 1, 2, ABdata, ldab, Bdata, ldb ); // eslint-disable-line max-len
	assert.equal( res.info, tc.info );
...
test( 'ztbrfs: lower_kd2_conj_trans (lower, conjugate-transpose, non-unit, N=4, KD=2)', function t() { // eslint-disable-line max-len
	...
	const res = runCase( 'lower', 'conjugate-transpose', 'non-unit', 4, 1, 2, ABdata, ldab, Bdata, ldb ); // eslint-disable-line max-len
	assert.equal( res.info, tc.info );
	assertArrayClose( getCol( res.xv, ldb, 4, 0 ), tc.x, 1e-12, 'x' );
	assertArrayClose( res.berr, tc.berr, 1e-12, 'berr' );
	assertArrayClose( res.ferr, tc.ferr, 1e-6, 'ferr' );
