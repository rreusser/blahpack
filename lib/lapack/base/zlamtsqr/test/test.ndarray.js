...
import zlamtsqr from './../lib/ndarray.js';


// FIXTURES //

const fixtureDir = path.join( import.meta.dirname, '..', '..', '..', '..', '..', 'test', 'fixtures' );
const rawLines = readFileSync( path.join( fixtureDir, 'zlamtsqr.jsonl' ), 'utf8' ).trim().split( '\n' );
...
function assertClose( actual, expected, tol, msg ) {
	...
	const ae = Math.abs( expected );
	const ref = ( ae > 1.0 ) ? ae : 1.0;
...
function runLeft( name, trans ) {
	...
	const A = makeComplex( tc.A ); // M-by-K (column-major, contiguous)
	const T = makeComplex( tc.T ); // NB-by-(numblk*K) (column-major, contiguous)
	const C = makeComplex( tc.C0 ); // M-by-N (column-major, contiguous)
	const WORK = new Complex128Array( tc.N * tc.NB );

	const info = zlamtsqr( 'left', trans, tc.M, tc.N, tc.K, tc.MB, tc.NB, A, 1, tc.M, 0, T, 1, tc.LDT, 0, C, 1, tc.M, 0, WORK, 1, 0 );
...
function runRight( name, trans ) {
	...
	const tc = findCase( name );
	const A = makeComplex( tc.A ); // N-by-K (column-major, contiguous)
	const T = makeComplex( tc.T ); // NB-by-(numblk*K) (column-major, contiguous)
	const C = makeComplex( tc.C0 ); // M-by-N (column-major, contiguous)
	const WORK = new Complex128Array( tc.M * tc.NB );

	const info = zlamtsqr( 'right', trans, tc.M, tc.N, tc.K, tc.MB, tc.NB, A, 1, tc.N, 0, T, 1, tc.LDT, 0, C, 1, tc.M, 0, WORK, 1, 0 );
...
test( 'zlamtsqr quick-return: M=0', function t() {
	...
	const info = zlamtsqr( 'left', 'no-transpose', 0, 4, 0, 4, 2, A, 1, 1, 0, T, 1, 1, 0, C, 1, 1, 0, WORK, 1, 0 );
...
test( 'zlamtsqr quick-return: N=0', function t() {
	...
	const T = new Complex128Array( 1 );
	const C = new Complex128Array( 1 );
	const info = zlamtsqr( 'left', 'no-transpose', 4, 0, 2, 4, 2, A, 1, 4, 0, T, 1, 2, 0, C, 1, 4, 0, WORK, 1, 0 );
...
test( 'zlamtsqr quick-return: K=0', function t() {
	...
	const info = zlamtsqr( 'left', 'no-transpose', 4, 4, 0, 4, 2, A, 1, 1, 0, T, 1, 1, 0, C, 1, 4, 0, WORK, 1, 0 );
