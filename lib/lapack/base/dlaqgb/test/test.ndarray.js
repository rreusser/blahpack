...
import dlaqgb from './../lib/ndarray.js';
...
test( 'dlaqgb: no equilibration (rowcnd >= thresh, colcnd >= thresh, amax in range)', function t() { // eslint-disable-line max-len
	...
	const tc = no_equil;
	const AB = inputAB();
	const equed = dlaqgb( 4, 5, 1, 2, AB, 1, 4, 0, inputR(), 1, 0, inputC(), 1, 0, 0.5, 0.6, 4.0 ); // eslint-disable-line max-len
...
test( 'dlaqgb: row equilibration only (rowcnd < thresh)', function t() {
	...
	const tc = row_equil;
	const AB = inputAB();
	const equed = dlaqgb( 4, 5, 1, 2, AB, 1, 4, 0, inputR(), 1, 0, inputC(), 1, 0, 0.01, 0.6, 4.0 ); // eslint-disable-line max-len
...
test( 'dlaqgb: column equilibration only (colcnd < thresh)', function t() {
	...
	const tc = col_equil;
	const AB = inputAB();
	const equed = dlaqgb( 4, 5, 1, 2, AB, 1, 4, 0, inputR(), 1, 0, inputC(), 1, 0, 0.5, 0.01, 4.0 ); // eslint-disable-line max-len
...
test( 'dlaqgb: both row and column equilibration', function t() {
	...
	const tc = both_equil;
	const AB = inputAB();
	const equed = dlaqgb( 4, 5, 1, 2, AB, 1, 4, 0, inputR(), 1, 0, inputC(), 1, 0, 0.01, 0.01, 4.0 ); // eslint-disable-line max-len
...
test( 'dlaqgb: row scaling triggered by amax > large', function t() {
	...
	const tc = amax_large;
	const AB = inputAB();
	const equed = dlaqgb( 4, 5, 1, 2, AB, 1, 4, 0, inputR(), 1, 0, inputC(), 1, 0, 0.5, 0.6, 1.0e300 ); // eslint-disable-line max-len
	assert.equal( equed, 'row' );
	assertArrayClose( AB, tc.ab, 1e-14, 'ab' );
});
...
test( 'dlaqgb: row scaling triggered by amax < small', function t() {
	...
	const tc = amax_small;
	const AB = inputAB();
	const equed = dlaqgb( 4, 5, 1, 2, AB, 1, 4, 0, inputR(), 1, 0, inputC(), 1, 0, 0.5, 0.6, 1.0e-320 ); // eslint-disable-line max-len
	assert.equal( equed, 'row' );
	assertArrayClose( AB, tc.ab, 1e-14, 'ab' );
});

test( 'dlaqgb: quick return M=0', function t() {
	const equed = dlaqgb( 0, 5, 1, 2, new Float64Array( 0 ), 1, 4, 0, new Float64Array( 0 ), 1, 0, new Float64Array( 5 ), 1, 0, 0.5, 0.6, 4.0 ); // eslint-disable-line max-len
	assert.equal( equed, 'none' );
});

test( 'dlaqgb: quick return N=0', function t() {
	const equed = dlaqgb( 4, 0, 1, 2, new Float64Array( 0 ), 1, 4, 0, new Float64Array( 4 ), 1, 0, new Float64Array( 0 ), 1, 0, 0.5, 0.6, 4.0 ); // eslint-disable-line max-len
	assert.equal( equed, 'none' );
});
