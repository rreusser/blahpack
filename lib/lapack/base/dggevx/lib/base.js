...
function dggevx( balanc, jobvl, jobvr, sense, N, A, strideA1, strideA2, offsetA, B, strideB1, strideB2, offsetB, ALPHAR, strideALPHAR, offsetALPHAR, ALPHAI, strideALPHAI, offsetALPHAI, BETA, strideBETA, offsetBETA, VL, strideVL1, strideVL2, offsetVL, VR, strideVR1, strideVR2, offsetVR, LSCALE, strideLSCALE, offsetLSCALE, RSCALE, strideRSCALE, offsetRSCALE, RCONDE, strideRCONDE, offsetRCONDE, RCONDV, strideRCONDV, offsetRCONDV, work, strideWork, offsetWork, iwork, strideIwork, offsetIwork, bwork, strideBwork, offsetBwork ) { // eslint-disable-line max-params
	...
	//   [offsetWork .. offsetWork+N-1]   TAU (QR reflectors, length N)
	//   [offsetWork+N .. end]            scratch for dgeqrf/dormqr/dorgqr/dhgeqz/dtgevc/dtgsna
	//
	// Minimum work size:
	//   sense='none':             max(1, 8*N)             (N TAU + 7N scratch)
	//   sense='eigenvalues':      max(1, 11*N)            (N TAU + 10N scratch for dtgevc 6N + sense loop N)
	//   sense='right-vectors'/'both':  max(1,N+2*N*(N+2)+16)   (N TAU + dtgsna workspace)
	...
	// Internal scratch arrays for the sense loop (sub-blocks of Fortran WORK(1..4N) reused from eigenvector computation):
	// workVL/workVR are N×2 column-major matrices holding one pair of left/right eigenvectors at a time.
	// workCN is scratch for dtgevc 'selected', and SNSWORK is scratch for dtgsna; they share space sequentially.
	// These cannot be carved from the caller's work without complicating the layout, so they are allocated internally.
	...
	if ( ilv || !wantsn ) {
		...
		if ( !wantsn ) {
			...
			// workVL/workVR: up to 2 columns × N rows for left/right eigenvectors in the sense loop (col-major, ld=N).
			// workCN: scratch for dtgevc 'selected' (needs 6*N).
			// SNSWORK: scratch for dtgsna (needs 2*N*(N+2)+16 for 'eigenvectors'/'both', N otherwise).
			// dtgevc and dtgsna run sequentially so workCN and SNSWORK can share space via SNSWORK.
			workVL = new Float64Array( 2 * N );			workVR = new Float64Array( 2 * N );			if ( wantsv || wantsb ) {
				iwrk1Len = ( 2 * N * ( N + 2 ) ) + 16;
			} else {
				iwrk1Len = Math.max( N, 1 );
			}
			SNSWORK = new Float64Array( Math.max( iwrk1Len, 6 * N ) );			workCN = SNSWORK; // dtgevc scratch; same buffer (sequential usage)
			pair = false;
