...
import dlaqr3 from '../../dlaqr3/lib/base.js';
import dlaqr4 from '../../dlaqr4/lib/base.js';
import dlaqr5 from '../../dlaqr5/lib/base.js';
...
const IPARMQ_NMIN = 75;     // ISPEC=12: crossover point
const IPARMQ_NIBBLE = 14;   // ISPEC=14: nibble crossover
const IPARMQ_K22MIN = 14;   // threshold for KACC22=2
const IPARMQ_KACMIN = 14;   // threshold for KACC22=1
const IPARMQ_KNWSWP = 500;  // deflation window NS vs 3*NS/2 threshold
...
*/
function dlaqr0( wantt, wantz, N, ilo, ihi, H, strideH1, strideH2, offsetH, WR, strideWR, offsetWR, WI, strideWI, offsetWI, iloz, ihiz, Z, strideZ1, strideZ2, offsetZ, WORK, strideWork, offsetWork ) {
	let nwupbd, nsmax, nwmax, sorted, itmax, kwtop, kbot, ktop, ndfl, ndec;
	let info, swap, nmin, nwr, nsr, nw, nh, nho, nve, ns, ks, ld, ls, kv, kt;
	let kwv, kwh, ku, kdu, aa, bb, cc, dd, ss, lv2, it, k, i, nibble, kacc22;
	let inf, aedResult, zdum;
	...
	// Caller-owned workspace: the available length is derived from the
	// provided WORK array (replaces the former scalar `lwork` argument).
	...
	if ( N <= NTINY ) {
		...
		for ( it = 1; it <= itmax; it++ ) {
			...
			aedResult = dlaqr3( wantt, wantz, N, ktop, kbot, nw, H, strideH1, strideH2, offsetH, iloz, ihiz, Z, strideZ1, strideZ2, offsetZ, WR, strideWR, offsetWR, WI, strideWI, offsetWI, H, strideH1, strideH2, hij( kv, 1 ), nho, H, strideH1, strideH2, hij( kv, kt ), nve, H, strideH1, strideH2, hij( kwv, 1 ), WORK, strideWork, offsetWork );
			ls = aedResult.ns;
			ld = aedResult.nd;
			...
			if ( ld === 0 || ( 100 * ld <= nw * nibble && kbot - ktop + 1 > Math.min( nmin, nwmax ) ) ) {
				...
				if ( ndfl % KEXSH === 0 ) {
					...
					if ( kbot - ks + 1 <= Math.floor( ns / 2 ) ) {
						...
						if ( ns > nmin ) {
							inf = dlaqr4( false, false, ns, 1, ns, H, strideH1, strideH2, hij( kt, 1 ), WR, strideWR, offsetWR + ( ks - 1 ) * strideWR, WI, strideWI, offsetWI + ( ks - 1 ) * strideWI, 1, 1, WORK, 1, 1, 0, WORK, strideWork, offsetWork );
						} else {
