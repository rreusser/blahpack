/**
* Property-based validation for zposvx, following the /blahpack-validate process.
*
...
* proven for its constituent routines (zposv/zporfs residual, zpocon rcond,
* zporfs FERR/BERR). We drive zposvx with fact='not-factored' (factor A in place,
...
*       reciprocal condition number (anorm from A0, ‖A0⁻¹‖₁ from a trusted zposv
...
* final workspace is a real RWORK of N (not IWORK); `rcond` is an explicit
* Float64Array out-array after offsetX. Only the uplo triangle of A is realized;
* the opposite stays poisoned, so a read of the wrong triangle trips a NaN.
*/
...
function invNorm1( A0, n, uplo ) {
	...
	const info = zposv( uplo, n, n, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], Br.data, Br.args[ 0 ], Br.args[ 1 ], Br.args[ 2 ] );
	if ( info !== 0 ) {
...
// Independent TRUE solution: solve A0*X = B0 on fresh copies with trusted zposv.
function trueSolution( A0, B0, N, nrhs, uplo ) {
	const Ar = schemes.dense.realize( sc, A0, { 'part': uplo }, TIGHT );
	const Br = schemes.dense.realize( sc, B0, { 'part': 'full' }, TIGHT );
	const info = zposv( uplo, N, nrhs, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], Br.data, Br.args[ 0 ], Br.args[ 1 ], Br.args[ 2 ] );
	if ( info !== 0 ) {
		throw new Error( 'oracle zposv failed (info='+info+'); matrix not HPD?' );
	}
	return Br;
...
// Drive zposvx (fact='not-factored', no equilibration) and return the physical
...
function drive( uplo, N, nrhs, A0, B0, layout ) {
	...
	const AFr = schemes.dense.realize( sc, A0, { 'part': uplo }, layout ); // overwritten by zlacpy+zpotrf
	const Br = schemes.dense.realize( sc, B0, { 'part': 'full' }, layout );
	const Xr = schemes.dense.realize( sc, B0, { 'part': 'full' }, layout ); // overwritten by zlacpy+zpotrs
	...
	const out = zposvx( 'not-factored', uplo, N, nrhs, Ar.data, Ar.args[ 0 ], Ar.args[ 1 ], Ar.args[ 2 ], AFr.data, AFr.args[ 0 ], AFr.args[ 1 ], AFr.args[ 2 ], 'none', s, 1, 0, Br.data, Br.args[ 0 ], Br.args[ 1 ], Br.args[ 2 ], Xr.data, Xr.args[ 0 ], Xr.args[ 1 ], Xr.args[ 2 ], rcond, FERR, 1, 0, BERR, 1, 0, WORK, 1, 0, RWORK, 1, 0 );
	return {
		'Xr': Xr,
		'rcond': rcond,
		'FERR': FERR,
...
// and nrhs.
test( 'zposvx: expert-driver residual + rcond + FERR/BERR (uplo x N x nrhs)', function t() {
...
function runProperty( uplo, N, nrhs ) {
	...
	const tag = 'zposvx '+uplo+' N='+N+' nrhs='+nrhs;

	if ( d.result.info !== 0 ) {
		throw new Error( tag+': zposvx returned info='+d.result.info+' (expected 0 for well-conditioned HPD input)' );
	}
	...
	checked( 'zposvx', 'residual', function run() {
		let j;
		for ( j = 0; j < nrhs; j++ ) {
	...
	checked( 'zposvx', 'property', function run() {
	...
	checked( 'zposvx', 'structural', function run() {
...
// Step 4: layout-invariance fuzz. zposvx composes a Cholesky factor + BLAS solves
...
// family (col / row split, as for zposv/zporfs). Cross-order correctness is
...
test( 'zposvx: bit-exact within storage-order family (col / row)', function t() {
	UPLOS.forEach( function eachUplo( uplo ) {
...
function runInvariance( uplo, variants, fam ) {
	...
	const nrhs = 2;
	const SEED = 0xBEEF;
	checked( 'zposvx', 'layout-invariance', function run() {
		layoutInvariant( variants, function build( layout ) {
			...
			return out;
		}, { 'label': 'zposvx '+uplo+' layout invariance '+fam+'-major' } );
	});
}
