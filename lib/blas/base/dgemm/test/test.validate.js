/**
* Property-based validation for dgemm, following the /blahpack-validate process.
*
...
* cross product, then fuzzed for bit-exact layout invariance (L3).
*/
...
import dgemm from './../lib/ndarray.js';
...
test( 'dgemm: C = alpha*op(A)*op(B) + beta*C (transa x transb x sizes x scalars)', function t() {
		...
		FLAGS.forEach( function eachB( fb, tj ) {
			...
			TRIPLES.forEach( function eachTriple( tr ) {
				...
				COMBOS.forEach( function eachCombo( combo ) {
					...
					dgemm( fa[ 0 ], fb[ 0 ], m, n, k, sc.apiScalar( alpha ), RA.data, RA.args[ 0 ], RA.args[ 1 ], RA.args[ 2 ], RB.data, RB.args[ 0 ], RB.args[ 1 ], RB.args[ 2 ], sc.apiScalar( beta ), RC.data, RC.args[ 0 ], RC.args[ 1 ], RC.args[ 2 ] );
					...
					const label = 'dgemm ' + ca + cb + ' m=' + m + ' n=' + n + ' k=' + k + ' ' + combo;
					checked( 'dgemm', 'residual', function run() {
...
test( 'dgemm: bit-exact across storage layouts', function t() {
	const m = 7;
	const n = 5;
	const k = 6;
	const SEED = 0xF00D;
	const layouts = schemes.dense.layouts();
	[ [ 'no-transpose', 'no-transpose' ], [ 'transpose', 'no-transpose' ], [ 'transpose', 'transpose' ] ].forEach( function eachCombo( fc ) {
		const ca = ( fc[ 0 ] === 'no-transpose' ) ? 'n' : 't';
		const cb = ( fc[ 1 ] === 'no-transpose' ) ? 'n' : 't';
		checked( 'dgemm', 'layout-invariance', function run() {
			layoutInvariant( layouts, function build( layout, idx ) {
				...
				dgemm( fc[ 0 ], fc[ 1 ], m, n, k, sc.apiScalar( alpha ), RA.data, RA.args[ 0 ], RA.args[ 1 ], RA.args[ 2 ], RB.data, RB.args[ 0 ], RB.args[ 1 ], RB.args[ 2 ], sc.apiScalar( beta ), RC.data, RC.args[ 0 ], RC.args[ 1 ], RC.args[ 2 ] );

				return check.flattenLogical( sc, readC( RC, m, n ) );
			}, { 'label': 'dgemm ' + ca + cb + ' layout invariance' } );
		});
	});
});
