...
test( 'main export: N=0 quick return via column-major layout', function t() {
	...
	const info = dsyconvf_rook( 'column-major', 'upper', 'convert', 0, A, 1, e, 1, IPIV, 1, 0 ); // eslint-disable-line max-len
...
test( 'main export: ndarray method N=0 quick return', function t() {
	...
	const e = new Float64Array( 0 );
	const IPIV = new Int32Array( 0 );
	const info = dsyconvf_rook.ndarray( 'upper', 'convert', 0, A, 1, 1, 0, e, 1, 0, IPIV, 1, 0 ); // eslint-disable-line max-len
	assert.strictEqual( info, 0, 'info is 0' );
});
