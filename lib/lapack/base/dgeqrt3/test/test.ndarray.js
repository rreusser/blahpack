...
const raw = readFileSync( path.join( fixtureDir, 'dgeqrt3.jsonl' ), 'utf8' ); // eslint-disable-line node/no-sync
...
function runCase( caseName, M, N, rows, tol ) {
	...
	const info = dgeqrt3( M, N, A, 1, M, 0, T, 1, N, 0 );
...
test( 'dgeqrt3: n=0 quick return', function t() {
	...
	const info = dgeqrt3( 0, 0, A, 1, 1, 0, T, 1, 1, 0 );
...
test( 'dgeqrt3: works with non-trivial offsets and strides', function t() {
	...
	const offsetA = 5;
	const offsetT = 3;
	const A = new Float64Array( ( LDA * 2 ) + offsetA );
	...
	const info = dgeqrt3( 4, 2, A, 1, LDA, offsetA, T, 1, LDT, offsetT );
	assert.strictEqual( info, 0 );
...
test( 'dgeqrt3: works on a row-major matrix layout', function t() {
	...
	const info = dgeqrt3( M, N, A, N, 1, 0, T, N, 1, 0 );
	assert.strictEqual( info, 0 );
