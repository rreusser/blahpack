...
test( 'dlanhs has expected arity', function t() {
	assert.strictEqual( dlanhs.length, 6, 'has expected arity' );
});

test( 'dlanhs throws TypeError for invalid order', function t() {
	assert.throws( function throws() {
		dlanhs( 'invalid', 'max', new Float64Array( 4 ), new Float64Array( 4 ), 2, new Float64Array( 4 ) );
	}, TypeError );
});

test( 'dlanhs throws TypeError for invalid norm', function t() {
	assert.throws( function throws() {
		dlanhs( 'row-major', 'invalid', new Float64Array( 4 ), new Float64Array( 4 ), 2, new Float64Array( 4 ) );
	}, TypeError );
});

test( 'dlanhs throws RangeError for negative N', function t() {
	assert.throws( function throws() {
		dlanhs( 'row-major', 'max', -1, new Float64Array( 4 ), 2, new Float64Array( 4 ) );
	}, RangeError );
});
