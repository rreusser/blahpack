...
var TRANSRS = [ 'no-transpose', 'transpose' ]; // RFP normal / transposed layout
...
function invertRFP( A0, n, transr, uplo, unit, rfpLayout ) {
	...
	var info = dtftri( transr, uplo, diagStr( unit ), n, rfp.data, rfp.stride, rfp.offset );
	if ( info !== 0 ) {
		...
		UPLOS.forEach( function eachUplo( uplo ) {
			DIAGS.forEach( function eachDiag( diag ) {
				...
				SIZES.forEach( function eachN( n ) {
					...
					var label = 'dtftri '+transr+' '+uplo+' '+diag+' n='+n;
