/**
* Property-based validation for dtfsm, following the /blahpack-validate process.
*
* Step 0 classification: `d` -> real scalar; `tf` -> triangular A in Rectangular
* Full Packed (RFP) storage; `sm` (triangular solve, multiple RHS) -> RESIDUAL:
* dtfsm solves op(A)*X = alpha*B (side left) or X*op(A) = alpha*B (side right).
* After solving, plug X back in and check op(A0)*X (left) / X*op(A0) (right)
* reproduces alpha*B0 against the FULL logical triangular oracle A0.
*
* RFP is a storage format only, so we bridge the triangular A through the
* already-validated converter dtrttf (TR->RFP); B is an ordinary dense matrix.
...
function toRFP( A0, transr, uplo, unit, na, rfpLayout ) {
	...
	dtrttf( transr, uplo, na, A.data, A.args[ 0 ], A.args[ 1 ], A.args[ 2 ], rfp.data, rfp.stride, rfp.offset );
	return rfp;
}
...
test( 'dtfsm: solve residual (transr x side x uplo x trans x diag x sizes)', function t() {
...
function runResidual( transr, side, uplo, trans, diag, M, N ) {
	...
	dtfsm( transr, side, uplo, trans, diag, M, N, sc.apiScalar( alpha ), rfp.data, rfp.stride, rfp.offset, Br.data, Br.args[ 0 ], Br.args[ 1 ], Br.args[ 2 ] );
	...
	const label = 'dtfsm '+transr+' '+side+' '+uplo+' '+trans+' '+diag+' M='+M+' N='+N;
	checked( 'dtfsm', 'residual', function run() {
...
// dense matrix; a correct dtfsm is BIT-EXACT across every RFP x B layout.
test( 'dtfsm: bit-exact across RFP x B layouts', function t() {
	const combos = [
		[ 'no-transpose', 'left', 'upper', 'no-transpose', 'non-unit' ],
		[ 'transpose', 'left', 'lower', 'transpose', 'unit' ],
		[ 'no-transpose', 'right', 'upper', 'transpose', 'non-unit' ],
		[ 'transpose', 'right', 'lower', 'no-transpose', 'unit' ]
	];
	const M = 6;
	const N = 5;
	...
	combos.forEach( function eachCombo( c ) {
		...
		checked( 'dtfsm', 'layout-invariance', function run() {
			...
			}, { 'label': 'dtfsm '+transr+' '+side+' '+uplo+' '+trans+' '+diag+' RFP x B layout invariance' } );
		});
	});
});
