...
* @returns {void}
*/
function dlar1v( N, b1, bn, lambda, D, strideD, L, strideL, LD, strideLD, LLD, strideLLD, pivmin, gaptol, Z, strideZ, wantnc, negcnt, ztz, mingma, r, ISUPPZ, strideISUPPZ, nrminv, resid, rqcorr, WORK ) {
	if ( N < 0 ) {
	...
	const oLLD = stride2offset( N, strideLLD );
	const oZ = stride2offset( N, strideZ );
	if ( WORK === null || WORK === void 0 ) {
		const minWork = Math.max( 1, 4 * N );
		WORK = new Float64Array( minWork );
	}
	const oISUPPZ = stride2offset( 2, strideISUPPZ );
	base( N, b1, bn, lambda, D, strideD, oD, L, strideL, oL, LD, strideLD, oLD, LLD, strideLLD, oLLD, pivmin, gaptol, Z, strideZ, oZ, wantnc, negcnt, ztz, mingma, r, ISUPPZ, strideISUPPZ, oISUPPZ, nrminv, resid, rqcorr, WORK, 1, 0 );
}
